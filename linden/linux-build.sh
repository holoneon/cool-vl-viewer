...
# build-linux.sh v2.41 (c)2013-2026 Henri Beauchamp.
...
function find_gcc()
{
	for i in $1".5" $1".4" $1".3" $1".2" $1".1" $1".0" $1 ; do
		for j in "$i.9" "$i.8" "$i.7" "$i.6" "$i.5" "$i.4" "$i.3" "$i.2" "$i.1" "$i.0" "$i" ; do
			if has_exec "gcc-$j" && has_exec "g++-$j" ; then
				echo "Will use gcc/g++ v$j to build the viewer"
				GCC="gcc-$j"
				GXX="g++-$j"
				return
			fi
...
{
	for i in "python3.13" "python3.12" "python3.11" "python3.10" "python3.9" "python3.8" "python3.7" "python3.6" "python3.5" "python3.4" "python3.3" "python3" "python"; do
...
if has_option "h" ; then
	echo "Usage: $0 [-option|--long-build-option...]|[-z|--zap]|[-h|--help][ -- <cmake options>]"
	echo "With  -v8           : use gcc v8 if possible/found."
	echo "      -v9           : use gcc v9 if possible/found."
	echo "      -v10          : use gcc v10 if possible/found."
	echo "      -v11          : use gcc v11 if possible/found."
	echo "      -v12          : use gcc v12 if possible/found."
	echo "      -v13          : use gcc v13 if possible/found."
	echo "      -v14          : use gcc v14 if possible/found."
	echo "      -v15          : use gcc v15 if possible/found."
	...
	echo "      -a, --asan    : enable ASAN sanitizing (for devel builds only)."
	...
	echo "      -m, --mirror  : use the mirror web site to download pre-built packages."
...
if has_option "z" ; then
	...
	# Remove all directories and files at the root directory which are not
	# pertaining to the original source tree. I.e. remove the pre-built
	# libraries directories and related files, the build directory and build
	# related files, etc...
	for i in * ; do
		if [ -d "$i" ] ; then
			# Only these three sub-directories must exist in the source root
			# directory.
			if [ "$i" != "doc" ] && [ "$i" != "indra" ] && [ "$i" != "scripts" ] ; then
				rm -rf "$i"
			fi
		elif [ "$i" != "install.xml" ] && [[ $i != *-build.sh ]] && [[ $i != windows-*.bat ]] && [ "i" != "applied_patches.txt" ] ; then
			# Only install.xml and the build scripts must exist in the source
			# root directory.
			rm -f "$i"
		fi
	done
	# Static data, textures, icons and fonts part of the "artwork" package
	...
	echo -n >indra/newview/cool_vl_viewer.icns
	rm -f indra/newview/res/*.png
	rm -rf indra/newview/fonts/
	# Skin textures part of the "artwork" package
	...
	# More potential build-time related files
...
find_python
if [ "$PYTHON" != "" ] ; then
	AUXILIARY_PATH="$RUN_PATH"
	ln -sf "$PYTHON" "$AUXILIARY_PATH/python"
	echo "Forcing the use of python interpreter: $PYTHON"
else
	echo "ERROR: could not find a compatible Python interpreter. Please install Python version 3.13 to 3.3."
	exit 1
fi
if [ -x /usr/bin/lbzip2 ] ; then
	AUXILIARY_PATH="$RUN_PATH"
	ln -sf /usr/bin/lbzip2 "$AUXILIARY_PATH/bzip2"
	echo "Forcing the use by 'tar' of the multi-threaded 'lbzip2' instead of the mono-threaded 'bzip2'."
fi
if [ "$AUXILIARY_PATH" != "" ] ; then
	export PATH="$AUXILIARY_PATH:$PATH"
fi

if has_option "v[189]" ; then
	if has_option "v8" ; then
		find_gcc "8" 
	elif has_option "v9" ; then
		find_gcc "9"
	elif has_option "v10" ; then
		find_gcc "10" 
	elif has_option "v11" ; then
		find_gcc "11" 
	elif has_option "v12" ; then
		find_gcc "12" 
	elif has_option "v13" ; then
		find_gcc "13" 
	elif has_option "v14" ; then
		find_gcc "14" 
	elif has_option "v15" ; then
		find_gcc "15" 
	fi
...
if has_option "a" ; then
	PROFILE="$PROFILE -DASAN:BOOL=TRUE"
	echo "Compiling with ASAN sanitizing (WARNING: slow viewer, for devel builds only)."
fi
...
if has_option "m" ; then
	USEMIRROR="-DUSE_MIRROR:BOOL=TRUE"
	echo "Using the mirror website to dowload pre-built packages."
fi

USEROPTS=""
if echo "$COMMAND_LINE" | grep ' -- ' &>/dev/null ; then
	USEROPTS=${COMMAND_LINE#*' -- '}
fi
...
start=`date +%s%2N`
if [ "$TUNE_FLAGS" != "" ] ; then
	./scripts/develop.py $USESYSTEMLIBS $GENERATOR -t $BUILD_TYPE configure $GCC $GXX $NO_FATAL_WARNINGS $STACK_OPTIONS $OPENMP $LTO $UNITYBUILD $PROFILE $USEMIRROR $USEROPTS \
