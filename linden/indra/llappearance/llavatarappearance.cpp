...
class LLAvatarSkeletonInfo final
{
	friend class LLAvatarAppearance;

public:
	LL_INLINE LLAvatarSkeletonInfo()
	:	mNumBones(0),
...
private:
	std::vector<LLAvatarBoneInfo*>			mBoneInfoList;
	S32										mNumBones;
...
void LLAvatarAppearance::makeJointAliases(LLAvatarBoneInfo* bone_info)
{
	...
	LLAvatarBoneInfo::vec_t& children = bone_info->mChildren;
	for (size_t i = 0, count = children.size(); i < count; ++i)
	{
		makeJointAliases(children[i]);
	}
}

//static
const std::vector<LLAvatarBoneInfo*>& LLAvatarAppearance::getBoneInfoList()
{
	llassert_always(sAvatarSkeletonInfo);
	return sAvatarSkeletonInfo->mBoneInfoList;
}

const strings_map_t& LLAvatarAppearance::getJointAliases()
{
	if (mJointAliasMap.empty())
	{
		LLAvatarBoneInfo::vec_t& bones = sAvatarSkeletonInfo->mBoneInfoList;
		for (size_t i = 0, count = bones.size(); i < count; ++i)
		{
			makeJointAliases(bones[i]);
		}
	}
								   ...
								   const std::string& skel_file)
{
	std::string avatar_file = lad_file.empty() ? AVATAR_DEFAULT_CHAR
											   : lad_file;
	avatar_file = gDirUtil.getFullPath(LL_PATH_CHARACTER, avatar_file);
	...
	LLXmlTreeNode* rootp = lad_xml_tree.getRoot();
	if (!rootp)
	{
	...
	if (!rootp->hasName("linden_avatar"))
	{
	...
	if (!rootp->getFastAttributeString(version_string, version) ||
	...
	rootp->getFastAttributeS32(wearable_version_string,
							   wearable_def_version);
	LLWearable::setCurrentDefinitionVersion(wearable_def_version);

	std::string mesh_file_name;

	LLXmlTreeNode* skeleton_node = rootp->getChildByName("skeleton");
	if (!skeleton_node)
	{
	...
	std::string skeleton_path = gDirUtil.getFullPath(LL_PATH_CHARACTER,
													 skeleton_file_name);
	...
	if (!sAvatarXmlInfo->parseXmlSkeletonNode(rootp))
	{
		llerrs << "Error parsing skeleton node in avatar XML file: "
			   << skeleton_path << llendl;
	}
	if (!sAvatarXmlInfo->parseXmlMeshNodes(rootp))
	{
		llerrs << "Error parsing skeleton node in avatar XML file: "
			   << skeleton_path << llendl;
	}
	if (!sAvatarXmlInfo->parseXmlColorNodes(rootp))
	{
		llerrs << "Error parsing skeleton node in avatar XML file: "
			   << skeleton_path << llendl;
	}
	if (!sAvatarXmlInfo->parseXmlLayerNodes(rootp))
	{
		llerrs << "Error parsing skeleton node in avatar XML file: "
			   << skeleton_path << llendl;
	}
	if (!sAvatarXmlInfo->parseXmlDriverNodes(rootp))
	{
		llerrs << "Error parsing skeleton node in avatar XML file: "
			   << skeleton_path << llendl;
	}
	if (!sAvatarXmlInfo->parseXmlMorphNodes(rootp))
	{
		llerrs << "Error parsing skeleton node in avatar XML file: "
			   << skeleton_path << llendl;
	}
										   ...
										   LLXmlTree& skel_xml_tree)
{
	...
	LLXmlTreeNode* rootp = skel_xml_tree.getRoot();
	if (!rootp)
	{
	...
	if (!rootp->hasName("linden_skeleton"))
	{
...
bool LLAvatarAppearance::setupBone(const LLAvatarBoneInfo* infop,
								   LLJoint* parentp,
								   S32& volume_num,
								   S32& joint_num)
{
	...
	LL_DEBUGS("Avatar") << "Bone info. Name: " << infop->mName
						<< " - Joint: " << (infop->mIsJoint ? "yes" : "no")
						<< " - Volume number: " << volume_num
						<< " - Joint number: " << joint_num << LL_ENDL;

	if (infop->mIsJoint)
	{
		jointp = getCharacterJoint(joint_num);
		if (!jointp)
		{
			llwarns << "Too many bones" << llendl;
			return false;
		}
		jointp->setName(infop->mName);
	}
	else	// Collision volume
	{
		...
		jointp = mCollisionVolumes[volume_num];
		jointp->setName(infop->mName);
	}

	// Add to parent
	if (parentp && jointp->getParent() != parentp)
	{
		parentp->addChild(jointp);
	}

	jointp->setPosition(infop->mPos);
	jointp->setDefaultPosition(infop->mPos);
	jointp->setRotation(mayaQ(infop->mRot.mV[VX], infop->mRot.mV[VY],
							  infop->mRot.mV[VZ], LLQuaternion::XYZ));
	jointp->setScale(infop->mScale);
	jointp->setDefaultScale(infop->mScale);
	jointp->setSupport(infop->mSupport);
	jointp->setEnd(infop->mEnd);

	if (infop->mIsJoint)
	{
		jointp->setSkinOffset(infop->mPivot);
		jointp->setJointNum(joint_num++);
		jointp->setIsBone(true);
	}
	else // collision volume
	{
		jointp->setJointNum(mNumBones + volume_num++);
	}

	// Setup children
	const LLAvatarBoneInfo::vec_t& children = infop->mChildren;
	for (size_t i = 0, count = children.size(); i < count; ++i)
	{
		LLAvatarBoneInfo* child_infop = children[i];
		if (!setupBone(child_infop, jointp, volume_num, joint_num))
		{
			return false;
		}
	}
...
bool LLAvatarAppearance::buildSkeleton(const LLAvatarSkeletonInfo* infop)
{
	if (!infop)
	{
	...
	LL_DEBUGS("Avatar") << "Sketeton info. Bones: " << infop->mNumBones
						<< " - Collision volumes: "
						<< infop->mNumCollisionVolumes << LL_ENDL;

	// Allocate joints
	if (!allocateCharacterJoints(infop->mNumBones))
	{
		llerrs << "Cannot allocate " << infop->mNumBones << " joints" << llendl;
	}

	// Allocate volumes
	if (infop->mNumCollisionVolumes)
	{
		if (!allocateCollisionVolumes(infop->mNumCollisionVolumes))
		{
			llerrs << "Cannot allocate " << infop->mNumCollisionVolumes
				   << " collision volumes" << llendl;
		}
	}

	S32 cur_joint_num = 0;
	S32 cur_volume_num = 0;
	const LLAvatarBoneInfo::vec_t& bones = infop->mBoneInfoList;
	for (size_t i = 0, count = bones.size(); i < count; ++i)
	{
		LLAvatarBoneInfo* bone_infop = bones[i];
		if (!setupBone(bone_infop, NULL, cur_volume_num, cur_joint_num))
		{
...
bool LLAvatarAppearance::loadAvatar()
{
	...
	if (LLJoint::sAvatarJointAliasMap.empty())
	{
		...
		for (strings_map_t::const_iterator
		 ...
		 iter != end; ++iter)
	{
		...
		if (driver_param->setInfo(info))
		{
			...
			if (!driver_param->linkDrivenParams(std::bind(avatar_function,
