...
bool LLTexLayerSetInfo::sUse2KBakes = false;
...
bool LLTexLayerSetInfo::parseXml(LLXmlTreeNode* node)
{
	...
	// Force the size down to 1024x1024 when not using 2K local bakes. HB
	if (mWidth == 2048 && mHeight == 2048 && !sUse2KBakes)
	{
		llinfos_once << "Local bakes set to 1024x1024 textures." << llendl;
		mWidth = mHeight = 1024;
	}
...
bool LLTexLayerSet::render(S32 x, S32 y, S32 width, S32 height)
{
	...
	// Clear buffer area to ensure we do not pick up UI elements
	{
...
LLTexLayer::~LLTexLayer()
{
	...
	for (alpha_cache_t::iterator iter = mAlphaCache.begin(),
		 ...
		 iter != end; ++iter)
	{
		ll_aligned_free((void*)iter->second);
	}
}
...
const U8* LLTexLayer::getAlphaData() const
{
	...
	return iter2 != mAlphaCache.end() ? iter2->second : NULL;
}
								  ...
								  bool force_render)
{
	...
	if (hasMorph() && success)
	{
		...
		U32 cache_index = alpha_mask_crc.getCRC();
		U8* alpha_datap = get_ptr_in_map(mAlphaCache, cache_index);
		if (!alpha_datap)
		{
			...
			while ((S32)mAlphaCache.size() >= max_cache_entries)
			{
				...
				alpha_datap = iter2->second;
				ll_aligned_free((void*)alpha_datap);
				mAlphaCache.hmap_erase(iter2);
			}
			// GPUs tend to be very uptight about memory alignment as the DMA
			// used to convey said data to the card works better when well
			// aligned so plain old default-aligned heap memory is a no-no.
			alpha_datap = (U8*)ll_aligned_malloc(width * height, 32);
			if (!alpha_datap)
			{
				LLMemory::allocationFailed(width * height);
				llwarns_once << "Failed to allocate morph mask: out of memory !"
							 << llendl;
				return;
			}
#if 0		// Old code. New code (introduced in PBR viewer) is below. HB
			if (LLGLManager::sIsIntel)	// Work-around for broken Intel drivers
			{
				// Let's use a static buffer for speed. HB
				static std::vector<U8> buffer;
				// Align start pointer on 32 bytes.
				buffer.reserve(width * height * 4 + 32);
				U8* buffp = (U8*)buffer.data();
				buffp += 32 - ((uintptr_t)buffp & 31);
				glReadPixels(x, y, width, height, GL_RGBA, GL_UNSIGNED_BYTE,
							 (void*)buffp);
				for (U32 i = 0, count = width * height; i < count; ++i)
				{
					alpha_datap[i] = buffp[i * 4 + 3];
				}
			}
			...
			mAlphaCache[cache_index] = alpha_datap;
		}

		getTexLayerSet()->getAvatarAppearance()->dirtyMesh();

		mMorphMasksValid = true;
		getTexLayerSet()->applyMorphMask(alpha_datap, width, height, 1);
	}
}
...
void LLTexLayerStaticImageList::deleteCachedImages()
{
	...
	{
		llinfos << "Clearing static textures. GL:" << mGLBytes / 1024
				<< "KB - TGA:" << mTGABytes / 1024 << "KB" << llendl;
...
LLImageTGA* LLTexLayerStaticImageList::getImageTGA(const std::string& file_name)
{
	...
	std::string path = gDirUtil.getFullPath(LL_PATH_CHARACTER, file_name);
