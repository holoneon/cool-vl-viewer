...
void LLWearable::createVisualParams(LLAvatarAppearance* avatarp)
{
		 ...
		 param_iter != end; ++param_iter)
	{
		...
		LLVisualParam*(LLWearable::*wearable_fn)(S32)const =
			&LLWearable::getVisualParam;
		// need this line to disambiguate between versions of
		// LLCharacter::getVisualParam()
		LLVisualParam*(LLAvatarAppearance::*param_fn)(S32)const =
			&LLAvatarAppearance::getVisualParam;
		param->resetDrivenParams();
		if (!param->linkDrivenParams(std::bind(wearable_fn, (LLWearable*)this,
											   _1),
									 false))
		{
			if (!param->linkDrivenParams(std::bind(param_fn, avatarp, _1),
										 true))
			{
...
void LLWearable::writeToAvatar(LLAvatarAppearance* avatarp)
{
	...
	// Pull params
	for (const LLVisualParam* paramp = avatarp->getFirstVisualParam(); paramp;
		 paramp = avatarp->getNextVisualParam())
	{
		if (paramp &&
			((LLViewerVisualParam*)paramp)->getWearableType() == mType &&
			 ...
			 // way.
			 !((LLViewerVisualParam*)paramp)->getCrossWearable())
		{
