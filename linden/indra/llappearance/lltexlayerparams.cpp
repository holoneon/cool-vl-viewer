...
LLTexLayerParam::LLTexLayerParam(LLTexLayerInterface* layerp)
:	LLViewerVisualParam(),
	mTexLayer(layerp),
...
void LLTexLayerParamAlpha::getCacheByteCount(S32* gl_bytes)
{
	*gl_bytes = 0;
	...
	for (param_alpha_ptr_list_t::iterator iter = sInstances.begin(),
		 ...
		 iter != end; ++iter)
	{
		LLTexLayerParamAlpha* instancep = *iter;
		LLGLTexture* texp = instancep->mCachedProcessedTexture.get();
		if (texp)
		{
			S32 bytes = texp->getWidth() * texp->getHeight() *
						texp->getComponents();

			if (texp->hasGLTexture())
			{
				*gl_bytes += bytes;
			}
		}
...
bool LLTexLayerParamAlpha::render(S32 x, S32 y, S32 width, S32 height)
{
	...
	if (!info->mStaticImageFileName.empty() && !mStaticImageInvalid)
	{
		...
		{
			// Do not load the image file until we actually need it the first
			// time. Like now.
			mStaticImageTGA =
				gTexLayerStaticImageList.getImageTGA(info->mStaticImageFileName);
			...
			if (!mCachedProcessedTexture)
			{
				...
				if (gTextureManagerBridgep)
				{
					mCachedProcessedTexture =
						gTextureManagerBridgep->getLocalTexture(image_tga_width,
																image_tga_height,
																1, false);
				}
