...
// On Linux, we use the native shm_open/mmap interface by using
//	#define USE_SHM_OPEN_SHARED_MEMORY 1
// in the appropriate sections below.

// For Windows, use:
//	#define USE_WINDOWS_SHARED_MEMORY 1
...
#	define WINDOWS_SHARED_MEMORY_PREFIX_STRING "Local\\LL_"
#else
	// Linux
...
#	include <sys/mman.h>
#	include <errno.h>
#elif USE_WINDOWS_SHARED_MEMORY
#	include <windows.h>
#endif
...
#elif USE_WINDOWS_SHARED_MEMORY
// MARK: Windows CreateFileMapping-based implementation
...
bool LLPluginSharedMemory::create(size_t size)
{
	mName = WINDOWS_SHARED_MEMORY_PREFIX_STRING;
	mName += createName();
	mSize = size;
