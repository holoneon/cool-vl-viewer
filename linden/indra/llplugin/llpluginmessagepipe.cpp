...
bool LLPluginMessagePipeOwner::writeMessageRaw(const std::string& message)
{
	...
	// It is "normal" to see a "shutdown_plugin" message dropped after a plugin
	// has been killed by other means... So do not log a warning for this. HB
	if (message.find("<string>shutdown_plugin</string>") == std::string::npos)
	{
...
bool LLPluginMessagePipe::pumpInput(F64 timeout)
{
	...
	if (mSocket)
	{
		...
		setSocketTimeout((apr_interval_time_t)(timeout * 1000000.0));
