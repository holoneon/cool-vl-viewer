							  ...
							  bool debug)
{
	...
	llinfos << "Initializing: " << plugin_filename << llendl;
...
void LLPluginClassMedia::reset()
{
	...
	mSleepTime = 0.01f;
	mCanCut = mCanCopy = mCanPaste = mCanUndo = mCanRedo = mCanDelete =
			  mCanSelectAll = mCanShowSource = false;
...
void LLPluginClassMedia::showSource()
{
										 ...
										 const std::string& cache_path)
{
	...
	message.setValue("cache", cache_path);
	sendMessage(message);
}
...
void LLPluginClassMedia::receivePluginMessage(const LLPluginMessage& message)
{
	...
	if (message_class == LLPLUGIN_MESSAGE_CLASS_MEDIA)
	{
...
#if 0			// Too spammy...
				LL_DEBUGS("PluginMessage") << "adjusted incoming rect is: ("
										   << new_rect.mLeft << ", "
										   << new_rect.mTop << ", "
										   << new_rect.mRight << ", "
										   << new_rect.mBottom
		...
		else if (message_name == "edit_state")
		{
			...
			{
				mCanDelete = message.getValueBoolean("delete");
			}
			if (message.hasValue("select_all"))
			{
				mCanSelectAll = message.getValueBoolean("select_all");
			}
			if (message.hasValue("show_source"))
			{
				mCanShowSource = message.getValueBoolean("show_source");
			}
		}
		...
		else if (message_name == "debug_message")
		{
			...
			{
				llinfos << mDebugMessageText << llendl;
			}
			else if (mDebugMessageLevel == "warn" ||
					 mDebugMessageLevel == "warning")
			{
				llwarns << mDebugMessageText << llendl;
			}
		}
		else
		{
	...
	{
		std::string message_name = message.getName();
		if (message_name == "audio_level")
		{
			mAudioLevel = message.getValueReal("level");
		}
		else if (message_name == "audio_devices")
		{
			mWebRTCDataMutex.lock();
			mCaptureDevices = message.getValueLLSD("capture");
			mRenderDevices = message.getValueLLSD("render");
			mWebRTCDataMutex.unlock();
		}
		else if (message_name == "webrtc_data")
		{
			LLUUID connection_id = message.getValueLLUUID("connection_id");
			if (connection_id.notNull())
			{
				mWebRTCDataMutex.lock();
				LLSD& data = getVoiceData(connection_id);
				if (data.isUndefined())
				{
					data = LLSD::emptyArray();
				}
				// Append the full LLSD "params" block to the array. HB
				data.append(message.getParams());
				mWebRTCDataMutex.unlock();
			}
		}
		else if (message_name == "log_message")
		{
			llinfos << message.getValue("message_text") << llendl;
		}
	}
}

LLSD& LLPluginClassMedia::getVoiceData(const LLUUID& connection_id)
{
	std::map<LLUUID, LLSD>::iterator it = mWebRTCDataMap.find(connection_id);
	if (it == mWebRTCDataMap.end())
	{
		auto result = mWebRTCDataMap.emplace(connection_id, LLSD());
		it = result.first;
	}
	return it->second;
}
...
void LLPluginClassMedia::refreshDevices()
{
	mCaptureDevices.clear();
	mRenderDevices.clear();
	LLPluginMessage message(LLPLUGIN_MESSAGE_CLASS_VOICE, "get_devices");
	sendMessage(message);
}

void LLPluginClassMedia::setCaptureDevice(const std::string& device_id)
{
	LLPluginMessage message(LLPLUGIN_MESSAGE_CLASS_VOICE, "set_capture");
	message.setValue("device_id", device_id);
	sendMessage(message);
}

void LLPluginClassMedia::setRenderDevice(const std::string& device_id)
{
	LLPluginMessage message(LLPLUGIN_MESSAGE_CLASS_VOICE, "set_render");
	message.setValue("device_id", device_id);
	sendMessage(message);
}

void LLPluginClassMedia::setMicGain(F32 gain)
{
	LLPluginMessage message(LLPLUGIN_MESSAGE_CLASS_VOICE, "set_mic_gain");
	message.setValueReal("gain", gain);
	sendMessage(message);
}

void LLPluginClassMedia::setConfiguration(const LLSD& config)
{
	LLPluginMessage message(LLPLUGIN_MESSAGE_CLASS_VOICE, "configure");
	message.setValueLLSD("config", config);
	sendMessage(message);
}

void LLPluginClassMedia::setTuningMode(bool tuning_on)
{
	LLPluginMessage message(LLPLUGIN_MESSAGE_CLASS_VOICE, "tuning_mode");
	message.setValueBoolean("enabled", tuning_on);
	sendMessage(message);
}

void LLPluginClassMedia::setTuningMicGain(F32 gain)
{
	LLPluginMessage message(LLPLUGIN_MESSAGE_CLASS_VOICE, "tuning_gain");
	message.setValueReal("gain", gain);
	sendMessage(message);
}

void LLPluginClassMedia::setGlobalMute(bool mute, F32 delay)
{
	LLPluginMessage message(LLPLUGIN_MESSAGE_CLASS_VOICE, "global_mute");
	message.setValueBoolean("enabled", mute);
	message.setValueReal("delay", delay);
	sendMessage(message);
}

void LLPluginClassMedia::sendVoiceCommand(const LLUUID& connection_id,
										  const std::string& command)
{
	LLPluginMessage message(LLPLUGIN_MESSAGE_CLASS_VOICE, command);
	message.setValueLLUUID("connection_id", connection_id);
	sendMessage(message);
}

void LLPluginClassMedia::sendVoiceCmdWithBool(const LLUUID& connection_id,
											  const std::string& command,
											  ...
											  bool param)
{
	LLPluginMessage message(LLPLUGIN_MESSAGE_CLASS_VOICE, command);
	message.setValueLLUUID("connection_id", connection_id);
	message.setValueBoolean(param_name, param);
	sendMessage(message);
}

void LLPluginClassMedia::sendVoiceCmdWithReal(const LLUUID& connection_id,
											  const std::string& command,
											  const std::string& param_name,
											  F64 param)
{
	LLPluginMessage message(LLPLUGIN_MESSAGE_CLASS_VOICE, command);
	message.setValueLLUUID("connection_id", connection_id);
	message.setValueReal(param_name, param);
	sendMessage(message);
}

void LLPluginClassMedia::sendVoiceCmdWithString(const LLUUID& connection_id,
												const std::string& command,
												const std::string& param_name,
												const std::string& param)
{
	LLPluginMessage message(LLPLUGIN_MESSAGE_CLASS_VOICE, command);
	message.setValueLLUUID("connection_id", connection_id);
	message.setValue(param_name, param);
	sendMessage(message);
}

void LLPluginClassMedia::sendVoiceCmdWithLLSD(const LLUUID& connection_id,
											  const std::string& command,
											  const std::string& param_name,
											  const LLSD& param)
{
	LLPluginMessage message(LLPLUGIN_MESSAGE_CLASS_VOICE, command);
	message.setValueLLUUID("connection_id", connection_id);
	message.setValueLLSD(param_name, param);
	sendMessage(message);
}

void LLPluginClassMedia::setVoiceDebug(bool enable)
{
	if (mDebugVoice != enable && mPlugin && mPlugin->isRunning())
	{
		mDebugVoice = enable;
		LLPluginMessage message(LLPLUGIN_MESSAGE_CLASS_VOICE, "debug");
		message.setValueBoolean("enabled", enable);
		sendMessage(message);
	}
}
