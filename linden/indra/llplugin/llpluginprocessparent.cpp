...
void LLPluginProcessParent::requestShutdown()
{
	...
	mPolling = pump.listen(name,
						   std::bind(&LLPluginProcessParent::pollTick, this));
}
								 ...
								 bool debug)
{
...
#if LL_LINUX
	// If the plugin we launch is not Dullahan/CEF, then do not preload the
	// libcef library (neither jemalloc: the latter will be loaded as needed
	// and is only preloaded via LD_PRELOAD in order to get the preloaded CEF
	// library to properly use it). Instead, use the LD_PRELOAD value found
	// on viewer launch. Note that SAVED_LD_PRELOAD and LD_PRELOAD are set by
	// the Linux wrapper script. HB
	if (plugin_filename.find("media_plugin_cef") == std::string::npos)
	{
		char* envvar = getenv("SAVED_LD_PRELOAD");
		if (envvar && *envvar)
		{
	...
	// If we are launching Dullahan/CEF, we however need to use a special
	// fontconfig configuration override which excludes bitmap fonts from the
	// list of usable fonts, because starting with CEF 142, those fonts do not
	// render at all any more in CEF. CEF_FONTCONFIG_FILE is set by the viewer
	// wrapper script, and we recopy it here in FONFTCONFIG_FILE for the plugin
	// process environment. HB
	else
	{
		char* envvar = getenv("CEF_FONTCONFIG_FILE");
		if (envvar && *envvar)
		{
			mProcess.setEnv("FONTCONFIG_FILE", envvar);
		}
	}
#endif
...
void LLPluginProcessParent::servicePoll()
{
	...
	// messages.
	if (mMessagePipe && !mMessagePipe->pumpInput())
	{
		llinfos << mPluginName
