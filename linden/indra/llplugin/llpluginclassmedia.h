...
public:
	...
	bool init(const std::string& launcher_filename,
			  ...
			  const std::string& plugin_filename, bool debug = false);
	...
	// get() methods above without checking for invalid return values until
	// you call idle() again.
	bool textureValid();
	...
	LL_INLINE void setLockupTimeout(F32 t)
	{
		if (mPlugin)
		{
			mPlugin->setLockupTimeout(t);
		}
	}
	...
	void undo();
	LL_INLINE bool canUndo() const						{ return mCanUndo; }

	void redo();
	LL_INLINE bool canRedo() const						{ return mCanRedo; }

	void doDelete();
	LL_INLINE bool canDelete() const					{ return mCanDelete; }

	void selectAll();
	LL_INLINE bool canSelectAll() const					{ return mCanSelectAll; }

	void showSource();
	LL_INLINE bool canShowSource() const				{ return mCanShowSource; }
	...
	LL_INLINE bool newMetaData() const					{ return mNewMetaData; }
	LL_INLINE void gotMetaData()						{ mNewMetaData = false; }

	///////////////////////////////////
	// Media time class methods
	...
	///////////////////////////////////
	// Voice plugin related methods (non const LLSD&, so that the owner can
	// wipe them out once recovered). HB
	LL_INLINE LLSD& getCaptureDevices()					{ return mCaptureDevices; }
	LL_INLINE LLSD& getRenderDevices()					{ return mRenderDevices; }
	void refreshDevices();
	void setCaptureDevice(const std::string& device_id);
	void setRenderDevice(const std::string& device_id);
	void setMicGain(F32 gain);
	void setConfiguration(const LLSD& config);
	LL_INLINE F32 getAudioLevel() const					{ return mAudioLevel; }
	void setTuningMode(bool tuning_on);
	// The following two methods are only functional for WebRTC m137 and newer:
	void setTuningMicGain(F32 gain);
	void setGlobalMute(bool mute, F32 delay);
	// Use these to lock the data before using or touching it. HB
	LL_INLINE void lockWebRTCData()						{ mWebRTCDataMutex.lock(); }
	LL_INLINE void unlockWebRTCData()					{ mWebRTCDataMutex.unlock(); }
	// To access all the data changes specific to a connection.
	LLSD& getVoiceData(const LLUUID& connection_id);
	// These methods send per-connection voice commands to the plugin. HB
	void sendVoiceCommand(const LLUUID& connection_id,
						  const std::string& command);
	void sendVoiceCmdWithBool(const LLUUID& connection_id,
							  const std::string& command,
							  const std::string& param_name, bool param);
	void sendVoiceCmdWithReal(const LLUUID& connection_id,
							  const std::string& command,
							  const std::string& param_name, F64 param);
	void sendVoiceCmdWithString(const LLUUID& connection_id,
								const std::string& command,
								const std::string& param_name,
								const std::string& param);
	void sendVoiceCmdWithLLSD(const LLUUID& connection_id,
							  const std::string& command,
							  const std::string& param_name,
							  const LLSD& param);
	LL_INLINE bool isVoiceDebugged() const				{ return mDebugVoice; }
	void setVoiceDebug(bool enable);
...
protected:
	...
	// Voice plugin related data. HB
	std::mutex								mWebRTCDataMutex;
	LLSD									mCaptureDevices;
	LLSD									mRenderDevices;
	std::map<LLUUID, LLSD>					mWebRTCDataMap;
	F32										mAudioLevel;
	bool									mDebugVoice;
