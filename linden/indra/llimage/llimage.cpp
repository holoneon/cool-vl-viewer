...
//static
bool LLImage::getDimensions(const std::string& filename, S32& width,
							S32& height, S8 codec)
{
	if (!LLFile::exists(filename))
	{
		setLastError("Unable to find file " + filename);
		return false;
	}

	LLFile file(filename, "rb");
	if (!file)
	{
	...
	if (codec == IMG_CODEC_INVALID)
	{
		...
		if (codec == IMG_CODEC_INVALID)
		{
			setLastError("Unable to determine image CODEC from file name: " +
						 filename);
		}
	}
	...
	switch (codec)
	{
		case IMG_CODEC_J2C:
			// *TODO: implement ?  HB
			llwarns << "getDimensions() not implemented for JPEG2000"
					<< llendl;
			llassert(false);
			break;

		case IMG_CODEC_BMP:
			return LLImageBMP::getDimensions(file, width, height);

		case IMG_CODEC_TGA:
			return LLImageTGA::getDimensions(file, width, height);

		case IMG_CODEC_JPEG:
			return LLImageJPEG::getDimensions(file, width, height);

		case IMG_CODEC_PNG:
			return LLImagePNG::getDimensions(file, width, height);
...
bool LLImageRaw::checkHasTransparentPixels()
{
	...
	U8* data = getData();
	for (U32 i = 0, pixels = getWidth() * getHeight(); i < pixels; ++i)
	{
		if (data[i * 4 + 3] != 255)
		{
			return true;
...
bool LLImageRaw::makeAlpha()
{
	...
	U32 height = getHeight();
	U32 pixels = width * height;
	// Alpha channel does not exist, make a new copy of data with an alpha
	// channel.
	U8* new_data = (U8*)allocate_texture_mem(width * height * 4);
	for (U32 i = 0; i < pixels; ++i)
	{
		U32 di = i * 4;
		U32 si = i * 3;
...
void LLImageRaw::fill(const LLColor4U& color)
{
	...
	if (!getData())
	{
		llwarns << "Invalid image buffer." << llendl;
		return;
	}
...
void LLImageRaw::tint(const LLColor3& color)
{
	...
	for (U32 i = 0, pixels = getWidth() * getHeight(); i < pixels; ++i)
	{
		F32 c0 = F32(data[0]) * color.mV[0];
		F32 c1 = F32(data[1]) * color.mV[1];
		F32 c2 = F32(data[2]) * color.mV[2];
...
file_extensions[] =
{
	{ std::string("bmp"),		IMG_CODEC_BMP },
	{ std::string("tga"),		IMG_CODEC_TGA },
	{ std::string("j2c"),		IMG_CODEC_J2C },
	{ std::string("texture"),	IMG_CODEC_J2C },
	{ std::string("jpg"),		IMG_CODEC_JPEG },
	{ std::string("jpeg"),		IMG_CODEC_JPEG },
	{ std::string("png"),		IMG_CODEC_PNG }
};
...
static std::string find_file(std::string& name, S8* codec)
{
	...
	{
		tname = name + "." + file_extensions[i].exten;
		...
		if (ifs.is_open())
		{
			...
			return file_extensions[i].exten;
		}
	}
	return "";
}
								...
								bool j2c_lowest_mip_only)
{
	...
	{
		case IMG_CODEC_BMP:
			imagep = new LLImageBMP();
			break;

		case IMG_CODEC_TGA:
			imagep = new LLImageTGA();
			break;

		case IMG_CODEC_JPEG:
			imagep = new LLImageJPEG();
			break;

		case IMG_CODEC_J2C:
			imagep = new LLImageJ2C();
			break;
	...
	U8* bufferp = imagep->allocateData(length);
	if (!bufferp)
	{
		return false;
	}

	ifs.read((char*)bufferp, length);
	ifs.close();

	bool success = imagep->updateData();
	if (success)
	{
		if (j2c_lowest_mip_only && codec == IMG_CODEC_J2C)
		{
			S32 width = imagep->getWidth();
			S32 height = imagep->getHeight();
			S32 discard_level = 0;
			...
			((LLImageJ2C*)imagep.get())->setDiscardLevel(discard_level);
		}
		success = imagep->decode(this);
	}

	imagep = NULL; // Delete image
	if (!success)
	{
		deleteData();
...
//static
LLImageFormatted* LLImageFormatted::createFromMimeType(const std::string& mimetype)
{
	S8 codec = IMG_CODEC_INVALID;
...
//static
LLImageFormatted* LLImageFormatted::loadFromMemory(const U8* datap, U32 size,
												   const std::string& mimetype)
{
	...
	LLImageFormatted* imagep = createFromMimeType(mimetype);
	if (!imagep)
	{
		return NULL;
	}
...
bool LLImageFormatted::load(const std::string& filename)
{
	...
	U8* datap = allocateData(file_size);
	if (!datap)
	{
	...
	if (infile.read(datap, file_size) != file_size)
	{
		deleteData();
...
bool LLImageFormatted::save(const std::string& filename)
{
	...
	S64 bytes = getDataSize();
	if (bytes <= 0)
	{
		llwarns << "Nothing to write. Not saving: " << filename << llendl;
		return false;
	}

	resetLastError();

	LLFile out(filename, "wb");	
	if (!out)
	{
		setLastError("Unable to open file for writing", filename);
		return false;
	}
	if (out.write(datap, bytes) != bytes)
	{
		setLastError("Short write (drive full ?)", filename);
		return false;		
	}
