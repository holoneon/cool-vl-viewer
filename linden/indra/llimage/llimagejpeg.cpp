...
static thread_local bool tGotError = false;
...
bool LLImageJPEG::updateData()
{
	...
	// an error so, instead, we use setjmp/longjmp to avoid this crash, which
...
bool LLImageJPEG::decode(LLImageRaw* raw_imagep)
{
	if (!raw_imagep)
	{
	...
	try
	{
		...
		raw_image_data = raw_imagep->getData();
...
bool LLImageJPEG::encode(const LLImageRaw* raw_imagep)
{
	if (!raw_imagep)
	{
	...
	switch (raw_imagep->getComponents())
	{
		case 1:
		case 3:
			break;
...
//static
bool LLImageJPEG::getDimensions(LLFile& file, S32& width, S32& height)
{
	tGotError = false;

	// Make sure this is a JPEG file.
	constexpr S64 JPEG_MAGIC_SIZE = 2;
	U8 bufferp[JPEG_MAGIC_SIZE];
	file.seek(0);	// Ensure we are at the start of file. HB
	if (file.read(bufferp, JPEG_MAGIC_SIZE) != JPEG_MAGIC_SIZE)
	{
		LLImage::setLastError("Premature end of file: " + file.getFileName());
		return false;
	}
	if (bufferp[0] != 0xff || bufferp[1] != 0xd8)
	{
		LLImage::setLastError("Not a JPEG file: " + file.getFileName());
		return false;
	}
	...
	try
	{
		// Now we can initialize the JPEG decompression object.
		jpeg_create_decompress(&cinfo);
		jpeg_stdio_src(&cinfo, file.getStream());
		jpeg_read_header(&cinfo, true);
		cinfo.out_color_space = JCS_RGB;
		jpeg_start_decompress(&cinfo);
