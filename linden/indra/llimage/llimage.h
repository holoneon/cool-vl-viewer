...
public:
	...
	// This replaces LLImageDimensionsInfo found in LL's sources.
	static bool getDimensions(const std::string& filename, S32& width,
							  S32& height, S8 codec = IMG_CODEC_INVALID);
...
public:
	...
	LL_INLINE U16 getWidth() const				{ return mWidth; }
	LL_INLINE U16 getHeight() const				{ return mHeight; }
	LL_INLINE S8 getComponents() const			{ return mComponents; }
	LL_INLINE S32 getDataSize() const			{ return mDataSize; }

	const U8* getData() const;
	U8* getData();

	LL_INLINE bool isBufferInvalid() const
	{
		return mBadBufferAllocation || !mData;
	}
...
public:
	...
	static LL_INLINE void setSizeOverride(bool enabled)
	{
...
public:
	...
	static LLImageFormatted* loadFromMemory(const U8* datap, U32 size,
...
public:
	...
	// Returns the maximum size of header; 0 indicates we do not know have a
	// header and have to lead the entire file
	LL_INLINE virtual S32 calcHeaderSize() 		{ return 0; };
	...
	LL_INLINE virtual S8 getRawDiscardLevel()	{ return mDiscardLevel; }
	...
	S8 getCodec() const;
	LL_INLINE bool isDecoding() const			{ return mDecoding != 0; }
	LL_INLINE bool isDecoded() const			{ return mDecoded != 0; }
	LL_INLINE void setDiscardLevel(S8 d)		{ mDiscardLevel = d; }
	LL_INLINE S8 getDiscardLevel() const		{ return mDiscardLevel; }

	// setLastError() needs to be deferred for J2C images since it may be
	// called from a DLL
