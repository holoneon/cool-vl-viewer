...
bool LLImageBMP::decode(LLImageRaw* raw_imagep)
{
	if (!raw_imagep)
	{
	...
	if (!raw_imagep->resize(getWidth(), getHeight(), 3))
	{
...
bool LLImageBMP::encode(const LLImageRaw* raw_imagep)
{
	if (!raw_imagep)
	{
		llwarns << "Attempted to decode a NULL raw image" << llendl;
	...
	S32 src_components = raw_imagep->getComponents();
	...
	setSize(raw_imagep->getWidth(), raw_imagep->getHeight(), dst_components);
...
// Helper function
static S32 read_reverse_s32(LLFile& file)
{
	U8 p[4];
	file.read(&p[0], 4);
	return (S32(p[0]) & 0x000000FF) | ((S32(p[1]) << 8) & 0x0000FF00) |
		   ((S32(p[2]) << 16) & 0x00FF0000) | ((S32(p[3]) << 24) & 0xFF000000);
}

//static
bool LLImageBMP::getDimensions(LLFile& file, S32& width, S32& height)
{
	// Make sure this is a BMP file. Minimum file size is BMP header (14) + DIB
	// header size (4) + width (4) + height (4).
	constexpr S64 HEADER_SIZE = 26;
	bool too_short = LLFile::getFileSize(file.getFileName()) < HEADER_SIZE;
	// Make sure this is a BMP file. We only support Windows bitmaps (BM),
	// in accordance with LLImageBMP::updateData().
	constexpr S64 BMP_MAGIC_SIZE = 2;
	U8 bufferp[BMP_MAGIC_SIZE];
	file.seek(0);	// Ensure we are at the start of file. HB
	if (too_short || file.read(bufferp, BMP_MAGIC_SIZE) != BMP_MAGIC_SIZE)
	{
		LLImage::setLastError("Premature end of file: " + file.getFileName());
		return false;
	}
	if (bufferp[0] != 'B' || bufferp[1] != 'M')
	{
		LLImage::setLastError("Not a BMP file: " + file.getFileName());
		return false;
	}

	// Read image dimensions.
	file.seek(16);
	width = read_reverse_s32(file);
	height = read_reverse_s32(file);
	return true;
}
