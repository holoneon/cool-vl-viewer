...
void LLStreamingAudio_FMOD::update()
{
	...
	FMOD::Sound* soundp = NULL;

	if (mFMODInternetStreamChannelp->getCurrentSound(&soundp) == FMOD_OK &&
		soundp)
	{
		FMOD_TAG tag;
		S32 tagcount, dirtytagcount;

		if (soundp->getNumTags(&tagcount, &dirtytagcount) == FMOD_OK &&
			dirtytagcount)
		{
			S32 count = llclamp(tagcount, 0, 1024);	// Paranoia
			if (count != tagcount)
			{
				llwarns << "Bogus tag count: " << tagcount << " - Clamped to: "
						<< count << llendl;
			}
			std::string token;
			for (S32 i = 0; i < count; ++i)
			{
				if (soundp->getTag(NULL, i, &tag) != FMOD_OK)
				{
					continue;
				}
		...
		else if (mLastStarved != 0.f && progress > 50)
		{
			...
			if (buffer_fill_time > buffer_size_seconds)
			{
				llwarns << "Starvation state cleared, resuming streaming music playing but new starvations will likely occur (time required to fill the buffer = "
						<< buffer_fill_time << " - buffer size in seconds = "
