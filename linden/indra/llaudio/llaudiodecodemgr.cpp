...
// gcc 15 sees array bound issues where there are none... HB
#if defined(GCC_VERSION) && GCC_VERSION >= 150000
...
void LLAudioDecodeMgr::Impl::startMoreDecodes()
{
	...
	// Note: gAudioDecodeMgrp->mImpl would change should the audio engine be
	// restarted while the decode queue is not empty, so let's ensure 'this' is
	// still valid... HB
	while (gAudioDecodeMgrp && gAudioDecodeMgrp->mImpl == this &&
		   !LLApp::isExiting() && !mDecodeQueue.empty())
	{
...
LLVorbisDecodeState::ptr_t LLAudioDecodeMgr::Impl::beginDecode(const LLUUID& id)
{
	...
	std::string d_path = gDirUtil.getFullPath(LL_PATH_CACHE, id.asString()) +
						 ".dsf";
												...
												LLVorbisDecodeState::ptr_t& state)
{
	...
	// restarted while the decode queue is not empty, so let's ensure 'this' is
	// still valid... HB
	if (!gAudioDecodeMgrp || gAudioDecodeMgrp->mImpl != this)
	{
		return;
	}
