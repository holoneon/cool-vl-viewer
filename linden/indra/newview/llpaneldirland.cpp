...
void LLPanelDirLand::performQuery()
{
	...
	msg->newMessage(_PREHASH_DirLandQuery);
	...
	msg->addUUID(_PREHASH_QueryID, getSearchID());
	msg->addU32(_PREHASH_QueryFlags, query_flags);
	msg->addU32(_PREHASH_SearchType, search_type);
	msg->addS32(_PREHASH_Price, childGetValue("priceedit").asInteger());
	msg->addS32(_PREHASH_Area, childGetValue("areaedit").asInteger());
