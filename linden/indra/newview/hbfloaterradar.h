...
public:
	HBRadarListEntry(LLVOAvatar* avatarp, const LLUUID& avid,
					 const std::string& name, const std::string& listed_name,
	...
	// Returns the legacy name of the avatar
	LL_INLINE const std::string& getLegacyName() const
	{
		return mLegacyName;
	}

	// Sets the name of the avatar as listed in the radar floater
	LL_INLINE void setListedName(const std::string& name)
	{
		mListedName = name;
	}

	// Returns the name of the avatar as listed in the radar floater
	LL_INLINE const std::string& getListedName() const
	{
		return mListedName;
	}
...
public:
	...
	// Used by hbviewerautomation.cpp
	LL_INLINE const avatar_list_t& getAvatarList() const
	{
		return mAvatars;
	}
	...
	// objects list (gObjectList.mBlackListedObjects) by adding or removing an
	// avatar. Using such a method prevents from having to check each listed
	// avatar render status at each avatar list update.
