...
U8 LLRegionInfoModel::sOnDeath = 0;
U32 LLRegionInfoModel::sCombatFlags = 0;
F32 LLRegionInfoModel::sDamageThrottle = 0.f;
F32 LLRegionInfoModel::sRegenerationRate = 0.f;
F32 LLRegionInfoModel::sInvulnerabilityTime = 0.f;
F32 LLRegionInfoModel::sDamageLimit = 0.f;
bool LLRegionInfoModel::sSupportsCombat2 = false;
...
:	mHandle(handle),
	...
	mHasPBRTerrainTransforms(false),
	mGLTFEnabled(false),
	mHasLuaCompileTarget(false),
	mHasWebRTCVoice(false),
	mBitsIn(0),
	mLastBitsIn(0),
	mFirstWindLayerReceivedTime(0.f),
	mGotClouds(false),
	mWidth(region_width_meters),				// Variable region size support
	mMinSimHeight(0.f)							// OpenSim min Z can go below 0
{
...
void LLViewerRegion::processRegionInfo(LLMessageSystem* msg, void**)
{
	...
	F32 whisper_range = 0.f;
	F32 chat_range = 0.f;
	F32 shout_range = 0.f;
	if (msg->has(_PREHASH_RegionInfo5))
	{
		msg->getF32Fast(_PREHASH_RegionInfo5, _PREHASH_ChatWhisperRange,
						whisper_range);
		msg->getF32Fast(_PREHASH_RegionInfo5, _PREHASH_ChatNormalRange,
						chat_range);
		msg->getF32Fast(_PREHASH_RegionInfo5, _PREHASH_ChatShoutRange,
						shout_range);
	}

	if (msg->has(_PREHASH_CombatSettings))
	{
		LLRegionInfoModel::sSupportsCombat2 = true;
		msg->getU32Fast(_PREHASH_CombatSettings, _PREHASH_CombatFlags,
						LLRegionInfoModel::sCombatFlags);
		msg->getU8Fast(_PREHASH_CombatSettings, _PREHASH_OnDeath,
					   LLRegionInfoModel::sOnDeath);
		msg->getF32Fast(_PREHASH_CombatSettings, _PREHASH_DamageThrottle,
						LLRegionInfoModel::sDamageThrottle);
		msg->getF32Fast(_PREHASH_CombatSettings, _PREHASH_RegenerationRate,
						LLRegionInfoModel::sRegenerationRate);
		msg->getF32Fast(_PREHASH_CombatSettings, _PREHASH_InvulnerabilyTime,
						LLRegionInfoModel::sInvulnerabilityTime);
		msg->getF32Fast(_PREHASH_CombatSettings, _PREHASH_DamageLimit,
						LLRegionInfoModel::sDamageLimit);
		llinfos << "Combat 2 system enabled. Combat flags: "
				<< std::hex << LLRegionInfoModel::sCombatFlags << std::dec
				<< " - On death: " << LLRegionInfoModel::sOnDeath
				<< " - Damage throttle: " << LLRegionInfoModel::sDamageThrottle
				<< " - Regeneration rate: "
				<< LLRegionInfoModel::sRegenerationRate
				<< " - Invulnerability time: "
				<< LLRegionInfoModel::sInvulnerabilityTime
				<< " - Damage limit: " << LLRegionInfoModel::sDamageLimit
				<< llendl;
	}
	else
	{
		LLRegionInfoModel::sSupportsCombat2 = false;
	}
	...
	if (self)
	{
		...
		if (whisper_range > 0.f)
		{
			self->mWhisperRange = whisper_range;
		}
		if (chat_range > 0.f)
		{
			self->mChatRange = chat_range;
		}
		if (shout_range > 0.f)
		{
			self->mShoutRange = shout_range;
		}
	}
...
void LLViewerRegion::addActiveCacheEntry(LLVOCacheEntry* entryp)
{
	if (!entryp || mDead ||
		// Ignore if already inserted
		entryp->isState(LLVOCacheEntry::ACTIVE))
	{
		return;
	}
	...
	entryp->setState(LLVOCacheEntry::ACTIVE);
	entryp->setVisible();

	llassert(entryp->getEntry()->hasDrawable());
	mActiveSet.insert(entryp);
}

void LLViewerRegion::removeActiveCacheEntry(LLVOCacheEntry* entryp,
											LLDrawable* drawablep)
{
	if (mDead || !entryp || !entryp->isValid() ||
	...
	if (drawablep && drawablep->getVObj().notNull())
	{
		...
		LLVector4a shift;
		shift.load3(pos.mV);
		shift.sub(entryp->getPositionGroup());
		entryp->shift(shift);
	}

	if (entryp->getParentID() > 0) // is a child
	{
		LLVOCacheEntry* parentp = getCacheEntry(entryp->getParentID());
		if (parentp)
		{
			parentp->addChild(entryp);
		}
		else
		{
			// Parent not in cache (happens only when it is not cacheable)
			mOrphanMap[entryp->getParentID()].push_back(entryp->getLocalID());
		}
	}
	else
	{
		// Insert to vo cache tree.
		entryp->updateParentBoundingInfo();
		entryp->saveBoundingSphere();
		addToVOCacheTree(entryp);
	}
...
void LLViewerRegion::updateReflectionProbes(bool reload)
{
	...
	for (U32 i = 0; i < grid_width; ++i)
	{
		...
		for (U32 j = 0; j < grid_width; ++j)
		{
			...
			LLReflectionMap* mapp = mReflectionMaps[idx].get();
			if (reload || !mapp)
			{
				mapp = gPipeline.mReflectionMapManager.addProbe();
				mReflectionMaps[idx] = mapp;
				if (!mapp)
				{
					continue;
				}
			...
			pos += origin;
			mapp->mOrigin.load3(pos.mV);
			mapp->mRadius = PROBE_RADIUS;
		}
	}
}

void LLViewerRegion::addToVOCacheTree(LLVOCacheEntry* entryp)
{
	if (entryp && sVOCacheCullingEnabled && !mDead &&
		entryp->getEntry() && entryp->isValid() &&
		entryp->getParentID() == 0 &&					// Child in octree
		!entryp->hasState(LLVOCacheEntry::IN_VO_TREE))	// Not in octree
	{
		llassert_always(!entryp->getGroup());			// Not in octree.
		llassert(!entryp->getEntry()->hasDrawable());	// No drawable

		if (mVOCachePartition->addEntry(entryp->getEntry()))
		{
			entryp->setState(LLVOCacheEntry::IN_VO_TREE);
		}
	}
}

void LLViewerRegion::removeFromVOCacheTree(LLVOCacheEntry* entryp)
{
	if (entryp && !mDead && entryp->getEntry() &&
		// Must be in tree
		entryp->hasState(LLVOCacheEntry::IN_VO_TREE))
	{
		entryp->clearState(LLVOCacheEntry::IN_VO_TREE);
		mVOCachePartition->removeEntry(entryp->getEntry());
	}
}

// Add child objects as visible entries
void LLViewerRegion::addVisibleChildCacheEntry(LLVOCacheEntry* parentp,
											   LLVOCacheEntry* childp)
{
	if (mDead)
	{
		return;
 	...
 	if (parentp &&
		(!parentp->isValid() || !parentp->isState(LLVOCacheEntry::ACTIVE)))
	{
	...
	if (childp &&
		(!childp->getEntry() || !childp->isValid() ||
		 !childp->isState(LLVOCacheEntry::INACTIVE)))
	{
	...
	if (childp)
	{
		childp->setState(LLVOCacheEntry::IN_QUEUE);
		mVisibleEntries.insert(childp);
	}
	else if (parentp && parentp->getNumOfChildren() > 0)
	{
		// Add all children
		while ((childp = parentp->getChild()))
		{
			addVisibleChildCacheEntry(NULL, childp);
		}
	}
}
...
void LLViewerRegion::clearCachedVisibleObjects()
{
		 ...
		 iter != mVisibleEntries.end(); )
	{
		...
		if (!entryp->getParentID() || parentp)
		{
			if (parentp) // Has a cache-able parent
			{
				parentp->addChild(entryp);
			}
...
void LLViewerRegion::killObject(LLVOCacheEntry* entryp,
								std::vector<LLDrawable*>& delete_list)
{
	...
	if (drawablep && !drawablep->getParent())
	{
		...
		if (vobj)
		{
				 ...
				 iter != end; ++iter)
			{
				...
				if (childp && childp->mDrawable)
				{
					if (!childp->mDrawable->getEntry() ||
						!childp->mDrawable->getEntry()->hasVOCacheEntry() ||
						childp->isSelected() ||
						(childp->flagAnimSource() &&
						 isAgentAvatarValid() &&
						 gAgentAvatarp->hasMotionFromSource(childp->getID())))
					{
					...
					LLOcclusionCullingGroup* groupp =
						(LLOcclusionCullingGroup*)childp->mDrawable->getGroup();
					if (groupp && groupp->isAnyRecentlyVisible())
					{
...
LLViewerObject* LLViewerRegion::addNewObject(LLVOCacheEntry* entryp)
{
	if (!entryp || !entryp->getEntry())
	{
		if (entryp)
		{
			mVisibleEntries.erase(entryp);
	...
	if (!entryp->getEntry()->hasDrawable())
	{
		 // Not yet added to the rendering pipeline... Add the object now.
		objp = gObjectList.processObjectUpdateFromCache(entryp, this);
		if (objp && !entryp->isState(LLVOCacheEntry::ACTIVE))
		{
			mWaitingSet.insert(entryp);
			entryp->setState(LLVOCacheEntry::WAITING);
		}
	}
	else
	{
		LLViewerRegion* old_regionp =
			((LLDrawable*)entryp->getEntry()->getDrawable())->getRegion();
		if (old_regionp != this)
		{
			...
			llwarns_once << "Entry: " << entryp->getLocalID()
												 ...
												 LLViewerObject* objectp)
{
	...
	if (!objectp) // Object not created
	{
		...
		objectp = addNewObject(entryp);
	}

	// Remove from cache
	killCacheEntry(entryp, true);
...
void LLViewerRegion::setSimulatorFeatures(const LLSD& sim_features)
{
	...
	LL_DEBUGS("SimulatorFeatures") << "Mesh upload "
	...
	if (mHasPBRTerrain &&
		mSimulatorFeatures.has("PBRTerrainTransformsEnabled"))
	{
		mHasPBRTerrainTransforms = true;
	}

	// Lua script compile target support
	if (mSimulatorFeatures.has("LuaScriptsEnabled"))
	{
		mHasLuaCompileTarget =
			mSimulatorFeatures["LuaScriptsEnabled"].asBoolean();
	}
	...
	{
		mHasWebRTCVoice =
			mSimulatorFeatures["VoiceServerType"].asString() == "webrtc";
	}
	LL_DEBUGS("SimulatorFeatures") << "WebRTC voice "
								   << (mHasWebRTCVoice ? "available"
													   : "unavailable")
								   << LL_ENDL;

	// OpenSim-specific feature for per-grid WebRTC voice support. HB
	if (!gIsInSecondLife && mSimulatorFeatures.has("stun-servers"))
	{
		mStunServers = mSimulatorFeatures["stun-servers"].asString();
		LL_DEBUGS("SimulatorFeatures") << "WebRTC stun servers: "
									   << mStunServers << LL_ENDL;
	}
	...
	if (mSimulatorFeatures.has("OpenSimExtras"))
	{
		...
		if (extras.has("MinSimHeight"))
		{
			mMinSimHeight = (F32)extras["MinSimHeight"].asReal();
			LL_DEBUGS("SimulatorFeatures") << "MinSimHeight: " << mMinSimHeight
										   << "m" << LL_ENDL;
		}
	}
...
void LLViewerRegion::decodeBoundingInfo(LLVOCacheEntry* entryp)
{
	if (!sVOCacheCullingEnabled)
	{
		gObjectList.processObjectUpdateFromCache(entryp, this);
		return;
	}
	if (!entryp || !entryp->isValid())
	{
	...
	if (entryp->getEntry()->hasDrawable())
	{
		...
		// Set parent id
		U32	parent_id = 0;
		LLViewerObject::unpackParentID(entryp->getDP(), parent_id);
		if (parent_id != entryp->getParentID())
		{
			entryp->setParentID(parent_id);
		}
	...
	// Must not be active.
	llassert_always(!entryp->isState(LLVOCacheEntry::ACTIVE));
	removeFromVOCacheTree(entryp); // Remove from cache octree if it is in.

	LLVector3 pos, scale;
	LLQuaternion rot;

	// Decode spatial info and parent info
	U32 parent_id = LLViewerObject::extractSpatialExtents(entryp->getDP(),
														  pos, scale, rot);
	U32 old_parent_id = entryp->getParentID();
	...
	if (parent_id > 0) // Got a new parent
	{
		...
		LLVOCacheEntry* parentp = getCacheEntry(parent_id);
		...
		if (!parentp)
		{
			if (!same_old_parent)
			{
				...
				{
					// Parent is visible, so is the child.
					addVisibleChildCacheEntry(NULL, entryp);
				}
				else
				{
					entryp->setBoundingInfo(pos, scale);
					mOrphanMap[parent_id].push_back(entryp->getLocalID());
				}
			}
			else
			{
		...
		// Not in cache.
		else if (!parentp->isState(LLVOCacheEntry::INACTIVE))
		{
			// Parent is visible, so is the child.
			addVisibleChildCacheEntry(parentp, entryp);
		}
		else
		{
			entryp->setBoundingInfo(pos, scale);
			...
			if (parentp->getGroup())
			{
				...
				// changed.
				removeFromVOCacheTree(parentp);
				addToVOCacheTree(parentp);
			}
		}
	...
	if (!parent_id) // a potential parent
	{
		...
		orphan_list_t::iterator iter = mOrphanMap.find(entryp->getLocalID());
		if (iter != mOrphanMap.end())
		{
			std::vector<U32>* orphans = &mOrphanMap[entryp->getLocalID()];
			for (S32 i = 0, count = orphans->size(); i < count; ++i)
			{
				LLVOCacheEntry* childp = getCacheEntry((*orphans)[i]);
				if (childp)
				{
					entryp->addChild(childp);
				}
			}
			orphans->clear();
			mOrphanMap.erase(entryp->getLocalID());
		}
	}

	if (!entryp->getGroup() && entryp->isState(LLVOCacheEntry::INACTIVE))
	{
		addToVOCacheTree(entryp);
	}
}

LLViewerRegion::eCacheUpdateResult
	LLViewerRegion::cacheFullUpdate(LLDataPackerBinaryBuffer& dp, U32 flags)
{
	...
	else
	{
		...
		LLPointer<LLVOCacheEntry> new_entryp = new LLVOCacheEntry(local_id,
																  crc, dp);
		mCacheMap[local_id] = new_entryp;
		decodeBoundingInfo(new_entryp);
		entryp = new_entryp;
	...
	if (flags != 0xffffffff)
	{
		...
		if (fullid.notNull())
		{
			LL_DEBUGS("ObjectCacheSpam") << "Set cache entryp flags for object "
										 << fullid << " to: " << flags
										 << LL_ENDL;
...
void LLViewerRegion::dumpCache()
{
	...
	for (LLVOCacheEntry::map_t::iterator iter = mCacheMap.begin(),
		 ...
		 iter != end; ++iter)
	{
		LLVOCacheEntry* entryp = iter->second;

		S32 hits = entryp->getHitCount();
		S32 changes = entryp->getCRCChangeCount();
...
static void refresh_terrain(const LLUUID& region_id, const LLTerrain* terrainp)
{
...
void LLViewerRegion::unpackRegionHandshake()
{
	...
	// Do not call this when there has been no change: refresh_terrain() would
	// cause PBR terrain render materials to be destroyed when entering the new
	// agent region from a neighbouring region, and anyway, when there has been
	// no change, this whole block of code is useless. HB
	if (changed)
	{
		const std::string& cap_url = getCapability("ModifyRegion");
		if (cap_url.empty())
		{
			if (this == gAgent.getRegion())
			{
				LLFloaterRegionInfo::updateFromRegionInfo();
			}
		}
		...
		{
			LLPBRTerrainFeatures::queueQuery(this,
...
void LLViewerRegion::buildCapabilityNames(LLSD& cap_names)
{
	...
	cap_names.append("SpatialVoiceModerationRequest");
...
void LLViewerRegion::setSeedCapability(const std::string& url)
{
	if (getCapability("Seed") == url)
	{
		std::string coro =
			gCoros.launch("LLViewerRegion::requestBaseCapabilitiesCompleteCoro",
						  std::bind(&LLViewerRegion::requestBaseCapabilitiesCompleteCoro,
								    mHandle));
	...
	std::string coro =
		gCoros.launch("LLViewerRegion::requestBaseCapabilitiesCoro",
					  std::bind(&LLViewerRegion::requestBaseCapabilitiesCoro,
								mHandle));
	llinfos << "Coroutine " << coro
	...
	else if (name == "SimulatorFeatures")
	{
		...
		std::string coro_name =
			gCoros.launch("LLViewerRegion::requestSimulatorFeatureCoro",
						  std::bind(&LLViewerRegion::requestSimulatorFeatureCoro,
									url, mHandle));
...
{
	strings_map_t::const_iterator iter = mCapabilities.find(name);
...
boost::signals2::connection
	LLViewerRegion::setCapsReceivedCB(const caps_received_cb_t& cb)
{
...
{
	U32 count = 0;
	for (strings_map_t::const_iterator iter = mCapabilities.begin(),
