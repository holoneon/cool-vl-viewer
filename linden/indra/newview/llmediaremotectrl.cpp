...
void LLMediaRemoteCtrl::draw()
{
	...
	if (mType == REMOTE_PARCEL_MEDIA)
	{
			...
			LLViewerParcelMedia::hasParcelMedia())
		{
			...
			LLViewerMediaImpl::EMediaStatus status =
				LLViewerParcelMedia::getStatus();
			switch (status)
			{
				...
				case LLViewerMediaImpl::MEDIA_PLAYING:
					...
					media_show_pause =
						LLMIMETypes::widgetType(parcel->getMediaType()) ==
							"movie";
	...
	else if (mType == REMOTE_PARCEL_MUSIC)
	{
		...
		if (mIcon)
		{
			...
			{
				LLStreamingAudioInterface* streamp =
					gAudiop->getStreamingAudioImpl();
				if (streamp)
				{
					if (streamp->newMetaData())
					{
						mCachedMetaData.clear();
						std::string temp = streamp->getArtist();
						if (!temp.empty())
						{
							mCachedMetaData += artist_str + temp;
						}
						temp = streamp->getTitle();
						if (!temp.empty())
						{
