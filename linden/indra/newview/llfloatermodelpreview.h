...
public:
	...
	{
	public:
		DecompRequest(const std::string& stage, LLModel* mdlp);
	...
	LL_INLINE static void setUploadAmount(S32 fee)	{ sUploadAmount = fee; }

	static LLModelPreview* getModelPreview();

	void setDetails(F32 x, F32 y, F32 z);

	F32 getModelScaling() const;
	...
	LL_INLINE LLSpinCtrl* getScaleSpinner() const	{ return mModelScaling; }
...
protected:
	...
	static void onTabChanged(void* datap, bool);

	static void onUpload(void* datap);
	static void onReset(void* datap);

	static void	onImportScaleCommit(LLUICtrl*, void* datap);
	static void	onPelvisOffsetCommit(LLUICtrl*, void* datap);
	static void	onUploadJointsCommit(LLUICtrl*, void* datap);
	static void	onUploadSkinCommit(LLUICtrl*, void* datap);

	static void onJointListSelection(LLUICtrl*, void* datap);

	// NOTE: for these, 'userdata' bears the LOD level instead of 'this'
	static void onBrowseLOD(void* userdata);
	static void	onPreviewLODCommit(LLUICtrl* ctrlp, void* userdata);
	...
	static void	onGenerateNormalsCommit(LLUICtrl*, void* datap);

	static void toggleGenerateNormals(LLUICtrl*, void* datap);

	static void	onExplodeCommit(LLUICtrl*, void* datap);

	static void	onAutoFillCommit(LLUICtrl*, void* datap);

	static void toggleCalculateButtonCallBack(LLUICtrl*, void* datap);

	static void onClickTextLOD(void* datap);

	static void onClickCalculateBtn(void* datap);

	static void onViewOptionChecked(LLUICtrl* ctrl, void* datap);

	// For these ones, datap bears a LLCDStageData pointer instead of 'this'
	static void onPhysicsStageExecute(LLUICtrl*, void* datap);
	static void onPhysicsParamCommit(LLUICtrl* ctrlp, void* datap);

	static void onPhysicsStageCancel(void* datap);
	static void onCancel(void* datap);

	static void onPhysicsBrowse(void* datap);
	static void onPhysicsUseLOD(LLUICtrl*, void* datap);

	static void onClickValidateURL(void* datap);
...
private:
	...
	void fillLODSourceStats(strings_map_t& lod_sources) const;
...
protected:
	...
	typedef std::map<std::string, bool, std::less<> > view_option_map_t;
	view_option_map_t					mViewOptionDisabled;
...
private:
	...
	struct JointOverrideData
	{
		...
		typedef std::map<std::string, LLVector3,
						 std::less<> > pos_override_map_t;
		pos_override_map_t	mPosOverrides;
		// Models without overrides
		strings_set_t		mModelsNoOverrides;
		bool				mHasConflicts;
	};
	typedef std::map<std::string, JointOverrideData,
					 std::less<> > overrides_map_t;
...
public:
	...
	void getJointAliases(strings_map_t& joint_map);
...
protected:
	...
	static void loadedCallback(LLModelLoader::scene& scene,
							   ...
							   S32 lod, void* datap);
	static void stateChangedCallback(U32 state, void* datap);

	static LLJoint* lookupJointByName(const std::string&, void* datap);
	static U32 loadTextures(LLImportMaterial& material, void* datap);
...
protected:
	...
	LLPointer<LLModel>		mDefaultPhysModel;
