...
S32 LLAvatarTracker::addBuddyList(const LLAvatarTracker::buddy_map_t& buds)
{
	...
	for (size_t i = 0, count = mDelayedOnlineList.size(); i < count; ++i)
	{
		const LLUUID& buddy_id =  mDelayedOnlineList[i];
		if (mBuddyInfo.count(buddy_id))
		{
			setBuddyOnline(buddy_id, true);
		}
		else
		{
			llwarns << "No buddy info found for buddy " << buddy_id
					<< ". Cannot set their status to: online." << llendl;
		}
	}
	mDelayedOnlineList.clear();
...
void LLAvatarTracker::terminateBuddy(const LLUUID& id)
{
	...
	if (buddy)
	{
		mBuddyInfo.erase(id);
		LLMessageSystem* msg = gMessageSystemp;
		msg->newMessage(_PREHASH_TerminateFriendship);
		msg->nextBlock(_PREHASH_AgentData);
		msg->addUUID(_PREHASH_AgentID, gAgentID);
		msg->addUUID(_PREHASH_SessionID, gAgentSessionID);
		msg->nextBlock(_PREHASH_ExBlock);
		msg->addUUID(_PREHASH_OtherID, id);
		gAgent.sendReliableMessage();
...
{
	LLRelationship* infop = get_ptr_in_map(mBuddyInfo, id);
	if (!infop)
	{
		if (is_online && LLStartUp::getStartupState() < STATE_MISC)
		{
			llinfos << "Received online status for buddy Id " << id
					<< " before the buddy list is received. Queuing."
					<< llendl;
			mDelayedOnlineList.emplace_back(id);
		}
		else
		{
			llwarns << "No buddy info found for buddy Id " << id
...
{
	const LLRelationship* infop = get_ptr_in_map(mBuddyInfo, id);
	return infop &&
...
void LLAvatarTracker::addObserver(LLFriendObserver* observerp)
{
	if (observerp)
	{
		mObservers.push_back(observerp);
	}
}

void LLAvatarTracker::removeObserver(LLFriendObserver* observerp)
{
...
{
	if (mIsNotifyObservers || LLStartUp::getStartupState() < STATE_MISC)
	{
	...
	for (observer_list_t::iterator it = observers.begin(),
		 ...
		 it != end; ++it)
	{
		LLFriendObserver* observerp = *it;
		if (observerp)
		{
			observerp->changed(mModifyMask);
			observerp->changedBuddies(mChangedBuddyIDs);
		}
	}
...
void LLAvatarTracker::processChange(LLMessageSystem* msg)
{
	...
	for (S32 i = 0; i < count; ++i)
	{
		msg->getUUIDFast(_PREHASH_Rights, _PREHASH_AgentRelated, agent_related,
						 i);
		msg->getS32Fast(_PREHASH_Rights, _PREHASH_RelatedRights, new_rights,
						i);
...
void LLAvatarTracker::processNotify(LLMessageSystem* msg, bool online)
{
	...
	bool inv_callbacks_registered = LLStartUp::getStartupState() >= STATE_MISC;
	LLUUID buddy_id;
	for (S32 i = 0; i < count; ++i)
	{
		msg->getUUIDFast(_PREHASH_AgentBlock, _PREHASH_AgentID, buddy_id, i);
		if (getBuddyInfo(buddy_id))
		{
			setBuddyOnline(buddy_id, online);
			if (chat_notify)
			{
				LLAvatarNameCache::get(buddy_id,
									   std::bind(&callbackLoadAvatarName, _1,
												 online, _2));
			}
		}
		else if (online && !inv_callbacks_registered)
		{
			llinfos << "Received online status for buddy Id " << buddy_id
					<< " before the buddy list is received. Queuing."
					<< llendl;
			mDelayedOnlineList.emplace_back(buddy_id);
		}
		else
		{
			llwarns << "Received online notification for unknown buddy: "
					<< buddy_id << "; cannot set status to: "
					<< (online ? "online" : "offline") << llendl;
		}

		if (tracking_id == buddy_id)
		{
			// We were tracking someone who went offline
			deleteTrackingData();
		}

		addChangedMask(LLFriendObserver::ONLINE, buddy_id);
	...
	// Only notify inventory after its callbacks have been initialized.
	if (inv_callbacks_registered)
	{
...
void LLAvatarTracker::formFriendship(const LLUUID& id)
{
	if (id.notNull())
	{
		LLRelationship* infop = get_ptr_in_map(gAvatarTracker.mBuddyInfo, id);
		if (!infop)
		{
			// The default for relationship establishment is to have both
			// parties visible online to each other.
			infop = new LLRelationship(LLRelationship::GRANT_ONLINE_STATUS,
									   LLRelationship::GRANT_ONLINE_STATUS,
									   false);
			gAvatarTracker.mBuddyInfo[id] = infop;
										  ...
										  LLRelationship* infop)
{
	...
	if (infop->isOnline() &&
		infop->isRightGrantedFrom(LLRelationship::GRANT_MAP_LOCATION))
	{
