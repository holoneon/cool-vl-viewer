...
// This structure is used to store event poll replies until we can safely
// process them in the main coroutine of the main thread. HB
	...
	mHttpHeaders(new LLCore::HttpHeaders)
{
	...
	static LLCachedControl<U32> slto(gSavedSettings, "EventPollTimeoutForSL");
	static LLCachedControl<U32> osto(gSavedSettings, "EventPollTimeoutForOS");
	mRequestTimeout = llclamp(U32(gIsInSecondLife ? slto : osto), 15, 180);
...
void LLEventPollImpl::start(const std::string& url)
{
	...
	std::string coroname =
		gCoros.launch("LLEventPollImpl::eventPollCoro",
					  std::bind(&LLEventPollImpl::eventPollCoro, url, this));
...
void LLEventPollImpl::handleMessage(const LLSD& content)
{
	...
	sReplies.emplace_back(mPollName, msg_name, mSenderIP, content["body"]);
}
									...
									LLPointer<LLEventPollImpl> impl)
{
	...
	while (!self->mDone && !gDisconnected)
	{
		...
		// If this request reply came faster than our preset "min delay for an
		...
		if (!status)
		{
			...
			else if (!status.isHttpStatus())
			{
				...
				llwarns << self->mPollName
						<< "Error returned from libraries. Ignoring and retrying..."
						<< llendl;
			}
			...
			if (error_count < max_retries)
			{
				...
				// Split the waiting delay in (5s) increments, to react faster
				// to any disconnection. HB
				for (S32 i = 0;
					 i < error_count && !self->mDone && !gDisconnected; ++i)
				{
					wait = EVENT_POLL_ERROR_RETRY_SECONDS +
						   i * EVENT_POLL_ERROR_RETRY_SECONDS_INC;
		...
		{
			const LLSD& metadata = result["metadata"];
			U32 first_event = 0;
			U32 last_event = 0;
			U32 dropped = 0;
			if (metadata.has("first_event"))
			{
				first_event = (U32)metadata["first_event"].asInteger();
			}
			if (metadata.has("last_event"))
			{
				last_event = (U32)metadata["last_event"].asInteger();
			}
			if (metadata.has("dropped"))
			{
				dropped = (U32)metadata["dropped"].asInteger();
			}
			if (last_event || first_event || dropped)
			{
				if (last_event < first_event)
				{
					last_event += U32(S32_MAX);
				}
				LL_CONT << "First event: " << first_event << " - Last event: "
						<< last_event << " - Dropped: " << dropped;
			}
			else
			{
				LL_CONT << "Unknown metadata: " << metadata;
			}
		}
		else
		{
...
{
	// Note: coroutines calling handleMessage() all belong to the main thread,
	// and LLEventPoll::dispatchMessages() is also called from the main thread,
	// so we do not need a mutex before touching sReplies; should this ever
	// change, a mutex lock would be needed here. HB
	for (U32 i = 0, count = sReplies.size(); i < count; ++i)
	{
		...
		LL_DEBUGS("EventPoll") << reply.mPollName << "Processing message: "
							   << reply.mMessageName << " - From: "
							   << reply.mSenderIP << LL_ENDL;
		LLMessageSystem::dispatch(reply.mMessageName, reply.mSenderIP,
								  reply.mMessageBody);
	}
	sReplies.clear();
}
