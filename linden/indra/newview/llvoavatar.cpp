...
bool LLVOAvatar::sNeedsAttachmentUpdate = false;
...
:	LLAvatarAppearance(&gAgentWearables),
	...
	mNextVoiceVisualizerUpdate(0.f),
...
void LLVOAvatar::dumpBakedStatus()
{
	...
	for (S32 i = 0, count = LLCharacter::sInstances.size(); i < count; ++i)
	{
		...
		if (!avatarp || avatarp->isDead()) continue;
		...
		if (avatarp->isDead())
		{
			llcont << " DEAD (" << avatarp->getNumRefs() << " refs)";
		}
		...
		F64 dist_to_camera = (avatarp->getPositionGlobal() -
							  camera_pos_global).length();
		llcont << " " << dist_to_camera << "m ";

		llcont << " " << avatarp->mPixelArea << " pixels";
		...
		else
		{
				 ...
				 iter != end; ++iter)
			{
				...
				if (avatarp->isTextureDefined(index)) continue;
...
void LLVOAvatar::deleteCachedImages(bool clearAll)
{
	if (LLViewerTexLayerSet::sHasCaches)
	{
		...
		for (S32 i = 0, count = LLCharacter::sInstances.size(); i < count; ++i)
		{
			...
			if (avatarp)
			{
				avatarp->deleteLayerSetCaches(clearAll);
			}
		}
...
void LLVOAvatar::initClass()
{
	...
	LLVOAvatarPuppet::sRegionChangedSlot =
		gAgent.addRegionChangedCB(std::bind(&LLVOAvatarPuppet::onRegionChanged));
}
										 ...
										 LLVector4a& new_max)
{
	...
	else if (box_detail >= 1)
	{
		for (polymesh_map_t::iterator i = mPolyMeshes.begin(),
			 ...
			 i != end; ++i)
		{
			LLPolyMesh* meshp = i->second;
			if (!meshp) continue;	// Paranoia

			for (S32 joint_num = 0, count = meshp->mJointRenderData.size();
				 joint_num < count; ++joint_num)
			{
				// Load translation:
				temp1.load3(meshp->mJointRenderData[joint_num]->mWorldMatrix->getTranslation().mV);
	...
	if (box_detail >= 2)
	{
		...
		{
			LLViewerObject* objp = mAttachedObjectsVector[i].first;
			...
			if (volp && volp->isAnimatedObject())
			{
				...
				if (puppetp)
				{
					temp2.load3(puppetp->mLastAnimExtents[0].mV);
					temp3.load3(puppetp->mLastAnimExtents[1].mV);
			...
			LLDrawable* drawp = objp->mDrawable;
			if (!drawp || drawp->isState(LLDrawable::RIGGED |
			...
			LLSpatialBridge* bridgep = drawp->getSpatialBridge();
			if (bridgep)
			{
				// Calculate distance:
				const LLVector4a* ext = bridgep->getSpatialExtents();
...
bool LLVOAvatar::lineSegmentIntersect(const LLVector4a& start,
									  ...
									  S32*, LLVector4a* intersectp,
									  LLVector2* tcoordp, LLVector4a* normalp,
									  LLVector4a* tangentp)
{
	...
	if (lineSegmentBoundingBox(start, end))
	{
		for (S32 i = 0, count = mCollisionVolumes.size(); i < count; ++i)
		{
								   ...
								   LLVector3::zero, 1.f, position, norm))
			{
				if (intersectp)
				{
					intersectp->load3(position.mV);
					mat.affineTransform(*intersectp, *intersectp);
				}
				if (normalp)
				{
					normalp->load3(norm.mV);
					normalp->normalize3fast();
					norm_mat.perspectiveTransform(*normalp, *normalp);
				}
				return true;
			}
		}
		...
		if (isSelf())
		{
				 ...
				 ++i)
			{
					...
					mAttachedObjectsVector[i].second->getValid())
				{
					...
					if (drawablep && drawablep->isState(LLDrawable::RIGGED))
					{
																  ...
																  LLVector4a* tangentp)
{
	...
	LLViewerObject* hitp = NULL;

	if (lineSegmentBoundingBox(start, end))
	{
		LLVector4a local_end = end;
		LLVector4a local_intersectionp;

		for (S32 i = 0, count = mAttachedObjectsVector.size(); i < count; ++i)
		{
			LLViewerObject* objp = mAttachedObjectsVector[i].first;
			if (objp &&
				objp->lineSegmentIntersect(start, local_end, face,
										   pick_transparent, pick_rigged,
										   face_hitp, &local_intersectionp,
										   tcoordp, normalp, tangentp))
			{
				local_end = local_intersectionp;
				if (intersectp)
				{
					*intersectp = local_intersectionp;
				}
				hitp = objp;
			}
		}
...
void LLVOAvatar::resetVisualParams()
{
	...
	for (LLAvatarXmlInfo::skeletal_distortion_info_list_t::iterator
		 ...
		 iter != end; ++iter)
	{
		LLPolySkeletalDistortionInfo* infop =
			(*iter)->asPolySkeletalDistortionInfo();
		if (!infop) continue;

		S32 id = infop->getID();
		LLPolySkeletalDistortion* paramp =
			getVisualParam(id)->asPolySkeletalDistortion();
		if (paramp)
		{
			*paramp = LLPolySkeletalDistortion(this);
			if (!paramp->setInfo(infop))
			{
	...
	for (LLAvatarXmlInfo::driver_info_list_t::iterator
		 ...
		 iter != end; ++iter)
	{
		LLDriverParamInfo* infop = *iter;
		if (!infop) continue;	// Paranoia

		LLVisualParam* vparamp = getVisualParam(infop->getID());
		if (!vparamp) continue;

		LLDriverParam* paramp = vparamp->asDriverParam();
		if (!paramp) continue;

		LLDriverParam::entry_list_t driven_list = paramp->getDrivenList();
		*paramp = LLDriverParam(this);
		if (paramp && paramp->setInfo(infop))
		{
			paramp->setDrivenList(driven_list);
		}
	}
}
...
void LLVOAvatar::resetSkeleton()
{
	...
	for (LLVisualParam* paramp = getFirstVisualParam(); paramp;
		 paramp = getNextVisualParam())
	{
		LLPolyMorphTarget* pmorph = paramp->asPolyMorphTarget();
		if (pmorph)
		{
...
void LLVOAvatar::releaseMeshData()
{
		 ...
		 iter != end; ++iter)
	{
		...
		{
			attachp->setAttachmentVisibility(false);
		}
	}
...
void LLVOAvatar::restoreMeshData()
{
		 ...
		 iter != end; ++iter)
	{
		...
		if (attachp && !attachp->getIsHUDAttachment())
		{
			attachp->setAttachmentVisibility(true);
		}
	}
	...
	for (S32 part_index = 0; part_index < num_parts; )
	{
			...
			buffp->getNumVerts() == num_verts)
		{
			for (S32 k = j; k < part_index; ++k)
			{
				...
				LLViewerJoint* meshp = getViewerJoint(k);
				if (meshp)
				{
					meshp->updateFaceData(facep, mAdjustedPixelArea,
										  k == MESH_ID_HAIR,
										  terse_update && !rigid);
				}
			}
...
LLViewerTexture* LLVOAvatar::getBakedTexture(U8 te)
{
	...
	if (!mIsEditingAppearance)
	{
		...
		LLViewerTexture* bakedtexp = getImage(i, 0);
		if (!bakedtexp)
		{
			return NULL;
		}

		return LLViewerTextureManager::staticCast(bakedtexp, true);
	}

	LLViewerTexLayerSet* layersetp = getTexLayerSet(te);
	if (!layersetp)
	{
		return NULL;
	}
	layersetp->createComposite();
	layersetp->setUpdatesEnabled(true);
	return layersetp->getViewerComposite();
}
...
void LLVOAvatar::idleUpdateMisc(bool detailed_update)
{
	...
	if (detailed_update && !impostor_not_needing_update)
	{
		...
		for (S32 i = 0, count = mAttachedObjectsVector.size(); i < count; ++i)
		{
			...
			if (!attachp || !attachp->getValid() || !attach_objp ||
				attach_objp->isDead() || !(drawablep = attach_objp->mDrawable))
			{
				continue;
			}
...
void LLVOAvatar::idleUpdateAppearanceAnimation()
{
	...
	if (appearance_anim_time >= APPEARANCE_MORPH_TIME)
	{
		...
		for (LLVisualParam* paramp = getFirstVisualParam(); paramp;
			 paramp = getNextVisualParam())
		{
			if (paramp->isTweakable())
			{
				paramp->stopAnimating(false);
			}
		}
		updateVisualParams();
		if (isSelf())
	...
	else
	{
		if (!isSelf())
		{
				 ...
				 paramp = getNextVisualParam())
			{
				if (paramp->isTweakable())
				{
					paramp->animate(morph_amt, false);
				}
			}
		}
...
void LLVOAvatar::idleUpdateLipSync()
{
	...
	if (sLipSyncEnabled && gVoiceClient.isVoiceWorking() &&
		gVoiceClient.getIsSpeaking(mID))
	{
...
void LLVOAvatar::idleUpdateLoadingEffect()
{
	...
	if (!mFullyLoaded && updateIsFullyLoaded()) // changed ?
	{
		if (isFullyLoaded())
		{
...
void LLVOAvatar::idleUpdateWindEffect()
{
	...
	F32 wind_freq = hover_strength + llclamp(8.f + speed * 0.7f +
...
bool LLVOAvatar::updateCharacter()
{
	...
	{
		static const char* fmt_string1 = "%s P%d %.2fs";
		static const char* fmt_string2 = "%s P%d";
		static const char* bento_idle = "bento_idle";
		std::string id_str;
		const char* name;
			 ...
			 iter != end; ++iter)
		{
			...
			if (motionp && motionp->getMinPixelArea() < getPixelArea())
			{
				...
				{
					const LLUUID& motion_id = motionp->getID();
					if (motion_id == ANIM_AGENT_BENTO_IDLE)
					{
						name = bento_idle;
					}
					else
					{
						id_str = motion_id.asString();
						name = id_str.c_str();
					}
				}
				else
				{
					name = motionp->getName().c_str();
				}
				U32 priority = (U32)motionp->getPriority();
				F32 duration = motionp->getDuration();
				if (duration > 0.01f)
				{
					addDebugText(llformat(fmt_string1, name, priority,
										  duration));
				}
				else
				{
					addDebugText(llformat(fmt_string2, name, priority));
				}
			}
		}
	}
	...
	else if (mDrawable.notNull())
	{
		...
		if (sDebugInvisible)
		{
				 ...
				 ++i)
			{
				...
				std::string name = LLTrans::getString(attachp->getName());

				LLViewerObject* objp = mAttachedObjectsVector[i].first;
				if (objp && objp->mDrawable && objp->mDrawable->isVisible())
				{
...
U32 LLVOAvatar::renderSkinned()
{
	...
	bool needs_rebuild = !facep || !facep->getVertexBuffer() ||
	...
	if (gViewerShaderMgrp->getShaderLevel(LLViewerShaderMgr::SHADER_AVATAR) <= 0)
	{
		if (mNeedsSkin)
		{
			...
			LLViewerJoint* jointmeshp = getViewerJoint(MESH_ID_LOWER_BODY);
			if (jointmeshp)
			{
				jointmeshp->updateJointGeometry();
			}
			jointmeshp = getViewerJoint(MESH_ID_UPPER_BODY);
			if (jointmeshp)
			{
				jointmeshp->updateJointGeometry();
			}
			jointmeshp = getViewerJoint(MESH_ID_SKIRT);
			if (jointmeshp && isWearingWearableType(LLWearableType::WT_SKIRT))
			{
				jointmeshp->updateJointGeometry();
			}
			if (!isSelf() || gAgent.needsRenderHead() ||
				LLPipeline::sShadowRender)
			{
				jointmeshp = getViewerJoint(MESH_ID_EYELASH);
				if (jointmeshp)
				{
					jointmeshp->updateJointGeometry();
				}
				jointmeshp = getViewerJoint(MESH_ID_HEAD);
				if (jointmeshp)
				{
					jointmeshp->updateJointGeometry();
				}
				jointmeshp = getViewerJoint(MESH_ID_HAIR);
				if (jointmeshp)
				{
					jointmeshp->updateJointGeometry();
				}
	...
	{
		if (mIsDummy && isTextureVisible(TEX_HAIR_BAKED))
		{
...
void LLVOAvatar::updateTextures()
{
	...
	for (U32 i = 0, count = mBakedTextureDatas.size(); i < count; ++i)
	{
		...
		if (baked && render_avatar && !LLGLManager::sIsDisabled &&
	...
	for (U32 tex_idx = 0, count = getNumTEs(); tex_idx < count;
		 ++tex_idx)
	{
		LLWearableType::EType wearable_type =
			LLAvatarAppearanceDictionary::getTEWearableType((ETextureIndex)tex_idx);
		U32 num_wearables = gAgentWearables.getWearableCount(wearable_type);
		const LLTextureEntry* te = getTE(tex_idx);
		...
		if (isSelf())	// There is no texture stats for non-self avatars
		{
			...
			{
				imagep =
					LLViewerTextureManager::staticCast(getImage(tex_idx,
																wearable_index),
													   true);
				if (!imagep) continue;

				const LLAvatarAppearanceDictionary::TextureEntry* t_dict =
					gAvatarAppDictp->getTexture((ETextureIndex)tex_idx);
				if (!t_dict) continue;
				// If the image for this texture layer is not fully loaded and
				// we are editing our appearance, we must flag the visual hints
				// for refresh. HB
				if (is_editing_appearance && !imagep->isFullyLoaded())
				{
					LLVisualParamHint::requestHintUpdates();
				}
				if (!t_dict->mIsLocalTexture) continue;

				const EBakedTextureIndex bt_idx = t_dict->mBakedTextureIndex;
				addLocalTextureStats((ETextureIndex)tex_idx, imagep,
									 texel_area_ratio, render_avatar,
									 layer_baked[bt_idx]);
			}
		}
		if (isIndexBakedTexture((ETextureIndex)tex_idx) && render_avatar)
		{
			imagep =
				LLViewerTextureManager::staticCast(getImage(tex_idx, 0),
												   true);
			// Spam if this is a baked texture, not set to default image, without
			// valid host info
			if (isIndexBakedTexture((ETextureIndex)tex_idx) &&
									 ...
									 LLVector3& out_norm)
{
	...
	LLViewerObject* objp;
	gWorld.resolveStepHeightGlobal(this, p0, p1, out_pos, out_norm, &objp);
	if (!objp)
	{
		mStepOnLand = true;
		mStepMaterial = 0;
...
void LLVOAvatar::processAnimationStateChanges()
{
		 ...
		 ++it)
	{
			...
			found_anim->second != it->second)
		{
			...
			{
				mPlayingAnimations[id] = it->second;
			}
		}
	}
...
U32 LLVOAvatar::initSkinningMatrixPalette(LLMatrix4a* matp,
										  const LLMeshSkinInfo* skinp)
{
	if (!matp || !skinp)
	{
		return 0;
...
#		pragma omp parallel for private(bind, world)
		...
		{
			LLJoint* jointp = getJoint(skinp->mJointKeys[j]);
			if (jointp)
			{
				bind.loadu(skinp->mInvBindMatrix[j]);
				world.loadu(jointp->getWorldMatrix());
				matp[j].matMul(bind, world);
			}
			else
			{
				...
				llwarns_once << "Rigged to invalid joint name: "
							 << skinp->mJointNames[j] << llendl;
				matp[j].loadu(skinp->mInvBindMatrix[j]);
			}
		}
	}
	else
...
LLVOAvatar::rtf_cache_it_t LLVOAvatar::initRiggedMatrixCache(const LLMeshSkinInfo* skinp,
															 U32& count)
{
	LL_TRACY_TIMER(TRC_RIGGED_MATRIX);

	const LLUUID& mesh_id = skinp->mMeshID;
	...
	count = rigmatp->mCount = initSkinningMatrixPalette(mat, skinp);
	F32* mp = rigmatp->mMatrix;
#ifdef __AVX2__
	// Kathrine Jansma's AVX2 optimized code
	// Offsets to copy
	U32 idx = 0;
...
const F32* LLVOAvatar::getRiggedMatrix(const LLMeshSkinInfo* skinp, U32& count)
{
	return initRiggedMatrixCache(skinp, count)->second->mMatrix;
}

const LLMatrix4a* LLVOAvatar::getRiggedMatrix4a(const LLMeshSkinInfo* skinp,
												U32& count)
{
	return initRiggedMatrixCache(skinp, count)->second->mMatrix4a;
}

// If viewer object is a rigged mesh, set the mesh Id and return true.
// Otherwise, null out the Id and return false.
//static
bool LLVOAvatar::getRiggedMeshID(LLViewerObject* objp, LLUUID& mesh_id)
{
	mesh_id.setNull();
	...
	{
		LLVOVolume* volp = objp->mDrawable->getVOVolume();
		if (volp)
		{
			const LLPointer<LLMeshSkinInfo> skindatap = volp->getSkinInfo();
			if (skindatap.notNull() &&
...
void LLVOAvatar::rebuildAttachmentOverrides()
{
	...
	for (S32 i = 0, count = mAttachedObjectsVector.size(); i < count; ++i)
	{
		...
		if (objp && !objp->isAnimatedObject())
		{
			addAttachmentOverridesForObject(objp);
		}
	}
}
...
void LLVOAvatar::updateAttachmentOverrides()
{
	...
	for (S32 i = 0, count = mAttachedObjectsVector.size(); i < count; ++i)
	{
		...
		if (objp && !objp->isAnimatedObject())
		{
			addAttachmentOverridesForObject(objp, &meshes_seen);
		}
	}
...
void LLVOAvatar::addAttachmentOverridesForObject(LLViewerObject* objp,
												 uuid_list_t* meshes_seen,
												 bool recursive)
{
	if (!objp) return;	// Paranoia

	LLVOAvatar* avp = objp->getAvatar();
	if (avp != this && objp->getAvatarAncestor() != this)
	{
	...
	if (recursive)
	{
		...
		const_child_list_t& children = objp->getChildren();
	...
	LLVOVolume* volp = objp->asVolume();
	if (!volp || !volp->getVolume() || !volp->isMesh() ||
		!volp->getVolume()->isMeshAssetLoaded())
	{
		return;
	}
	...
	if (mActiveOverrideMeshes.count(mesh_id))
	{
		LL_DEBUGS("Avatar") << "Skipping add attachment overrides (already loaded) for mesh "
							...
							<< volp->getRootEdit()->getID() << ", on avatar: "
							<< avname << LL_ENDL;
		return;
	}
...
void LLVOAvatar::removeAttachmentOverridesForObject(LLViewerObject* objp)
{
	LLVOAvatar* avp = objp ? objp->getAvatar() : NULL;
	if (avp != this)
	{
	...
	// Process all children
	LLViewerObject::const_child_list_t& children = objp->getChildren();
	...
	// Process self.
	LLUUID mesh_id;
	if (getRiggedMeshID(objp ,mesh_id))
	{
		removeAttachmentOverridesForObject(mesh_id);
	}
}
...
void LLVOAvatar::initAttachmentPoints(bool ignore_hud_joints)
{
	...
	for (LLAvatarXmlInfo::attachment_info_list_t::iterator
		 ...
		 iter != end; ++iter)
	{
		LLAvatarXmlInfo::LLAvatarAttachmentInfo* infop = *iter;
		if (infop->mIsHUDAttachment && (ignore_hud_joints || !isSelf()))
		{
		...
		S32 attachment_id = infop->mAttachmentID;
		if (attachment_id < 1 || attachment_id > 255)
		{
			llwarns << "Avatar: " << getFullname(true)
					...
					<< infop->mName << ", skipping." << llendl;
			continue;
		}

		LLJoint* parent_joint = getJoint(infop->mJointKey);
		if (!parent_joint)
		{
			...
			llwarns << "Avatar: " << getFullname(true)
					<< " - No parent joint by name " << infop->mJointName
					<< " found for attachment point " << infop->mName
		...
		attachment->setName(infop->mName);

		if (infop->mHasPosition)
		{
			attachment->setOriginalPosition(infop->mPosition);
			attachment->setDefaultPosition(infop->mPosition);
		}

		if (infop->mHasRotation)
		{
			LLQuaternion rotation;
			rotation.setEulerAngles(infop->mRotationEuler.mV[VX] * DEG_TO_RAD,
									infop->mRotationEuler.mV[VY] * DEG_TO_RAD,
									infop->mRotationEuler.mV[VZ] * DEG_TO_RAD);
			attachment->setRotation(rotation);
		}

		S32 group = infop->mGroup;
		if (group >= 0)
		{
			if (group < 0 || group >= 9)
			{
				llwarns << "Avatar: " << getFullname(true)
						...
						<< ") for attachment point " << infop->mName
						<< llendl;
				llassert(false);
			}
			else
		...
		attachment->setPieSlice(infop->mPieMenuSlice);
		attachment->setVisibleInFirstPerson(infop->mVisibleFirstPerson);
		attachment->setIsHUDAttachment(infop->mIsHUDAttachment);
...
bool LLVOAvatar::updateGeometry(LLDrawable* drawp)
{
	U32 type = isPuppetAvatar() ? LLPipeline::RENDER_TYPE_PUPPET
								: LLPipeline::RENDER_TYPE_AVATAR;
...
LLViewerJointAttachment* LLVOAvatar::getTargetAttachmentPoint(LLViewerObject* objp)
{
	S32 attachment_id = ATTACHMENT_ID_FROM_STATE(objp->getAttachmentState());
	...
	LLViewerJointAttachment* attachp = get_ptr_in_map(mAttachmentPoints,
													  attachment_id);
	if (!attachp)
	{
		llwarns_once << getFullname(true)
		...
		attachp = get_ptr_in_map(mAttachmentPoints, 1);
	}
...
//virtual
const LLViewerJointAttachment* LLVOAvatar::attachObject(LLViewerObject* objp)
{
	LLViewerJointAttachment* attachp = getTargetAttachmentPoint(objp);
	if (!attachp || !attachp->addObject(objp, isSelf()))
	{
		return NULL;
	}
...
void LLVOAvatar::lazyAttach()
{
	...
	{
		LLPointer<LLViewerObject> objp = mPendingAttachment[i];
		if (objp->isDead()) continue;

		const LLViewerJointAttachment* jointp = NULL;
		if (objp->mDrawable)
		{
			jointp = attachObject(objp);
		}
		if (jointp)
		{
			if (isSelf())
			{
				LL_DEBUGS("Attachment") << "Attaching object "
										<< objp->mID << " from "
										<< LLTrans::getString(jointp->getName())
										<< LL_ENDL;
				gAttachmentsListDirty = true;
				gAttachmentsTimer.reset();
			}
...
void LLVOAvatar::rebuildRiggedAttachments()
{
	...
	for (S32 i = 0, count = mAttachedObjectsVector.size(); i < count; ++i)
	{
		const LLViewerObject* objp = mAttachedObjectsVector[i].first;
		if (objp && objp->mDrawable.notNull())
		{
			gPipeline.markRebuild(objp->mDrawable);
		}
	}
}

#if 0	// Would it be at all needed ???
void LLVOAvatar::cleanupAttachedMesh(LLViewerObject* objp)
{
	LLUUID mesh_id;
	if (getRiggedMeshID(objp, mesh_id))
	{
...
//virtual
bool LLVOAvatar::detachObject(LLViewerObject* objp)
{
	...
	{
		LLViewerJointAttachment* attachp = iter->second;
		if (attachp && attachp->isObjectAttached(objp))
		{
			if (isSelf())
			{
				LL_DEBUGS("Attachment") << "Detaching object "
										<< objp->mID << " from "
										<< LLTrans::getString(attachp->getName())
										<< LL_ENDL;
			}
			bool animated = objp->isAnimatedObject();
#if 0
			cleanupAttachedMesh(objp);
#endif
			vector_replace_with_last(mAttachedObjectsVector,
									 std::make_pair(objp, attachp));
			attachp->removeObject(objp, isSelf());
			if (!animated)
			{
				updateAttachmentOverrides();
...
//static
LLVOAvatar* LLVOAvatar::findAvatarFromAttachment(LLViewerObject* objp)
{
	...
	else if (global_color == mTexHairColor)
	{
		...
		if (!isTextureDefined(mBakedTextureDatas[BAKED_HAIR].mTextureIndex))
		{
			...
			for (avatar_joint_mesh_list_t::iterator
				 ...
				 iter != end; ++iter)
			{
				LLAvatarJointMesh* meshp = *iter;
				if (meshp)
				{
					meshp->setColor(color);
				}
			}
		}
	}
...
// Called periodically to keep isFullyLoaded() up to date. Returns true if the
// value has changed.
bool LLVOAvatar::updateIsFullyLoaded()
{
	static LLCachedControl<bool> wait_attach(gSavedSettings,
											 "RenderAvatarWaitForAttachments");
	static LLCachedControl<bool> render_unloaded(gSavedSettings,
												 "RenderUnloadedAvatar");

	// First check for missing visual params or missing baked textures.
	bool loading = getIsCloud();

	// Then check for missing attachments, if configured to wait for them.
	bool must_wait_for_attachments = !loading && wait_attach && !isSelf();
	if (must_wait_for_attachments && !mPendingAttachment.empty())
	{
		loading = true;
	}
	else if (must_wait_for_attachments && mLastCloudAttachmentCount >= 0)
	{
		S32 old_count = mLastCloudAttachmentCount;
		S32 expected_count = mSimAttachments.size();
		mLastCloudAttachmentCount = mAttachedObjectsVector.size();
		// Are there still un-rezzed attachments ?
		if (mLastCloudAttachmentCount < expected_count)
		{
			if (mLastCloudAttachmentCount > old_count)
			{
				// We rezzed new attachments, but some are still misssing...
				mLastCloudAttachmentChangeTime.reset();
				loading = true;
			}
			else if (!render_unloaded ||
					 mLastCloudAttachmentChangeTime.getElapsedTimeF32() <
						DERUTHING_TIMEOUT_SECONDS)
			{
				// We do not allow rendering unloaded avatars, or the Ruth
				// timer delay has not yet elasped. HB
				loading = true;
			}
		}
	}

	// Request an appearance update should the Ruth timer expires.
...
{
 	if (!loading || mPreviousFullyLoaded || isSelf())
	{
		return;
	}
	...
	size_t current_count = mAttachedObjectsVector.size();
	size_t expected_count = mSimAttachments.size();

	llwarns_once << "Declouding timeout for: " << getFullname(true)
				 << " - Visual parameters loaded: "
				 << (visualParamWeightsAreDefault() ? "no" : "yes")
				 << " - Lower body baked: "
				 << (isTextureDefined(TEX_LOWER_BAKED) ? "yes" : "no")
				 << " - Upper body baked: "
				 << (isTextureDefined(TEX_UPPER_BAKED) ? "yes" : "no")
				 << " - Head baked: "
				 << (isTextureDefined(TEX_HEAD_BAKED) ? "yes" : "no")
				 << " - Number of rezzed attachments: " << current_count
				 << " - Number of expected attachments: " << expected_count
				 << llendl;
	...
	// When in SL with an avatar lacking attachments and the avatar is visible
	// on screen, set the flag for triggering a de-clouding workaround. HB
	if (current_count < expected_count && gIsInSecondLife &&
		mDrawable.notNull() && mDrawable->isVisible())
	{
		static LLCachedControl<U32> max_requests(gSavedSettings,
												 "AutoDecloudMaxRequests");
		// Only fire the workaround when this avatar has not yet reached its
		// maximum de-clouding attempts.
		if (!max_requests || mAttachmentsUpdateRequest < (U32)max_requests)
		{
			++mAttachmentsUpdateRequest;
			sNeedsAttachmentUpdate = true;
		}
	}

	LL_DEBUGS("Attachment") << "Attachments status for " << getFullname(true)
							<< ":";
	for (auto it = mSimAttachments.begin(), end = mSimAttachments.end();
		 it != end; ++it)
	{
		const LLUUID& id = it->first;
		const LLViewerObject* objp = gObjectList.findObject(id);
		if (!objp)
		{
			LL_CONT << "\n - Unrezzed attachment ";
		}
		else if (objp->getAvatarAncestor() != this)
		{
			LL_CONT << "\n - Unparented attachment ";
		}
		else
		{
			LL_CONT << "\n - Rezzed attachment ";
		}
		LL_CONT << id << " on point " << it->second;
	}
	LL_CONT << LL_ENDL;
}
...
bool LLVOAvatar::processFullyLoadedChange(bool loading)
{
	...
	// down
	mFullyLoaded = mFullyLoadedTimer.getElapsedTimeF32() >
					llmax(1.f, 5.f * gFrameIntervalSeconds);

	// Did our loading state "change" from last call ?
	constexpr U32 UPDATE_RATE = 30;
	...
	{
		mVisualComplexityStale = true;
		if (mAttachmentsUpdateRequest &&
			mAttachedObjectsVector.size() >= mSimAttachments.size())
		{
			mAttachmentsUpdateRequest = 0;
			llinfos << "All attachments finally received for: "
...
bool LLVOAvatar::isFullyLoaded(bool truly)
{
	if (mFullyLoaded)
	{
		return true;
	...
	// When using the auto-declouding workaround in SL, and when configured
	// to give up after AutoDecloudMaxRequests, allow rendering the incomplete
	// avatar nonetheless. HB
	...
	static LLCachedControl<U32> max_requests(gSavedSettings,
											 "AutoDecloudMaxRequests");
	if (decloud && render_stuck_avatar && max_requests && gIsInSecondLife &&
		mAttachmentsUpdateRequest >= (U32)max_requests)
	{
		if (mAttachmentsUpdateRequest == max_requests)
		{
			++mAttachmentsUpdateRequest;	// To avoid warning at each call
			llwarns << "Rendering " << getFullname(true)
					<< " with missing attachments: " << mSimAttachments.size()
					<< " expected and only " << mAttachedObjectsVector.size()
					<< " sent by the sim server." << llendl;
		}
		return true;
	}
	return false;
...
void LLVOAvatar::updateMeshTextures()
{
	...
	for (U32 i = 0; i < count; ++i)
	{
		...
		bool layerset_invalid =
			layersetp && (!layersetp->isLocalTextureDataAvailable() ||
						 !layersetp->getViewerComposite()->isInitialized());
	...
	LLViewerFetchedTexture* bakedtexp;
	for (U32 i = 0; i < count; ++i)
	{
		...
		else if (!local_appearance && is_layer_baked[i])
		{
			bakedtexp =
				LLViewerTextureManager::staticCast(getImage(mBakedTextureDatas[i].mTextureIndex,
												  			0),
												   true);
			if (bakedtexp->getID() == mBakedTextureDatas[i].mLastTextureID)
			{
				// Even though the file may not be finished loading, we will
				// consider it loaded and use it (rather than doing
				// compositing).
				useBakedTexture(bakedtexp->getID());
			}
			else
			{
				mBakedTextureDatas[i].mIsLoaded = false;
				if (bakedtexp->getID() != IMG_INVISIBLE &&
					(i == BAKED_HEAD || i == BAKED_UPPER || i == BAKED_LOWER))
				{
					bakedtexp->setLoadedCallback(onBakedTextureMasksLoaded,
												 MORPH_MASK_REQUESTED_DISCARD,
												 true, true,
												 new LLTextureMaskData(mID),
												 src_cb_list, paused);
				}
				bakedtexp->setLoadedCallback(onBakedTextureLoaded,
											 SWITCH_TO_BAKED_DISCARD,
											 false, false, new LLUUID(mID),
											 src_cb_list, paused);
			}
		}
		else if (layersetp && local_appearance)
		{
	...
	if (isSelf())
	{
			 ...
			 baked_iter != end; ++baked_iter)
		{
			...
			{
				const ETextureIndex tex_idx = *local_tex_iter;
				bool is_baked_ready =
					is_layer_baked[baked_idx] &&
					mBakedTextureDatas[baked_idx].mIsLoaded;
				setBakedReady(tex_idx, is_baked_ready);
			}
		}
	}
...
bool LLVOAvatar::hasHUDAttachment() const
{
		 ...
		 iter != end; ++iter)
	{
		...
		if (attachmentp && attachmentp->getIsHUDAttachment() &&
			attachmentp->getNumObjects() > 0)
		{
			return true;
		}
	}
...
LLBBox LLVOAvatar::getHUDBBox() const
{
	...
	for (S32 i = 0, count = mAttachedObjectsVector.size(); i < count; ++i)
	{
		...
		if (objp && objp->isHUDAttachment())
		{
			...
			bbox.addPointLocal(objp->getPosition());
			...
			if (layer_baked)
			{
				LLViewerFetchedTexture* texp =
					LLViewerTextureManager::staticCast(getImage(mBakedTextureDatas[i].mTextureIndex,
																0),
													   true);
				mBakedTextureDatas[i].mLastTextureID = texp->getID();
				// If we have more than one texture for the other baked layers,
				// we'll want to call this for them too.
				if (texp->getID() != IMG_INVISIBLE &&
					(i == BAKED_HEAD || i == BAKED_UPPER || i == BAKED_LOWER))
				{
					texp->setLoadedCallback(onBakedTextureMasksLoaded,
											MORPH_MASK_REQUESTED_DISCARD,
											true, true,
											new LLTextureMaskData(mID),
											src_cb_list, paused);
				}
				texp->setLoadedCallback(onInitialBakedTextureLoaded,
...
bool LLVOAvatar::visualParamWeightsAreDefault()
{
	     ...
	     paramp = getNextVisualParam())
	{
		if (paramp->isTweakable())
		{
			LLViewerVisualParam* vparamp = paramp->asViewerVisualParam();
			bool is_skirt_param =
				vparamp &&
				vparamp->getWearableType() == LLWearableType::WT_SKIRT;
		    // We have to ignore whether skirt weights are default, if we are
			// not actually wearing a skirt
			if ((wearing_skirt || !is_skirt_param) &&
				paramp->getWeight() != paramp->getDefaultWeight())
			{
				return false;
			}
		}
...
# endif
	...
	// Get attachment info when available.
	S32 attach_count = msg->getNumberOfBlocksFast(_PREHASH_AttachmentBlock);
	if (attach_count > 0)
	{
		size_t old_size = mSimAttachments.size();
		LLUUID attach_id;
		for (S32 i = 0; i < attach_count; ++i)
		{
			msg->getUUIDFast(_PREHASH_AttachmentBlock, _PREHASH_ID, attach_id,
							 i);
			U8 point;
			msg->getU8Fast(_PREHASH_AttachmentBlock, _PREHASH_AttachmentPoint,
						   point, i);
			if (attach_id.isNull())
			{
				LL_DEBUGS("Avatar") << getFullname(true)
									<< " has null attachment on point "
									<< (S32)point << LL_ENDL;
				continue;
			}
			mSimAttachments[attach_id] = point;
		}
		if (mSimAttachments.size() != old_size)
		{
	...
	if (num_blocks > 1)
	{
		...
		LLVisualParam* paramp = getFirstVisualParam();
		...
		else
		{
			for (S32 i = 0; i < num_blocks; ++i)
			{
				...
				while (paramp &&
					   paramp->getGroup() != VISUAL_PARAM_GROUP_TWEAKABLE)
				{
					paramp = getNextVisualParam();
				}
				...
				msg->getU8Fast(_PREHASH_VisualParam, _PREHASH_ParamValue,
							   value, i);
				F32 new_weight = U8_to_F32(value, paramp->getMinWeight(),
										  paramp->getMaxWeight());
				contents.mParamWeights.push_back(new_weight);
				contents.mParams.push_back(paramp);
...
void LLVOAvatar::processAvatarAppearance(LLMessageSystem* msg)
{
	...
	if (isSelf() && mIsEditingAppearance)
	{
	...
	if (num_params <= 1)
	{
		...
		llinfos_once << "Ignoring appearance message due to lack of parameters for: "
					 << getFullname(true) << llendl;
		return;
	}
	...
	{
		llwarns_once << "Bad appearance version info, discarding for: "
					 << getFullname(true) << llendl;
		return;
	}
	...
	if (isSelf())
	{
		...
		{
			llwarns << "Received AvatarAppearance message for self in non-server-bake region for: "
					<< getFullname(true) << llendl;
		}
			...
			this_update_cof_version < last_update_request_cof_version)
		{
			llwarns << "Stale appearance update, wanted version "
					...
					<< this_update_cof_version << " for: "
					<< getFullname(true) << ". Ignoring." << llendl;
			return;
		}
	}
											  ...
											  bool slam_params)
{
	...
	if (num_params > 1)
	{
		...
		{
			LLVisualParam* paramp = contents.mParams[i];
			if (!paramp) continue;	// Paranoia
			F32 new_weight = contents.mParamWeights[i];

			if (is_first_appearance_message ||
				paramp->getWeight() != new_weight)
			{
				params_changed = true;
				if (is_first_appearance_message || slam_params)
				{
					paramp->setWeight(new_weight, false);
				}
				else
				{
					interp_params = true;
					paramp->setAnimationTarget(new_weight, false);
				}
			}
		}
										   ...
										   void* userdatap)
{
	...
	LLTextureMaskData* mask_datap = (LLTextureMaskData*)userdatap;
	LLVOAvatar* avatarp = gObjectList.findAvatar(mask_datap->mAvatarID);

	// If discard level is 2 less than last discard level we processed, or we
	// hit 0, then generate morph masks
	if (avatarp && success &&
		(discard_level == 0 ||
		 discard_level < mask_datap->mLastDiscardLevel - 2))
	{
		if (auximgp && auximgp->getComponents() == 1)
		{
			if (!auximgp->getData())
			{
			...
			unitp->bindManual(LLTexUnit::TT_TEXTURE, gl_name);
			LLImageGL::setManualImage(GL_TEXTURE_2D, 0, GL_ALPHA8,
									  auximgp->getWidth(), auximgp->getHeight(),
				 ...
				 iter != end; ++iter)
			{
				...
				if (t_dict->mIsUsedByBakedTexture)
				{
					const ETextureIndex tex_idx = iter->first;
					const LLViewerTexture* bakedtexp =
						avatarp->getImage(tex_idx, 0);
					if (bakedtexp && id == bakedtexp->getID())
					{
						const EBakedTextureIndex baked_idx =
							t_dict->mBakedTextureIndex;
						avatarp->applyMorphMask(auximgp->getData(),
												auximgp->getWidth(),
												auximgp->getHeight(), 1,
												baked_idx);
						mask_datap->mLastDiscardLevel = discard_level;
						auto baked = avatarp->mBakedTextureDatas[baked_idx];
						if (baked.mMaskTexName)
						{
							LLImageGL::deleteTextures(1, &(baked.mMaskTexName));
						}
						baked.mMaskTexName = gl_name;
		...
		else
		{
			...
			// llinfos_once instead of a llwarns...
			llinfos_once << "No aux raw image source for baked mask of "
						 << avatarp->getFullname(true) << llendl;
		}
	}
...
void LLVOAvatar::onInitialBakedTextureLoaded(bool success,
											 ...
											 LLImageRaw*, LLImageRaw*, S32,
											 bool is_final, void* userdatap)
{
	LLUUID* avatar_idp = (LLUUID*)userdatap;
	LLVOAvatar* avatarp = gObjectList.findAvatar(*avatar_idp);
	if (!success && avatarp)
	{
		avatarp->removeMissingBakedTextures();
	}
	if (is_final || !success)
	{
		delete avatar_idp;
									  ...
									  bool is_final, void* userdatap)
{
	...
	if (is_final && avatarp && success)
	{
		avatarp->useBakedTexture(id);
	}
}
...
void LLVOAvatar::dumpArchetypeXML(const std::string& filename)
{
		 ...
		 ++type)
	{
		...
		for (LLVisualParam* paramp = getFirstVisualParam();
			 paramp; paramp = getNextVisualParam())
		{
			LLViewerVisualParam* viewer_param = (LLViewerVisualParam*)paramp;
...
void LLVOAvatar::cullAvatarsByPixelArea()
{
	...
	U32 arank = 1;	// Avatar rank 0 is reserved for self
	...
	for (S32 i = 0, count = LLCharacter::sInstances.size(); i < count; ++i)
	{
		...
		if (avatarp->mDrawable->isVisible())
		{
			...
			// Note: when not fully loaded, do not let the avatar be
			// impostored, so that the attachments get properly updated and
			// fetched. HB
			else if (avatarp->mFullyLoaded)
			{
				new_rank = arank++;
			}
			avatarp->mVisibilityRank = new_rank;
		}
	}
...
void LLVOAvatar::setIsUsingServerBakes(bool newval)
{
	...
	LLVisualParam* appearance_version_paramp = getVisualParam(11000);
	if (appearance_version_paramp)
	{
		appearance_version_paramp->setWeight(newval ? 1.f : 0.f, false);
	}
}
...
const LLHost& LLVOAvatar::getObjectHost() const
{
	...
	return regionp ? regionp->getHost() : LLHost::invalid;
}
...
{
	// Let's avoid constant reallocations on stack, and instead use a static
	// vector that will end up large enough to contain the most complex avatar
	// rendered in the session. HB
	...
	S32 max_lod;
	for (size_t i = 0, count = mAttachedObjectsVector.size(); i < count; ++i)
	{
		LLViewerObject* objp = mAttachedObjectsVector[i].first;
		if (!objp || objp->isHUDAttachment())
		{
		...
		if (volp->isMesh() && volp->getVolume())
		{
			...
			hash.update(mesh_id);
			max_lod = llmax(volp->getLOD(), volp->mLastRiggingInfoLOD);
			hash.update(max_lod);
		}
	...
	static LLUUID current_digest;
	hash.digest(current_digest);
	// Check for digest change, which indicates some change in volume
	// composition or LOD.
	if (current_digest != mLastRiggingInfoDigest)
	{
		mLastRiggingInfoDigest = current_digest;
...
{
	return mVisuallyMuteSetting != AV_ALWAYS_RENDER && useImpostors() &&
...
//static
void LLVOAvatar::loadingSettingsChanged()
{
	if (gSavedSettings.getBool("RenderUnloadedAvatar") &&
		!gSavedSettings.getBool("RenderAvatarWaitForAttachments"))
	{
		// Nothing to do: de-clouding of still clouded avatars will happen
		// automatically. HB
		return;
	}
	// Update clouding status based on the new criteria: will re-cloud all
	// avatars not matching these criteria. HB
	const LLViewerObjectList::avatars_map_t& avs = gObjectList.getAvatarMap();
	for (LLViewerObjectList::avatars_map_t::const_iterator it = avs.begin(),
														   end =avs.end();
		 it != end; ++it)
	{
		LLVOAvatar* avatarp = it->second.get();
		// Call updateIsFullyLoaded() regardless of mFullyLoaded current value,
		// so that the latter boolean gets updated in accordance with the new
		// criteria. HB
		avatarp->updateIsFullyLoaded();
	}
}
...
void LLVOAvatar::accountRenderComplexityForObject(LLViewerObject* objp,
												  U32& cost)
{
	if (!objp || objp->isHUDAttachment())
	{
	...
	const LLDrawable* drawp = objp->mDrawable;
	if (!drawp) return;

	const LLVOVolume* volume = drawp->getVOVolume();
	if (!volume) return;
	...
	LL_DEBUGS("ARCdetail") << getFullname(true) << ", costs for attachment: "
						   << objp->getAttachmentItemID() << " - total: "
...
void LLVOAvatar::calculateUpdateRenderComplexity()
{
	...
	{
		LLViewerObject* objp = mAttachedObjectsVector[i].first;
		if (!objp->isTempAttachment())
		{
			accountRenderComplexityForObject(objp, cost);
		}
	}
