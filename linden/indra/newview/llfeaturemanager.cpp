...
F32 LLFeatureManager::benchmarkGPU()
{
	...
	LLTexUnit* unit0p = gGL.getTexUnit(0);
	for (U32 i = 0; i < count; ++i)
	{
		...
		unit0p->setHasMipMaps(false);
		unit0p->setTextureFilteringOption(LLTexUnit::TFO_POINT);
	}
...
void LLFeatureManager::loadGPUClass(bool benchmark_gpu)
{
	...
	const std::string raw_renderer = LLGLManager::getRawGLString();
	...
	if (benchmark_gpu)
	{
		...
		if (gbps < 0.f)
		{
			...
			if (LLGLManager::sGLVersion <= 2.f)
			{
				mGPUClass = GPU_CLASS_0;
			}
			else if (LLGLManager::sGLVersion <= 3.f)
			{
				mGPUClass = GPU_CLASS_1;
			}
			else if (LLGLManager::sGLVersion < 3.3f)
			{
				mGPUClass = GPU_CLASS_2;
			}
			else if (LLGLManager::sGLVersion < 4.f)
			{
				mGPUClass = GPU_CLASS_3;
			}
			else if (LLGLManager::sGLVersion < 4.4f)
			{
			...
			if (LLGLManager::sIsIntel && mGPUClass > GPU_CLASS_1)
			{
		...
		else if (gbps < class0_gbps || LLGLManager::sGLVersion <= 2.f)
		{
			mGPUClass = GPU_CLASS_0;
		}
		else if (gbps < 2.f * class0_gbps || LLGLManager::sGLVersion <= 3.f)
		{
			mGPUClass = GPU_CLASS_1;
		}
		else if (gbps < 4.f * class0_gbps || LLGLManager::sGLVersion < 3.3f)
		{
			mGPUClass = GPU_CLASS_2;
		}
		else if (gbps < 8.f * class0_gbps || LLGLManager::sGLVersion < 4.f)
		{
			mGPUClass = GPU_CLASS_3;
		}
		else if (gbps < 16.f * class0_gbps || LLGLManager::sGLVersion < 4.4f)
		{
			mGPUClass = GPU_CLASS_4;
		}
		else
	...
	for (U32 line = 0; !found && !file.eof(); ++line)
	{
		...
		if (label.empty() || expr.empty() || cls.empty())
		{
	...
	if (raw_renderer != last_seen)
	{
		...
		if (LLGLManager::sIsNVIDIA && LLGLManager::sGLVersion >= 3.f &&
