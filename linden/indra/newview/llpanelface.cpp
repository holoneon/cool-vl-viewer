 ...
 *
 * Copyright (c) 2001-2009, Linden Research, Inc, (c) 2009-2024 Henri Beauchamp
 *
...
:	LLPanel(name),
	mHasMatNormal(false),
	mHasMatSpecular(false),
	mHasMatPBR(false),
	mHasMultiplePbrSelection(false),
	mIsAlpha(false)
{
	sInstance = this;
}
...
bool LLPanelFace::postBuild()
{
	...
	mButtonAlignMaps = getChild<LLButton>("btn_align_maps");
	mButtonAlignMaps->setClickedCallback(onAlignTextureLayers, this);
	...
	mPbrThumbBorder = getChild<LLView>("pbr_mat_border");
	mThumbnailRect = mPbrThumbBorder->getRect();
	...
	mLabelPbrPreview = getChild<LLTextBox>("pbr_mat_text");

	LLUUID tex_id = LLUUID(gSavedSettings.getString("DefaultObjectTexture"));
	if (tex_id.isNull())
	{
		tex_id = IMG_PLYWOOD;	// Use this, if setting is not a valid UUID.
	}
	mTextureCtrl = getChild<LLTextureCtrl>("texture control");
	mTextureCtrl->setDefaultImageAssetID(tex_id);
	...
	tex_id = LLUUID(gSavedSettings.getString("DefaultNormalTexture"));
	mNormalCtrl->setDefaultImageAssetID(tex_id);
	tex_id = LLUUID(gSavedSettings.getString("BlankNormalTexture"));
	mNormalCtrl->setBlankImageAssetID(tex_id);
...
// Special map layer value to align all maps with the parameters from the
// diffuse map of the center face. HB
constexpr S32 PROPAGATE_FROM_CENTER_DIFFUSE = -2;
...
struct LLPanelFaceSetAlignedTEFunctor final : public LLSelectedTEFunctor
{
					 ...
					 const LLUUID& only_for_obj_id = LLUUID::null)
	{
		...
		{
			LLSelectedTEEditMaterial(LLPanelFace* panelp,
	...
	LL_INLINE static void setNormalOffsetX(LLPanelFace* panelp, F32 data,
										   S32 te = -1,
										   const LLUUID& obj_id = LLUUID::null)
	{
		edit<&LLMaterial::setNormalOffsetX>(panelp, data, te, obj_id);
	}

	LL_INLINE static void setNormalOffsetY(LLPanelFace* panelp, F32 data,
										   S32 te = -1,
										   const LLUUID& obj_id = LLUUID::null)
	{
		edit<&LLMaterial::setNormalOffsetY>(panelp, data, te, obj_id);
	}

	LL_INLINE static void setNormalRepeatX(LLPanelFace* panelp, F32 data,
										   S32 te = -1,
										   const LLUUID& obj_id = LLUUID::null)
	{
		edit<&LLMaterial::setNormalRepeatX>(panelp, data, te, obj_id);
	}

	LL_INLINE static void setNormalRepeatY(LLPanelFace* panelp, F32 data,
										   S32 te = -1,
										   const LLUUID& obj_id = LLUUID::null)
	{
		edit<&LLMaterial::setNormalRepeatY>(panelp, data, te, obj_id);
	}

	LL_INLINE static void setSpecularOffsetX(LLPanelFace* panelp, F32 data,
											 S32 te = -1,
											 const LLUUID& obj_id = LLUUID::null)
	{
		edit<&LLMaterial::setSpecularOffsetX>(panelp, data, te, obj_id);
	}

	LL_INLINE static void setSpecularOffsetY(LLPanelFace* panelp, F32 data,
											 S32 te = -1,
											 const LLUUID& obj_id = LLUUID::null)
	{
		edit<&LLMaterial::setSpecularOffsetY>(panelp, data, te, obj_id);
	}

	LL_INLINE static void setSpecularRepeatX(LLPanelFace* panelp, F32 data,
											 S32 te = -1,
											 const LLUUID& obj_id = LLUUID::null)
	{
		edit<&LLMaterial::setSpecularRepeatX>(panelp, data, te, obj_id);
	}

	LL_INLINE static void setSpecularRepeatY(LLPanelFace* panelp, F32 data,
											 S32 te = -1,
											 const LLUUID& obj_id = LLUUID::null)
	{
		edit<&LLMaterial::setSpecularRepeatY>(panelp, data, te, obj_id);
	}

	LL_INLINE static void setNormalRotation(LLPanelFace* panelp, F32 data,
											S32 te = -1,
											const LLUUID& obj_id = LLUUID::null)
	{
		edit<&LLMaterial::setNormalRotation>(panelp, data, te, obj_id);
	}

	LL_INLINE static void setSpecularRotation(LLPanelFace* panelp, F32 data,
											  S32 te = -1,
											  const LLUUID& obj_id = LLUUID::null)
	{
		edit<&LLMaterial::setSpecularRotation>(panelp, data, te, obj_id);
	}
	...
	bool apply(LLViewerObject* objectp, S32 te) override
	{
		...
		// Special case: we apply the offsets, scales and rotation of the
		// center face diffuse map to all maps of all selected faces. HB
		if (mMap == PROPAGATE_FROM_CENTER_DIFFUSE)
		{
			const LLTextureEntry* reftep = mCenterFace->getTextureEntry();
			if (!reftep)	// Paranoia
			{
				return true;
			}

			F32 uv_rot = reftep->getRotation();
			F32 offset_x = reftep->getOffsetS();
			F32 offset_y = reftep->getOffsetT();
			F32 scale_x = reftep->getScaleS();
			F32 scale_y = reftep->getScaleT();

			// Apply to normal and specular material maps, if any. HB
			...
			if (tep && tep->getMaterialParams().notNull())
			{
				...
				setNormalRepeatX(mPanel, scale_x, te, obj_id);
				setNormalRepeatY(mPanel, scale_y, te, obj_id);
				...
				setSpecularRepeatX(mPanel, scale_x, te, obj_id);
				setSpecularRepeatY(mPanel, scale_y, te, obj_id);
			}

			// Apply to diffuse map, if this is not the center face. HB
			if (facep != mCenterFace)
			{
				objectp->setTEOffset(te, offset_x, offset_y);
				objectp->setTERotation(te, uv_rot);
				objectp->setTEScale(te, scale_x, scale_y);
			}
		...
		if (set_aligned)
		{
			...
			if (set_aligned)
			{
				...
				if (mMap < 0 || mMap == LLRender::NORMAL_MAP)
				{
					setNormalOffsetX(mPanel, offset_x, te, obj_id);
					setNormalOffsetY(mPanel, offset_y, te, obj_id);
					setNormalRotation(mPanel, uv_rot, te, obj_id);
				}
				if (mMap < 0 || mMap == LLRender::SPECULAR_MAP)
				{
					setSpecularOffsetX(mPanel, offset_x, te, obj_id);
					setSpecularOffsetY(mPanel, offset_y, te, obj_id);
					setSpecularRotation(mPanel, uv_rot, te, obj_id);
				}
				if (mMap < 0 || mMap == LLRender::DIFFUSE_MAP)
				{
					objectp->setTEOffset(te, offset_x, offset_y);
			...
			// Align any PBR material, using PBR rotation, scales and offsets.
			if ((mMap < 0 || mMap == LLRender::BASECOLOR_MAP) &&
				facep->calcAlignedPlanarTE(mCenterFace, &uv_offset, &uv_scale,
										   &uv_rot, LLRender::BASECOLOR_MAP))
			{
				LLGLTFMaterial omat;
				const LLTextureEntry* tep = objectp->getTE(te);
				if (tep && tep->getGLTFMaterialOverride())
				{
					omat = *tep->getGLTFMaterialOverride();
				}
				// Update all texture transforms for the glTF material
				bool any_changed = false;
				for (U32 i = 0; i < LLGLTFMaterial::GLTF_TEXTURE_INFO_COUNT;
					 ++i)
				{
					if (facep->calcAlignedPlanarTE(mCenterFace, &uv_offset,
												   &uv_scale, &uv_rot, i))
					{
						any_changed = true;
						LLGLTFMaterial::TextureTransform& tt =
							omat.mTextureTransform[i];
						tt.mOffset.set(uv_offset.mV[0], uv_offset.mV[1]);
						tt.mScale.set(uv_scale.mV[0], uv_scale.mV[1]);
						tt.mRotation = uv_rot;
					}
				}
				if (any_changed)
				{
					LLGLTFMaterialList::queueModify(objectp, te, &omat);
				}
			}
		}
...
// This is for now only used to draw the material preview thumbnail, when
// needed and possible. HB
//virtual
void LLPanelFace::draw()
{
	// Let's avoid calling setText(getString(...)) at every frame.
	enum
	{
		NEEDS_PROBES = -3,
		NEEDS_PBR_MODE = -2,
	...
	static S32 pbr_preview_label = UNSET_PBR_LABEL;

	// Draw all UI elements before we would draw the texture.
	LLPanel::draw();

	if (!mPbrThumbBorder->getVisible())
	{
		// Preview thumbnail square hidden, so nothing to draw.
		return;
	}

	if (!mPbrThumbBorder->getEnabled())	// Object not editable
	{
		if (pbr_preview_label != HAS_PBR_MAT)
		{
			pbr_preview_label = HAS_PBR_MAT;
			mLabelPbrPreview->setText(getString("base_material_text"));
		}
	}
	else if (!gUsePBRShaders)
	{
		if (pbr_preview_label != NEEDS_PBR_MODE)
		{
			pbr_preview_label = NEEDS_PBR_MODE;
			mLabelPbrPreview->setText(getString("needs_pbr_text"));
		}
	}
	// For now, material preview does not render at all when reflection probes
	// are disabled. *TODO: find out why and fix it.
	else if (!LLPipeline::sReflectionProbesEnabled)
	{
		if (pbr_preview_label != NEEDS_PROBES)
		{
			pbr_preview_label = NEEDS_PROBES;
			mLabelPbrPreview->setText(getString("needs_probes_text"));
		}
	}
	else if (mGLTFMaterial.isNull())
	{
		if (mHasMultiplePbrSelection)
		{
			if (pbr_preview_label != MULTIPLE_PBR_MAT)
			{
				pbr_preview_label = MULTIPLE_PBR_MAT;
				mLabelPbrPreview->setText(getString("multiple_mat_text"));
			}
		}
		else if (pbr_preview_label != NO_PBR_MAT)
		{
			pbr_preview_label = NO_PBR_MAT;
			mLabelPbrPreview->setText(getString("no_mat_text"));
		}
	}
	else if (mMatTexture.notNull())
	{
		if (pbr_preview_label != HAS_PBR_MAT)
		{
			pbr_preview_label = HAS_PBR_MAT;
		...
		gl_draw_scaled_image(mThumbnailRect.mLeft, mThumbnailRect.mBottom,
							 mThumbnailRect.getWidth(),
							 mThumbnailRect.getHeight(), mMatTexture);
		return;	// It has been drawn now.
	}
	else	// Material data/textures still loading
	{
		if (pbr_preview_label != LOADING_PBR_MAT)
		{
			pbr_preview_label = LOADING_PBR_MAT;
			mLabelPbrPreview->setText(getString("loading_mat_text"));
		}
		// Try and see if it is ready now...
		mMatTexture = LLGLTFPreviewTexture::getPreview(mGLTFMaterial);
	}

	// Not in PBR mode, or no reflection probes, or material preview not ready,
	// or material not unique: draw a grey square.
...
{
	static LLUUID last_selected_object_id;
	static LLUUID last_pbr_id;

	mHasMatNormal = mHasMatSpecular = mHasMatPBR = false;
		...
		(objectp->permModify() || gAgent.isGodlikeWithoutAdminMenuFakery()))
	{
		...
		mHasMatPBR = gAgent.hasInventoryMaterial();
		mButtonLocalPBR->setEnabled(editable && mHasMatPBR);
		mButtonLoadPBR->setEnabled(editable && mHasMatPBR);
		...
		if (mHasMatPBR)
		{
			struct pbr_id_get final : public LLSelectedTEGetFunctor<LLUUID>
			{
				...
				{
					const LLUUID& mat_id = objectp->getRenderMaterialID(face);
					if (mat_id.notNull())
					{
						has_mat = true;
					}
			...
			mHasMatPBR = pbr_id.notNull();
			mHasMultiplePbrSelection = func.has_mat && !identical;
			if (mHasMatPBR && identical)
			{
				mGLTFMaterial = gGLTFMaterialList.getMaterial(pbr_id);
				mMatTexture = LLGLTFPreviewTexture::getPreview(mGLTFMaterial);
			}
		}
		if (!mHasMatPBR || !identical)
		{
			mMatTexture = NULL;
			mGLTFMaterial = NULL;
			LLPreviewMaterial::closeLiveEditorInstance();
		}
		mLabelPbrPreview->setEnabled(editable);
		mButtonEditPBR->setEnabled(editable && mHasMatPBR && identical &&
								   LLPreviewMaterial::canModifyObjectsMaterial());
		mButtonSavePBR->setEnabled(editable && mHasMatPBR && identical &&
		...
		if (transparency > 0.f || mHasMatPBR)
		{
		...
		mComboAlphaMode->setEnabled(!mHasMatPBR);
		updateAlphaControls();
		...
		mHasMatNormal = normmap_id.notNull();
		...
		mHasMatSpecular = specmap_id.notNull();
		...
		mButtonAlignMaps->setEnabled(editable &&
									 (mHasMatNormal || mHasMatSpecular ||
									  selection->getTECount() > 1));
		...
		mBumpyScaleU->setEnabled(editable && mHasMatNormal);
		mCheckBumpyFlipS->setValue(LLSD(scale_s < 0));
		mCheckBumpyFlipS->setTentative(!identical);
		mCheckBumpyFlipS->setEnabled(editable && mHasMatNormal);
		...
		mBumpyScaleV->setEnabled(editable && mHasMatNormal);
		mCheckBumpyFlipT->setValue(LLSD(scale_t < 0));
		mCheckBumpyFlipT->setTentative(!identical);
		mCheckBumpyFlipT->setEnabled(editable && mHasMatNormal);
		...
		mBumpyOffsetU->setEnabled(editable && mHasMatNormal);
		...
		mBumpyOffsetV->setEnabled(editable && mHasMatNormal);
		...
		mBumpyRot->setEnabled(editable && mHasMatNormal);
		...
		mShinyScaleU->setEnabled(editable && mHasMatSpecular);
		mCheckShinyFlipS->setValue(LLSD(scale_s < 0));
		mCheckShinyFlipS->setTentative(!identical);
		mCheckShinyFlipS->setEnabled(editable && mHasMatSpecular);
		...
		mShinyScaleV->setEnabled(editable && mHasMatSpecular);
		mCheckShinyFlipT->setValue(LLSD(scale_t < 0));
		mCheckShinyFlipT->setTentative(!identical);
		mCheckShinyFlipT->setEnabled(editable && mHasMatSpecular);
		...
		mShinyOffsetU->setEnabled(editable && mHasMatSpecular);
		...
		mShinyOffsetV->setEnabled(editable && mHasMatSpecular);
		...
		mShinyRot->setEnabled(editable && mHasMatSpecular);
		...
		mPbrScaleU->setEnabled(editable && mHasMatPBR);
		...
		mPbrScaleV->setEnabled(editable && mHasMatPBR);
		...
		mPbrOffsetU->setEnabled(editable && mHasMatPBR);
		...
		mPbrOffsetV->setEnabled(editable && mHasMatPBR);
		...
		mPbrRot->setEnabled(editable && mHasMatPBR);
		...
		switch (map)
		{
			case MATTYPE_SPECULAR:
			{
				...
				identical = selection->getSelectedTEValue(&func, repeats,
														  0.001f);
				enabled = shiny == SHINY_TEXTURE && mHasMatSpecular;
				break;
			}
			...
			case MATTYPE_NORMAL:
			{
				...
				identical = selection->getSelectedTEValue(&func, repeats,
														  0.001f);
				enabled = bump == BUMPY_TEXTURE && mHasMatNormal;
				break;
			}
		...
		mLabelRepeats->setEnabled(enabled);

		if (mHasMatPBR || mIsAlpha || mHasMatNormal || mHasMatSpecular)
		{
...
void LLPanelFace::sendTextureInfo()
{
	...
	{
		LLFace* last_facep = getLastSelectedFace();
		LLPanelFaceSetAlignedTEFunctor setfunc(this, last_facep);
...
void LLPanelFace::onSelectMapType(LLUICtrl* ctrl, void* userdata)
{
	...
	self->mPbrThumbBorder->setVisible(show_pbr);
	self->mLabelPbrPreview->setVisible(show_pbr);
	...
	// Hide incompatible or unrelated controls. HB
	self->mLabelDiffuseColor->setVisible(!show_pbr);
	self->mColorSwatch->setVisible(!show_pbr);
	self->mTransparency->setVisible(!show_pbr);
	self->mLabelColorTransp->setVisible(!show_pbr);
	self->mGlow->setVisible(!show_pbr);
	self->mCheckFullbright->setVisible(!show_pbr);
	self->mButtonAlignMaps->setVisible(!show_pbr);
	self->mCheckPlanarAlign->setVisible(!show_pbr);
	self->mLabelRepeats->setVisible(!show_pbr);
	self->mRepeats->setVisible(!show_pbr);
		...
		bool apply(LLViewerObject* objectp, S32 face) override
		{
			...
			for (U32 i = mStart; i < mEnd; ++i)
			{
				...
				switch (mParam)
				{
					...
					case PBR_ROT:
						new_tt.mRotation = mValue * DEG_TO_RAD;
...
void LLPanelFace::onCommitRepeatsPerMeter(LLUICtrl* ctrl, void* userdata)
{
	...
	struct f_objscale_s final : public LLSelectedTEGetFunctor<F32>
	{
		F32 get(LLViewerObject* objectp, S32 face) override
		{
			...
			U32 t_axis = VY;
			LLPrimitive::getTESTAxes(face, &s_axis, &t_axis);
			return objectp->getScale().mV[s_axis];
		}
	} scale_s_func;
	...
	struct f_objscale_t final : public LLSelectedTEGetFunctor<F32>
	{
		F32 get(LLViewerObject* objectp, S32 face) override
		{
			...
			U32 t_axis = VY;
			LLPrimitive::getTESTAxes(face, &s_axis, &t_axis);
			return objectp->getScale().mV[t_axis];
		}
	} scale_t_func;
...
void LLPanelFace::onAlignTextureLayers(void* userdata)
{
	...
	{
		LLObjectSelectionHandle selection = gSelectMgr.getSelection();
		LLFace* facep = self->getLastSelectedFace();
		LLPanelFaceSetAlignedTEFunctor setfunc(self, facep,
											   PROPAGATE_FROM_CENTER_DIFFUSE);
		selection->applyToTEs(&setfunc);
