...
public:
	...
	bool handle(const LLSD& tokens, const LLSD&, LLMediaCtrl*) override
	{
		...
		const std::string action = tokens[0].asString();
		if (action == "create")
		{
		...
		const std::string verb = tokens[1].asString();
...
{
	LLFloaterGroupInfo* self = get_ptr_in_map(sInstances, group_id);
	if (self)
	{
		if (self->mPanelGroupp)
		{
			self->mPanelGroupp->close();
		}
	}
}
...
void LLFloaterGroupInfo::callbackLoadGroupName(const LLUUID& id,
											   ...
											   bool is_group)
{
	LLFloaterGroupInfo* self = get_ptr_in_map(sInstances, id);
	if (self)
	{
									  ...
									  const std::string& tab_name)
{
	...
	if (!self)
	{
		self = new LLFloaterGroupInfo("groupinfo", "FloaterGroupInfoRect",
									  FLOATER_TITLE, group_id, tab_name);
		sInstances[group_id] = self;
									...
									LLOfferInfo* inventory_offerp)
{
	if (group_id.isNull())
	{
		...
		if (inventory_offerp)
		{
			inventory_offerp->forceResponse(IOR_DECLINE);
		}
		return;
	}
	...
	self->mPanelGroupp->showNotice(subject, message, has_inventory,
								   inventory_name, inventory_offerp);
}
