...
// Exported constants
const std::string SCREEN_HOME_FILENAME = "screen_home.png";
const std::string SCREEN_LAST_FILENAME = "screen_last.png";
const std::string SCREEN_LAST_BETA_FILENAME = "screen_last-beta.png";
...
static void process_messages(bool state_agent_wait = false)
{
	constexpr S32 MAX_STARTUP_FRAME_MESSAGES = 100;
	constexpr U64 MAX_STARTUP_FRAME_TIME = 2000;	// usec
	U64 t0 = LLTimer::totalTime();
...
#endif
	{
...
#if 0	// This should not be needed any more... HB
		if (state_agent_wait)
		{
			if (gAgentMovementCompleted)
			{
				// Sometimes we have more than one message in the queue. Break
				// out of this loop and continue processing. If we do not, then
				// this could skip one or more login steps.
				break;
			}
			else
			{
		...
		if (++num_messages >= MAX_STARTUP_FRAME_MESSAGES ||
			LLTimer::totalTime() - t0 > MAX_STARTUP_FRAME_TIME)
		{
			needs_drain = true;
			break;
		}
	}
	if (needs_drain || msg->mPacketRing.getNumBufferedPackets() > 0)
	{
		msg->drainUdpSocket();
	}
#if LL_USE_FIBER_AWARE_MUTEX
...
bool LLStartUp::idleStartup()
{
	...
	if (getStartupState() == STATE_FIRST)
	{
		...
		if (found_template)
		{
			...
			U32 packet_ring_max_size =
				gSavedSettings.getU32("PacketRingMaxBufferSize");
			if (!start_messaging_system(msg_template_path, port,
										packet_ring_max_size,
		...
		if (msg && msg->isOK())
		{
			...
			if (xfer_throttle_bps >= 131072.f)	// 16KBps
			{
				gXferManagerp->setUseAckThrottling(true);
				gXferManagerp->setAckThrottleBPS(xfer_throttle_bps);
			}
#if 0		// Do not throttle at all if we do not want it !  HB
			else
			{
...
//mk
		...
		switch (start_slurl.getType())
		{
			case LLSLURL::LOCATION:
			{
				...
				start = llformat("uri:%s&%d&%d&%d",
								 start_slurl.getRegion().c_str(),
								 (S32)position.mV[VX], (S32)position.mV[VY],
								 (S32)position.mV[VZ]);
				break;
			}
		...
		gUserAuth.authenticate(grid_uri, auth_method, gLoginFirstName,
							   gLoginLastName, password, start, true,
							   gAcceptTOS, gAcceptCriticalMessage,
		...
		// Allow large texture bakes, on a per grid basis.
		LLTexLayerSetInfo::sUse2KBakes =
			gSavedPerAccountSettings.getBool("Use2KAvatarLocalBakes");
		gPipeline.allocateBakeBuffer();
	...
	if (getStartupState() == STATE_LOGIN_PROCESS_RESPONSE)
	{
		...
		if (successful_login)
		{
			...
			{
				// Kick up a parcel change since the one which occurred at
				// login was done too soon and got ignored. HB
				gVoiceClient.onParcelChange();
	...
	if (getStartupState() == STATE_WORLD_INIT)
	{
		...
		expcache->setCapabilityQuery(std::bind(&LLAgent::getRegionCapability,
											   &gAgent, _1));
	...
	if (getStartupState() == STATE_MULTIMEDIA_INIT)
	{
		...
		// Cleanup remnants of old CEF instances private caches. HB
	...
	if (getStartupState() == STATE_AGENT_SEND)
	{
		...
		llinfos << "Registering AvatarTracker callbacks" << llendl;
		gAvatarTracker.registerCallbacks(gMessageSystemp);
	...
	if (getStartupState() == STATE_INVENTORY_SEND)
	{
		...
		// Set up callbacks
		llinfos << "Registering Inventory callbacks" << llendl;
		LLInventoryModel::registerCallbacks(gMessageSystemp);
		llinfos << "Registering Landmark callbacks" << llendl;
		LLLandmark::registerCallbacks(gMessageSystemp);
...
//mk
		...
		// If needed (and not already done) let the user know about the new
		// skin overrides directory. HB
		if (gDirUtil.hasLegacySkinOverrides())
		{
			LLFirstUse::newSkinOverridesFolder();
		}
...
bool LLStartUp::loginShow(bool update_servers)
{
	...
	strings_set_t listed_name;	// Only the 'other' grids.
...
void LLStartUp::initStartScreen(S32 location_id)
{
	...
	S8 codec = IMG_CODEC_PNG;
	if (!LLFile::exists(temp_str))
	{
		// Try the old BMP version of the screenshot. HB
		temp_str = temp_str.substr(0, temp_str.size() - 3) + "bmp";
		codec = IMG_CODEC_BMP;
	}
	LLPointer<LLImageFormatted> imgp = LLImageFormatted::createFromType(codec);
	if (!imgp->load(temp_str))
	{
		return;
	}
	llinfos << "Loaded image: " << temp_str << llendl;

	gStartImageWidth = imgp->getWidth();
	gStartImageHeight = imgp->getHeight();

	LLPointer<LLImageRaw> rawp = new LLImageRaw;
	if (!imgp->decode(rawp))
	{
		llwarns << "Image decode failed for: " << temp_str << llendl;
		gStartTexture = NULL;
		return;
	}

	rawp->expandToPowerOfTwo();
	gStartTexture = LLViewerTextureManager::getLocalTexture(rawp.get(), false);
}
