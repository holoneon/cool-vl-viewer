...
public:
	...
	void update(const LLMeshSkinInfo* skinp, LLVOAvatar* avatarp,
				const LLVolume* src_volp, S32 face_index = UPDATE_ALL_FACES,
...
public:
	...
	typedef fast_hmap<LLUUID, U32> texture_cost_t;
	...
	// Call with recreate = true to force a rebuild of the reflection probe. HB
	void updateReflectionProbePtr(bool recreate = false);
	...
	// Used in LLViewerFetchedTexture::postCreateTexture().
	LL_INLINE void setSculptLODChanged()				{ mSculptChanged = true; }
	...
	const LLPointer<LLMeshSkinInfo> getSkinInfo() const;
	...
	// Used by the mesh repository to mark volumes as using its caches. HB
	LL_INLINE void setInMeshCache()						{ mInMeshCache = true; }
	LL_INLINE void setInSkinCache()						{ mInSkinCache = true; }

	// Used when refreshing an avatar, to force-retry failed skin info
	// fetches. HB
	LL_INLINE void resetSkinInfoFailed()				{ mSkinInfoFailed = false; }
...
public:
	...
	static uuid_list_t								sObjectMediaPending;
