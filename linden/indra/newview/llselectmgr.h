...
// This is used by the DeRezObject message to determine where to put derezed
// objects.
enum EDeRezDestination
{
	...
	DRD_RETURN_TO_LAST_OWNER = 10,			// Deeded object back to last
											// owner's inventory
	DRD_COUNT = 11
};
...
// Do something to all objects in the selection manager. The bool return value
// can be used to indicate if all objects are identical (gathering information)
...
// Do something to all select nodes in the selection manager. The bool return
// value can be used to indicate if all objects are identical (gathering
// information) or if the operation was successful.
struct LLSelectedNodeFunctor
{
	virtual ~LLSelectedNodeFunctor() = default;
	virtual bool apply(LLSelectNode* nodep) = 0;
};
...
// Contains information about a selected object, particularly which TEs are
// selected.
class LLSelectNode
{
protected:
...
public:
	...
	S32				mSelectedGLTFNode;
	S32				mSelectedGLTFPrim;
...
public:
	...
	struct is_non_null
	{
		bool operator()(LLSelectNode* nodep)
		{
			return nodep->getObject() != NULL;
		}
	};
	...
	struct is_valid
	{
		bool operator()(LLSelectNode* nodep)
		{
			return nodep->getObject() && nodep->mValid;
		}
	};
...
public:
	...
	U32 getSelectedObjectRenderCost();
...
public:
	...
	void addAsIndividual(LLViewerObject* object, S32 te = SELECT_ALL_TES,
						 bool undoable = true, S32 gltf_node = -1,
						 S32 gltf_prim = -1);
...
private:
	...
	void sendListToRegions(const char* message_name,
	...
	static void packAgentAndSessionID(void* datap);
	static void packAgentAndGroupID(void* datap);
	static void packAgentAndSessionAndGroupID(void* datap);
	static void packAgentIDAndSessionAndAttachment(void*);
	static void packAgentGroupAndCatID(void*);
	static void packDeleteHeader(void* datap);
	static void packDeRezHeader(void* datap);
	static void packObjectID(LLSelectNode* nodep, void*);
	static void packObjectIDAsParam(LLSelectNode* nodep, void*);
	static void packObjectIDAndRotation(LLSelectNode* nodep, void*);
	static void packObjectLocalID(LLSelectNode* nodep, void*);
	static void packObjectClickAction(LLSelectNode* nodep, void*);
	static void packObjectIncludeInSearch(LLSelectNode* nodep, void*);
	static void packObjectName(LLSelectNode* nodep, void* datap);
	static void packObjectDescription(LLSelectNode* nodep, void* datap);
	static void packObjectCategory(LLSelectNode* nodep, void* datap);
	static void packObjectSaleInfo(LLSelectNode* nodep, void* datap);
	static void packBuyObjectIDs(LLSelectNode* nodep, void* datap);
	static void packDuplicate(LLSelectNode* nodep, void*);
	static void packDuplicateHeader(void*);
	static void packDuplicateOnRayHead(void* datap);
	static void packPermissions(LLSelectNode* nodep, void* datap);
	static void packDeselect(LLSelectNode* nodep, void* datap);
	static void packMultipleUpdate(LLSelectNode* nodep, void* datap);
	static void packOwnerHead(void* datap);
	static void packPermissionsHead(void* datap);
	static void packGodlikeHead(void* datap);
