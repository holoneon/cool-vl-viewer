...
private:
	...
	typedef std::map<std::string, LLWLSkyKey, std::less<> > skykey_map_t;
...
void LLPanelWLDayCycle::refreshNames(std::string new_selection)
{
	...
	for (strings_set_t::iterator it = LLWLDayCycle::sPresetNames.begin(),
...
private:
	...
	static strings_set_t	sDefaultPresets;
};
...
//statis
strings_set_t LLPanelWLSky::sDefaultPresets;
...
//static
strings_set_t LLPanelWLWater::sDefaultPresets;
										 ...
										 const LLSD& response)
{
	...
	strings_set_t::iterator it = sDefaultPresets.find(name);
	if (it != sDefaultPresets.end())
	{
		gNotifications.add("PresetNoEditDefault");
		return false;
