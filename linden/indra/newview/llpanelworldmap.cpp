...
void LLPanelWorldMap::draw()
{
	...
	static S32 font_height = S32(fontp->getLineHeight() + 0.99f);
		 ...
		 it != end; ++it)
	{
		...
		{
			mesg = info->mName + " (" + mesg + ")";
		}

		static LLCachedControl<bool> show_coords(gSavedSettings,
												 "MapShowGridCoords");
		if (!mesg.empty())
		{
			S32 text_bottom = llfloor(bottom + 2);
			if (show_coords)
			{
				text_bottom += font_height;
			}
			fontp->renderUTF8(mesg, 0, llfloor(left + 3), text_bottom,
				...
				simtexp->getDiscardLevel() != 0)
			{
				...
				fontp->render(loading, 0, llfloor(left + 18),
		...
		// Add the sim grid coordinates below the sim name if requested.
		if (show_coords)
		{
			S32 grid_x = region_pos.mdV[VX] / REGION_WIDTH_UNITS;
			S32 grid_y = region_pos.mdV[VY] / REGION_WIDTH_UNITS;
			mesg = llformat("%d, %d", grid_x, grid_y);
			fontp->renderUTF8(mesg, 0, llfloor(left + 3), llfloor(bottom + 2),
							  LLColor4::white, LLFontGL::LEFT,
							  LLFontGL::BASELINE, LLFontGL::DROP_SHADOW_SOFT);
		}
	}
...
{
	static LLFontGL* fontp = LLFontGL::getFontSansSerifSmall();
	static F32 font_height = fontp->getLineHeight();
	...
	S32 half_text_width = llfloor(fontp->getWidthF32(label) * 0.5f);
	...
	fontp->renderUTF8(label, 0, text_x, text_y, LLColor4::white,
	...
	fontp->renderUTF8(tooltip, 0, text_x, text_y - (S32)font_height,
								   ...
								   const std::string& second_line)
{
	...
	fontp->renderUTF8(first_line, 0, text_x, text_y, color, LLFontGL::HCENTER,
					  LLFontGL::TOP, LLFontGL::DROP_SHADOW_SOFT);

	text_y -= ll_roundp(font_height);

	// Render second line of text
	fontp->renderUTF8(second_line, 0, text_x, text_y, color, LLFontGL::HCENTER,
					  LLFontGL::TOP, LLFontGL::DROP_SHADOW_SOFT);
}
