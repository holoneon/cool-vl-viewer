	...
	if (hasMouseCapture())
	{
			 ...
			 iter != end; ++iter)
		{
			...
			if (object->permMove() && !object->isPermanentEnforced() &&
				...
				(object->isRootEdit() || nodep->mIndividualSelection))
			{
...
void LLManipRotate::drag(S32 x, S32 y)
{
		 ...
		 iter != end; ++iter)
	{
			...
			(object->isRootEdit() || nodep->mIndividualSelection))
		{
			if (nodep->mSelectedGLTFNode != -1)
			{
				...
				gltf_mode = true;
				LLQuaternion new_rot = nodep->mSavedRotation * mRotation;
				object->setGLTFNodeRotationAgent(nodep->mSelectedGLTFNode,
												 new_rot);
			}
			if (gltf_mode)
			{
	...
	if (!gltf_mode)
	{
			 ...
			 iter != end; ++iter)
		{
			...
			// To avoid cumulative position changes we calculate the objects
			// new position using its saved position
			if (object->permMove() && !object->isPermanentEnforced() &&
				(!root_object || !root_object->isPermanentEnforced()))
			{
				LLVector3 center =
					gAgent.getPosAgentFromGlobal(mRotationCenter);
				...
				{
					// Need to work in drawable space to handle selected items
					// from multiple attachments (which have no shared frame of
					// reference other than their render positions).
					LLXform* parent_xform = object->mDrawable->getXform()->getParent();
					new_position = nodep->mSavedPositionLocal *
				...
				{
					new_position =
						gAgent.getPosAgentFromGlobal(nodep->mSavedPositionGlobal);
				...
				if (object->isRootEdit() && !object->isAttachment())
				{
					LLVector3d new_pos_global =
						gAgent.getPosGlobalFromAgent(new_position);
					new_pos_global =
						gWorld.clipToVisibleRegions(nodep->mSavedPositionGlobal,
													new_pos_global);
					new_position =
				...
				if (!object->isRootEdit() && nodep->mIndividualSelection)
				{
					...
					if (!parentp->isSelected())
					{
						...
						{
							// Find position relative to render position of
							// parent
						...
						{
							object->setPositionParent((new_position -
				...
				// For individually selected roots, we need to counter-
				//translate all unselected children
				if (object->isRootEdit() && nodep->mIndividualSelection)
				{
		 ...
		 iter != end; ++iter)
	{
		...
		if (!object->isAvatar() && object->permModify() &&
			...
			(!root_object || !root_object->isPermanentEnforced()))
		{
			nodep->mLastRotation = object->getRotation();
...
void LLManipRotate::renderSnapGuides()
{
	...
	if (mObjectSelection->getSelectType() != SELECT_TYPE_HUD)
	{
			...
			sNumTimesHelpTextShown < sMaxTimesShowHelpText)
		{
			...
			hud_render_text(text1, help_text_pos, big_fontp, NULL,
			...
			hud_render_text(text2, help_text_pos, big_fontp, NULL,
