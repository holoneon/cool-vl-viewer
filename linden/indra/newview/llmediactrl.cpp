...
#include "llviewermenu.h"		// show_viewer_paths_floater(), gMenuHolderp
	...
	mHidingInitialLoad(true)
{
	...
	(new LLMenuCut())->registerListener(this, "MediaCtrl.Cut");
	(new LLMenuCanCut())->registerListener(this, "MediaCtrl.CanCut");
	(new LLMenuCopy())->registerListener(this, "MediaCtrl.Copy");
	(new LLMenuCanCopy())->registerListener(this, "MediaCtrl.CanCopy");
	(new LLMenuPaste())->registerListener(this, "MediaCtrl.Paste");
	(new LLMenuCanPaste())->registerListener(this, "MediaCtrl.CanPaste");
	(new LLMenuUndo())->registerListener(this, "MediaCtrl.Undo");
	(new LLMenuCanUndo())->registerListener(this, "MediaCtrl.CanUndo");
	(new LLMenuRedo())->registerListener(this, "MediaCtrl.Redo");
	(new LLMenuCanRedo())->registerListener(this, "MediaCtrl.CanRedo");
	(new LLMenuDelete())->registerListener(this, "MediaCtrl.Delete");
	(new LLMenuCanDelete())->registerListener(this, "MediaCtrl.CanDelete");
	(new LLMenuSelectAll())->registerListener(this, "MediaCtrl.SelectAll");
	(new LLMenuCanSelectAll())->registerListener(this,
												 "MediaCtrl.CanSelectAll");
	(new LLMenuShowSource())->registerListener(this, "MediaCtrl.ShowSource");
	(new LLMenuCanShowSource())->registerListener(this,
												  "MediaCtrl.CanShowSource");
	(new LLMenuReloadPage())->registerListener(this, "MediaCtrl.ReloadPage");

	LLMenuGL* menup =
		LLUICtrlFactory::getInstance()->buildMenu("menu_media_ctrl.xml", this);
	if (menup)
	{
...
bool LLMediaCtrl::handleRightMouseDown(S32 x, S32 y, MASK mask)
{
	...
	if (menup)
	{
		// Parent the menu to 'this' so that we can use mPtr->findControl()
		// in the LLMenuCan*::handleEvent() methods: for some reason, using
		// gMenuHolderp->findControl() does not work (and would be slower,
		// anyway, for many other menu control names exist in gMenuHolderp). HB
		menup->updateParent(this);
		// Now draw the labels (at which point LLMenuCan*::handleEvent()
		// methods get called). HB
		menup->buildDrawLabels();
		// Parent the menu to gMenuHolderp, so that it pops up at the right
		// position on screen and properly captures mouse clicks). HB
		menup->updateParent(gMenuHolderp);
		LLMenuGL::showPopup(this, menup, x0, y0);
	}
...
void LLMediaCtrl::onVisibilityChange(bool new_visibility)
{
	...
	mFrequentUpdates = new_visibility;
...
void LLMediaCtrl::navigateTo(const std::string& url_in,
									  ...
									  const std::string& filename_in)
{
	...
	std::string expanded_filename = gDirUtil.findSkinnedFilename("html",
																 language,
																 filename);
	bool found = LLFile::exists(expanded_filename);
	if (!found && language != "en-us")
	{
		expanded_filename = gDirUtil.findSkinnedFilename("html", "en-us",
														 filename);
	...
	// those start with '/')... Let's be more explicit !  HB
	expanded_filename = "file:///" + expanded_filename;
#else
	// CEF does not accept file names without a "file://" prefix. HB
	expanded_filename = "file://" + expanded_filename;
#endif
...
void LLMediaCtrl::draw()
{
	...
	const LLUICtrl* ctrlp = findRootMostFocusRoot();
	setFrequentUpdates(ctrlp && ctrlp->hasFocus());
...
{
	LLPluginClassMedia* pluginp = mMediaSource ? mMediaSource->getMediaPlugin()
											   : NULL;
	bool flipped = pluginp && pluginp->getTextureCoordsOpenGL();
								  ...
								  const std::string& cookie)
{
	...
	for (instances_list_t::iterator it = sMediaCtrlInstances.begin(),
		 ...
		 it != end; ++it)
	{
		LLMediaCtrl* ctrlp = *it;
		if (!ctrlp) continue;	// Paranoia

		LLPluginClassMedia* pluginp = ctrlp->getMediaPlugin();
		if (pluginp)
		{
			pluginp->setCookie(url, cookie_name, cookie_value, cookie_host,
...
//virtual
bool LLMediaCtrl::LLMenuCut::handleEvent(LLPointer<LLEvent>, const LLSD&)
{
	LLViewerMediaImpl* mediap = mPtr->getMediaSource();
	if (mediap)
	{
		mediap->cut();
	}
...
//virtual
bool LLMediaCtrl::LLMenuCanCut::handleEvent(LLPointer<LLEvent>,
											const LLSD& userdata)
{
	...
	{
		LLViewerMediaImpl* mediap = mPtr->getMediaSource();
		ctrlp->setValue(LLSD::Boolean(mediap && mediap->canCut()));
	}
	return true;
}

//virtual
bool LLMediaCtrl::LLMenuCopy::handleEvent(LLPointer<LLEvent>, const LLSD&)
{
	LLViewerMediaImpl* mediap = mPtr->getMediaSource();
	if (mediap)
	{
...
//virtual
bool LLMediaCtrl::LLMenuCanCopy::handleEvent(LLPointer<LLEvent>,
											 const LLSD& userdata)
{
	LLControlVariable* ctrlp =
		mPtr->findControl(userdata["control"].asString());
	...
	{
		LLViewerMediaImpl* mediap = mPtr->getMediaSource();
		ctrlp->setValue(LLSD::Boolean(mediap && mediap->canCopy()));
	}
	return true;
}

//virtual
bool LLMediaCtrl::LLMenuPaste::handleEvent(LLPointer<LLEvent>, const LLSD&)
{
	LLViewerMediaImpl* mediap = mPtr->getMediaSource();
	if (mediap)
	{
...
//virtual
bool LLMediaCtrl::LLMenuCanPaste::handleEvent(LLPointer<LLEvent>,
											  const LLSD& userdata)
{
	LLControlVariable* ctrlp =
		mPtr->findControl(userdata["control"].asString());
	...
	{
		LLViewerMediaImpl* mediap = mPtr->getMediaSource();
		ctrlp->setValue(LLSD::Boolean(mediap && mediap->canPaste()));
	}
	return true;
}

//virtual
bool LLMediaCtrl::LLMenuUndo::handleEvent(LLPointer<LLEvent>, const LLSD&)
{
	LLViewerMediaImpl* mediap = mPtr->getMediaSource();
	if (mediap)
	{
...
//virtual
bool LLMediaCtrl::LLMenuCanUndo::handleEvent(LLPointer<LLEvent>,
											 const LLSD& userdata)
{
	LLControlVariable* ctrlp =
		mPtr->findControl(userdata["control"].asString());
	...
	{
		LLViewerMediaImpl* mediap = mPtr->getMediaSource();
		ctrlp->setValue(LLSD::Boolean(mediap && mediap->canUndo()));
	}
	return true;
}

//virtual
bool LLMediaCtrl::LLMenuRedo::handleEvent(LLPointer<LLEvent>, const LLSD&)
{
	LLViewerMediaImpl* mediap = mPtr->getMediaSource();
	if (mediap)
	{
...
//virtual
bool LLMediaCtrl::LLMenuCanRedo::handleEvent(LLPointer<LLEvent>,
											 const LLSD& userdata)
{
	LLControlVariable* ctrlp =
		mPtr->findControl(userdata["control"].asString());
	...
	{
		LLViewerMediaImpl* mediap = mPtr->getMediaSource();
		ctrlp->setValue(LLSD::Boolean(mediap && mediap->canRedo()));
	}
	return true;
}

//virtual
bool LLMediaCtrl::LLMenuDelete::handleEvent(LLPointer<LLEvent>, const LLSD&)
{
	LLViewerMediaImpl* mediap = mPtr->getMediaSource();
	if (mediap)
	{
...
//virtual
bool LLMediaCtrl::LLMenuCanDelete::handleEvent(LLPointer<LLEvent>,
											   const LLSD& userdata)
{
	LLControlVariable* ctrlp =
		mPtr->findControl(userdata["control"].asString());
	...
	{
		LLViewerMediaImpl* mediap = mPtr->getMediaSource();
		ctrlp->setValue(LLSD::Boolean(mediap && mediap->canDelete()));
	}
	return true;
}

//virtual
bool LLMediaCtrl::LLMenuSelectAll::handleEvent(LLPointer<LLEvent>, const LLSD&)
{
	LLViewerMediaImpl* mediap = mPtr->getMediaSource();
	if (mediap)
	{
...
//virtual
bool LLMediaCtrl::LLMenuCanSelectAll::handleEvent(LLPointer<LLEvent>,
												  const LLSD& userdata)
{
	LLControlVariable* ctrlp =
		mPtr->findControl(userdata["control"].asString());
	...
	{
		LLViewerMediaImpl* mediap = mPtr->getMediaSource();
		ctrlp->setValue(LLSD::Boolean(mediap && mediap->canSelectAll()));
	}
	return true;
}

//virtual
bool LLMediaCtrl::LLMenuShowSource::handleEvent(LLPointer<LLEvent>,
												const LLSD&)
{
	LLPluginClassMedia* pluginp = mPtr->getMediaPlugin();
	if (pluginp)
	{
		pluginp->showSource();
	}
...
//virtual
bool LLMediaCtrl::LLMenuCanShowSource::handleEvent(LLPointer<LLEvent>,
												   const LLSD& userdata)
{
	LLControlVariable* ctrlp =
		mPtr->findControl(userdata["control"].asString());
	...
	{
		LLPluginClassMedia* pluginp = mPtr->getMediaPlugin();
		ctrlp->setValue(LLSD::Boolean(pluginp && pluginp->canShowSource()));
	}
	return true;
}

//virtual
bool LLMediaCtrl::LLMenuReloadPage::handleEvent(LLPointer<LLEvent>,
												const LLSD&)
{
	LLPluginClassMedia* pluginp = mPtr->getMediaPlugin();
	if (pluginp)
	{
		pluginp->browse_reload(true);	// true = ignore cache
	}
	return true;
}
...
public:
	...
	bool handle(const LLSD& params, const LLSD&, LLMediaCtrl* web) override
	{
		...
		std::string param0 = params[0].asString();

		// Support for opening the "viewer paths" floater from the about
		// floater (but won't work, on purpose, from a web browser). HB
		if (!web && param0 == "viewer_paths" && params[1].asString() == "open")
		{
			show_viewer_paths_floater(NULL);
			return true;
		}
		...
		if (floaterp && params[1].asString() == "close")
		{
			floaterp->close();
			return true;
		}
