...
void LLPanelDirFind::search(const std::string& search_text)
{
	...
	U32 classified_flags = pack_classified_flags_request(filter_auto_renew,
														 inc_pg, inc_mature,
														 inc_adult);
	msg->newMessage(_PREHASH_DirClassifiedQuery);
	...
	msg->addUUID(_PREHASH_QueryID, mSearchID);
	msg->addString(_PREHASH_QueryText, search_text);
	msg->addU32(_PREHASH_QueryFlags, classified_flags);
	msg->addU32(_PREHASH_Category, 0);	// All categories
	msg->addS32(_PREHASH_QueryStart, 0);
	...
	msg->newMessage(_PREHASH_DirPlacesQuery);
	msg->nextBlock(_PREHASH_AgentData);
	msg->addUUID(_PREHASH_AgentID, gAgentID);
	msg->addUUID(_PREHASH_SessionID, gAgentSessionID);
	msg->nextBlock(_PREHASH_QueryData);
	...
	// Always get the first 100 when using find ALL
	msg->addS32(_PREHASH_QueryStart, 0);
	msg->addS8(_PREHASH_Category, LLParcel::C_ANY);
	msg->addString(_PREHASH_SimName, NULL);
	gAgent.sendReliableMessage();
