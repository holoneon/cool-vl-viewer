...
// Local prototypes
void commit_select_tool(LLUICtrl*, void* datap);
void select_next_part(void*);
void select_previous_part(void*);
void click_popup_grab_drag(LLUICtrl*, void*);
void click_popup_grab_lift(LLUICtrl*, void*);
void click_popup_grab_spin(LLUICtrl*, void*);
void click_popup_dozer_mode(LLUICtrl*, void* datap);
...
//static
void* LLFloaterTools::createPanelPermissions(void* datap)
{
	LLFloaterTools* self = (LLFloaterTools*)datap;
	self->mPanelPermissions = new LLPanelPermissions("General");
	return self->mPanelPermissions;
}

//static
void* LLFloaterTools::createPanelObject(void* datap)
{
	LLFloaterTools* self = (LLFloaterTools*)datap;
	self->mPanelObject = new LLPanelObject("Object");
	return self->mPanelObject;
}

//static
void* LLFloaterTools::createPanelVolume(void* datap)
{
	LLFloaterTools* self = (LLFloaterTools*)datap;
	self->mPanelVolume = new LLPanelVolume("Features");
	return self->mPanelVolume;
}

//static
void* LLFloaterTools::createPanelFace(void* datap)
{
	LLFloaterTools* self = (LLFloaterTools*)datap;
	self->mPanelFace = new LLPanelFace("Texture");
	return self->mPanelFace;
}

//static
void* LLFloaterTools::createPanelContents(void* datap)
{
	LLFloaterTools* self = (LLFloaterTools*)datap;
	self->mPanelContents = new LLPanelContents("Contents");
	return self->mPanelContents;
}

//static
void* LLFloaterTools::createPanelContentsInventory(void* datap)
{
	LLFloaterTools* self = (LLFloaterTools*)datap;
	self->mPanelContents->mPanelInventory =
		new LLPanelInventory("ContentsInventory", LLRect());
	return self->mPanelContents->mPanelInventory;
}

//static
void* LLFloaterTools::createPanelLandInfo(void* datap)
{
	LLFloaterTools* self = (LLFloaterTools*)datap;
	self->mPanelLandInfo = new LLPanelLandInfo("land info panel");
	return self->mPanelLandInfo;
}
...
bool LLFloaterTools::postBuild()
{
	...
	mSliderZoom = getChild<LLSliderCtrl>("slider zoom");
...
{
	strings_map_t::iterator iter = mStatusText.find(text);
...
void LLFloaterTools::refresh()
{
	...
	S32 cost = (S32)(selection->getSelectedObjectCost() + 0.5f);
...
void commit_select_tool(LLUICtrl*, void* datap)
{
	...
	LLTool* toolp = (LLTool*)datap;
	gFloaterToolsp->setEditTool(toolp);
...
//static
void LLFloaterTools::setObjectType(void* datap)
{
	LLPCode pcode = *(LLPCode*)datap;
...
//static
void LLFloaterTools::onClickGridOptions(void* datap)
{
	LLFloaterBuildOptions::toggleInstance();

	// Makes grid options dependent on build tools window
	LLFloaterTools* self = (LLFloaterTools*)datap;
	LLFloaterBuildOptions* floaterp = LLFloaterBuildOptions::findInstance();
	if (self && floaterp)
	{
		self->addDependentFloater(floaterp);
	}
}

//static
void LLFloaterTools::onClickLink(void*)
{
	gSelectMgr.linkObjects();
}

//static
void LLFloaterTools::onClickUnlink(void*)
{
	gSelectMgr.unlinkObjects();
}

void LLFloaterTools::setEditTool(void* datap)
{
	gToolMgr.getCurrentToolset()->selectTool((LLTool*)datap);
}
...
void LLFloaterTools::updateTreeGrassCombo(bool visible)
{
	if (visible)
	{
		...
		species_list_t::iterator it, end;
		std::string selected;
		if (pcode == LLToolPlacerPanel::sTree)
		{
...
//static
void LLFloaterTools::onClickBtnAddMedia(void* datap)
{
	LLFloaterTools* self = (LLFloaterTools*)datap;
	if (self)
	{
...
// face.
void LLFloaterTools::onClickBtnEditMedia(void* datap)
{
	LLFloaterTools* self = (LLFloaterTools*)datap;
	if (self)
	{
...
//static
void LLFloaterTools::onClickBtnDeleteMedia(void*)
{
...
void LLFloaterTools::updateMediaSettings()
{
    ...
    // Height in pixels (if not auto scaled)
