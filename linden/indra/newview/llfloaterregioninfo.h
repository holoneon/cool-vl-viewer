...
{
public:
	LLPBRTerrainFeatures() = delete;
	~LLPBRTerrainFeatures() = delete;

	typedef void (*done_cb_t)(const LLUUID& region_id, bool success,
							  const LLTerrain* terrainp);

	static void queueQuery(LLViewerRegion* regionp, done_cb_t cb);
	static void queueModify(LLViewerRegion* regionp,
							const LLTerrain* terrainp);
private:
	static void queryRegionCoro(const std::string& cap_url, LLUUID region_id,
								done_cb_t cb);
	static void modifyRegionCoro(const std::string& cap_url, LLUUID region_id,
								 const LLSD& updates);
};
...
public:
	...
	virtual bool refreshFromRegion(LLViewerRegion* regionp);
...
protected:
	...
	static void onChangeAnything(LLUICtrl*, void* user_data);
...
public:
	bool refreshFromRegion(LLViewerRegion* regionp) override;
...
private:
	...
	static void onChangeCombat(LLUICtrl*, void* user_data);
};
...
private:
	...
	void clearMaterialPreview(const LLUUID& mat_id);

	bool validateTextureSizes() const;
	bool validateMaterials() const;

	static void onTerrainTypeCommit(LLUICtrl*, void* userdata);
	static void onMatPreviewClicked(void* userdata);
	static void onSelectInventoryMat(const std::vector<std::string>&,
									 const uuid_vec_t& ids, void* userdata,
									 bool);

private:
	LLRadioGroup*	mTerrainTypeCtrl;
	// Note: 4 = LLTerrain::ASSET_COUNT (avoids #including llvlcomposition.h)
	LLTextureCtrl*	mTexPickers[4];
	LLSpinCtrl*		mMaterialScaleUCtrl[4];
	LLSpinCtrl*		mMaterialScaleVCtrl[4];
	LLSpinCtrl*		mMaterialOffsetUCtrl[4];
	LLSpinCtrl*		mMaterialOffsetVCtrl[4];
	LLSpinCtrl*		mMaterialRotationCtrl[4];
	LLView*			mMatBorders[4];
	LLTextBox*		mMatClickTexts[4];
	LLRect			mThumbnailRect[4];
	LLUUID			mMatAssetIds[4];
	...
	std::string		mStatusStrings[STATUS_COUNT];

	bool			mHasTerrainTransforms;
};
...
private:
	...
	static const std::string& regionCapabilityQuery(LLViewerRegion* regionp,
													const char* cap);

	void setupList(LLPanelExperienceListEditor* panelp,
...
protected:
	...
	void refreshFromRegion(LLViewerRegion* regionp);
