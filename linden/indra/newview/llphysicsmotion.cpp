...
typedef std::map<std::string, F32, std::less<> > default_controller_map_t;
...
public:
	...
	*/
	LLPhysicsMotion(const std::string& param_driver_name, U32 joint_key,
					...
					const strings_map_t& controllers)
...
protected:
	F32 getParamValue(eParamName param)
	{
		...
		{
			const strings_map_t::const_iterator& entry =
...
private:
	...
	const strings_map_t mParamControllers;
...
bool LLPhysicsMotion::onUpdate(F32 time)
{
	...
	for (U32 i = 0; i < steps; ++i)
	{
		...
		if (llisnan(mPosition_local) || llisnan(mVelocity_local) ||
		    llisnan(new_pos_local))
		{
