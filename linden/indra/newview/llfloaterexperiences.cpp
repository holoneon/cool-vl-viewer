...
void LLFloaterExperiences::refreshContents()
{
	...
	if (!url2.empty())
	{
		...
		retrieveExperienceList(url2, handle, name_map,
							   ...
							   std::bind(&LLFloaterExperiences::checkPurchaseInfo,
										 this, _1, _2));
	}
}
...
void LLFloaterExperiences::onOpen()
{
	...
	regionp->setCapsReceivedCB(std::bind(&LLFloaterExperiences::refreshContents,
										 this));
}
...
void LLPanelExperiences::addExperience(const LLUUID& id)
{
	if (!mExperiencesList->getItem(id))
	{
		...
		exp->get(id,
				 std::bind(&LLPanelExperiences::cacheCallback,
													  ...
													  invokationFn_t invoker)
{
	...
	LLPanelExperiences* tab;
	for (strings_map_t::iterator it = tab_map.begin(), end = tab_map.end();
		 it != end; ++it)
	{
