...
public:
	...
	void done() override
	{
		...
		doOnIdleOneTime(std::bind(&LLDiscardAgentOffer::oneShotIdleCallback,
								  this));
		gInventory.removeObserver(this);
	}
										 ...
										 const LLSD& response)
{
	...
	LLInventoryObserver* openerp = NULL;
		...
		case IOR_ACCEPT:
			...
			switch (mIM)
			{
				case IM_INVENTORY_OFFERED:
				{
					...
					{
						LLOpenAgentOffer* open_agent_offerp =
							new LLOpenAgentOffer(from_string);
						open_agent_offerp->fetchItems(items);
						openerp = open_agent_offerp;
					}
					break;
				}
...
//mk
			...
			if (mIM == IM_INVENTORY_OFFERED)
			{
				...
				items.emplace_back(mObjectID);
				LLDiscardAgentOffer* discard_agent_offerp =
					new LLDiscardAgentOffer(mFolderID, mObjectID);
				discard_agent_offerp->fetch(folders, items);
				if ((catp && gInventory.isCategoryComplete(mObjectID)) ||
					(itemp && itemp->isFinished()))
				{
					discard_agent_offerp->done();
				}
				else
				{
					openerp = discard_agent_offerp;
				}
			}
...
//mk
	...
	if (openerp)
	{
		gInventory.addObserver(openerp);
	}
...
void LLOfferInfo::inventoryOfferHandler()
{
	...
	p.substitutions(args).payload(payload).functor(std::bind(&LLOfferInfo::inventoryOfferCallback,
															 this, _1, _2));
...
void process_improved_im(LLMessageSystem* msg, void**)
{
	...
	{
		S32 md_size = msg->getSizeFast(_PREHASH_MetaData, 0, _PREHASH_Data);
		std::string buffer;
		buffer.resize(md_size, 0);
		msg->getBinaryDataFast(_PREHASH_MetaData, _PREHASH_Data,
							   (void*)&buffer.front(), md_size, 0, md_size);
		std::istringstream streamed(buffer);
		if (LLSDSerialize::fromBinary(metadata, streamed,
									  md_size) == LLSDParser::PARSE_FAILURE)
		{
			metadata.clear();
		}
	}
...
// Sends avatar and camera information to simulator when needed.
void send_agent_update(bool force_send, bool send_reliable)
{
	static LLQuaternion last_body_rot, last_head_rot;
	static LLVector3 last_cam_pos, last_camera_at;
	static F32 last_send_time = 0.f;
	static U32 last_control_flags = 0;
	static U8 last_render_state = 0;
	static U8 last_flags = AU_FLAGS_NONE;

	// Do not send an agent update if the target simulator is not ready to
	// process it, or when there is no such simulator any more (logged out).
	if ((gAgent.teleportInProgress() &&
		 gAgent.getTeleportState() != LLAgent::TELEPORT_ARRIVING) ||
		!gAgent.getRegion() || gDisconnected ||
		gAppViewerp->logoutRequestSent())
	{
		return;
	}

	// Compute last_send_delay
	constexpr F32 MAX_UPDATE_PERIOD = 1.f;	// Keep-alive rate
	static LLCachedControl<U32> rate(gSavedSettings, "MaxAgentUpdateRate");
	const F32 MIN_UPDATE_PERIOD = 1.f / llmax(100.f, rate);
	F32 last_send_delay = gFrameTimeSeconds - last_send_time;
	...
	if (spoof_mouse_look)
	{
		// Let's the scripts believe we are in mouse-look even when not. HB
	...
	// Any change in control_flags should be sent ASAP, so we fold that into
	// force_send
	force_send = force_send || (control_flags != last_control_flags);

	if (!force_send && last_send_delay < MIN_UPDATE_PERIOD)
	{
		// Throttle less-important AgentUpdates
		return;
	}

	const LLVector3* cam_posp;
	const LLVector3* camera_at_axisp;
	const LLVector3* camera_up_axisp;
	const LLVector3* camera_left_axisp;
	if (spoof_mouse_look)
	{
		cam_posp = &gAgent.getPositionAgent();
		camera_at_axisp = &gAgent.getAtAxis();
		camera_up_axisp = &gAgent.getUpAxis();
		camera_left_axisp = &gAgent.getLeftAxis();
	}
	else
	{
		// Local to agent region
		cam_posp = &gAgent.getCameraPositionAgent();
		camera_at_axisp = &gViewerCamera.getAtAxis();
		camera_up_axisp = &gViewerCamera.getUpAxis();
		camera_left_axisp = &gViewerCamera.getLeftAxis();
	}
	...
	while (!force_send && last_send_delay < MAX_UPDATE_PERIOD)
	{
		// Check to see if anything changed. Start with the easy evaluations
		// and progress to more complicated

		// Check render state and flags
		if (last_render_state != render_state || last_flags != flags)
		{
			break;	// Send message now
		}

		// Check translation
		constexpr F32 TRANSLATE_THRESHOLD = 0.01f;
		constexpr F32 TRANS_THRES_SQUARED = TRANSLATE_THRESHOLD *
											TRANSLATE_THRESHOLD;
		if ((last_cam_pos - *cam_posp).lengthSquared() > TRANS_THRES_SQUARED)
		{
			break;	// Send message now
		}
		...
		constexpr F32 CAMERA_AT_THRESHOLD = 0.1f * DEG_TO_RAD;
		constexpr F32 CAM_AT_THRES_SQUARED = CAMERA_AT_THRESHOLD *
											 CAMERA_AT_THRESHOLD;
		if ((last_camera_at - *camera_at_axisp).lengthSquared() >
				CAM_AT_THRES_SQUARED)
		{
			break;	// Send message now
		}

		// Check head rotation
		constexpr F32 MIN_HEAD_ROT_QDOT = 0.9997f;	// ~= 2.5 degrees
		constexpr F32 MAX_HEAD_ROT_QDOT = 0.99999f;	// ~= 0.5 degrees
		if (fabsf(dot(last_body_rot, body_rotation)) < MIN_HEAD_ROT_QDOT)
		{
			break;	// Send message now
		}
		F32 head_rot_qdot = fabsf(dot(last_head_rot, head_rotation));
		if (head_rot_qdot > MAX_HEAD_ROT_QDOT)
		{
			return;	// Close enough. No need to send an update just now.
		}
		else if (head_rot_qdot < MIN_HEAD_ROT_QDOT)
		{
			break;	// Way off. Send message now
		}

		// Finally, if we get here then head_rot_qdot is somewhere between
		// MIN_HEAD_ROT_QDOT and MAX_HEAD_ROT_QDOT

		// The head_rotation is sent for updating things like attached guns.
		// We only trigger a new update when head_rotation deviates beyond some
		// threshold from the last update, however this can break fine
		// adjustments when trying to aim an attached gun, so what we do here
		// (where we would normally skip sending an update when nothing has
		// changed) is linearly increase the min threshold until an update is
		// sent. Min threshold should update to MAX_HEAD_ROT_QDOT within
		// 0.5 seconds.
		constexpr F32 GROWTH = (MAX_HEAD_ROT_QDOT - MIN_HEAD_ROT_QDOT) * 2.f;
		if (head_rot_qdot < MIN_HEAD_ROT_QDOT + last_send_delay * GROWTH)
		{
			break;	// Send message now
		}

		// No need to send an update just now.
		return;
	}
	...
	msg->addVector3Fast(_PREHASH_CameraCenter, *cam_posp);
	msg->addVector3Fast(_PREHASH_CameraAtAxis, *camera_at_axisp);
	msg->addVector3Fast(_PREHASH_CameraLeftAxis, *camera_left_axisp);
	msg->addVector3Fast(_PREHASH_CameraUpAxis, *camera_up_axisp);
	...
	last_send_time = gFrameTimeSeconds;
	last_control_flags = control_flags;
	last_render_state = render_state;
	last_flags = flags;
	last_body_rot = body_rotation;
	last_head_rot = head_rotation;
	last_cam_pos = *cam_posp;
	last_camera_at = *camera_at_axisp;
}
...
void process_kill_object(LLMessageSystem* msg, void**)
{
	...
	bool non_agent_region = agent_regionp && regionp != agent_regionp;
	...
	for (S32 i = 0; i < num_objects; ++i)
	{
			...
			LLVOAvatar::findAvatarFromAttachment(objectp) == gAgentAvatarp)
		{
			...
			gObjectList.registerKilledAttachment(id);
												 ...
												 std::string desc)
{
	...
	if (!is_name_group)
	{
		LLAvatarNameCache::get(name_id,
							   std::bind(&money_balance_avatar_notify, _1, _2,
										 args, message));
	}
	else if (gCacheNamep)
	{
		gCacheNamep->get(name_id, true,
						 std::bind(&money_balance_group_notify, _1, _2, _3,
								   args, message));
	}
}
...
bool script_question_cb(const LLSD& notification, const LLSD& response)
{
	...
	if (response["BlockExperience"] && exp_id.notNull())
	{
		...
		cachep->setExperiencePermission(exp_id, "Block",
										std::bind(&block_experience, exp_id,
												  _1));
	}
...
//mk
	...
	for (S32 i = 0; i < SCRIPT_PERMISSION_EOF; ++i)
	{
		if (questions & LSCRIPTRunTimePermissionBits[i])
		{
			...
			// Check whether permission question should cause special caution
			// dialog
			if (!is_owner_linden)
			{
...
//MK
	if (auto_acceptable_permission && !(caution && caution_enabled))
	{
...
//mk
	...
	else if (experienceid.notNull())
	{
		...
		exp->get(experienceid,
				 std::bind(process_script_experience_details, _1, args,
						   payload));
		return;
	}
...
{
	LLMessageSystem* msg = gMessageSystemp;
	msg->newMessage(_PREHASH_PlacesQuery);
...
void process_script_teleport_request(LLMessageSystem* msg, void**)
{
	...
	if (gFloaterWorldMapp)
	{
		...
		{
			bool old_auto_focus = gFloaterWorldMapp->getAutoFocus();
			gFloaterWorldMapp->setAutoFocus(flags & BEACON_FOCUS_MAP);
			LLFloaterWorldMap::show(NULL, true);
			gFloaterWorldMapp->setAutoFocus(old_auto_focus);
		}
	}
#else
...
void LLOfferInfo::forceResponse(InventoryOfferResponse response)
{
	...
	params.functor(std::bind(&LLOfferInfo::inventoryOfferCallback, this, _1,
							 _2));
