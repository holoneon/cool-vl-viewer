...
void display(bool rebuild, F32 zoom_factor, S32 subfield, bool for_snapshot)
{
	...
	if (!(gViewerWindowp->getActive() || gSnapshot) ||
		!gWindowp->getVisible() || gWindowp->getMinimized())
	{
	...
	if (!gSnapshot && gUpdateDrawDistance)
	{
	...
	if (!gDisconnected && !LLApp::isExiting())
	{
...

		{
			...
			for (S32 i = 0; i < LLGLManager::sNumTextureImageUnits; ++i)
			{
		...
		if (LLPipeline::sRenderDeferred)
		{
			bool use_pbr_ssr = gUsePBRShaders && !gCubeSnapshot &&
			...
			if (use_pbr_ssr)
			{
				// Copy screen to scene map for SSR to trace against next
				// frame/pass. Must happen after deferred lighting so the lit
				// scene is available.
...
void display_cube_face()
{
	...
	S32 probe_idx = gPipeline.mHeroProbeManager.mCurrentRenderingProbeIdx;
	if (probe_idx < 0 ||
		!gPipeline.mHeroProbeManager.isMirrorPass() ||
		!gPipeline.mHeroProbeManager.mHeroShadowsComplete[probe_idx])
	{
		// We need to update environment related uniforms and mark them dirty
		// in shaders used during the cube snapshot.
...

	{
		...
		LLViewerCamera::sCurCameraID = LLViewerCamera::CAMERA_WORLD;
		LLPipeline::sUnderWaterRender = gViewerCamera.cameraUnderWater();
		// During water probe rendering, the reflected camera is below water
		// but we are rendering the above-water scene for the reflection.
		if (LLPipeline::sUnderWaterRender &&
			gPipeline.mHeroProbeManager.isMirrorPass() &&
		...
		probe_idx = gPipeline.mHeroProbeManager.mCurrentRenderingProbeIdx;
		bool mirror_pass = probe_idx >= 0 &&
						   gPipeline.mHeroProbeManager.isMirrorPass();
		if (!mirror_pass ||
			!gPipeline.mHeroProbeManager.mHeroShadowsComplete[probe_idx])
		{
...
void render_disconnected_background()
{
	...
	if (!gDisconnectedImagep && gDisconnected)
	{
		...
		U8* pixelsp = rawp->getData();
		S32 npixels = (S32)imagep->getWidth() * (S32)imagep->getHeight();
		for (S32 i = 0; i < npixels; ++i)
		{
			S32 sum = 0;
			sum = *pixelsp + *(pixelsp + 1) + *(pixelsp + 2);
			sum /= 3;
			*pixelsp = ((S32)sum * 6 + *pixelsp) / 7;
			pixelsp++;
			*pixelsp = ((S32)sum * 6 + *pixelsp) / 7;
			pixelsp++;
			*pixelsp = ((S32)sum * 6 + *pixelsp) / 7;
		...
		gDisconnectedImagep =
			LLViewerTextureManager::getLocalTexture(rawp.get(), false);
...
void hud_render_text(const LLWString& wstr, const LLVector3& pos_agent,
					 ...
					 U8 style, F32 x_offset, F32 y_offset,
					 const LLColor4& color, bool orthographic)
{
	...
	if (fontvbp)
	{
		fontvbp->render(fontp, wstr, 0, 0, 0, color, LLFontGL::LEFT,
						LLFontGL::BASELINE, style, wstr.length(), 1000,
						&right_x);
	}
	else
	{
		fontp->render(wstr, 0, 0, 0, color, LLFontGL::LEFT, LLFontGL::BASELINE,
