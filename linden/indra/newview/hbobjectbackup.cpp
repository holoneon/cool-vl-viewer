...
{
	mFileName = filename;
	mFolder = LLDir::getDirName(mFileName) + LL_DIR_DELIM_STR;
	mTexturesList.clear();
...
void HBObjectBackup::doImportObject(std::string filename)
{
	...
	mFolder = LLDir::getDirName(filename) + LL_DIR_DELIM_STR;
...
public:
	...
	void fire(const LLUUID& inv_item_id) override
	{
		...
		std::make_shared<LLBufferedAssetUploadInfo>(
			...
			std::bind(&HBBackupMatInvCB::uploadDone, _2, this),
			std::bind(&HBBackupMatInvCB::uploadFailed, this));
