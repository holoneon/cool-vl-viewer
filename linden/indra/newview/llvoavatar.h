...
public:
	...
	bool lineSegmentIntersect(const LLVector4a& start, const LLVector4a& end,
							  ...
							  bool pick_rigged = false, S32* face_hitp = NULL,
							  LLVector4a* intersectp = NULL,
							  LLVector2* tcoordp = NULL,
							  LLVector4a* normalp = NULL,
							  LLVector4a* tangentp = NULL) override;
...
public:
	...
	void addAttachmentOverridesForObject(LLViewerObject* objp,
										 uuid_list_t* meshes_seen = NULL,
										 bool recursive = true);
	void removeAttachmentOverridesForObject(LLViewerObject* objp);
...
private:
	...
	// Used by updateRiggingInfo() only, to detect rigging changes in worn
	// rigged meshes or their LOD.
	LLUUID						mLastRiggingInfoDigest;
...
public:
	...
	// Method called when debug settings governing the clouding criteria are
	// changed; needed because updateIsFullyLoaded() is not called after a
	// first, successful de-clouding, the latter now maybe not meeting any more
	// the newly configured criteria. HB
	static void loadingSettingsChanged();
...
public:
	...
	// Used in idle loop to kick in a 360 interest list toggling to get a
	// refresh from the server. HB
	static bool		sNeedsAttachmentUpdate;
...
public:
	...
	//--------------------------------------------------------------------
public:
	bool isFullyLoaded(bool truly = false);
...
private:
	...
	U32				mAttachmentsUpdateRequest;
	U32				mFullyLoadedFrameCounter;
	bool			mFullyLoaded;
...
public:
	...
	const F32* getRiggedMatrix(const LLMeshSkinInfo* skinp, U32& count);
	const LLMatrix4a* getRiggedMatrix4a(const LLMeshSkinInfo* skinp,
										U32& count);
	// Returns the number of joints in the skin, or 0 on error (NULL matp or
	// skinp). Note: this used to be a static method in (the now gone)
	// LLSkinningUtil, but it really belongs here !  HB
	U32 initSkinningMatrixPalette(LLMatrix4a* matp,
...
private:
	...
	rtf_cache_it_t initRiggedMatrixCache(const LLMeshSkinInfo* skinp,
										 U32& count);
...
public:
	...
	virtual const LLViewerJointAttachment* attachObject(LLViewerObject* objp);
	virtual bool detachObject(LLViewerObject* objp);
#if 0	// Not used/needed
	void cleanupAttachedMesh(LLViewerObject* objp);
#endif
	static bool getRiggedMeshID(LLViewerObject* objp, LLUUID& mesh_id);
	static LLVOAvatar* findAvatarFromAttachment(LLViewerObject* objp);
	bool isWearingWearableType(LLWearableType::EType type) const override;
	LLViewerJointAttachment* getTargetAttachmentPoint(LLViewerObject* objp);
...
public:
	...
	// Attachments as sent in the appeareance message by the simulator.
	typedef fast_hmap<LLUUID, S32> sim_attachment_map_t;
	sim_attachment_map_t							mSimAttachments;
...
private:
	...
	F32					mNextVoiceVisualizerUpdate;
