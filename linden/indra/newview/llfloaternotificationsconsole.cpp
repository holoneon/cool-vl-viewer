 ...
 * Copyright (c) 2023-2025, Henri Beauchamp.
 *
	...
	mNotifRejectsList(NULL)
{
	...
	const LLNotificationChannelBase::filter_t& filter =
		mChannelPtr->getFilter();
	auto negated_filter =
		[filter](LLNotificationPtr notifp)
		{
			return !filter(notifp);
		};
	mChannelRejectsPtr =
...
LLNotificationChannelPanel::~LLNotificationChannelPanel()
{
	...
	LLScrollListCtrl::items_vec_t data_list = mNotifList->getAllData();
	for (size_t i = 0, count = data_list.size(); i < count; ++i)
	{
		LLScrollListItem* itemp = data_list[i];	// Cannot be NULL. HB
		LLNotification* notifp = (LLNotification*)itemp->getUserdata();
		delete notifp;
	}
}
...
bool LLNotificationChannelPanel::postBuild()
{
	...
	mChannelPtr->connectChanged(std::bind(&LLNotificationChannelPanel::update,
										  this, _1, true));
	mChannelRejectsPtr->connectChanged(std::bind(&LLNotificationChannelPanel::update,
												 this, _1, false));
...
bool LLFloaterNotificationConsole::postBuild()
{
	...
	strings_vec_t names;
	gNotifications.getTemplateNames(names);
	for (size_t i = 0, count = names.size(); i < count; ++i)
	{
		mNotifTypesCombo->add(names[i]);
	}
