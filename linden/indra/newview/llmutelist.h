...
public:
	...
	// In SL, returns a list of muted agents username, and the muted agents
	// full name in OpenSim. Note that legacy mutes (mutes by name only) are
	// not included in this list. When 'with_parenthesis' is true, also adds
	// parenthesis around each name (this is how LL represents usernames in
	// their server messages). This is for now only used to allow rejecting
	// group invites sent by muted agents (see IM_GROUP_INVITATION case in
	// llimmgr.cpp). HB
	static std::vector<std::string> getMutedUserNames(bool with_parenthesis = false);

	// Requests the mute list
...
private:
	...
	static strings_set_t				sLegacyMutes;

	typedef std::set<LLMuteListObserver*> observer_set_t;
	static observer_set_t				sObservers;

	static strings_set_t				sGodLastNames;
	static strings_set_t				sGodFullNames;
