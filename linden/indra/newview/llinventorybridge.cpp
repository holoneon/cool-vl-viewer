...
static void warn_move_inventory(LLViewerObject* object, LLMoveInv* move_inv)
{
	...
	gNotifications.add(dialog, LLSD(), LLSD(),
					   std::bind(move_task_inventory_callback, _1, _2,
								 move_inv));
}
									  ...
									  void* user_data)
{
	...
	if (drop && accept)
	{
		...
		else
		{
			...
			params.functor(std::bind(move_task_inventory_callback, _1, _2,
									 move_inv));
...
//virtual
LLFontGL::StyleFlags LLInvFVBridge::getLabelStyle() const
{
...
bool LLInvFVBridge::isItemMovable()
{
	...
	if (modelp)
	{
		can_move = modelp->isObjectDescendentOf(mUUID,
...
{
	const LLUUID* idp = &mUUID;
	const LLInventoryObject* invobjp = getInventoryObject();
	if (invobjp && invobjp->getIsLinkType())
	{
		idp = &invobjp->getLinkedUUID();
	}
	if (idp->notNull())
	{
		LLFloaterProperties::show(*idp, LLUUID::null, mInventoryPanel);
	}
}
	...
	if (gRLenabled)
	{
		...
		if (catp)
		{
			for (size_t i = 0; i < count; ++i)
			{
				...
				const LLInventoryObject* objectp = modelp->getObject(obj_id);
				if (!objectp)
				{
					continue;
				}
				if (gRLInterface.isFolderLocked(catp) ||
					gRLInterface.isFolderLocked(modelp->getCategory(objectp->getParentUUID())))
				{
					return false;
				}
			}
...
{
	if (!HBInventoryClipboard::hasCopiedContents() || !isAgentInventory())
	{
		return false;
	}

	const LLInventoryModel* modelp = mInventoryPanel->getModel();
	if (!modelp)
	{
		return false;
	}

	static LLCachedControl<bool> cat_link_ok(gSavedSettings,
											 "AllowInventoryFolderLinking");
	...
	for (size_t i = 0; i < count; ++i)
	{
		...
		if (!modelp->isObjectDescendentOf(object_id, root_id))
		{
			return false;
		}

		const LLViewerInventoryItem* itemp = modelp->getItem(object_id);
		if (itemp && !LLAssetType::lookupCanLink(itemp->getActualType()))
		{
			return false;
		}
		else
		{
			const LLViewerInventoryCategory* catp =
				modelp->getCategory(object_id);
			if (catp && (!cat_link_ok || catp->isProtected()))
			{
				return false;
			}
		}
...
//MK
	if (gRLenabled)
	{
		// Do not allow if either the destination folder or the source folder
		// is locked
		LLViewerInventoryCategory* current_catp = modelp->getCategory(mUUID);
		if (current_catp)
		{
			for (size_t i = 0; i < count; ++i)
			{
				...
				const LLUUID& p_id = objectp->getParentUUID();
				if (gRLInterface.isFolderLocked(current_catp) ||
					gRLInterface.isFolderLocked(modelp->getCategory(p_id)))
				{
					return false;
				}
			}
...
{
	LLInventoryModel* modelp = mInventoryPanel->getModel();
	return modelp ? modelp->getObject(mUUID) : NULL;
}
										   ...
										   U32 flags)
{
	...
	case LLAssetType::AT_GLTF_BIN:
		if (inv_type != LLInventoryType::IT_GLTF)
		{
			warn = true;
		}
		// Same no-op bridge as for a SLM mesh, for now. HB
		self = new LLMeshBridge(panelp, uuid);
		break;
#endif
								 ...
								 const std::string& action)
{
	...
	else if (action == "restoreToWorld")
	{
		gNotifications.add("ObjectRestoreToWorld", LLSD(), LLSD(),
						   std::bind(restore_to_world_callback, _1, _2, this));
	}
#endif
...
bool LLFolderBridge::isClipboardPasteableAsLink() const
{
	...
	const LLViewerInventoryCategory* current_catp = getCategory();
	if (current_catp)
	{
		const LLUUID& current_cat_id = current_catp->getUUID();
		uuid_vec_t objects;
		HBInventoryClipboard::retrieve(objects);
		for (size_t i = 0, count = objects.size(); i < count; ++i)
		{
			const LLUUID& id = objects[i];
			const LLViewerInventoryCategory* catp = modelp->getCategory(id);
			if (catp)
			{
				const LLUUID& cat_id = catp->getUUID();
				// Do not allow recursive pasting
				if (cat_id == current_cat_id ||
					modelp->isObjectDescendentOf(current_cat_id, cat_id))
				{
					return false;
				}
			}
...
bool LLFindOutfitItems::operator()(LLInventoryCategory*, LLInventoryItem* itemp)
{
	if (!itemp)
	{
		return false;
	...
	LLAssetType::EType t = itemp->getType();
	return t == LLAssetType::AT_CLOTHING || t == LLAssetType::AT_BODYPART ||
		   t == LLAssetType::AT_OBJECT || t == LLAssetType::AT_GESTURE;
}
...
//mk
	...
	else if (action == "refetch")
	{
		...
		if (vcatp && AISAPI::isAvailable())
		{
			AISAPI::fetchCategoryChildren(vcatp->getUUID());
			LLSD args;
			args["NAME"] = vcatp->getName();
			gNotifications.add("FolderFetchSent", args);
		}
	}
...
void LLFolderBridge::pasteLinkFromClipboard()
{
		 ...
		 iter != end; ++iter)
	{
		...
		{
			// Do not attempt to link a notecard or material with a null asset
			// UUID, since AIS does not allow it.
			if (vitemp && vitemp->getAssetUUID().isNull())
			{
				LLAssetType::EType type = vitemp->getActualType();
				if (type == LLAssetType::AT_NOTECARD)
				{
					LLSD args;
					args["NAME"] = vitemp->getName();
					gNotifications.add("CannotLinkUnsavedNotecard", args);
					continue;
				}
				else if (type == LLAssetType::AT_MATERIAL)
				{
					LLSD args;
					args["NAME"] = vitemp->getName();
					gNotifications.add("CannotLinkUnsavedMaterial", args);
					continue;
				}
			}
		...
		else if (LLViewerInventoryCategory* vcatp =
					modelp->getCategory(object_id))
		{
			link_inventory_object(mUUID, vcatp->getLinkedUUID(), nullptr);
		}
	}
}
...
//mk
	...
	if (AISAPI::isAvailable() && isAgentInventory())
	{
		mItems.emplace_back("Refresh Contents");
	}
...
{
	return modelp->hasMatchingDescendents(mUUID, false, is_type);
}
		...
		if (vitemp)
		{
			...
			LLLandmark* landmarkp =
				gLandmarkList.getAsset(asset_id,
									   std::bind(&LLLandmarkBridge::showOnMap,
												 this, _1));
			if (landmarkp)
			{
				showOnMap(landmarkp);
