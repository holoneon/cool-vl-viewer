	...
	mDirty(false)
{
	...
	{
		mRegionCrossingSlot =
			gAgent.addRegionChangedCB(std::bind(&LLOverlayBar::handleRegionBoundaryCrossed,
												this));
	}
	...
	{
		mAgentStateSlot =
			pfmgr->registerAgentStateListener(std::bind(&LLOverlayBar::handleAgentState,
														this, _1));
	}
...
void LLOverlayBar::layoutButtons()
{
	...
	S32 btn_width = llmin(MAX_BUTTON_WIDTH, segment_width - mStatusBarPad);
...
void LLOverlayBar::createNavMeshStatusListenerForCurrentRegion()
{
	...
	mNavMeshSlot =
		pfmgr->registerNavMeshListenerForRegion(region,
												std::bind(&LLOverlayBar::handleNavMeshStatus,
														  this, _2));
...
void LLOverlayBar::onClickRebakeRegion(void* userdata)
{
	...
	LLPathfindingManager* pfmgr = LLPathfindingManager::getInstance();
	pfmgr->requestRebakeNavMesh(std::bind(&LLOverlayBar::handleRebakeNavMeshResponse,
										  self, _1));
}
