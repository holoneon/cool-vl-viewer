...
void LLPanelExperiencePicker::find()
{
	if (gAgent.hasRegionCapability("FindExperienceByName"))
	{
		...
		exp->findExperienceByName(text, mCurrentPage,
								  std::bind(&LLPanelExperiencePicker::findResults,
...
{
	LLScrollListCtrl::items_vec_t items = results->getAllSelected();
	for (size_t i = 0, count = items.size(); i < count; ++i)
	{
		LLScrollListItem* itemp = items[i];	// Cannot be NULL. HB
		if (itemp->getUUID().notNull())
		{
			experience_ids.emplace_back(itemp->getUUID());
		}
	}
}
...
void LLPanelExperiencePicker::filterContent()
{
		 ...
		 it != end; ++it)
	{
		...
		LLAvatarNameCache::get(experience[LLExperienceCache::AGENT_ID],
							   std::bind(&LLPanelExperiencePicker::nameCallback,
										 getDerivedHandle<LLPanelExperiencePicker>(),
