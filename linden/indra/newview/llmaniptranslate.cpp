...
//mk
	...
	// Compute unit vectors for arrow hit and a plane through that vector
	bool axis_exists = getManipAxis(objp, mManipPart, axis_f);
	...
	LLVector3d clamped_rel_move = axis_magnitude * axis_d;
	LLVector3 clamped_rel_move_f = (F32)axis_magnitude * axis_f;
		 ...
		 iter != end; ++iter)
	{
		...
		if (nodep->mSelectedGLTFNode != -1)
		{
			...
			clamped_rel_move_f -= nodep->mLastMoveLocal;
			objp->moveGLTFNode(nodep->mSelectedGLTFNode,
							   clamped_rel_move_f);
			nodep->mLastMoveLocal += clamped_rel_move_f;
			continue;
		}

		// Only apply motion to root objects and objects selected as
		// "individual".
		if (!objp->isRootEdit() && !nodep->mIndividualSelection)
		{
			continue;
		}
		...
		if (!objp->isRootEdit())
		{
			...
			LLViewerObject* editable_root = (LLViewerObject*)objp->getParent();
		...
		if (objp->permMove() && !objp->isPermanentEnforced() &&
			(!rootp || !rootp->isPermanentEnforced()))
		{
			// Handle attachments in local space
			if (objp->isAttachment() && objp->mDrawable.notNull())
			{
				// Calculate local version of relative move
				LLXform* parentxformp = objp->mDrawable->mXform.getParent();
				if (!parentxformp)
				{
					continue;
				}
				LLQuaternion obj_world_rot = parentxformp->getWorldRotation();
				obj_world_rot.transpose();

				LLVector3 old_pos_local = objp->getPosition();
				LLVector3 new_pos_local = nodep->mSavedPositionLocal +
										  clamped_rel_move_f * obj_world_rot;

				// RN: I forget, but we need to do this because of snapping
				// which doesn't often result in position changes even when the
				// mouse moves
				objp->setPositionLocal(new_pos_local);
				rebuild(objp);
				gAgentAvatarp->clampAttachmentPositions();
				new_pos_local = objp->getPosition();

				if (nodep->mIndividualSelection)
				{
					// Counter-translate child objects if we are moving the
					// root as an individual
					objp->resetChildrenPosition(old_pos_local -
												new_pos_local, true);
				}
			}
			else
			{
				...
				F64 min_height = gWorld.getMinAllowedZ(objp);
				...
				// to the ground
				if (objp->getPCode() == LL_PCODE_LEGACY_GRASS)
				{
				...
				{
					new_pos_global =
						gWorld.clipToVisibleRegions(objp->getPositionGlobal(),
													new_pos_global);
				}

				// PR: Only update if changed
				LLVector3 old_position_agent = objp->getPositionAgent();
				...
				if (objp->isRootEdit())
				{
					...
					objp->setPositionAgent(new_position_agent);
					rebuild(objp);
				}
				else
				{
					LLViewerObject* rootp = objp->getRootEdit();
					new_position_agent -= rootp->getPositionAgent();
					new_position_agent = new_position_agent *
										 ~rootp->getRotation();
					objp->setPositionParent(new_position_agent, false);
					rebuild(objp);
				}
				...
				if (nodep->mIndividualSelection)
				{
					// Counter-translate child objects if we are moving the
					// root as an individual
					objp->resetChildrenPosition(old_position_agent -
												new_position_agent, true);
				}
			}
			nodep->mLastPositionLocal  = objp->getPosition();
		}
	}
	...
	if (mManipPart >= LL_X_ARROW && mManipPart <= LL_Z_ARROW)
	{
		...
		if (mObjectSelection->getSelectType() != SELECT_TYPE_HUD)
		{
					...
					sHelpTextVisibleTime + sHelpTextFadeTime)
			{
				...
				hud_render_text(wtext1, help_text_pos, big_fontp, NULL,
				...
				hud_render_text(wtext2, help_text_pos, big_fontp, NULL,
