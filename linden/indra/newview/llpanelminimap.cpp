 ...
 *  - Added per-avatar dots coloring (via Lua).
 ...
 *  - Added optional plots color legend.
 *
...
:	LLPanel(name),
	...
	mFontp(LLFontGL::getFontMonospace()),
	mFriendLegendDeltaX(100),
	mScale(128.f),
	mObjectMapTPM(1.f),
	...
	mHasDrawnParcels(false)
{
	...
	(new LLDrawLegend())->registerListener(this, "MiniMap.DrawLegend");
	(new LLCheckDrawLegend())->registerListener(this,
												"MiniMap.CheckDrawLegend");
...
bool LLPanelMiniMap::postBuild()
{
	...
	mLegendAvatar = getString("avatar");
	mLegendFriend = getString("friend");
	mLegendAnimesh = getString("animesh");
	mLegendPhysical = getString("physical");
	mLegendPathFinding = getString("pathfinding");

	if (mFontp)
	{
		mFriendLegendDeltaX = mFontp->getWidth(mLegendAvatar) + 4;
	}
...
void LLPanelMiniMap::draw()
{
	...
	// Whether to plot various objects or not 
...
//mk
	...
	// Draw plots legend. HB
	static LLCachedControl<bool> plots_legend(gSavedSettings,
											  "MiniMapPlotsLegend");
	if (plots_legend && mFontp)
	{
		static LLCachedControl<LLColor4U> map_friend(gColors, "MapFriend");
		constexpr S32 DELTA_Y = 9;
		constexpr S32 BORDER_MARGIN = 4;
		S32 bottom = BORDER_MARGIN;
		mFontp->renderUTF8(mLegendAvatar, 0, BORDER_MARGIN, bottom,
						   LLColor4(map_avatar), LLFontGL::LEFT,
						   LLFontGL::BASELINE, LLFontGL::DROP_SHADOW_SOFT);
		mFontp->renderUTF8(mLegendFriend, 0,
						   BORDER_MARGIN + mFriendLegendDeltaX, bottom,
						   LLColor4(map_friend), LLFontGL::LEFT,
						   LLFontGL::BASELINE, LLFontGL::DROP_SHADOW_SOFT);
		bottom += DELTA_Y;	
		if (draw_objects)
		{
			if (plot_characters)
			{
				mFontp->renderUTF8(mLegendPathFinding, 0, BORDER_MARGIN,
								   bottom, LLColor4(map_chars), LLFontGL::LEFT,
								   LLFontGL::BASELINE,
			...
			if (plot_physical)
			{
				mFontp->renderUTF8(mLegendPhysical, 0, BORDER_MARGIN, bottom,
								   LLColor4(map_phys), LLFontGL::LEFT,
								   LLFontGL::BASELINE,
								   LLFontGL::DROP_SHADOW_SOFT);
				bottom += DELTA_Y;
			}
		...
		if (plot_puppets)
		{
			mFontp->renderUTF8(mLegendAnimesh, 0, BORDER_MARGIN, bottom,
							   LLColor4(map_puppets), LLFontGL::LEFT,
							   LLFontGL::BASELINE, LLFontGL::DROP_SHADOW_SOFT);
		}
	}
											 ...
											 F32 radius_meters)
{
	...
	if (diameter_pixels <= 4)
	{
		return;	// Do not bother rendering tiny objects. HB
	}
...
bool LLPanelMiniMap::LLDrawLegend::handleEvent(LLPointer<LLEvent>, const LLSD&)
{
	gSavedSettings.setBool("MiniMapPlotsLegend",
						   !gSavedSettings.getBool("MiniMapPlotsLegend"));
	return true;
}

bool LLPanelMiniMap::LLCheckDrawLegend::handleEvent(LLPointer<LLEvent>,
													const LLSD& userdata)
{
	LLPanelMiniMap* self = mPtr;
	bool enabled = gSavedSettings.getBool("MiniMapPlotsLegend");
	self->findControl(userdata["control"].asString())->setValue(enabled);
	return true;
}
