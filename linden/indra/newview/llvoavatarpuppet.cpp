...
#if 1
	const LLPointer<LLMeshSkinInfo> infop = mRootVolp->getSkinInfo();
	if (infop.notNull())
	{
		LLQuaternion bindrot = infop->mBindShapeMatrix.getUnscaledQuaternion();
		setRotation(bindrot * obj_rot);
		mRoot->setWorldRotation(bindrot * obj_rot);
	}
	else
#endif
	{
