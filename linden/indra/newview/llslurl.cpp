				 ...
				 size_t& end)
{
	...
	// e.g., "https://foo.com/." should just match "https://foo.com/"
...
//static
bool LLSLURL::findSLURLs(LLTextEditor* editorp, const std::string& new_text,
						 bool with_mentions, bool echo_to_console)
{
	if (!editorp)	// Paranoia
	{
		return false;
	...
	const std::string& txt = new_text.empty() ? editorp->getText() : new_text;
	if (txt.size() < 66 || txt.find("://") == std::string::npos)
	{
		// If no chance of an SLURL in the text, return right now. HB
		return false;
	}

	uuid_list_t pending_ids;
	std::string temp;
	size_t start = 0;
	size_t end = 0;
	LLUUID slurl_id;
	...
	editorp->registerPendingSLURLs(pending_ids, with_mentions,
								   echo_to_console);
	return true;
}
...
//mk
		 ...
		 it != end; )
	{
		if (it->second == id)
		{
			...
			else if (slurl.find("/mention") != std::string::npos)
			{
...
//mk
			LLTextEditor::substituteSLURL(id, slurl, substitute);
	...
	LLTextEditor::substitutionDone(id);
}
...
//mk
		 ...
		 it != end; )
	{
		...
		{
			const std::string& slurl = it->first;
			bool is_mention = !is_group &&
							   slurl.find("/mention") != std::string::npos;
			LLTextEditor::substituteSLURL(id, slurl, substitute, is_mention);
			sPendingSLURLs.erase(it++);
		}
		else
		{
...
void LLSLURL::experienceNameCallback(const LLSD& experience_details)
{
		 ...
		 it != end; )
	{
		...
		{
			const std::string& slurl = it->first;
			LLTextEditor::substituteSLURL(id, slurl, name);
			sPendingSLURLs.erase(it++);
		}
		else
		{
...
void LLSLURL::resolveSLURLs()
{
	...
	LLExperienceCache* expcachep = LLExperienceCache::getInstance();
	for (uuid_list_t::iterator it = sExperienceUUIDs.begin(),
							   end = sExperienceUUIDs.end();
		 it != end; ++it)
	{
		expcachep->get(*it, experienceNameCallback);
	}
...
LLSLURL::LLSLURL(const std::string& slurl)
{
	...
	else
	{
		...
		if (slurl.find(':') == std::string::npos)
		{
			...
			// https://slurl.com/secondlife/ for maingrid, or
			...
			// So, we strip off the prepended '/' so we do not end up with
			// https://slurl.com/secondlife/<region>/<x>/<y>/<z> or some such.
			if (slurl[0] == '/')
		    {
				 ...
				 scheme == LLSLURL::SLURL_X_GRID_LOCATION_INFO_SCHEME)
		{
			...
			{
				// Do not try to match any old http[s]://<host>/ as a SLURL.
...
std::string LLSLURL::getTypeString(eType type)
{
