...
S32				gDebugRaycastGLTFNodeHit = -1;
S32				gDebugRaycastGLTFPrimHit = -1;
...
public:
	...
	void update()
	{
		...
		static LLCachedControl<bool> show_size(gSavedSettings,
											   "DebugShowResizing");
		S32 size_x = 0;
		S32 size_y = 0;
		if (show_size && LLFloater::resizing(size_x, size_y))
		{
		...
		static LLCachedControl<bool> show_fps(gSavedSettings, "DebugShowFPS");
		if (show_fps)
		{
		...
		static LLCachedControl<bool> show_time(gSavedSettings,
											   "DebugShowTime");
		if (show_time)
		{
		...
		static LLCachedControl<bool> poll_age(gSavedSettings,
		...
		static LLCachedControl<bool> avatars_info(gSavedSettings,
												  "DebugShowAvatarRenderInfo");
		if (avatars_info)
		{
			typedef std::map<std::string, LLVOAvatar*, std::less<> > sorted_av_map_t;
			sorted_av_map_t sorted_avs;
...
//mk
			for (sorted_av_map_t::reverse_iterator rit = sorted_avs.rbegin(),
												   rend = sorted_avs.rend();
				 rit != rend; ++rit)
			{
//MK
				if (!hide_names)
//mk
		...
		static LLCachedControl<bool> render_matrices(gSavedSettings,
		...
		static LLCachedControl<bool> show_color(gSavedSettings,
												"DebugShowColor");
		if (show_color)
		{
			static U8 color[4];
...
//mk
		...
		static LLCachedControl<bool> mesh_queue(gSavedSettings,
												"DebugShowMeshQueue");
		if (mesh_queue)
		{
	...
	mCurrResolutionIndex(0)
{
	...
	// Make sure it is a multiple of 2 less or equal to 16 (and try avoiding any
	// bogus compiler optimization). HB
	U32 samples = llmin(16, gSavedSettings.getU32("RenderFSAASamples"));
	samples &= (U32_MAX - 1);	// Must be even. HB
	// Ensure a valid value is stored since it is going to be used just after
	// we are done here by LLPipeline::init(). HB
	gSavedSettings.setU32("RenderFSAASamples", samples);
	...
	// can turn off stuff if we think that it will crash the viewer.
	...
	// If we crashed while initializing GL stuff last time, disable certain
	// features.
	if (gSavedSettings.getBool("RenderInitError"))
	{
		...
		gSavedSettings.setBool("RenderUsePBR", false);
	}
	...
	{
		gl_threads = gSavedSettings.getS32("GLWorkerThreads");
	}
	LLImageGL::initThread(gWindowp, gl_threads);
	...
	// Initialize the fonts system and load default fonts.
	initFonts();
...
void LLViewerWindow::adjustRectanglesForFirstUse()
{
	...
	adjust_rect_top_center("FloaterDebugTagsRect", view_rect);
...
bool LLViewerWindow::handleKey(KEY key, MASK mask)
{
	...
	if (keyboard_focus)
	{
		...
		{
			if (gChatBarp->hasTextEditor() || !gChatBarp->hasChatText() ||
...
//virtual
bool LLViewerWindow::handleAnyMouseClick(LLWindow*, LLCoordGL pos, MASK mask,
...
//virtual
bool LLViewerWindow::handleMouseDown(LLWindow* windowp, LLCoordGL pos,
									 MASK mask)
{
	return handleAnyMouseClick(windowp, pos, mask, LLMouseHandler::CLICK_LEFT,
							   true);	// down
}

//virtual
bool LLViewerWindow::handleDoubleClick(LLWindow* windowp, LLCoordGL pos,
									   MASK mask)
{
	// try handling as a double-click first, then a single-click if that wasn't
	// handled.
	return handleAnyMouseClick(windowp, pos, mask,
							   LLMouseHandler::CLICK_DOUBLELEFT, true) ||
		   handleMouseDown(windowp, pos, mask);
}

//virtual
bool LLViewerWindow::handleMouseUp(LLWindow* windowp, LLCoordGL pos, MASK mask)
{
	return handleAnyMouseClick(windowp, pos, mask, LLMouseHandler::CLICK_LEFT,
							   false);	// up
}
...
//virtual
bool LLViewerWindow::handleRightMouseDown(LLWindow* windowp, LLCoordGL pos,
										  MASK mask)
{
	S32 x = pos.mX;
	S32 y = pos.mY;
	...
	if (handleAnyMouseClick(windowp, pos, mask, LLMouseHandler::CLICK_RIGHT,
							true))
	{
		return true;
	}
...
//virtual
bool LLViewerWindow::handleRightMouseUp(LLWindow* windowp, LLCoordGL pos,
										MASK mask)
{
	return handleAnyMouseClick(windowp,pos,mask,LLMouseHandler::CLICK_RIGHT,
							   false);	// Up
}

//virtual
bool LLViewerWindow::handleMiddleMouseDown(LLWindow* windowp, LLCoordGL pos,
										   MASK mask)
{
	...
	handleAnyMouseClick(windowp, pos, mask, LLMouseHandler::CLICK_MIDDLE,
						true);
...
//virtual
bool LLViewerWindow::handleMiddleMouseUp(LLWindow* windowp, LLCoordGL pos,
										 MASK mask)
{
	if (gVoiceClient.ready())
	{
...
//virtual
void LLViewerWindow::handleMouseMove(LLWindow* windowp, LLCoordGL pos,
									 MASK mask)
{
	S32 x = pos.mX;
	S32 y = pos.mY;
...
//virtual
void LLViewerWindow::handleMouseLeave(LLWindow*)
{
...
//virtual
bool LLViewerWindow::handleCloseRequest(LLWindow*, bool from_user)
{
...
//virtual
bool LLViewerWindow::handleSessionExit(LLWindow*)
{
	if (!LLApp::isExiting() && !LLApp::isStopped())
	{
		// OS is asking us to quit, and viewer will be killed soon if it does
		// not respond.
		LLAppViewer::sendSimpleLogoutRequest();
		// This is not our fault if we exit abruptly: remove marker files so
		// that statistics would not consider this to be a freeze or a crash.
		gAppViewerp->removeMarkerFile();
		// Do not quit immediately: let the OS kill us...
		return false;
	}
	return true;
}
...
//virtual
void LLViewerWindow::handleQuit(LLWindow*)
{
...
//virtual
void LLViewerWindow::handleResize(LLWindow*, S32 width, S32 height)
{
...
//virtual
void LLViewerWindow::handleFocus(LLWindow*)
{
...
//virtual
void LLViewerWindow::handleFocusLost(LLWindow*)
{
...
//virtual
bool LLViewerWindow::handleActivate(LLWindow*, bool activated)
{
	if (activated)
	{
		mActive = true;
...
//virtual
bool LLViewerWindow::handleActivateApp(LLWindow*, bool activating)
{
...
//virtual
void LLViewerWindow::handleMenuSelect(LLWindow*, S32 menu_item)
{
}

//virtual
bool LLViewerWindow::handlePaint(LLWindow*, S32, S32, S32, S32)
{
	return false;
}

//virtual
void LLViewerWindow::handleScrollWheel(LLWindow*, S32 clicks)
{
	handleScrollWheel(clicks);
}

//virtual
void LLViewerWindow::handleWindowBlock(LLWindow*)
{
	LLWorld::sendAgentPause();
}

//virtual
void LLViewerWindow::handleWindowUnblock(LLWindow*)
{
	LLWorld::sendAgentResume();
}

//virtual
void LLViewerWindow::handleDataCopy(LLWindow*, S32 data_type, void* data)
{
	...
	if (data_type == SLURL_MESSAGE_TYPE)
	{
		...
		if (LLURLDispatcher::dispatch(url, "", NULL, false))
		{
...
#if LL_WINDOWS
//virtual
bool LLViewerWindow::handleTimerEvent(LLWindow*)
{
...
//virtual
bool LLViewerWindow::handleDeviceChange(LLWindow*)
{
...
//virtual
bool LLViewerWindow::handleDPIChanged(LLWindow*, F32 ui_scale_factor,
...
//virtual
bool LLViewerWindow::handleWindowDidChangeScreen(LLWindow*)
{
	LLCoordScreen size;
	gWindowp->getSize(&size);
...
bool LLViewerWindow::handlePerFrameHover()
{
	...
	S32 dx = S32((F32)(mCurrentMousePoint.mX - mLastMousePoint.mX) *
					   LLUI::sGLScaleFactor.mV[VX]);
	S32 dy = S32((F32)(mCurrentMousePoint.mY - mLastMousePoint.mY) *
	...
	// This used to be hard-coded to 200 pixels. Let's make it configurable. HB
	static LLCachedControl<U32> tooltip_max_width(gSavedSettings,
												  "ToolTipMaxWidth");
	S32 max_tooltip_width = llmin((S32)tooltip_max_width,
								   mWindowRect.getWidth() / 3);
	max_tooltip_width = llmax(max_tooltip_width, 200);
		...
		gMouseIdleTimer.getElapsedTimeF32() > tooltip_delay)
	{
		...
		if (shouldShowToolTipFor(mh))
		{
			...
			if (mToolTip && tool_tip_handled && !tool_tip_msg.empty())
			{
				...
				mToolTip->setWrappedText(tool_tip_msg, max_tooltip_width);
	...
	{
		gDebugRaycastFaceHit = gDebugRaycastGLTFNodeHit =
							   gDebugRaycastGLTFPrimHit = -1;
		gDebugRaycastObject = cursorIntersect(-1, -1, 512.f, NULL, -1,
											  false, false,
											  &gDebugRaycastFaceHit,
											  &gDebugRaycastGLTFNodeHit,
											  &gDebugRaycastGLTFPrimHit,
...
{
	LLViewerWindow::sSnapshotBaseName = LLDir::getBaseFileName(filepath, true);
	LLViewerWindow::sSnapshotDir = LLDir::getDirName(filepath);
}
								  ...
								  bool show_ui, bool do_rebuild)
{
	...
	LLPointer<LLImageRaw> rawp = new LLImageRaw;
	bool success = rawSnapshot(rawp, image_width, image_height, true, false,
							   show_ui, do_rebuild);
	if (success)
	{
		LLPointer<LLImagePNG> imagep = new LLImagePNG;
		success = imagep->encode(rawp);
		if (success)
		{
			success = imagep->save(filepath);
		}
		else
		{
			llwarns << "Unable to encode PNG snapshot" << llendl;
		}
	}
	else
	{
...
bool LLViewerWindow::thumbnailSnapshot(LLImageRaw* rawp, S32 preview_width,
									   S32 preview_height, bool show_ui,
									   bool do_rebuild, U32 type)
{
	return rawSnapshot(rawp, preview_width, preview_height, false, false,
					   show_ui, do_rebuild, type);
}

// Saves the image from the screen to the specified filename and path.
bool LLViewerWindow::rawSnapshot(LLImageRaw* rawp,
								 S32 image_width, S32 image_height,
...
//mk
	...
	S32 max_width = llmin(window_width, LLGLManager::sGLMaxTextureSize);
	S32 max_height = llmin(window_height, LLGLManager::sGLMaxTextureSize);
	...
	{
		rawp->resize(image_buffer_x, image_buffer_y, 3);
	}
	else
	{
	...
	if (gUsePBRShaders && LLPipeline::sRenderDeferred &&
		LLPipeline::sRenderSSR)
	{
		gPipeline.copyScreenSpaceReflections(&gPipeline.mRT->mScreen,
											 &gPipeline.mSceneMap);
		gDisplaySwapBuffers = false;
		gDepthDirty = true;
		...
		U32 read_height = llmax(0,
								window_height - subimage_y_offset -
						  		llmax(0,
									  ...
									  buffer_y_offset - rawp->getHeight()));
				...
				for (U32 out_y = 0; out_y < read_height; ++out_y)
				{
					...
					else	// SNAPSHOT_TYPE_DEPTH
					{
						...
						for (S32 i = 0; i < (S32)read_width; ++i)
						{
							...
							for (S32 j = 0, count = rawp->getComponents();
								 j < count; ++j)
							{
								*(rawp->getData() + output_buffer_offset +
								  i * rawp->getComponents() + j) = depth_byte;
							}
						}
					}
				}
	...
	{
		ret = rawp->scale(image_width, image_height);
	}
	else if (image_width != image_buffer_x || image_height != image_buffer_y)
	{
		ret = rawp->scale(image_width, image_height, false);
	}
...
void LLViewerWindow::cubeSnapshot(const LLVector3& origin,
								  ...
								  bool custom_clip, const LLPlane& clip_plane,
								  const LLVector3* lookatp,
								  const LLVector3* updirp)
{
	...
	LLPlane orig_clip_plane;
	if (custom_clip)
	{
		orig_clip_plane = gViewerCamera.getUserClipPlane();
		gViewerCamera.setUserClipPlane(clip_plane);
	}
	...
	if (!dynamic_render)
	{
		...
		{
			if (gPipeline.hasRenderType(dynamic_render_types[i]))
			{
	...
	if (lookatp && updirp)
	{
		gViewerCamera.lookDir(*lookatp, *updirp);
	}
	else
	{
	...
	if (custom_clip)
	{
		gViewerCamera.setUserClipPlane(orig_clip_plane);
	}
...
{
	// Make sure all old fonts have been destroyed.
	LLFontGL::destroyAllGL();
										   ...
										   bool show_progress_bar)
{
	...
	// Make sure it is a multiple of 2 less or equal to 16 (and try avoiding any
	// bogus compiler optimization). HB
	U32 fsaa = llmin(16, gSavedSettings.getU32("RenderFSAASamples"));
	fsaa &= (U32_MAX - 1);	// Must be even. HB
	...
	// Make sure the pipeline is made aware of any change in FSAA samples. HB
	fsaa = gWindowp->getFSAASamples();
	LLPipeline::RenderFSAASamples = fsaa;
	// Ensure the active value is stored. HB
	gSavedSettings.setU32("RenderFSAASamples", fsaa);
...
void LLPickInfo::fetchResults()
{
	...
	if (mPickParticle)
	{
		...
		// point
		S32 part_face = -1;
		LLVOPartGroup* groupp =
			gPipeline.lineSegmentIntersectParticle(start, particle_end, NULL,
												   &part_face);
		if (groupp)
		{
			mParticleOwnerID = groupp->getPartOwner(part_face);
			mParticleSourceID = groupp->getPartSource(part_face);
		}
	}
