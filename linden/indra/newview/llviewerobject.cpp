...
LLViewerObject::obj_data_map_t LLViewerObject::sObjectDataMap;
...
bool LLViewerObject::setParent(LLViewerObject* parentp)
{
	if (mParent != parentp)
	{
		LLViewerObject* old_parentp = (LLViewerObject*)mParent;
		bool ret = LLPrimitive::setParent(parentp);
		if (ret && old_parentp && parentp)
		{
			old_parentp->removeChild(this);
		}
		return ret;
	}
										 ...
										 LLDataPacker* dp)
{
	...
	if (mesgsys)
	{
		...
		if (regionp != mRegionp && regionp && mRegionp)	// Region crossing
		{
...
//mk
				...
				if (size > 0)
				{
					...
					for (U8 param = 0; param < num_parameters; ++param)
					{
						...
						dp.unpackBinaryData(param_block,
											MAX_OBJECT_PARAMS_SIZE, param_size,
											"param_data");
		...
		switch (update_type)
		{
			...
			case OUT_FULL_CACHED:
			{
				...
				else if (value & 0x1)
				{
					...
					delete[] mData;
					mData = new U8[size];
					dp->unpackBinaryData((U8*)mData, size, sp_size,
										 "PartData");
				}
				else
				{
...
//mk
				...
				for (U8 param = 0; param < num_parameters; ++param)
				{
					...
					S32 param_size;
					dp->unpackU16(param_type, "param_type");
					dp->unpackBinaryData(param_block, MAX_OBJECT_PARAMS_SIZE,
	...
	if (sUseNewTargetOmegaCode)
	{
		...
		if (is_new_rot || new_angv != old_angv)
		{
			...
			if ((mFlags & FLAGS_SERVER_AUTOPILOT) && asAvatar() &&
				asAvatar()->isSelf())
			{
				gAgent.resetAxes();
...
void LLViewerObject::updatePuppetAvatar()
{
	...
	bool animated = rootp->isAnimatedObject();
	LLVOAvatarPuppet* puppet = rootp->mPuppetAvatar.get();
	...
	if (animated)
	{
		is_rigged_mesh = rootp->isRiggedMesh();
		if (!is_rigged_mesh)
		{
			const_child_list_t& child_list = rootp->getChildren();
	...
	if (animated && is_rigged_mesh)
	{
		if (no_puppet)
		{
			rootp->linkPuppetAvatar();
		}
	}
	else if (!no_puppet)
	{
		rootp->unlinkPuppetAvatar();
	}
...
LLMatrix4a LLViewerObject::getGLTFNodeTransformAgent(S32 node_index) const
{
		...
		node_index < (S32)mGLTFAsset->mNodes.size())
	{
		...
		mat.matMul(am, getGLTFAssetToAgentTransform());
	}
	else
	{
		mat.setIdentity();
											  ...
											  const LLQuaternion& rotation)
{
	...
	if (node.mParent != -1)
	{
		...
		agent_to_node.matMul(agent_to_asset, ami);
	}
	...
	decompose_mat(mat, pos, rot, scale);
...
const LLQuaternion LLViewerObject::getRenderRotation() const
{
	...
	if (!mDrawable->isRoot())
	{
		...
		if (parentp)
		{
			return getRotation() * LLQuaternion(parentp->getWorldMatrix());
		}
	}
									   ...
									   bool damped)
{
	...
	else
	{
		...
		// Mark reflection probe in the linkset to update position after moving
		// via script.
		if (gUsePBRShaders)
		{
			for (child_list_t::iterator iter = mChildList.begin(),
			...
			{
				LLViewerObject* childp = *iter;
				if (childp && childp->isReflectionProbe() &&
					childp->mDrawable.notNull())
				{
					gPipeline.markMoved(childp->mDrawable, damped);
				}
			}
		}
	}
...
{
	const LLViewerObject* rootp = this;
	while (rootp->mParent && !rootp->mParent->isAvatar())
	{
		rootp = (LLViewerObject*)rootp->mParent;
	}
	return (LLViewerObject*)rootp;
}
...
LLViewerTexture* LLViewerObject::getBakedTextureForMagicId(const LLUUID& id)
{
	...
	LLViewerObject* rootp = getRootEdit();
	bool is_animesh = rootp->isAnimatedObject();

	LLVOAvatar* avatarp = is_animesh ? rootp->getAvatarAncestor()
									 : getAvatar();
	if (avatarp)
	{
...
void LLViewerObject::updateTEMaterialTextures(U8 te)
{
	...
	else if (mat_id.notNull())
	{
		...
		matp = tep->getGLTFRenderMaterial()->asFetched();
	}
...
LL_NO_INLINE static void set_te_overide_mat(const LLUUID& obj_id, U8 te)
{
	LLViewerObject* objp = gObjectList.findObject(obj_id);
	if (objp)
	{
		objp->initRenderMaterial(te);
	}
}
										 ...
										 bool update_server, bool local_origin)
{
	...
	for (U8 te = start_idx; te < end_idx; ++te)
	{
		...
		if (material_changed && matp)
		{
			...
			// material here, if present. Also, sometimes, the material has
			// baked textures, which requires a copy unique to this object;
...
// Sets render material if there are overrides or if the base material is has a
// baked texture. Otherwise, sets it to null. If you are setting the material
// override and not sending an update message, you should probably call this
// method.
S32 LLViewerObject::initRenderMaterial(U8 te)
{
	...
	const LLFetchedGLTFMaterial* matp = tep->getGLTFMaterial()->asFetched();
	if (!matp) return 0;

	const LLGLTFMaterial* omatp = tep->getGLTFMaterialOverride();
	LLFetchedGLTFMaterial* rmatp = NULL;
	bool need_render_material = omatp != NULL;
	if (need_render_material)
	{
		for (const LLUUID& tex_id : matp->mTextureId)
		{
			if (LLAvatarAppearanceDictionary::isBakedImageId(tex_id))
			{
...
#endif
		rmatp = new LLFetchedGLTFMaterial(*matp);
		if (omatp)
		{
			rmatp->applyOverride(*omatp);
			rmatp->clearFetchedTextures();
		}
	}
	return tep->setGLTFRenderMaterial(rmatp);
}
...
S32 LLViewerObject::setTEGLTFMaterialOverride(U8 te, LLGLTFMaterial* matp)
{
	...
	if (!tep)
	{
		LL_DEBUGS("GLTF") << "No texture entry for te " << (S32)te
						  << " of object " << mID << LL_ENDL;
...
LLPointer<LLGLTFMaterial> LLViewerObject::getPreservedPBROverride(U8 te)
{
	LLPointer<LLGLTFMaterial> pmatp;	// Preserved override
	LLTextureEntry* tep = getTE(te);
	if (!tep)
	{
		return pmatp;
	...
	const auto& default_tf = LLGLTFMaterial::sDefault.mTextureTransform[0];
	// Only preserve transforms from existing GLTF material override.
	// Do not fall back to texture entry transforms when switching
	// between PBR materials.
	LLGLTFMaterial* omatp = tep->getGLTFMaterialOverride();
	if (omatp)
	{
		// Check if existing override has non-default transforms. Only preserve
		// non-default overrides.
		const auto& tf = omatp->mTextureTransform[0];
		if (tf.mScale != default_tf.mScale ||
			tf.mOffset != default_tf.mOffset ||
			tf.mRotation != default_tf.mRotation)
		{
			pmatp = new LLGLTFMaterial();
			for (U32 i = 0; i < LLGLTFMaterial::GLTF_TEXTURE_INFO_COUNT; ++i)
			{
				auto& tf_entry = pmatp->mTextureTransform[i];
				tf_entry.mScale = tf.mScale;
				tf_entry.mOffset = tf.mOffset;
				tf_entry.mRotation = tf.mRotation;
			}
		}
	}
	else
	{
		...
		F32 offset_s, offset_t;
		tep->getOffset(&offset_s, &offset_t);
		F32 rot = tep->getRotation();
		if (scale_s != default_tf.mScale.mV[0] ||
			scale_t != default_tf.mScale.mV[1] ||
			offset_s != default_tf.mOffset.mV[0] ||
			offset_t != default_tf.mOffset.mV[1] ||
			rot != default_tf.mRotation)
		{
			pmatp = new LLGLTFMaterial();
			...
			for (U32 i = 0; i < LLGLTFMaterial::GLTF_TEXTURE_INFO_COUNT; ++i)
			{
				LLGLTFMaterial::textureTransformToPBR(scale_s, scale_t,
													  offset_s, offset_t,
													  rot, pbr_scale,
													  pbr_offset, pbr_rot);
				pmatp->mTextureTransform[i].mScale = pbr_scale;
				pmatp->mTextureTransform[i].mOffset = pbr_offset;
				pmatp->mTextureTransform[i].mRotation = pbr_rot;
			}
		}
	}
	return pmatp;
...
LLBBox LLViewerObject::getBoundingBoxAgent() const
{
	...
	if (rootp)
	{
		LLXform* root_parentp = rootp->getParent();
		if (!root_parentp || !root_parentp->isAvatar())
		{
			rootp = NULL;
		}
	}
	...
	LLQuaternion rot;
	if (rootp && rootp->mDrawable.notNull() &&
		rootp->mDrawable->getXform() &&
		rootp->mDrawable->getXform()->getParent())
	{
		LLXform* parent_xform = rootp->mDrawable->getXform()->getParent();
...
void LLViewerObject::clearTEInvisiprim(U8 te)
{
	if (!permModify())
	{
		return;
	...
	static LLCachedControl<std::string> invis_id_str(gSavedSettings,
													 "UIImgInvisibleUUID");
	LLUUID invis_id = LLUUID(invis_id_str);
	if (invis_id.isNull() || texp->getID() != invis_id)
	{
		return;
	}

	// Get default object texture (plywood, normally).
	static LLCachedControl<std::string> deflt_id_str(gSavedSettings,
													 "DefaultObjectTexture");
	LLUUID tex_id = LLUUID(deflt_id_str);
	if (tex_id.isNull())
	{
		tex_id = IMG_PLYWOOD;	// Use this, if setting is not a valid UUID.
	}
	LLViewerFetchedTexture* vtexp =
		LLViewerTextureManager::getFetchedTexture(tex_id, FTT_DEFAULT,
												  MIPMAP_YES,
												  LLGLTexture::BOOST_NONE,
												  LLViewerTexture::LOD_TEXTURE);
	if (vtexp)
	{
		// Reset texture for te to default object texture
		setTEImage(te, vtexp);

		// Reset texture repeats
		U32 s_axis, t_axis;
		if (getTESTAxes(te, &s_axis, &t_axis))
		{
			F32 new_s = getScale().mV[s_axis] * 2.f;
			F32 new_t = getScale().mV[t_axis] * 2.f;
...
{
	LLNetworkData* paramp =
		getExtraParameterEntryCreate(LLNetworkData::PARAMS_SCULPT);
	if (!paramp)
	{
		llwarns << "Failed to create parameter entry of type PARAMS_SCULPT"
				<< llendl;
		return;
	}
	LLSculptParams* sculptp = (LLSculptParams*)paramp;
	sculptp->setSculptTexture(id, LL_SCULPT_TYPE_GLTF);
	setParameterEntryInUse(LLNetworkData::PARAMS_SCULPT, true, true);

	// Update volume
	LLVolumeParams vol_params;
	vol_params.setSculptID(id, LL_SCULPT_TYPE_GLTF);
	updateVolume(vol_params);
}
...
void LLStaticViewerObject::updateDrawable(bool force_damped)
{
	...
	if (mDrawable.notNull() && mDrawable->getVObj().notNull())
	{
		mDrawable->updateXform(true);
		gPipeline.markRebuild(mDrawable);
	}
										   ...
										   bool skip_avatar_child)
{
	...
	else	// Rotation matrix might change too.
	{
		...
		{
			LLXform* xformp = mDrawable->getXform()->getParent();
			LLQuaternion parent_rot = getRotation() *
									  xformp->getWorldRotation();
			child_offset = offset * ~parent_rot;
		}
		else
		{
...
{
	LLUUID item_id;
	LLNameValue* nvp = getNVPair("AttachItemID");
	if (nvp)
	{
		const char* s = nvp->getString();
		if (s)
		{
			item_id.set(s);
		}
...
const std::string& LLViewerObject::getAttachmentItemName()
{
	if (isAttachment())
	{
		LLInventoryItem* itemp = gInventory.getItem(getAttachmentItemID());
		if (itemp)
		{
...
LLVOAvatar* LLViewerObject::getAvatar() const
{
	...
	if (!avatarp && isAttachment())
	{
		...
		while (objp && !objp->asAvatar())
		{
			objp = (LLViewerObject*)objp->getParent();
		}

		avatarp = (LLVOAvatar*)objp;
	}
...
LLVOAvatar* LLViewerObject::getAvatarAncestor() const
{
	LLViewerObject* objp = (LLViewerObject*)getParent();
	while (objp)
	{
		LLVOAvatar* avatarp = objp->asAvatar();
		if (avatarp)
		{
			return avatarp;
		}
