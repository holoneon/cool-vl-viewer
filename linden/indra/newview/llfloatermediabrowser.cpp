...
void LLFloaterMediaBrowser::onClickAssign(void* user_data)
{
	...
	if (parcelp->getMediaType() != HTTP_CONTENT_TEXT_HTML)
	{
		parcelp->setMediaURL(media_url);
		parcelp->setMediaCurrentURL(media_url);
		parcelp->setMediaType(HTTP_CONTENT_TEXT_HTML);
