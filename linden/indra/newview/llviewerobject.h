...
// Define to 1 to add support for live debugging based on rezzed object Id. HB
#define HB_DEBUG_OBJECT 0
...
public:
	...
	LLVOAvatar* getAvatarAncestor() const;
	...
	virtual bool setParent(LLViewerObject* parentp);

	LL_INLINE virtual void onReparent(LLViewerObject* old_parentp,
	...
	void getGLTFNodeTransformAgent(S32 node_index, LLVector3* positionp,
								   LLQuaternion* rotationp,
								   LLVector3* scalep) const;
	// Moves the node at the given index by the given offset in agent space
	void moveGLTFNode(S32 node_index, const LLVector3& offset);
	// Sets the rotation in agent space of the given node
	void setGLTFNodeRotationAgent(S32 node_index, const LLQuaternion& rot);
	...
	LLPointer<LLGLTFMaterial> getPreservedPBROverride(U8 te);
	...
	LL_INLINE void setUsePBR()							{ mUsePBR = true; }
	LL_INLINE bool getUsePBR() const					{ return mUsePBR; }
	...
	U32 recursiveGetTriangleCount(S32* vcountp = NULL) const;
	...
	LL_INLINE U8 getAttachmentState() const				{ return mAttachmentState; }
	...
	// If the te bears an invisiprim texture, reset it to the default object
	// texture (normally, plywood).
	void clearTEInvisiprim(U8 te);

	// Sets the glTF asset for this LLViewerObject to the specified asset Id
	// which *must* correspond to an AT_GLTF asset type. Will release any
	// currently held references to a glTF asset on Id change.
...
#if HB_DEBUG_OBJECT
	LL_INLINE bool getDebugUpdateMsg() const		{ return mDebugUpdateMsg; }
	...
	LL_INLINE bool isDebugged() const
	{
		return mID == sDebugObjectId;
	}
...
public:
	...
	LLPointer<LLGLTF::Asset>			mGLTFAsset;
	...
	// This is a special case for mirrors and other high resolution probes.
	bool								mIsHeroProbe;

	bool								mIsGLTFAssetMissing;
...
private:
	...
	typedef std::map<std::string, U32, std::less<> > obj_data_map_t;
	static obj_data_map_t				sObjectDataMap;
};
