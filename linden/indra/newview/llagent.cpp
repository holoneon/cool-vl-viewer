...
strings_map_t LLAgent::sTeleportErrorMessages;
strings_map_t LLAgent::sTeleportProgressMessages;
...
void LLAgent::init()
{
	...
	setFlying(gSavedPerAccountSettings.getBool("FlyingAtExit"));
	...
	if (maturity)
	{
		maturity->getValidateSignal()->connect(std::bind(&LLAgent::validateMaturity,
														 this, _2));
		maturity->getSignal()->connect(std::bind(&LLAgent::handleMaturity,
												 this, _2));
	}
...
{
	if (mSimFeaturesReceivedSlot.connected())
	{
		mSimFeaturesReceivedSlot.disconnect();
	}
...
bool LLAgent::setUploadedBakesLimit()
{
		 ...
		 isAgentAvatarValid())
	{
		llinfos << "Detected change in uploaded bakes number, scheduling a rebake..."
				<< llendl;
		mRebakeNeeded = false;
		doAfterInterval(std::bind(&LLVOAvatarSelf::forceBakeAllTextures,
...
void LLAgent::onSimFeaturesReceived(const LLUUID& region_id)
{
	if (!mRegionp)
	{
		return;
	}
	if (mRegionp->getRegionID() != region_id)
	{
		// Region changed, we do not care any more...
		return;
	}
	if (!mRegionp->getFeaturesReceived())
	{
		mSimFeaturesReceivedSlot =
			mRegionp->setFeaturesReceivedCB(std::bind(&LLAgent::onSimFeaturesReceived,
													  this, _1));
		return;
	}
	// WebRTC voice. HB
	gVoiceClient.handleSimFeaturesReceived(mRegionp->getSimulatorFeatures());
}
...
void LLAgent::handleServerFeaturesTransition()
{
	...
	// WebRTC voice. HB
	onSimFeaturesReceived(mRegionp->getRegionID());
	...
	// Request the region info for the agent region: especially useful for
	// chat ranges that are not otherwise transmitted in SL. Note: placed here
	// instead of in setRegion() to make sure that the sim server did fully
	// connect and did register the agent as inside the new region. HB
	LLMessageSystem* msgp = gIsInSecondLife ? gMessageSystemp : NULL;
	if (msgp)
	{
		msgp->newMessage(_PREHASH_RequestRegionInfo);
		msgp->nextBlock(_PREHASH_AgentData);
		msgp->addUUID(_PREHASH_AgentID, gAgentID);
		msgp->addUUID(_PREHASH_SessionID, gAgentSessionID);
		sendReliableMessage();
	}
}
...
//mk
	...
	if (regionp)
	{
		...
		else
		{
			...
			regionp->setCapsReceivedCB(std::bind(&LLAgent::handleServerFeaturesTransition,
												 this));
		}

		// Play the default/idle Bento animation, if desired.
		if (gSavedSettings.getBool("PlayDefaultBentoAnimation"))
		{
		...
		U32 sim_change_type =
			mTeleportState == TELEPORT_NONE ? AFTER_CROSS_BORDER
											: AFTER_FAR_TP;
...
S32 LLAgent::getRegionMaxTextureSize() const
{
	return mRegionp ? mRegionp->getMaxTextureSize() : 1024;
}
...
LLVector3d LLAgent::calcCameraPositionTargetGlobal(bool* hit_limit)
{
	...
	LLVector2 heights =
		gWorld.resolveLandAndMinHeightGlobal(camera_position_global);
	F32 camera_land_height = heights.mV[0];
	// Note: in OpenSim, the ground level can be at a negative altitude.
	F32 min_z = llmax(heights.mV[1],
					  camera_land_height + camera_min_off_ground);
	if (camera_position_global.mdV[VZ] < min_z)
	{
		camera_position_global.mdV[VZ] = min_z;
		constrained = true;
	}
...
void LLAgent::setFocusGlobal(const LLPickInfo& pick)
{
	...
	if (objectp && !objectp->isRiggedMesh() && pick.mGLTFNodeIndex == -1)
	{
...
bool LLAgent::sendMaturityPreferenceToServer(U8 maturity)
{
	...
	http_cb_t cbsucc = std::bind(&LLAgent::processMaturityPreferenceFromServer,
								 _1, matstr);
	http_cb_t cbfail = std::bind(&LLAgent::handlePreferredMaturityError,
								 maturity);
...
bool LLAgent::requestPostCapability(const char* cap_name, LLSD& data,
									http_cb_t cbsucc, http_cb_t cbfail)
{
...
bool LLAgent::requestGetCapability(const char* cap_name, http_cb_t cbsucc,
								   http_cb_t cbfail)
{
	const std::string& url = getRegionCapability(cap_name);
	if (url.empty())
...
//mk
	...
	{
		LLLandmark* landmarkp = gLandmarkList.getAsset(lm_asset_id);
		if (landmarkp)
		{
			landmarkp->getGlobalPos(pos_global);
		}
	}
		...
		case TELEPORT_NONE:
				...
				!mPosGlobalTPdeparture.isNull())
			{
				...
				if (distance > (F32)(draw_distance * draw_distance) ||
					...
					distance == 0.f)
				{
					schedule_objects_visibility_refresh(AFTER_FAR_TP);
				}
			}
...
void LLAgent::stopCurrentAnimations()
{
	...
	// Reassert default/idle Bento animation, if desired.
	if (gSavedSettings.getBool("PlayDefaultBentoAnimation"))
	{
		sendAnimationRequest(ANIM_AGENT_BENTO_IDLE, ANIM_REQUEST_START);
	}
...
void LLAgent::sendAgentUserInfoRequest()
{
	...
	http_cb_t succ = std::bind(&LLAgent::userInfoRequestCallback, _1, true);
	http_cb_t fail = std::bind(&LLAgent::userInfoRequestCallback, _1, false);
									  ...
									  const std::string& dir_visibility)
{
	...
	http_cb_t succ = std::bind(&LLAgent::userInfoUpdateCallback, _1, true,
							   im_via_email, dir_visibility);
	http_cb_t fail = std::bind(&LLAgent::userInfoUpdateCallback, _1, false,
							   im_via_email, dir_visibility);
...
void LLAgent::parseTeleportMessages(const std::string& xml_filename)
{
		 ...
		 message_set = message_set->getNextSibling())
	{
		...
		strings_map_t* teleport_msg_map = NULL;
