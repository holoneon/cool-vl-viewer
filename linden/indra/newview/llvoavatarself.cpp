...
// Globals
LLPointer<LLVOAvatarSelf> gAgentAvatarp;
...
void LLVOAvatarSelf::initInstance()
{
	...
	mTeleportFinishedSlot =
		gViewerParcelMgr.setTPFinishedCallback(std::bind(&LLVOAvatarSelf::handleTeleportFinished,
														 this));
...
void LLVOAvatarSelf::setHoverIfRegionEnabled()
{
	...
	if (region)
	{
		region->setFeaturesReceivedCB(std::bind(&LLVOAvatarSelf::onSimulatorFeaturesReceived,
												this, _1));
	}
}
...
bool LLVOAvatarSelf::buildMenus()
{
	...
	for (S32 i = 0; i < 8; ++i)
	{
		...
		else
		{
				 ...
				 iter != mAttachmentPoints.end(); )
			{
				...
				LLViewerJointAttachment* attachp = curiter->second;
				if (attachp && attachp->getGroup() == i)
				{
					itemp = new LLMenuItemCallGL(LLTrans::getString(attachp->getName()),
												 NULL,
												 object_selected_and_point_valid);
					itemp->addListener(gMenuHolderp->getListenerByName("Object.AttachToAvatar"),
									   "on_click", curiter->first);
					gAttachPieMenup->append(itemp);
				 ...
				 iter != mAttachmentPoints.end(); )
			{
				...
				if (attachp && attachp->getGroup() == i)
				{
					itemp = new LLMenuItemCallGL(LLTrans::getString(attachp->getName()),
												 ...
												 object_attached, attachp);
					gDetachPieMenup->append(itemp);
		 ...
		 iter != mAttachmentPoints.end(); )
	{
		attachment_map_t::iterator curiter = iter++;
		LLViewerJointAttachment* attachp = curiter->second;
		if (attachp && attachp->getGroup() == 8)
		{
			std::string pt_name = LLTrans::getString(attachp->getName());
			itemp = new LLMenuItemCallGL(pt_name, NULL,
										 object_selected_and_point_valid);
			itemp->addListener(gMenuHolderp->getListenerByName("Object.AttachToAvatar"),
							   "on_click", curiter->first);
			gAttachScreenPieMenup->append(itemp);

			itemp = new LLMenuItemCallGL(pt_name, &handle_detach_from_avatar,
										 object_attached, attachp);
			gDetachScreenPieMenup->append(itemp);
		}
	}
	...
	for (S32 pass = 0; pass < 2; ++pass)
	{
			 ...
			 iter != mAttachmentPoints.end(); )
		{
			attachment_map_t::iterator curiter = iter++;
			LLViewerJointAttachment* attachp = curiter->second;
			if (attachp && attachp->getIsHUDAttachment() != (pass == 1))
			{
				continue;
			}
			std::string pt_name = LLTrans::getString(attachp->getName());
			itemp = new LLMenuItemCallGL(pt_name, &handle_detach_from_avatar,
										 ...
										 attachp);
			gDetachSubMenup->append(itemp);
		}
		if (pass == 0)
		{
			// Insert a separator between non-HUD and HUD attachments
	...
	for (S32 group = 0; group < 8; ++group)
	{
			 ...
			 attach_it != attachment_pie_menu_map.end(); ++attach_it)
		{
			...
			if (attachp)
			{
				...
				itemp = new LLMenuItemCallGL(pt_name, NULL,
											 object_selected_and_point_valid);
				gAttachBodyPartPieMenusp[group]->append(itemp);
				itemp->addListener(gMenuHolderp->getListenerByName("Object.AttachToAvatar"),
								   "on_click", attach_index);

				itemp = new LLMenuItemCallGL(pt_name,
											 &handle_detach_from_avatar,
											 object_attached, attachp);
				gDetachBodyPartPieMenusp[group]->append(itemp);
				++cur_pie_slice;
			}
		}
	}
...
//mk
	...
	gInventory.addChangedMask(LLInventoryObserver::LABEL, attach_id);
	...
	if (gAutomationp)
	{
		gAutomationp->onObjectAttached(objp->getID(), attach_id);
	}
...
#if 0	// Nope: item_id is not an object Id !  HB
...
bool LLVOAvatarSelf::needsRenderBeam()
{
	...
	if (private_point_at && objp &&
		(objp->getPositionGlobal() - agent_pos).length() > (F32)point_at_limit)
	{
		return false;
	}

	// If touching, editing or grabbing...
	if (gToolMgr.isCurrentTool(&gToolGrab) && gToolGrab.isEditing())
	{
		objp = gToolGrab.getEditingObject();
		// Do not render selection beam on attached objects, or on far objects
		// when beyond PrivatePointAtLimit.
		return objp && !objp->isAttachment() &&
			   (objp->getPositionGlobal() - agent_pos).length() <=
					(F32)point_at_limit;
	}

	return (gAgent.getRenderState() & AGENT_STATE_EDITING) &&
...
{
	doAfterInterval(std::bind(&LLVOAvatarSelf::refreshAttachments, this), 2);
}
