...
bool LLPanelClassified::canClose()
{
	...
	gNotifications.add("ClassifiedSave", args, LLSD(),
					   std::bind(&LLPanelClassified::saveCallback, this, _1,
								 _2));
	return false;
}
...
void LLPanelClassified::sendClassifiedInfoRequest()
{
	...
	LLCoreHttpUtil::HttpCoroutineAdapter::callbackHttpPost(url, body,
														   std::bind(&LLPanelClassified::handleSearchStatResponse,
																	mClassifiedID,
																	_1));
}
...
//virtual
void LLPanelClassified::processProperties(S32 type, void* datap)
{
	...
	LLAvatarClassifiedInfo* infop = (LLAvatarClassifiedInfo*)datap;
	if (infop->mClassifiedId != mClassifiedID)
	{
		return;	// Not for us.
	}

	mCreatorID = infop->mAvatarId;

	mParcelID = infop->mParcelId;
	// "Location text" is actually the original name that the owner gave to
	// the parcel, and the location.
	std::string location_text = infop->mParcelName;
	if (!location_text.empty())
	{
		location_text.append(", ");
	}
	mSimName = infop->mSimName;
	mPosGlobal = infop->mPosGlobal;
	...
	mPriceForListing = infop->mListingPrice;

	mNameEditor->setText(infop->mName);

	mDescEditor->clear();
	mDescEditor->setParseHTML(true);
	if (mCreatorID == gAgentID)
	{
		mDescEditor->setText(infop->mDesc);
		mDescEditor->setEnabled(!mInFinder);
		if (mInFinder)
		{
			// Support for SLURLs substitutions in search floater for our own
			// classifieds (since the description cannot be edited there). HB
			LLSLURL::findSLURLs(mDescEditor);
		}
	}
	else
	{
		mDescEditor->appendColoredText(infop->mDesc, false, false,
	...
	mSnapshotCtrl->setImageAssetID(infop->mSnapshotId);

	mCategoryCombo->setCurrentByIndex(infop->mCategory - 1);

	if (is_cf_mature(infop->mFlags))
	{
	...
	if (mAutoRenewCheck)
	{
		mAutoRenewCheck->set(is_cf_auto_renew(infop->mFlags));
	}

	time_t tim = infop->mCreationDate;
...
//static
void LLPanelClassified::onClickUpdate(void* datap)
{
	LLPanelClassified* self = (LLPanelClassified*)datap;
	if (!self) return;
	...
	if (self->mMatureCombo->getCurrentIndex() == DECLINE_TO_STATE)
	{
		...
		gNotifications.add("SetClassifiedMature", LLSD(), LLSD(),
						   std::bind(&LLPanelClassified::confirmMature, self,
									 _1, _2));
	}
	else
	{
...
void LLPanelClassified::gotMature()
{
	...
	if (mPaidFor)
	{
		...
		params.functor(std::bind(&LLPanelClassified::confirmPublish, this, _1,
								 _2));
												   ...
												   void* datap)
{
	...
	gNotifications.add("PublishClassified", args, LLSD(),
					   std::bind(&LLPanelClassified::confirmPublish, self, _1,
								 _2));
}
...
//static
void LLPanelClassified::onClickTeleport(void* datap)
{
	LLPanelClassified* self = (LLPanelClassified*)datap;
...
//static
void LLPanelClassified::onClickMap(void* datap)
{
	LLPanelClassified* self = (LLPanelClassified*)datap;
	if (self)
	{
...
//static
void LLPanelClassified::onClickProfile(void* datap)
{
	LLPanelClassified* self = (LLPanelClassified*)datap;
	if (self)
	{
...
#if 0
//static
void LLPanelClassified::onClickLandmark(void* datap)
{
	LLPanelClassified* self = (LLPanelClassified*)datap;
...
//static
void LLPanelClassified::onClickSet(void* datap)
{
	LLPanelClassified* self = (LLPanelClassified*)datap;
...
//static
void LLPanelClassified::onCommitAny(LLUICtrl*, void* datap)
{
	LLPanelClassified* self = (LLPanelClassified*)datap;
	if (self)
	{
...
//static
void LLPanelClassified::focusReceived(LLFocusableElement* ctrlp, void* datap)
{
	// Allow the data to be saved
	onCommitAny((LLUICtrl*)ctrlp, datap);
}
...
//static
void LLFloaterPriceForListing::onClickSetPrice(void* datap)
{
	buttonCore(0, datap);
}

//static
void LLFloaterPriceForListing::onClickCancel(void* datap)
{
	buttonCore(1, datap);
}

//static
void LLFloaterPriceForListing::buttonCore(S32 button, void* datap)
{
	LLFloaterPriceForListing* self = (LLFloaterPriceForListing*)datap;
