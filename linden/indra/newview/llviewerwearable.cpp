...
{
	std::string fname = gDirUtil.getFullPath(LL_PATH_CACHE,
											 asset_id.asString());
	return fname + ".wbl";
}
...
{
	if (!isAgentAvatarValid() || avatarp != (LLAvatarAppearance*)gAgentAvatarp)
	{
		return;
	}
	...
	for (S32 te = 0; te < TEX_NUM_INDICES; ++te)
	{
		...
		gAgentAvatarp->setLocalTextureTE(te, texp, 0);
	}
