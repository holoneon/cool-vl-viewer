...
LLViewerTexLayerSetBuffer::LLViewerTexLayerSetBuffer(LLTexLayerSet* const ownerp,
													 S32 width, S32 height)
:	LLTexLayerSetBuffer(ownerp),
			...
			if (valid)
			{
					...
					mUploadFailCount < BAKE_UPLOAD_ATTEMPTS - 1)
				{
					...
					gCoros.launch("uploadBakedTextureCoro",
								  std::bind(&LLViewerTexLayerSetBuffer::uploadBakedTextureCoro,
											url, mUploadID,
											baked_upload_data));
				}
