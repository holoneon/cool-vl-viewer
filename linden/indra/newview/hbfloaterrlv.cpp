...
static void parse_command(std::string& cmd, strings_set_t& restrictions,
						  strings_map_t& exceptions)
{
	// Note: this list can be found by grep'ing the sources for
	// containsWithoutException
	static const strings_set_t exception_types =
	{
		"edit",
		"recvchat",
		"recvemote",
...
void HBFloaterRLV::draw()
{
	...
	if (mIsDirty)
	{
		...
		if (!gRLInterface.mSpecialObjectBehaviours.empty())
		{
			...
			strings_map_t::const_iterator iter;
			strings_map_t::const_iterator except_end = exceptions.end();
			for (strings_set_t::const_iterator it = restrictions.begin(),
