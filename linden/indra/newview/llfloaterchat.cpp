...
static strings_set_t sHighlightWords;
...
void LLFloaterChat::resolveSLURLs(const LLChat& chat)
{
	...
	bool found_slurl = LLSLURL::findSLURLs(self->mHistoryWithoutMutes,
										   chat.mText, true, true);
	// Do not bother if no SLURL was found in text without mutes. HB
	if (found_slurl)
	{
		LLSLURL::findSLURLs(self->mHistoryWithMutes, chat.mText, true);
	}
}
