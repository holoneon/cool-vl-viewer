										   ...
										   LLPointer<LLViewerFetchedTexture>& emissivep)
{
	...
	if (mr_imgp)
	{
		...
		if (occl_imgp)
		{
					...
					material.occlusionTexture.index)
			{
				...
				if (occlusion_idx != mr_idx)
				{
					...
					occl_imgp->scale(mr_imgp->getWidth(),
									 mr_imgp->getHeight());
		...
		else if (material.occlusionTexture.index == -1)
		{
			...
			occl_imgp = new LLImageRaw(mr_imgp->getWidth(),
									   mr_imgp->getHeight(), 3);
			occl_imgp->clear(255, 255, 255);
			copy_red_channel(occl_imgp, mr_imgp);
		}
	}
	else if (occl_imgp)
	{
		...
		// red channel over.
		mr_imgp = new LLImageRaw(occl_imgp->getWidth(), occl_imgp->getHeight(),
								 3);
											...
											std::string& mat_name, bool flip)
{
	...
	LLPointer<LLImageRaw> base_img =
		getTexture(folder, model_in,
				   mat_in.pbrMetallicRoughness.baseColorTexture.index, flip);
	// Get normal map
	...
	LLPointer<LLImageRaw> emissive_imgp =
		getTexture(folder, model_in, mat_in.emissiveTexture.index, flip);
			...
			mat_in.pbrMetallicRoughness.metallicRoughnessTexture.index)
	{
		occl_imgp = getTexture(folder, model_in,
							   mat_in.occlusionTexture.index, flip);
	}
