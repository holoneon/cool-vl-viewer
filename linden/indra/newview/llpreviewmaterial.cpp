...
//static
void LLPreviewMaterial::closeLiveEditorInstance()
{
	if (sLiveEditorInstance)
	{
		sLiveEditorInstance->close();
		sLiveEditorInstance = NULL;
		sOverrideInProgress = sSelectionNeedsUpdate = false;
	}
}
...
bool LLPreviewMaterial::postBuild()
{
	...
	std::string tex_id = gSavedSettings.getString("DefaultNormalTexture");
	mNormalTexCtrl->setDefaultImageAssetID(LLUUID(tex_id));
	tex_id = gSavedSettings.getString("BlankNormalTexture");
	mNormalTexCtrl->setBlankImageAssetID(LLUUID(tex_id));

	mBaseColorScaledText = getChild<LLTextBox>("base_color_scaled_text");
	mMetallicScaledText = getChild<LLTextBox>("metallic_scaled_text");
	mEmissiveScaledText = getChild<LLTextBox>("emissive_scaled_text");
	mNormalScaledText = getChild<LLTextBox>("normal_scaled_text");
	...
	if (!mIsOverride)
	{
		setRectControl("PreviewMaterialRect");
		applyRectControl();
	}
	else if (LLFloaterTools::isVisible())
	{
		gFloaterToolsp->addDependentFloater(this);
	}
	else
	{
		center();
...
static S32 get_texture_cost(LLViewerFetchedTexture* texp)
{
	if (!texp)
	{
		return 0;
	}
	return LLEconomy::getInstance()->getTextureUploadCost(texp->getWidth(),
														  texp->getHeight());
}
...
void LLPreviewMaterial::markChangesUnsaved(U32 dirty_flag)
{
	...
	{
		mExpectedUploadCost = get_texture_cost(mBaseColorFetched.get());
	}
	if (mMetallicTextureUploadId.notNull() &&
		mMetallicTextureUploadId == getMetallicRoughnessId())
	{
		mExpectedUploadCost +=
			get_texture_cost(mMetallicRoughnessFetched.get());
	}
	if (mEmissiveTextureUploadId.notNull() &&
		mEmissiveTextureUploadId == getEmissiveId())
	{
		mExpectedUploadCost += get_texture_cost(mEmissiveFetched.get());
	}
	if (mNormalTextureUploadId.notNull() &&
		mNormalTextureUploadId == getNormalId())
	{
		mExpectedUploadCost += get_texture_cost(mNormalFetched.get());
	}
	mUploadFeeText->setTextArg("[COST]", llformat("%d", mExpectedUploadCost));
	mUploadFeeText->setVisible(mExpectedUploadCost > 0);

	bool scaled =
		mBaseColorFetched.notNull() && mBaseColorJ2C.notNull() &&
		(mBaseColorFetched->getWidth() != mBaseColorJ2C->getWidth() ||
		 mBaseColorFetched->getHeight() != mBaseColorJ2C->getHeight());
	mBaseColorScaledText->setVisible(scaled);

	scaled = mMetallicRoughnessFetched.notNull() &&
			 mMetallicRoughnessJ2C.notNull() &&
			 (mMetallicRoughnessFetched->getWidth() !=
				mMetallicRoughnessJ2C->getWidth() ||
			  mMetallicRoughnessFetched->getHeight() !=
				mMetallicRoughnessJ2C->getHeight());
	mMetallicScaledText->setVisible(scaled);

	scaled = mNormalFetched.notNull() && mNormalJ2C.notNull() &&
			 (mNormalFetched->getWidth() != mNormalJ2C->getWidth() ||
			  mNormalFetched->getHeight() != mNormalJ2C->getHeight());
	mNormalScaledText->setVisible(scaled);

	scaled = mEmissiveFetched.notNull() && mEmissiveJ2C.notNull() &&
			 (mEmissiveFetched->getWidth() != mEmissiveJ2C->getWidth() ||
			  mEmissiveFetched->getHeight() != mEmissiveJ2C->getHeight());
	mEmissiveScaledText->setVisible(scaled);
}
												   ...
												   std::string texture_type)
{
	...
	image_uri = LLDir::getBaseFileName(image_uri, true);
...
void LLPreviewMaterial::setFromGltfMetaData(const std::string& filename,
											...
											S32 index)
{
	mMaterialNameShort = LLDir::getBaseFileName(filename, true);
									 ...
									 const std::string& filename, S32 index)
{
	...
	{
		mNormalJ2C = LLViewerTextureList::convertToUploadFile(norm_imgp,
...
void LLPreviewMaterial::loadLive()
{
	...
	if (!sSelectionUpdateSlot.connected())
	{
		sSelectionUpdateSlot =
			gSelectMgr.mUpdateSignal.connect(std::bind(&onSelectionChanged));
	}
												   ...
												   S32 index)
{
	...
	bool loaded = false;
	std::string exten = LLDir::getExtension(filename);
	if (exten == "gltf")
	{
...
void LLPreviewMaterial::onClickCancel(void* userdata)
{
	...
	{
		if (self->mIsOverride && gFloaterToolsp)
		{
			gFloaterToolsp->dirty();
		}
		self->close();
	...
	gNotifications.add("UnsavedMaterialChanges", LLSD(), LLSD(),
					   std::bind(&LLPreviewMaterial::onCancelMsgCallback,
								 self, _1, _2));
}
...
void LLPreviewMaterial::onClickSaveAs(void* userdata)
{
	...
	gNotifications.add("SaveMaterialAs", args, LLSD(),
					   std::bind(&LLPreviewMaterial::onSaveAsMsgCallback,
								 self, _1, _2));
}
									 ...
									 LLPermissions& permissions)
{
	...
	LLPermissions item_perms(gAgentID);	// Defaults to owned by agent. HB
	LLViewerInventoryItem* itemp =
		objectp->getInventoryItemByAsset(func.mMaterialId);
	if (itemp)
	{
		item_perms = itemp->getPermissions();
		for (U32 i = 0, count = ops.size(); i < count; ++i)
		{
	...
	LLPermissions obj_perms(gAgentID);	// Defaults to owned by agent. HB
...
void LLPreviewMaterial::saveObjectsMaterial()
{
	...
	if (localmatp.isNull() && func.mInvItemId.notNull())
	{
		...
		// New material created and owned by agent.
		permissions = LLPermissions(gAgentID);
	}
	gNotifications.add("SaveMaterialAs", args, payload,
					   std::bind(&LLPreviewMaterial::onSaveObjectsMaterialCB,
								 _1, _2, permissions));
}
...
//static
void LLPreviewMaterial::finishTaskUpload(const LLUUID& new_asset_id,
										 void* userdata)
{
	LLPreviewMaterial* self = (LLPreviewMaterial*)userdata;
	if (self && sEditorInstances.count(self))	// Floater still there ?
											...
											const LLUUID& task_id)
{
	...
	void* userdatap = (void*)this;
	auto failcb = std::bind(uploadFailure, userdatap);
	LLResourceUploadInfo::ptr_t infop;
	if (task_id.notNull())
	{
		...
		if (!task_url.empty())
		{
			// Not using std::bind here, because it gets confused by the
			// fact that LLBufferedAssetUploadInfo's inv_uploaded_cb_t got
			// the same signature as task_uploaded_cb_t: only the meaning
			// of the two last LLUUIDs changes... HB
			auto succcb =
				[userdatap](LLUUID, LLUUID, LLUUID new_asset_id, LLSD)
				{
					finishTaskUpload(new_asset_id, userdatap);
				};
			urlp = &task_url;
	...
	else
	{
		...
		if (!inv_url.empty())
		{
			...
			// of the two last LLUUIDs changes... HB
			auto succcb =
				[userdatap](LLUUID item_id, LLUUID new_asset_id,
							LLUUID new_item_id, LLSD)
				{
					finishInventoryUpload(item_id, new_asset_id, new_item_id,
										  userdatap);
				};
			urlp = &inv_url;
			infop = std::make_shared<LLBufferedAssetUploadInfo>(
...
bool LLPreviewMaterial::saveTexture(LLImageJ2C* imgp, U32 tex_type,
									const std::string& name,
									const LLUUID& asset_id)
{
	if (asset_id.isNull() || !imgp || !imgp->getDataSize())
	{
		return false;
	}
	...
	std::string buffer;
	buffer.assign((const char*)imgp->getData(), imgp->getDataSize());
	...
	LLResourceUploadInfo::ptr_t info =
		std::make_shared<LLNewBufferedResourceUploadInfo>(
			...
			LLEconomy::getInstance()->getTextureUploadCost(imgp->getWidth(),
														   imgp->getHeight()),
			std::bind(uploadSuccess, _1, _2, tex_type, (void*)this),
			std::bind(uploadFailure, (void*)this));
