...
void populate_groups_list(LLFloater* self, LLScrollListCtrl* group_list,
						  ...
						  const std::string& filter = LLStringUtil::null)
{
	group_list->deleteAllItems();

	bool filter_groups = !filter.empty();
	for (U32 i = 0, count = gAgent.mGroups.size(); i < count; ++i)
	{
		...
		if (filter_groups)
		{
			std::string name = group_data.mName;
...
void LLFloaterGroups::reset()
{
	...
	populate_groups_list(this, mGroupsList, gAgent.getGroupID(),
						 GP_ALL_POWERS, true, mFilterString);
	enableButtons();
}
...
//static
void LLFloaterGroups::onSearchEdit(const std::string& search_string,
								   void* userdata)
{
	LLFloaterGroups* self = (LLFloaterGroups*)userdata;
	if (self)
