...
void LLSpeaker::lookupName()
{
	LLAvatarNameCache::get(mID,
						   std::bind(&LLSpeaker::onAvatarNameLookup, _1, _2,
...
void LLSpeaker::onAvatarNameLookup(const LLUUID& id,
								   ...
								   void* userdatap)
{
	LLSpeaker* speaker_ptr = ((LLHandle<LLSpeaker>*)userdatap)->get();
	...
	delete (LLHandle<LLSpeaker>*)userdatap;
}
...
//
LLPanelActiveSpeakers::LLPanelActiveSpeakers(LLSpeakerMgr* managerp,
...
bool LLPanelActiveSpeakers::postBuild()
{
	std::string sort_column =
		gSavedSettings.getString("FloaterActiveSpeakersSortColumn");
	bool sort_ascending =
		gSavedSettings.getBool("FloaterActiveSpeakersSortAscending");
	...
	mMuteBtn = getChild<LLButton>("mute_btn", true, false);
	if (mMuteBtn)
	{
		// For the mute buttons, everywhere else
		mMuteBtn->setClickedCallback(onClickMuteVoice, this);
	}

	mDevicesBtn = getChild<LLButton>("devices_btn", true, false);
	if (mDevicesBtn)
	{
		mDevicesBtn->setClickedCallback(onClickDeviceSettings, this);
	}

	mSpeakerVolumeSlider = getChild<LLSliderCtrl>("speaker_volume", true,
												  false);
...
void LLPanelActiveSpeakers::addSpeaker(const LLUUID& speaker_id, bool force)
{
	...
	if (speakerp.notNull())
	{
		...
		{
			speaker_name = gCacheNamep->getDefaultName();
		}
		else
		{
...
void LLPanelActiveSpeakers::handleSpeakerSelect()
{
	...
	// Since setting these values is delayed by a round trip to the voice
	// server; update them only when selecting a new speaker or asynchronously
	// when an update arrives
	if (mModeratorAllowVoiceCtrl)
	{
		mModeratorAllowVoiceCtrl->setValue(!speakerp->mModeratorMutedVoice);
	}
	if (mModeratorAllowTextCtrl)
	{
		mModeratorAllowTextCtrl->setValue(!speakerp->mModeratorMutedText);
	}
	if (mMuteBtn)
	{
		mMuteBtn->setToggleState(LLMuteList::isMuted(speaker_id,
...
void LLPanelActiveSpeakers::refreshSpeakers(bool force)
{
	if (mDevicesBtn)
	{
		mDevicesBtn->setEnabled(!gVoiceClient.tuningModeActive());
	}
...
//mk
	...
	LLScrollListCtrl::items_vec_t items = mSpeakerList->getAllData();
...
//static
void LLPanelActiveSpeakers::onClickMuteTextCommit(LLUICtrl*, void* userdatap)
{
	LLPanelActiveSpeakers* panelp = (LLPanelActiveSpeakers*)userdatap;
	if (!panelp) return;
...
//static
void LLPanelActiveSpeakers::onClickMuteVoice(void* userdatap)
{
	onClickMuteVoiceCommit(NULL, userdatap);
}

//static
void LLPanelActiveSpeakers::onClickDeviceSettings(void*)
{
	LLFloaterVoiceDeviceSettings::showInstance();
}

//static
void LLPanelActiveSpeakers::onClickMuteVoiceCommit(LLUICtrl* ctrlp,
												   void* userdatap)
{
	LLPanelActiveSpeakers* panelp = (LLPanelActiveSpeakers*)userdatap;
	if (!panelp) return;
	...
	LLPointer<LLSpeaker> speakerp =
		panelp->mSpeakerMgr->findSpeaker(speaker_id);
	if (speakerp.isNull())
	{
		return;
	}
...
//static
void LLPanelActiveSpeakers::onVolumeChange(LLUICtrl* source, void* userdatap)
{
	LLPanelActiveSpeakers* panelp = (LLPanelActiveSpeakers*)userdatap;
...
//static
void LLPanelActiveSpeakers::onClickProfile(void* userdatap)
{
	LLPanelActiveSpeakers* panelp = (LLPanelActiveSpeakers*)userdatap;
	if (!panelp) return;
	...
	{
		LLViewerObject* objp = gObjectList.findObject(speaker_id);
		if (!objp)
		{
			// Others' HUDs are not in our objects list: use the HUD owner
			// to find out their actual position...
			objp = gObjectList.findObject(speakerp->mOwnerID);
		}
		if (objp
//MK
...
//mk
		{
			LLVector3 pos = objp->getPositionRegion();
			...
			location << objp->getRegion()->getName() << "/" << x << "/"
					 << y << "/" << z;
...
//static
void LLPanelActiveSpeakers::onDoubleClickSpeaker(void* userdatap)
{
	LLPanelActiveSpeakers* panelp = (LLPanelActiveSpeakers*)userdatap;
	if (!panelp) return;
...
//static
void LLPanelActiveSpeakers::onSelectSpeaker(LLUICtrl* source, void* userdatap)
{
	LLPanelActiveSpeakers* panelp = (LLPanelActiveSpeakers*)userdatap;
	if (panelp)
	{
...
//static
void LLPanelActiveSpeakers::onSortChanged(void* userdatap)
{
	LLPanelActiveSpeakers* panelp = (LLPanelActiveSpeakers*)userdatap;
	if (panelp)
	{
...
//static
void LLPanelActiveSpeakers::onModeratorMuteVoice(LLUICtrl* ctrlp,
												 void* userdatap)
{
	LLPanelActiveSpeakers* self = (LLPanelActiveSpeakers*)userdatap;
	if (!self || !self->mSpeakerList || !ctrlp) return;

	const LLUUID& agent_id = self->mSpeakerList->getValue();
	bool muted = !ctrlp->getValue();

	if (self->mSpeakerMgr &&
		gAgent.hasRegionCapability("SpatialVoiceModerationRequest"))
	{
		self->mSpeakerMgr->requestMuteChange(agent_id, muted);
	}

	const LLUUID& session_id = self->mSpeakerMgr->getSessionID();
	LLAgent::http_cb_t fail =
		std::bind(&LLPanelActiveSpeakers::moderatorActionFailedCallback, _1,
				  session_id);
	LLSD data;
	data["method"] = "mute update";
	data["session-id"] = session_id;
	data["params"] = LLSD::emptyMap();
	data["params"]["agent_id"] = agent_id;
	data["params"]["mute_info"] = LLSD::emptyMap();
	// Ctrl value represents ability to type, so invert
	data["params"]["mute_info"]["voice"] = muted;
...
//static
void LLPanelActiveSpeakers::onModeratorMuteText(LLUICtrl* ctrlp,
												void* userdatap)
{
	LLPanelActiveSpeakers* self = (LLPanelActiveSpeakers*)userdatap;
	if (!self || !self->mSpeakerList || !ctrlp) return;
	...
	data["params"]["mute_info"] = LLSD::emptyMap();
	// Ctrl value represents ability to type, so invert
	data["params"]["mute_info"]["text"] = !ctrlp->getValue();
...
//static
void LLPanelActiveSpeakers::onChangeModerationMode(LLUICtrl* ctrlp,
												   void* userdatap)
{
	LLPanelActiveSpeakers* self = (LLPanelActiveSpeakers*)userdatap;
	...
	if (ctrlp->getValue().asString() == "unmoderated")
	{
		data["params"]["update_info"]["moderated_mode"]["voice"] = false;
	}
	else if (ctrlp->getValue().asString() == "moderated")
	{
...
void LLSpeakerMgr::updateSpeakerList()
{
	...
	LLVoiceClient::participants_vec_t participants;
	if (!gVoiceClient.getParticipants(participants))
	{
		return;
	}

	// Add new participants to our list of known speakers
	for (size_t i = 0, count = participants.size(); i < count; ++i)
	{
		LLVoiceClient::ParticipantData& data = participants[i];
		setSpeaker(data.mId, data.mName, LLSpeaker::STATUS_VOICE_ACTIVE,
				   (data.mIsAvatar ? LLSpeaker::SPEAKER_AGENT
...
//static
void LLSpeakerMgr::requestMuteChangeCB(const LLSD& result, std::string name,
									   bool muted, bool success)
{
	...
	// *TODO: translate
	std::string msg = success ? "Success to " : "Failure to ";
	msg += muted ? "mute" : "unmute";
	msg += " avatar: " + name;
	if (success)
	{
		llinfos << msg << llendl;
...
void LLSpeakerMgr::requestMuteChange(const LLUUID& agent_id, bool mute)
{
	if (gDisconnected)
	{
		return;
	...
	{
		llwarns << "No capabity received yet for this region." << llendl;
		return;
	}

	const LLPointer<LLSpeaker> speakerp = findSpeaker(agent_id);
	if (speakerp.isNull())
	{
		llwarns << "No voice speaker found for Id: " << agent_id << llendl;
		return;
	}
	std::string name = speakerp->mLegacyName;
	if (name.empty())
	{
		name = agent_id.asString();
	}

	LLAgent::http_cb_t succ =
		std::bind(&LLSpeakerMgr::requestMuteChangeCB, _1, name, mute, true);
	LLAgent::http_cb_t fail =
		std::bind(&LLSpeakerMgr::requestMuteChangeCB, _1, name, mute, false);
	LLSD body;
	body["operand"] = mute ? "mute" : "unmute";
	body["agent_id"] = agent_id;
	gAgent.requestPostCapability("SpatialVoiceModerationRequest", body);
}

//
// LLIMSpeakerMgr
//
LLIMSpeakerMgr::LLIMSpeakerMgr(LLVoiceChannel* channelp)
:	LLSpeakerMgr(channelp)
{
}
