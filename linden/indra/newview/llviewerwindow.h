...
public:
	...
	bool handleAnyMouseClick(LLWindow*, LLCoordGL pos, MASK mask,
							 LLMouseHandler::EClickType clicktype,
							 bool down);
	bool handleMouseDown(LLWindow*, LLCoordGL pos, MASK mask) override;
	bool handleMouseUp(LLWindow*, LLCoordGL pos, MASK mask) override;
	bool handleRightMouseDown(LLWindow* wp, LLCoordGL pos, MASK mask) override;
	bool handleRightMouseUp(LLWindow* wp, LLCoordGL pos, MASK mask) override;
	bool handleMiddleMouseDown(LLWindow* wp, LLCoordGL pos, MASK mask) override;
	bool handleMiddleMouseUp(LLWindow* wp, LLCoordGL pos, MASK mask) override;
	void handleMouseMove(LLWindow*, LLCoordGL pos, MASK mask) override;
	void handleMouseLeave(LLWindow*) override;
	bool handleCloseRequest(LLWindow*, bool from_user = true) override;
	bool handleSessionExit(LLWindow*) override;
	void handleQuit(LLWindow*) override;
	void handleResize(LLWindow*, S32 x, S32 y) override;

	void handleFocus(LLWindow*) override;
	void handleFocusLost(LLWindow*) override;
	bool handleActivate(LLWindow*, bool activated) override;
	bool handleActivateApp(LLWindow*, bool activating) override;

	void handleMenuSelect(LLWindow*, S32 menu_item) override;
	bool handlePaint(LLWindow* w, S32 x, S32 y, S32 wdth, S32 hght) override;
	void handleScrollWheel(LLWindow*, S32 clicks) override;
	bool handleDoubleClick(LLWindow* winp, LLCoordGL pos, MASK mask) override;
	void handleWindowBlock(LLWindow*) override;
	void handleWindowUnblock(LLWindow*) override;
	void handleDataCopy(LLWindow* windowp, S32 data_type, void* data) override;
#if LL_WINDOWS	// Not used for now under Linux...
	bool handleTimerEvent(LLWindow*) override;
	bool handleDeviceChange(LLWindow*) override;
	bool handleDPIChanged(LLWindow*, F32 ui_scale_factor, S32 window_width,
						  S32 window_height) override;
#endif
	bool handleWindowDidChangeScreen(LLWindow*) override;
	...
	void cubeSnapshot(const LLVector3& origin, LLCubeMapArray* cubemapp,
					  ...
					  const LLVector3* lookatp, const LLVector3* updirp);
	...
	LLViewerObject* cursorIntersect(S32 mouse_x = -1, S32 mouse_y = -1,
									...
									S32* face_hit = NULL,
									S32* gltf_node_hit = NULL,
									S32* gltf_prim_hit = NULL,
