	...
	for (S32 i = 0, count = agent_ids.size(); i < count; ++i)
	{
		...
		else
		{
			...
			LLAvatarNameCache::get(agent_ids[i],
								   std::bind(onAvatarNameCache, _1, _2,
											 selfp));
		}
	}
}
...
void LLPanelGroupBulkImpl::handleRemove()
{
	...
	LLScrollListCtrl::items_vec_t items = mBulkAgentList->getAllSelected();
	if (items.empty())
	{
		return;
	}
	...
	for (size_t i = 0, count = items.size(); i < count; ++i)
	{
		...
		mInviteeIDs.erase(itemp->getUUID());
	}
...
{
	if (mBulkAgentList && mRemoveButton)
	{
		mRemoveButton->setEnabled(!mBulkAgentList->getAllSelected().empty());
	}
}
			...
			if (LLAvatarTracker::isAgentFriend(agent_id))
			{
				...
				if (!LLAvatarNameCache::get(agent_id, &av_name))
				{
					...
					LLAvatarNameCache::get(LLUUID(agent_id),
										   std::bind(&LLPanelGroupBulk::addUserCallback,
													 this, _1, _2));
