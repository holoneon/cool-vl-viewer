...
LLPanelEditWearable::~LLPanelEditWearable()
{
	...
	for (param_map_t::iterator iter = mColorList.begin();
...
void LLPanelEditWearable::setSubpart(ESubpart subpart)
{
	...
	{
		LLButton* btnp = getChild<LLButton>(iter->second->mButtonName.c_str(),
											true, false);
		if (btnp)
		{
			btnp->setToggleState(subpart == iter->first);
		}
	}

	LLSubpart* partp = get_ptr_in_map(mSubpartList, (ESubpart)subpart);
	if (partp && isAgentAvatarValid())
	{
		...
		if (itemp)
		{
			perm_mask = itemp->getPermissions().getMaskOwner();
			is_complete = itemp->isFinished();
			if (!is_complete)
			{
				// Trigger a force-fetching. HB
		...
		for (LLViewerVisualParam* prmp =
				(LLViewerVisualParam*)gAgentAvatarp->getFirstVisualParam();
			 prmp;
			 prmp = (LLViewerVisualParam*)gAgentAvatarp->getNextVisualParam())
		{
			if (!prmp || prmp->getID() == -1 || !prmp->isTweakable() ||
				prmp->getEditGroup() != partp->mEditGroup ||
				!(prmp->getSex() & avatar_sex))
			{
				continue;
			}
			...
			param_name = prmp->getName();
			LLStringUtil::toLower(param_name);
			if (param_name.find("wrinkles") != std::string::npos &&
				(!LLTexLayerSet::sAllowFaceWrinkles || prmp->getID() != 163))
			{
				continue;
			}

			// Check for duplicates
			llassert(sorted_params.find(-prmp->getDisplayOrder()) ==
						sorted_params.end());

			// Negative getDisplayOrder() to make lowest order the highest
			// priority
			sorted_params.emplace(-prmp->getDisplayOrder(),
								  LLFloaterCustomize::editable_param(editable,
																	 prmp));
		}
		LLJoint* jointp = gAgentAvatarp->getJoint(partp->mTargetJointKey);
		...
		gMorphViewp->setCameraTargetOffset(partp->mTargetOffset);
		gMorphViewp->setCameraOffset(partp->mCameraOffset);
...
void LLPanelEditWearable::onColorCommit(LLUICtrl* ctrl, void* userdata)
{
	...
	{
		param_map_t::const_iterator cl_itr =
...
void LLPanelEditWearable::draw()
{
	...
	if (has_wearable)
	{
		itemp = gAgentWearables.getWearableInventoryItem(mType, mLayer);
		if (itemp)
		{
	...
	if (has_wearable && !is_modifiable)
	{
		if (mTitleNoModify)
		{
			...
			// *TODO:Translate
			mTitleNoModify->setTextArg("[DESC]", itemp ? itemp->getName()
													   : mWearable->getName());
		}
	...
	else if (has_wearable && is_modifiable)
	{
		if (mTitle)
		{
			...
			mTitle->setTextArg("[DESC]", itemp ? itemp->getName()
											   : mWearable->getName());
		}
		...
		for (param_map_t::iterator iter = mTextureList.begin(),
		...
		for (param_map_t::iterator iter = mInvisibilityList.begin(),
