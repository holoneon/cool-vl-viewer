...
//static
void* LLPanelGroupRoles::createTab(void* datap)
{
	LLUUID* group_id = (LLUUID*)datap;
...
//static
void LLPanelGroupRoles::onClickSubTab(void* datap, bool from_click)
{
	LLPanelGroupRoles* self = (LLPanelGroupRoles*)datap;
	if (self)
	{
...
bool LLPanelGroupRoles::attemptTransition()
{
	...
	gNotifications.add("PanelGroupApply", args, LLSD(),
					   std::bind(&LLPanelGroupRoles::handleNotifyCallback,
								 this, _1, _2));
	mHasModal = true;
	...
	if (!apply(apply_mesg))
	{
		...
		if (!apply_mesg.empty())
		{
			...
			gNotifications.add("GenericAlert", args, LLSD(),
							   std::bind(&LLPanelGroupRoles::onModalClose,
										 this, _1, _2));
		}
		// Skip switching tabs.
		return false;
...
//static
void LLPanelGroupSubTab::onSearchKeystroke(LLLineEditor* caller, void* datap)
{
	LLPanelGroupSubTab* self = (LLPanelGroupSubTab*)datap;
	if (self)
	{
...
//static
void LLPanelGroupSubTab::onClickSearch(void* datap)
{
	LLPanelGroupSubTab* self = (LLPanelGroupSubTab*)datap;
	if (self)
	{
...
//static
void LLPanelGroupSubTab::onClickShowAll(void* datap)
{
	LLPanelGroupSubTab* self = (LLPanelGroupSubTab*)datap;
	if (self)
	{
...
void LLPanelGroupSubTab::buildActionsList(LLScrollListCtrl* ctrlp,
										  U64 allowed_by_some,
										  U64 allowed_by_all,
										  strings_map_t& icons,
										  ...
										  bool is_owner_role)
{
	...
	{
		buildActionCategory(ctrlp, allowed_by_some, allowed_by_all,
							gGroupMgr.mRoleActionSets[i], icons,
							commit_callback, show_all, filter, is_owner_role);
	}
}

void LLPanelGroupSubTab::buildActionCategory(LLScrollListCtrl* ctrlp,
											 U64 allowed_by_some,
											 U64 allowed_by_all,
											 ...
											 bool is_owner_role)
{
	...
	strings_map_t::iterator iter = icons.find("folder");
	if (iter != icons.end())
	{
		columns[0]["type"] = "icon";
	...
	LLScrollListItem* title_row = ctrlp->addElement(row, ADD_BOTTOM,
	...
	for (U32 i = 0, count = action_set->mActions.size(); i < count; ++i)
	{
		...
		{
			strings_map_t::iterator iter = icons.find("full");
			if (iter != icons.end())
			{
				columns[column_index]["column"] = "checkbox";
		...
		{
			strings_map_t::iterator iter = icons.find("partial");
			if (iter != icons.end())
			{
				columns[column_index]["column"] = "checkbox";
				columns[column_index]["type"] = "icon";
		...
		LLScrollListItem* itemp = ctrlp->addElement(row, ADD_BOTTOM, rap);
		LLScrollListCheck* check_cell =
			(LLScrollListCheck*)itemp->getColumn(check_box_index);
		LLCheckBoxCtrl* checkp = check_cell->getCheckBox();
		checkp->setEnabled(can_change_actions);
		checkp->setCommitCallback(commit_callback);
		checkp->setCallbackUserData(ctrlp->getCallbackUserData());
		checkp->setToolTip(checkp->getLabel());

		if (show_all)
		{
			checkp->setTentative(false);
			checkp->set((allowed_by_some & rap->mPowerBit) != 0);
		}
		else
		{
			checkp->set(true);
			checkp->setTentative(!show_full_strength);
		}
	...
	{
		S32 title_index = ctrlp->getItemIndex(title_row);
		ctrlp->deleteSingleItem(title_index);
	}
}
...
//static
void* LLPanelGroupMembersSubTab::createTab(void* datap)
{
	LLUUID* group_id = (LLUUID*)datap;
...
bool LLPanelGroupMembersSubTab::postBuildSubTab(LLView* rootp)
{
	// Upcast parent so we can ask it for sibling controls.
	LLPanelGroupRoles* parentp = (LLPanelGroupRoles*)rootp;

	mHeader = parentp->getChild<LLPanel>("members_header", true, false);
	mFooter = parentp->getChild<LLPanel>("members_footer", true, false);

	mMembersList = parentp->getChild<LLNameListCtrl>("member_list",
													 true, false);
	mAssignedRolesList =
		parentp->getChild<LLScrollListCtrl>("member_assigned_roles", true,
										    false);
	mAllowedActionsList =
		parentp->getChild<LLScrollListCtrl>("member_allowed_actions", true,
										    false);
	...
	LLButton* btnp = parentp->getChild<LLButton>("member_invite",
												 true, false);
	if (btnp)
	{
		btnp->setClickedCallback(onInviteMember);
		btnp->setCallbackUserData(this);
		btnp->setEnabled(gAgent.hasPowerInGroup(mGroupID, GP_MEMBER_INVITE));
	}

	mEjectBtn = parentp->getChild<LLButton>("member_eject", true, false);
	if (mEjectBtn)
	{
	...
	mBanBtn = parentp->getChild<LLButton>("member_ban", true, false);
	if (mBanBtn)
	{
...
//static
void LLPanelGroupMembersSubTab::onMemberSelect(LLUICtrl*, void* datap)
{
	LLPanelGroupMembersSubTab* self = (LLPanelGroupMembersSubTab*)datap;
	if (self)
	{
...
void LLPanelGroupMembersSubTab::handleMemberSelect()
{
	...
	LLScrollListCtrl::items_vec_t selection = mMembersList->getAllSelected();
	if (selection.empty())
	{
		return;
	}
		 ...
		 iter != end; ++iter)
	{
		...
		if (gdatap->getRoleData(role_id, rd))
		{
			...
			LLScrollListItem* itemp = mAssignedRolesList->addElement(row);

			// Extract the checkbox that was created.
			LLScrollListCheck* check_cell;
			check_cell = (LLScrollListCheck*)itemp->getColumn(0);
			LLCheckBoxCtrl* checkp = check_cell->getCheckBox();
			checkp->setCommitCallback(onRoleCheck);
			checkp->setCallbackUserData(this);
			checkp->set(count > 0);
			checkp->setTentative(count > 0 &&
								 count != (S32)selected_members.size());
...
//static
void LLPanelGroupMembersSubTab::onMemberDoubleClick(void* datap)
{
	LLPanelGroupMembersSubTab* self = (LLPanelGroupMembersSubTab*)datap;
...
//static
void LLPanelGroupMembersSubTab::onInviteMember(void* datap)
{
	LLPanelGroupMembersSubTab* self = (LLPanelGroupMembersSubTab*)datap;
	if (self)
	{
...
void LLPanelGroupMembersSubTab::onEjectMembers(void* datap)
{
	LLPanelGroupMembersSubTab* self = (LLPanelGroupMembersSubTab*)datap;
	if (self)
	{
...
//static
void LLPanelGroupMembersSubTab::onRoleCheck(LLUICtrl* ctrlp, void* datap)
{
	LLPanelGroupMembersSubTab* self = (LLPanelGroupMembersSubTab*)datap;
	LLCheckBoxCtrl* checkp = (LLCheckBoxCtrl*)ctrlp;
	if (!checkp || !self || !self->mAssignedRolesList)
	{
		return;
	}
...
bool LLPanelGroupMembersSubTab::apply(std::string& mesg)
{
	...
	gNotifications.add("AddGroupOwnerWarning", args, LLSD(),
					   std::bind(&LLPanelGroupMembersSubTab::addOwnerCB, this,
								 _1, _2));
	return true;
}
...
void LLPanelGroupMembersSubTab::onBanMember(void* datap)
{
	LLPanelGroupMembersSubTab* self = (LLPanelGroupMembersSubTab*)datap;
	if (self)
	{
...
//static
void* LLPanelGroupRolesSubTab::createTab(void* datap)
{
	LLUUID* group_id = (LLUUID*)datap;
...
bool LLPanelGroupRolesSubTab::postBuildSubTab(LLView* rootp)
{
	// Upcast parent so we can ask it for sibling controls.
	LLPanelGroupRoles* parentp = (LLPanelGroupRoles*)rootp;
	if (!parentp) return false;

	mHeader = parentp->getChild<LLPanel>("roles_header", true, false);
	mFooter = parentp->getChild<LLPanel>("roles_footer", true, false);

	mRolesList = parentp->getChild<LLScrollListCtrl>("role_list", true, false);
	mAssignedMembersList =
		parentp->getChild<LLNameListCtrl>("role_assigned_members", true,
										  false);
	mAllowedActionsList	=
		parentp->getChild<LLScrollListCtrl>("role_allowed_actions", true,
											false);

	mRoleName = parentp->getChild<LLLineEditor>("role_name", true, false);
	mRoleTitle = parentp->getChild<LLLineEditor>("role_title", true, false);
	mRoleDescription = parentp->getChild<LLTextEditor>("role_description",
													   true, false);

	mMemberVisibleCheck =
		parentp->getChild<LLCheckBoxCtrl>("role_visible_in_list", true, false);
	...
	mCreateRoleButton = parentp->getChild<LLButton>("role_create", true,
													false);
	...
	mDeleteRoleButton = parentp->getChild<LLButton>("role_delete", true,
													false);
												  ...
												  std::string roletitle)
{
	...
	if (rolename.find(mSearchFilter) != std::string::npos)
	{
		return true;
	}
	LLStringUtil::toLower(roletitle);
	return roletitle.find(mSearchFilter) != std::string::npos;
}
...
//static
void LLPanelGroupRolesSubTab::onAssignedMemberDoubleClick(void* datap)
{
	LLPanelGroupRolesSubTab* self = (LLPanelGroupRolesSubTab*)datap;
...
//static
void LLPanelGroupRolesSubTab::onRoleSelect(LLUICtrl* ctrlp, void* datap)
{
	LLPanelGroupRolesSubTab* self = (LLPanelGroupRolesSubTab*)datap;
	if (self)
	{
...
void LLPanelGroupRolesSubTab::handleRoleSelect()
{
	...
	// Check if there is anything selected.
	LLScrollListItem* itemp = mRolesList->getFirstSelected();
	if (!itemp)
	{
		setFooterEnabled(false);
		return;
	...
	LLRoleData rd;
	if (gdatap->getRoleData(itemp->getUUID(), rd))
	{
		bool is_owner_role = (gdatap->mOwnerRole == itemp->getUUID());
		if (is_owner_role)
		{
...
void LLPanelGroupRolesSubTab::buildMembersList()
{
	...
	else
	{
		...
		rit = gdatap->mRoles.find(itemp->getUUID());
...
//static
void LLPanelGroupRolesSubTab::onActionCheck(LLUICtrl* ctrlp, void* datap)
{
	LLPanelGroupRolesSubTab* self = (LLPanelGroupRolesSubTab*)datap;
	LLCheckBoxCtrl* checkp = (LLCheckBoxCtrl*)ctrlp;
	if (checkp && self)
	{
		self->handleActionCheck(checkp);
	}
}
...
void LLPanelGroupRolesSubTab::handleActionCheck(LLCheckBoxCtrl* checkp,
												bool force)
{
	if (!mAssignedMembersList || !mRolesList) return;
	...
	LLRoleData rd;
	LLSD args;
	bool is_enabling_ability = checkp->get();
		...
		(GP_ROLE_ASSIGN_MEMBER == power || GP_ROLE_CHANGE_ACTIONS == power))
	{
		...
		if (gdatap->getRoleData(role_id, rd))
		{
			...
			gNotifications.add(warning, args, LLSD(),
							   std::bind(&LLPanelGroupRolesSubTab::addActionCB,
										 this, _1, _2, checkp));
		}
		else
		{
	...
	if (power == GP_GROUP_BAN_ACCESS)
	{
		...
		if (gdatap->getRoleData(role_id, rd))
		{
			...
			LLScrollListCtrl::items_vec_t all_data =
										  ...
										  LLCheckBoxCtrl* checkp)
{
	...
	if (0 == option)
	{
		...
		handleActionCheck(checkp, force_add);
	}
	return false;
}

//static
void LLPanelGroupRolesSubTab::onPropertiesKey(LLLineEditor*, void* datap)
{
	LLPanelGroupRolesSubTab* self = (LLPanelGroupRolesSubTab*)datap;
	if (self)
	{
...
//static
void LLPanelGroupRolesSubTab::onDescriptionFocus(LLFocusableElement*,
												 void* datap)
{
	LLPanelGroupRolesSubTab* self = (LLPanelGroupRolesSubTab*)datap;
	if (self)
...
//static
void LLPanelGroupRolesSubTab::onDescriptionCommit(LLUICtrl*, void* datap)
{
	LLPanelGroupRolesSubTab* self = (LLPanelGroupRolesSubTab*)datap;
	if (self)
	{
...
//static
void LLPanelGroupRolesSubTab::onMemberVisibilityChange(LLUICtrl* ctrlp,
													   void* datap)
{
	LLPanelGroupRolesSubTab* self = (LLPanelGroupRolesSubTab*)datap;
	LLCheckBoxCtrl* checkp = (LLCheckBoxCtrl*)ctrlp;
	if (checkp && self)
	{
		self->handleMemberVisibilityChange(checkp->get());
	}
}
...
void LLPanelGroupRolesSubTab::handleMemberVisibilityChange(bool value)
{
	...
	if (value)
	{
		gdatap->addRolePower(itemp->getUUID(), GP_MEMBER_VISIBLE_IN_DIR);
	}
	else
	{
		gdatap->removeRolePower(itemp->getUUID(), GP_MEMBER_VISIBLE_IN_DIR);
	}
}

//static
void LLPanelGroupRolesSubTab::onCreateRole(void* datap)
{
	LLPanelGroupRolesSubTab* self = (LLPanelGroupRolesSubTab*)datap;
	if (self)
	{
...
//static
void LLPanelGroupRolesSubTab::onDeleteRole(void* datap)
{
	LLPanelGroupRolesSubTab* self = (LLPanelGroupRolesSubTab*)datap;
	if (self)
	{
...
void LLPanelGroupRolesSubTab::handleDeleteRole()
{
	...
	if (itemp->getUUID().isNull() || itemp->getUUID() == gdatap->mOwnerRole)
	{
		LLSD args;
	...
	gdatap->deleteRole(itemp->getUUID());
...
void LLPanelGroupRolesSubTab::saveRoleChanges()
{
	...
	if (gdatap && mRolesList && mHasRoleChange)
	{
		...
		LLScrollListItem* itemp = mRolesList->addElement(row, ADD_BOTTOM,
														 this);
		itemp->setSelected(true);
...
//static
void* LLPanelGroupActionsSubTab::createTab(void* datap)
{
	LLUUID* group_id = (LLUUID*)datap;
...
bool LLPanelGroupActionsSubTab::postBuildSubTab(LLView* rootp)
{
	// Upcast parent so we can ask it for sibling controls.
	LLPanelGroupRoles* parentp = (LLPanelGroupRoles*)rootp;

	mHeader = parentp->getChild<LLPanel>("actions_header", true, false);
	mFooter = parentp->getChild<LLPanel>("actions_footer", true, false);

	mActionDescription = parentp->getChild<LLTextEditor>("action_description",
														 true, false);
	if (!mActionDescription) return false;

	mActionList = parentp->getChild<LLScrollListCtrl>("action_list",
													  true, false);
	if (!mActionList) return false;

	mActionRoles = parentp->getChild<LLScrollListCtrl>("action_roles",
													   true, false);
	if (!mActionRoles) return false;

	mActionMembers = parentp->getChild<LLNameListCtrl>("action_members",
													   true, false);
...
//static
void LLPanelGroupActionsSubTab::onActionSelect(LLUICtrl*, void* datap)
{
	LLPanelGroupActionsSubTab* self = (LLPanelGroupActionsSubTab*)datap;
	if (self)
	{
...
void LLPanelGroupActionsSubTab::handleActionSelect()
{
	...
	LLScrollListCtrl::items_vec_t selection = mActionList->getAllSelected();
	if (selection.empty()) return;
	...
	if (selection.size() == 1)
	{
		LLScrollListItem* itemp = selection[0];
		rap = (LLRoleAction*)(itemp->getUserdata());
...
//static
void* LLPanelGroupBanListSubTab::createTab(void* datap)
{
	LLUUID* group_id = (LLUUID*)datap;
...
bool LLPanelGroupBanListSubTab::postBuildSubTab(LLView* rootp)
{
	// Upcast parent so we can ask it for sibling controls.
	LLPanelGroupRoles* parentp = (LLPanelGroupRoles*)rootp;

	mHeader = parentp->getChild<LLPanel>("banlist_header", true, false);
	mFooter = parentp->getChild<LLPanel>("banlist_footer", true, false);

	mBanList = parentp->getChild<LLNameListCtrl>("ban_list", true, false);

	mCreateBanButton = parentp->getChild<LLButton>("ban_create", true, false);
	mDeleteBanButton = parentp->getChild<LLButton>("ban_delete", true, false);
	mRefreshBanListButton = parentp->getChild<LLButton>("ban_refresh",
														true, false);
	...
	mBanNotSupportedText = parentp->getChild<LLTextBox>("ban_not_supported",
													 	true, false);
	mBanCountText = parentp->getChild<LLTextBox>("ban_count", true, false);
	if (mBanCountText)
	{
...
void LLPanelGroupBanListSubTab::onBanEntrySelect(LLUICtrl*, void* datap)
{
	LLPanelGroupBanListSubTab* self = (LLPanelGroupBanListSubTab*)datap;
	if (self)
	{
...
void LLPanelGroupBanListSubTab::onCreateBanEntry(void* datap)
{
	LLPanelGroupBanListSubTab* self = (LLPanelGroupBanListSubTab*)datap;
	if (self)
	{
...
void LLPanelGroupBanListSubTab::onDeleteBanEntry(void* datap)
{
	LLPanelGroupBanListSubTab* self = (LLPanelGroupBanListSubTab*)datap;
	if (self)
	{
...
void LLPanelGroupBanListSubTab::handleDeleteBanEntry()
{
	...
	LLScrollListCtrl::items_vec_t selection = mBanList->getAllSelected();
	if (selection.empty())
	{
		return;
	}
...
void LLPanelGroupBanListSubTab::onRefreshBanList(void* datap)
{
	LLPanelGroupBanListSubTab* self = (LLPanelGroupBanListSubTab*)datap;
	if (self)
	{
...
//static
void LLPanelGroupBanListSubTab::onBanListMemberDoubleClick(void* datap)
{
	LLPanelGroupBanListSubTab* self = (LLPanelGroupBanListSubTab*)datap;
