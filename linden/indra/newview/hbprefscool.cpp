...
private:
	...
	LLSD					mDictsList;
	strings_set_t			mInstalledDicts;

	strings_set_t			mWarnedAfterRestart;
...
# define EXT_EDIT "\"C:\\Program Files\\Notepad++\\notepad++.exe\" %s"
	...
	control->getSignal()->connect(std::bind(&HBPrefsCoolImpl::setDirty, this));
...
void HBPrefsCoolImpl::draw()
{
	...
	if (mIsDirty)
	{
		...
		combo->removeall();
		for (strings_set_t::iterator it = mInstalledDicts.begin();
		...
		if (combo)
		{
			...
			if (!label.empty())
			{
				...
				gCoros.launch("HBPrefsCoolImpl::fetchDictionaryCoro(aff)",
							  std::bind(&HBPrefsCoolImpl::fetchDictionaryCoro,
										dict_name + ".aff"));
