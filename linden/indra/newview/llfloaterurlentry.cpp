...
void LLFloaterURLEntry::onBtnOK(void* userdata)
{
	...
	if (scheme == "http" || scheme == "https")
	{
		gCoros.launch("LLFloaterURLEntry::getMediaTypeCoro",
					  std::bind(&LLFloaterURLEntry::getMediaTypeCoro,
										 ...
										 LLHandle<LLFloater> handle)
{
	...
	if (status)
	{
		...
		else if (headers.has(HTTP_IN_HEADER_X_CONTENT_TYPE_OPTIONS))
		{
			std::string val = headers[HTTP_IN_HEADER_X_CONTENT_TYPE_OPTIONS];
			if (val == HTTP_NOSNIFF)
			{
				// Site does not permit 'sniffing' mime type, default to
				// either html or plain.
				resolved_mime_type = HTTP_CONTENT_TEXT_HTML;
			}
		}
	}
...
void LLFloaterURLEntry::onBtnClear(void* userdata)
{
	gNotifications.add("ConfirmClearMediaUrlList", LLSD(), LLSD(),
					   std::bind(&LLFloaterURLEntry::callback_clear_url_list,
