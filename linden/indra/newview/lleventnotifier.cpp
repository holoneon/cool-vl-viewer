			 ...
			 iter != mEventNotifications.end();)
		{
			...
			if (np && np->getEventDate() < alert_time)
			{
				...
				gNotifications.add("EventNotification", args, LLSD(),
								   std::bind(&LLEventNotification::handleResponse,
											 np, _1, _2));
...
void LLEventNotifier::add(LLEventInfo& event_info)
{
	...
	msg->newMessage(_PREHASH_EventNotificationAddRequest);
	msg->nextBlockFast(_PREHASH_AgentData);
	msg->addUUIDFast(_PREHASH_AgentID, gAgentID );
	msg->addUUIDFast(_PREHASH_SessionID, gAgentSessionID);
	msg->nextBlock(_PREHASH_EventData);
	msg->addU32(_PREHASH_EventID, event_info.mID);
...
void LLEventNotifier::remove(U32 event_id)
{
	...
	LLMessageSystem* msg = gMessageSystemp;
	msg->newMessage(_PREHASH_EventNotificationRemoveRequest);
	msg->nextBlockFast(_PREHASH_AgentData);
	msg->addUUIDFast(_PREHASH_AgentID, gAgentID );
	msg->addUUIDFast(_PREHASH_SessionID, gAgentSessionID);
	msg->nextBlock(_PREHASH_EventData);
	msg->addU32(_PREHASH_EventID, event_id);
	gAgent.sendReliableMessage();
