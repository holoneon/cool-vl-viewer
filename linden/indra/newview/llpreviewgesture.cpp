	...
	else
	{
		if (!mSaveDialogShown)
		{
			...
			gNotifications.add("SaveChanges", LLSD(), LLSD(),
							   std::bind(&LLPreviewGesture::handleSaveChangesDialog,
										 this, _1, _2));
		}
		return false;
	}
...
LLPreviewGesture::~LLPreviewGesture()
{
	...
	LLScrollListCtrl::items_vec_t data_list = mStepList->getAllData();
	for (S32 i = 0, count = data_list.size(); i < count; ++i)
	{
		LLScrollListItem* itemp = data_list[i];	// Cannot be NULL. HB
		LLGestureStep* stepp = (LLGestureStep*)itemp->getUserdata();
		if (stepp)
		{
			delete stepp;
		}
...
void LLPreviewGesture::refresh()
{
	...
	if (have_step)
	{
		...
		LLGestureStep* stepp = (LLGestureStep*)step_item->getUserdata();
		EStepType type = stepp->getType();

		switch (type)
		{
			case STEP_ANIMATION:
			{
				LLGestureStepAnimation* anim_stepp =
					(LLGestureStepAnimation*)stepp;
				optionstext = getString("step_anim");
				mAnimationCombo->setVisible(true);
				mAnimationRadio->setVisible(true);
				mAnimationRadio->setSelectedIndex((anim_stepp->mFlags &
												   ANIM_FLAG_STOP) ? 1 : 0);
				mAnimationCombo->setCurrentByID(anim_stepp->mAnimAssetID);
				break;
			}

			case STEP_SOUND:
			{
				LLGestureStepSound* sound_stepp = (LLGestureStepSound*)stepp;
				optionstext = getString("step_sound");
				mSoundCombo->setVisible(true);
				mSoundCombo->setCurrentByID(sound_stepp->mSoundAssetID);
				break;
			}

			case STEP_CHAT:
			{
				LLGestureStepChat* chat_stepp = (LLGestureStepChat*)stepp;
				optionstext = getString("step_chat");
				mChatEditor->setVisible(true);
				mChatEditor->setText(chat_stepp->mChatText);
				break;
			}

			case STEP_WAIT:
			{
				LLGestureStepWait* wait_stepp = (LLGestureStepWait*)stepp;
				optionstext = getString("step_wait");
				mWaitAnimCheck->setVisible(true);
				mWaitAnimCheck->set(wait_stepp->mFlags & WAIT_FLAG_ALL_ANIM);
				mWaitTimeCheck->setVisible(true);
				mWaitTimeCheck->set(wait_stepp->mFlags & WAIT_FLAG_TIME);
				mWaitTimeEditor->setVisible(true);
				mWaitTimeEditor->setText(llformat("%.1f",
												  wait_stepp->mWaitSeconds));
			}
...
void LLPreviewGesture::initDefaultGesture()
{
	...
	LLGestureStepChat* chat_stepp = (LLGestureStepChat*)item->getUserdata();
	chat_stepp->mChatText = "Hello, avatar!";
	updateLabel(item);
...
void LLPreviewGesture::loadUIFromGesture(LLMultiGesture* gesture)
{
	...
	{
		LLGestureStep* stepp = gesture->mSteps[i];
		if (!stepp) continue;	// Paranoia

		LLGestureStep* new_stepp = NULL;

		switch (stepp->getType())
		{
			case STEP_ANIMATION:
			{
				LLGestureStepAnimation* anim_stepp =
					(LLGestureStepAnimation*)stepp;
				LLGestureStepAnimation* new_anim_stepp;
				new_anim_stepp = new LLGestureStepAnimation(*anim_stepp);
				new_stepp = new_anim_stepp;
				break;
			}
			...
			{
				LLGestureStepSound* sound_stepp = (LLGestureStepSound*)stepp;
				LLGestureStepSound* new_sound_stepp;
				new_sound_stepp = new LLGestureStepSound(*sound_stepp);
				new_stepp = new_sound_stepp;
				break;
			}
			...
			{
				LLGestureStepChat* chat_stepp = (LLGestureStepChat*)stepp;
				LLGestureStepChat* new_chat_stepp;
				new_chat_stepp = new LLGestureStepChat(*chat_stepp);
				new_stepp = new_chat_stepp;
				break;
			}
			...
			{
				LLGestureStepWait* wait_stepp = (LLGestureStepWait*)stepp;
				LLGestureStepWait* new_wait_stepp;
				new_wait_stepp = new LLGestureStepWait(*wait_stepp);
				new_stepp = new_wait_stepp;
				break;
			}
			...
			if (!agent_url.empty() && !task_url.empty())
			{
				...
				if (mObjectUUID.isNull() && !agent_url.empty())
				{
					...
					info = std::make_shared<LLBufferedAssetUploadInfo>(mItemUUID,
																	   ...
																	   buffer,
																	   std::bind(&LLPreviewGesture::finishInventoryUpload,
																				 _1, _2));
					url = &agent_url;
					delayed_upload = true;
				}
				...
				{
					info = std::make_shared<LLBufferedAssetUploadInfo>(mObjectUUID,
																	   mItemUUID,
																	   LLAssetType::AT_GESTURE,
																	   buffer,
																	   nullptr);
		...
		switch (stepp->getType())
		{
			case STEP_ANIMATION:
			{
				...
				LLGestureStepAnimation* anim_stepp =
					(LLGestureStepAnimation*)stepp;
				LLGestureStepAnimation* new_anim_stepp =
					new LLGestureStepAnimation(*anim_stepp);
				gesture->mSteps.push_back(new_anim_stepp);
				break;
			}
			...
			case STEP_SOUND:
			{
				...
				LLGestureStepSound* sound_stepp = (LLGestureStepSound*)stepp;
				LLGestureStepSound* new_sound_stepp =
					new LLGestureStepSound(*sound_stepp);
				gesture->mSteps.push_back(new_sound_stepp);
				break;
			}
			...
			case STEP_CHAT:
			{
				...
				LLGestureStepChat* chat_stepp = (LLGestureStepChat*)stepp;
				LLGestureStepChat* new_chat_stepp =
					new LLGestureStepChat(*chat_stepp);
				gesture->mSteps.push_back(new_chat_stepp);
				break;
			}
			...
			case STEP_WAIT:
			{
				...
				LLGestureStepWait* wait_stepp = (LLGestureStepWait*)stepp;
				LLGestureStepWait* new_wait_stepp =
					new LLGestureStepWait(*wait_stepp);
				gesture->mSteps.push_back(new_wait_stepp);
				break;
			}
...
{
	LLGestureStep* stepp = (LLGestureStep*)item->getUserdata();
	if (!stepp) return;	// Paranoia

	LLScrollListCell* cell = item->getColumn(0);
	if (!cell) return;	// Paranoia

	LLScrollListText* text_cell = (LLScrollListText*)cell;
	std::string label = stepp->getLabel();
...
void LLPreviewGesture::onCommitAnimation(LLUICtrl* ctrl, void* data)
{
	...
	{
		LLGestureStep* stepp = (LLGestureStep*)step_item->getUserdata();
		if (stepp->getType() == STEP_ANIMATION)
		{
			...
			{
				anim_stepp->mAnimName.clear();
				anim_stepp->mAnimAssetID.setNull();
			}
			else
			{
				anim_stepp->mAnimName = self->mAnimationCombo->getSimple();
				anim_stepp->mAnimAssetID =
					self->mAnimationCombo->getCurrentID();
			}
#if 0
			anim_stepp->mFlags = 0x0;
#endif
...
void LLPreviewGesture::onCommitAnimationTrigger(LLUICtrl* ctrl, void *data)
{
	...
	if (step_item)
	{
		...
		if (stepp->getType() == STEP_ANIMATION)
		{
			...
			{
				// Start
				anim_stepp->mFlags &= ~ANIM_FLAG_STOP;
			}
			else
			{
				// Stop
...
void LLPreviewGesture::onCommitSound(LLUICtrl* ctrl, void* data)
{
	...
	{
		LLGestureStep* stepp = (LLGestureStep*)step_item->getUserdata();
		if (stepp->getType() == STEP_SOUND)
		{
			// Assign the sound name
			LLGestureStepSound* sound_stepp = (LLGestureStepSound*)stepp;
			sound_stepp->mSoundName = self->mSoundCombo->getSimple();
			sound_stepp->mSoundAssetID = self->mSoundCombo->getCurrentID();
			sound_stepp->mFlags = 0x0;
...
void LLPreviewGesture::onCommitChat(LLUICtrl* ctrl, void* data)
{
	...
	LLGestureStep* stepp = (LLGestureStep*)step_item->getUserdata();
	if (stepp->getType() != STEP_CHAT) return;

	LLGestureStepChat* chat_stepp = (LLGestureStepChat*)stepp;
	chat_stepp->mChatText = self->mChatEditor->getText();
	chat_stepp->mFlags = 0x0;
...
void LLPreviewGesture::onCommitWaitTime(LLUICtrl* ctrl, void* data)
{
	...
	LLGestureStep* stepp = (LLGestureStep*)step_item->getUserdata();
	if (stepp && stepp->getType() == STEP_WAIT)
	{
...
LLScrollListItem* LLPreviewGesture::addStep(const EStepType step_type)
{
	...
	LLGestureStep* stepp = NULL;
	switch (step_type)
	{
		case STEP_ANIMATION:
			stepp = new LLGestureStepAnimation();
			break;
		case STEP_SOUND:
			stepp = new LLGestureStepSound();
			break;
		case STEP_CHAT:
			stepp = new LLGestureStepChat();
			break;
		case STEP_WAIT:
			stepp = new LLGestureStepWait();
			break;
		default:
			llerrs << "Unknown stepp type: " << (S32)step_type << llendl;
	}

	// Create an enabled item with this step
	LLSD row;
	row["columns"][0]["value"] = stepp->getLabel();
	row["columns"][0]["font"] = "SANSSERIF_SMALL";
	LLScrollListItem* step_item = mStepList->addElement(row);
	if (step_item)	// Out of memory...
	{
		step_item->setUserdata(stepp);
