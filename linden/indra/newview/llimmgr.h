...
public:
	...
	void addMessage(const LLUUID& session_id, const LLUUID& target_id,
					...
					bool link_name = false, bool is_bot = false);
	...
	LLUUID addSession(const std::string& name, EInstantMessage dialog,
					  ...
					  const LLSD& voice_call_info = LLSD());
	...
	// calls
	LL_INLINE LLUUID addP2PSession(const std::string& name,
								   const LLUUID& other_participant_id,
								   const LLSD& voice_call_info)
	{
		return addSession(name, IM_NOTHING_SPECIAL, other_participant_id,
						  voice_call_info);
	}
...
private:
	...
	static void startP2PVoiceCoro(const std::string& url,
								  LLUUID temp_session_id, LLUUID creator_id,
								  LLUUID other_participant_id);
