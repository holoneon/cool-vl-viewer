...
LLGroupDropTarget::LLGroupDropTarget(const std::string& name,
									 const LLRect& rect,
									 LLPanelGroupNotices* panelp,
									 const LLUUID& group_id)
:	LLView(name, rect, false, FOLLOWS_ALL),
	mGroupNoticesPanel(panelp),
	mGroupID(group_id)
{
}
...
//static
void* LLPanelGroupNotices::createTab(void* datap)
{
	LLUUID* group_id = static_cast<LLUUID*>(datap);
...
void LLPanelGroupNotices::onClickRemoveAttachment(void* datap)
{
	LLPanelGroupNotices* self = (LLPanelGroupNotices*)datap;
...
//static
void LLPanelGroupNotices::onClickOpenAttachment(void* datap)
{
	LLPanelGroupNotices* self = (LLPanelGroupNotices*)datap;
	if (!self || !self->mInitOK) return;
...
void LLPanelGroupNotices::onClickSendMessage(void* datap)
{
	LLPanelGroupNotices* self = (LLPanelGroupNotices*)datap;
	if (!self || !self->mInitOK) return;
...
//static
void LLPanelGroupNotices::onClickNewMessage(void* datap)
{
	LLPanelGroupNotices* self = (LLPanelGroupNotices*)datap;
	if (!self || !self->mInitOK) return;
...
void LLPanelGroupNotices::onClickRefreshNotices(void* datap)
{
	LLPanelGroupNotices* self = (LLPanelGroupNotices*)datap;
	if (!self || !self->mInitOK) return;
													   ...
													   void**)
{
	...
	LLPanelGroupNotices* self = it->second;
	if (!self)
	{
		llinfos << "Group Panel Notices " << group_id
				<< " no longer in existence." << llendl;
...
void LLPanelGroupNotices::processNotices(LLMessageSystem* msg)
{
	...
	S32 count = msg->getNumberOfBlocks(_PREHASH_Data);
	for (S32 i = 0; i < count; ++i)
	{
		...
		msg->getString(_PREHASH_Data, _PREHASH_Subject, subj, i);
		msg->getString(_PREHASH_Data, _PREHASH_FromName, name, i);
		msg->getBool(_PREHASH_Data, _PREHASH_HasAttachment, has_attachment, i);
		msg->getU8(_PREHASH_Data, _PREHASH_AssetType, asset_type, i);
		msg->getU32(_PREHASH_Data, _PREHASH_Timestamp, timestamp, i);
...
void LLPanelGroupNotices::onSelectNotice(LLUICtrl*, void* datap)
{
	LLPanelGroupNotices* self = (LLPanelGroupNotices*)datap;
	if (!self || !self->mInitOK) return;

	LLScrollListItem* itemp = self->mNoticesList->getFirstSelected();
	if (!itemp) return;

	LLMessageSystem* msg = gMessageSystemp;
	msg->newMessage(_PREHASH_GroupNoticeRequest);
	msg->nextBlock(_PREHASH_AgentData);
	msg->addUUID(_PREHASH_AgentID, gAgentID);
	msg->addUUID(_PREHASH_SessionID, gAgentSessionID);
	msg->nextBlock(_PREHASH_Data);
	msg->addUUID(_PREHASH_GroupNoticeID, itemp->getUUID());
	gAgent.sendReliableMessage();

	LL_DEBUGS("GroupPanel") << "Item " << itemp->getUUID() << " selected."
							<< LL_ENDL;
}
...
void LLPanelGroupNotices::showNotice(const std::string& subject,
									 ...
									 LLOfferInfo* inventory_offerp)
{
	arrangeNoticeView(VIEW_PAST_NOTICE);

	if (mViewSubject)
	{
		mViewSubject->setText(subject);
	}
	...
	if (mViewMessage)
	{
		...
		LLSLURL::findSLURLs(mViewMessage);
	}
	...
	if (inventory_offerp)
	{
		mInventoryOffer = inventory_offerp;
		if (mViewInventoryIcon)
		{
			std::string icon_name =
