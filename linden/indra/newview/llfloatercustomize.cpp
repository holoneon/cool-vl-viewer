...
uuid_list_t LLFloaterCustomize::sFetchingRequests;

struct WearablePanelData
{
	WearablePanelData(LLFloaterCustomize* floaterp, LLWearableType::EType type)
	:	mFloater(floaterp),
		mType(type)
	{
	}
										  ...
										  void* userdata)
{
	if (isDirty())
	{
		...
		gNotifications.add("SaveClothingBodyChanges", LLSD(), LLSD(),
						   std::bind(&LLFloaterCustomize::onSaveDialog, this,
									 _1, _2));
		return;
	}
...
{
public:
	LLCurrentlyWorn() = default;
	~LLCurrentlyWorn() override = default;
...
void LLFloaterCustomize::updateInventoryUI()
{
	...
	for (S32 i = 0; i < LLWearableType::WT_COUNT; ++i)
	{
		...
		LLPanelEditWearable* panelp = mWearablePanelList[i];
		if (panelp)
		{
			U32 index = panelp->getWearableIndex();
			itemp = gAgentWearables.getWearableInventoryItem(panelp->getType(),
															 index);
		}
		if (itemp)
		{
			...
			{
				all_complete = false;
				if (!sFetchingRequests.count(itemp->getUUID()))
				{
					itemp->fetchFromServer();
					sFetchingRequests.emplace(itemp->getUUID());
				}
			}
		...
		if (i == sCurrentWearableType)
		{
			if (panelp)
			{
				panelp->setUIPermissions(perm_mask, is_complete);
			}
			bool is_vis = panelp && itemp && is_complete &&
											...
											LLViewerWearable* wearablep)
{
	...
	bool is_complete = false;
	if (!wearablep && gAgentWearables.getWearableCount(type))
	{
		// Select the first layer
		wearablep = gAgentWearables.getViewerWearable(type, 0);
	}
	if (wearablep)
	{
		LLViewerInventoryItem* itemp =
			(LLViewerInventoryItem*)gInventory.getItem(wearablep->getItemID());
		if (itemp)
		{
			perm_mask = itemp->getPermissions().getMaskOwner();
			is_complete = itemp->isFinished();
			if (!is_complete && !sFetchingRequests.count(itemp->getUUID()))
			{
				itemp->fetchFromServer();
				sFetchingRequests.emplace(itemp->getUUID());
			}
	...
	setWearable(type, wearablep, perm_mask, is_complete);
}
