...
public:
	...
	// media rejected). When 'ask_user' is false, a simple check is done
	// against the current white and black lists, and no change is performed on
	// the LLViewerMediaImpl.
	static bool filterMedia(LLViewerMediaImpl* implp, bool ask_user = true);
...
public:
	...
	static strings_set_t	sMediaQueries;
	static strings_set_t	sAllowedMedia;
	static strings_set_t	sDeniedMedia;

private:
	// For the media filter implementation:
	static strings_map_t	sDNSlookups;
...
public:
	...
	LL_INLINE const std::string& getHomeURL() const		{ return mHomeURL; }
	const std::string& getCurrentMediaURL() const;
	...
	LL_INLINE const std::string& getMimeType() const	{ return mMimeType; }
	void scaleMouse(S32* mouse_x, S32* mouse_y);
	void scaleTextureCoords(const LLVector2& texture_coords, S32* x, S32* y);

	LL_INLINE const LLUUID& getMediaTextureID() const	{ return mTextureId; }
	...
	void setDisabled(bool disable, bool force_play_on_enable = false);
	LL_INLINE bool isMediaDisabled() const				{ return mIsDisabled; }

	LL_INLINE void setInNearbyMediaList(bool b)			{ mInNearbyMediaList = b; }
	LL_INLINE bool getInNearbyMediaList() const			{ return mInNearbyMediaList; }
	...
	static LLPluginClassMedia* newSourceFromMediaType(const std::string& media_type,
...
private:
	...
	void mimeDiscoveryCoro(const std::string& url);
