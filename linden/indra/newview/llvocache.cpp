...
static bool check_read(LLFile* infilep, U8* srcp, S32 bytes)
{
	// Note: eof() is true when getStream() is NULL, so there is no need to
	// test for the latter. HB
	return srcp && !infilep->eof() && infilep->read(srcp, bytes) == bytes;
}

static bool check_write(LLFile* outfilep, U8* dstp, S32 bytes)
{
	return dstp && outfilep->getStream() &&
		   outfilep->write(dstp, bytes) == bytes;
}
...
LLVOCacheEntry::LLVOCacheEntry(LLFile* infilep)
:	LLViewerOctreeEntryData(LLViewerOctreeEntry::LLVOCACHEENTRY),
	mBuffer(NULL),
	mUpdateFlags(-1),
	mState(INACTIVE),
	...
	mBSphereRadius(-1.f)
{
	...
	bool success = check_read(infilep, (U8*)data_buffer, 6 * sizeof(U32));
	if (success)
	{
		U32* ptr = data_buffer;
		mLocalID = *ptr++;
	...
	{
		mBuffer = new U8[size];
		success = check_read(infilep, mBuffer, size);
		if (success)
		{
...
//virtual
void LLVOCacheEntry::setOctreeEntry(LLViewerOctreeEntry* entryp)
{
	if (!entryp && mDP.getBufferSize() > 0)
	{
		LLUUID fullid;
		LLViewerObject::unpackUUID(&mDP, fullid, "ID");

		LLViewerObject* objp = gObjectList.findObject(fullid);
		if (objp && objp->mDrawable)
		{
			entryp = objp->mDrawable->getEntry();
		}
	}

	LLViewerOctreeEntryData::setOctreeEntry(entryp);
}
...
void LLVOCacheEntry::keepVisible()
{
	mKeepVisible = true;
	setVisible();
}

void LLVOCacheEntry::addChild(LLVOCacheEntry* entryp)
{
	if (!entryp || !entryp->getEntry() || entryp->getParentID() != mLocalID)
	{
		llassert(false);
		return;
	}

	mChildrenList.insert(entryp);

	// Update parent bbox
	if (getEntry() && isState(INACTIVE))
	{
		updateParentBoundingInfo(entryp);
		if (!mKeepVisible)
		{
			resetVisible();
		}
...
void LLVOCacheEntry::removeChild(LLVOCacheEntry* entryp)
{
	entryp->setParentID(0);

	set_t::iterator iter = mChildrenList.find(entryp);
	...
	if (LLFile::exists(mHeaderFileName))
	{
		...
		if (success)
		{
			...
			while (num_read++ < MAX_NUM_OBJECT_ENTRIES)
			{
				if (!entryp)
				{
					entryp = new HeaderEntryInfo();
				}
				success = check_read(&infile, (U8*)entryp,
				...
				if (entryp->mTime == INVALID_TIME)
				{
					continue; // An empty entry
				}

				entryp->mIndex = mNumEntries++;
				mHeaderEntryQueue.insert(entryp);
				mHandleEntryMap[entryp->mHandle] = entryp;
				entryp = NULL;
			}
			if (entryp)
			{
...
void LLVOCache::writeCacheHeader()
{
	...
	bool success = true;
	{
		...
		{
			HeaderEntryInfo* entryp = new HeaderEntryInfo();
			entryp->mTime = INVALID_TIME;
			for (S32 i = mNumEntries;
				 success && i < (S32)MAX_NUM_OBJECT_ENTRIES; ++i)
			{
				// Fill the cache with the default entry.
				success = check_write(&outfile, (U8*)entryp,
									  sizeof(HeaderEntryInfo));
			}
			delete entryp;
		}
...
bool LLVOCache::updateEntry(const HeaderEntryInfo* entryp)
{
	// NOT using "wb" here, since we seek to an entry to update it.
	LLFile outfile(mHeaderFileName, "r+b");
	S64 offset = entryp->mIndex * sizeof(HeaderEntryInfo) +
				 sizeof(HeaderMetaInfo);
	if (outfile.seek(offset) != offset)
	{
		llwarns << "Failed to seek to entry index " << entryp->mIndex
				<< llendl;
		return false;
	}

	if (!check_write(&outfile, (U8*)entryp, sizeof(HeaderEntryInfo)))
	{
		llwarns << "Failed to write entry at index " << entryp->mIndex
				<< llendl;
		return false;
	}
...
void LLVOCache::purgeEntries(U32 size)
{
	while (mHeaderEntryQueue.size() > size)
	{
		...
		HeaderEntryInfo* entryp = *iter;
		mHandleEntryMap.erase(entryp->mHandle);
		mHeaderEntryQueue.erase(iter);
		removeFromCache(entryp);
		delete entryp;
	}
	mNumEntries = mHandleEntryMap.size();
}
							 ...
							 bool removal_enabled)
{
	...
	if (iter == mHandleEntryMap.end()) // New entry
	{
		...
		entryp = new HeaderEntryInfo();
		entryp->mHandle = handle;
		entryp->mTime = time(NULL);
		entryp->mIndex = mNumEntries++;
		mHeaderEntryQueue.insert(entryp);
		mHandleEntryMap[handle] = entryp;
	}
	else
	{
		// Update access time.
		...
		// Resort
		mHeaderEntryQueue.erase(entryp);
	...
	// Update cache header
	if (!updateEntry(entryp))
	{
		llwarns << "Failed to update cache header index " << entryp->mIndex
...
{
	bool success;
	LLVOCacheEntry::map_t* entry_mapp = NULL;
	...
	else
	{
		...
		if (success)
		{
			entry_mapp = new LLVOCacheEntry::map_t();
			S32 num_entries;
			...
			if (success)
			{
				...
				{
					LLPointer<LLVOCacheEntry> entryp =
						new LLVOCacheEntry(&infile);
					if (!entryp->getLocalID())
					{
						success = infile.eof() && !entry_mapp->empty();
						if (!success)
						{
					...
					entry_mapp->emplace(entryp->getLocalID(), entryp);
				}
			}
		}
	}
	...
	LLVOCacheEntry::emap_t* extras_mapp = NULL;
	while (true)
	{
		LLVOCache::getInstance()->getObjectCacheFilename(mHandle, filename,
													     true);
		...
		while (success)
		{
			...
			extras_mapp = new LLVOCacheEntry::emap_t();
			for (S32 i = 0; i < num_entries && infile.good(); ++i)
			{
				...
				{
					extras_mapp->emplace(local_id, entry);
				}
				else
				{
	...
	if (is_main_thread())
	{
		LLViewerRegion::cacheLoadedCallback(mHandle, entry_mapp, extras_mapp);
	}
