										 ...
										 const LLUUID& thumbnail_id)
{
	...
	if (!url.empty())
	{
		...
		gCoros.launch("LLInventoryModel::createNewCategoryCoro",
					  std::bind(&LLInventoryModel::createNewCategoryCoro,
...
bool LLInventoryModel::hasMatchingDescendents(const LLUUID& id,
											  bool include_trash,
											  LLInventoryCollectFunctor& match)
{
	if (!include_trash && id == getTrashID())
	{
		return false;
	}

	cat_array_t* cats = get_ptr_in_map(mParentChildCategoryTree, id);
	for (cat_array_t::iterator it = cats->begin(), end = cats->end();
		 it != end; ++it)
	{
		LLViewerInventoryCategory* catp = *it;
		if (match(catp, NULL))
		{
			return true;
		}
		if (hasMatchingDescendents(catp->getUUID(), include_trash, match))
		{
			return true;
		}
	}

	item_array_t* items = get_ptr_in_map(mParentChildItemTree, id);
											  ...
											  U32 mask)
{
	...
	for (cat_array_t::iterator cat_iter = cat_array.begin(),
	...
	for (item_array_t::iterator iter = item_array.begin(),
									   end = item_array.end();
		 iter != end; ++iter)
	{
...
U32 LLInventoryModel::updateItem(const LLViewerInventoryItem* item, U32 mask)
{
	...
	if (type == LLAssetType::AT_MESH || type == LLAssetType::AT_GLTF ||
		type == LLAssetType::AT_GLTF)
	{
		return mask;
	}
#endif
...
{
	item_array_t item_array = collectLinkedItems(baseobj_id);
	...
	if (item_array.size() > 0)
	{
		...
		for (item_array_t::const_iterator iter = item_array.begin(),
										  end = item_array.end();
			iter != end; ++iter)
		{
...
void LLInventoryModel::fetchDescendentsOf(const LLUUID& folder_id)
{
	...
	LLViewerInventoryCategory* catp = getCategory(folder_id);
	if (!catp)
	{
...
std::string LLInventoryModel::getCacheFileName(const LLUUID& agent_id)
{
	...
	std::string filename = gDirUtil.getFullPath(LL_PATH_CACHE, agent_id_str);
	if (!gIsInSecondLife)
	{
							 ...
							 const LLUUID& agent_id)
{
	...
	{
		llinfos << "Empty inventory, nothing to do." << llendl;
		return;
	}
	std::string inventory_filename = getCacheFileName(agent_id);
	if (!saveToFile(inventory_filename, categories, items))
	{
									...
									const LLUUID& owner_id)
{
	...
	if (!temp_cats.empty())
	{
		...
		bool ais_fetching = LLInventoryModelFetch::useAISFetching();
		for (cat_set_t::iterator invalid_cat_it = invalid_categories.begin();
			 invalid_cat_it != invalid_categories.end();
			 invalid_cat_it++)
		{
			LLViewerInventoryCategory* catp = invalid_cat_it->get();
			catp->setVersionUnknown();
			// Fetch the invalidated folder.
			if (ais_fetching)
			{
				// With AIS, perform a force-fetching. HB
				LLInventoryModelFetch::forceFetchFolder(catp->getUUID());
			}
			else
			{
				catp->fetch();
								  ...
								  const item_array_t& items)
{
	...
	LLSD tmp_llsd;
	tmp_llsd["inv_cache_version"] = INVENTORY_CACHE_VERSION;
	file << LLSDOStreamer<LLSDNotationFormatter>(tmp_llsd) << std::endl;
	if (file.fail())
	{
	...
	{
		LLViewerInventoryCategory* catp = categories[i].get();
		if (!catp || catp->isVersionUnknown())
		{
			continue;
		}
		tmp_llsd = catp->exportLLSD();
		file << LLSDOStreamer<LLSDNotationFormatter>(tmp_llsd) << std::endl;
		if (file.fail())
		{
	...
	{
		LLViewerInventoryItem* itemp = items[i].get();
		if (!itemp)
		{
			continue;
		}
		tmp_llsd = itemp->asLLSD();
		file << LLSDOStreamer<LLSDNotationFormatter>(tmp_llsd) << std::endl;
		if (file.fail())
		{
...
void LLInventoryModel::removeCategory(const LLUUID& category_id)
{
	...
	cat_array_t descendent_categories;
...
void LLInventoryModel::checkTrashOverflow()
{
	...
	if (items.size() + cats.size() >= (size_t)max_capacity)
	{
		...
		gNotifications.add("TrashIsFull", LLSD(), LLSD(),
						   std::bind(&trash_full_callback, _1, _2));
	}
}
...
void LLInventoryFetchObserver::fetchItems(const uuid_vec_t& ids)
{
	...
	for (U32 i = 0, count = ids.size(); i < count; ++i)
	{
		...
		// do not know the parent category for... HB
		LLSD item_entry;
		item_entry["owner_id"] = itemp ? itemp->getPermissions().getOwner()
										 // Assume it is agent inventory.
									   : gAgentID;
