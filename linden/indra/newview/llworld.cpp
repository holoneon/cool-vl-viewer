...
LLVector2 LLWorld::resolveLandAndMinHeightGlobal(const LLVector3d& pos_global)
{
	F32 height = 0.f;
	F32 min = 0.f;
	LLViewerRegion* regionp = getRegionFromPosGlobal(pos_global);
	if (regionp)
	{
		height = regionp->getLand().resolveHeightGlobal(pos_global);
		min = regionp->getMinSimHeight();
	}
	return LLVector2(height, min);
}
...
void LLWorld::updateNetStats()
{
	...
	S32 packets_lost = msg->mLostPackets - mLastPacketsLost;
	S32 packets_dropped = msg->mDroppedPackets - mLastPacketsDropped;

	F32 actual_in_bits = msg->mPacketRing.getAndResetActualInBits();
	F32 actual_out_bits = msg->mPacketRing.getAndResetActualOutBits();
	...
	gViewerStats.addpPacketsStats(packets_in, packets_out, packets_lost,
								  packets_dropped);

	mLastPacketsIn = msg->mPacketsIn;
	mLastPacketsOut = msg->mPacketsOut;
	mLastPacketsLost = msg->mLostPackets;
	mLastPacketsDropped = msg->mDroppedPackets;
}
...
void LLWorld::reRegisterReflectionProbes()
{
	for (region_list_t::iterator it = mRegionList.begin(),
								 end = mRegionList.end();
		 it != end; ++it)
	{
		LLViewerRegion* regionp = *it;
		regionp->updateReflectionProbes(true);
	}
}
