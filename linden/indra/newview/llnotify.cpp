...
#include "llchat.h"
#include "llfloaterchat.h"			// For addChatHistory(), resolveSLURLs()
...
LLNotifyBox::unique_map_t LLNotifyBox::sOpenUniqueNotifyBoxes;
	...
	if (sigtype == "add" || sigtype == "change")
	{
		...
		if (gAutomationp)
		{
			if (is_script_dialog)
			{
				...
				for (size_t i = 0, count = self->mBtnCallbackData.size();
					 i < count; ++i)
				{
					CallbackData* userdata = self->mBtnCallbackData[i];
					if (userdata)
					{
						buttons.emplace_back(userdata->mButtonName);
					}
				}
...
LLNotifyBox::LLNotifyBox(LLNotificationPtr notifp, bool script_dialog,
						 bool is_ours)
:	LLPanel(notifp->getName(), LLRect(), BORDER_NO),
			LLEventTimer(notifp->getExpiration() == LLDate() ?
						 LLDate(LLTimer::getEpochSeconds() +
								(F64)gSavedSettings.getF32("NotifyTipDuration")) :
						 notifp->getExpiration()),
	LLInstanceTracker<LLNotifyBox, LLUUID>(notifp->getID()),
	mNotification(notifp),
	mIsTip(notifp->getType() == "notifytip"),
	mAnimating(true),
	mNextBtn(NULL),
	mNumOptions(0),
	mNumButtons(0),
...
//mk
	...
	mIsCaution = notifp->getPriority() >= NOTIFICATION_PRIORITY_HIGH;
	...
	else if (notifp->getName().find("Lua") == 0)
	{
	...
	if (mIsCaution && !mIsTip)
	{
		...
		LLTextBox* textp = new LLTextBox(std::string("caution_box"),
										 LLRect(x, y, getRect().getWidth() - 2,
											    caution_height),
												LLStringUtil::null, fontp,
												false);

		textp->setFontStyle(LLFontGL::BOLD);
		textp->setColor(gColors.getColor("NotifyCautionWarnColor"));
		textp->setBackgroundColor(mBackgroundColor);
		textp->setBorderVisible(false);
		textp->setWrappedText(notifp->getMessage());
	...
	else
	{
		...
		LLSLURL::findSLURLs(mTextEditor);
	}
