...
void LLPanelGroupBulkBan::submit()
{
	...
	LLScrollListCtrl::items_vec_t agents =
		mImplementation->mBulkAgentList->getAllData();
	for (size_t i = 0, count = agents.size(); i < count; ++i)
	{
		LLScrollListItem* itemp = agents[i];	// Cannot be NULL. HB
		banned_agent_list.emplace_back(itemp->getUUID());
	}

	// Max bans (= max invites) per request to match server cap.
	if ((S32)banned_agent_list.size() >
			LLPanelGroupBulkImpl::MAX_GROUP_INVITES)
	{
		// Fail!
		LLSD msg;
