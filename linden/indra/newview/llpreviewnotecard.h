...
public:
	...
	static LLFontGL* getCustomFont()					{ return sCustomFont; }
...
private:
	...
	static void onSetFontName(void* userdata);
	static void onSetFontSize(void* userdata);
	...
	static bool checkFontName(void* userdata);
	static bool checkFontSize(void* userdata);
...
private:
	...
	struct CallbackData
	{
		LL_INLINE CallbackData(LLPreviewNotecard* floaterp,
							   LLMenuItemCheckGL* ctrlp)
		:	mFloater(floaterp),
			mCheck(ctrlp)
		{
		}

		LLPreviewNotecard*	mFloater;
		LLMenuItemCheckGL*	mCheck;
	};
	std::vector<CallbackData>			mCallbackData;
