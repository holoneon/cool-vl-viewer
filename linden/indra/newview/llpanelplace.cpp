...
bool LLPanelPlace::postBuild()
{
	...
	mTeleportArrivingConnection =
		gViewerParcelMgr.setTPArrivingCallback(std::bind(&LLPanelPlace::onTeleportArriving,
														 this));
	mTeleportFailedConnection =
		gViewerParcelMgr.setTPFailedCallback(std::bind(&LLPanelPlace::onTeleportFailed,
													   this));
	return true;
}
	...
	if (!for_sale && mOwnerText && mOwnerID.notNull() && gCacheNamep)
	{
		...
		else
		{
			gCacheNamep->get(mOwnerID, mOwnerIsGroup,
							 std::bind(&LLPanelPlace::nameCallback, _1, _2, _3,
									   this));
		}
	}
