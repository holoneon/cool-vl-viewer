...
void LLFloaterAuction::onClickSnapshot(void* data)
{
	...
	if (success)
	{
		...
		raw->biasedScaleToPowerOfTwo(gMaxImageSizeDefault);
...
void LLFloaterAuction::onClickOK(void* data)
{
	...
	msg->newMessage(_PREHASH_ViewerStartAuction);
	...
	msg->addS32(_PREHASH_LocalID, self->mParcelID);
	msg->addUUID(_PREHASH_SnapshotID, self->mImageID);
