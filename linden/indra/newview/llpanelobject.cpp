...
#include "hbpreprocessor.h"		// For HBPreprocessor::evaluate()
...
static std::string preprocess_math(const std::string& text)
{
	std::string result = text;
	if (!HBPreprocessor::evaluate(result))
	{
		gNotifications.add("InvalidMathExpression");
		return text;
	}
	return result;
}
...
bool LLPanelObject::postBuild()
{
	...
	mCtrlPosX->setPrevalidate(LLLineEditor::prevalidateASCII);
	mCtrlPosX->setPreProcessCallback(preprocess_math);

	mCtrlPosY = getChild<LLSpinCtrl>("Pos Y");
	mCtrlPosY->setCommitCallback(onCommitPosition);
	mCtrlPosY->setCallbackUserData(this);
	mCtrlPosY->setPrevalidate(LLLineEditor::prevalidateASCII);
	mCtrlPosY->setPreProcessCallback(preprocess_math);

	mCtrlPosZ = getChild<LLSpinCtrl>("Pos Z");
	mCtrlPosZ->setCommitCallback(onCommitPosition);
	mCtrlPosZ->setCallbackUserData(this);
	mCtrlPosZ->setPrevalidate(LLLineEditor::prevalidateASCII);
	mCtrlPosZ->setPreProcessCallback(preprocess_math);
	...
	mCtrlScaleX->setPrevalidate(LLLineEditor::prevalidateASCII);
	mCtrlScaleX->setPreProcessCallback(preprocess_math);

	mCtrlScaleY = getChild<LLSpinCtrl>("Scale Y");
	mCtrlScaleY->setCommitCallback(onCommitScale);
	mCtrlScaleY->setCallbackUserData(this);
	mCtrlScaleY->setPrevalidate(LLLineEditor::prevalidateASCII);
	mCtrlScaleY->setPreProcessCallback(preprocess_math);

	mCtrlScaleZ = getChild<LLSpinCtrl>("Scale Z");
	mCtrlScaleZ->setCommitCallback(onCommitScale);
	mCtrlScaleZ->setCallbackUserData(this);
	mCtrlScaleZ->setPrevalidate(LLLineEditor::prevalidateASCII);
	mCtrlScaleZ->setPreProcessCallback(preprocess_math);
	...
	mCtrlRotX->setPrevalidate(LLLineEditor::prevalidateASCII);
	mCtrlRotX->setPreProcessCallback(preprocess_math);

	mCtrlRotY = getChild<LLSpinCtrl>("Rot Y");
	mCtrlRotY->setCommitCallback(onCommitRotation);
	mCtrlRotY->setCallbackUserData(this);
	mCtrlRotY->setPrevalidate(LLLineEditor::prevalidateASCII);
	mCtrlRotY->setPreProcessCallback(preprocess_math);

	mCtrlRotZ = getChild<LLSpinCtrl>("Rot Z");
	mCtrlRotZ->setCommitCallback(onCommitRotation);
	mCtrlRotZ->setCallbackUserData(this);
	mCtrlRotZ->setPrevalidate(LLLineEditor::prevalidateASCII);
	mCtrlRotZ->setPreProcessCallback(preprocess_math);
	...
	LLAggregatePermissions perms;
	if (gSelectMgr.selectGetAggregateTexturePermissions(perms))
	{
		bool can_copy =
			perms.getValue(PERM_COPY) == LLAggregatePermissions::AP_EMPTY ||
			perms.getValue(PERM_COPY) == LLAggregatePermissions::AP_ALL;
		bool can_transfer =
			perms.getValue(PERM_TRANSFER) == LLAggregatePermissions::AP_EMPTY ||
			perms.getValue(PERM_TRANSFER) == LLAggregatePermissions::AP_ALL;
...
void LLPanelObject::getState()
{
	...
	if (!objectp)
	{
		...
		if (objectp)
		{
			// Note: getRootEdit() returns objectp if the latter is root edit.
			root_objectp = objectp->getRootEdit();
		}
	}
...
#if HB_DEBUG_OBJECT
	static LLCachedControl<bool> debug_edited(gSavedSettings,
											  "DebugEditedObject");
	if (debug_edited)
	{
		LLViewerObject::setDebugObjectId(objectp->getID());
	}
#endif
...
//mk
	...
	else
	{
		...
		{
			// Spirals are not supported. Make it into a sphere.  JC
			selected_item = MI_SPHERE;
		}
		else if (path == LL_PCODE_PATH_CIRCLE &&
