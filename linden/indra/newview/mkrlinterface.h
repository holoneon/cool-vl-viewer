...
#define RL_VERSION_NUM "2092930"
#define RL_VERSION "2.09.29.30"
...
public:
	...
	// Returns true if catp or one of its parents is locked, or not shared
	// while @unshared is active
	bool isFolderLocked(LLInventoryCategory* catp);
	...
	std::string getWornItems(LLInventoryCategory* catp);
	// Returns the pointer to the #RLV folder or NULL if does not exist
	LLInventoryCategory* getRlvShare();
	bool isUnderRlvShare(LLInventoryItem* itemp);
	bool isUnderRlvShare(LLInventoryCategory* catp);
	...
	bool isInventoryFolderNew(LLInventoryCategory* catp);
	bool isInventoryItemNew(LLInventoryItem* itemp);
#endif

	LLViewerJointAttachment* findAttachmentPointFromName(std::string obj_name,
														 bool exact_name = false);
	LLViewerJointAttachment* findAttachmentPointFromParentName(LLInventoryItem* itemp);
	S32 findAttachmentPointNumber(LLViewerJointAttachment* attachp);
	void addAttachmentPointName(LLViewerObject* vobj);

	bool canDetachAllObjectsFromAttachment(LLViewerJointAttachment* attachp);
	void fetchInventory(LLInventoryCategory* rootp = NULL);
	...
	bool canAttachCategory(LLInventoryCategory* catp,
						   bool with_exceptions = true);
	bool canDetachCategory(LLInventoryCategory* catp,
						   bool with_exceptions = true);
	bool canUnwear(LLViewerInventoryItem* itemp);
	bool canUnwear(LLWearableType::EType type);
	bool canWear(LLViewerInventoryItem* itemp);
	bool canWear(LLWearableType::EType type);
	bool canDetach(LLViewerInventoryItem* itemp);
	bool canDetach(LLViewerObject* vobjp);
	bool canDetach(std::string attachpt);
	bool canAttach(LLViewerObject* object_to_attach, std::string attachpt);
	bool canAttach(LLViewerInventoryItem* itemp);
	...
	bool canEdit(LLViewerObject* vobjp);
	// Sets pick_intersection to force the check on this position
	bool canTouch(LLViewerObject* vobjp,
				  LLVector3 pick_intersection = LLVector3::zero);
	// Sets pick_intersection to force the check on this position
	bool canTouchFar(LLViewerObject* vobjp,
					 LLVector3 pick_intersection = LLVector3::zero);
...
private:
	...
	void removeWearableItemFromAvatar(LLViewerInventoryItem* itemp);
	...
	// "attach" or "detach"
	EFolderLock isFolderLockedWithoutException(LLInventoryCategory* catp,
											  std::string attach_or_detach);
	// Helper method used by isFolderLockedWithoutException()
	EFolderLock isFolderLockedWithoutExceptionAux(LLInventoryCategory* catp,
												  std::string attach_or_detach,
	...
	std::deque<LLInventoryItem*> getListOfLockedItems(LLInventoryCategory* rootp);
	...
	void detachObject(LLViewerObject* vobjp);
	void detachAllObjectsFromAttachment(LLViewerJointAttachment* attachp);
	...
	std::string getFullPath(LLInventoryCategory* catp);
	std::string getFullPath(LLInventoryItem* itemp,
							const std::string& option = LLStringUtil::null,
							bool full_list = true);

	// Various helper methods

	LLInventoryItem* getItemAux(LLViewerObject* vobjp,
								LLInventoryCategory* rootp);

	bool canAttachCategoryAux(LLInventoryCategory* catp, bool in_parent,
							  bool in_no_mod, bool with_exceptions = true);
	bool canDetachCategoryAux(LLInventoryCategory* catp, bool in_parent,
							  bool in_no_mod, bool with_exceptions = true);
...
public:
	...
	typedef std::map<std::string, S32, std::less<> > rl_command_map_t;
	typedef std::map<std::string, S32, std::less<> >::iterator rl_command_map_it_t;
	...
	strings_set_t				mReceivedInventoryFolders;
#endif
