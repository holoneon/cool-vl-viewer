...
LLVector3 LLManip::getPivotPoint()
{
	...
	LLViewerObject*	objectp = mObjectSelection->getFirstRootObject(true);
	if (objectp && (at_root || mObjectSelection->getObjectCount() == 1) &&
		mObjectSelection->getSelectType() != SELECT_TYPE_HUD)
	{
		LLSelectNode* nodep = mObjectSelection->getFirstNode();
		if (nodep->mSelectedGLTFNode != -1)
		{
			return objectp->getGLTFNodePositionAgent(nodep->mSelectedGLTFNode);
		}
		return objectp->getPivotPositionAgent();
	}
...
void LLManip::renderTickText(const LLVector3& pos, const std::string& text)
{
	...
	hud_render_text(wstr, render_pos, big_fontp, NULL, LLFontGL::NORMAL,
					text_width, 3.f, shadow_color, is_hud);
	gViewerWindowp->setupViewport();
	hud_render_text(wstr, render_pos, big_fontp, NULL, LLFontGL::NORMAL,
				    text_width, 3.f, color, is_hud);
							  ...
							  const LLColor4& color)
{
	...
	if (val_to_print < 0.f)
	{
		...
		{
			val_string = llformat("-%d%s", S32(fabsf(val_to_print)),
								  suffix.c_str());
		}
		else
		{
			val_string = llformat("-%d", S32(fabsf(val_to_print)));
		}
	}
	...
	{
		val_string = llformat("%d", S32(val_to_print));
	}
	...
	if (fractional_portion != 0)
	{
		...
		hud_render_text(wstr2, render_pos, small_fontp, NULL, LLFontGL::NORMAL,
