...
public:
	...
	:	LLScriptAssetUpload(task_id, item_id, compile_target, running, exp_id,
...
//static
LLFloaterCompileQueue* LLFloaterCompileQueue::create(const std::string& target)
{
	LLFloaterCompileQueue* self = new LLFloaterCompileQueue();
	self->mCompileTarget = target;
	return self;
}
			 ...
			 iter != end; ++iter)
		{
			...
			if (!url.empty())
			{
				expcache->fetchAssociatedExperience(itemp->getParentUUID(),
													itemp->getUUID(), url,
													std::bind(LLFloaterCompileQueue::requestAsset,
															  datap, _1));
			}
			else
			{
										  ...
										  LLExtStat)
{
	...
	if (queue && status == 0)
	{
		...
		LLBufferedAssetUploadInfo::task_uploaded_cb_t proc =
			std::bind(&LLFloaterCompileQueue::finishLSLUpload, _1, _2, _3, _4,
					  script_name, queue->getID());
		LLResourceUploadInfo::ptr_t
			info(new LLQueuedScriptAssetUpload(data->mTaskId,
											   data->mItem->getUUID(),
											   asset_id,
											   queue->mCompileTarget,
											   true, script_name,
											   queue->getID(),
...
bool LLFloaterCompileQueue::startQueue()
{
	...
	if (!url.empty())
	{
		LLCoreHttpUtil::HttpCoroutineAdapter::completionCallback_t
			succ = std::bind(&LLFloaterCompileQueue::processExperienceIdResults,
							 _1, getID());
		LLCoreHttpUtil::HttpCoroutineAdapter::completionCallback_t
			fail = std::bind(&LLFloaterCompileQueue::processExperienceIdResults,
							 LLSD(), getID());
