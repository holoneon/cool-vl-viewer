/**
 * @file llappviewerwindows.cpp
 * @brief The LLAppViewerWindows class definitions
 *
...
void LLWinDebug::initExceptionHandler(LPTOP_LEVEL_EXCEPTION_FILTER filter_func)
{
	...
	if (s_first_run)
	{
		...
		std::string local_dll_name =
				gDirUtil.findFile("dbghelp.dll", gDirUtil.getWorkingDir(),
								  gDirUtil.getExecutableDir());
...
void LLWinDebug::generateCrashStacks(struct _EXCEPTION_POINTERS* ex_infop)
{
	...
	std::string dump_path = gDirUtil.getFullPath(LL_PATH_LOGS, sDumpFilename);
...
{
	LLFile::remove(gDirUtil.getFullPath(LL_PATH_LOGS, sDumpFilename));
}

///////////////////////////////////////////////////////////////////////////////
// LLAppViewerWindows stuff proper
///////////////////////////////////////////////////////////////////////////////
...
{
	LLWindowWindows::setDPIAwareness();
	...
	LLAppViewerWindows* viewer_app_ptr = new LLAppViewerWindows(cmd_line);
	...
	// Request a discrete GPU if any.
	LLDXHardware::requestHighPerformanceGPU();
...
LLAppViewerWindows::LLAppViewerWindows(const char* cmd_line)
:	mCmdLine(cmd_line),
...
//virtual
LLApp::InitState LLAppViewerWindows::init()
{
	// Turn off Windows error reporting (do not send our data to Microsoft)
	std::string executable_name = gDirUtil.getExecutableFilename();
	...
	executable_name = gDirUtil.getExecutableDir() + "\\SLVoice.exe";
	utf16_exec_name = ll_convert_string_to_wide(executable_name);
	sErrorReportingDisabled &=
		WerAddExcludedApplication(utf16_exec_name.c_str(), FALSE) == S_OK;
	executable_name = gDirUtil.getExecutableDir() + "\\SLPlugin.exe";
	utf16_exec_name = ll_convert_string_to_wide(executable_name);
	sErrorReportingDisabled &=
		WerAddExcludedApplication(utf16_exec_name.c_str(), FALSE) == S_OK;
	executable_name = gDirUtil.getLLPluginDir() + "\\dullahan_host.exe";
	utf16_exec_name = ll_convert_string_to_wide(executable_name);
	sErrorReportingDisabled &=
		WerAddExcludedApplication(utf16_exec_name.c_str(), FALSE) == S_OK;
...
//virtual
bool LLAppViewerWindows::cleanup()
{
...
//virtual
void LLAppViewerWindows::initLogging()
{
...
//virtual
void LLAppViewerWindows::initConsole()
{
...
// driver versions.
//virtual
bool LLAppViewerWindows::initHardwareTest()
{
	...
	if (!LLGLManager::sVRAM)
	{
		llinfos << "Detecting available VRAM via DXGI..." << llendl;
		LLGLManager::sVRAM = LLDXHardware::getMBVideoMemoryViaDXGI();
	}
	llinfos << "Detected VRAM: " << LLGLManager::sVRAM << llendl;
...
//virtual
bool LLAppViewerWindows::initParseCommandLine(LLCommandLineParser& clp)
{
...
//virtual
bool LLAppViewerWindows::beingDebugged()
{
...
//virtual
bool LLAppViewerWindows::restoreErrorTrap()
{
...
//virtual
void LLAppViewerWindows::handleSyncCrashTrace()
{
...
//virtual
bool LLAppViewerWindows::sendURLToOtherInstance(const std::string& url)
{
...
//virtual
std::string LLAppViewerWindows::generateSerialNumber()
{
...
//virtual
void LLAppViewerWindows::handleLoginComplete()
{
	LLAppViewer::handleLoginComplete();
	// Now that the viewer is running, Windoze should not try and change the
	// GPU any more...
	LLDXHardware::detectGPUChange();
	LLDXHardware::clearHighPerformanceGPURequest();
}
...
//virtual
bool LLAppViewerWindows::probeVulkan(std::string& version)
{
