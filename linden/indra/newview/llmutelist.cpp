...
public:
	bool operator()(const LLDispatcher*, const std::string&, const LLUUID&,
					const strings_vec_t&) override
	{
...
strings_set_t LLMuteList::sGodLastNames;
strings_set_t LLMuteList::sGodFullNames;
strings_set_t LLMuteList::sLegacyMutes;
...
void LLMuteList::shutDownClass()
{
	...
	{
		std::string user_dir = gDirUtil.getLindenUserDir();
...
bool LLMuteList::add(const LLMute& mute, U32 flags)
{
	...
	if (mute.mType == LLMute::BY_NAME)
	{
		...
		std::pair<strings_set_t::iterator, bool> result =
...
bool LLMuteList::remove(const LLMute& mute, U32 flags)
{
	...
	strings_set_t::iterator legacy_it = sLegacyMutes.find(mute.mName);
		...
		if (full_name.empty())
		{
			...
			else
			{
				...
				gCacheNamep->get(agent_id, false,
								 std::bind(&notify_automute_callback, _1, _2,
										   _3, reason));
			}
		}
		else
...
std::vector<LLMute> LLMuteList::getMutes()
{
	...
	for (strings_set_t::const_iterator it = sLegacyMutes.begin(),
...
//static
std::vector<std::string> LLMuteList::getMutedUserNames(bool with_parenthesis)
{
		 ...
		 it != end; ++it)
	{
		if (it->mType == LLMute::AGENT)
		{
			...
			if (gIsInSecondLife)
			{
				// Remove " Resident" last name, if any.
				LLStringUtil::replaceString(name, " Resident", "");
				// Create the corresponding username
...
bool LLMuteList::loadFromFile(const std::string& filename)
{
	if (!sRegionBoundaryCrossingSlot.connected())
	{
		...
		sRegionBoundaryCrossingSlot =
			gAgent.addRegionChangedCB(std::bind(&onRegionBoundaryCrossed));
	}
	if (filename.empty())
	{
...
bool LLMuteList::saveToFile(const std::string& filename)
{
	...
	for (strings_set_t::iterator it = sLegacyMutes.begin(),
								 end = sLegacyMutes.end();
		 it != end; ++it)
	{
						 ...
						 LLMute::EType type)
{
	...
	{
		// Look in legacy pile
		strings_set_t::const_iterator legacy_it = sLegacyMutes.find(name);
...
void LLMuteList::processMuteListUpdate(LLMessageSystem* msg, void**)
{
	...
	std::string* local_filename_and_path =
		new std::string(gDirUtil.getFullPath(LL_PATH_CACHE, filename));
	...
	if (regionp)
	{
		...
		else if (!sSimFeaturesReceivedSlot.connected())
		{
			sSimFeaturesReceivedSlot =
				regionp->setFeaturesReceivedCB(std::bind(&getGodsNames));
		}
	}
}
