						   ...
						   const LLSD& notification, const LLSD& response)
{
	if (LLNotification::getSelectedOption(notification, response) == 0)
	{
		...
		inventory_func_t func = std::bind(&move_to_folder_cb, _1,
	...
	if (panelp && action == "group")
	{
		if (movable_objects_with_same_parent(selected_items))
		{
			gNotifications.add("CreateSubfolder", LLSD(), LLSD(),
						   	   std::bind(&move_to_folder, panelp,
...
protected:
	...
	bool handleEvent(LLPointer<LLEvent> event, const LLSD& userdata)
	{
		...
		gNotifications.add("ConfirmEmptyTrash", LLSD(), LLSD(),
						   std::bind(&LLEmptyTrash::cb_empty_trash, this, _1,
									 _2));
		return true;
	}
...
protected:
	...
	bool handleEvent(LLPointer<LLEvent> event, const LLSD&)
	{
		...
		gNotifications.add("ConfirmEmptyLostAndFound", LLSD(), LLSD(),
						   std::bind(&LLEmptyLostAndFound::cb_purge_laf, this,
									 _1, _2));
		return true;
	}
					  ...
					  const std::string& type, LLFolderBridge* self = NULL)
{
	if (type == "category")
	{
		...
		inventory_func_t func = std::bind(&create_category_cb, _1, handle);
...
class LLSetSearchType : public inventory_listener_t
{
	bool handleEvent(LLPointer<LLEvent> event, const LLSD& userdata)
	{
		...
		mPtr->getActivePanel()->getRootFolder()->toggleSearchType(toggle);
		mPtr->updateSearchControls();
		return true;
	}
};

class LLSearchShowLinks : public inventory_listener_t
{
	bool handleEvent(LLPointer<LLEvent> event, const LLSD& userdata)
	{
		mPtr->getActivePanel()->getRootFolder()->toggleSearchShowLinks();
		mPtr->updateSearchControls();
		return true;
	}
};
...
void init_inventory_actions(LLFloaterInventory* floaterp)
{
	(new LLDoToSelectedFloater())->registerListener(floaterp,
													"Inventory.DoToSelected");
	(new LLCloseAllFoldersFloater())->registerListener(floaterp,
													   "Inventory.CloseAllFolders");
	(new LLHideEmptySystemFolders())->registerListener(floaterp,
													   "Inventory.HideEmptySystemFolders");
	(new LLHideMarketplaceFolder())->registerListener(floaterp,
													  "Inventory.HideMarketplaceFolder");
	(new LLHideCurrentOutfitFolder())->registerListener(floaterp,
														"Inventory.HideCurrentOutfitFolder");
	(new LLCheckSystemFolders())->registerListener(floaterp,
												   "Inventory.CheckSystemFolders");
	(new LLResyncCallingCards())->registerListener(floaterp,
												   "Inventory.ResyncCallingCards");
	(new LLMakeNewOutfit())->registerListener(floaterp,
											  "Inventory.MakeNewOutfit");
	(new LLEmptyTrashFloater())->registerListener(floaterp,
												  "Inventory.EmptyTrash");
	(new LLDoCreateFloater())->registerListener(floaterp,
												"Inventory.DoCreate");

	(new LLNewWindow())->registerListener(floaterp, "Inventory.NewWindow");
	(new LLShowFilters())->registerListener(floaterp, "Inventory.ShowFilters");
	(new LLResetFilter())->registerListener(floaterp, "Inventory.ResetFilter");
	(new LLSetSortBy())->registerListener(floaterp, "Inventory.SetSortBy");
	(new LLSetSearchType())->registerListener(floaterp,
											  "Inventory.SetSearchType");
	(new LLSearchShowLinks())->registerListener(floaterp,
												"Inventory.SearchShowLinks");

	(new LLEnableUniversal())->registerListener(floaterp,
												"Inventory.EnableUniversal");
	(new LLEnableSettings())->registerListener(floaterp,
...
//mk
	if (itemp && itemp->getActualType() == LLAssetType::AT_LINK)
	{
...
//mk
	const LLUUID& linked_item_id = gInventory.getLinkedItemID(item_id);
	// See if we can bring an exiting preview to the front
	if (!LLPreview::show(linked_item_id, take_focus))
	{
		// There is none, so make a new preview
		S32 left, top;
		gFloaterViewp->getNewFloaterPosition(&left, &top);
...
{
	LLUUID id = get_calling_card_buddy_id(itemp, true);
	if (id.notNull())
	{
				  ...
				  const LLUUID& object_id, bool take_focus)
{
	...
	LLPreviewGesture* previewp = LLPreviewGesture::show(title, linked_item_id,
