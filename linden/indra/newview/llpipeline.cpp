...
#include "llrenderutils.h"			// For gl_draw_box*()
#include "lltexlayer.h"				// For LLTexLayerSetInfo::sUse2KBakes
...
S32	LLPipeline::sRenderPBRTerrainDetail = -4;
bool LLPipeline::sRenderPBRTerrain = false;
bool LLPipeline::sRenderSSR = false;
bool LLPipeline::sRenderMirrors = false;
...
bool LLPipeline::sRenderAvatarCloth = false;
...
// *TODO: move all of this to llshadermgr.* uniforms. HB
...
static LLStaticHashedString sExpParams2("dynamic_exposure_params2");
static LLStaticHashedString sExposure("exposure");
static LLStaticHashedString sToneMapMix("tonemap_mix");
static LLStaticHashedString sToneMapType("tonemap_type");
...
static LLStaticHashedString sLuminanceScale("diffuse_luminance_scale");
...
bool LLPipeline::sRenderHDR = false;
S32 LLPipeline::sUseOcclusion = 0;
S32 LLPipeline::sVisibleLightCount = 0;

// Per-frame materials usage flags. HB
bool LLPipeline::sUsingALM = false;
bool LLPipeline::sUsingPBR = false;
bool LLPipeline::sLacksLegacyFallback = false;
bool LLPipeline::sLacksBaseColorFallback = false;
...
static bool add_deferred_attachments(LLRenderTarget& target)
{
	if (gUsePBRShaders)
	{
		bool hdr = LLPipeline::sRenderHDR;
		// frag-data[1] specular *or* PBR ORM
		if (!target.addColorAttachment(GL_RGBA))
		{
			return false;
		}
		if (!target.addColorAttachment(hdr ? GL_RGBA16F : GL_RGB10_A2))
		{
			return false;
		}
		// frag_data[3] PBR emissive *or* material env intensity
		static LLCachedControl<bool> emissive(gSavedSettings,
											  "RenderEnableEmissiveBuffer");
		return !emissive ||
			   target.addColorAttachment(hdr ? GL_RGB16F : GL_RGB);
	}
	return target.addColorAttachment(GL_SRGB8_ALPHA8) &&	// Specular
		   target.addColorAttachment(GL_RGBA12);			// Normal + z
}

// LL_NO_INLINE so that the cached control site does not get duplicated
// everywhere. HB
LL_NO_INLINE static void clear_target(LLRenderTarget* targetp,
									  U32 mask = 0xFFFFFFFF)
{
	static LLCachedControl<bool> do_reset(gSavedSettings,
										  "RenderClearAllTargets");
	if (do_reset)
	{
		targetp->clear(mask);
	}
...
void LLPipeline::connectRefreshCachedSettingsSafe(const char* name)
{
	...
	cvp->getSignal()->connect(std::bind(&LLPipeline::refreshCachedSettings));
}
...
void LLPipeline::init()
{
	...
	// Note: this setting is sanitized by LLViewerWindow (which may also resync
	// it with LLPipeline::RenderFSAASamples on resolution changes) and should
	// only be taken into account after a GL window (re-)creation (for forward
	// rendering SSAA), thus why it is not set in refreshCachedSettings(). HB
	...
	connectRefreshCachedSettingsSafe("RenderHeroProbeResLevel");
	...
	connectRefreshCachedSettingsSafe("RenderSSR");
...
void LLPipeline::toggleRenderer()
{
	...
	while (true)	// Avoids a goto.
	{
		...
		else
		{
			...
			removePool(mInvisiblePool);		// = mWaterExclusionPool for LL. HB
		...
		// Do not keep a pointer for a pool that might have been removed...
		mLastRebuildPool = NULL;
		...
		if (gUsePBRShaders && !LLViewerShaderMgr::sInitialized)
		{
			...
			// crash later on while rendering).
			gSavedSettings.setBool("RenderUsePBR", false); // Resync setting
			// Must be set back to true so that setShaders() can proceed.
	...
	// Force-rebuild all the draw info.
	rebuildDrawInfo();
	// Rebuild draw pools
	rebuildPools();
	...
	if (gUsePBRShaders)
	{
		// Force a reflection probes rebuild.
		mReflectionMapManager.reset(true);
		mHeroProbeManager.reset();
...
void LLPipeline::allocateScreenBuffer(U32 res_x, U32 res_y)
{
	...
	if (!doAllocateScreenBuffer(res_x, res_y))
	{
		releaseScreenBuffers();
		// Reduce resolution
...
bool LLPipeline::doAllocateScreenBuffer(U32 res_x, U32 res_y)
{
	refreshCachedSettings();
	...
	llcont << "target buffers at size " << res_x << "x" << res_y << llendl;

	// *HACK: allocate auxillary PBR buffers when mRT == &mMainRT.
	// Note: we now always need auxillary buffers (even when reflection probes
	// are disabled), because they are used for dynamic textures and PBR
	// materials preview... HB
	if (gUsePBRShaders && mRT == &mMainRT)
	{
		U32 res = 512;
		if (sReflectionProbesEnabled)
		{
			...
			res = mReflectionMapManager.mProbeResolution * 4;
		}
		// Note: gCubeSnapshot acts as a flag for the auxilliary buffers
		// allocation step. HB
		gCubeSnapshot = true;
		mRT = &mAuxillaryRT;
		doAllocateScreenBuffer(res, res);
		if (sRenderMirrors)
		{
			mHeroProbeManager.initReflectionMaps();
			// We also scale the hero probe RT to the probe res since we do not
			// super sample it.
	...
	if (!add_deferred_attachments(mRT->mDeferredScreen))
	{
	...
	else
	{
		...
		if (LLGLManager::sIsAMD)
		{
			if (LLGLManager::sGLVersion < 4.f ||
				!gSavedSettings.getBool("RenderUseRGBA16ATI"))
			{
				screen_format = GL_RGBA12;
			}
		}
		else if (LLGLManager::sIsNVIDIA && LLGLManager::sGLVersion < 4.f)
		{
	...
	// Note: when RenderResolutionDivisor is larger than 1 (i.e. we ask for a
	// lower resolution render), we do not really need any anti-aliasing... HB
	if (RenderFSAASamples > 1 && res_mod <= 1 && !gCubeSnapshot)
	{
		if (gUsePBRShaders)
		{
			if (!mFXAABuffer.allocate(res_x, res_y, GL_RGBA))
			{
				llwarns << "Failed to allocate the FXAA buffer." << llendl;
				return false;
			}
			if (!mSMAABlendBuffer.allocate(res_x, res_y, GL_RGBA))
			{
		...
		else
		{
			...
			if (!mSMAAEdgeBuffer.allocate(res_x, res_y, GL_RGBA, true, true))
			{
				llwarns << "Failed to allocate the SMAA edge buffer."
						<< llendl;
				return false;
			}
			if (!mSMAABlendBuffer.allocate(res_x, res_y, GL_RGBA, false,
										   false))
			{
				llwarns << "Failed to allocate the SMAA blend buffer."
						<< llendl;
			...
			if (!mScratchBuffer.allocate(res_x, res_y, GL_RGBA, false, false))
			{
	...
	U32 pbr_scr_fmt = sRenderHDR ? GL_RGBA16F : GL_RGBA;
	if (RenderFSAASamples > 1 || RenderShadowDetail || RenderDeferredSSAO ||
		RenderDepthOfField)
	{
		// Only need mDeferredLight for shadows or SSAO or DOF or FXAA
		if (gUsePBRShaders)
		{
			// Note: always use GL_RGBA16F to avoid banding artifacts
			if (!mRT->mDeferredLight.allocate(res_x, res_y, pbr_scr_fmt))
			{
	...
	if (gUsePBRShaders && !gCubeSnapshot)
	{
		if (sRenderSSR)
		{
			if (!mSceneMap.allocate(res_x, res_y, pbr_scr_fmt, true,
			...
			U32 ssr_scale =
				llclamp(gSavedSettings.getU32("RenderSSRResolutionDivider"),
						1, 4);
			U32 ssr_w = res_x / ssr_scale;
			U32 ssr_h = res_y / ssr_scale;
			// At full screen resolution do not use own depth but share from
			// deferred instead for zero-copy. At reduced resolution allocate
			// own depth and blit from deferred before alpha/water passes.
			if (!mSSRBuffer.allocate(ssr_w, ssr_h, GL_RGBA16F, ssr_scale != 1,
									 LLTexUnit::TT_TEXTURE,
									 LLTexUnit::TMG_AUTO, "SSR buffer"))
			{
				llwarns << "Failed to allocate the SSR buffer." << llendl;
				return false;
			}
			if (ssr_scale == 1)
			{
				mRT->mDeferredScreen.shareDepthBuffer(mSSRBuffer);
			}
			// Allocate per-mip temp targets for Gaussian ping-pong (one per
			// mip level > 0)
			U32 mip_levels = mSSRBuffer.getMipLevels();
			if (mip_levels > 1 && mip_levels - 1 != (U32)mSSRMipTemp.size())
			{
				mSSRMipTemp.resize(mip_levels - 1);
				for (U32 i = 0; i < mip_levels - 1; ++i)
				{
					U32 mip = i + 1;
					U32 w = llmax(1U, ssr_w >> mip);
					U32 h = llmax(1U, ssr_h >> mip);
					if (!mSSRMipTemp[i].allocate(w, h, GL_RGBA16F))
					{
						llwarns << "Failed to allocate the SSR buffer mip "
								<< mip << llendl;
						return false;
					}
				}
		...
		if (!mPostMap.allocate(res_x, res_y, pbr_scr_fmt))
		{
			llwarns << "Failed to allocate the post map buffer." << llendl;
			return false;
		}
		if (!mWaterExclusionMask.allocate(res_x, res_y, GL_R8, true))
		{
			llwarns << "Failed to allocate the water exclusion mask buffer."
					<< llendl;
			return false;
		}
	}
...
void LLPipeline::allocateBakeBuffer()
{
	U32 bake_size = LLTexLayerSetInfo::sUse2KBakes ? 2048 : 1024;
	if (!mBakeBuffer.allocate(bake_size, bake_size, GL_RGBA))
	{
		llwarns << "Failed to allocate buffer for local 2K bakes: expect rendering issues."
				<< llendl;
	}
}
...
bool LLPipeline::allocateShadowBuffer(U32 res_x, U32 res_y)
{
	...
	constexpr U32 occlusion_divisor = 3;
	// Note: do not scale probe shadow maps (allocated when gCubeSnapshot is
	// true).
	F32 scale = gCubeSnapshot ? 1.f : RenderShadowResolutionScale;
	...
	if (RenderShadowDetail)
	{
		...
		llinfos << (gCubeSnapshot ? "Reflection probes " : "")
				<< "Sun shadows maps (4) size: " << sun_shadow_map_width << "x"
				<< sun_shadow_map_height << " pixels." << llendl;
		if (gUsePBRShaders)
		{
			for (U32 i = 0; i < 4; ++i)
			{
	...
	if (RenderShadowDetail > 1)
	{
		...
		llinfos << (gCubeSnapshot ? "Reflection probes s" : "S")
				<< "pot shadows maps (2) size: " << size << "x" << size
				<< " pixels." << llendl;
		if (!gUsePBRShaders)
		{
			for (U32 i = 4; i < 6; ++i)
...
void LLPipeline::refreshCachedSettings()
{
	...
	RenderResolutionDivisor = llclamp((U32)res_divisor, 1, 8);
	...
	RenderShadowResolutionScale = llclamp((F32)shadow_res_scl, 0.25f, 1.f);
	...
	RenderBufferVisualization = llclamp(S32(pbr_visu), -1, 7);

	static LLCachedControl<U32> reflections(gSavedSettings,
											"RenderReflectionProbes");
	sReflectionProbesEnabled = gUsePBRShaders && LLViewerShaderMgr::sHasRP &&
							   reflections;

	static LLCachedControl<bool> hdr(gSavedSettings, "RenderHDREnabled");
	sRenderHDR = hdr && LLGLManager::sGLVersion >= 4.1f;

	bool had_mirrors = sRenderMirrors;
	static LLCachedControl<U32> hero_res(gSavedSettings,
										 "RenderHeroProbeResLevel");
	sRenderMirrors = sReflectionProbesEnabled && hero_res > 0;
	if (gUsePBRShaders && sRenderMirrors != had_mirrors)
	{
	...
	sRenderSSR = sReflectionProbesEnabled && ssr && reflections;

	static LLCachedControl<bool> pbr_terrain(gSavedSettings,
											 "RenderTerrainPBREnabled");
	sRenderPBRTerrain = gUsePBRShaders && pbr_terrain &&
						LLGLManager::sNumTextureImageUnits >= 16;
	static LLCachedControl<U32> pbr_terrain_detail(gSavedSettings,
												   "RenderTerrainPBRDetail");
	S32 terrain_details = 0;
	if (sRenderPBRTerrain)
	{
		terrain_details = pbr_terrain_detail;
		if (terrain_details > 3 && LLGLManager::sNumTextureImageUnits <= 16)
		{
			terrain_details = 3;	// Remove emissive map support
		}
	}
	sRenderPBRTerrainDetail = llmin(0, terrain_details - 4);
...
void LLPipeline::releaseScreenBuffers()
{
	...
	if (gUsePBRShaders)
	{
		...
		releasePackBuffers(&mHeroProbeRT);
		mReflectionMapManager.cleanup();
		mHeroProbeManager.cleanup();
		mSceneMap.release();
		mSSRBuffer.release();
...
void LLPipeline::createGLBuffers()
{
	...
	if (gUsePBRShaders)
	{
		// We need this buffer to be as large as the screen space, since it is
		// used to copy the screen buffer for screen-wide water and atmospheric
		// haze as well as for water distortions and reflections... HB
		U32 pbr_scr_fmt = sRenderHDR ? GL_RGBA16F : GL_RGBA;
		mWaterDis.allocate(res_x, res_y, pbr_scr_fmt, true);
	}
	else
	{
...
void LLPipeline::createLUTBuffers()
{
	...
	LLImageGL::setManualImage(LLTexUnit::getInternalType(LLTexUnit::TT_TEXTURE),
							  0, GL_R16F, light_res_x, light_res_y,
	...
	if (!gUsePBRShaders || !LLViewerShaderMgr::sHasLut)
	{
		return;
	}
...
{
	if (gUsePBRShaders && type == LLDrawPool::POOL_ALPHA)
	{
		llwarns_sparse << "Trying to get POOL_ALPHA, but this pool does not exist in PBR mode: returning POOL_ALPHA_POST_WATER."
					   << llendl;
		llassert(false);
...
// Workaround for a bug in my PBR backport, where alpha-blend PBR material
// textures do not render (or more likely, get overwritten with a second
// render pass and a blank texture, because they do reach the right point
// in LLDrawPoolAlpha::renderAlpha(), with the right parameters and right
// shader to which they do get bound and rendered with, and yet, somehow
// appear like if they were blank textures in the final render). Remove
// this hack once the bug is fixed. HB
#define HB_PBR_ALPHA_BLEND_WORKAROUND 1
								  ...
								  LLViewerTexture* imagep)
{
...
#if HB_PBR_ALPHA_BLEND_WORKAROUND
	if (gUsePBRShaders && tep->getGLTFRenderMaterial())
	{
		// Yep, alpha-blend PBR materials seem to render just fine too with
		// this render pool... HB
	...
	if (gUsePBRShaders)
	{
		...
		// Special case for alpha-blend PBR materials
		if (gltfp &&
...
{
	if (sRenderMirrors && gPipeline.mHeroProbeManager.isMirrorPass())
	{
		// We always pretend that we are not clipping water when rendering
		// mirrors.
		return false;
	}
...
void LLPipeline::doOcclusionPBR(LLCamera& camera)
{
	...
	if (sRenderMirrors && !sShadowRender && !gCubeSnapshot)
	{
		gGL.setColorMask(false, false);
		LLGLDepthTest depth(GL_TRUE, GL_FALSE);
		LLGLDisable cull(GL_CULL_FACE);
	...
	// Note: only actually done in processOccluders() for CAMERA_WORLD. HB
	if (LLViewerCamera::sCurCameraID == LLViewerCamera::CAMERA_WORLD)
	{
...
void LLPipeline::doOcclusion(LLCamera& camera)
{
	...
	// Note: we do not stop occluding during teleportation for PBR, thus why
	// this boolean is only tested here. HB
...
void LLPipeline::touchTexture(LLViewerTexture* texp, F32 vsize)
{
	if (texp)
	{
		LLImageGL* imagep = texp->getGLImage();
		if (imagep && imagep->updateBindStats())
		{
...
{
	LL_TRACY_TIMER(TRC_TOUCH_TEXTURES);

	// Update out materials usage statistics. HB
	static LLCachedControl<bool> attachments(gSavedSettings,
											 "AccountAttachmentsMaterials");
	static LLCachedControl<U32> min_vsize(gSavedSettings,
										 "AccountMaterialsMinVSize");
	if (infop->mVSize >= F32(min_vsize) &&
		(!infop->mIsAgentAttachment || attachments))
	{
		if (infop->mHasALM)
		{
			sUsingALM = true;
		...
		{
			sUsingPBR = true;
			if (!infop->mHasLegacyFallback)
			{
				sLacksLegacyFallback = true;
				if (!infop->mHasBasecolorFallback)
				{
					sLacksBaseColorFallback = true;
				}
			}
		}
	}
...
#define REBUILD_ON_GEOM_DIRTY 0
#if REBUILD_ON_GEOM_DIRTY
...
void render_hud_elements()
{
	...
	{
		bool sample = !gUsePBRShaders && LLPipeline::RenderFSAASamples > 1;
...
void LLPipeline::renderHighlights()
{
	...
	if (selected_count)
	{
		...
		F32 max_area = gMaxImageSizeDefault * gMaxImageSizeDefault;
		mFaceSelectImagep->addTextureStats(max_area);
		...
		for (S32 i = 0; i < selected_count; ++i)
		{
			...
			LLDrawable* drawablep = facep ? facep->getDrawable().get() : NULL;
			if (!drawablep || drawablep->isDead())
			{
				llwarns << "Bad face in selection. Clearing selection."
						<< llendl;
			...
			if (shader_interface)
			{
				...
				else if (active_channel == LLRender::SPECULAR_MAP && matp &&
...
void LLPipeline::renderGeom(LLCamera& camera)
{
	...
	LLGLEnable multisample(RenderFSAASamples > 1 ? GL_MULTISAMPLE : 0);

	// Toggle backface culling for debugging
	// Note: this used to be a toggle based on a mBackfaceCull boolean, but the
	// latter was always true. HB
		...
		while (iter1 != pools_end)
		{
				...
				(passes = poolp->getNumPasses()) > 0)
			{
				...
				for (S32 i = 0; i < passes; ++i)
				{
					...
					{
						std::string ptype = LLDrawPool::getPoolName(cur_type);
						LLGLState::checkStates(llformat("%s pass %d",
														ptype.c_str(), i));
					}
				}
			}
...
void LLPipeline::renderGeomPostDeferred(LLCamera& camera, bool do_occlusion)
{
	...
	else
	{
		...
		sample = LLPipeline::RenderFSAASamples > 1;
	}
	LLGLEnable cull(GL_CULL_FACE);
	LLGLEnable multisample(sample ? GL_MULTISAMPLE : 0);

	// Skip PBR atmospherics/haze when in EE rendering, or when rendering HUDs,
	// or when rendering impostors, or when not rendering atmospherics, or when
	// rendering probes and using maual reflection probes only.
	...
	bool done_water_exclusion = !gUsePBRShaders;
	// Do atmospheric haze just before water pass when below water, or just
	// before post water alpha when above water.
	...
	constexpr U32 water_exclusion_pass = LLDrawPool::POOL_WATEREXCLUSION;
	...
	while (iter1 != pools_end)
	{
		...
		// Used only in EE rendering mode. HB
		if (occlude && cur_type >= LLDrawPool::POOL_GRASS)
		{
			occlude = false;
			gGLLastMatrix = NULL;
		...
		// Used only in PBR rendering mode. HB
		if (!done_water_exclusion && cur_type >= water_exclusion_pass)
		{
...
{
	LL_TRACY_TIMER(TRC_DO_WATER_EXCLUSION);
...
void LLPipeline::doAtmospherics()
{
	...
	{
		// Copy depth buffer for use in haze shader (use water displacement map
		// as temp storage).
		LLGLDepthTest depth(GL_TRUE, GL_TRUE, GL_ALWAYS);
		mRT->mScreen.flush();
		...
		gGL.getTexUnit(diff_map)->bind(&mRT->mScreen);
		gGL.getTexUnit(depth_map)->bind(&mRT->mDeferredScreen, true);
	...
	// Apply haze
	bindDeferredShader(gHazeProgram, NULL, &mWaterDis);
...
void LLPipeline::doWaterHaze()
{
...

	{
		...
		// Apply haze
		bindDeferredShader(gHazeWaterProgram, NULL, &mWaterDis);

		gHazeWaterProgram.uniform4fv(LLShaderMgr::WATER_WATERPLANE, 1,
									 sWaterPlane.getF32ptr());
		gHazeWaterProgram.bindTexture(LLShaderMgr::WATER_EXCLUSIONTEX,
									  &mWaterExclusionMask);
		if (sUnderWaterRender)
		{
	...
	// Moved from the now removed LLDrawPoolWater::beginPostDeferredPass(): the
	// latter was pointlessly executed for *each* water pool, while all we need
	// is a frame buffer copy of the screen just before the first pass of the
	// first water pool: do this here, since doWaterHaze() is called precisely
	// at this right time !   HB
	if (waterReflectionType() && !gCubeSnapshot)
	{
		...
		S32 depth_chan =
			gCopyDepthProgram.getTextureChannel(LLShaderMgr::DEFERRED_DEPTH);
		gGL.getTexUnit(diff_chan)->bind(&mRT->mScreen);
...
{
	// List of partitions which can actually contain physical objects. HB
	static std::vector<U32> physics_partitions =
	{
	...
	static U32 num_phys_parts = physics_partitions.size();

	// Terrain physics shape not rendered any more by LL's PBR viewer: let's
	// give the user a choice. HB
	static LLCachedControl<bool> terrain_phys(gSavedSettings,
											  "RenderTerrainPhysicsShape");
	U32 start_part = terrain_phys ? 0 : 1;
	...
	if (!gUsePBRShaders)
	{
		...
		{
			LLViewerRegion* regionp = *iter;
			for (U32 i = start_part; i < num_phys_parts; ++i)
			{
				LLSpatialPartition* partp =
					regionp->getSpatialPartition(physics_partitions[i]);
				if (hasRenderType(partp->mDrawableType))
				{
	...
	LLGLEnable polygon(GL_POLYGON_OFFSET_LINE);
...
void LLPipeline::renderDebug()
{
	...
	constexpr U32 GLTF_DEBUG_MASK = RENDER_DEBUG_BBOXES |
									RENDER_DEBUG_RAYCAST |
									RENDER_DEBUG_NODES;
	if (gUsePBRShaders && hasRenderDebugMask(GLTF_DEBUG_MASK))
	{
		LLGLTFSceneManager::renderDebug();
	}
		...
		!gVisibleSelectedGroups.empty())
	{
			 ...
			 iter != end; ++iter)
		{
			...
			// Used to be called in drawBox(), now replaced with gl_draw_box()
			// which does not call this. HB
			LLVertexBuffer::unbind();

			gl_draw_box(bounds[0], size);
		}
	}
...
void LLPipeline::addToQuickLookup(LLDrawPool* new_poolp)
{
	switch (new_poolp->getType())
	{
		...
		case LLDrawPool::POOL_WATEREXCLUSION:
			if (mInvisiblePool)
			{
				llwarns << "Ignoring duplicate invisible pool." << llendl;
				llassert(false);
			}
			else
			{
...
{
	if (sReflectionRender || sRenderingHUDs || sAvatarPreviewRender ||
		gCubeSnapshot)
	{
		return;
	}
...
void LLPipeline::cacheEnvironment()
{
	...
	static LLCachedControl<F32> tone_mix(gSavedSettings, "RenderTonemapMix");
	skyp->setTonemapMix(llclamp((F32)tone_mix, 0.f, 1.f));

	mTotalAmbient = skyp->getTotalAmbient();
	if (gUsePBRShaders)
	{
		// Note: when reflections are off, we must always consider the sky as
		// a PBR one to avoid too dark a rendering (missing ambiant). HB
		LLRender::sClassicMode = !adjust && sReflectionProbesEnabled &&
								 skyp->canAutoAdjust();
		mNoProbeAmbiance = !sReflectionProbesEnabled ||
						   skyp->getReflectionProbeAmbiance(adjust) <= 0.f;
		mHDRMin = skyp->getHDRMin(adjust);
		mHDRMin = skyp->getHDRMin(adjust);
		mHDROffset = skyp->getHDROffset(adjust);
		mHDRScale = sqrtf(skyp->getGamma()) * 2.f;
		mTonemapMix = skyp->getTonemapMix(adjust);
	}
	...
	if (!mIsSunUp && !mIsMoonUp)
	{
		...
		mSunLightColor.setToBlack();
		mMoonLightColor.setToBlack();
	}
	else
	{
		mSunDir.set(skyp->getSunDirection(), 0.f);
		mMoonDir.set(skyp->getMoonDirection(), 0.f);
		mSunLightColor.set(skyp->getSunlightColor(), 1.f);
		mMoonLightColor.set(skyp->getMoonlightColor(), 1.f);
	}

	// Sun or Moon (All objects)
	F32 max_color = llmax(mSunLightColor.mV[0], mSunLightColor.mV[1],
						  mSunLightColor.mV[2]);
	if (max_color > 1.f)
	{
		mSunLightColor *= 1.f / max_color;
	}
	mSunLightColor.clamp();
	mSunLightColorScaled = mSunLightColor;
	if (gUsePBRShaders && adjust && skyp->canAutoAdjust())
	{
		mSunLightColorScaled *= adj_scale;
	}

	max_color = llmax(mMoonLightColor.mV[0], mMoonLightColor.mV[1],
					  mMoonLightColor.mV[2]);
	if (max_color > 1.f)
	{
		mMoonLightColor *= 1.f / max_color;
	}
	mMoonLightColor.clamp();
...
void LLPipeline::setupHWLights()
{
	...
	LLColor4 light_diffuse = mIsSunUp ? mSunLightColor : mMoonLightColor;
	mHWLightColors[0] = light_diffuse;
	lightp->setDiffuse(light_diffuse);
	lightp->setSunPrimary(mIsSunUp);
	lightp->setDiffuseB(mMoonLightColor);
														...
														LLVector4a* tangentp)
{
	...
	if (!sPickAvatar)
	{
		...
		if (face_hitp)
		{
			local_face_hit = *face_hitp;
		}
		if (tcoordp)
		{
			local_texcoord = *tcoordp;
		}
		if (tangentp)
		{
			local_tangent = *tangentp;
		}
		else
		{
			local_tangent.clear();
			 ...
			 iter != wend; ++iter)
		{
			...
			LLDrawable* hitp =
				partp->lineSegmentIntersect(start, local_end, pick_transparent,
											pick_rigged, face_hitp, &position,
			...
			if (hitp)
			{
				...
				else
				{
					...
					if (face_hitp)
					{
						*face_hitp = local_face_hit;
					}
					if (tcoordp)
					{
						*tcoordp = local_texcoord;
					}
					if (tangentp)
					{
						*tangentp = local_tangent;
					}
					if (normalp)
					{
						*normalp = local_normal;
					}
				}
			}
		}
	...
	if (gUsePBRShaders)
	{
		S32 node_hit = -1;
		S32 prim_hit = -1;
		LLDrawable* hit_drawp =
			LLGLTFSceneManager::lineSegmentIntersect(start, local_end,
													 pick_transparent,
													 pick_rigged, &node_hit,
													 &prim_hit, &position,
													 tcoordp, normalp,
													 tangentp);
		if (hit_drawp)
		...
		if (gltf_node_hitp)
		{
			*gltf_node_hitp = node_hit;
		}
		if (gltf_prim_hitp)
		{
			*gltf_prim_hitp = prim_hit;
		}
	}
...
void LLPipeline::doResetVertexBuffers(bool forced)
{
	...
	{
		LLSpatialPartition::sTeleportRequested = false;
		// Do not do this for the PBR renderer. HB
		if (!gUsePBRShaders)
		{
...
void LLPipeline::renderGLTFObjects(U32 type, bool texture, bool rigged)
{
	...
	if (rigged)
	{
		LLRenderPass::pushRiggedGLTFBatches(type + 1, texture);
	}
	else
	{
		LLRenderPass::pushGLTFBatches(type, texture);
	}
	...
	if (!rigged)
	{
		LLGLTFSceneManager::renderOpaque();
	}
}
...
void LLPipeline::renderAlphaObjectsPBR(bool rigged)
{
	...
	for (U32 i = 0, count = draw_list.size(); i < count; )
	{
		...
		if (has_pbr_mat)
		{
			LLRenderPass::pushGLTFBatch(*paramsp);
		}
		else
		{
...
void LLPipeline::generateLuminance(LLRenderTarget* srcp, LLRenderTarget* dstp)
{
	...
	chan = gLuminanceProgram.enableTexture(LLShaderMgr::NORMAL_MAP);
	if (chan > -1)
	{
		mRT->mDeferredScreen.bindTexture(2, chan, LLTexUnit::TFO_POINT);
	}

	static LLCachedControl<F32> diffuse_scale(gSavedSettings,
											  "RenderDiffuseLuminanceScale");
	gLuminanceProgram.uniform1f(sLuminanceScale, diffuse_scale);
								  ...
								  bool use_history)
{
	...
	static LLCachedControl<bool> dyn_exp(gSavedSettings,
										 "RenderDynamicExposureEnabled");
	static LLCachedControl<bool> sky_exp(gSavedSettings,
										 "RenderExposureUsesSkySettings");
	static LLCachedControl<F32> exp_coeff(gSavedSettings,
										  "RenderDynamicExposureCoefficient");
	static LLCachedControl<F32> speed_error(gSavedSettings,
											"RenderDynamicExposureSpeedError");
	static LLCachedControl<F32> speed_tgt(gSavedSettings,
	...
	LLGLSLShader& shader = use_history ? gExposureProgram
									   : gExposureProgramNoFade;
	shader.bind();

	S32 chan = shader.enableTexture(LLShaderMgr::DEFERRED_EMISSIVE);
	if (chan > -1)
	{
		srcp->bindTexture(0, chan, LLTexUnit::TFO_TRILINEAR);
	}
	...
	shader.uniform1f(sDT, gFrameIntervalSeconds);
	shader.uniform2f(sNoiseVec, ll_frand() * 2.f - 1.f,
					 ll_frand() * 2.f - 1.f);

	F32 exp_min = mHDROffset;
	F32 exp_max = mHDROffset;
	F32 exp_offset = mHDROffset;
	if (mNoProbeAmbiance)
	{
		exp_min = exp_max = exp_offset = 1.f;
	}
	else if (sky_exp)
	{
		if (dyn_exp)
		...
		// Else use default mHDROffset for both exp_min & exp_max
	}
	else if (dyn_exp && mHDRScale > 1.f)
	{
		exp_min = 1.f / mHDRScale;
		exp_max = mHDRScale;
	}
	shader.uniform4f(sExpParams, exp_coeff, exp_min, exp_max, speed_error);
	shader.uniform4f(sExpParams2, exp_offset, exp_min, exp_max, speed_tgt);
...
// PBR renderer only
void LLPipeline::toneMap(LLRenderTarget* srcp, LLRenderTarget* dstp)
{
	LL_TRACY_TIMER(TRC_RENDER_TONE_MAP);
	...
	LLGLSLShader* shaderp;
	if (!type || mNoProbeAmbiance || gSnapshotNoPost ||
		(no_post && gToolMgr.inBuildMode()))
	{
		mToneMapped = false;
		shaderp = &gNoPostTonemapProgram;
		if (shaderp != last_shaderp)
		{
			llinfos << "Tone mapping shader in use: gNoPostTonemapProgram"
					<< llendl;
		}
	}
	else
	{
		...
		if (shaderp != last_shaderp)
		{
			llinfos << "Tone mapping shader in use: gDeferredPostTonemapProgram"
					<< llendl;
		}
	}
	...
	shaderp->bindTexture(LLShaderMgr::DEFERRED_DIFFUSE, srcp, false,
						 LLTexUnit::TFO_POINT);
	if (mToneMapped)	// No point in setting unused uniforms... HB
	{
		...
		shaderp->uniform1i(sToneMapType, llclamp(S32(type - 1), 0, 1));
		shaderp->uniform1f(sToneMapMix, mTonemapMix);
	}
...
void LLPipeline::gammaCorrect(LLRenderTarget* srcp, LLRenderTarget* dstp)
{
	...
	static LLGLSLShader* last_shaderp = NULL;
	LLGLSLShader* shaderp;
	// When not using tone mapping, use legacy gamma correction. HB
	if (mToneMapped)
	{
...
// PBR renderer only.
void LLPipeline::applyFXAA(LLRenderTarget* srcp, LLRenderTarget* dstp)
{
	LL_TRACY_TIMER(TRC_RENDER_FXAA);

	if (!LLViewerShaderMgr::sHasFXAA || RenderFSAASamples <= 1 ||
		!mFXAABuffer.isComplete())
	{
	...
	clear_target(&mFXAABuffer, GL_COLOR_BUFFER_BIT);
	...
	F32 inv_width = 1.f / F32(mFXAABuffer.getWidth());
	F32 inv_height = 1.f / F32(mFXAABuffer.getHeight());
...
// PBR renderer only. Returns true when SMAA can actually be applied. HB
bool LLPipeline::generateSMAABuffers(LLRenderTarget* srcp)
{
	if (!LLViewerShaderMgr::sHasSMAA || RenderFSAASamples <= 1 ||
		!mAreaMap || !mSearchMap || !mSMAABlendBuffer.isComplete() ||
		!mFXAABuffer.isComplete())
	{
		return false;
	}
	...
	mFXAABuffer.bindTarget();
	mFXAABuffer.clear(GL_COLOR_BUFFER_BIT);
	
	LLGLSLShader* shaderp = &gPostSMAAEdgeDetect[RenderDeferredAAQuality];
	shaderp->bind();
	shaderp->uniform4fv(sSmaaRTMetrics, 1, rt_metrics);

	S32 channel = shaderp->enableTexture(LLShaderMgr::DEFERRED_DIFFUSE,
										 srcp->getUsage());
	LLTexUnit* unit1p = channel > -1 ? gGL.getTexUnit(channel) : NULL;
	if (unit1p)
	{
		srcp->bindTexture(0, channel, LLTexUnit::TFO_POINT);
		unit1p->setTextureAddressMode(LLTexUnit::TAM_CLAMP);
	}
	...
	if (unit1p)
	{
		unit1p->unbindFast(LLTexUnit::TT_TEXTURE);
	}

	// Weights blending

	mSMAABlendBuffer.bindTarget();
	clear_target(&mSMAABlendBuffer, GL_COLOR_BUFFER_BIT);
	
	shaderp = &gPostSMAABlendWeights[RenderDeferredAAQuality];
	shaderp->bind();
	shaderp->uniform4fv(sSmaaRTMetrics, 1, rt_metrics);

	channel = shaderp->enableTexture(LLShaderMgr::SMAA_EDGE_TEX,
									 mFXAABuffer.getUsage());
	unit1p = channel > -1 ? gGL.getTexUnit(channel) : NULL;
	if (unit1p)
	{
		mFXAABuffer.bindTexture(0, channel, LLTexUnit::TFO_BILINEAR);
		unit1p->setTextureAddressMode(LLTexUnit::TAM_CLAMP);
	}
	channel = shaderp->enableTexture(LLShaderMgr::SMAA_AREA_TEX,
									 LLTexUnit::TT_TEXTURE);
	LLTexUnit* unit2p = channel > -1 ? gGL.getTexUnit(channel) : NULL;
	if (unit2p)
	{
		unit2p->bindManual(LLTexUnit::TT_TEXTURE, mAreaMap);
		unit2p->setTextureFilteringOption(LLTexUnit::TFO_BILINEAR);
		unit2p->setTextureAddressMode(LLTexUnit::TAM_CLAMP);
	}
	channel = shaderp->enableTexture(LLShaderMgr::SMAA_SEARCH_TEX,
									 LLTexUnit::TT_TEXTURE);
	LLTexUnit* unit3p = channel > -1 ? gGL.getTexUnit(channel) : NULL;
	if (unit3p)
	{
		unit3p->bindManual(LLTexUnit::TT_TEXTURE, mSearchMap);
		unit3p->setTextureFilteringOption(LLTexUnit::TFO_BILINEAR);
		unit3p->setTextureAddressMode(LLTexUnit::TAM_CLAMP);
	}
	...
	if (unit2p)
	{
		unit2p->unbindFast(LLTexUnit::TT_TEXTURE);
	}
	if (unit3p)
	{
		unit3p->unbindFast(LLTexUnit::TT_TEXTURE);
	}
...
// PBR renderer only.
void LLPipeline::applySMAA(LLRenderTarget* srcp, LLRenderTarget* dstp)
{
	...
	dstp->bindTarget();
	clear_target(dstp, GL_COLOR_BUFFER_BIT);
	...
	channel = shaderp->enableTexture(LLShaderMgr::SMAA_BLEND_TEX);
	LLTexUnit* unit2p = channel > -1 ? gGL.getTexUnit(channel) : NULL;
	if (unit2p)
	{
		mSMAABlendBuffer.bindTexture(0, channel, LLTexUnit::TFO_BILINEAR);
	}
...
// PBR renderer only.
void LLPipeline::renderDoF(LLRenderTarget* srcp, LLRenderTarget* dstp)
{
	...
	dstp->bindTarget();
	if (RenderFSAASamples > 1 && mFXAABuffer.isComplete())
	{
...
{
	LL_TRACY_TIMER(TRC_RENDER_SSR_TRACE);
...
{
	LL_TRACY_TIMER(TRC_RENDER_SSR_ALPHA);
	...
	U32 ssrdepth = mSSRBuffer.getDepth();
	if (ssrdepth && ssrdepth != mRT->mDeferredScreen.getDepth())
	{
		mSSRBuffer.blitDepthFrom(mRT->mDeferredScreen);
	}
	...
	static const F32 sIdentityTransform[8] =
		{ 1.f, 1.f, 0.f, 0.f, 0.f, 0.f, 0.f, 0.f };

	LLViewerTexture* white_texp = LLViewerFetchedTexture::sWhiteImagep.get();
	LLTexUnit* unit0 = gGL.getTexUnit(0);
	LLVOAvatar* last_avatarp = NULL;
	U64 last_hash = 0;
	for (U32 rigged = 0; rigged < 2; ++rigged)
	{
		LLGLSLShader* shaderp = rigged ? &gSkinnedSSRAlphaProgram
									   : &gSSRAlphaProgram;
		// Bind shader once per pass, then use fast path for re-syncing
		bindDeferredShader(*shaderp);
		shaderp->mCanBindFast = true;

		U32 type = rigged ? LLRenderPass::PASS_ALPHA_RIGGED
						  : LLRenderPass::PASS_ALPHA;
		LLCullResult::sg_list_t& groups =
			rigged ? sCull->getRiggedAlphaGroups() : sCull->getAlphaGroups();
		for (U32 i = 0, count = groups.size(); i < count; ++i)
		{
			LLSpatialGroup* groupp = groups[i];
			if (!groupp || groupp->isDead() || !groupp->getSpatialPartition() ||
				!groupp->getSpatialPartition()->mRenderByGroup)
			{
				continue;
			}
			LLSpatialGroup::drawmap_elem_t& draw_info = groupp->mDrawMap[type];
			for (auto it = draw_info.begin(); it != draw_info.end(); ++it)
			{
				LLDrawInfo& params = **it;
				if ((bool)params.mAvatar != (rigged == 1))
				{
					continue;
				}

				LLRenderPass::applyModelMatrix(params);
				bindDeferredShaderFast(*shaderp);

				if (params.mGLTFMaterial)
				{
					// glTF material: bind all material textures and uniforms
					// (diffuseMap, specularMap/ORM, bumpMap, emissiveMap,
					// roughnessFactor, texture transforms, etc.)
					LLGLDisable cull_face(params.mGLTFMaterial->mDoubleSided ?
											GL_CULL_FACE : 0);
					params.mGLTFMaterial->bind(params.mTexture, params.mVSize);
				}
				else
				{
					// Legacy: bind diffuse texture for alpha sampling
					LLViewerTexture* texp = params.mTexture.get();
					if (!texp)
					{
						texp = white_texp;
					}
					shaderp->bindTexture(LLShaderMgr::DIFFUSE_MAP, texp);

					// Bind white for ORM so shader reads roughness=1.0 from
					// green channel, making final roughness = roughness_factor
					// directly.
					shaderp->bindTexture(LLShaderMgr::SPECULAR_MAP,
										 white_texp);

					// Legacy glossiness from specular color alpha
					F32 roughness = 1.f - params.mSpecColor.mV[3];
					shaderp->uniform1f(LLShaderMgr::ROUGHNESS_FACTOR,
									   roughness);

					// Identity PBR texture transforms for legacy materials
					shaderp->uniform4fv(LLShaderMgr::TEXTURE_BASE_COLOR_TRANSFORM,
										2, sIdentityTransform);
					shaderp->uniform4fv(LLShaderMgr::TEXTURE_ROUGHNESS_TRANSFORM,
										2, sIdentityTransform);
					shaderp->uniform1f(LLShaderMgr::MINIMUM_ALPHA, -1.f);
				}

				// Handle texture animation (LSL texture anim)
...
{
	LL_TRACY_TIMER(TRC_RENDER_SSR_WATER);
	...
	LLDrawPoolWater* poolp = (LLDrawPoolWater*)mWaterPool;
	if (!poolp) return;		// Paranoia
	...
	// modelview that would corrupt light volume positions in deferred
	// lighting.
	gGLLastMatrix = NULL;
	gGL.matrixMode(LLRender::MM_MODELVIEW);
	gGL.loadMatrix(gGLModelView);
...
{
	LL_TRACY_TIMER(TRC_RENDER_SSR_FILTER);
	...
	U32 mips = mSSRBuffer.getMipLevels();
	U32 tempmips = mSSRMipTemp.size();
	if (mips <= 1 || !tempmips)
	{
		return;
	}
	...
	static LLCachedControl<F32> scale(gSavedSettings, "RenderSSRFilterScale");
	gGaussianProgram.bind();
	S32 chan = gGaussianProgram.enableTexture(LLShaderMgr::DEFERRED_DIFFUSE,
											  LLTexUnit::TT_TEXTURE);
	LLTexUnit* unitp = chan > -1 ? gGL.getTexUnit(chan) : NULL;
	if (unitp)
	{
		S32 ssr_w = mSSRBuffer.getWidth();
		for (U32 m = 1; m < mips && m - 1 < tempmips; ++m)
		{
			LLRenderTarget& temp = mSSRMipTemp[m - 1];

			// Horizontal pass: read SSR mip m-1, write to temp (at mip m
			// resolution)
			F32 w = llmax(1.f, ssr_w >> (m - 1));
			gGaussianProgram.uniform1f(sResScale, scale / w);
			gGaussianProgram.uniform2f(sDirection, 1.f, 0.f);

			unitp->bind(&mSSRBuffer);
			...
			// Vertical pass: read temp, write to SSR mip m (same resolution as
			// temp)
			gGaussianProgram.uniform1f(sResScale,
									   scale / (F32)temp.getHeight());
		...
		// bindColorMipLevel() bypasses the RT stack (with direct
		// glBindFramebuffer), so we must manually unbind the FBO here to avoid
		// corrupting GL state for renderDeferredLighting().
...
{
	LL_TRACY_TIMER(TRC_RENDER_SSR_HIZ);

	U32 mips = mSceneMap.getMipLevels();
	if (mips <= 1)
	{
		return;
	}
	...
	gHiZReduceProgram.bindTexture(LLShaderMgr::DEFERRED_DEPTH, &mSceneMap,
								  true, LLTexUnit::TFO_POINT);

	for (U32 mip = 1; mip < mips; ++mip)
	{
		// Restrict readable mip range to [0, mip-1] to avoid feedback
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_BASE_LEVEL, 0);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAX_LEVEL, mip - 1);

		gHiZReduceProgram.uniform1i(sHiZSrcLevel, mip - 1);

		mSceneMap.bindDepthMipLevel(mip);
											...
											LLRenderTarget* dstp)
{
	...
	gGL.getTexUnit(depth_chan)->bind(&mRT->mDeferredScreen, true);
	gGL.getTexUnit(depth_chan)->setTextureFilteringOption(LLTexUnit::TFO_POINT);
	mScreenTriangleVB->setBuffer();
	mScreenTriangleVB->drawArrays(LLRender::TRIANGLES, 0, 3);
...
void LLPipeline::renderFinalizePBR()
{
	...
	if (sRenderHDR && mLuminanceMap.isComplete() && mExposureMap.isComplete())
	{
		generateLuminance(&mRT->mScreen, &mLuminanceMap);
		generateExposure(&mLuminanceMap, &mExposureMap);

		toneMap(&mRT->mScreen, &mPostMap);
	}
	else
	{
		mToneMapped = false;
		// Needed to merge the deferred screen buffer. HB
		copyRenderTarget(&mRT->mScreen, &mPostMap);
	}
	...
	// Note generateSMAABuffers() will return false if anything goes wrong or
	// if anti-aliasing has been disabled by the user. HB
	bool do_use_smaa = use_smaa && generateSMAABuffers(work_targetp);

	gammaCorrect(work_targetp, final_targetp);

	LLVertexBuffer::unbind();

	generateGlow(final_targetp);
	combineGlow(final_targetp, work_targetp);
	...
	{
		applyCAS(final_targetp, work_targetp);
	}
	else
	{
		std::swap(final_targetp, work_targetp);
	}

	renderDoF(work_targetp, final_targetp);
	...
	if (RenderBufferVisualization > -1)
	{
		...
		else if (RenderBufferVisualization == 5)
		{
			visualizeBuffers(&mFXAABuffer, final_targetp, 0);
		}
		else if (RenderBufferVisualization == 6)
		{
			visualizeBuffers(&mSMAABlendBuffer, final_targetp, 0);
		}
		else if (RenderBufferVisualization == 7)
		{
			visualizeBuffers(&mSSRBuffer, final_targetp, 0);
		}
		else
		{
	...
	// Add noise as part of final render to screen pass to avoid damaging
	// other post effects.
	...
	gDeferredPostNoDoFNoiseProgram.bindTexture(LLShaderMgr::DEFERRED_DIFFUSE,
											   final_targetp);
	gDeferredPostNoDoFNoiseProgram.bindTexture(LLShaderMgr::DEFERRED_DEPTH,
											   &mRT->mDeferredScreen, true);
	gDeferredPostNoDoFNoiseProgram.uniform2f(LLShaderMgr::DEFERRED_SCREEN_RES,
											 final_targetp->getWidth(),
											 final_targetp->getHeight());
	{
		LLGLDepthTest depth_test(GL_TRUE, GL_TRUE, GL_ALWAYS);
		mScreenTriangleVB->setBuffer();
		mScreenTriangleVB->drawArrays(LLRender::TRIANGLES, 0, 3);
	...
	if (sRenderDeferred)
	{
		...
		if (multisample)
		{
			...
			if (use_smaa && LLViewerShaderMgr::sHasSMAA && mAreaMap &&
				mSearchMap && mSMAAEdgeBuffer.isComplete() &&
				mSMAABlendBuffer.isComplete() && mScratchBuffer.isComplete())
			{
				// Note: all buffers got the same size.
				S32 width = mRT->mScreen.getWidth();
				S32 height = mRT->mScreen.getHeight();
				mFXAABuffer.copyContents(mRT->mDeferredLight, 0, 0, width,
			...
			else if (LLViewerShaderMgr::sHasFXAA)
			{
				...
				if (RenderDeferredAASharpen)
				{
					mScratchBuffer.bindTarget();
					glClearColor(0.f, 0.f, 0.f, 0.f);
					mScratchBuffer.clear(GL_COLOR_BUFFER_BIT);
				}
				else
				{
					gViewerWindowp->setupViewport();
...
void LLPipeline::bindLightFunc(LLGLSLShader& shader)
{
	...
	if (gUsePBRShaders && mPbrBrdfLut.isComplete())
	{
...
void LLPipeline::bindReflectionProbes(LLGLSLShader& shader)
{
	...
	if (sRenderMirrors)
	{
		chan = shader.enableTexture(LLShaderMgr::HERO_PROBE,
									LLTexUnit::TT_CUBE_MAP_ARRAY);
		if (chan > -1 && mHeroProbeManager.mTexture.notNull())
		{
			mHeroProbeManager.mTexture->bind(chan);
			bound = true;
		}
	}
	...
	// from the SSR buffer. Forward alpha shaders also need trace bindings so
	// that transparent surfaces can trace their own SSR inline rather than
	// reading incorrect opaque-surface SSR from the pre-computed buffer.
	bool in_trace = &shader == &gScreenSpaceReflTraceProgram ||
	...
	if (chan > -1)
	{
		if (!in_trace && !gCubeSnapshot && mSSRBuffer.isComplete())
		{
			// Lighting pass: bind pre-computed SSR buffer with trilinear for
			// mip sampling
			gGL.getTexUnit(chan)->bind(&mSSRBuffer);
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
							GL_LINEAR_MIPMAP_LINEAR);

			// Pass mip scale for roughness -> mip level mapping
			shader.uniform1f(sSsrMipScale, F32(mSSRBuffer.getMipLevels() - 1));
		}
		else
		{
			// If in_trace, bind previous frame's lit scene for ray marching
			// and if not (fallback or cube snapshot), bind scene map (probes
			// should not see the main camera's SSR buffer).
	...
	static LLCachedControl<U32> ssritr(gSavedSettings, "RenderSSRIterations");
	static LLCachedControl<F32> ssrthick(gSavedSettings,
										 "RenderSSRMaxThickness");
	static LLCachedControl<F32> ssrbias(gSavedSettings, "RenderSSRDepthBias");
	static LLCachedControl<U32> ssrdepth(gSavedSettings, "RenderSSRMaxDepth");
	static LLCachedControl<F32> ssrrough(gSavedSettings,
										 "RenderSSRMaxRoughness");
	static LLCachedControl<U32>	ssrgloss(gSavedSettings,
										 "RenderSSRGlossySamples");
	shader.uniform1i(LLShaderMgr::DEFERRED_SSR_ITR_COUNT, ssritr());
	shader.uniform1f(LLShaderMgr::DEFERRED_SSR_MAX_THICKNESS, ssrthick());
	shader.uniform1f(LLShaderMgr::DEFERRED_SSR_DEPTH_BIAS, ssrbias());
	shader.uniform1f(LLShaderMgr::DEFERRED_SSR_GLOSSY_SAMPLES, F32(ssrgloss));
	...
	// with the count varying by what is visible, destabilizing the noise
	// seed between frames and causing SSR to flicker.
	if (&shader == &gScreenSpaceReflTraceProgram ||
		&shader == &gSSRAlphaProgram || &shader == &gSkinnedSSRAlphaProgram ||
		&shader == &gSSRWaterProgram)
	{
		if (++mPoissonOffset + ssrgloss > 128)
		{
			mPoissonOffset = 0;
		}
	}
	shader.uniform1f(LLShaderMgr::DEFERRED_SSR_NOISE_SINE,
					 F32(mPoissonOffset));
	shader.uniform1f(LLShaderMgr::DEFERRED_SSR_MAX_Z, F32(ssrdepth));
	shader.uniform1f(LLShaderMgr::DEFERRED_SSR_MAX_ROUGHNESS, ssrrough());

	// *TODO: implement SMAA T2x. HB
	shader.uniform2f(sSSRJitterOffset, 0.f, 0.f);
	...
	shader.uniform1i(sHiZMipCount, (S32)mSceneMap.getMipLevels());
}
									...
									LLRenderTarget* depth_targetp)
{
	...
	if (!gUsePBRShaders)
	{
		depth_targetp = &mDeferredDepth;
	}
	else if (!depth_targetp)
	{
		depth_targetp = deferred_targetp;
	}
	shader.bind();
	...
	chan = shader.enableTexture(LLShaderMgr::NORMAL_MAP, usage);
	if (chan > -1)
	{
	...
	{
		chan = shader.enableTexture(LLShaderMgr::EXPOSURE_MAP);
		if (chan > -1 && mExposureMap.isComplete())
		{
...
#endif
	{
		if (RenderDeferredSSAO && (!gUsePBRShaders || !gCubeSnapshot))
		{
			shader.uniform1f(LLShaderMgr::DEFERRED_SSAO_RADIUS,
							 RenderSSAOScale);
			shader.uniform1f(LLShaderMgr::DEFERRED_SSAO_MAX_RADIUS,
							 RenderSSAOMaxScale);
			shader.uniform1f(LLShaderMgr::DEFERRED_SSAO_FACTOR,
							 RenderSSAOFactor);
		}
	}
#if DEBUG_OPTIMIZED_UNIFORMS && OPTIMIZED_UNIFORMS
	...
	if (gUsePBRShaders)
	{
		shader.uniform3fv(LLShaderMgr::SUNLIGHT_COLOR, 1,
						  mSunLightColorScaled.mV);
		shader.uniform3fv(LLShaderMgr::MOONLIGHT_COLOR, 1, mMoonLightColor.mV);
		shader.uniform1f(LLShaderMgr::REFLECTION_PROBE_MAX_LOD,
						 mReflectionMapManager.mMaxProbeLOD);
	}
	else
	{
		shader.uniform4fv(LLShaderMgr::SUNLIGHT_COLOR, 1, mSunLightColor.mV);
		shader.uniform4fv(LLShaderMgr::MOONLIGHT_COLOR, 1, mMoonLightColor.mV);
	}
...
void LLPipeline::renderDeferredLightingPBR()
{
...

	{
		...
		if ((RenderDeferredSSAO && !gCubeSnapshot) || RenderShadowDetail > 0)
		{
			...
			shaderp = gCubeSnapshot ? &gDeferredSunProbeProgram
									: &gDeferredSunProgram;

			// Note: bindDeferredShader() takes care of passing the
			// DEFERRED_SSAO* and DEFERRED_NOISE uniforms when needed. HB
			bindDeferredShader(*shaderp, light_targetp);
			mScreenTriangleVB->setBuffer();
			clear_target(light_targetp, GL_COLOR_BUFFER_BIT);
...
#if 0	// No more needed: shadow softening is now automatic in PBR shaders. HB
		if (RenderShadowSoften && !gCubeSnapshot)
		{
			...
			glClearColor(1.f, 1.f, 1.f, 1.f);
			clear_target(screen_targetp, GL_COLOR_BUFFER_BIT);
			glClearColor(0.f, 0.f, 0.f, 0.f);
		...
		if (RenderDeferredAtmospheric)
		{
			...
			// Use cube snapshot variant shader when rendering to reflection
			// probes
			shaderp = gCubeSnapshot ? &gDeferredSoftenCubeProgram
			...
			// Pass default_probe_render flag when rendering to reflection
			// probes
			if (gCubeSnapshot)
			{
				shaderp->uniform1i(LLShaderMgr::DEFAULT_PROBE_RENDER,
								   sDefaultProbeRender ? 1 : 0);
			}
					 ...
					 iter != end; ++iter)
				{
						...
						cam_z > c[2] + s + 0.2f || cam_z < c[2] - s - 0.2f)
					{
						...
						if (RenderLocalLightCount)
						{
							...
							shaderp->uniform1i(LLShaderMgr::CLASSIC_MODE,
											   LLRender::sClassicMode);
							gGL.syncMatrices();
...
void LLPipeline::unbindDeferredShader(LLGLSLShader& shader)
{
	...
	shader.disableTexture(LLShaderMgr::NORMAL_MAP, usage);
							  ...
							  bool depth_clamp)
{
	...
	if (depth_clamp)
	{
		...
		if (!dclamp || !LLGLManager::sUseDepthClamp)
		{
			depth_clamp = false;
		}
	}
	...
	for (U32 rigged = 0; rigged < 2; ++rigged)
	{
		...
		if (rigged)
		{
			LLRenderPass::pushRiggedGLTFBatches(type + 1);
		}
		else
		{
			LLRenderPass::pushGLTFBatches(type);
		}
		gGL.loadMatrix(gGLModelView);
		gGLLastMatrix = NULL;
	}
							  ...
							  U32 target_width)
{
	...
	LLDisableOcclusionCulling no_occlusion(!use_occlusion);
...
void LLPipeline::generateSunShadowPBR()
{
	...
	if (mSunLightColor == LLColor4::black)
	{
...
void LLPipeline::renderGroups(LLRenderPass* passp, U32 type, U32 mask,
							  bool texture)
{
	LLCullResult::sg_list_t& visible_groups = sCull->getVisibleGroups();
	for (U32 i = 0, count = visible_groups.size(); i < count; ++i)
	{
		...
		if (groupp && !groupp->isDead() &&
			(!sUseOcclusion ||
			 !groupp->isOcclusionState(LLSpatialGroup::OCCLUDED)) &&
			hasRenderType(groupp->getSpatialPartition()->mDrawableType) &&
			groupp->mDrawMap.count(type))
		{
			passp->renderGroup(groupp, type, mask, texture);
		}
	}
}

void LLPipeline::renderRiggedGroups(LLRenderPass* passp, U32 type, U32 mask,
									bool texture)
{
	LLCullResult::sg_list_t& visible_groups = sCull->getVisibleGroups();
	for (U32 i = 0, count = visible_groups.size(); i < count; ++i)
	{
		...
		if (groupp && !groupp->isDead() &&
			...
			groupp->mDrawMap.count(type))
		{
			passp->renderRiggedGroup(groupp, type, mask, texture);
		}
	}
}
...
//mk
...

	{
		...
		if (!avatarp->mImpostor.isComplete())
		{
			...
			if (sRenderDeferred)
			{
				add_deferred_attachments(avatarp->mImpostor);
			}
...
void LLPipeline::previewAvatar(LLVOAvatar* avatarp)
{
	...
	sReflectionRender = sShadowRender = false;
...
void LLPipeline::rebuildDrawInfo()
{
		 ...
		 iter != end; ++iter)
	{
		...
		partp = regionp->getSpatialPartition(LLViewerRegion::PARTITION_AVATAR);
		dirty.traverse(partp->mOctree);
	}
}
