...
bool HBFloaterTeleportHistory::postBuild()
{
	...
	mTeleportArrivingConnection =
		gViewerParcelMgr.setTPArrivingCallback(std::bind(&HBFloaterTeleportHistory::onTeleportArriving));
	mTeleportFinishConnection =
		gViewerParcelMgr.setTPFinishedCallback(std::bind(&HBFloaterTeleportHistory::onTeleportFinished,
														   _1, _2));
	mTeleportFailedConnection =
		gViewerParcelMgr.setTPFailedCallback(std::bind(&HBFloaterTeleportHistory::onTeleportFailed));
...
std::string HBFloaterTeleportHistory::getHistoryFileName(bool fallback) const
{
	std::string name = gIsInSecondLifeBetaGrid ? "beta_tp_history.xml"
											   : "tp_history.xml";
	name = gDirUtil.getFullPath(LL_PATH_PER_ACCOUNT, name);
	if (fallback && !LLFile::isfile(name))
	{
		name = gDirUtil.getFullPath(LL_PATH_PER_ACCOUNT,
...
void HBFloaterTeleportHistory::populateLists(const LLSD& file_data)
{
	...
	typedef std::map<std::string, HBTeleportLocation*, std::less<> > loc_map_t;
	loc_map_t favorites;
	...
	for (S32 idx = 0, entries = file_data.size(); idx < entries; ++idx)
	{
			...
			has_timestamp)
		{
			...
			if (is_arrival)
			{
				...
				{
					loc_map_t::iterator it = favorites.find(value);
					if (it != favorites.end())
					{
						++it->second->mVisits;
	...
	if (!favorites.empty())
	{
		...
		S32 count = 0;
		for (loc_map_t::iterator it = favorites.begin(), end = favorites.end();
			 it != end; ++it)
		{
...
void HBFloaterTeleportHistory::updateSearchResults()
{
	...
	strings_set_t places;
	std::string name;
	LLScrollListCtrl::items_vec_t data = mPlacesList->getAllData();
	S32 results = 0;
