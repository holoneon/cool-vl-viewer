...
bool friendship_offer_callback(const LLSD& notification, const LLSD& response)
{
	...
	if (option == 0)		// Accept
	{
		...
		gCoros.launch("acceptFriendshipOffer",
					  std::bind(&accept_friendship_coro, url, payload));
	}
	else if (option == 1)	// Decline
	{
		...
		gCoros.launch("declineFriendshipOffer",
					  std::bind(&decline_friendship_coro, url, payload));
	}
									   ...
									   LLIMMgr::EInvitationType type)
{
	...
	LLSD params;
	params["voice_server_type"] = "webrtc";
	...
	switch (option)
	{
			...
			else
			{
				...
				if (!url.empty())
				{
					...
					gCoros.launch("chatterBoxInvitationCoro",
								  std::bind(&LLIMMgr::chatterBoxInvitationCoro,
		...
		case 1: // Decline
		{
			...
			if (type == IM_SESSION_P2P_INVITE)
			{
				...
				if (server_type == LLVoiceClient::WEBRTC_SERVER)
				{
					const std::string& url =
						gAgent.getRegionCapability("ChatSessionRequest");
					if (!url.empty())
									   ...
									   EInstantMessage dialog, bool use_p2p)
{
	...
	if (dialog == IM_SESSION_CONFERENCE_START)
	{
		...
		else
		{
			gCoros.launch("startConferenceCoro",
					  std::bind(&LLIMMgr::startConferenceCoro, url,
	...
	else
	{
		gCoros.launch("startP2PVoiceCoro",
					  std::bind(&LLIMMgr::startP2PVoiceCoro, url,
								temp_session_id, gAgentID,
		...
		if (caller_name.empty())
		{
			if (gCacheNamep)
			{
				gCacheNamep->get(caller_id, false,
								 std::bind(&LLIMMgr::onInviteNameLookup, _1,
										   _2, _3, payload));
			}
		}
		else
...
void LLIMMgr::processNewMessage(const LLUUID& from_id, bool from_group,
								...
								const LLSD& metadata, const LLUUID& aux_id)
{
	...
	// New (2024-10) IM meta data
	bool is_bot = metadata.has("sender") && metadata["sender"].has("bot") &&
				  metadata["sender"]["bot"].asBoolean();
	if (metadata.has("notice") && metadata["notice"].has("id"))
	{
		// The simulator has injected some sort of notice into the conversation
		name = SYSTEM_FROM;
		LLSD notice_args;
		if (metadata["notice"].has("data"))
		{
			notice_args = metadata["notice"]["data"];
		}
		std::string msg =
			LLTrans::getString(metadata["notice"]["id"].asString(),
							   notice_args);
		if (!msg.empty())
		{
			message = msg;
	...
	bool is_muted = name != SYSTEM_FROM &&
					LLMuteList::isMuted(from_id, name, LLMute::flagTextChat);
	bool is_linden = name != SYSTEM_FROM && LLMuteList::isLinden(name);
...
//mk
					 offline == IM_ONLINE && !is_linden && name != SYSTEM_FROM)
			{
				...
				addMessage(session_id, from_id, name, buffer,
						   ...
						   region_id, position, true, is_bot);
...
//mk
			{
				addMessage(computeSessionID(dialog, from_id), from_id,
						   ...
						   region_id, position, false, is_bot);
			}
			break;
		}
...
//mk
		...
		case IM_GROUP_NOTICE_REQUESTED:
		{
			...
			LLOfferInfo* infop = NULL;
			is_muted = LLMuteList::isMuted(LLUUID::null, name, 0,
										   LLMute::AGENT);
			if (has_inventory && !is_muted)
			{
				infop = new LLOfferInfo;
				infop->mIM = IM_GROUP_NOTICE;
				infop->mFromID = from_id;
				infop->mFromObject = false;
				infop->mFromGroup = from_group;
				infop->mTransactionID = session_id;
				infop->mType = (LLAssetType::EType)asset_type;
				infop->mFolderID =
					gInventory.findCategoryUUIDForType(LLFolderType::assetTypeToFolderType(infop->mType));
				infop->mFromName = "A group member named " + name;
				infop->mDesc = item_name;
				infop->mHost = sender;
			...
			if (dialog != IM_GROUP_NOTICE_REQUESTED && !is_muted)
			{
				...
				payload["inventory_offer"] = infop ? infop->asLLSD() : LLSD();
		...
		{
			// Note: here is_muted can only match to a formerly muted group. HB
			if (is_busy || is_muted)
			{
			...
			// Sadly, IM_GROUP_INVITATION does not include the Id of the
			// inviting agent, so we cannot easily mute inviter bots. We need
			// to parse the message, which contains the inviting agent name (user
			// name between parenthesis in SL, and, hopefully, the full name in
			// OpenSim grids). HB
			LLStringUtil::trimHead(message);
			std::vector<std::string> muted_agents =
				LLMuteList::getMutedUserNames(gIsInSecondLife);
			for (size_t i = 0, count = muted_agents.size(); i < count; ++i)
			{
				const std::string& agent_name = muted_agents[i];
				if (message.find(agent_name) != std::string::npos)
				{
					llinfos << "Ignoring IM_GROUP_INVITATION from muted agent "
							<< agent_name << llendl;
					return;
				}
			}
			llinfos << "Received IM_GROUP_INVITATION message. from_id="
					<< from_id << " - aux_id=" << aux_id << llendl;
//MK
...
//mk
		...
		case IM_TASK_INVENTORY_OFFERED:
		{
			...
			LLOfferInfo* infop = new LLOfferInfo;
			...
			if (dialog == IM_INVENTORY_OFFERED)
			{
				...
				infop->mType = (LLAssetType::EType)bucketp->asset_type;
				infop->mObjectID = bucketp->object_id;
				infop->mFromObject = false;
			}
			...
			else
			{
				...
				{
					infop->mType = (LLAssetType::EType)binary_bucket[0];
				}
				else
				{
					...
					infop->mType = (LLAssetType::EType)type;
				...
				infop->mObjectID.setNull();
				infop->mFromObject = true;
			}

			// In the case of an offline message, the transaction Id is in
			// aux_id and the session_id is null
			infop->mTransactionID = session_id.notNull() ? session_id : aux_id;
...
//MK
			...
			if (gRLenabled &&
				...
				infop->mType == LLAssetType::AT_CATEGORY &&
				gRLInterface.getRlvShare() &&
				folder_name.find(RL_RLV_REDIR_FOLDER_PREFIX) == 1)
			{
				infop->mFolderID = gRLInterface.getRlvShare()->getUUID();
			}
			else
//mk
			{
...
//mk
					 ...
					 gSavedSettings.getBool("RejectNewInventoryWhenBusy"))
			{
				...
				infop->forceResponse(IOR_BUSY);
			}
			else
			{
				infop->inventoryOfferHandler();
			}
			break;
		}
...
//mk
			...
			addMessage(session_id, from_id, name, buffer,
					   ...
					   parent_estate_id, region_id, position, true, is_bot);
			if (gAutomationp)
			{
...
//mk
			if (message.empty())
			{
				...
				gNotifications.add("OfferFriendshipNoMessage", args, payload);
			}
			else
			{
				args["[MESSAGE]"] = message;
							 ...
							 EInstantMessage dialog)
{
	...
	const LLRelationship* infop =
		gAvatarTracker.getBuddyInfo(other_participant_id);
	U8 offline = (!infop || infop->isOnline()) ? IM_ONLINE : IM_OFFLINE;
...
bool LLIMMgr::requestOfflineMessages()
{
	...
	gCoros.launch("requestOfflineMessagesCoro",
				  std::bind(&LLIMMgr::requestOfflineMessagesCoro, cap_url));
	return true;
}
