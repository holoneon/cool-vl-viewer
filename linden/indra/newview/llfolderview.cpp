...
void LLFolderViewItem::connectRefreshCachedSettingsSafe(const char* name)
{
	...
	controlp->getSignal()->connect(std::bind(&LLFolderViewItem::refreshCachedSettings));
}
...
// potential_ancestor.
bool LLFolderViewItem::isDescendantOf(const LLFolderViewFolder* potential_ancestorp)
{
	LLFolderViewItem* rootp = this;
	while (rootp->mParentFolder)
	{
		if (rootp->mParentFolder == potential_ancestorp)
		{
			return true;
		}
		rootp = rootp->mParentFolder;
	}
	return false;
}
...
void LLFolderViewItem::refreshFromListener()
{
	if (mListener)
	{
		...
		// Clear font vertex buffers
		mLabelFontBuffer.reset();
		mSuffixFontBuffer.reset();

		LLInventoryItem* itemp = gInventory.getItem(mListener->getUUID());
		if (itemp)
		{
			std::string desc = itemp->getDescription();
			if (!desc.empty())
			{
...
{
	LLFolderViewItem* rootp = getRoot();
	if (rootp)
	{
		rootp->filter(*((LLFolderView*)rootp)->getFilter());
	}
}
...
void LLFolderViewItem::setSelectionFromRoot(LLFolderViewItem* selectionp,
										    bool openitem,
										    bool take_keyboard_focus)
{
	getRoot()->setSelection(selectionp, openitem, take_keyboard_focus);
}

// Helper method to change the selection from the root.
void LLFolderViewItem::changeSelectionFromRoot(LLFolderViewItem* selectionp,
											   bool selected)
{
	getRoot()->changeSelection(selectionp, selected);
}

void LLFolderViewItem::extendSelectionFromRoot(LLFolderViewItem* selectionp)
{
	std::vector<LLFolderViewItem*> selected_items;
	getRoot()->extendSelection(selectionp, NULL, selected_items);
}
...
//virtual
bool LLFolderViewItem::addToFolder(LLFolderViewFolder* folderp,
								   LLFolderView* rootp)
{
	if (!folderp || !rootp || !getListener())
	{
		return false;
	}

	mParentFolder = folderp;
	rootp->addItemID(getListener()->getUUID(), this);
	return folderp->addItem(this);
}
...
bool LLFolderViewItem::setSelection(LLFolderViewItem* selectionp,
									bool openitem, bool take_keyboard_focus)
{
	if (selectionp == this && !mIsSelected)
	{
		selectItem();
		if (mListener)
		{
...
bool LLFolderViewItem::changeSelection(LLFolderViewItem* selectionp,
									   bool selected)
{
	if (selectionp == this && mIsSelected != selected)
	{
	  	if (mIsSelected)
		{
			deselectItem();
...
void LLFolderViewItem::deselectItem()
{
	...
	if (parentp)
	{
		parentp->recursiveIncrementNumDescendantsSelected(-1);
	}
}
...
void LLFolderViewItem::buildContextMenu(LLMenuGL& menu, U32 flags)
{
	...
	{
		LLMenuItemGL* itemp = menu.getChild<LLMenuItemGL>("New Universal",
														  true, false);
		if (itemp)
		{
	...
	if ((want_desc || want_creator) && mListener)
	{
		...
		if (fetch_desc || fetch_creator)
		{
			...
			if (itemp)
			{
				if (fetch_desc)
				{
					mSearchableDesc = itemp->getDescription();
...
void LLFolderViewItem::draw()
{
	...
	if (!mLabel.empty())
	{
		...
		mLabelFontBuffer.render(fontp, mWLabel, 0, text_left, y, color,
								LLFontGL::LEFT, LLFontGL::BOTTOM, style,
								S32_MAX, S32_MAX, &right_x, false);
		if (!mLabelSuffix.empty())
		{
			mSuffixFontBuffer.render(fontp, mWLabelSuffix, 0, right_x, y,
									 sSuffixColor, LLFontGL::LEFT,
									 LLFontGL::BOTTOM, style, S32_MAX, S32_MAX,
									 &right_x, false);
		}
...
LLFolderViewFolder::LLFolderViewFolder(const std::string& name,
									   ...
									   LLFolderView* rootp,
									   LLFolderViewEventListener* listenerp)
:	LLFolderViewItem(name, iconp, 0, rootp, listenerp),	// 0 = no create time
...
//virtual
bool LLFolderViewFolder::addToFolder(LLFolderViewFolder* folderp,
									 LLFolderView* rootp)
{
	if (!folderp || !rootp || !getListener())
	{
...
void LLFolderViewFolder::recursiveIncrementNumDescendantsSelected(S32 increment)
{
	...
	do
	{
		parentp->mNumDescendantsSelected += increment;

		// Make sure we do not have negative values.
		llassert(parentp->mNumDescendantsSelected >= 0);
...
//virtual
bool LLFolderViewFolder::setSelection(LLFolderViewItem* selectionp,
									  bool openitem,
									  bool take_keyboard_focus)
{
	bool rv = false;
	...
	{
		folders_t::iterator fit = iter++;
		if ((*fit)->setSelection(selectionp, openitem, take_keyboard_focus))
		{
			rv = true;
			child_selected = true;
		}
	...
	{
		items_t::iterator iit = iter++;
		if ((*iit)->setSelection(selectionp, openitem, take_keyboard_focus))
		{
			rv = true;
			child_selected = true;
		}
...
// was changed.
bool LLFolderViewFolder::changeSelection(LLFolderViewItem* selectionp,
										 bool selected)
{
	bool rv = false;
	...
	{
		folders_t::iterator fit = iter++;
		if ((*fit)->changeSelection(selectionp, selected))
		{
			rv = true;
		}
	}
	...
	{
		items_t::iterator iit = iter++;
		if ((*iit)->changeSelection(selectionp, selected))
		{
			rv = true;
		}
	}
...
void LLFolderViewFolder::extendSelection(LLFolderViewItem* selectionp,
										 LLFolderViewItem* last_selectedp,
										 std::vector<LLFolderViewItem*>& selected_items)
{
	...
	{
		folders_t::iterator fit = iter++;
		(*fit)->extendSelection(selectionp, last_selectedp, selected_items);
	}
		 ...
		 iter != mFolders.end(); )
	{
		...
		if (found_selection || found_last_selected)
		{
			...
			if (itemp->isSelected())
			{
				itemp->changeSelection(itemp, false);
			}
			items_to_select.push_back(itemp);
		}
	...
	if (found_last_selected && found_selection)
	{
		...
		{
			LLFolderViewItem* itemp = items_to_select[index];
			if (itemp->changeSelection(itemp, true))
			{
				selected_items.push_back(itemp);
			}
		}
	}
	else if (found_selection)
	{
		...
		// the new itemp
		if (selectionp->changeSelection(selectionp, true))
		{
			selected_items.push_back(selectionp);
		}
	}
}
...
void LLFolderViewFolder::recursiveDeselect(bool deselect_self)
{
		 ...
		 iter != mFolders.end(); )
	{
		...
		if (folderp)
		{
			folderp->recursiveDeselect(true);
		}
	}
}
...
void LLFolderViewFolder::destroyView()
{
		 ...
		 iter != mItems.end(); )
	{
		...
		{
			getRoot()->removeItemID(itemp->getListener()->getUUID());
		}
	}
...
// item was deleted.
bool LLFolderViewFolder::removeItem(LLFolderViewItem* itemp)
{
	if (itemp->remove())
	{
...
// listeners
void LLFolderViewFolder::removeView(LLFolderViewItem* itemp)
{
	if (!itemp || itemp->getParentFolder() != this)
	{
		return;
	}
	...
	getRoot()->removeFromSelectionList(itemp);
	extractItem(itemp);
	delete itemp;
}

// Removes the specified item from the folder, but does not delete it.
void LLFolderViewFolder::extractItem(LLFolderViewItem* itemp)
{
	items_t::iterator it = std::find(mItems.begin(), mItems.end(), itemp);
	if (it == mItems.end())
	{
		...
		LLFolderViewFolder* f = (LLFolderViewFolder*)itemp;
...
// date.
void LLFolderViewFolder::resort(LLFolderViewItem* itemp)
{
...
void LLFolderViewFolder::sortBy(U32 order)
{
	...
	if (order & LLInventoryFilter::SO_DATE)
	{
		...
		if (!mItems.empty())
		{
			LLFolderViewItem* itemp = *(mItems.begin());
			latest = itemp->getCreationDate();
		}

		if (!mFolders.empty())
		{
			LLFolderViewFolder* folderp = *(mFolders.begin());
			if (folderp->getCreationDate() > latest)
			{
				latest = folderp->getCreationDate();
			}
		}
...
//virtual
bool LLFolderViewFolder::addItem(LLFolderViewItem* itemp)
{
	if (!itemp) return false;	// Paranoia

	items_t::iterator it = std::lower_bound(mItems.begin(), mItems.end(),
											itemp, mSortFunction);
	mItems.insert(it, itemp);
	if (itemp->isSelected())
	{
		recursiveIncrementNumDescendantsSelected(1);
	}
	itemp->setRect(LLRect(0, 0, getRect().getWidth(), 0));
	itemp->setVisible(false);
	addChild(itemp);
	itemp->dirtyFilter();
	requestArrange();
...
// This is an internal method used for adding items to folders.
//virtual
bool LLFolderViewFolder::addFolder(LLFolderViewFolder* folderp)
{
	folders_t::iterator it = std::lower_bound(mFolders.begin(), mFolders.end(),
											  folderp, mSortFunction);
	mFolders.insert(it, folderp);
	if (folderp->numSelected())
	{
		recursiveIncrementNumDescendantsSelected(folderp->numSelected());
	}
	folderp->setOrigin(0, 0);
	folderp->reshape(getRect().getWidth(), 0);
...
LLFolderViewItem* LLFolderViewFolder::getNextFromChild(LLFolderViewItem* itemp,
...
LLFolderViewItem* LLFolderViewFolder::getPreviousFromChild(LLFolderViewItem* itemp,
														   bool include_children)
{
	bool found_item = false;
...
public:
	...
	// Set the sort order.
	LL_INLINE void doFolder(LLFolderViewFolder* folderp) override
	{
		folderp->setItemSortOrder(mSortOrder);
	}
...
//virtual
void LLOpenFolderByID::doFolder(LLFolderViewFolder* folderp)
{
	if (folderp->getListener() && folderp->getListener()->getUUID() == mID)
	{
		folderp->setOpenArrangeRecursively(true,
...
//	LLFolderView* rootp = getRoot();
//	if (rootp)
//	{
//		rootp->arrange(NULL, NULL);
...
public:
	...
	void doFolder(LLFolderViewFolder* folderp) override;
	void doItem(LLFolderViewItem* itemp) override;
...
// Set the sort order.
void LLCloseAllFoldersFunctor::doFolder(LLFolderViewFolder* folderp)
{
	folderp->setOpenArrangeRecursively(mOpen);
}

// Do nothing.
void LLCloseAllFoldersFunctor::doItem(LLFolderViewItem*)
{
}
...
LLFolderView::~LLFolderView()
{
	...
	if (mRenamer && gFocusMgr.getTopCtrl() == mRenamer)
	{
...
bool LLFolderView::toggleSearchShowLinks()
{
	bool show_links = !getFilterShowLinks();
	mFilter.setFilterShowLinks(show_links);
	if (mFilter.isActive())
	{
		mFilter.setModified();
	}
...
//virtual
bool LLFolderView::addFolder(LLFolderViewFolder* folderp)
{
	if (!folderp) return false;	// Paranoia

	// Enforce sort order of "My inventory" followed by Library
	if (folderp->getListener() &&
		folderp->getListener()->getUUID() == gInventory.getLibraryRootFolderID())
	{
		mFolders.push_back(folderp);
	}
	else
	{
		mFolders.insert(mFolders.begin(), folderp);
	}
	if (folderp->numSelected())
	{
		recursiveIncrementNumDescendantsSelected(folderp->numSelected());
...
void LLFolderView::addToSelectionList(LLFolderViewItem* itemp)
{
	if (itemp->isSelected())
	{
		removeFromSelectionList(itemp);
	}
	if (!mSelectedItems.empty())
	{
		mSelectedItems.back()->setIsCurSelection(false);
	}
	itemp->setIsCurSelection(true);
	mSelectedItems.push_back(itemp);
}

void LLFolderView::removeFromSelectionList(LLFolderViewItem* itemp)
{
...
//virtual
bool LLFolderView::setSelection(LLFolderViewItem* selectionp, bool openitem,
								bool take_keyboard_focus)
{
	if (selectionp == this)
	{
	...
	if (selectionp && take_keyboard_focus)
	{
		setFocus(true);
	}
	...
	if (selectionp)
	{
		addToSelectionList(selectionp);
	}

	bool rv = LLFolderViewFolder::setSelection(selectionp, openitem,
											   take_keyboard_focus);
	if (openitem && selectionp)
	{
		selectionp->getParentFolder()->requestArrange();
	}
...
bool LLFolderView::changeSelection(LLFolderViewItem* selectionp, bool selected)
{
	// Cannot select the root folder
	if (!selectionp || selectionp == this)
	{
		return false;
	}
	...
	if (!selected && on_list)
	{
		removeFromSelectionList(selectionp);
	}

	bool rv = LLFolderViewFolder::changeSelection(selectionp, selected);
...
void LLFolderView::extendSelection(LLFolderViewItem* selectionp,
								   LLFolderViewItem* last_selectedp,
								   std::vector<LLFolderViewItem*>& items)
{
	...
	if (mAllowMultiSelect)
	{
		LLFolderViewItem* currentp = getCurSelectedItem();
		LLFolderViewFolder::extendSelection(selectionp, currentp, items);
	...
	{
		setSelection(selectionp, false, false);
	}
...
void LLFolderView::sanitizeSelection()
{
		 ...
		 ++item_iter)
	{
		...
		if (parentp)
		{
			...
			else
			{
				...
				while (parentp)
				{
					visible = visible && parentp->isOpen() &&
							  parentp->potentiallyVisible();
					parentp = parentp->getParentFolder();
				}
			}
		}
		...
		if (!visible)
		{
			items_to_remove.push_back(itemp);
		}
			 ...
			 other_item_iter != mSelectedItems.end(); ++other_item_iter)
		{
			...
			for (parentp = other_item->getParentFolder(); parentp;
				 parentp = parentp->getParentFolder())
			{
				if (parentp == itemp)
				{
	...
	if (mSelectedItems.empty())
	{
		...
		// Inventory" otherwise
		LLFolderViewItem* new_selectionp = NULL;
		if (orig_selected)
		{
		...
		else
		{
			...
			// guess
			new_selectionp = getItemByID(gInventory.getRootFolderID());
		}

		if (new_selectionp)
		{
			setSelection(new_selectionp, false, false);
		}
	}
}
...
bool LLFolderView::getSelectionList(uuid_list_t& selection)
{
	for (selected_items_t::iterator it = mSelectedItems.begin(),
		 ...
		 it != end; ++it)
	{
		LLFolderViewItem* itemp = *it;
		if (itemp && itemp->getListener())
		{
			selection.emplace(itemp->getListener()->getUUID());
		}
	}
...
bool LLFolderView::getSelection(uuid_vec_t& selection)
{
		 ...
		 it != end; ++it)
	{
		...
		if (itemp && itemp->getListener())
		{
			selection.emplace_back(itemp->getListener()->getUUID());
		}
	}
	return !selection.empty();
}
...
bool LLFolderView::startDrag(LLToolDragAndDrop::ESource source)
{
	...
	if (!mSelectedItems.empty())
	{
			 ...
			 it != end; ++it)
		{
			...
			LLUUID id;
			can_drag = can_drag && itemp && itemp->getListener() &&
					   itemp->getListener()->startDrag(&type, &id);
...
void LLFolderView::commitRename(LLUICtrl*, void* datap)
{
	LLFolderView* rootp = reinterpret_cast<LLFolderView*>(datap);
	if (rootp)
	{
		rootp->finishRenamingItem();
	}
}
...
void LLFolderView::removeSelectedItems(bool confirm)
{
	if (getVisible() && getEnabled())
	{
			 ...
			 it != end; ++it)
		{
			...
			{
				llinfos << "Cannot delete " << itemp->getName() << llendl;
				return;
			}
		}
		...
		if (count == 1)
		{
			...
			LLFolderViewItem* new_selectionp =
				item_to_delete->getNextOpenNode(false);
			if (!new_selectionp)
			{
				new_selectionp = item_to_delete->getPreviousOpenNode(false);
			}
			if (parent && parent->removeItem(item_to_delete))
			{
				...
				if (new_selectionp)
				{
					setSelectionFromRoot(new_selectionp,
										 new_selectionp->isOpen(), has_focus);
				}
				else
				{
		...
		else if (count > 1)
		{
			...
			LLFolderViewEventListener* listenerp;
			LLFolderViewItem* last_itemp = items[count - 1];
			LLFolderViewItem* new_selectionp =
				last_itemp->getNextOpenNode(false);
			while (new_selectionp && new_selectionp->isSelected())
			{
				new_selectionp = new_selectionp->getNextOpenNode(false);
			}
			if (!new_selectionp)
			{
				new_selectionp = last_itemp->getPreviousOpenNode(false);
				while (new_selectionp && new_selectionp->isSelected())
				{
					new_selectionp = new_selectionp->getPreviousOpenNode(false);
				}
			}
			if (new_selectionp)
			{
				setSelectionFromRoot(new_selectionp, new_selectionp->isOpen(),
									 has_focus);
			}
			else
			{
			...
			{
				LLFolderViewItem* itemp = items[i];
				if (!itemp) continue;	// Paranoia

				listenerp = itemp->getListener();
				if (listenerp &&
					std::find(listeners.begin(), listeners.end(),
							  listenerp) == listeners.end())
				{
					listeners.push_back(listenerp);
				}
			}
			listenerp = listeners[0];
			if (listenerp)
			{
				listenerp->removeBatch(listeners);
			}
		}
		arrangeAll();
		scrollToShowSelection();
...
void LLFolderView::openSelectedItems()
{
		...
		new LLMultiProperties(LLRect(left, top, left + 300, top - 100));
	{
			 ...
			 it != end; ++it)
		{
			...
			bool is_prop = false;
			LLFolderViewEventListener* listenerp = itemp->getListener();
			if (listenerp)
			{
...
void LLFolderView::propertiesSelectedItems()
{
		...
		new LLMultiProperties(LLRect(left, top, left + 100, top - 100));
	{
			 ...
			 it != end; ++it)
		{
			...
			if (itemp && itemp->getListener())
			{
				itemp->getListener()->showProperties();
			}
		}
	}
...
void LLFolderView::autoOpenItem(LLFolderViewFolder* itemp)
{
	if (mAutoOpenItems.check() == itemp ||
	...
	while (close_item && close_item != itemp->getParentFolder())
	{
		mAutoOpenItems.pop();
	...
	itemp->requestArrange();

	mAutoOpenItems.push(itemp);

	itemp->setOpen(true);
	scrollToShowItem(itemp);
}
...
bool LLFolderView::autoOpenTest(LLFolderViewFolder* folderp)
{
	if (folderp && mAutoOpenCandidate == folderp)
	{
...
bool LLFolderView::canCopy() const
{
	...
	for (selected_items_t::const_iterator it = mSelectedItems.begin(),
		 ...
		 it != end; ++it)
	{
		const LLFolderViewItem* itemp = *it;
		if (!itemp || !itemp->getListener() ||
			!itemp->getListener()->isItemCopyable())
		{
			return false;
		}
	}
...
bool LLFolderView::canCut() const
{
		 ...
		 it != end; ++it)
	{
		LLFolderViewItem* itemp = *it;
		if (!itemp || !itemp->getListener() ||
			!itemp->getListener()->isItemMovable())
		{
			return false;
		}
	}
...
bool LLFolderView::canPaste() const
{
		 ...
		 it != end; ++it)
	{
		...
		const LLFolderViewEventListener* listenerp = itemp->getListener();
		if (!listenerp || !listenerp->isClipboardPasteable())
		{
			const LLFolderViewFolder* folderp = itemp->getParentFolder();
			if (!folderp)
			{
				return false;
			}
			listenerp = folderp->getListener();
			if (!listenerp || !listenerp->isClipboardPasteable())
			{
				return false;
			}
...
void LLFolderView::doPaste(bool confirm)
{
	...
	if (getVisible() && getEnabled())
	{
			 ...
			 it != end; ++it)
		{
			...
			{
				itemp = itemp->getParentFolder();
			}
			folder_set.insert(itemp);
		}
...
void LLFolderView::startRenamingSelectedItem(bool confirm)
{
	...
	// Lazy renamer creation, so to avoid allocating one for each and every
	// item in the inventory when only a few, if any, would ever be used in the
	// whole session. HB
	if (!mRenamer)
	{
		mRenamer = new LLLineEditor("renamer", getRect(), LLStringUtil::null,
									sFont, DB_INV_ITEM_NAME_STR_LEN,
									commitRename, NULL, NULL, this,
									LLLineEditor::prevalidatePrintableNotPipe);
		// Escape is handled by reverting the rename, not commiting it (default
		// behaviour)
		mRenamer->setCommitOnFocusLost(true);
		// Since the renamer gets closed (and the rename action cancelled) when
		// loosing focus, the context menu is of no use at all (selecting an
		// item in it would steal focus and close the renamer as a result). HB
		mRenamer->disableContextMenu();
		mRenamer->setVisible(false);
		addChild(mRenamer);
	}
	...
	if (count > 0)
	{
		itemp = mSelectedItems.front();
	}
	if (getVisible() && getEnabled() && count == 1 && itemp &&
		itemp->getListener() && itemp->getListener()->isItemRenameable())
	{
		mRenameItem = itemp;

		S32 x = ARROW_SIZE + TEXT_PAD + ICON_WIDTH + ICON_PAD - 1 +
				itemp->getIndentation();
		S32 y = llfloor(itemp->getRect().getHeight() - sFontLineHeight - 2);
		itemp->localPointToScreen(x, y, &x, &y);
		...
		S32 width = llmax(llmin(itemp->getRect().getWidth() - x,
		...
		mRenamer->setText(itemp->getName());
		mRenamer->selectAll();
		mRenamer->setVisible(true);
		// Set focus will fail unless itemp is visible
...
bool LLFolderView::handleKeyHere(KEY key, MASK mask)
{
	...
	switch (key)
	{
		...
		case KEY_RETURN:
			if (mask == MASK_NONE)
			{
				if (mRenameItem && mRenamer && mRenamer->getVisible())
				{
					finishRenamingItem();
					mSearchString.clear();
					handled = true;
		...
		case KEY_DOWN:
			...
			{
				LLFolderViewItem* last_selectedp = getCurSelectedItem();

				if (!mKeyboardSelection)
				{
					setSelection(last_selectedp, false, true);
					mKeyboardSelection = true;
				}

				LLFolderViewItem* nextp = NULL;
				if (mask & MASK_SHIFT)
				{
					// Do not shift select down to children of folders (they
					// are implicitly selected through parent)
					nextp = last_selectedp->getNextOpenNode(false);
					if (nextp)
					{
						if (nextp->isSelected())
						{
							// Shrink selection
							changeSelectionFromRoot(last_selectedp, false);
						}
						else if (last_selectedp->getParentFolder() ==
									nextp->getParentFolder())
						{
							// Grow selection
							changeSelectionFromRoot(nextp, true);
						}
					}
				}
				else
				{
					nextp = last_selectedp->getNextOpenNode();
					if (nextp)
					{
						if (nextp == last_selectedp)
						{
							return false;
						}
						setSelection(nextp, false, true);
					}
				}
				scrollToShowSelection();
				mSearchString.clear();
		...
		case KEY_UP:
			...
			{
				LLFolderViewItem* last_selectedp = mSelectedItems.back();
				...
				LLFolderViewItem* prevp = NULL;
				if (mask & MASK_SHIFT)
				{
					// Do not shift select down to children of folders (they
					// are implicitly selected through parent)
					prevp = last_selectedp->getPreviousOpenNode(false);
					if (prevp)
					{
						if (prevp->isSelected())
						{
							...
							changeSelectionFromRoot(last_selectedp, false);
									...
									prevp->getParentFolder())
						{
							// Grow selection
							changeSelectionFromRoot(prevp, true);
						}
					}
				}
				else
				{
					prevp = last_selectedp->getPreviousOpenNode();
					if (prevp)
					{
						if (prevp == this)
						{
		...
		case KEY_LEFT:
			...
			{
				LLFolderViewItem* last_selectedp = getCurSelectedItem();
				LLFolderViewItem* parentp =
					last_selectedp->getParentFolder();
				if (!last_selectedp->isOpen() &&
					parentp && parentp->getParentFolder())
				{
					setSelection(parentp, false, true);
				}
				else
				{
					last_selectedp->setOpen(false);
				}
				mSearchString.clear();
				scrollToShowSelection();
				handled = true;
...
bool LLFolderView::canDelete() const
{
	if (mSelectedItems.empty())
	{
		return false;
		 ...
		 it != end; ++it)
	{
		LLFolderViewItem* itemp = *it;
		if (!itemp || !itemp->getListener() ||
			!itemp->getListener()->isItemRemovable())
		{
			return false;
		}
	}
...
{
	if (mRenamer && mRenamer == gFocusMgr.getTopCtrl())
	{
		gFocusMgr.setTopCtrl(NULL);
	}
...
// maximally visible.
void LLFolderView::scrollToShowItem(LLFolderViewItem* itemp)
{
	...
	if (itemp && mScrollContainer)
	{
		LLRect local_rect = itemp->getRect();
		...
		itemp->localPointToOtherView(itemp->getIndentation(),
									 llmax(0,
										   local_rect.getHeight() -
										   max_height_to_show),
									 ...
									 mScrollContainer);
		itemp->localPointToOtherView(local_rect.getWidth(),
									 local_rect.getHeight(),
...
//static
void LLFolderView::onRenamerLost(LLUICtrl* renamerp, void*)
{
	if (renamerp)
	{
		renamerp->setVisible(false);
	}
}
...
bool LLInventoryFilter::check(LLFolderViewItem* itemp)
{
	if (!itemp) return false;

	LLFolderViewEventListener* listenerp = itemp->getListener();
	if (!listenerp) return false;

	const LLUUID& item_id = listenerp->getUUID();
	...
	LLInventoryType::EType object_type = listenerp->getInventoryType();
	...
	else
	{
		...
		mSubStringMatchOffset = itemp->getSearchableData().find(search_string);
	...
	if (mFilterSubType != -1 && listenerp->getSubType() != mFilterSubType)
	{
		return false;
	}
	...
	if ((listenerp->getPermissionMask() & mFilterOps.mPermissions) !=
	...
	if (listenerp->getCreationDate() < earliest ||
		listenerp->getCreationDate() > mFilterOps.mMaxDate)
	{
		return false;
	}
	...
	{
		if (LLViewerFolderType::lookupIsHiddenIfEmpty(listenerp->getPreferredType()))
		{
...
void LLSelectFirstFilteredItem::doItem(LLFolderViewItem* itemp)
{
	if (itemp && itemp->getFiltered() && !mItemSelected)
	{
		LLFolderView* rootp = itemp->getRoot();
		if (rootp)
		{
			rootp->setSelection(itemp, false, false);
		}
		LLFolderViewFolder* parentp = itemp->getParentFolder();
		if (parentp)
		{
...
void LLSelectFirstFilteredItem::doFolder(LLFolderViewFolder* folderp)
{
	if (folderp && folderp->getFiltered() && !mItemSelected)
	{
		LLFolderView* rootp = folderp->getRoot();
		if (rootp)
		{
			rootp->setSelection(folderp, false, false);
		}
		LLFolderViewFolder* parentp = folderp->getParentFolder();
		if (parentp)
		{
			parentp->setOpenArrangeRecursively(true,
											   LLFolderViewFolder::RECURSE_UP);
...
//virtual
const std::string& LLInventoryPanel::getTag() const
{
	return LL_INVENTORY_PANEL_TAG;
}
...
{
	LLFolderViewFolder* parentp = NULL;
	...
	else if (objectp)
	{
		...
		parentp = (LLFolderViewFolder*)mFolders->getItemByID(parent_id);
		if (parentp)
		{
			...
			{
				// Build new folder view for category
				LLInvFVBridge* listenerp =
					LLInvFVBridge::createBridge(objectp->getType(),
												objectp->getType(),
												LLInventoryType::IT_CATEGORY,
												this, objectp->getUUID());
				if (listenerp)	// Could be NULL for unknown types
				{
					LLFolderViewFolder* folderp =
						new LLFolderViewFolder(listenerp->getDisplayName(),
											   listenerp->getIcon(),
											   mFolders, listenerp);
					folderp->setItemSortOrder(mFolders->getSortOrder());
					folderp->addToFolder(parentp, mFolders);
				}
			}
			else
			{
				// Build new inventory view for item
				LLInventoryItem* invitemp = (LLInventoryItem*)objectp;
				LLInvFVBridge* listenerp =
					LLInvFVBridge::createBridge(invitemp->getType(),
												invitemp->getActualType(),
												invitemp->getInventoryType(),
												this, invitemp->getUUID(),
												invitemp->getFlags());
				if (listenerp)	// Could be NULL for unknown types
				{
					LLFolderViewItem* itemp =
						new LLFolderViewItem(listenerp->getDisplayName(),
											 listenerp->getIcon(),
											 listenerp->getCreationDate(),
											 mFolders, listenerp);
					if (itemp && mDoubleClickDisabled)
					{
						itemp->disableDoubleClick();
					}
					itemp->addToFolder(parentp, mFolders);
				}
			}
		}
	}
