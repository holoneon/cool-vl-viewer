												...
												U32 fetch_type)
{
	...
	{
		// This may happen "normally", now that we force-refetch folders with
		// broken links in the cached inventory tree on login. So do not spit
		// out a warning unless login has been fully completed already. HB
		...
		{
			LL_DEBUGS("InventoryFetch") << "Unexpected folder response for: "
										<< cat_id << llendl;
		}
	}
