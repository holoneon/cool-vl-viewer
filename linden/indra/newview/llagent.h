...
//mk
	...
	S32 getRegionMaxTextureSize() const;
	...
	// On the very first login, gender is not chosen until the user clicks
	...
	typedef LLCoreHttpUtil::HttpCoroutineAdapter::completionCallback_t http_cb_t;

	bool requestPostCapability(const char* cap_name, LLSD& data,
							   http_cb_t cbsucc = nullptr,
							   http_cb_t cbfail = nullptr);
...
private:
	...
	void onSimFeaturesReceived(const LLUUID& region_id);
...
public:
	...
	// mappings... yup
	static strings_map_t			sTeleportErrorMessages;
	static strings_map_t			sTeleportProgressMessages;

private:
	boost::signals2::connection		mSimFeaturesReceivedSlot;
