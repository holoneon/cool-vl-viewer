...
LLToolDragAndDrop::LLDragAndDropDictionary::LLDragAndDropDictionary()
{
	...
	addEntry(DAD_GLTF, 			new DragAndDropEntry(&LLToolDragAndDrop::dad3dNULL,	&LLToolDragAndDrop::dad3dNULL,					&LLToolDragAndDrop::dad3dGiveInventory,			&LLToolDragAndDrop::dad3dNULL,						&LLToolDragAndDrop::dad3dNULL));
	addEntry(DAD_GLTF_BIN, 		new DragAndDropEntry(&LLToolDragAndDrop::dad3dNULL,	&LLToolDragAndDrop::dad3dNULL,					&LLToolDragAndDrop::dad3dGiveInventory,			&LLToolDragAndDrop::dad3dNULL,						&LLToolDragAndDrop::dad3dNULL));
#endif
											...
											const LLUUID& src_id)
{
	...
	if (asset_id.notNull())
	{
		pmatp = hit_objp->getPreservedPBROverride(hit_face);
	}

	// Remove the insiprim diffuse texture, if any.
	hit_objp->clearTEInvisiprim(hit_face);

	LLTextureEntry* tep = hit_objp->getTE(hit_face);
	if (tep && pmatp.notNull())
	{
		// Apply material with preserved transforms
		LLGLTFMaterialList::queueApply(hit_objp, hit_face, asset_id,
									   pmatp.get());
		// Update local state
		hit_objp->setRenderMaterialID(hit_face, asset_id, false, true);
											 ...
											 const LLUUID& src_id)
{
	...
	LLPointer<LLGLTFMaterial> pmatp;
	for (S32 te = 0; te < hit_objp->getNumTEs(); ++te)
	{
		pmatp = hit_objp->getPreservedPBROverride(te);
		// Remove the insiprim diffuse texture, if any.
		hit_objp->clearTEInvisiprim(te);
		LLTextureEntry* tep = hit_objp->getTE(te);
		if (tep && pmatp.notNull())
		{
			// Apply material with preserved transforms
			LLGLTFMaterialList::queueApply(hit_objp, te, asset_id,
										   pmatp.get());
			// Update local state
			hit_objp->setRenderMaterialID(te, asset_id);
			tep->setGLTFMaterialOverride(pmatp);
		}
		else
		{
