...
// Static members
LLSpatialGroup::groups_set_t LLSpatialGroup::sNodes;
...
public:
	...
	LLDrawable* check(const OctreeNode* nodep)
	{
		...
		for (U32 i = 0; i < nodep->getChildCount(); ++i)
		{
			...
			if (partp)
			{
				...
				if (bridgep)
				{
					LLDrawable* drawp = bridgep->mDrawable.get();
					if (drawp)
					{
	...
	bool check(LLViewerOctreeEntry* entryp)
	{
		...
		bool not_building = !gPickingProbe &&	// Skip when rendering probes
							(!edit_pick || !LLFloaterTools::isVisible());
...
bool LLSpatialGroup::addObject(LLDrawable* drawablep)
{
	...
	setState((U32)OBJECT_DIRTY | (U32)GEOM_DIRTY);
...
LLSpatialGroup::LLSpatialGroup(OctreeNode* nodep, LLSpatialPartition* partp)
:	LLOcclusionCullingGroup(nodep, partp),
	mObjectBoxSize(1.f),
	mGeometryBytes(0),
	mSurfaceArea(0.f),
	mAvatarp(NULL),
...
//static
void LLSpatialGroup::reRegisterProbes()
{
	for (groups_set_t::iterator it = sNodes.begin(), end = sNodes.end();
		 it != end; ++it)
	{
		LLSpatialGroup* groupp = *it;
		if (!groupp->isDead())
		{
			groupp->mReflectionProbe =
				gPipeline.mReflectionMapManager.registerSpatialGroup(groupp);
		}
	}
}
...
void LLSpatialGroup::updateDistance(LLCamera& camera)
{
	...
	if (!isEmpty())
	{
		...
		mRadius = partp->mRenderByGroup ? mObjectBounds[1].getLength3().getF32()
										: mOctreeNode->getSize().getLength3().getF32();
		mDistance = partp->calcDistance(this, camera);
		mPixelArea = partp->calcPixelArea(this, camera);
	}
}
...
{
	if (hasState((U32)ALPHA_DIRTY | (U32)OBJECT_DIRTY))
	{
...
{
	LLOctreeDirty dirty(sTeleportRequested && !gUsePBRShaders);
...
void renderOctree(LLSpatialGroup* groupp)
{
	...
	if (groupp->mBuilt > 0.f)
	{
		...
		gGL.lineWidth(5.f, true);
		const LLVector4a* bounds = groupp->getObjectBounds();
		gl_draw_box_outline(bounds[0], bounds[1]);
		gGL.lineWidth(1.f, true);
		gGL.flush();
...
{
	if (!drawablep->isVisible() || !drawablep->getVObj())
	{
		return;
	}
	...
	LLVolume::face_list_t* facesp = NULL;
	LLDrawable::face_list_t* draw_facesp = NULL;
	if (volp)
	{
		facesp = &volp->getVolumeFaces();
	}
	else
	{
		draw_facesp = &drawablep->getFaces();
	}
	...
	if (!facesp)
	{
		if (!gUsePBRShaders)
		{
			// No available shader in EE mode to render draw-face normals. HB
			return;
		}
		if (!draw_facesp)
		{
	...
	if (vovolp)
	{
		LLVector3 scale_v3 = vovolp->getScale();
		scale_len = scale_v3.length();
		LLVector4a obj_scale(scale_v3.mV[VX], scale_v3.mV[VY],
							 scale_v3.mV[VZ]);
		obj_scale.normalize3();

		// Create inverse-scale vector for normals
		inv_scale.set(1.f / scale_v3.mV[VX], 1.f / scale_v3.mV[VY],
					  1.f / scale_v3.mV[VZ]);
	...
	LLGLEnable blend(gUsePBRShaders ? 0 : GL_BLEND);
	LLGLDepthTest gl_depth(GL_TRUE, GL_FALSE, GL_LEQUAL, gUsePBRShaders);

	gGL.pushMatrix();
	if (vovolp)
	{
		gGL.multMatrix(vovolp->getRelativeXform().getF32ptr());
	}
	...
	{
		LLVector4a p, v;
		for (size_t i = 0, count = facesp->size(); i < count; ++i)
		{
			const LLVolumeFace& face = (*facesp)[i];
			...
			for (S32 j = 0; j < face.mNumVertices; ++j)
			{
				...
				// Pre-scale normal, so it is left with an inverse-transpose
				// xform after MVP
				v.mul(inv_scale);
				v.normalize3fast();
				v.mul(draw_length);
	...
	else if (draw_facesp && gUsePBRShaders)	// Paranoia
	{
		// *HACK: prepare to restore previous shader as other debug code
		// depends on a simpler shader being present.
		LLGLSLShader* prev_shaderp = LLGLSLShader::sCurBoundShaderPtr;

		constexpr U32 MASK_VN = LLVertexBuffer::TYPE_VERTEX |
								LLVertexBuffer::TYPE_NORMAL;
		for (size_t i = 0, count = draw_facesp->size(); i < count; ++i)
		{
			LLFace* facep = (*draw_facesp)[i];
			if (!facep)	// Paranoia
			{
				continue;
			}
			...
			LLVertexBuffer* bufp = facep->getVertexBuffer();
			if (!bufp)
			{
				continue;
			}
			U32 mask = bufp->getTypeMask();
			if ((mask & MASK_VN) != MASK_VN)
			{
				continue;
			}

			S32 type = (mask & LLVertexBuffer::TYPE_TANGENT) ?
							NORMAL_DEBUG_SHADER_WITH_TANGENTS :
							NORMAL_DEBUG_SHADER_DEFAULT;
			LLGLSLShader* shaderp = &gNormalDebugProgram[type];
			shaderp->bind();
			shaderp->uniform1f(LLShaderMgr::DEBUG_NORMAL_DRAW_LENGTH,
							   draw_length);
			LLRenderPass::applyModelMatrix(&drawp->getRegion()->mRenderMatrix);

			bufp->setBuffer();
			U32 geom_idx = facep->getGeomIndex();
			bufp->drawRange(LLRender::TRIANGLES, geom_idx,
							geom_idx + facep->getGeomCount() - 1,
...
{
	// Used to be called in drawBox(), now replaced with gl_draw_box() which
	// does not call this. HB
	LLVertexBuffer::unbind();
...
void renderLights(LLDrawable* drawablep)
{
	...
	if (drawablep->getNumFaces())
	{
		...
		gl_draw_box_outline(pos, LLVector4a(rad));
		stop_glerror();
	}
}
...
void renderOctreeRaycast(const LLVector4a& start, const LLVector4a& end,
						 const LLVolumeOctree* octreep)
{
	F32 t = 1.f;
	LLRenderOctreeRaycast render(start, end, &t);
	render.traverse(octreep);
}
...
void renderRaycast(LLDrawable* drawablep)
{
		...
		if (volp)
		{
				 ...
				 ++i)
			{
				...
				if (!volp->isUnique())
				{
					...
					renderOctreeRaycast(start, end, face.mOctree);
				}
	...
	if (drawablep->getVObj() == gDebugRaycastObject)
	{
		...
		gGL.diffuseColor4f(1.f, 0.f, 0.f, 0.5f);
		gl_draw_box(LLVector3::zero, LLVector3(0.1f, 0.022f, 0.022f));
		gGL.diffuseColor4f(0.f, 1.f, 0.f, 0.5f);
		gl_draw_box(LLVector3::zero, LLVector3(0.021f, 0.1f, 0.021f));
		gGL.diffuseColor4f(0.f, 0.f, 1.f, 0.5f);
		gl_draw_box(LLVector3::zero, LLVector3(0.02f, 0.02f, 0.1f));
		gGL.popMatrix();
	...
	void visit(const OctreeNode* branch) override
	{
		...
		if (gPipeline.hasRenderDebugMask(LLPipeline::RENDER_DEBUG_BBOXES))
		{
			if (!group->isEmpty())
			{
				...
				gl_draw_box_outline(obj_bounds[0], obj_bounds[1]);
			}
		}
						...
						LLColor4& line_color)
{
	...
	if (decompp)
	{
		...
		else
		{
			...
			gGL.diffuseColor4f(0.f, 1.f, 1.f, 1.f);
			gl_draw_box_outline(LLVector3::zero, size);
		}
	}
	else
	{
...
void renderPhysicsShape(LLVOVolume* volp, bool wireframe)
{
	...
	LLGLEnable polygon(gUsePBRShaders ? 0 : GL_POLYGON_OFFSET_LINE);
	if (!gUsePBRShaders)
	{
		...
		else
		{
			...
			if (!phys_volp->mHullPoints)
			{
				...
				LLConvexDecomposition* decomp =
					LLConvexDecomposition::getHACDInstance();
...
void renderPhysicsShapes(LLSpatialGroup* groupp, bool wireframe)
{
	...
	for (U32 i = 0, count = data_vec.size(); i < count; ++i)
	{
		...
		if (partp)
		{
			...
			if (bridgep && bridgep->mDrawable.notNull())
			{
				gGL.pushMatrix();
		...
		// Terrain physics shape rendering
...
void LLSpatialGroup::drawObjectBox(LLColor4 col)
{
	...
	gl_draw_box(mObjectBounds[0], size);
}
...
:	mVertexBuffer(bufferp),
	...
	mIsDebugged(false),
#endif
	mIsAgentAttachment(false),
	mHasALM(false),
	mHasPBR(false),
	mHasLegacyFallback(false),
	mHasBasecolorFallback(false)
{
	if (gDebugGL)
	{
...
LLSpatialBridge::LLSpatialBridge(LLDrawable* rootp, bool render_by_group,
								 U32 data_mask, LLViewerRegion* regionp)
:	LLDrawable(rootp->getVObj(), true),
	LLSpatialPartition(data_mask, render_by_group, regionp)
{
	llassert(rootp && rootp->getRegion());

	mOcclusionEnabled = false;
	mBridge = this;
	mDrawable = rootp;
	rootp->setSpatialBridge(this);
...
{
	LLSpatialGroup* rootp = (LLSpatialGroup*)mOctree->getListener(0);
	...
	const LLVector4a* root_bounds = rootp->getBounds();
								 ...
								 bool for_select)
{
							...
							camera_in.mFrustumCornerDist)))
	{
		if (!LLPipeline::sImpostorRender && !LLPipeline::sShadowRender &&
			LLPipeline::calcPixelArea(center, size,
									  camera_in) < FORCE_INVISIBLE_AREA)
		{
			return;
		}
...
void LLSpatialBridge::updateDistance(LLCamera& camera_in, bool force_update)
{
	...
	LLViewerObject* vobjb = mDrawable->getVObj().get();
	if (!vobjb || vobjb->isDead())
	{
		return;
	}

	if (vobjb->isAttachment())
	{
		LLDrawable* parentp = mDrawable->getParent();
		if (parentp && parentp->getVObj())
		{
			LLVOAvatar* avp = parentp->getVObj()->asAvatar();
			if (avp && (avp->isImpostor() || avp->isDead()))
			{
				return;
			}
		}
	...
	LLViewerObject::const_child_list_t& child_list = vobjb->getChildren();
	for (LLViewerObject::child_list_t::const_iterator it = child_list.begin(),
													  end = child_list.end();
		 it != end; ++it)
	{
...
{
	if (mDrawable.notNull() && mDrawable->mVObjp && mDrawable->getRegion())
	{
...
void LLSpatialBridge::cleanupReferences()
{
	...
	{
		LLViewerObject* vobjp = mDrawable->getVObj().get();
		if (vobjp)
		{
