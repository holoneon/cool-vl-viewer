...
#include "llagent.h"					// For getRegionMaxTextureSize()
...
:	mFilename(filename),
	mShortName(LLDir::getBaseFileName(filename, true)),
	mLastModified(0),
	mValid(false),
	mLinkStatus(LS_ON),
	mUpdateRetries(LL_LOCAL_UPDATE_RETRIES)
{
	...
	mCodec = LLImageBase::getCodecFromExtension(exension);
	if (mCodec == IMG_CODEC_INVALID)
	{
...
bool LLLocalBitmap::updateSelf(EUpdateType optional_firstupdate)
{
	...
	time_t new_last_modified = LLFile::lastModified(mFilename);
...
bool LLLocalBitmap::decodeBitmap(LLPointer<LLImageRaw> rawimg)
{
	...
	S32 max_texture_size = gAgent.getRegionMaxTextureSize();
	...
	switch (mCodec)
	{
		case IMG_CODEC_J2C:
		{
			...
			{
				rawimg->biasedScaleToPowerOfTwo(max_texture_size);
									 ...
									 LLWearableType::EType type)
{
		 ...
		 ++i)
	{
		...
		for (size_t j = 0, count2 = texture_list.size(); j < count2; ++j)
		{
			LLLocalTextureObject* ltop = texture_list[j];
			if (!ltop || ltop->getID() != old_id)
			{
				continue;
			}
			LLTexLayer* tex_layerp = ltop->getTexLayerByIdx(0);
			if (!tex_layerp)
			{
				continue;
			}
			LLTexLayerSet* tex_layer_setp = tex_layerp->getTexLayerSet();
			if (!tex_layer_setp)
			{
				continue;
			}
			auto baked_texind = tex_layer_setp->getBakedTexIndex();
...
void LLLocalBitmap::updateGLTFMaterials(const LLUUID& old_id,
										const LLUUID& new_id)
{
	for (size_t i = 0, count = mGLTFMaterialWithLocalTextures.size();
		 i < count; )
	{
	...
	// mGLTFMaterialWithLocalTextures, so pre-collect all entries that need to
	// be updated.
	std::vector<std::pair<LLFetchedGLTFMaterial*, LLTextureEntry*> > updates;
		 ...
		 i < count; ++i)
	{
		...
		if (matp)
		{
				 ...
				 it != end; ++it)
			{
				...
				if (tep)	// Paranoia
				{
					updates.emplace_back(matp, tep);
				}
			}
		}
	}

	// Normally a change in applied material Id is supposed to drop overrides
	// thus reset material, but local materials currently reuse their existing
	// asset Id, since their purpose is to preview how material will work in-
	// world, overrides included, so do an override to render update instead.
	for (size_t i = 0, count = updates.size(); i < count; ++i)
	{
		LLFetchedGLTFMaterial* matp = updates[i].first;
		LLTextureEntry* tep = updates[i].second;
		LLGLTFMaterial* omatp = tep->getGLTFMaterialOverride();
		if (!matp || !omatp || !tep->getGLTFRenderMaterial())
		{
			continue;
		}
...
//static
const LLUUID& LLLocalBitmap::getTrackingID(const LLUUID& world_id)
{
	for (list_t::const_iterator it = sBitmapList.begin(),
								end = sBitmapList.end();
		 it != end; ++it)
...
// For use by the llgltf library. Passed via LLGLTF::Asset::setHooks() by
// LLGLTFSceneManager::init(). HB
const std::string& get_local_texture_filename(const LLUUID& tex_id)
{
	const LLUUID& tracking_id = LLLocalBitmap::getTrackingID(tex_id);
	if (tracking_id.isNull())
	{
		return LLStringUtil::null;
	}
	return LLLocalBitmap::getFilename(tracking_id);
}
