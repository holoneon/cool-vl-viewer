...
void no_op()
{
		...
		// new std::bind/coroutine model. This is temporary/transition code.
...
void LLViewerInventoryItem::updateServer(bool is_new) const
{
	...
	if (AISAPI::isAvailable())
	{
		...
		AISAPI::completion_t cr =
			std::bind(&doInventoryCb, (LLPointer<LLInventoryCallback>)NULL,
					  _1);
...
void LLViewerInventoryCategory::updateServer(bool is_new) const
{
	...
	if (AISAPI::isAvailable())
	{
		...
		AISAPI::completion_t cr = std::bind(&doInventoryCb,
								...
								bool move_no_copy_items)
{
		 ...
		 iter != end; ++iter)
	{
		...
		LLPointer<LLInventoryCallback> cb =
			new LLBoostFuncInventoryCallback(std::bind(update_folder_cb,
													   new_cat_id));
		if (is_link)
		{
						 ...
						 LLPointer<LLInventoryCallback> cb)
{
	...
	LLUUID curcat_id = itemp->getParentUUID();
	std::string cur_name = itemp->getName();
	...
	{
		gInventory.changeItemParent(itemp, parent_id, false);
	}
	...
	if (cb.notNull())
	{
		...
		doAfterInterval(std::bind(&LLInventoryCallbackManager::fire,
								  &gInventoryCallbacks,
								  callback_id, item_id),
						CALLBACK_DELAY);
	}
...
bool movable_objects_with_same_parent(const uuid_vec_t& inv_items)
{
	...
	LLViewerInventoryCategory* catp = itemp ? NULL
	...
	if (count == 1)
	{
		return itemp || !catp->isUnique();
	}

	const LLUUID& parent_id = itemp ? itemp->getParentUUID()
									: catp->getParentUUID();
	for (U32 i = 1; i < count; ++i)
	{
		...
		if (itemp)
		{
			if (itemp->getParentUUID() != parent_id)
			{
				return false;
			}
			continue;
		...
		if (!catp || catp->getParentUUID() != parent_id || catp->isUnique())
		{
			return false;
		}
	}
...
{
	if (const LLViewerInventoryItem* linked_itemp = getLinkedItem())
	{
		return linked_itemp->getType();
	}
	if (const LLViewerInventoryCategory* linked_catp = getLinkedCategory())
	{
		return linked_catp->getType();
	}
...
const LLUUID& LLViewerInventoryItem::getAssetUUID() const
{
	if (const LLViewerInventoryItem* linked_itemp = getLinkedItem())
	{
		return linked_itemp->getAssetUUID();
	}
...
const std::string& LLViewerInventoryItem::getName() const
{
	if (const LLViewerInventoryItem* linked_itemp = getLinkedItem())
	{
		return linked_itemp->getName();
	}
	if (const LLViewerInventoryCategory* linked_catp = getLinkedCategory())
	{
		return linked_catp->getName();
	}
...
const LLUUID& LLViewerInventoryItem::getCreatorUUID() const
{
	if (const LLViewerInventoryItem* linked_itemp = getLinkedItem())
	{
		return linked_itemp->getCreatorUUID();
	}
...
const std::string& LLViewerInventoryItem::getDescription() const
{
	if (const LLViewerInventoryItem* linked_itemp = getLinkedItem())
	{
		return linked_itemp->getDescription();
	}
...
const LLSaleInfo& LLViewerInventoryItem::getSaleInfo() const
{
	if (const LLViewerInventoryItem* linked_itemp = getLinkedItem())
	{
		return linked_itemp->getSaleInfo();
	}
...
LLViewerInventoryItem* LLViewerInventoryItem::getLinkedItem() const
{
	if (mType == LLAssetType::AT_LINK)
	{
		LLViewerInventoryItem* linked_itemp = gInventory.getItem(mAssetUUID);
		if (linked_itemp && linked_itemp->getIsLinkType())
		{
