...
public:
	...
	// This is called from llstartup.cpp *after* we determined what local
	// textures bake size to use for the grid we connected to. HB
	void allocateBakeBuffer();
	...
	// Used only by the PBR renderer - SSR only
	void buildHiZBuffer();
	void renderSSRTrace();
	void renderSSRAlpha();
	void renderSSRWater();
...
private:
	...
	bool doAllocateScreenBuffer(U32 res_x, U32 res_y);
	...
	void toneMap(LLRenderTarget* srcp, LLRenderTarget* dstp);
	void gammaCorrect(LLRenderTarget* srcp, LLRenderTarget* dstp);
	void applyFXAA(LLRenderTarget* srcp, LLRenderTarget* dstp);
	bool generateSMAABuffers(LLRenderTarget* srcp);
	void applySMAA(LLRenderTarget* srcp, LLRenderTarget* dstp);
	void applyCAS(LLRenderTarget* srcp, LLRenderTarget* dstp);
	void renderDoF(LLRenderTarget* srcp, LLRenderTarget* dstp);
	...
	// Generates the water exclusion surface mask.
...
public:
	...
	// For local (dynamic) texture bakes rendering.
	LLRenderTarget						mBakeBuffer;

	// For FXAA, SMAA and CAS shaders (for deferred modes only).
	LLRenderTarget						mFXAABuffer;
	LLRenderTarget						mSMAAEdgeBuffer;	// EE mode only
	LLRenderTarget						mSMAABlendBuffer;
	LLRenderTarget						mScratchBuffer;		// EE mode only
	...
	// Per-mip temp targets for SSR Gaussian ping-pong
	std::vector<LLRenderTarget>			mSSRMipTemp;
...
private:
	...
	// to keep a quick-lookup to avatar pools, since there is only one per
	// avatar.
	...
	// Note: also replaces mWaterExclusionPool as used in LL's viewer.
...
public:
	...
	static bool							sRenderAvatarCloth;
...
{
public:
	LL_INLINE LLDisableOcclusionCulling(const bool do_disable = true)
...
{
public:
	LL_INLINE HBPreviewLighting(bool preview_light)
		...
		mSavedMoonUp(gPipeline.mIsMoonUp)
	{
		...
		// Do *not* do this in forward rendering mode: it would over-expose the
		// scene.
		...
		else if (preview_light)
		{
			gPipeline.enableLightsPreview();
		}
		gGL.freezeLightState(true);
		// Must be set *after* calling setupHWLights()
		LLPipeline::sAvatarPreviewRender = true;
	}

	LL_INLINE ~HBPreviewLighting()
	{
