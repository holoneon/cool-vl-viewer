...
void LLFloaterBuyLand::updateParcelInfo()
{
	...
	// Check that we can buy the land
...
void LLFloaterBuyLand::runWebSitePrep(const std::string& password)
{
	...
	bool remove_contrib = childGetValue("remove_contribution").asBoolean();
	...
	if (mSiteMembershipUpgrade)
	{
		...
		if (levels)
		{
			...
			new_level = mSiteMembershipPlanIDs[mUserPlanChoice];
		}
	}
...
void LLFloaterBuyLand::refreshUI()
{
	...
	if (!showing_error)
	{
		...
		else
		{
			...
			childSetTextArg("currency_est", "[LOCAL_AMOUNT]",
							mCurrency.getLocalEstimate());
		}
