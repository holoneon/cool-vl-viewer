...
//static
std::string LLDrawPool::getPoolName(U32 type)
{
...
LLDrawPool* LLDrawPool::createPool(U32 type, LLViewerTexture* tex0)
{
	...
	switch (type)
	{
		...
		case POOL_INVISIBLE:
			...
			if (gUsePBRShaders)
			{
				llwarns << "Created POOL_INVISIBLE in PBR mode !" << llendl;
				llassert(false);
			}
			break;
		...
		case POOL_WATEREXCLUSION:
			...
			if (!gUsePBRShaders)
			{
				llwarns << "Created POOL_WATEREXCLUSION in non-PBR mode !"
						<< llendl;
				llassert(false);
			}
			break;
		...
		case POOL_ALPHA:
			...
			if (gUsePBRShaders)
			{
				llwarns << "Created POOL_ALPHA_PRE_WATER in PBR mode !"
						<< llendl;
				llassert(false);
			}
			break;
		...
		case POOL_ALPHA_PRE_WATER:
			...
			if (!gUsePBRShaders)
			{
				llwarns << "Created POOL_ALPHA_PRE_WATER in EE mode !"
						<< llendl;
				llassert(false);
			}
			break;
		...
		case POOL_ALPHA_POST_WATER:
			...
			if (!gUsePBRShaders)
			{
				llwarns << "Created POOL_ALPHA_POST_WATER in EE mode !"
						<< llendl;
				llassert(false);
			}
			break;
		...
		case POOL_MAT_PBR:
			...
			if (!gUsePBRShaders)
			{
				llwarns << "Created POOL_MAT_PBR in EE mode !" << llendl;
				llassert(false);
			}
			break;
		...
		case POOL_MAT_PBR_ALPHA_MASK:
			...
			if (!gUsePBRShaders)
			{
				llwarns << "Created POOL_MAT_PBR_ALPHA_MASK in EE mode !"
						<< llendl;
				llassert(false);
			}
			break;
...
bool LLFacePool::verify() const
{
	...
	for (size_t i = 0, count = mDrawFace.size(); i < count; ++i)
	{
...
{
	llinfos << "Pool: " << std::hex << intptr_t(this) << std::dec
			<< " - Type: " << getPoolName(getType()) << llendl;
}
