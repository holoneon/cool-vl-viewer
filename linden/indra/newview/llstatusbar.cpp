...
#include "llviewershadermgr.h"		// LLViewerShaderMgr::sCanRenderGLTFScene
#include "llviewerstats.h"
#include "llviewertexture.h"		// LLViewerTexture::sDesiredDiscardBias
...
constexpr S32 STAT_GRAPH_WIDTH = 8;
...
:	LLPanel("status bar", LLRect(), false),	// Not mouse opaque
	...
	mUsingALM(false),
	mUsingPBR(false),
	mLacksLegacyFallback(false),
	mLacksBaseColorFallback(false),
	mUsingGLTFScene(false)
{
	...
	// Note: this is just an approximation so we use the smallest expected
	// "max" bandwidth for UDP, which should be OpenSim's one (when this
	// constructor is called, the viewer is not yet logged to a simulator, so
	// we do not yet know what grid we will connect to). HB
	mAbsoluteMaxBandwidth = gSavedSettings.getU32("ThrottleBandwidthKbpsOS");
	...
	mMaterials = getChild<LLTextBox>("materials");
	mMaterials->setClickedCallback(onClickMaterials, this);
	...
	S32 y = 0;
	LLRect r;
	r.set(x - STAT_GRAPH_WIDTH, y + gMenuBarHeight - 1, x, y + 1);
	...
	r.set(x - STAT_GRAPH_WIDTH, y + gMenuBarHeight - 1, x, y + 1);
	mSGDiscardBias = new LLStatGraph("DiscardBias", r);
	mSGDiscardBias->setFollows(FOLLOWS_BOTTOM | FOLLOWS_RIGHT);
	text = getString("discard_bias_tooltip") + ": ";
	mSGDiscardBias->setLabel(text);
	mSGDiscardBias->setMax(5.f);
	mSGDiscardBias->setThreshold(0, 2.125f);
	mSGDiscardBias->setThreshold(1, 3.125f);
	mSGDiscardBias->setThreshold(2, 4.125f);
	mSGDiscardBias->setPrecision(3);
	mSGDiscardBias->setPerSec(false);
	mSGDiscardBias->setClickedCallback(onClickDiscardGraph);
	addChild(mSGDiscardBias);
	x -= STAT_GRAPH_WIDTH + 2;
	...
	// Tool tips for the rendered materials indicator.
	std::string account = " " + getString("attachments_stats");
	mAllMaterialsOK = getString("all_mats") + account;
	mDiffuseFallback = getString("diffuse") + account;
	mLegacyFallback = getString("fallback") + account;
	mBasecolorFallback = getString("basecolor") + account;
	mMaterialMissing = getString("missing") + account;
	mNoMaterial = getString("diffuse_only") + account;
	mGLTFSceneInUse = getString("gltf_scene") + account;
	mGLTFSceneNoShader = getString("gltf_no_shader") + account;

	// Tool tips for the parcel name.
	mParcelTooltipOK = getString("parcel_tool_tip");
	mParcelTooltipWarn1 = getString("parcel_tool_tip_yellow");
	mParcelTooltipWarn2 = getString("parcel_tool_tip_orange");
	mParcelTooltipWarn3 = getString("parcel_tool_tip_red");
...
void LLStatusBar::refresh()
{
	...
	F32 bias = LLViewerTexture::sDesiredDiscardBias;
	mSGDiscardBias->setValueF32(bias);
	if (showing && gLowMemory && bias >= F32(MAX_DISCARD_LEVEL))
	{
		showing = ((S32)(gFrameTimeSeconds * FLASH_FREQUENCY) & 1) != 0;
	}
	mSGDiscardBias->setVisible(mVisibility && showing);
...
//mk
	...
	if (mAgentRegionFailedEventPolls != last_event_poll_failures)
	{
		...
		if (mAgentRegionFailedEventPolls == 0)
		{
			...
			mTextParcelName->setToolTip(mParcelTooltipOK);
		}
		...
		else if (mAgentRegionFailedEventPolls >= 15)
		{
			...
			mTextParcelName->setToolTip(mParcelTooltipWarn3);
		}
		else if (mAgentRegionFailedEventPolls >= 10)
		{
			mTextParcelName->setColor(LLColor4::orange);
			mTextParcelName->setToolTip(mParcelTooltipWarn2);
		}
		else
		{
			mTextParcelName->setColor(LLColor4::yellow);
			mTextParcelName->setToolTip(mParcelTooltipWarn1);
		}
	}
	...
	r = mMaterials->getRect();
	r.translate(new_right - r.mRight, 0);
	mMaterials->setRect(r);
	new_right -= r.getWidth() + 6;
	if (mUsingGLTFScene)
	{
		mMaterials->setText("glTF");
		if (gUsePBRShaders && LLViewerShaderMgr::sCanRenderGLTFScene)
		{
			mMaterials->setColor(LLColor4::green);
			mMaterials->setToolTip(mAllMaterialsOK);
		}
		else
		{
			mMaterials->setColor(LLColor4::red);
			if (gUsePBRShaders)
			{
				mMaterials->setToolTip(mGLTFSceneNoShader);
			}
			else
			{
				mMaterials->setToolTip(mGLTFSceneInUse);
			}
		}
	}
	else if (mUsingPBR)
	{
		mMaterials->setText("PBR");
		if (gUsePBRShaders)
		{
			mMaterials->setColor(LLColor4::green);
			mMaterials->setToolTip(mAllMaterialsOK);
		}
		else if (mLacksLegacyFallback)
		{
			if (mLacksBaseColorFallback)
			{
				mMaterials->setColor(LLColor4::red);
				mMaterials->setToolTip(mMaterialMissing);
			}
			else
			{
				mMaterials->setColor(LLColor4::orange1);
				mMaterials->setToolTip(mBasecolorFallback);
			}
		}
		else
		{
			mMaterials->setColor(LLColor4::yellow);
			mMaterials->setToolTip(mLegacyFallback);
		}
	}
	else if (mUsingALM)
	{
		mMaterials->setText("ALM");
		...
		{
			mMaterials->setColor(LLColor4::yellow);
			mMaterials->setToolTip(mDiffuseFallback);
		}
	}
	else
	{
		mMaterials->setText("DIF");
		mMaterials->setColor(LLColor4::green);
		mMaterials->setToolTip(mNoMaterial);
	}
	mMaterials->setFontStyle(attachments ? LLFontGL::BOLD : LLFontGL::NORMAL);
...
void LLStatusBar::setVisibleForMouselook(bool visible)
{
	...
	mSGDiscardBias->setVisible(visible);
...
//static
void LLStatusBar::onClickMaterials(void*)
{
	static LLCachedControl<bool> attachments(gSavedSettings,
											 "AccountAttachmentsMaterials");
	gSavedSettings.setBool("AccountAttachmentsMaterials", !attachments);
	if (attachments)
	{
		gNotifications.add("RenderedAttachmentsAccounted");
	}
	else
	{
		gNotifications.add("RenderedAttachmentsIgnored");
	}
}
...
//static
void LLStatusBar::onClickDiscardGraph(void*)
{
	if (gTextureViewp)
	{
		gTextureViewp->setVisible(!gTextureViewp->getVisible());
	}
}
