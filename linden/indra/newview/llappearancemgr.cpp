...
static void wear_library_category(const LLSD& query_map, bool append)
{
	LLUUID cat_id;
	if (query_map.has("folder_id"))
	{
		cat_id = query_map["folder_id"].asUUID();
		if (!gInventory.getCategory(cat_id))
		{
			llwarns << "Cannot find library category Id: " << cat_id << llendl;
			cat_id.setNull();
		}
	}
	if (cat_id.isNull() && query_map.has("folder_name"))
	{
		std::string folder_name = query_map["folder_name"].asString();
		...
		if (cat_array.size())
		{
			LLViewerInventoryCategory* catp = cat_array[0];
			if (catp)
			{
				cat_id = catp->getUUID();
			}
		...
		{
			llwarns << "Could not find library category name: " << folder_name
					<< llendl;
		}
	}
	if (cat_id.notNull())
	{
		LLPointer<LLInventoryCategory> new_catp =
			new LLInventoryCategory(cat_id, LLUUID::null,
									LLFolderType::FT_CLOTHING,
									"Quick Appearance");

		if (gInventory.getCategory(cat_id) != NULL)
		{
			gAppearanceMgr.wearInventoryCategory(new_catp, true, append);
		}
	}
}
...
public:
	...
	{
		wear_library_category(query_map, false);
		return true;
	}
};
LLWearFolderHandler gWearFolderHandler;

// SLapp for easy-adding of a stock (library) outfit
class LLAddFolderHandler final : public LLCommandHandler
{
public:
	// Not allowed from outside the app
	LLAddFolderHandler()
	:	LLCommandHandler("add_folder", UNTRUSTED_BLOCK)
	{
	}
...
void LLOutfitObserver::done()
{
	...
	// items into it.
	inventory_func_t func = std::bind(&LLOutfitObserver::createCategoryCB,
									  this, _1);
				...
				if (replace)
				{
					...
					if (old_wearable && old_wearable->isDirty())
					{
						...
						gNotifications.add("WearableSave", LLSD(), payload,
										   std::bind(onSetWearableDialog, _1,
													 _2, old_wearable));
						return false;
					}
				}
		...
		case LLAssetType::AT_BODYPART:
		{
			if (gAgentWearables.areWearablesLoaded())
			{
				...
				if (old_wearable && old_wearable->isDirty())
				{
					...
					payload["item_id"] = item_id_to_wear;
					gNotifications.add("WearableSave", LLSD(), payload,
									   std::bind(onSetWearableDialog, _1, _2,
												 old_wearable));
					return false;
				}
											...
											bool copy, bool append)
{
	...
	if (copy && AISAPI::isAvailable())
	{
        ...
        AISAPI::completion_t cr = std::bind(&doInventoryCb, copy_cb, _1);
...
void LLAppearanceMgr::restoreInitialOutfit(bool from_cof)
{
	static LLCachedControl<U32> restore_min_delay(gSavedSettings,
												  "OutfitRestorationMinDelay");
	static LLCachedControl<U32> restore_max_delay(gSavedSettings,
												  "OutfitRestorationMaxDelay");
	F32 min_delay = (F32)restore_min_delay;
	if (min_delay < 5.f)
	{
		min_delay = 5.f;
	}
	F32 max_delay = (F32)restore_max_delay;
...
//mk
		bool can_retry = gAttachmentsTimer.getElapsedTimeF32() < max_delay;
...
//mk
			...
			// Cancel the "OutfitRestorationDelayed" notification, if any
			...
			std::string msg = status == DONE ? "OutfitRestorationCompleted"
		...
		else
		{
			...
			// Cancel the "OutfitRestorationDelayed" notification, if any and
			// still active.
			if (mLoadingNotificationID.notNull())
			{
				LLNotificationPtr n =
			...
			gNotifications.add(from_cof ? "OutfitCOFRestorationFailed"
										: "OutfitRestorationFailed");
		}
	}
}
...
void LLAppearanceMgr::checkOutfit()
{
		...
		gAgentAvatarp->mPendingAttachment.size() == 0)
	{
		if (mIsRestoringInitialOutfit)
		{
			restoreInitialOutfit(restore_from_cof);
		}
...
void LLAppearanceMgr::requestServerAppearanceUpdate()
{
	...
	LLAgent::http_cb_t succ =
		std::bind(&LLAppearanceMgr::serverAppearanceUpdateSuccess, this, _1);
	LLAgent::http_cb_t fail =
		std::bind(&LLAppearanceMgr::serverAppearanceUpdateFailure, this, _1);
...
void LLAppearanceMgr::serverAppearanceUpdateFailure(const LLSD& http_results)
{
	...
	else
	{
		...
		if (mBakeRetryPolicy->shouldRetry(seconds_to_wait))
		{
			...
			doAfterInterval(std::bind(&LLAppearanceMgr::requestServerAppearanceUpdate,
									  &gAppearanceMgr),
							seconds_to_wait);
			return;
		}
