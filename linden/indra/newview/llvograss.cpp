...
U8 LLVOGrass::sMaxGrassSpecies = 0;
...
void LLVOGrass::updateSpecies()
{
	...
	if (!sSpeciesTable.count(mSpecies))
	{
		...
		mSpecies = it->first;
		llwarns << "Object " << getID() << " got an invalid grass type: "
				<< (U32)mAttachmentState << ". Substituting grass type: "
				<< (U32)mSpecies << llendl;
	}
	setTEImage(0,
	...
	LL_DEBUGS("LindenGrass") << "Object " << getID() << " set to grass type: "
							 << (U32)mSpecies << LL_ENDL;
}

void LLVOGrass::initClass()
{
	std::string xml_filename = gDirUtil.getFullPath(LL_PATH_APP_SETTINGS,
													"grass.xml");
	...
	std::string name;
	for (LLXmlTreeNode* nodep = rootp->getFirstChild(); nodep;
		 nodep = rootp->getNextChild())
	{
		if (!nodep->hasName("grass"))
		{
			llwarns << "Invalid grass definition node " << nodep->getName()
					<< llendl;
			continue;
		}
		...
		S32 species;
		if (!nodep->getFastAttributeS32(species_id_string, species))
		{
		...
		{
			llwarns << "Grass species " << species
					<< " already defined !  Duplicate discarded." << llendl;
			continue;
		}

		GrassSpeciesData* grassp = new GrassSpeciesData();

		success = nodep->getFastAttributeUUID(texture_id_string,
											  grassp->mTextureID);
		success &= nodep->getFastAttributeF32(blade_sizex_string,
											  grassp->mBladeSizeX);
		success &= nodep->getFastAttributeF32(blade_sizey_string,
											  grassp->mBladeSizeY);
		sSpeciesTable[species] = grassp;

		if (species >= (S32)sMaxGrassSpecies)
		{
			sMaxGrassSpecies = species + 1;
		}

		success &= nodep->getFastAttributeString(name_string, name);
	...
	std::string err;
	for (U8 i = 0; i < sMaxGrassSpecies; ++i)
	{
...
{
	LLViewerTexture* texp = getTEImage(0);
	if (!texp)
	{
		return;
	}
	texp->addTextureStats(mPixelArea);
...
bool LLVOGrass::updateLOD()
{
	...
	S32 num_blades = llclamp(S32(tan_angle * 5.f), 1, GRASS_MAX_BLADES);
	if (num_blades >= (mNumBlades << 1))
	{
		...
		facep->setSize(mNumBlades * 8, mNumBlades * 12);
		gPipeline.markRebuild(mDrawable);
	}
...
void LLVOGrass::plantBlades()
{
	...
	facep->setState(LLFace::GLOBAL);
	facep->setSize(mNumBlades * 8, mNumBlades * 12);
	facep->setVertexBuffer(NULL);
	facep->setTEOffset(0);
	facep->mCenterLocal = mPosition + mRegionp->getOriginAgent();
	const LLVector4a* exts = mDrawable->getSpatialExtents();
	if (exts)
	{
		facep->mExtents[0] = exts[0];
		facep->mExtents[1] = exts[1];
	}

	mDepth = (facep->mCenterLocal - gViewerCamera.getOrigin()) *
			  gViewerCamera.getAtAxis();
#if 0	// LLDrawable::setPosition() is a no-operation in LL's code, so this
		// line is useless (and I removed LLDrawable::setPosition() too).
		// Since we are dealing with mCenterLocal, maybe we should instead
		// call LLViewerObject::setPositionLocal() on mVObjp ?... HB
	mDrawable->setPosition(facep->mCenterLocal);
#endif
...
:	LLSpatialPartition((U32)LLDrawPoolAlpha::VERTEX_DATA_MASK |
					   (U32)LLVertexBuffer::MAP_TEXTURE_INDEX,
					   true, regionp)
{
	...
	for (U32 i = 0, count = mFaceList.size(); i < count; ++i)
	{
		...
		if (idx >= 0 &&
			...
			(U32)(draw_vec[idx]->mEnd - draw_vec[idx]->mStart + geomcount) <=
				(U32)LLGLManager::sGLMaxVertexRange &&
#if 0
			draw_vec[idx]->mCount + indicescount <=
				(U32)LLGLManager::sGLMaxIndexRange &&
#endif
