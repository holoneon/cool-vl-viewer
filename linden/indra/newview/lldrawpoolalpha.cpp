...
// EE renderer variant
static void prepare_ee_alpha_shader(LLGLSLShader* shaderp, bool texture_gamma,
									bool deferred)
{
	if (deferred)
	{
	...
	{
		prepare_ee_alpha_shader(shaderp->mRiggedVariant, texture_gamma,
								deferred);
	}
}

// PBR renderer variant
static void prepare_alpha_shader(LLGLSLShader* shaderp, bool deferred,
								 F32 water_sign)
{
	...
	if (shaderp->mRiggedVariant && shaderp->mRiggedVariant != shaderp)
	{
		prepare_alpha_shader(shaderp->mRiggedVariant, deferred, water_sign);
	}
}
...
void LLDrawPoolAlpha::renderPostDeferred(S32 pass)
{
	...
	prepare_ee_alpha_shader(mEmissiveShader, true, false);
	prepare_ee_alpha_shader(mFullbrightShader, true, false);
	// Prime simple shader (loads shadow relevant uniforms)
	prepare_ee_alpha_shader(mSimpleShader, false, true);
	...
	{
		prepare_ee_alpha_shader(&shader_array[i], false, false);
	}
...
void LLDrawPoolAlpha::renderPostDeferredPBR(S32 pass)
{
	...
	prepare_alpha_shader(mEmissiveShader, false, water_sign);

	mPBREmissiveShader = &gPBRGlowProgram;
	prepare_alpha_shader(mPBREmissiveShader, false, water_sign);
	...
	prepare_alpha_shader(mFullbrightShader, true, water_sign);
	prepare_alpha_shader(mSimpleShader, true, water_sign);

	LLGLSLShader* mat_shaderp = gDeferredMaterialProgram;
	for (U32 i = 0; i < LLMaterial::SHADER_COUNT * 2; ++i)
	{
		prepare_alpha_shader(&mat_shaderp[i], true, water_sign);
	}

    prepare_alpha_shader(mPBRShader, true, water_sign);
...
void LLDrawPoolAlpha::render(S32 pass)
{
	if (gUsePBRShaders)
	{
		llwarns_once << "Called in PBR mode !" << llendl;
		llassert(false);
		return;
	}
...
void LLDrawPoolAlpha::forwardRender(bool rigged)
{
	...
	if (rigged && mType == LLDrawPool::POOL_ALPHA_POST_WATER)
	{
		...
		LLGLTFSceneManager::render(false);
		LLGLTFSceneManager::render(false, true);
		LLGLTFSceneManager::render(false, false, true);
		LLGLTFSceneManager::render(false, true, true);
	}
							   ...
							   LLTexUnit* unitp)
{
	...
	if (gUsePBRShaders && drawp->mGLTFMaterial.notNull())
	{
...
void LLDrawPoolAlpha::renderAlpha(U32 mask, bool depth_only, bool rigged)
{
	...
	for (U32 i = 0, acount = alpha_groups.size(); i < acount; ++i)
	{
		...
		for (U32 j = 0, dcount = draw_info.size(); j < dcount; ++j)
		{
			...
			if (gltf_alpha_blend)
			{
				mTargetShader = rigged ? mPBRShader->mRiggedVariant
									   : mPBRShader;
			...
			else
			{
				...
				if (gUsePBRShaders)
				{
					...
					mTargetShader->uniform1f(LLShaderMgr::ALPHA_GAMMA, agamma);
				}
			}
