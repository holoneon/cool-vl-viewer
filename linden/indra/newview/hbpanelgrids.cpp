...
void HBPanelGridsImpl::getParams()
{
	...
	gCoros.launch("HBPanelGridsImpl::getParamsCoro",
			   	  std::bind(&HBPanelGridsImpl::getParamsCoro,
							mEnteredLoginURI));
}
...
void HBPanelGridsImpl::copyParams()
{
	...
	if (i != -1)
	{
		mGridCustomName = mGridName = mGridNick =
			sGridsList["grids"][i].get("label").asString();
...
void HBPanelGridsImpl::deleteGrid()
{
	...
	else
	{
		...
		if (j != -1)
		{
			...
			sGridsList["grids"][j]["login_uri"] =
				mLoginURI = mEnteredLoginURI =
					grids["grids"][i].get("login_uri").asString();
