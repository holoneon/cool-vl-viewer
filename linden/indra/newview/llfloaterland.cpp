...
void LLPanelLandGeneral::refresh()
{
	...
	LLParcel* parcelp = mParcel ? mParcel->getParcel() : NULL;
	...
	bool is_leased = parcelp->getOwnershipStatus() == LLParcel::OS_LEASED;
	...
	bool estate_manager_sellable =
		!parcelp->getAuctionID() && gAgent.canManageEstate() && regionp &&
		// Estate manager/owner can only sell parcels owned by estate owner
		parcelp->getOwnerID() == regionp->getOwner();
	bool owner_sellable =
		region_xfer && !parcelp->getAuctionID() &&
	...
	bool is_public = parcelp->isPublic();
	...
	else
	{
		...
		bool got_group = parcelp->getGroupID().notNull();
		mTextGroup->setEnabled(got_group);
		if (!got_group)
		{
			mTextGroup->setText(getString("none_text"));
		}
		if (got_group && parcelp->getIsGroupOwned())
		{
			// Group owned, so "Info"
		...
		std::string datestr = formatted_time(parcelp->getClaimDate());
	...
	bool can_edit_agent_only =
		LLViewerParcelMgr::isParcelModifiableByAgent(parcelp, GP_NO_POWERS);
	mBtnSetGroup->setEnabled(can_edit_agent_only &&
							 !parcelp->getIsGroupOwned());
	...
	mCheckDeedToGroup->set(parcelp->getAllowDeedToGroup());
	mCheckContributeWithDeed->setEnabled(enable_deed &&
										 parcelp->getAllowDeedToGroup());
	mCheckContributeWithDeed->set(parcelp->getContributeWithDeed());
	...
	mBtnDeedToGroup->setEnabled(parcelp->getAllowDeedToGroup() &&
								group_id.notNull() && can_deed &&
								!parcelp->getIsGroupOwned());

	mEditName->setText(parcelp->getName());
	mEditDesc->setText(parcelp->getDesc());

	bool for_sale = parcelp->getForSale();
	...
	if (for_sale)
	{
		...
		if (area > 0)
		{
			cost_per_sqm = (F32)parcelp->getSalePrice() / (F32)area;
		}

		mSaleInfoForSale1->setTextArg("[PRICE]",
									  llformat("%d", parcelp->getSalePrice()));
	...
	mBtnBuyLand->setEnabled(gViewerParcelMgr.canAgentBuyParcel(parcelp,
															   false));
	mBtnBuyGroupLand->setEnabled(gViewerParcelMgr.canAgentBuyParcel(parcelp,
																	true));
	...
	else
	{
		bool is_owner_release =
			LLViewerParcelMgr::isParcelOwnedByAgent(parcelp, GP_LAND_RELEASE);
		bool is_manager_release = gAgent.canManageEstate() && regionp &&
								  parcelp->getOwnerID() != regionp->getOwner();
		mBtnReleaseLand->setEnabled(is_owner_release || is_manager_release);
	}

	bool use_pass = parcelp->getOwnerID() != gAgentID &&
					parcelp->getParcelFlag(PF_USE_PASS_LIST) &&
...
void LLPanelLandGeneral::refreshNames()
{
	...
	else if (gCacheNamep)
	{
		...
		gCacheNamep->getFullName(parcelp->getOwnerID(), owner);
	}

	if (parcelp->getOwnershipStatus() == LLParcel::OS_LEASE_PENDING)
	{
	...
	std::string group;
	if (parcelp->getGroupID().notNull() && gCacheNamep)
	{
		gCacheNamep->getGroupName(parcelp->getGroupID(), group);
	}
	mTextGroup->setText(group);

	const LLUUID& auth_buyer_id = parcelp->getAuthorizedBuyerID();
...
void LLPanelLandGeneral::onClickProfile(void* data)
{
	...
	{
		const LLUUID& avatar_id = parcelp->getOwnerID();
...
void LLPanelLandGeneral::setGroup(const LLUUID& group_id)
{
	...
	parcelp->setGroupID(group_id);
#if 0
	parcelp->setGroupName(group_name);
	mTextGroup->setText(group_name);
#endif

	// Send update
	gViewerParcelMgr.sendParcelPropertiesUpdate(parcelp);
...
bool LLPanelLandGeneral::enableDeedToGroup(void* data)
{
	...
	LLParcel* parcelp = self->mParcel->getParcel();
	return parcelp && parcelp->getParcelFlag(PF_ALLOW_DEED_TO_GROUP);
}
#endif
...
bool LLPanelLandGeneral::enableBuyPass(void* data)
{
	...
	return parcelp && parcelp->getParcelFlag(PF_USE_PASS_LIST) &&
		   !gViewerParcelMgr.isCollisionBanned();
}
...
void LLPanelLandGeneral::onClickBuyPass(void* data)
{
	...
	S32 pass_price = parcelp->getPassPrice();
	std::string parcel_name = parcelp->getName();
	F32 pass_hours = parcelp->getPassHours();
...
void LLPanelLandGeneral::onCommitAny(LLUICtrl* ctrl, void* userdata)
{
	...
	parcelp->setName(name);
	parcelp->setDesc(desc);

	bool allow_deed_to_group = self->mCheckDeedToGroup->get();
	parcelp->setParcelFlag(PF_ALLOW_DEED_TO_GROUP, allow_deed_to_group);

	bool contribute_with_deed = self->mCheckContributeWithDeed->get();
	parcelp->setContributeWithDeed(contribute_with_deed);
...
void LLPanelLandGeneral::onClickStopSellLand(void* data)
{
	...
	parcelp->setParcelFlag(PF_FOR_SALE, false);
	parcelp->setSalePrice(0);
	parcelp->setAuthorizedBuyerID(LLUUID::null);
...
void LLPanelLandObjects::refresh()
{
	...
	{
		S32 sw_max = parcelp->getSimWideMaxPrimCapacity();
		S32 sw_total = parcelp->getSimWidePrimCount();
		S32 max = ll_roundp(parcelp->getMaxPrimCapacity() *
							parcelp->getParcelPrimBonus());
		S32 total = parcelp->getPrimCount();
		S32 owned = parcelp->getOwnerPrimCount();
		S32 group = parcelp->getGroupPrimCount();
		S32 other = parcelp->getOtherPrimCount();
		S32 selected = parcelp->getSelectedPrimCount();
		F32 parcel_object_bonus = parcelp->getParcelPrimBonus();
		mOtherTime = parcelp->getCleanOtherTime();
													...
													const LLSD& response)
{
	...
	{
		LLUUID owner_id = parcelp->getOwnerID();
		LLSD args;
		...
		send_return_objects_message(parcelp->getLocalID(), RT_OWNER);
	}
													...
													const LLSD& response)
{
		...
		LLNotification::getSelectedOption(notification, response) == 0)
	{
		...
		if (gCacheNamep)
		{
			gCacheNamep->getGroupName(parcelp->getGroupID(), group_name);
		}
		LLSD args;
		args["GROUPNAME"] = group_name;
		gNotifications.add("GroupObjectsReturned", args);
		send_return_objects_message(parcelp->getLocalID(), RT_GROUP);
	}
													...
													const LLSD& response)
{
		...
		LLNotification::getSelectedOption(notification, response) == 0)
	{
		...
		send_return_objects_message(parcelp->getLocalID(), RT_OTHER);
	}
												 ...
												 const LLSD& response)
{
		...
		LLNotification::getSelectedOption(notification, response) == 0)
	{
		...
		if (selected != mSelectedOwners.end())
		{
			...
			send_return_objects_message(parcelp->getLocalID(), RT_LIST,
										&(mSelectedOwners));
		}
	}
...
void LLPanelLandObjects::onClickReturnOwnerList(void* userdata)
{
	...
	if (self->mSelectedIsGroup)
	{
		gNotifications.add("ReturnObjectsDeededToGroup", args, LLSD(),
						   std::bind(&LLPanelLandObjects::callbackReturnOwnerList,
									 self, _1, _2));
	}
	else
	{
									   ...
									   owners_list_t* list)
{
	...
	send_parcel_select_objects(parcelp->getLocalID(), return_type, list);
}
...
void LLPanelLandObjects::onClickReturnOwnerObjects(void* userdata)
{
	...
	send_parcel_select_objects(parcelp->getLocalID(), RT_OWNER);

	LLSD args;
	args["N"] = llformat("%d", parcelp->getOwnerPrimCount());

	const LLUUID& owner_id = parcelp->getOwnerID();
	if (owner_id == gAgentID)
	{
		gNotifications.add("ReturnObjectsOwnedBySelf", args, LLSD(),
						   std::bind(&LLPanelLandObjects::callbackReturnOwnerObjects,
									 self, _1, _2));
	}
	else
	{
...
void LLPanelLandObjects::onClickReturnGroupObjects(void* userdata)
{
	...
	send_parcel_select_objects(parcelp->getLocalID(), RT_GROUP);
	...
	args["N"] = llformat("%d", parcelp->getGroupPrimCount());

	// Create and show confirmation text box
	gNotifications.add("ReturnObjectsDeededToGroup", args, LLSD(),
					   std::bind(&LLPanelLandObjects::callbackReturnGroupObjects,
								 self, _1, _2));
}
...
void LLPanelLandObjects::onClickReturnOtherObjects(void* userdata)
{
	...
	send_parcel_select_objects(parcelp->getLocalID(), RT_OTHER);

	LLSD args;
	args["N"] = llformat("%d", parcelp->getOtherPrimCount());
	...
	if (parcelp->getIsGroupOwned())
	{
		...
		gNotifications.add("ReturnObjectsNotOwnedByGroup", args, LLSD(),
						   std::bind(&LLPanelLandObjects::callbackReturnOtherObjects,
									 self, _1, _2));
		return;
	}
...
void LLPanelLandObjects::onCommitClean(LLUICtrl* caller, void* user_data)
{
	...
	{
		self->mOtherTime =
			atoi(self->mCleanOtherObjectsTime->getText().c_str());

		parcelp->setCleanOtherTime(self->mOtherTime);
		send_other_clean_time_message(parcelp->getLocalID(), self->mOtherTime);
	}
}
...
void LLPanelLandOptions::refresh()
{
	...
	bool can_change_options =
		LLViewerParcelMgr::isParcelModifiableByAgent(parcelp, GP_LAND_OPTIONS);

	mCreateObjectsCheck->set(parcelp->getAllowModify());
	mCreateObjectsCheck->setEnabled(can_change_options);

	mCreateGrpObjectsCheck->set(parcelp->getAllowGroupModify() ||
								parcelp->getAllowModify());
	// If others edit is enabled, then this is explicitly enabled:
	mCreateGrpObjectsCheck->setEnabled(can_change_options &&
									   !parcelp->getAllowModify());

	mAllObjectEntryCheck->set(parcelp->getAllowAllObjectEntry());
	mAllObjectEntryCheck->setEnabled(can_change_options);

	mGroupObjectEntryCheck->set(parcelp->getAllowGroupObjectEntry() ||
								parcelp->getAllowAllObjectEntry());
	mGroupObjectEntryCheck->setEnabled(can_change_options &&
									   !parcelp->getAllowAllObjectEntry());

	mEditLandCheck->set(parcelp->getAllowTerraform());
	mEditLandCheck->setEnabled(LLViewerParcelMgr::isParcelModifiableByAgent(parcelp,
																			GP_LAND_EDIT));

	mNoDamageCheck->set(!parcelp->getAllowDamage());
	mNoDamageCheck->setEnabled(can_change_options);

	mCanFlyCheck->set(parcelp->getAllowFly());
	mCanFlyCheck->setEnabled(can_change_options);

	mGroupScriptsCheck->set(parcelp->getAllowGroupScripts() ||
							parcelp->getAllowOtherScripts());
	mGroupScriptsCheck->setEnabled(can_change_options &&
								   !parcelp->getAllowOtherScripts());

	mAllScriptsCheck->set(parcelp->getAllowOtherScripts());
	mAllScriptsCheck->setEnabled(can_change_options);

	mPushRestrictionCheck->set(parcelp->getRestrictPushObject());
	if (parcelp->getRegionPushOverride())
	{
	...
	mPrivacyCheck->set(parcelp->getSeeAVs() ||
					   !parcelp->getHaveNewParcelLimitData());
	...
	mTeleportRoutingCombo->setCurrentByIndex((S32)parcelp->getLandingType());
	mTeleportRoutingCombo->setEnabled(can_change_landing);

	bool can_change_identity =
		LLViewerParcelMgr::isParcelModifiableByAgent(parcelp,
													 GP_LAND_CHANGE_IDENTITY);
	mSnapshotCtrl->setImageAssetID(parcelp->getSnapshotID());
	mSnapshotCtrl->setEnabled(can_change_identity);

	LLVector3 pos = parcelp->getUserLocation();
	...
	if (regionp)
	{
		...
		else if (access == SIM_ACCESS_MATURE)
		{
			...
			mMatureCheck->set(parcelp->getMaturePublish());
		}
...
void LLPanelLandOptions::refreshSearch()
{
	...
	bool show_directory = parcelp->getParcelFlag(PF_SHOW_DIRECTORY);
	mShowDirectoryCheck->set(show_directory);

	// Set by string in case the order in UI doesn't match the order by index.
	// *TODO:Translate
	LLParcel::ECategory cat = parcelp->getCategory();
	...
	bool large_enough = parcelp->getArea() > MIN_PARCEL_AREA_FOR_SEARCH;
	if (large_enough)
	{
		if (can_change)
		{
...
void LLPanelLandOptions::onCommitAny(LLUICtrl* ctrl, void* userdata)
{
	...
	parcelp->setParcelFlag(PF_CREATE_OBJECTS, create_objects);
	parcelp->setParcelFlag(PF_CREATE_GROUP_OBJECTS, create_group_objects);
	parcelp->setParcelFlag(PF_ALLOW_ALL_OBJECT_ENTRY, all_object_entry);
	parcelp->setParcelFlag(PF_ALLOW_GROUP_OBJECT_ENTRY, group_object_entry);
	parcelp->setParcelFlag(PF_ALLOW_TERRAFORM, allow_terraform);
	parcelp->setParcelFlag(PF_ALLOW_DAMAGE, allow_damage);
	parcelp->setParcelFlag(PF_ALLOW_FLY, allow_fly);
	// Cannot restrict landmark creation:
	parcelp->setParcelFlag(PF_ALLOW_LANDMARK, true);
	parcelp->setParcelFlag(PF_ALLOW_GROUP_SCRIPTS, allow_group_scripts);
	parcelp->setParcelFlag(PF_ALLOW_OTHER_SCRIPTS, allow_other_scripts);
	parcelp->setParcelFlag(PF_SHOW_DIRECTORY, show_directory);
	parcelp->setParcelFlag(PF_ALLOW_PUBLISH, allow_publish);
	parcelp->setParcelFlag(PF_MATURE_PUBLISH, mature_publish);
	parcelp->setParcelFlag(PF_RESTRICT_PUSHOBJECT, push_restriction);
	parcelp->setCategory((LLParcel::ECategory)category_index);
	parcelp->setLandingType((LLParcel::ELandingType)landing_type_index);
	parcelp->setSnapshotID(snapshot_id);
	parcelp->setSeeAVs(see_avs);

	// Send current parcelp data upstream to server
	gViewerParcelMgr.sendParcelPropertiesUpdate(parcelp);
...
void LLPanelLandOptions::onClickPublishHelp(void*)
{
	LLViewerRegion* regionp = gViewerParcelMgr.getSelectionRegion();
	LLParcel* parcelp =
		gViewerParcelMgr.getFloatingParcelSelection()->getParcel();
	if (regionp && parcelp &&
		LLViewerParcelMgr::isParcelModifiableByAgent(parcelp,
													 GP_LAND_CHANGE_IDENTITY) &&
		!regionp->getRegionFlag(REGION_FLAGS_BLOCK_PARCEL_SEARCH))
	{
...
void LLPanelLandAccess::refresh()
{
	...
	bool use_access_list = parcelp->getParcelFlag(PF_USE_ACCESS_LIST);
	bool use_group = parcelp->getParcelFlag(PF_USE_ACCESS_GROUP);
	bool public_access = !use_access_list;

	// Estate owner may have disabled allowing the parcel owner from managing
	// access
	if (parcelp->getRegionAllowAccessOverride())
	{
	...
	S32 count = parcelp->mAccessList.size();
	mListAccess->setToolTipArg("[LISTED]", llformat("%d", count));
	mListAccess->setToolTipArg("[MAX]",
							   llformat("%d", PARCEL_MAX_ACCESS_LIST));
	for (access_map_t::const_iterator cit = parcelp->mAccessList.begin(),
									  end = parcelp->mAccessList.end();
		 cit != end; ++cit)
	{
	...
	count = parcelp->mBanList.size();
	mListBanned->setToolTipArg("[LISTED]", llformat("%d", count));
	mListBanned->setToolTipArg("[MAX]",
							   llformat("%d", PARCEL_MAX_ACCESS_LIST));
	for (access_map_t::const_iterator cit = parcelp->mBanList.begin(),
									  end = parcelp->mBanList.end();
		 cit != end; ++cit)
	{
		const LLAccessEntry& entry = cit->second;
		if (entry.mTime)
	...
	if (parcelp->getRegionDenyAnonymousOverride())
	{
		mCheckLimitPayment->set(true);
	}
	else
	{
		mCheckLimitPayment->set(parcelp->getParcelFlag(PF_DENY_ANONYMOUS));
	}
	if (parcelp->getRegionDenyAgeUnverifiedOverride())
	{
		mCheckLimitAge->set(true);
	}
	else
	{
		mCheckLimitAge->set(parcelp->getParcelFlag(PF_DENY_AGEUNVERIFIED));
	}

	bool use_pass = parcelp->getParcelFlag(PF_USE_PASS_LIST);
	...
	mPriceSpin->setValue((F32)parcelp->getPassPrice());
	mHourSpin->setValue(parcelp->getPassHours());
}
...
void LLPanelLandAccess::refreshUI()
{
	...
	if (parcelp)
	{
		...
		S32 allowed_list_count = parcelp->mAccessList.size();
		...
		S32 banned_list_count = parcelp->mBanList.size();
...
void LLPanelLandAccess::onCommitAny(LLUICtrl* ctrl, void* userdata)
{
	...
	// Push data into current parcel
	parcelp->setParcelFlag(PF_USE_ACCESS_GROUP, use_access_group);
	parcelp->setParcelFlag(PF_USE_ACCESS_LIST, use_access_list);
	parcelp->setParcelFlag(PF_USE_PASS_LIST, use_pass_list);
	parcelp->setParcelFlag(PF_USE_BAN_LIST, true);
	parcelp->setParcelFlag(PF_DENY_ANONYMOUS, limit_payment);
	parcelp->setParcelFlag(PF_DENY_AGEUNVERIFIED, limit_age_verified);

	parcelp->setPassPrice(pass_price);
	parcelp->setPassHours(pass_hours);
...
void LLPanelLandAccess::onClickRemoveAccess(void* userdata)
{
	...
	if (self && self->mParcel)
	{
		...
		if (parcelp)
		{
			LLScrollListCtrl::items_vec_t names =
				self->mListAccess->getAllSelected();
			for (size_t i = 0, count = names.size(); i < count; ++i)
			{
				LLScrollListItem* itemp = names[i];	// Cannot be NULL. HB
				parcelp->removeFromAccessList(itemp->getUUID());
			}
			gViewerParcelMgr.sendParcelAccessListUpdate(AL_ACCESS);
			self->refresh();
		}
											   ...
											   void* userdata)
{
	...
	LLFloaterBanDuration* durationp =
	...
	if (durationp)
	{
		...
		if (parentp)
		{
			parentp->addDependentFloater(durationp);
		}
	}
}
												...
												S32 duration, void* userdata)
{
	...
	for (U32 i = 0, count = ids.size(); i < count; ++i)
	{
		...
		if (parcelp->addToBanList(id, duration))
		{
			...
			if (parcelp->removeFromAccessList(id))
			{
...
void LLPanelLandAccess::onClickRemoveBanned(void* data)
{
	...
	if (self && self->mParcel && self->mListBanned)
	{
		...
		if (parcelp)
		{
			LLScrollListCtrl::items_vec_t items =
				self->mListBanned->getAllSelected();
			for (size_t i = 0, count = items.size(); i < count; ++i)
			{
				LLScrollListItem* itemp = items[i];	// Cannot be NULL. HB
				parcelp->removeFromBanList(itemp->getUUID());
			}
...
bool LLPanelLandExperiences::postBuild()
{
	...
	// No privileged ones
	mBlocked->addFilter(std::bind(LLExperienceCache::FilterWithoutProperties,
								  _1,
									   ...
									   U32 access_type)
{
	...
	panel->setAddedCallback(std::bind(&LLPanelLandExperiences::experienceAdded,
									  this, _1, xp_type, access_type));
	panel->setRemovedCallback(std::bind(&LLPanelLandExperiences::experienceRemoved,
										this, _1, access_type));
}
											 ...
											 U32 access_type)
{
	...
	if (parcelp)
	{
		parcelp->setExperienceKeyType(id, xp_type);
											   ...
											   U32 access_type)
{
	...
	if (parcelp)
	{
		parcelp->setExperienceKeyType(id, EXPERIENCE_KEY_TYPE_NONE);
		gViewerParcelMgr.sendParcelAccessListUpdate(access_type);
		refresh();
	}
}
										  ...
										  U32 xp_type)
{
	...
	access_map_t entries = parcelp->getExperienceKeysByType(xp_type);
