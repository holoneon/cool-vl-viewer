...
{
	const LLRelationship* infop = gAvatarTracker.getBuddyInfo(agent_id);
...
uuid_vec_t LLFloaterFriends::getSelectedIDs()
{
	...
	if (self)
	{
		LLScrollListCtrl::items_vec_t selected =
		...
		if (ids.size() == 1)
		{
			...
			if (command == GRANT)
			{
				gNotifications.add("GrantModifyRights", args, LLSD(),
								   std::bind(&LLFloaterFriends::modifyRightsConfirmation,
							    			this, _1, _2, rights));
			}
			else
			{
...
void LLFloaterFriends::applyRightsToFriends()
{
	...
	LLUUID id;
	LLScrollListCtrl::items_vec_t selected = mFriendsList->getAllSelected();
	for (S32 i = 0, count = selected.size(); i < count; ++i)
	{
		LLScrollListItem* itemp = selected[i];	// Cannot be NULL. HB
		id = itemp->getUUID();
