...
///////////////////////////////////////////////////////////////////////////////
// LLHUDText::LLHUDTextSegment sub-class
///////////////////////////////////////////////////////////////////////////////
...
F32 LLHUDText::LLHUDTextSegment::getWidth(const LLFontGL* fontp)
{
	if (fontp != mLastFont)
	{
		mLastFont = fontp;
									 ...
									 bool debug_render)
{
	...
	LLVector3 render_pos = mPositionAgent +
						   x_pixel_vec * screen_offset.mV[VX] +
						   y_pixel_vec * screen_offset.mV[VY];

	if (mUseBubble)
	{
		LLVector3 bg_pos = render_pos + (F32)mOffsetY * y_pixel_vec -
...
//mk
	...
	// Render label
	{
		...
		for (S32 i = 0, count = mLabelSegments.size(); i < count; ++i)
		{
			...
			hud_render_text(segment.getText(), render_pos, fontp,
							segment.getFontVB(), segment.mStyle, x_offset,
							y_offset, label_color, mOnHUDAttachment);
		}
	}
	...
	// Render text
	{
			 ...
			 ++i)
		{
			...
			hud_render_text(segment.getText(), render_pos, fontp,
							segment.getFontVB(), style, x_offset, y_offset,
							text_color, mOnHUDAttachment);
		}
	}
...
#if 0	// Failed attempt at preventing bleeding through walls: sadly, the
		// culling code does not seem to properly flag culled objects as non-
		// visible drawables... HB
	if (mSourceObject->mDrawable.isNull() ||
		!mSourceObject->mDrawable->isVisible())
	{
		mVisible = false;
		return;
	}
...
//mk
	...
	mOffScreen = false;
	if (!gViewerCamera.sphereInFrustum(render_pos, mRadius))
	{
