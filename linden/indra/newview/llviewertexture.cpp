...
// Static members
LLPointer<LLViewerTexture> LLViewerTexture::sNullImagep;
LLPointer<LLViewerTexture> LLViewerTexture::sCloudImagep;
LLPointer<LLViewerFetchedTexture> LLViewerFetchedTexture::sWhiteImagep;
LLPointer<LLViewerFetchedTexture> LLViewerFetchedTexture::sDefaultImagep;
LLPointer<LLViewerFetchedTexture> LLViewerFetchedTexture::sSmokeImagep;
LLPointer<LLViewerFetchedTexture> LLViewerFetchedTexture::sFlatNormalImagep;
LLPointer<LLViewerFetchedTexture> LLViewerFetchedTexture::sDefaultIrradiancePBRp;
LLPointer<LLViewerFetchedTexture> LLViewerFetchedTexture::sDefaultSunImagep;
LLPointer<LLViewerFetchedTexture> LLViewerFetchedTexture::sDefaultMoonImagep;
LLPointer<LLViewerFetchedTexture> LLViewerFetchedTexture::sDefaultCloudsImagep;
LLPointer<LLViewerFetchedTexture> LLViewerFetchedTexture::sDefaultCloudNoiseImagep;
LLPointer<LLViewerFetchedTexture> LLViewerFetchedTexture::sBloomImagep;
LLPointer<LLViewerFetchedTexture> LLViewerFetchedTexture::sOpaqueWaterImagep;
LLPointer<LLViewerFetchedTexture> LLViewerFetchedTexture::sWaterImagep;
LLPointer<LLViewerFetchedTexture> LLViewerFetchedTexture::sWaterNormapMapImagep;
...
S32 LLViewerTexture::sAdoptedVRAM = 0;
S32 LLViewerTexture::sFreeVRAMMargin = 0;
S32 LLViewerTexture::sBoundTexMemoryMB = 0;
S32 LLViewerTexture::sLastBoundTexMemoryMB = 0;
S32 LLViewerTexture::sGLTexMemoryMB = 0;
S32 LLViewerTexture::sLastGLTexMemoryMB = 0;
S32 LLViewerTexture::sMaxBoundTexMemMB = 0;
S32 LLViewerTexture::sUsableGLTexMemMB = 0;
S32 LLViewerTexture::sLastFreeVRAMMB = -1;
S32 LLViewerTexture::sLastFreePhysMemMB = -1;
bool LLViewerTexture::sVRAMAmountOverriden = false;
bool LLViewerTexture::sLowFreeVRAM = false;
...
// Amount of RAM to reserve for non-texture usage, in MB.
constexpr S32 MIN_MEM_FOR_NON_TEXTURE = 512;
																  ...
																  LLHost request_from_host)
{
	return gTextureList.getImage(image_id, f_type, usemipmaps, boost_priority,
								 texture_type, request_from_host);
}
...
//static
S32 LLViewerTexture::getMinVideoRamSetting()
{
	...
	// min texture mem sets to 256MB if total physical memory is more than
	// 1.5GB, and 128MB otherwise.
	return system_ram > 1500 ? 256 : 128;
}

// Returns max setting for GLTextureMemory (in MB)
//static
S32 LLViewerTexture::getMaxVideoRamSetting()
{
	sVRAMAmountOverriden = false;
	S32 max_texmem;
	S32 adopted_vram_amount = LLGLManager::sVRAM;	// Default to detected VRAM
	S32 vram_manual_override = gSavedSettings.getU32("VRAMOverride");
	if (vram_manual_override >= 128)
	{
		sVRAMAmountOverriden = true;
		adopted_vram_amount = vram_manual_override;	// Overridden by user
		if (vram_manual_override != sAdoptedVRAM)
		{
			sAdoptedVRAM = vram_manual_override;
			llinfos << "Overriding the detected VRAM amount with the VRAMOverride debug settings: "
					<< vram_manual_override << "MB of VRAM assumed." << llendl;
		}
		// 75% of total VRAM at most for textures. HB
		max_texmem = 3 * sAdoptedVRAM / 4;
	}
	else if (LLGLManager::sTexVRAM > 0)
	{
		sAdoptedVRAM = LLGLManager::sTexVRAM;
		if (adopted_vram_amount <= 0)	// This should not happen, but... HB
		{
			adopted_vram_amount = 10 * sAdoptedVRAM / 9; // Approximation...
		}
		// Treat any card with < 128 MB (shudder) as having 128 MB; it is going
		// to be trashing constantly regardless
		max_texmem = llmax(sAdoptedVRAM, getMinVideoRamSetting());
	}
	else if (LLGLManager::sVRAM > 0)
	{
		sAdoptedVRAM = LLGLManager::sVRAM;
		// 75% of total VRAM at most for textures. HB
		max_texmem = 3 * sAdoptedVRAM / 4;
		// Treat any card with < 128 MB (shudder) as having 128 MB; it is going
		// to be trashing constantly regardless.
	...
	{
		sVRAMAmountOverriden = true;
		sAdoptedVRAM = 4 * getMinVideoRamSetting();	// 512MB or 1024MB
		adopted_vram_amount = sAdoptedVRAM;	// Extrapolated
		llwarns << "VRAM amount not detected, defaulting to " << sAdoptedVRAM
				<< " MB" << llendl;
		// 75% of total VRAM at most for textures. HB
		max_texmem = 3 * sAdoptedVRAM / 4;
	}
	// Make sure we have the right numbers. HB
	LLMemory::updateMemoryInfo();
	// Verify that we have enough RAM to map all the textures and buffers that
	// will be allocated in VRAM. If not, reduce the usable VRAM amount. HB
	S32 system_ram = LLMemory::getPhysicalMemoryKB() >> 10;
	if (vram_manual_override < 128 && sAdoptedVRAM > 128 &&
		// We need approximately twice the VRAM amount in RAM to fit the viewer
		// (mapped VRAM data in RAM + viewer program and data). HB
		2 * sAdoptedVRAM > system_ram)
	{
		sVRAMAmountOverriden = true;
		sAdoptedVRAM = system_ram / 2;
		llwarns << "Imbalanced system: not enough RAM to use all the available VRAM. Limiting VRAM usage to: "
				<< sAdoptedVRAM << "MB" << llendl;
	}
	// Set the mimimum free VRAM for textures to a percentage of its initial
	// size (as reported by GL at viewer startup), a minimum further clamped
	// between 16 and 1024 MB. HB
	S32 tex_vram = LLGLManager::sTexVRAM;
	if (tex_vram <= 0)
	{
		tex_vram = 3 * sAdoptedVRAM / 4;	// Very rough estimate. HB
	}
	// Compute the free VRAM margin. HB
	S32 pct = llclamp((S32)gSavedSettings.getU32("VRAMFreeMarginPercent"),
					  5, 25);
	S32 margin = gSavedSettings.getU32("VRAMFreeMargin");
	if (margin < 16 || 4 * margin > adopted_vram_amount)
	{
		sFreeVRAMMargin = adopted_vram_amount * pct / 100;
	}
	else
	{
		sFreeVRAMMargin = margin;
	}
	// If the user configured a lower VRAM amount, we need to adjust the margin
	// accordingly. HB
	if (vram_manual_override >= 128)
	{
		S32 margin2 = llclamp(vram_manual_override * pct / 100, 16, 1024);
		if (sFreeVRAMMargin > margin2)
		{
			sFreeVRAMMargin = margin2;
		}
	}
	// Further clamp the free VRAM margin between 16 and 1024MB. HB
	sFreeVRAMMargin = llclamp(sFreeVRAMMargin, 16, 1024);
	if (margin != sFreeVRAMMargin)
	{
		gSavedSettings.setU32("VRAMFreeMargin", sFreeVRAMMargin);
	}
	llinfos << "Free VRAM margin set to: " << sFreeVRAMMargin << "MB"
			<< llendl;
...
//static
void LLViewerTexture::updateUsableGLTexMem()
{
	S32 cur_mem = gSavedSettings.getU32("GLTextureMemory");
	S32 max_mem = getMaxVideoRamSetting();
	// Clamp to reasonnable values
	S32 mem = cur_mem > 0 ? cur_mem : llmin(max_mem / 2, 3072);
	sUsableGLTexMemMB = llclamp(mem, getMinVideoRamSetting(), max_mem);
	if (sUsableGLTexMemMB != cur_mem)
	{
		gSavedSettings.setU32("GLTextureMemory", mem);
		// At this point the setting listener re-entered this method already.
		return;
	}
	// System memory in MB.
	S32 system_ram = LLMemory::getPhysicalMemoryKB() >> 10;
	// Minimum memory reserved for non-texture use. If system_ram >= 1GB then
	...
	if (sUsableGLTexMemMB > system_ram - min_non_texture_mem)
	{
		sUsableGLTexMemMB = system_ram - min_non_texture_mem;
	}
	llinfos << "Total usable VRAM: " << sAdoptedVRAM << " MB"
			<< " - Maximum GL texture memory set to: " << sUsableGLTexMemMB
			<< " MB" << llendl;
}
											...
											bool& can_decrease_discard)
{
	...
	static bool can_do = LLGLManager::sHasATIMemInfo ||
						 LLGLManager::sHasNVXMemInfo;
	if (!can_do)
	{
		sLastFreeVRAMMB = -1;	// No info
		return false;
	}
	...
	S32 extrapolation = free_vram + delta;

	if (free_vram < sFreeVRAMMargin && extrapolation < sFreeVRAMMargin)
	{
	...
	if (extrapolation > 3 * sFreeVRAMMargin / 2)
	{
	...
	can_decrease_discard = false;
	// If we predict that more than 1.20 times the min memory will be available
	// at next check, or the delta since last check is negative or null , i.e.
	// the VRAM usage decreased or did not increase, then we still leave the
	// discard bias unchanged for now.
	if (delta <= 0 || extrapolation >= 6 * sFreeVRAMMargin / 5)
	{
	...
	F32 factor = (F32)delta / (F32)sFreeVRAMMargin;
	if (factor >= 1.5f)
	{
...
void LLViewerTexture::updateClass()
{
	...
	// IMPORTANT: sGLTexMemoryMB is used to compare with sUsableGLTexMemMB
	// which allows to keep *RAM* usage in line (GL textures are also stored in
	// RAM) on systems with low RAM amount, while sBoundTexMemoryMB is used to
	// compare with sMaxBoundTexMemMB which allows to keep *VRAM* usage in line
	// and avoid spilling GL textures or vertex buffers over to the RAM. The
	// Cool VL Viewer can also optionnally (off by default) check for the free
	// physical RAM as reported by the OS and further adjust the discard bias
	// to keep this free memory above a configurable limit. HB

	// Everything in MB
	sGLTexMemoryMB = BYTES2MEGABYTES(LLImageGL::sGlobalTexMemBytes);
	sBoundTexMemoryMB = BYTES2MEGABYTES(LLImageGL::sBoundTexMemBytes);
	// Problem: not all bound textures are resident in VRAM... In texture-heavy
	// conditions (e.g. in venues with a couple hundreds avatars), and after a
	// while, textures may get pushed to RAM by the driver while not destroyed,
	// causing a lower VRAM consumption than what would be observed with
	// residents textures. Normally, all bound textures should have a GL
	// texture associated with them (excepted for cube maps and any other GL
	// textures that got "manually" created). So the bound texture VRAM
	// consumption should never be much larger than the GL allocated textures.
	// We however use a fudge factor to be safe (and account for cube maps,
	// etc)... *TODO: properly track cube maps and manual GL images consumption
	// in a separate stats. HB
	static LLCachedControl<F32> fudge(gSavedSettings, "GL2BoundTexFactor");
	sBoundTexMemoryMB = llmin(sBoundTexMemoryMB,
							  llmax(1.f, fudge) * (F32)sGLTexMemoryMB);
	// Memory that cannot be used by textures since already allocated for VBs
	// or FBs, or part of the free VRAM margin. HB
	S32 non_tex_mem = LLVertexBuffer::getVRAMMegabytes() + sFreeVRAMMargin +
					  BYTES2MEGABYTES(LLRenderTarget::sBytesAllocated);
	// Note: available VRAM divided by 2 because the bound textures are also
	// (normally) accounted as GL textures and resizing GL textures via the
	// discard bias will also resize the corresponding bound textures. HB
	sMaxBoundTexMemMB = (sAdoptedVRAM - non_tex_mem) / 2;

	if (!gLowMemory)
	{
		sLastFreePhysMemMB = -1;
	}
	...
	if (is_check_time)
	{
		...
		if (sLastGLTexMemoryMB > 0)
		{
			gl_delta = sGLTexMemoryMB - sLastGLTexMemoryMB;
		}
		sLastGLTexMemoryMB = sGLTexMemoryMB;
	}
	...
	// If we are using too much memory, increase discard bias. HB
	else if (is_check_time && gLowMemory)
	{
		can_decrease_discard = false;
		S32 available = LLMemory::getAvailablePhysicalMemKB() >> 10;
		// Only increase discard if memory usage increased, to avoid yo-yo. HB
		if (available <= 0 || sLastFreePhysMemMB > available ||
			sLastFreePhysMemMB < 0)	// First check after a low memory event
		{
			LL_DEBUGS("TextureMemory") << "Increasing discard bias: not enough free memory."
									   << LL_ENDL;
			increased_discard = true;
			sDesiredDiscardBias += DISCARD_BIAS_DELTA;
		}
		sLastFreePhysMemMB = available;
	}
	...
	{
		S32 extrapolated_bound = sBoundTexMemoryMB + bound_delta;
		S32 extrapolated_gl = sGLTexMemoryMB + gl_delta;
		if (bound_delta >= 0 && extrapolated_bound >= sMaxBoundTexMemMB)
		{
			LL_DEBUGS("TextureMemory") << "Increasing discard bias: too much bound GL texture memory used."
									   << LL_ENDL;
			increased_discard = true;
		}
		else if (gl_delta >= 0 && extrapolated_gl >= sUsableGLTexMemMB)
		{
			LL_DEBUGS("TextureMemory") << "Increasing discard bias: too much GL texture memory used."
									   << LL_ENDL;
			increased_discard = true;
		}
		else
		{
			S32 supposed_free = S32_MAX; // If LLGLManager::sVRAM cannot be used
			if (!sVRAMAmountOverriden)
			{
				supposed_free = LLGLManager::sVRAM - non_tex_mem -
								sBoundTexMemoryMB - sGLTexMemoryMB;
			}
			// If we do not have an idea of the actual free VRAM, or if the
			// actual free VRAM is lower than what we computed, check for
			// extrapolated VRAM usage and increase discard when too high. HB
			if (sLastFreeVRAMMB <= supposed_free)
			{
				if (extrapolated_bound + extrapolated_gl + non_tex_mem >=
						sAdoptedVRAM)
				{
					LL_DEBUGS("TextureMemory") << "Increasing discard bias: too much VRAM used."
											   << LL_ENDL;
					increased_discard = true;
				}
			}
		}
		if (increased_discard)
		{
			can_decrease_discard = false;
			sDesiredDiscardBias += DISCARD_BIAS_DELTA;
		}
		...
		bound_delta <= 0 && gl_delta <= 0)
	{
		...
		F32 texmem_lower_bound_scale = llclamp((F32)lower_bound, 0.8f, 0.99f);
		if (sBoundTexMemoryMB < sMaxBoundTexMemMB * texmem_lower_bound_scale &&
			sGLTexMemoryMB < sUsableGLTexMemMB * texmem_lower_bound_scale)
		{
			F32 factor = llmin(2.f, bias_mult);
		 ...
		 sDesiredDiscardBias != initial_discard))
	{
		...
		if (gLowMemory)
		{
			// Expire the memory check timer now to force a heap trimming and
			// memory check at next frame. HB
			gMemoryCheckTimer.setTimerExpirySec(0.f);
		}
	}
}
...
{
	mMaxVirtualSize = 0.f;
	mMaxTextureSize = gMaxImageSizeDefault * gMaxImageSizeDefault;
...
void LLViewerTexture::setBoostLevel(U32 level)
{
	...
	{
		mMaxVirtualSize = mMaxTextureSize;
	}
}
									  ...
									  bool needs_gltexture) const
{
	...
	{
		mMaxVirtualSize = llmin(virtual_size, mMaxTextureSize);
	}
}
...
void LLViewerFetchedTexture::addToCreateTexture()
{
	...
	else
	{
			...
			mRequestedDiscardLevel <= mDesiredDiscardLevel)
		{
			...
			{
				S32 d_level =
					llmin(mRequestedDiscardLevel, mDesiredDiscardLevel) -
					mRawDiscardLevel;
...
bool LLViewerFetchedTexture::preCreateTexture(S32 usename)
{
	...
	mNeedsCreateTexture	= false;
	if (mRawImage.isNull() || mRawImage->isBufferInvalid())
	{
	...
	{
		llwarns << "Tried to create texture " << mID.asString()
				<< " with a negative discard level. Setting texture as missing."
				<< llendl;
		setIsMissingAsset();
		destroyRawImage();
		return false;
	}
	S32 raw_width = mRawImage->getWidth();
	S32 raw_height = mRawImage->getHeight();
	if (mRawDiscardLevel > 0)
	{
		raw_width <<= mRawDiscardLevel;
		raw_height <<= mRawDiscardLevel;
	}
	bool size_okay = true;
	const S32 max_size = gMaxImageSizeDefault * 2;
...
{
	if (!mNeedsCreateTexture || mIsMissingAsset || mWasDeleted ||
		mImageGLp.isNull())
	{
		return false;
	}
...
void LLViewerFetchedTexture::postCreateTexture()
{
	...
	// Rebuild any volume using this texture for sculpts in case their LOD has
	// changed.
	for (U32 i = 0; i < mNumVolumes[LLRender::SCULPT_TEX]; ++i)
	{
		LLVOVolume* volp = mVolumeList[LLRender::SCULPT_TEX][i];
		if (volp && !volp->isDead())
		{
			volp->setSculptLODChanged();
	...
	if (mFullyLoaded)
	{
		if (mDesiredDiscardLevel > mMinDesiredDiscardLevel)
		{
			...
			mDesiredDiscardLevel = llmin(mMinDesiredDiscardLevel,
										 mLoadedCallbackDesiredDiscardLevel);
			mFullyLoaded = false;
		}
	}
	else
		...
		else
		{
			...
			else if (mKnownDrawSizeChanged)	// Known draw size is set
			{
				...
				mDesiredDiscardLevel =
					llmin(mDesiredDiscardLevel, mMinDesiredDiscardLevel,
						  mLoadedCallbackDesiredDiscardLevel);
			}
			mKnownDrawSizeChanged = false;
...
F32 LLViewerFetchedTexture::calcDecodePriority()
{
	...
	// The above number is halved for ALM/PBR secondary channels textures. HB
	if (priority > 0.f)
	{
		...
		// ALM (and PBR) secondary channels textures get their priority halved
		// to favour the diffuse/base color channel loading. HB
		if (mBoostLevel == BOOST_ALM)
		{
			priority *= 0.5f;
		}
	}
...
void LLViewerFetchedTexture::updateVirtualSize()
{
	...
	{
		constexpr F32 tex2k = 2048.f * 2048.f;
		mMaxTextureSize = tex2k;
	}
	else if (level == 0)
	{
		constexpr F32 tex5 = 512.f * 512.f;
		mMaxTextureSize = tex5;
	}
	else
	{
		constexpr F32 tex1k = 1024.f * 1024.f;
		mMaxTextureSize = tex1k;
	}
	mMaxVirtualSize = llmin(mMaxVirtualSize, mMaxTextureSize);
											   ...
											   bool pause)
{
	...
	{
		mLoadedCallbackDesiredDiscardLevel =
			llmin(mLoadedCallbackDesiredDiscardLevel, discard_level);
	}
...
bool LLViewerFetchedTexture::doLoadedCallbacks()
{
	...
	if (mIsRawImageValid)
	{
		...
		current_aux_discard = best_aux_discard;
	}
	else
	{
...
void LLViewerLODTexture::processTextureStats()
{
	...
	F32 min_virtual_size = llmax(10.f, min_vsize_discard);
	if (textures_fullres)
	{
		mDesiredDiscardLevel = 0;
	}
	...
	else
	{
		...
		if (mKnownDrawWidth && mKnownDrawHeight)
		{
			F32 draw_texels = llclamp(mKnownDrawWidth * mKnownDrawHeight,
									  MIN_IMAGE_AREA, mMaxTextureSize);
		...
		else
		{
				...
				mAdditionalDecodePriority < 0.3f)
			{
				...
				mMaxVirtualSize = llmin(mMaxVirtualSize, sMinLargeImageSize);
			}
#endif
#if 0
		...
		mDesiredDiscardLevel = llmin(getMaxDiscardLevel() + 1, discard_level);
			...
			mBoostLevel < BOOST_HIGH)
		{
			...
			else if (sGLTexMemoryMB >
						sUsableGLTexMemMB * texmem_middle_bound_scale
					 && (!getBoundRecently() ||
						 mDesiredDiscardLevel >= mCachedRawDiscardLevel))
			{
...
void LLViewerMediaTexture::updateClass()
{
	...
	F32 max_inactive_time = llmax(5.f, lazy_flush_timeout / 2);
