...
{
	LLControlVariable* ctrlp = gSavedSettings.getControl("CameraAngle");
	if (!ctrlp)
	{
		llwarns << "Missing CameraAngle debug setting." << llendl;
		return;
	}
	ctrlp->getSignal()->connect(std::bind(&LLViewerCamera::updateCameraAngle,
										  _2));
...
{
	gViewerCamera.setDefaultFOV(value.asReal());
}
					 ...
					 window_coordinates))
	{
		...
		else
		{
			...
			if (fabs(line_slope) > rect_slope)
			{
				...
				int_x = S32(((GLdouble)int_y - center_y) / line_slope + center_x);
			}
			else if (fabs(line_slope) < rect_slope)
			{
				...
				int_y = S32(((GLdouble)int_x - center_x) * line_slope + center_y);
			}
			else
			{
