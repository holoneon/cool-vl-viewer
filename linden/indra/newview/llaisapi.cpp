...
#include "lleventcoro.h"			// For llcoro::suspendUntilEventOn()
							 ...
							 completion_t callback)
{
	...
	invokationFn_t postfn =
		std::bind(COROCAST(&LLCoreHttpUtil::HttpCoroutineAdapter::postAndSuspend),
						   // _1 -> adapter
						   // _2 -> url
						   // _3 -> body
						   // _4 -> options
	...
	LLCoprocedureManager::coprocedure_t
		proc(std::bind(&AISAPI::invokeAISCommandCoro, _1, postfn, url,
						...
						completion_t callback)
{
	...
	invokationFn_t putfn =
		std::bind(COROCAST(&LLCoreHttpUtil::HttpCoroutineAdapter::putAndSuspend),
						   // _1 -> adapter
						   // _2 -> url
						   // _3 -> body
						   // _4 -> options
	...
	LLCoprocedureManager::coprocedure_t
		proc(std::bind(&AISAPI::invokeAISCommandCoro, _1, putfn, url,
...
void AISAPI::removeCategory(const LLUUID& cat_id, completion_t callback)
{
	...
	invokationFn_t delfn =
		std::bind(COROCAST2(&LLCoreHttpUtil::HttpCoroutineAdapter::deleteAndSuspend),
						    // _1 -> adapter
						    // _2 -> url
						    // _3 -> body
						    // _4 -> options
	...
	LLCoprocedureManager::coprocedure_t
		proc(std::bind(&AISAPI::invokeAISCommandCoro, _1, delfn, url,
								 ...
								 completion_t callback)
{
	...
	invokationFn_t copyfn =
		std::bind(COROCAST3(&LLCoreHttpUtil::HttpCoroutineAdapter::copyAndSuspend),
						    // _1 -> adapter
						    // _2 -> url
						    // _3 -> body
						    // _4 -> options
	...
	LLCoprocedureManager::coprocedure_t
		proc(std::bind(&AISAPI::invokeAISCommandCoro, _1, copyfn, url,
							...
							completion_t callback)
{
	...
	invokationFn_t patchfn =
		std::bind(COROCAST(&LLCoreHttpUtil::HttpCoroutineAdapter::patchAndSuspend),
						   // _1 -> adapter
						   // _2 -> url
						   // _3 -> body
						   // _4 -> options
	...
	LLCoprocedureManager::coprocedure_t
		proc(std::bind(&AISAPI::invokeAISCommandCoro, _1, patchfn, url,
					   ...
					   completion_t callback)
{
	...
	invokationFn_t getfn =
		std::bind(COROCAST2(&LLCoreHttpUtil::HttpCoroutineAdapter::getAndSuspend),
							// _1 -> adapter
							// _2 -> url
							// _3 -> body
							// _4 -> options
	...
	LLCoprocedureManager::coprocedure_t
		proc(std::bind(&AISAPI::invokeAISCommandCoro, _1, getfn, url,
...
void AISUpdate::checkTimeout()
{
	...
	{
		// If we are taking too long, do not starve other tasks, yield to
		// mainloop. If we use normal suspend(), there will be a chance of
		// waking up from other suspends, before main coro had a chance, so
		// wait for a frame tick instead.
		LLEventPumpOrPumpName pump("mainloop");
		llcoro::suspendUntilEventOn(pump);
		mTimer.setTimerExpirySec(CORO_YIELD_SECONDS);
	}
}
...
void AISUpdate::parseMeta(const LLSD& update)
{
	...
	if (update.has(ucv))
	{
			...
			it != end; ++it)
		{
			...
			mCatVersionsUpdated[cat_id] = it->second.asInteger();
		}
	}
}
...
void AISUpdate::parseLink(const LLSD& link_map, U32 depth)
{
	...
	{
		LLPermissions perms;
		perms.set(gAgentID, gAgentID, LLUUID::null, LLUUID::null, PERM_NONE,
				  PERM_NONE, PERM_NONE, PERM_NONE, PERM_NONE);
									 ...
									 const LLSD& embedded)
{
		...
		embedded.has("items"))
	{
		S32 count = embedded["categories"].size() + embedded["links"].size() +
					embedded["items"].size();
		mCatDescendentsKnown[cat_id] = count;
	}
	...
	{
		mCatDescendentsKnown[cat_id] = embedded["links"].size();
	}
}
