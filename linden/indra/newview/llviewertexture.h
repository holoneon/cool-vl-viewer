...
public:
	...
	void addTextureStats(F32 virtual_size,
						 bool needs_gltexture = true) const override;
	...
	static void updateUsableGLTexMem();
	static S32 getMinVideoRamSetting();
	static S32 getMaxVideoRamSetting();
...
protected:
	...
	// Absolute maximum texture size in pixels: used to limit fetched textures
	// size to 512x512, 1024x1024 or 2048x2048, depending on the user choice
	// ("TextureMaxResLevel" setting) and on this texture usage (determined by
	// the mIsMegaTexture boolean). The default for non-fetched textures is
	// gMaxImageSizeDefault^2. HB
	mutable F32				mMaxTextureSize;
...
public:
	...
	static S32				sAdoptedVRAM;
	static S32				sFreeVRAMMargin;
	static S32				sBoundTexMemoryMB;
	static S32				sLastBoundTexMemoryMB;
	static S32				sGLTexMemoryMB;
	static S32				sLastGLTexMemoryMB;
	static S32				sMaxBoundTexMemMB;
	static S32				sUsableGLTexMemMB;
	static S32				sLastFreeVRAMMB;
	static S32				sLastFreePhysMemMB;
	...
	static bool				sVRAMAmountOverriden;
	static bool				sLowFreeVRAM;
};
...
protected:
	LOG_CLASS(LLViewerFetchedTexture);
...
{
protected:
	LOG_CLASS(LLViewerMediaTexture);
...
public:
	...
	static LLViewerFetchedTexture* staticCast(LLGLTexture* texp,
