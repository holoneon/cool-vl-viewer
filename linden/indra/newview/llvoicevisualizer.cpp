	...
	mMaxGesticulationAmplitude(DEFAULT_MAXIMUM_GESTICULATION_AMPLITUDE)
{
	...
	gSavedSettings.getControl("LipSyncEnabled")->getSignal()->connect(std::bind(&handleVoiceVisualizerPrefsChanged, _2));
	gSavedSettings.getControl("LipSyncOohAahRate")->getSignal()->connect(std::bind(&handleVoiceVisualizerPrefsChanged, _2));
	gSavedSettings.getControl("LipSyncOoh")->getSignal()->connect(std::bind(&handleVoiceVisualizerPrefsChanged, _2));
	gSavedSettings.getControl("LipSyncAah")->getSignal()->connect(std::bind(&handleVoiceVisualizerPrefsChanged, _2));
	gSavedSettings.getControl("LipSyncOohPowerTransfer")->getSignal()->connect(std::bind(&handleVoiceVisualizerPrefsChanged, _2));
	gSavedSettings.getControl("LipSyncAahPowerTransfer")->getSignal()->connect(std::bind(&handleVoiceVisualizerPrefsChanged, _2));
}
