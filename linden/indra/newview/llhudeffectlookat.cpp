...
static bool loadAttentions()
{
	...
	std::string filename = gDirUtil.getFullPath(LL_PATH_CHARACTER,
												"attentions.xml");
	LLXmlTree xml_tree;
...
bool LLHUDEffectLookAt::setLookAt(ELookAtType target_type,
								  LLViewerObject* objectp, LLVector3 pos)
{
	if (!mSourceObject)
	{
		return false;
	...
	if (target_type != mTargetType || objectp != mTargetObject.get() ||
		// or if lookat position has moved a certain amount and we do not have
		// just sent an update
		(dist_vec(pos, mLastSentOffsetGlobal) > MIN_DELTAPOS_FOR_UPDATE &&
		 mTimer.getElapsedTimeF32() - mLastSendTime > 1.f / MAX_SENDS_PER_SEC))
	{
		mLastSentOffsetGlobal = pos;
	...
	else
	{
		...
		if (objectp)
		{
			mTargetOffsetGlobal.set(pos);
		}
		else
		{
			mTargetOffsetGlobal = gAgent.getPosGlobalFromAgent(pos);
		}
...
{
	static const LLFontGL* fontp = LLFontGL::getFontSansSerif();
...
//mk
		...
		if (offset.length() <= (F32)sMaxLookAtBroadcastDist)
		{
			...
			F32 delta_x = -0.5f * fontp->getWidthF32(text.c_str());
			LLGLEnable gl_blend(GL_BLEND);
			gGL.pushMatrix();
			gViewerWindowp->setupViewport();
			hud_render_text(text, target + shadow_offset, fontp, NULL,
							LLFontGL::NORMAL, delta_x + 1.f, -1.f,
							LLColor4::black, false);
			hud_render_text(text, target + offset, fontp, NULL,
