...
S32 LLFloaterImagePreview::getExpectedUploadCost() const
{
	...
	{
		llwarns << "Null raw image pointer !" << llendl;
		return -1;
	}
	return LLEconomy::getInstance()->getTextureUploadCost(mRawImagep->getWidth(),
														  mRawImagep->getHeight());
}
...
bool LLFloaterImagePreview::loadImage(const std::string& src_filename)
{
	...
	if (LLViewerTextureList::createRawImageFromFile(src_filename, imagep))
	{
		imagep->biasedScaleToPowerOfTwo(gAgent.getRegionMaxTextureSize());
		mRawImagep = imagep;
		return true;
	}
	return false;
