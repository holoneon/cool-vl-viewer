...
#include "lldrawpoolwlsky.h"			// For HDRI sky
...
#include "llspatialpartition.h"			// LLSpatialGroup::reRegisterProbes()
...
#include "llviewerassetupload.h"		// For the upload queue
...
// This is the cost of a 2K texture upload for the agent, when 2K textures are
// indeed supported on the grid *and* the cost for such texture is greater than
// for smaller textures. Else this is 0. HB
static S32 s2KTextureCost = 0;
...
static LLMenuParcelObserver* sMenuParcelObserverp = NULL;
...
LLVOAvatar* find_avatar_from_object(LLViewerObject* objectp)
{
	if (objectp)
	{
...
void load_from_xml_callback(HBFileSelector::ELoadFilter,
							std::string& filename, void*)
{
	...
	LLSampleFloater* floaterp = new LLSampleFloater("sample_floater");
	if (LLUICtrlFactory::getInstance()->buildFloater(floaterp, filename))
	{
	...
	delete floaterp;	// do not keep the failed build attempt
	floaterp = new LLSampleFloater("sample_floater");
	LLPanel* panelp = new LLPanel("sample_panel");
	floaterp->mPanelp = panelp;
	if (!LLUICtrlFactory::getInstance()->buildPanel(panelp, filename,
													&panelp->getFactoryMap()))
	{
	...
	panelp->setUseBoundingRect(true);
	panelp->updateBoundingRect();
	LLRect rect = panelp->getBoundingRect();
	rect.setOriginAndSize(0, 0, rect.getWidth() + 64,
						  rect.getHeight() + 64 + LLFLOATER_HEADER_SIZE);
	floaterp->initFloater(filename, false, rect.getWidth(), rect.getHeight(),
						  false, true, true);
	floaterp->setRect(rect);
	floaterp->setTitleVisible(true);
	floaterp->addChild(panelp);
	rect.mTop -= LLFLOATER_HEADER_SIZE;
	panelp->centerWithin(rect);
	panelp->setBorderVisible(true);
	floaterp->center();
	floaterp->open();
}
...
void update_upload_costs_in_menus()
{
	...
	S32 upload_2k_cost = economyp->get2KTextureUploadCost();
	if (gMaxImageSizeDefault > 1024 && upload_cost >= 0 &&
		upload_2k_cost > upload_cost)
	{
		s2KTextureCost = upload_2k_cost;
	}
	else
	{
		s2KTextureCost = 0;
	...
	if (upload_cost >= 0)
	{
		...
		if (s2KTextureCost)
		{
			cost = llformat("%d/%d", upload_cost, upload_2k_cost);
		}
		else
		{
			cost = llformat("%d", upload_cost);
		}
...
void init_menus()
{
	...
	gPieSelfp = factoryp->buildPieMenu("menu_pie_self.xml", gMenuHolderp);
	...
	gPieAvatarp = factoryp->buildPieMenu("menu_pie_avatar.xml", gMenuHolderp);
	gMutesPieMenup = gMenuHolderp->getChild<LLPieMenu>("Mutes", true, false);

	gPieObjectp = factoryp->buildPieMenu("menu_pie_object.xml", gMenuHolderp);
	...
	gPieAttachmentp = factoryp->buildPieMenu("menu_pie_attachment.xml",
											 gMenuHolderp);

	gPieLandp = factoryp->buildPieMenu("menu_pie_land.xml", gMenuHolderp);

	gPieParticlep = factoryp->buildPieMenu("menu_pie_particle.xml",
										   gMenuHolderp);
	new HBLuaPieMenu();
	...
	gMenuBarViewp = (LLMenuBarGL*)factoryp->buildMenu("menu_viewer.xml",
													  gMenuHolderp);
	...
	sMenuParcelObserverp = new LLMenuParcelObserver();

	// Debug menu visiblity
	show_debug_menus();

	gLoginMenuBarViewp = (LLMenuBarGL*)factoryp->buildMenu("menu_login.xml",
														   gMenuHolderp);
...
void handle_rebake_textures(void*)
{
	...
	else
	{
		...
		// This trick will force a rebake even if no wearable or attachment
		// link gets updated.
...
void handle_refresh_attachments(void*)
{
	if (!isAgentAvatarValid())
	{
		return;
	...
	LLVOAvatar::attachments_vec_t& attachments =
			gAgentAvatarp->mAttachedObjectsVector;
	for (S32 i = 0, count = attachments.size(); i < count; ++i)
	{
		LLViewerObject* attach_objp = attachments[i].first;
		if (attach_objp->isTempAttachment() || !attachments[i].second)
		{
			continue;
		}
		const LLUUID& inv_item_id = attach_objp->getAttachmentItemID();
		if (inv_item_id.notNull())
		{
			LLViewerInventoryItem* itemp = gInventory.getItem(inv_item_id);
			if (itemp)
			{
				items.emplace_back(itemp);
			}
		}
	}
	if (!items.empty())
	{
		llinfos << "Re-sending worn inventory objects attach order to the sim"
				<< llendl;
		LLAgentWearables::userAttachMultipleAttachments(items);
	}
}

bool auto_refresh_attachments_enabled(void*)
{
	static LLCachedControl<U32> delay1(gSavedSettings,
									   "VisibilityAutoRefreshBorder");
	static LLCachedControl<U32> delay2(gSavedSettings,
									   "VisibilityAutoRefreshFarTP");
	return gIsInSecondLife && (delay1 > 0 || delay2 > 0);
}

bool auto_refresh_attachments_checked(void*)
{
	static LLCachedControl<bool> enabled(gSavedSettings,
										 "AutoRefreshAttachmentsInSL");
	return gIsInSecondLife && enabled &&
		   auto_refresh_attachments_enabled(NULL);
}
...
bool is_2k_bakes_checked(void*)
{
	static LLCachedControl<bool> enabled(gSavedPerAccountSettings,
										 "Use2KAvatarLocalBakes");
	return enabled;
}

void toggle_2k_bakes(void*)
{
	bool b = gSavedPerAccountSettings.getBool("Use2KAvatarLocalBakes");
	gSavedPerAccountSettings.setBool("Use2KAvatarLocalBakes", !b);
}
...
void edit_automation_script(void*)
{
	...
	// as a static object will allow for self-cleaning on viewer exit. HB
	...
	if (!lua_script.empty())
	{
		lua_script = gDirUtil.getFullPath(LL_PATH_USER_SETTINGS, lua_script);
...
# endif
	sub->appendSeparator();
	sub->append(new LLMenuItemCallGL("Open the Lua console",
									 open_lua_console, NULL, NULL,
									 'L', MASK_CONTROL|MASK_ALT));
	sub->createJumpKeys();
}
...
void init_caches_menu(LLMenuGL* sub)
{
	...
	sub->append(new LLMenuItemCheckGL("Evict objects based on visibility",
									  menu_toggle_control, NULL,
									  menu_check_control,
									  (void*)"UseObjectCacheVisibility"));
...
void restart_voice(void*)
{
	gSavedSettings.setBool("EnableVoiceChat", false);
	doAfterInterval([] { gSavedSettings.setBool("EnableVoiceChat", true); },
					2.f);
}

bool voice_enabled(void*)
{
	static LLCachedControl<bool> voice(gSavedSettings, "EnableVoiceChat");
	return voice;
}

bool can_use_vlc(void*)
{
	static LLCachedControl<bool> use_cef(gSavedSettings, "MediaStreamsUseCEF");
	return !gHasCEF || !use_cef;
}

bool use_vlc_checked(void*)
{
	static LLCachedControl<bool> enabled(gSavedSettings,
										 "MediaStreamsUseVLC");
	return enabled && can_use_vlc(NULL);
}
...
void init_media_menu(LLMenuGL* sub)
{
	...
	if (gHasCEF)
	{
		sub->append(new LLMenuItemCheckGL("Play media streams with CEF",
										  menu_toggle_control, NULL,
										  menu_check_control,
										  (void*)"MediaStreamsUseCEF"));
	}
	if (gHasVLC)
	{
		sub->append(new LLMenuItemCheckGL("Play media streams with VLC",
										  menu_toggle_control, can_use_vlc,
										  use_vlc_checked,
										  (void*)"MediaStreamsUseVLC"));
	}
	sub->appendSeparator();
	...
	sub->append(new LLMenuItemCheckGL("Voice enabled",
									  menu_toggle_control, NULL,
									  menu_check_control,
									  (void*)"EnableVoiceChat"));
	sub->append(new LLMenuItemCallGL("Restart voice", restart_voice,
									 voice_enabled));
...
void handle_reload_settings(void*)
{
	...
	std::string color_file = gDirUtil.getFullPath(LL_PATH_APP_SETTINGS,
												  "colors_base.xml");
...
{
	LLFloater* floaterp = new LLFloater("font test");
...
{
	LLFloater* floaterp = new LLFloater("skin preview");
	LLUICtrlFactory* factoryp = LLUICtrlFactory::getInstance();
	factoryp->buildFloater(floaterp, "floater_skin_preview_template.xml");
}
								  ...
								  std::string& filename, void* user_data)
{
	if (!filename.empty())
	{
		...
		if (out.is_open())
		{
			LLXMLNodePtr nodep = gMenuBarViewp->getXML();
			nodep->writeToOstream(out);
			out.close();
		}
		else
		{
...
void init_debug_ui_menu(LLMenuGL* menu)
{
	...
	menu->appendSeparator();
	menu->append(new LLMenuItemCheckGL("Allow colored fonts in UI",
									   menu_toggle_control, NULL,
									   menu_check_control,
									  (void*)"AllowColoredFonts"));
	menu->appendSeparator();
...
bool reset_vb_after_login_enabled(void*)
{
	static LLCachedControl<U32> login_delay(gSavedSettings,
											"VisibilityAutoRefreshLogin");
	return login_delay > 0;
}

bool reset_vb_after_login_check(void*)
{
	static LLCachedControl<bool> do_reset(gSavedSettings, "ResetVBAfterLogin");
	return do_reset && reset_vb_after_login_enabled(NULL);
}
...
void handle_selected_pbr_info(void*)
{
		 ...
		 iter != end; ++iter)
	{
		...
		if (faces_per_material.empty())
		{
			msg = "No glTF material on primitive: " + nodep->mName;
			emit_chat_text(msg);
			continue;
		}
...
bool tonemap_enabled(void*)
{
	return gUsePBRShaders && LLPipeline::sRenderHDR &&
		   !LLRender::sClassicMode && !gPipeline.mNoProbeAmbiance;
}

void tonemap_toggle(void* userdata)
{
	gSavedSettings.setU32("RenderTonemapType", (U32)(intptr_t)userdata);
}

bool tonemap_check_control(void* userdata)
{
	if (tonemap_enabled(NULL))
	{
		static LLCachedControl<U32> type(gSavedSettings, "RenderTonemapType");
		return (U32)type == (U32)(intptr_t)userdata;
	}
	return !userdata;
}

bool alpha_gamma_enabled(void*)
{
	static LLCachedControl<bool> enabled(gSavedSettings,
										 "RenderLegacyAlphaGammaEnable");
	return gUsePBRShaders && enabled;
}
...
bool ee_shadows_enabled(void*)
{
	static LLCachedControl<U32> shadows(gSavedSettings, "RenderShadowDetail");
	return !gUsePBRShaders && LLPipeline::sRenderDeferred && shadows;
}

bool soften_check_control(void* userdata)
{
	static LLCachedControl<bool> soften(gSavedSettings, "RenderShadowSoften");
	return soften && ee_shadows_enabled(NULL);
}

bool got_reflection_probes(void*)
{
	return gUsePBRShaders && LLViewerShaderMgr::sHasRP;
}

void probes_toggle(void* userdata)
{
	gSavedSettings.setU32("RenderReflectionProbeCount",
						  (U32)(intptr_t)userdata);
}

bool probes_check_control(void* userdata)
{
	if (!got_reflection_probes(NULL))
	{
		return false;
	}
	static LLCachedControl<U32> count(gSavedSettings,
									  "RenderReflectionProbeCount");
	return (U32)count == (U32)(intptr_t)userdata;
}

void auto_alloc_probes_toggle(void*)
{
	static LLCachedControl<S32> count(gSavedSettings,
									  "RenderReflectionProbeDynamicAlloc");
	gSavedSettings.setS32("RenderReflectionProbeDynamicAlloc",
						  count >= 0 ? -1 : 1);
}

bool auto_alloc_probes_check_control(void*)
{
	if (!got_reflection_probes(NULL))
	{
		return false;
...
bool auto_decloud_stuck_enabled(void*)
{
	static LLCachedControl<bool> render_unloaded(gSavedSettings,
												 "RenderUnloadedAvatar");
	static LLCachedControl<bool> decloud(gSavedSettings, "AutoDecloudAvatars");
	static LLCachedControl<U32> max_requests(gSavedSettings,
											 "AutoDecloudMaxRequests");
	return gIsInSecondLife && !render_unloaded && decloud && max_requests > 0;
}

bool auto_decloud_stuck_checked(void*)
{
	static LLCachedControl<bool> render_stuck_avatar(gSavedSettings,
													 "RenderUnloadedStuckAvatar");
	static LLCachedControl<bool> render_unloaded(gSavedSettings,
												 "RenderUnloadedAvatar");
	return !render_unloaded && render_stuck_avatar &&
		    auto_decloud_stuck_enabled(NULL);
}
...
void handle_objects_visibility(void* userdata)
{
	...
	llinfos << "Refreshing objects visibility";
	switch (type)
	{
		case AFTER_LOGIN:
			llcont << " after login";
		...
		case AFTER_CROSS_BORDER:
			llcont << " after sim border crossing";
			break;
		case AFTER_FAR_TP:
			llcont << " after far TP";
			break;
	...
	for (S32 i = 0, count = gObjectList.getNumObjects(); i < count; ++i)
	{
		...
		if (objectp && !objectp->isDead())
		{
			...
			// With PBR, something fishy happens with spatial partitions and
			// some objects find themselves in the wrong one, causing non-
			// rezzing objects that only reappear after a LOD change (the
			// latter triggering a call to markPartitionMove()). So we must
			// also refresh the partition ownership for each object !  HB
			if (gUsePBRShaders && objectp->mDrawable)
			{
				gPipeline.markPartitionMove(objectp->mDrawable);
			}
		}
	}

	static LLCachedControl<bool> refresh_attach(gSavedSettings,
												"AutoRefreshAttachmentsInSL");
	if (gIsInSecondLife && refresh_attach &&
		(type == AFTER_CROSS_BORDER || type == AFTER_FAR_TP))
	{
		handle_refresh_attachments(NULL);
	}

	static LLCachedControl<bool> reset_vb(gSavedSettings, "ResetVBAfterLogin");
	if (type == AFTER_LOGIN && reset_vb)
	{
		// Post-login refresh: also trigger a vertex buffers reset to work
		// around slower fps rates sometimes seen at login (likely an occlusion
		// culling race condition issue). HB
		doAfterInterval(std::bind(reset_vertex_buffers, nullptr), 1.f);
	}
}
...
void handle_refresh_probes(void*)
{
	...
	llinfos << "Refreshing reflection probes" << llendl;

	gPipeline.mReflectionMapManager.reset(true);
	gPipeline.mHeroProbeManager.reset();
	gPipeline.mHeroProbeManager.initReflectionMaps();
}
...
// region or TPed beyond draw distance in the same region. 'type' is 1 for
// refresh on login, 2 for refresh on sim border crossing, and 4 for refresh
// after a TP. HB
void schedule_objects_visibility_refresh(U32 type)
{
	static LLCachedControl<U32> login_delay(gSavedSettings,
											"VisibilityAutoRefreshLogin");
	static LLCachedControl<U32> cross_delay(gSavedSettings,
											"VisibilityAutoRefreshBorder");
	static LLCachedControl<U32> tp_delay(gSavedSettings,
										 "VisibilityAutoRefreshFarTP");
	F32 delay;
	switch (type)
	{
		case AFTER_LOGIN:			delay = login_delay; break;
		case AFTER_CROSS_BORDER:	delay = cross_delay; break;
		case AFTER_FAR_TP:			delay = tp_delay; break;
		default:					delay = 0.f;
	}
	...
	doAfterInterval(std::bind(handle_objects_visibility, (void*)((U64)type)),
...
bool can_use_font_vb(void*)
{
	static LLCachedControl<bool> vbcache(gSavedSettings, "RenderGLUseVBCache");
	return gUsePBRShaders || vbcache;
}

bool font_vb_check_control(void*)
{
	static LLCachedControl<bool> fontvb(gSavedSettings, "UseFontVertexBuffers");
	return fontvb && can_use_font_vb(NULL);
}

void textures_size_toggle(void* userdata)
{
	gSavedSettings.setU32("TextureMaxResLevel", (U32)(intptr_t)userdata);
}

bool textures_size_check_control(void* userdata)
{
	static LLCachedControl<U32> level(gSavedSettings, "TextureMaxResLevel");
	return (U32)level == (U32)(intptr_t)userdata;
}
...
void init_debug_rendering_menu(LLMenuGL* menu)
{
	...
	sub->append(new LLMenuItemCheckGL("Bump",
									  ...
									  '/', MASK_CONTROL|MASK_ALT|MASK_SHIFT));
	sub->append(new LLMenuItemCheckGL("Materials",
									  &LLPipeline::toggleRenderTypeControl, NULL,
									  &LLPipeline::hasRenderTypeControl,
									  (void*)LLPipeline::RENDER_TYPE_MATERIALS));
	sub->append(new LLMenuItemCheckGL("PBR materials",
									  &LLPipeline::toggleRenderTypeControl, NULL,
									  &LLPipeline::hasRenderTypeControl,
									  (void*)LLPipeline::RENDER_TYPE_MAT_PBR));
	sub->append(new LLMenuItemCheckGL("Alpha mask",
									  &LLPipeline::toggleRenderTypeControl, NULL,
									  &LLPipeline::hasRenderTypeControl,
									  (void*)LLPipeline::RENDER_TYPE_ALPHA_MASK));
	sub->append(new LLMenuItemCheckGL("Alpha mask (full bright)",
									  &LLPipeline::toggleRenderTypeControl, NULL,
									  &LLPipeline::hasRenderTypeControl,
									  (void*)LLPipeline::RENDER_TYPE_FULLBRIGHT_ALPHA_MASK));
	sub->append(new LLMenuItemCheckGL("Full bright",
									  &LLPipeline::toggleRenderTypeControl, NULL,
									  &LLPipeline::hasRenderTypeControl,
									  (void*)LLPipeline::RENDER_TYPE_FULLBRIGHT));
	sub->append(new LLMenuItemCheckGL("Glow",
									  &LLPipeline::toggleRenderTypeControl, NULL,
									  &LLPipeline::hasRenderTypeControl,
									  (void*)LLPipeline::RENDER_TYPE_GLOW));
	sub->createJumpKeys();
	menu->appendMenu(sub);
	...
	sub->append(new LLMenuItemCheckGL("glTF preview nodes",
									  LLPipeline::toggleRenderDebug, NULL,
									  LLPipeline::toggleRenderDebugControl,
									  (void*)LLPipeline::RENDER_DEBUG_NODES));
	...
	sub->append(new LLMenuItemCheckGL("No tone mapping", tonemap_toggle,
									  tonemap_enabled, tonemap_check_control,
									  NULL));

	sub->append(new LLMenuItemCheckGL("Neutral tone mapping", tonemap_toggle,
									  tonemap_enabled, tonemap_check_control,
									  (void*)1));

	sub->append(new LLMenuItemCheckGL("ACES tone mapping", tonemap_toggle,
									  tonemap_enabled, tonemap_check_control,
									  (void*)2));

	sub->append(new LLMenuItemCheckGL("Fix legacy alpha-gamma",
									  menu_toggle_control, in_pbr_mode,
									  alpha_gamma_enabled,
									  (void*)"RenderLegacyAlphaGammaEnable"));

	LLMenuGL* subsub = new LLMenuGL("Number of reflection probes");
	subsub->append(new LLMenuItemCheckGL("1 probe", probes_toggle,
										 got_reflection_probes,
										 probes_check_control, (void*)1));
	subsub->append(new LLMenuItemCheckGL("2 probes", probes_toggle,
										 got_reflection_probes,
										 probes_check_control, (void*)2));
	subsub->append(new LLMenuItemCheckGL("4 probes", probes_toggle,
										 got_reflection_probes,
										 probes_check_control, (void*)4));
	subsub->append(new LLMenuItemCheckGL("8 probes", probes_toggle,
										 got_reflection_probes,
										 probes_check_control, (void*)8));
	subsub->append(new LLMenuItemCheckGL("16 probes", probes_toggle,
										 got_reflection_probes,
										 probes_check_control, (void*)16));
	subsub->append(new LLMenuItemCheckGL("32 probes", probes_toggle,
										 got_reflection_probes,
										 probes_check_control, (void*)32));
	subsub->append(new LLMenuItemCheckGL("64 probes", probes_toggle,
										 got_reflection_probes,
										 probes_check_control, (void*)64));
	subsub->append(new LLMenuItemCheckGL("128 probes", probes_toggle,
										 got_reflection_probes,
										 probes_check_control, (void*)128));
	subsub->append(new LLMenuItemCheckGL("256 probes", probes_toggle,
										 got_reflection_probes,
										 probes_check_control, (void*)256));
	subsub->appendSeparator();
	subsub->append(new LLMenuItemCheckGL("Automatic allocation",
										 auto_alloc_probes_toggle,
										 got_reflection_probes,
										 auto_alloc_probes_check_control,
										 NULL));
	subsub->createJumpKeys();
	sub->appendMenu(subsub);
	...
	sub->append(new LLMenuItemCheckGL("Soften shadows", menu_toggle_control,
									  ee_shadows_enabled, soften_check_control,
	...
	sub->append(new LLMenuItemCheckGL("Fetch/render at 512x512 pixels max",
									  textures_size_toggle, NULL,
									  textures_size_check_control,
									  NULL));

	sub->append(new LLMenuItemCheckGL("Fetch/render at 1024x1024 pixels max",
									  textures_size_toggle, NULL,
									  textures_size_check_control,
									  (void*)1));

	sub->append(new LLMenuItemCheckGL("Fetch/render at 2048x2048 pixels max",
									  textures_size_toggle, NULL,
									  textures_size_check_control,
									  (void*)2));
	...
	menu->append(new LLMenuItemCallGL("Refresh reflection probes",
									  handle_refresh_probes, in_pbr_mode,
									  NULL));
	menu->appendSeparator();
	menu->append(new LLMenuItemCheckGL("Render unloaded avatars",
									   menu_toggle_control, NULL,
									   menu_check_control,
									   (void*)"RenderUnloadedAvatar"));
	menu->append(new LLMenuItemCheckGL("Auto-decloud avatars",
									   menu_toggle_control, in_sl,
									   menu_check_control,
									   (void*)"AutoDecloudAvatars"));
	menu->append(new LLMenuItemCheckGL("Render avatars failing auto-decloud",
									   menu_toggle_control,
									   auto_decloud_stuck_enabled,
									   auto_decloud_stuck_checked,
									   (void*)"RenderUnloadedStuckAvatar"));
	menu->appendSeparator();
	...
	menu->append(new LLMenuItemCheckGL("Reset vertex buffers at login",
									   menu_toggle_control,
									   reset_vb_after_login_enabled,
									   reset_vb_after_login_check,
									   (void*)"ResetVBAfterLogin"));
	...
	menu->append(new LLMenuItemCheckGL("Use font vertex buffers",
									   menu_toggle_control, can_use_font_vb,
									   font_vb_check_control,
									   (void*)"UseFontVertexBuffers"));
...
void handle_dump_attachments(void*)
{
	...
	for (S32 i = 0, count = gAgentAvatarp->mAttachedObjectsVector.size();
		 i < count; ++i)
	{
		attachp = gAgentAvatarp->mAttachedObjectsVector[i].second;
		if (!attachp)
		{
		...
		objectp = gAgentAvatarp->mAttachedObjectsVector[i].first;
		if (!objectp)
		{
			llwarns << attachp->getName() << ": NULL object attached"
					<< llendl;
		}
		else if (objectp->mDrawable.isNull())
		{
			llwarns << attachp->getName() << ": "
					<< objectp->getAttachmentItemID() << " got a NULL drawable"
					<< " - object position = " << objectp->getPosition()
					<< llendl;
		}
		else
		{
			llinfos << attachp->getName() << ": "
					<< objectp->getAttachmentItemID()
					<< (objectp->mDrawable->isRenderType(0) ? " - invisible"
														   : " - visible")
					<< " - drawable position = "
					<< objectp->mDrawable->getPosition()
					<< " - object position = " << objectp->getPosition()
					<< llendl;
		}
	}
...
void handle_grab_baked_texture(void* data)
{
	...
	if (isAgentAvatarValid())
	{
		...
		if (folder_id.notNull())
		{
			...
			item_id.generate();
			LLPermissions perm;
			constexpr U32 next_owner_perm = PERM_MOVE | PERM_TRANSFER;
			perm.set(gAgentID, gAgentID, LLUUID::null, LLUUID::null, PERM_ALL,
					 PERM_ALL, PERM_NONE, PERM_NONE, next_owner_perm);
...
void init_debug_character_menu(LLMenuGL* menu)
{
	...
	menu->appendSeparator();
	menu->append(new LLMenuItemCallGL("Refresh attachments",
									  handle_refresh_attachments));
	menu->append(new LLMenuItemCheckGL("Refresh attachments on sim changes",
									   menu_toggle_control,
									   auto_refresh_attachments_enabled,
									   auto_refresh_attachments_checked,
									   (void*)"AutoRefreshAttachmentsInSL"));
	...
	menu->append(new LLMenuItemCheckGL("Local baking/appearance",
									   ...
									   MASK_CONTROL | MASK_ALT| MASK_SHIFT));
	menu->append(new LLMenuItemCheckGL("Use 2K local bakes (after restart)",
									   toggle_2k_bakes, NULL,
									   is_2k_bakes_checked, NULL));
...
void derez_objects(EDeRezDestination dest, const LLUUID& dest_id)
{
	...
	LLViewerRegion* first_regionp = NULL;
		 ...
		 iter != end; ++iter)
	{
		...
		if (!first_regionp)
		{
			first_regionp = regionp;
		}
		else if (regionp != first_regionp)
		{
		...
		if (objectp->getNVPair("AssetContainer") &&
		...
		switch (dest)
		{
			...
			case DRD_TRASH:
				if (!objectp->isPermanentEnforced() &&
					((nodep->mPermissions->allowTransferTo(gAgentID) &&
					  objectp->permModify()) ||
					 nodep->allowOperationOnNode(PERM_OWNER,
		...
		{
			objects_list.push_back(objectp);
		}
	}
	...
	if (error.empty() && objects_list.size() > 0)
	{
		...
		for (U8 packet_number = 0; packet_number < packet_count; ++packet_number)
		{
			...
			msg->sendReliable(first_regionp->getHost());
		}
...
void handle_claim_public_land(void*)
{
	...
	LLMessageSystem* msg = gMessageSystemp;
	msg->newMessage(_PREHASH_GodlikeMessage);
	...
	msg->addString(_PREHASH_Method, "claimpublicland");
...
//mk
	...
	menu->append(new LLMenuItemCheckGL("Memory usage safety checks",
									   &menu_toggle_control, NULL,
									   &menu_check_control,
									   (void*)"MinFreePhysMemCheck"));
	menu->appendSeparator();
	...
	menu->append(new LLMenuItemCallGL("Paths for anti-virus whitelisting...",
									  show_viewer_paths_floater, NULL, NULL));
	menu->appendSeparator();
#endif
...
void cleanup_menus()
{
	delete sMenuParcelObserverp;
	sMenuParcelObserverp = NULL;
...
//MK
		if (gRLenabled && !gRLInterface.canTouch(objp, pick.mIntersection))
		{
			return true;
		}
//mk
		...
		msg->addUUIDFast(_PREHASH_SessionID, gAgentSessionID);
		msg->nextBlockFast(_PREHASH_ObjectData);
		msg->addU32Fast(_PREHASH_LocalID, objp->mLocalID);
		...
		msg->sendMessage(objp->getRegion()->getHost());
...
class LLObjectEnableTouch final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		...
		if (objp)
		{
			new_value |= objp->flagHandleTouch() || !objp->flagsLoaded();
			LLViewerObject* parentp = (LLViewerObject*)objp->getParent();
			if (parentp)
			{
				new_value |= parentp->flagHandleTouch() ||
							 !parentp->flagsLoaded();
			}
		}
		gMenuHolderp->findControl(userdata["control"].asString())->setValue(new_value);
		...
		LLSD label;
		if (nodep && nodep->mValid && !nodep->mTouchName.empty())
		{
			label =  LLSD(nodep->mTouchName);
		}
		else
		{
			label = userdata["data"];
...
void load_gltf_scene_callback(HBFileSelector::ELoadFilter,
							  std::string& filename, void* datap)
{
	...
	if (gUsePBRShaders)
	{
		const LLUUID* handle_object_idp = (const LLUUID*)datap;
		error = LLGLTFSceneManager::load(filename, *handle_object_idp);
	}
	else
	{
		error = "Not in PBR rendering mode.";
	}
	if (!error.empty())
	{
		LLSD args;
		...
		args["REASON"] = error;
		gNotifications.add("GLTFLoadFailed", args);
	}
}

void load_gltf_scene()
{
	static LLUUID handle_object_id;

	if (!gUsePBRShaders || HBFileSelector::isInUse())
	{
		return;
	}
	LLViewerObject* objectp = gSelectMgr.getSelection()->getFirstRootObject();
	if (!objectp || objectp->isDead() || objectp->isAttachment())
	{
		gNotifications.add("GLTFLoadSelection");
		return;
	}

	handle_object_id = objectp->getID();
	HBFileSelector::loadFile(HBFileSelector::FFLOAD_GLTF,
							 load_gltf_scene_callback,
							 (void*)&handle_object_id);
}

class LLObjectLoadGltfScene final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		load_gltf_scene();
...
class LLObjectCanLoadGltfScene final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		bool enable = gUsePBRShaders && !HBFileSelector::isInUse() &&
					  LLViewerShaderMgr::sCanRenderGLTFScene;
		LLViewerObject* objp = NULL;
		if (enable)
		{
			objp = gSelectMgr.getSelection()->getFirstRootObject();
			enable = objp && !objp->isDead();
		}
		if (enable)
		{		
			LLPermissions* permsp = gSelectMgr.findObjectPermissions(objp);
			enable = permsp && permsp->allowModifyBy(gAgentID);
		}
...
void save_gltf_scene_callback(HBFileSelector::ESaveFilter,
							  std::string& filename, void* datap)
{
	if (filename.empty())
	{
	...
	{
		const LLUUID* handle_object_idp = (const LLUUID*)datap;
		error = LLGLTFSceneManager::save(filename, *handle_object_idp);
	}
	else
	{
		error = "Not in PBR rendering mode.";
	...
	if (!error.empty())
	{
		...
		args["FILENAME"] = filename;
		args["REASON"] = error;
		gNotifications.add("GLTFSaveFailed", args);
	}
}
...
void save_gltf_scene()
{
	...
	LLViewerObject* objectp = gSelectMgr.getSelection()->getFirstRootObject();
	if (!objectp || objectp->isDead() || objectp->mGLTFAsset.isNull())
	{
		gNotifications.add("GLTFSaveSelection");
		return;
	}

	handle_object_id = objectp->getID();
	HBFileSelector::saveFile(HBFileSelector::FFSAVE_GLTF, "scene.gltf",
							 save_gltf_scene_callback,
							 (void*)&handle_object_id);
}

class LLObjectSaveGltfScene final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		save_gltf_scene();
...
class LLObjectCanSaveGltfScene final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		bool enable = gUsePBRShaders && !HBFileSelector::isInUse() &&
		...
		{
			objp = gSelectMgr.getSelection()->getFirstRootObject();
			enable = objp && !objp->isDead() && objp->mGLTFAsset.notNull();
		}
		if (enable)
		{
			bool has_export_perm = gAgent.regionHasExportPermSupport();
			ExportPolicy policy = gIsInSecondLife || !has_export_perm ?
									ep_full_perm : ep_export_bit;
			LLPermissions* permsp = gSelectMgr.findObjectPermissions(objp);
			enable = permsp && permsp->allowExportBy(gAgentID, policy);
		}
		gMenuHolderp->findControl(userdata["control"].asString())->setValue(enable);
		return true;
	}
...
void upload_gltf_scene()
{
	LLViewerObject* objectp = gSelectMgr.getSelection()->getFirstRootObject();
	if (!objectp || objectp->isDead() || objectp->mGLTFAsset.isNull())
	{
		gNotifications.add("GLTFUploadSelection");
		return;
	}

	std::string error = LLGLTFSceneManager::upload(objectp->getID());
	if (!error.empty())
	{
		LLSD args;
		args["REASON"] = error;
		gNotifications.add("GLTFUploadFailed", args);
	}
}

class LLObjectUploadGltfScene final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		upload_gltf_scene();
...
class LLObjectCanUploadGltfScene final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		bool enable = gIsInSecondLifeBetaGrid && gUsePBRShaders &&
					  LLViewerShaderMgr::sCanRenderGLTFScene;
		LLViewerObject* objp = NULL;
		if (enable)
		{
...
void clear_gltf_scene()
{
	LLViewerObject* objectp = gSelectMgr.getSelection()->getFirstRootObject();
	if (!objectp || objectp->isDead() || objectp->mGLTFAsset.isNull())
	{
		gNotifications.add("GLTFClearSelection");
		return;
	}
	LLGLTFSceneManager::load("", objectp->getID());
}

class LLObjectClearGltfScene final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		clear_gltf_scene();
...
class LLObjectCanClearGltfScene final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		bool enable = gUsePBRShaders && LLViewerShaderMgr::sCanRenderGLTFScene;
		LLViewerObject* objp = NULL;
		if (enable)
		{
			objp = gSelectMgr.getSelection()->getFirstRootObject();
...
//mk
	...
	gObjectList.mBlackListedObjects.emplace(object_id);
...
//mk
		...
		bool new_value = objp && !objp->permYouOwner();
		if (new_value)
		{
			LLVOAvatar* avatar = find_avatar_from_object(objp);
			if (avatar)
			{
				// It is an avatar
...
void handle_refresh_avatar(LLVOAvatar* avatarp, bool refresh_all)
{
	...
	reload_avatar_texture(avatarp, TEX_HAIR_BAKED);
	reload_avatar_texture(avatarp, TEX_EYES_BAKED);
	reload_avatar_texture(avatarp, TEX_HEAD_BAKED);
	reload_avatar_texture(avatarp, TEX_UPPER_BAKED);
	reload_avatar_texture(avatarp, TEX_LOWER_BAKED);
	reload_avatar_texture(avatarp, TEX_SKIRT_BAKED);
	...
	avatarp->sendAvatarTexturesRequest(true);

	// If we just wanted to refresh the baked textures, then we are done. HB
	if (!refresh_all) return;

	// Reset the avatar skeleton to un-deform it (and re-apply the attachments
	// joint overrides to it). HB
	avatarp->resetSkeleton();

	// Set all mesh attachments LOD to a different LOD than the current one
	// (they will stay at new LOD only till the next LLVOVolume::updateLOD()
		 ...
		 i < count; ++i)
	{
		...
		if (volp)
		{
			// Make is so that any failed skin info will be retried. HB
			volp->resetSkinInfoFailed();
	...
	// The above might have modified the visual complexity, so... HB
	avatarp->updateVisualComplexity();

	// Also restart the avatar's animations. HB
	for (LLVOAvatar::anim_it_t it = avatarp->mPlayingAnimations.begin(),
							   end = avatarp->mPlayingAnimations.end();
		 it != end; ++it)
	{
		const LLUUID& anim_id = it->first;
...
bool enable_buy()
{
	...
	if (nodep)
	{
		...
		if (objp && objp->permAnyOwner() && nodep->mSaleInfo.isForSale() &&
...
bool sit_on_object(LLViewerObject* objp, const LLVector3& offset)
{
	if (!objp || objp->getPCode() != LL_PCODE_VOLUME || !objp->getRegion())
	{
		return false;
	}
...
//MK
	if (gRLenabled)
	{
		...
		{
			LLVector3 pos = objp->getPositionRegion() + offset;
...
//mk
	...
	msg->addUUIDFast(_PREHASH_TargetID, objp->mID);
	msg->addVector3Fast(_PREHASH_Offset, offset);

	objp->getRegion()->sendReliableMessage();
...
//mk
		LLVOAvatar* avatarp =
			find_avatar_from_object(gSelectMgr.getSelection()->getPrimaryObject());
		bool new_value = avatarp &&
						 !LLAvatarTracker::isAgentFriend(avatarp->getID());
		gMenuHolderp->findControl(userdata["control"].asString())->setValue(new_value);
		return true;
	}
};
...
bool sitting_on_selection()
{
	...
	LLViewerObject* root_object = nodep->getObject();
	if (!root_object)
	{
		return false;
	}
...
bool handle_sit_or_stand()
{
	...
	if (!objp || pick.mPickType == LLPickInfo::PICK_FLORA)
	{
		return true;
	}
...
//mk
	...
	sit_on_object(objp, pick.mObjectOffset);
	return true;
}
	...
	else
	{
		...
		if (region)
		{
			...
			else
			{
				struct f final : public LLSelectedObjectFunctor
				{
					bool apply(LLViewerObject* objp) override
					{
//MK
						if (gRLenabled && gRLInterface.mContainsUnsit &&
							objp->isAgentSeat())
...
//mk
						return objp->permModify() || objp->isReturnable();
					}
				} func;
...
//mk
		 ...
		 iter != end; ++iter)
	{
		...
		if (!objp->isPermanentEnforced() &&
			(nodep->mPermissions->getOwner() == gAgentID ||
			 (objp->permModify() &&
			  nodep->mPermissions->allowTransferTo(gAgentID))))
		{
			return true;
...
bool is_selection_buy_not_take()
{
		 ...
		 iter != end; ++iter)
	{
		...
		LLViewerObject* objp = nodep->getObject();
		if (objp && !objp->permYouOwner() && nodep->mSaleInfo.isForSale())
		{
...
S32 selection_price()
{
		 ...
		 iter != end; ++iter)
	{
		...
		if (objp && !(objp->permYouOwner()) && (nodep->mSaleInfo.isForSale()))
		{
			// You do not own the object and it is for sale. Add its price.
			total_price += nodep->mSaleInfo.getSalePrice();
		}
	}
...
class LLToolsSaveToObjectInventory final : public view_listener_t
{
	...
	{
		LLSelectNode* nodep = gSelectMgr.getSelection()->getFirstRootNode();
		if (nodep && nodep->mValid && nodep->mFromTaskID.notNull())
		{
			// *TODO: check to see if the fromtaskid object exists.
			derez_objects(DRD_SAVE_INTO_TASK_INVENTORY, nodep->mFromTaskID);
		}
		return true;
	}
};
...
class LLToolsSnapObjectXY final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
			 ...
			 iter != end; ++iter)
		{
			...
			if (objp->permModify())
			{
				LLVector3d pos_global = objp->getPositionGlobal();
				...
				objp->setPositionGlobal(pos_global, false);
			}
		}
...
class LLEditEnableDelete final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		bool new_value = gEditMenuHandlerp && gEditMenuHandlerp->canDelete();
//MK
		if (gRLenabled && gRLInterface.mContainsRez &&
...
class LLEditDelete final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		...
		if (gEditMenuHandlerp && gEditMenuHandlerp->canDelete())
		{
...
{
	bool can_delete = gSelectMgr.canDelete();
//MK
	if (gRLenabled && gRLInterface.mContainsRez)
	{
		can_delete = false;
...
void handle_export_selected(void*)
{
		 ...
		 iter != end; ++iter)
	{
		...
		LLViewerObject* objp = nodep->getObject();
		msg->nextBlockFast(_PREHASH_ObjectData);
		msg->addUUIDFast(_PREHASH_ObjectID, objp->getID());
		llinfos << "Object: " << objp->getID() << llendl;
	}
...
class LLAvatarRender final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		...
		if (avatarp)
		{
			if (data == "normal")
			{
				avatarp->setVisualMuteSettings(LLVOAvatar::AV_RENDER_NORMALLY);
			}
			else if (data == "never")
			{
				avatarp->setVisualMuteSettings(LLVOAvatar::AV_DO_NOT_RENDER);
			}
			else if (data == "always")
			{
				avatarp->setVisualMuteSettings(LLVOAvatar::AV_ALWAYS_RENDER);
			}
		}
#if 0	// Avatar puppets "jelly-dollifying" does not work anyway...
		LLVOAvatarPuppet* puppet = objp->getPuppetAvatar();
		if (puppet)
		{
			if (data == "normal")
			{
...
//mk
		LLVOAvatar* avatarp =
			find_avatar_from_object(gSelectMgr.getSelection()->getPrimaryObject());
		if (avatarp && !LLAvatarTracker::isAgentFriend(avatarp->getID()))
		{
			LLAvatarActions::requestFriendshipDialog(avatarp->getID());
		}
		return true;
	}
};
...
//mk
			LLFloaterPay::payDirectly(objectp->getID(), false);
		}
		else
		{
			LLFloaterPay::payViaObject(objectp->getID());
		}
	}
...
bool handle_give_money_dialog()
{
	...
	params.functor(std::bind(complete_give_money, _1, _2,
...
//mk
		...
		{
			LLSelectNode* nodep = gSelectMgr.getSelection()->getFirstRootNode();
			if (nodep && nodep->mValid && !nodep->mSitName.empty())
			{
				label = LLSD(nodep->mSitName);
			}
			else
			{
				label = LLSD(sit_text);
...
{
	// Since this could be called while dispatching a SLURL (e.g. via Lua or
	// passed by another viewer instance), we must ensure we can indeed open a
	// floater just yet. HB
	if (!gFloaterViewp)
	{
		return;
	...
	static LLHandle<LLFloater> paths_handle;

	LLFloater* floaterp = paths_handle.get();
	if (floaterp)
	{
		gFloaterViewp->bringToFront(floaterp);
		return;
	}
	floaterp = new LLFloater("viewer paths");
	paths_handle = floaterp->getHandle();
	LLUICtrlFactory* factoryp = LLUICtrlFactory::getInstance();
	factoryp->buildFloater(floaterp, "floater_viewer_paths.xml");

	floaterp->center();

	std::string exe_path, full_path;
	exe_path = gDirUtil.getExecutableDir() + LL_DIR_DELIM_STR +
			   gDirUtil.getExecutableFilename();
	floaterp->childSetValue("viewer", exe_path);
	full_path = gDirUtil.getLLPluginLauncher();
	floaterp->childSetValue("plugin", full_path);
	// We "cheat" for those, since we do not have a way to get the path from
	// the viewer code (the path is set by Dullahan itself). HB
#if LL_LINUX
	static const char* target_str = "SLPlugin";
	static const char* edited_str = "dullahan_host";
#elif LL_WINDOWS
	static const char* target_str = "SLPlugin";
	static const char* edited_str = "llplugin\\dullahan_host.exe";
#endif
	LLStringUtil::replaceString(full_path, target_str, edited_str);
	floaterp->childSetValue("dullahan", full_path);
	// For directories, append a separator to make it clear they are not
	// files. HB
	full_path = gDirUtil.getOSUserAppDir() + LL_DIR_DELIM_STR;
	floaterp->childSetValue("settings", full_path);
	full_path = gDirUtil.getCacheDir() + LL_DIR_DELIM_STR;
	floaterp->childSetValue("cache", full_path);
}
		...
		else if (floater_name == "appearance")
		{
			if (gAgentWearables.areWearablesLoaded())
			{
				gAgent.changeCameraToCustomizeAvatar();
				 ...
				 floater_name == "outfitpicker2")
		{
			if (gIsInSecondLife && gAgentWearables.areWearablesLoaded())
			{
				std::string url;
				if (floater_name == "outfitpicker")
				{
					url = gSavedSettings.getString("OutfitPickerURL");
				}
				else
				{
					url = gSavedSettings.getString("OutfitPickerURL2");
				}
				if (!url.empty())
				{
...
//mk
		...
		LLVOAvatar* avatarp = find_avatar_from_object(agent_id);
		if (avatarp)
		{
			LLFloaterAvatarInfo::show(avatarp->getID());
		}
		return true;
	}
};
...
class LLWorldSetSkySunlightScale final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		F32 value = (F32)userdata.asReal();
		gSavedSettings.setF32("RenderSkySunlightScale", value);
		return true;
	}
};

class LLWorldShowSkySunlightScale final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		static LLCachedControl<F32> scale(gSavedSettings,
										  "RenderSkySunlightScale");
		F32 setting = (F32)userdata["data"].asReal();
		bool b = gUsePBRShaders && fabsf((F32)scale - setting) <= 0.05f;
		gMenuHolderp->findControl(userdata["control"].asString())->setValue(b);
		return true;
	}
};

class LLWorldHasSkySunlightScale final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
...
class LLWorldHasAdjustPBR final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		static LLCachedControl<bool>auto_hdr(gSavedSettings,
											 "RenderSkyAutoAdjustLegacy");
		bool active = gUsePBRShaders && auto_hdr &&
					  LLPipeline::sReflectionProbesEnabled;
		gMenuHolderp->findControl(userdata["control"].asString())->setValue(active);
		return true;
	}
};

void load_from_exr_callback(HBFileSelector::ELoadFilter,
							std::string& filename, void*)
{
	if (filename.empty() || !gUsePBRShaders)
	{
		return;
	}

	std::string error = LLDrawPoolWLSky::loadHDRISky(filename);
	if (error.empty())
	{
		gSavedSettings.setBool("RenderHDRIEnabled", true);
	}
	else
	{
		LLSD args;
		args["FILENAME"] = filename;
		args["REASON"] = error;
		gNotifications.add("FailedToLoadEXR", args);
	}
}
...
void load_hdri_sky(bool force)
{
	...
	if (!force && LLDrawPoolWLSky::getHDRISky())
	{
		return;
	}
	HBFileSelector::loadFile(HBFileSelector::FFLOAD_HDRI,
							 load_from_exr_callback);
}

class LLWorldLoadHDRISky final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		load_hdri_sky(true);
...
class LLWorldCanLoadHDRISky final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		bool new_value = gUsePBRShaders && !HBFileSelector::isInUse();
		gMenuHolderp->findControl(userdata["control"].asString())->setValue(new_value);
		return true;
	}
};

class LLWorldUseHDRISky final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		static LLCachedControl<bool> hdri(gSavedSettings, "RenderHDRIEnabled");
		bool new_value = hdri && gUsePBRShaders &&
						 LLDrawPoolWLSky::getHDRISky();
		gMenuHolderp->findControl(userdata["control"].asString())->setValue(new_value);
		return true;
	}
};

class LLWorldHasHDRISky final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		bool new_value = gUsePBRShaders && LLDrawPoolWLSky::getHDRISky();
		gMenuHolderp->findControl(userdata["control"].asString())->setValue(new_value);
		return true;
	}
};

class LLWorldReflectionsEnabled final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		bool enable = gUsePBRShaders && LLPipeline::sReflectionProbesEnabled;
		gMenuHolderp->findControl(userdata["control"].asString())->setValue(enable);
		return true;
	}
};

class LLWorldHasDesaturate final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		static LLCachedControl<bool>desaturate(gSavedSettings,
											   "RenderSkyReflectionDesaturate");
		bool active = gUsePBRShaders && LLRender::sClassicMode && desaturate;
		gMenuHolderp->findControl(userdata["control"].asString())->setValue(active);
		return true;
	}
};

class LLWorldDesaturateEnabled final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		bool enabled = gUsePBRShaders && LLRender::sClassicMode;
		gMenuHolderp->findControl(userdata["control"].asString())->setValue(enabled);
		return true;
	}
};
...
//mk
		...
		LLViewerObject* objp = sObjectSelection->getFirstRootObject();
		if (objp)
		{
...
{
	LLViewerJointAttachment* attachp = (LLViewerJointAttachment*)user_data;
...
void confirm_replace_attachment(S32 option, void* user_data)
{
	...
	LLViewerObject* objp = gSelectMgr.getSelection()->getFirstRootObject();
	if (!objp)
	{
	...
	F32 stop_dist = SCALE_FUDGE * objp->getMaxScale() + ARM_LENGTH;
	if (stop_dist < MIN_STOP_DISTANCE)
	{
		stop_dist = MIN_STOP_DISTANCE;
	}

	LLVector3 dest = objp->getPositionAgent();
...
{
	LLViewerJointAttachment* attachp = (LLViewerJointAttachment*)user_data;
	if (attachp && attachp->getNumObjects() > 0)
	{
//MK
		if (gRLenabled &&
			!gRLInterface.canDetachAllObjectsFromAttachment(attachp))
		{
			return;
		}
//mk
		...
		for (LLViewerJointAttachment::attachedobjs_vec_t::const_iterator
				iter = attachp->mAttachedObjects.begin(),
				end = attachp->mAttachedObjects.end();
			 iter != end; ++iter)
		{
			LLViewerObject* objp = iter->get();
			if (objp)
			{
				msg->nextBlockFast(_PREHASH_ObjectData);
...
void attach_label(std::string& label, void* user_data)
{
	...
	if (attachp)
	{
		label = LLTrans::getString(attachp->getName());
		for (LLViewerJointAttachment::attachedobjs_vec_t::const_iterator
				iter = attachp->mAttachedObjects.begin(),
				end = attachp->mAttachedObjects.end();
			 iter != end; ++iter)
		{
			const LLViewerObject* objp = iter->get();
			if (objp)
			{
				LLViewerInventoryItem* itemp =
					gInventory.getItem(objp->getAttachmentItemID());
				if (itemp)
				{
					label += " (" + itemp->getName() + ")";
					break;
...
//mk
		...
		if (!objp || objp->isAvatar())
		{
			llwarns << "No objp to detach" << llendl;
			return true;
		}
...
//mk
	...
	LLViewerObject* avatarp = objectp;
	while (avatarp)
	{
		// ...if it is you, good to detach
		if (avatarp->getID() == gAgentID)
		{
			return true;
		}
		avatarp = (LLViewerObject*)avatarp->getParent();
	}
...
class LLAttachmentEnableDrop final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		...
		{
			S32 attach_id =
				ATTACHMENT_ID_FROM_STATE(objp->getAttachmentState());
...
{
	LLViewerJointAttachment* attachp = (LLViewerJointAttachment*)user_data;
	return attachp && attachp->getNumObjects() > 0;
}
...
//mk
		...
		if (avatarp)
		{
			LLAvatarActions::startIM(avatarp->getID());
		}
		return true;
	}
};
...
namespace
{
	struct QueueObjects final : public LLSelectedObjectFunctor
	{
		...
		bool apply(LLViewerObject* objp) override
		{
			scripted = objp->flagScripted();
			modifiable = objp->permModify();
...
{
	LLFloaterCompileQueue* queuep = LLFloaterCompileQueue::create(to_lang);
	queue_actions(queuep, "CannotRecompileSelectObjectsNoScripts",
				  "CannotRecompileSelectObjectsNoPermission");
}

void handle_reset_selection()
{
	LLFloaterResetQueue* queuep = LLFloaterResetQueue::create();
	queue_actions(queuep, "CannotResetSelectObjectsNoScripts",
				  "CannotResetSelectObjectsNoPermission");
}

void handle_set_run_selection()
{
	LLFloaterRunQueue* queuep = LLFloaterRunQueue::create();
	queue_actions(queuep, "CannotSetRunningSelectObjectsNoScripts",
				  "CannotSerRunningSelectObjectsNoPermission");
}

void handle_set_not_run_selection()
{
	LLFloaterStopQueue* queuep = LLFloaterStopQueue::create();
	queue_actions(queuep, "CannotSetRunningNotSelectObjectsNoScripts",
...
//mk
		...
		{
			handle_compile_queue("lsl2");
		}
		else if (action == "compile lua")
		{
			handle_compile_queue("lsl-luau");
		}
...
// This is used in the GL menus to set control values.
class LLToggleControlPerAccount final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		std::string control_name = userdata.asString();
		bool checked = gSavedPerAccountSettings.getBool(control_name.c_str());
		gSavedPerAccountSettings.setBool(control_name.c_str(), !checked);
		return true;
	}
};
...
class LLEditableSelectedMono final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		...
		{
			bool enable = is_editable_selected() &&
						  gAgent.hasRegionCapability("UpdateScriptTask");
			gMenuHolderp->findControl(userdata["control"].asString())->setValue(enable);
			return true;
		}
		return false;
...
class LLEditableSelectedLua final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		if (gMenuHolderp &&
		...
		{
			LLViewerRegion* regionp = gAgent.getRegion();
			bool enable = regionp && regionp->hasLuaCompileTarget() &&
						  is_editable_selected();
			gMenuHolderp->findControl(userdata["control"].asString())->setValue(enable);
			return true;
		}
		return false;
...
//mk
				return !objp->permCopy() || objp->isAttachment();
			}
		} func;
...
//mk
		...
		if (LLDrawPoolAlpha::sShowDebugAlpha)
		{
			...
			// sShowDebugAlpha is true, so rebuild batches whenever toggling
			// this flag on.
			gPipeline.rebuildDrawInfo();
			// Needed too (the above rebuilds *visible* groups, but we also
			// need to rebuild individual objects since they might be part
			// of a group only containing render-skipped objects). HB
			handle_objects_visibility(NULL);
		}
...
class LLFileCheckUploadBulk2K final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		static LLCachedControl<bool> upload_2k(gSavedPerAccountSettings,
											   "BulkUpload2KTextures");
		bool checked = !s2KTextureCost || upload_2k;
		if (checked && s2KTextureCost)
		{
			checked = can_afford_transaction(s2KTextureCost);
		}
		gMenuHolderp->findControl(userdata["control"].asString())->setValue(checked);
		return true;
	}
...
class LLFileEnableUploadBulk2K final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		bool enabled = gMaxImageSizeDefault > 1024 && s2KTextureCost &&
					   can_afford_transaction(s2KTextureCost);
		gMenuHolderp->findControl(userdata["control"].asString())->setValue(enabled);
		return true;
	}
};
...
static std::string IMAGE_EXTENSIONS = "tga png jpg jpeg j2c bmp";
static std::string MATERIAL_EXTENSIONS = "gltf glb";
static std::string ANIM_EXTENSIONS =  "bvh anim";
static std::string XML_EXTENSIONS = "xml";
static std::string LSL_EXTENSIONS = "lsl slua";
						  ...
						  std::string& filename, void*)
{
	...
	std::string ext = LLDir::getExtension(filename);
...
class LLFileEnableCloseWindow final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		...
		// select the File -> Quit menu item anyway (it is not yet drawn)... HB
							 ...
							 std::deque<std::string>& files, void*)
{
	...
	{
		infile = files.front();
		extension = LLDir::getExtension(infile);
		EImageCodec codec = LLImageBase::getCodecFromExtension(extension);
		if (codec == IMG_CODEC_INVALID)
		{
			report = "Error compressing image: " + infile +
					 " - Unknown codec !";
			llinfos << report << llendl;
		}
		else if (codec == IMG_CODEC_J2C)
		{
			report = "Image '" + infile + "' is already in JPEG2000 format.";
			llinfos << report << llendl;
		}
		else
		{
			outfile = LLDir::getDirName(infile) + LL_DIR_DELIM_STR +
					  LLDir::getBaseFileName(infile, true) + ".j2c";

			llinfos << "Compressing image... Input: " << infile
					<< " - Output: " << outfile << llendl;

			if (LLViewerTextureList::createUploadFile(infile, outfile, codec))
			{
				llinfos << "Compression complete." << llendl;
				report = "'" + infile + "' successfully compressed to '" +
						 outfile + "'";
			}
			else
			{
...
class LLFileEnableBackupObject final : public view_listener_t
{
	...
	{
		LLViewerObject* objp = gSelectMgr.getSelection()->getPrimaryObject();
		bool new_value = objp && !HBFileSelector::isInUse() &&
		...
		if (new_value)
		{
			struct ff final : public LLSelectedNodeFunctor
			{
				...
				bool apply(LLSelectNode* nodep) override
				{
					...
					LLPermissions* item_permissions = nodep->mPermissions;
...
static void addMenu(view_listener_t* menup, const std::string& name)
{
	sMenus.push_back(menup);
	menup->registerListener(gMenuHolderp, name);
}
...
void initialize_menus()
{
	...
	addMenu(new LLFileCheckUploadBulk2K(), "File.CheckUploadBulk2K");
	addMenu(new LLFileEnableUploadBulk2K(), "File.EnableUploadBulk2K");
...
//mk
	...
	addMenu(new LLWorldSetSkySunlightScale(), "World.SetSkySunlightScale");
	addMenu(new LLWorldShowSkySunlightScale(), "World.ShowSkySunlightScale");
	addMenu(new LLWorldHasSkySunlightScale(), "World.HasSkySunlightScale");
	addMenu(new LLWorldHasAdjustPBR(), "World.HasAdjustPBR");
	addMenu(new LLWorldHasDesaturate(), "World.HasDesaturate");
	addMenu(new LLWorldDesaturateEnabled(), "World.DesaturateEnabled");
	addMenu(new LLWorldReflectionsEnabled(), "World.ReflectionsEnabled");
	addMenu(new LLWorldLoadHDRISky(), "World.LoadHDRISky");
	addMenu(new LLWorldCanLoadHDRISky(), "World.CanLoadHDRISky");
	addMenu(new LLWorldUseHDRISky(), "World.UseHDRISky");
	addMenu(new LLWorldHasHDRISky(), "World.HasHDRISky");
	...
	addMenu(new LLObjectLoadGltfScene(), "Object.LoadGLTF");
	addMenu(new LLObjectCanLoadGltfScene(), "Object.CanLoadGLTF");
	addMenu(new LLObjectSaveGltfScene(), "Object.SaveGLTF");
	addMenu(new LLObjectCanSaveGltfScene(), "Object.CanSaveGLTF");
	addMenu(new LLObjectUploadGltfScene(), "Object.UploadGLTF");
	addMenu(new LLObjectCanUploadGltfScene(), "Object.CanUploadGLTF");
	addMenu(new LLObjectClearGltfScene(), "Object.ClearGLTF");
	addMenu(new LLObjectCanClearGltfScene(), "Object.CanClearGLTF");
	...
	addMenu(new LLToggleControlPerAccount(), "ToggleControlPerAccount");
	...
	addMenu(new LLEditableSelectedLua(), "EditableSelectedLua");
}
