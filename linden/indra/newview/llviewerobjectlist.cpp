...
// viewers to issue warnings when they set "Set" as a bool, which it should
// be). HB
...
#	pragma omp parallel for private(objectp) reduction(|:got_avatars)
	...
	// result in OpenGL calls, that might themselves be threaded in the driver
	// via pthread !  HB
...
void LLViewerObjectList::fetchObjectCosts()
{
	...
	gCoros.launch("LLViewerObjectList::fetchObjectCostsCoro",
				  std::bind(&LLViewerObjectList::fetchObjectCostsCoro, this,
							url));
}
...
void LLViewerObjectList::fetchPhysicsFlags()
{
	...
	gCoros.launch("LLViewerObjectList::fetchPhysicsFlagsCoro",
				  std::bind(&LLViewerObjectList::fetchPhysicsFlagsCoro, this,
							url));
}
...
void LLViewerObjectList::renderObjectsForMap(LLPanelMiniMap* mapp)
{
	...
	for (S32 i = 0, count = mMapObjects.size(); i < count; ++i)
	{
		...
		if (objectp->flagCharacter())
		{
			if (objectp->isRoot())
			{
				...
				// itself.
				mapp->addPathFindingCharacter(pos);
			}
			continue;
		}
		...
		// Physcical objects (mobiles) are ploted by the mini-map code itself,
		// on top of the objects image layer. HB
		if (objectp->flagUsePhysics())
		{
			if (objectp->isRoot())
			{
				mapp->addPhysicalObject(pos);
			}
			continue;
		}
		...
		mapp->renderScaledPointGlobal(pos, color, approx_radius);
	}
}
...
void LLViewerObjectList::reRegisterReflectionProbes()
{
	for (S32 i = 0, count = mObjects.size(); i < count; ++i)
	{
		LLViewerObject* objectp = mObjects[i];
		if (objectp && !objectp->isDead() && objectp->isReflectionProbe())
		{
			LLVOVolume* vovolp = objectp->asVolume();
			if (vovolp && vovolp->getVolume())
			{
				vovolp->updateReflectionProbePtr(true);
			}
		}
	}
}
...
void LLViewerObjectList::renderObjectBeacons()
{
...

	{
			 ...
			 iter != end; ++iter)
		{
			...
			{
				gGL.lineWidth((F32)line_width, true);
