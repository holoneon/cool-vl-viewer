...
// instance is created and destroyed in LLViewerWindow::initBase(). HB
LLChatBar* gChatBarp = NULL;

//static
bool LLChatBar::sSwappedShortcuts = false;
strings_set_t LLChatBar::sIgnoredNames;

// Helper functions

// Checks for all nearby avatars in turn and their legacy name, then replace
// any occurrence of @fullname_or_lastname_or_firstname with the corresponding
// mention SLURL. Note: this algorithm is slow, but since sending chat lines is
// a "rare" event (once every hundreds of frames at worst), it does not really
// impact performances... HB
void replace_arobase_names_with_mentions(std::string& text)
{
	LLViewerRegion* aregionp = gAgent.getRegion();
	if (!aregionp) return;	// Paranoia
	uuid_vec_t avatars;
	std::vector<LLVector3d> positions;
	gWorld.getAvatars(avatars, &positions, NULL, gAgent.getPositionGlobal(),
					  (F32)aregionp->getChatRange());
	std::string first_name, last_name, slurl_str, pattern;
	for (size_t i = 0, count = avatars.size();
		 i < count && text.find('@') != std::string::npos; ++i)
	{
		const LLUUID& id = avatars[i];
		if (gCacheNamep && gCacheNamep->getName(id, first_name, last_name))
		{
			slurl_str = LLSLURL("agent", id, "mention").getSLURLString();
			if (!last_name.empty() && last_name != "Resident")
			{
				// First, replace "@Full Name" occurrences (case insensitive)
				pattern = "@" + first_name + " " + last_name;
				LLStringUtil::replaceString(text, pattern, slurl_str, false);
				// Then, replace "@Lastname" occurrences (case insensitive)
				pattern = "@" + last_name;
				LLStringUtil::replaceString(text, pattern, slurl_str, false);
			}
			// Finally, replace "@Firstname" occurrences (case insensitive)...
			pattern = "@" + first_name;
			LLStringUtil::replaceString(text, pattern, slurl_str, false);
		}
	}
...
//MK
	...
	std::string final_text = utf8_out_text;
	...
	{
		final_text = gRLInterface.crunchEmote(final_text);
	}
//mk

	if (channel == 0 && type != CHAT_TYPE_START && type != CHAT_TYPE_STOP &&
		!(gRLenabled && gRLInterface.mContainsShownames) &&
		gSavedSettings.getBool("SendMentionsInChat") &&
		final_text.find('@') != std::string::npos)
	{
		replace_arobase_names_with_mentions(final_text);
	}

	// In case we try and send more than MAX_MSG_STR_LEN characters to the
	// public chat (which can happen with MU*-pose style and with @mentions
	// SLURLs), split the text into several posts. HB
	while (channel == 0 && final_text.size() > (size_t)MAX_MSG_STR_LEN)
	{
		std::string chunk = final_text.substr(0, MAX_MSG_STR_LEN - 5);
		size_t i = chunk.find_last_of(' ');
		if (i == std::string::npos)
		{
			llwarns << "Cannot split chat text to make it fit "
					<< MAX_MSG_STR_LEN
					<< " characters (UDP messaging limit): truncating."
					<< llendl;
			final_text = utf8str_truncate(final_text, MAX_MSG_STR_LEN);
			break;
		}
		chunk.erase(i + 1);	// Keep the space, wipe out the rest.
		chunk += "...";
		msg->newMessageFast(_PREHASH_ChatFromViewer);
		msg->nextBlockFast(_PREHASH_AgentData);
		msg->addUUIDFast(_PREHASH_AgentID, gAgentID);
		...
		msg->nextBlockFast(_PREHASH_ChatData);
		msg->addStringFast(_PREHASH_Message, chunk);
		msg->addU8Fast(_PREHASH_Type, type);
		msg->addS32(_PREHASH_Channel, channel);
		gAgent.sendReliableMessage();

		final_text = "/me ..." + final_text.substr(i);	// With leading space
	}
	...
	if (channel >= 0)
	{
		...
		msg->addUUIDFast(_PREHASH_SessionID, gAgentSessionID);
		msg->nextBlockFast(_PREHASH_ChatData);
		msg->addStringFast(_PREHASH_Message, final_text);
		msg->addU8Fast(_PREHASH_Type, type);
		msg->addS32(_PREHASH_Channel, channel);
	}
	else
	{
		...
		msg->addString(_PREHASH_ButtonLabel, final_text);
	}
...
void LLChatBar::setMaxTextLength()
{
	static LLCachedControl<U32> len(gSavedSettings, "ChatInputMaxCharacters");
	if (mInputEditor)
	{
		mInputEditor->setMaxTextLength(llclamp((U32)len, DB_CHAT_MSG_STR_LEN,
											   4 * DB_CHAT_MSG_STR_LEN - 40));
	}
}
...
bool LLChatBar::handleKeyHere(KEY key, MASK mask)
{
	...
	else if (mInputEditor && key == KEY_TAB && mask == MASK_NONE &&
			 gSavedSettings.getBool("TabAutoCompleteName"))
	{
		...
		if (text.length() > 0)
		{
			...
			// could be the end of the text, a space, or a punctuation)
	...
	else if (key == KEY_ESCAPE && mask == MASK_NONE && gChatBarp &&
			 gChatBarp == this)
	{
		stopChat();
		handled = true;
...
bool LLChatBar::hasTextEditor() const
{
...
bool LLChatBar::hasChatText() const
{
	return mInputEditor && mInputEditor->getLength() > 0;
}
								   ...
								   bool animate, bool lua_propagate)
{
	...
	// Note: we now can deal with more than 1023 characters for channel 0
	// (public chat) in send_chat_from_viewer(). HB
	if (channel != 0 && !utf8_out_text.empty())
	{
...
//mk
	// Do not animate for chats people cannot hear (chat to scripts)
	if (animate && channel == 0)
	{
		LLUUID anim;
		if (type == CHAT_TYPE_WHISPER)
		{
			LL_DEBUGS("SendChat") << "You whisper something." << LL_ENDL;
			anim = ANIM_AGENT_WHISPER;
		}
		else if (type == CHAT_TYPE_NORMAL)
		{
			LL_DEBUGS("SendChat") << "You say something" << LL_ENDL;
			anim = ANIM_AGENT_TALK;
		}
		else if (type == CHAT_TYPE_SHOUT)
		{
			LL_DEBUGS("SendChat") << "You shout something." << LL_ENDL;
	...
	else if (channel != 0 && type != CHAT_TYPE_START && type != CHAT_TYPE_STOP)
	{
		LL_DEBUGS("SendChat") << "You send some text over chat channel: "
							  << channel << LL_ENDL;
	}
...
static void capitalize(std::string& str)
{
	...
	// Do not attempt to capitalize non-ASCII characters, of characters which
	// are already capitalized. HB
	char& c = str.front();
	if (c >= 'a' && c <= 'z')
	{
		c -= 'a' - 'A';
...
//mk
	...
	for (size_t i = 0, count = avatars.size(); i < count; ++i)
	{
		const LLUUID& id = avatars[i];
		if (gCacheNamep && gCacheNamep->getName(id, first_name, last_name))
		{
			...
			if (!name.empty() && name.find(pattern) == 0)
			{
			...
			LLAvatarNameCache::get(id, &avatar_name))
		{
				...
				display_name != first_name + " " + last_name)
			{
				...
				if (j > 0)
				{
					part_name = display_name.substr(0, j);
					...
					display_name = display_name.substr(j + 1);
					name = display_name;
					LLStringUtil::toLower(name);
				}
				if (!name.empty() && name.find(pattern) == 0)
		...
		if (add_to_ignore)
		{
			name = first_name + " " + last_name + " " + part_name + " " +
				   display_name;
			// Display names can change, so do not rely on avatar UUIDs
	...
	else if (matches.size() > 1)
	{
		...
		for (size_t i = match.length(); i <= len; ++i)
		{
			...
			LLStringUtil::toLower(pattern);
			for (strings_set_t::iterator it = matches.begin(),
										 end = matches.end();
				 it != end; ++it)
			{
				name = *it;
