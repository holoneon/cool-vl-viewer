...
void LLViewerTextureList::init()
{
	...
	// Set the default flat normal map. Note: in SL, this texture, which exists
	// server-side, has compression artifacts, so we must force-replace it here
	// with a texture we load from our own viewer distribution files.
...
static std::string get_texture_list_name()
{
	...
	filename += ".xml";
	return gDirUtil.getFullPath(LL_PATH_PER_ACCOUNT, filename);
}
...
void LLViewerTextureList::doPrefetchImages()
{
	...
	{
		LLSD image_llsd = *iter;
		LLUUID tex_id = image_llsd["uuid"];
		S32 pixel_area = image_llsd["area"];
		S32 texture_type = image_llsd["type"];
		if (texture_type == LLViewerTexture::FETCHED_TEXTURE ||
			texture_type == LLViewerTexture::LOD_TEXTURE)
		{
			...
			if (texp)
			{
				texp->addTextureStats((F32)pixel_area);
			}
		}
	}
}
...
void LLViewerTextureList::shutdown()
{
	...
	if (count > 0 && !gDirUtil.getLindenUserDir().empty())
	{
		std::string filename = get_texture_list_name();
...
void LLViewerTextureList::dump()
{
		 ...
		 it != end; ++it)
	{
		...
		llinfos << "priority " << image->getDecodePriority()
				...
				<< " https://asset.siva.lindenlab.com/" << image->getID()
				<< ".texture" << llendl;
	}
															  ...
															  const LLUUID& force_id)
{
	...
	std::string full_path = gDirUtil.findSkinnedFilename("textures", filename);
	if (full_path.empty())
	{
													  ...
													  LLHost from_host)
{
	...
	else
	{
		...
		{
			// We need BOOST_ALM texture for something, 'rise' to NONE. HB
...
void LLViewerTextureList::addImage(LLViewerFetchedTexture* new_texp)
{
	if (!new_texp)
	{
		return;
	...
	const LLUUID& tex_id = new_texp->getID();

	LLViewerFetchedTexture* texp = findImage(tex_id);
	if (texp)
	{
		llwarns << "Texture with ID " << tex_id << " already in list"
				<< llendl;
	}
	++sNumImages;

	addImageToList(new_texp);
	mUUIDMap[tex_id] = new_texp;
}
...
//static
bool LLViewerTextureList::createRawImageFromFile(const std::string& filename,
												 LLPointer<LLImageRaw> rawp,
												 S8 codec)
{
	if (codec == IMG_CODEC_INVALID)
	{
		std::string exension = LLDir::getExtension(filename);
		codec = LLImageBase::getCodecFromExtension(exension);
		if (codec == IMG_CODEC_INVALID)
		{
			return false;
		}
	...
	// *TODO: implement JPEG2000 support for getDimensions() ?  HB
	if (codec >= IMG_CODEC_BMP && codec <= IMG_CODEC_PNG)
	{
		S32 width = 0;
		S32 height = 0;
		bool success = LLImage::getDimensions(filename, width, height, codec);
		if (!success)
		{
			llwarns << LLImage::getLastError() << llendl;
			return false;
		}
		constexpr S32 MAX_TEXTURE_AREA = 8192 * 8192;	// 256MB max raw image
		if (width * height >= MAX_TEXTURE_AREA)
		{
			llwarns << "Source image dimensions (" << width << "x" << height
					<< ") too large to create a raw image from it." << llendl;
			return false;
		}
	}
	...
	switch (codec)
	{
		case IMG_CODEC_J2C:
		{
			LLPointer<LLImageJ2C> imagep = new LLImageJ2C;
			if (!imagep->load(filename) || !imagep->decode(rawp))
			{
				return false;
			}
			break;
		...
		case IMG_CODEC_BMP:
		{
			LLPointer<LLImageBMP> imagep = new LLImageBMP;
			if (!imagep->load(filename) || !imagep->decode(rawp))
			{
				return false;
			}
		...
		case IMG_CODEC_TGA:
		{
			LLPointer<LLImageTGA> imagep = new LLImageTGA;
			if (!imagep->load(filename))
			{
				return false;
			}
			if (imagep->getComponents() != 3 && imagep->getComponents() != 4)
			{
				imagep->setLastError("Image files with less than 3 or more than 4 components are not supported.");
				return false;
			}
			if (!imagep->decode(rawp))
			{
				return false;
			}
			break;
		...
		case IMG_CODEC_JPEG:
		{
			LLPointer<LLImageJPEG> imagep = new LLImageJPEG;
			if (!imagep->load(filename) || !imagep->decode(rawp))
			{
				return false;
			}
...
bool LLViewerTextureList::createUploadFile(const std::string& filename,
										   ...
										   U8 codec, bool* is_2k_texturep)
{
	// First, load the image.
	LLPointer<LLImageRaw> raw_imagep = new LLImageRaw;
	if (!createRawImageFromFile(filename, raw_imagep, codec))
	{
		return false;
	}

	LLPointer<LLImageJ2C> j2cp = convertToUploadFile(raw_imagep);
	...
	if (is_2k_texturep)
	{
		constexpr U32 tex_1k_area = 1024 * 1024;
		U32 tex_area = j2cp->getWidth() * j2cp->getHeight();
		*is_2k_texturep = tex_area > tex_1k_area;
	}

	// Test to see if the encode and save worked.
	LLPointer<LLImageJ2C> imagep = new LLImageJ2C;
	if (!imagep->loadAndValidate(out_filename))
	{
															   ...
															   bool force_lossless)
{
	if (max_dimentions < 0)
	{
		max_dimentions = gMaxImageSizeDefault;
	}
...
bool LLUIImageList::initFromFile()
{
	...
	std::string base_file_path = gDirUtil.getFullPath(LL_PATH_SKINS, "default",
													  "textures",
													  "textures.xml");
	LLXMLNodePtr root;
	...
	paths.emplace_back(gDirUtil.getSkinDir() + LL_DIR_DELIM_STR + "textures" +
					   LL_DIR_DELIM_STR + "textures.xml");
	// Path to user overrides on current skin
	paths.emplace_back(gDirUtil.getUserSkinDir() + LL_DIR_DELIM_STR +
