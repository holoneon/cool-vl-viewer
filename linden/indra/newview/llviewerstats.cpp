...
void LLViewerStats::addpPacketsStats(U32 in, U32 out, U32 lost, U32 dropped)
{
	mPacketsInTotal += in;
	mPacketsLostTotal += lost;
	mPacketsDroppedTotal += dropped;
	mPacketsInStat.addValue(in);
	mPacketsOutStat.addValue(out);
	mPacketsLostPercentStat.addValue(in ? (F32)(100 * lost + dropped) / (F32)in
										: 0.f);
}
...
void LLViewerStats::sendStats()
{
	...
	if (system.isUndefined())
	{
		...
		std::string gpu_desc =
			llformat("%-6s Class %d ",
					 LLGLManager::sGLVendorShort.substr(0,6).c_str(),
		...
		system["gpu_vendor"] = LLGLManager::sGLVendorShort;
		system["gpu_version"] = LLGLManager::sDriverVersionVendorString;
		system["opengl_version"] = LLGLManager::sGLVersionString;
		LLGLManager::asLLSD(system["gl"]);
	}
	body["system"] = system;
	...
	fail["dropped"] = (S32)msg->mLostPackets;
	fail["dropped_by_viewer"] = (S32)msg->mDroppedPackets;
...
void LLViewerStats::avatarRenderingStats()
{
		 ...
		 it != end; ++it)
	{
		...
		gCoros.launch("LLViewerStats::getAvatarRenderStatsCoro",
					  std::bind(&LLViewerStats::getAvatarRenderStatsCoro, url,
								regionp->getName()));
		...
		gCoros.launch("LLViewerStats::sendAvatarRenderStatsCoro",
					  std::bind(&LLViewerStats::sendAvatarRenderStatsCoro,
								url, regionp->getName(),
								data[region_id_str]));
	}
