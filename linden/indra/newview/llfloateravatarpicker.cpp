...
//mk
	...
	for (U32 i = 0, count = avatar_ids.size(); i < count; ++i)
	{
		...
		element["id"] = av; // value
		std::string fullname;
		if (!gCacheNamep->getFullName(av, fullname))
		{
			element["columns"][0]["value"] = gCacheNamep->getDefaultName();
			all_loaded = false;
		}
		else
		{
...
void LLFloaterAvatarPicker::find()
{
	...
	if (!url.empty() && LLAvatarNameCache::useDisplayNames())
	{
		...
		gCoros.launch("LLFloaterAvatarPicker::findCoro",
					  std::bind(&LLFloaterAvatarPicker::findCoro, url,
								mQueryID));
	}
	else
	{
...
static void getSelectedAvatarData(const LLScrollListCtrl* listp,
								  ...
								  uuid_vec_t& avatar_ids)
{
	...
	for (size_t i = 0, count = items.size(); i < count; ++i)
	{
		...
		avatar_names.emplace_back(itemp->getColumn(0)->getValue().asString());
		avatar_ids.emplace_back(item_id);
	}
}
