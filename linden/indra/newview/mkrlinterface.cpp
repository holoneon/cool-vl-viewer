...
{
	static F32 last_refresh = 0.f;
	if (mNeedsVisibilityRefresh && gFrameTimeSeconds - last_refresh > 1.f)
	{
		handle_objects_visibility(NULL);
		last_refresh = gFrameTimeSeconds;
		mNeedsVisibilityRefresh = false;
	}
	...
	if (!mAssetsToReattach.empty())
	{
		...
		if (!mReattaching || timeout)
		{
			...
			if (elapsed >= reattach_delay)
			{
				...
				if (attachp)
				{
					tmp_attachpt_nb = findAttachmentPointNumber(attachp);
				}
...
bool RLInterface::isFolderLocked(LLInventoryCategory* catp)
{
	if (!catp) return false;

	const LLFolderType::EType folder_type = catp->getPreferredType();
	if (LLFolderType::lookupIsProtectedType(folder_type)) return false;

	bool shared = isUnderRlvShare(catp);
	if (!shared && contains("unsharedwear")) return true;
	if (shared && contains("sharedwear")) return true;

	if (isFolderLockedWithoutException(catp, "attach") != FolderNotLocked)
	{
		return true;
	}

	return isFolderLockedWithoutException(catp, "detach") != FolderNotLocked;
}

RLInterface::EFolderLock
RLInterface::isFolderLockedWithoutException(LLInventoryCategory* catp,
											std::string attach_or_detach)
{
	if (!catp) return FolderNotLocked;

	LL_DEBUGS("RestrainedLove") << "Category: " << catp->getName()
		 ...
		 it != end; ++it)
	{
			...
			parseCommand(command + "=n", behav, option, param))
		{
			...
			{
				if (getCategoryUnderRlvShare(option) == catp)
				{
			...
			{
				if (isUnderFolder(getCategoryUnderRlvShare(option), catp))
				{
					this_object_locks = true;
				}
			}
			...
			if (this_object_locks)
			{
				...
				EFolderLock this_lock =
					isFolderLockedWithoutExceptionAux(catp, attach_or_detach,
													  commands_list);
...
RLInterface::EFolderLock
RLInterface::isFolderLockedWithoutExceptionAux(LLInventoryCategory* catp,
											   std::string attach_or_detach,
											   std::deque<std::string> restrictions)
{
	...
	// For each folder from catp up to the root folder, check :
	// - if we are on catp and we find "{attach|detach}this_except", there is
	//   an exception, keep looking up
	// - if we are on catp and we find "{attach|detach}this", there is no
...
bool RLInterface::forceTeleport(const std::string& location, bool keep_lookat)
{
	...
	else
	{
		...
		LLWorldMap::url_callback_t cb = std::bind(&force_tp_callback, _1,
...
void RLInterface::removeWearableItemFromAvatar(LLViewerInventoryItem* item_or_link)
{
	...
	LLViewerInventoryItem* itemp = item_or_link;
	...
	wearable = gAgentWearables.getWearableFromItemID(itemp->getUUID());
	if (!wearable)
	{
		return;
	}
...
std::string RLInterface::getAttachments(const std::string& attachpt)
{
		 ...
		 iter != end; ++iter)
	{
		...
		{
			if (attachp->getNumObjects() > 0)
			{
				res += "1";		// attachp->getName();
			}
			else
			{
				res += "0";
...
bool RLInterface::forceDetach(const std::string& attachpt)
{
		 ...
		 iter != end; ++iter)
	{
		...
		if (attachpt.empty() || attachpt == name)
		{
			...
			detachAllObjectsFromAttachment(attachp);
			res = true;
		}
	}
	return res;
...
bool RLInterface::forceDetachByUuid(const std::string& object_id)
{
	...
	LLViewerObject* vobjp = gObjectList.findObject(LLUUID(object_id));
	if (vobjp)
	{
		vobjp = vobjp->getRootEdit();
		for (LLVOAvatar::attachment_map_t::iterator
				iter = gAgentAvatarp->mAttachmentPoints.begin(),
				end = gAgentAvatarp->mAttachmentPoints.end();
			 iter != end; ++iter)
		{
			LLViewerJointAttachment* attachp = iter->second;
			if (attachp && attachp->isObjectAttached(vobjp))
			{
				detachObject(vobjp);
				res = true;
			}
...
std::deque<LLInventoryItem*> RLInterface::getListOfLockedItems(LLInventoryCategory* rootp)
{
	std::deque<LLInventoryItem*> res;
	std::deque<LLInventoryItem*> tmp;

	if (rootp && isAgentAvatarValid())
	{
		LLInventoryModel::cat_array_t* cats;
		LLInventoryModel::item_array_t* items;
		gInventory.getDirectDescendentsOf(rootp->getUUID(), cats, items);
		...
		for (S32 i = 0, count = items->size(); i < count; ++i)
		{
			...
			if (itemp && itemp->getType() == LLAssetType::AT_OBJECT)
			{
				LLViewerObject* vobjp =
					gAgentAvatarp->getWornAttachment(itemp->getUUID());
				if (vobjp)
				{
					attach_point_name =
						gAgentAvatarp->getAttachedPointName(itemp->getLinkedUUID());
					if (!canDetach(vobjp))
					{
			...
			else if (itemp && itemp->getType() == LLAssetType::AT_CLOTHING)
			{
		...
		for (S32 i = 0, count = cats->size(); i < count; ++i)
		{
			LLInventoryCategory* catp = (*cats)[i];
			tmp = getListOfLockedItems(catp);
			for (S32 j = 0, count2 = tmp.size(); j < count2; ++j)
			{
				LLInventoryItem* itemp = tmp[j];
				if (itemp)
				{
					res.push_back(itemp);
				}
										  ...
										  bool with_worn_info)
{
	...
	LLInventoryModel::cat_array_t* cats;
	LLInventoryModel::item_array_t* items;
	LLInventoryCategory* rootp = NULL;
	if (path.empty())
	{
		rootp = getRlvShare();
	}
	...
	{
		rootp = getCategoryUnderRlvShare(path);
	}
	...
	if (rootp)
	{
		...
		if (cats)
		{
			...
			if (with_worn_info)
			{
				std::string worn_items = getWornItems(rootp);
				res += "|";
				found_one = true;
				if (worn_items == "n")
				{
			...
			{
				LLInventoryCategory* catp = (*cats)[i];
				const std::string& name = catp->getName();
				...
				{				// hidden folders => invisible to the list
					...
					if (with_worn_info)
					{
						std::string worn_items = getWornItems(catp);
						res += "|";
						found_one = true;
						if (worn_items == "n")
						{
...
std::string RLInterface::getWornItems(LLInventoryCategory* catp)
{
	...
	S32 nb_worn = 0;
	bool no_mod = false;
	bool is_rlv_root = getRlvShare() == catp;

	// if catp exists, scan all the items inside it
	if (catp)
	{
		LLInventoryModel::cat_array_t* cats;
		LLInventoryModel::item_array_t* items;
		...
		if (!is_rlv_root && items)	// Do not scan the shared root
		{
			...
			for (S32 i = 0, count = items->size(); i < count; ++i)
			{
				itemp = (LLViewerInventoryItem*)(*items)[i];
				if (itemp)
				{
					if (itemp->getType() == LLAssetType::AT_OBJECT ||
						itemp->getType() == LLAssetType::AT_CLOTHING ||
						itemp->getType() == LLAssetType::AT_BODYPART)
					{
						++nb_items;
					}
					...
					// number
					if (count == 1 &&
						itemp->getType() == LLAssetType::AT_OBJECT &&
						!itemp->getPermissions().allowModifyBy(gAgentID))
					{
						if (findAttachmentPointFromName(catp->getName()))
						{
							no_mod = true;
						}
					}
...
bool RLInterface::isUnderRlvShare(LLInventoryItem* itemp)
{
	if (!itemp) return false;
	const LLUUID& cat_id = itemp->getParentUUID();
	return isUnderFolder(getRlvShare(), gInventory.getCategory(cat_id));
}

bool RLInterface::isUnderRlvShare(LLInventoryCategory* catp)
{
	return isUnderFolder(getRlvShare(), catp);
}
														   ...
														   LLInventoryCategory* rootp)
{
	...
	// it
	LLInventoryCategory* catp = (*cats)[max_size_index];
	...
	return getCategoryUnderRlvShare(subcat, catp);
}
	...
	if (cats)
	{
		...
		for (S32 i = 0, count = cats->size(); i < count; ++i)
		{
			...
			if (!name.empty() && name[0] != '.' && name[0] != '~')
			{
				...
				found = findCategoryUnderRlvShare(cat_name, catp);
				if (found)
				{
					return found;
				}
																		  ...
																		  LLInventoryCategory* rootp)
{
	...
	if (cats)
	{
		...
		for (S32 i = 0, count = cats->size(); i < count; ++i)
		{
			...
			{
				// Search recursively deeper
				found = findCategoriesUnderRlvShare(cat_name, catp);
		...
		if (attachp)
		{
			...
			else
			{
					...
					obj_name.substr(ind).find(')') != std::string::npos)
				{
					...
					new_candidate.attachment = attachp;
...
LLViewerJointAttachment* RLInterface::findAttachmentPointFromParentName(LLInventoryItem* itemp)
{
	if (itemp)
	{
		...
		LLViewerInventoryCategory* catp = gInventory.getCategory(parent_id);
		if (catp)
		{
			return findAttachmentPointFromName(catp->getName());
		}
	}
	return NULL;
}

S32 RLInterface::findAttachmentPointNumber(LLViewerJointAttachment* attachp)
{
	if (isAgentAvatarValid())
	{
		for (LLVOAvatar::attachment_map_t::iterator
...
// Helper class to allow and force a re-fetch of the parent folder contents,
// where a sub-folder was created to move a no-mod item inside, so to get rid
// of the ghost item left behind after the move (seen happening in SL with
// AISv3). HB
class RLNewCatCreationCB final : public LLInventoryCallback
{
protected:
	LOG_CLASS(RLNewCatCreationCB);

public:
	RLNewCatCreationCB(const LLUUID& parent_id)
	:	mParentId(parent_id)
	{
	}
	...
	{
		AISAPI::fetchCategoryChildren(mParentId);
	}
...
void RLInterface::addAttachmentPointName(LLViewerObject* vobj)
{
	...
	if (!itemp || !itemp->isFinished() || !isUnderRlvShare(itemp) ||
		findAttachmentPointFromName(itemp->getName()))
	{
		// Nothing to do.
		return;
	}
	...
	else
	{
		...
		{
			cb = new RLNewCatCreationCB(parent_id);
		}
		move_inventory_item(item_id, cat_id, itemp->getName(), cb);
	}
}

// Handles the detach message to the sim here, after a check
void RLInterface::detachObject(LLViewerObject* vobjp)
{
	if (vobjp && (!gRLenabled || canDetach(vobjp)))
	{
		LLMessageSystem* msg = gMessageSystemp;
		...
		msg->addU32Fast(_PREHASH_ObjectLocalID, vobjp->getLocalID());
		msg->sendReliable(gAgent.getRegionHost());
	}
}

void RLInterface::detachAllObjectsFromAttachment(LLViewerJointAttachment* attachp)
{
	if (!attachp) return;

	// We need to remove all the objects from attachp->mAttachedObjects, one
	...
	LLViewerJointAttachment::attachedobjs_vec_t attached_objects =
		attachp->mAttachedObjects;

	for (S32 i = 0, count = attached_objects.size(); i < count; ++i)
	{
		LLViewerObject* vobjp = attached_objects[i];
		detachObject(vobjp);
	}
}

bool RLInterface::canDetachAllObjectsFromAttachment(LLViewerJointAttachment* attachp)
{
	if (!attachp) return false;

	for (U32 i = 0; i < attachp->mAttachedObjects.size(); ++i)
	{
		LLViewerObject* vobjp = attachp->mAttachedObjects[i];
		if (!canDetach(vobjp))
		{
			return false;
		}
...
void RLInterface::fetchInventory(LLInventoryCategory* rootp)
{
	...
	if (rootp)
	{
		LLViewerInventoryCategory* vrootp = (LLViewerInventoryCategory*)rootp;
		vrootp->fetch();
		...
		gInventory.getDirectDescendentsOf(vrootp->getUUID(), cats, items);
		if (cats)
		{
			for (S32 i = 0, count = cats->size(); i < count; ++i)
			{
				LLInventoryCategory* catp = (LLInventoryCategory*)(*cats)[i];
				fetchInventory(catp);
			}
		}
	}
							  ...
							  EAttachMethod how)
{
	...
	LLInventoryCategory* catp = getCategoryUnderRlvShare(category);
	if (!catp)
	{
	...
	if (!is_rlv_root && items)
	{
		...
		for (S32 i = 0, count = items->size(); i < count; ++i)
		{
			LLViewerInventoryItem* itemp = (LLViewerInventoryItem*)(*items)[i];
			...
			if (itemp && itemp->getType() == LLAssetType::AT_OBJECT)
			{
				...
				if (attachp)
				{
					...
					if (replacing)
					{
						...
						// confirmation dialog
						S32 number = findAttachmentPointNumber(attachp);
						if (canDetach(attachp->getName()) && canAttach(itemp))
						{
							attachObjectByUUID(itemp->getLinkedUUID(), number,
											   true);
						}
					}
					else
					{
						...
						gAppearanceMgr.rezAttachment(itemp, attachp, false);
					}
				}
				else
				{
			...
			else if (itemp->getType() == LLAssetType::AT_CLOTHING ||
					 itemp->getType() == LLAssetType::AT_BODYPART)
			{
				gAppearanceMgr.wearInventoryItemOnAvatar(itemp, replacing);
			}
			// This is a gesture: activate
			else if (itemp->getType() == LLAssetType::AT_GESTURE)
			{
				if (!gGestureManager.isGestureActive(itemp->getLinkedUUID()))
				{
					gGestureManager.activateGesture(itemp->getLinkedUUID());
				}
			}
		...
		if (!is_rlv_root && attachp)
		{
			...
			if (subcatitems && subcatitems->size() == 1)
			{
				...
				if (itemp &&
					itemp->getType() == LLAssetType::AT_OBJECT &&
					!itemp->getPermissions().allowModifyBy(gAgentID) &&
					findAttachmentPointFromParentName(itemp))
				{
		...
		for (S32 i = 0, count = items->size(); i < count; ++i)
		{
			...
			if (itemp->getType() == LLAssetType::AT_OBJECT)
			{
					 ...
					 iter != end; ++iter)
				{
					...
					LLViewerObject* vobjp =
						gAgentAvatarp->getWornAttachment(itemp->getUUID());
					if (vobjp && attachp && attachp->isObjectAttached(vobjp))
					{
						detachObject(vobjp);
						break;
					}
			...
			else if (itemp->getType() == LLAssetType::AT_CLOTHING)
			{
				...
				if (canDetach(itemp))
				{
					removeWearableItemFromAvatar(itemp);
				}
			}
			else if (itemp->getType() == LLAssetType::AT_GESTURE)
			{
				...
				if (gGestureManager.isGestureActive(itemp->getLinkedUUID()))
				{
					gGestureManager.deactivateGesture(itemp->getLinkedUUID());
				}
			}
				...
				findAttachmentPointFromParentName(itemp))
			{
					 ...
					 iter != end; ++iter)
				{
					...
					LLViewerObject* vobjp =
						gAgentAvatarp->getWornAttachment(itemp->getUUID());
					if (vobjp && attachp &&
						attachp->isObjectAttached(vobjp))
					{
						detachObject(vobjp);
						break;
					}
...
std::string RLInterface::getCensoredMessage(std::string str)
{
	...
	for (U32 i = 0, count = avatar_ids.size(); i < count; ++i)
	{
		...
		if (gCacheNamep && gCacheNamep->getFullName(avatar_id, name))
		{
			...
			str = stringReplace(str, name, dummy_name, true);
			size_t j = name.find(" Resident");
			if (j > 0)
			{
				name = name.substr(0, j);
				// Legacy name, without " Resident", case insensitive
				str = stringReplace(str, name, dummy_name, true);
			}
		}
...
std::string RLInterface::getFullPath(LLInventoryCategory* catp)
{
	if (!catp) return "";
...
std::string RLInterface::getFullPath(LLInventoryItem* itemp,
									 const std::string& option, bool full_list)
{
	LL_DEBUGS("RestrainedLove") << "Item: "
								<< (itemp ? itemp->getName() : "NULL")
	...
	if (!option.empty())
	{
		...
		if (LLUUID::validate(option))
		{
			...
			if (id.notNull())
			{
				...
				// object
				itemp = getItem(id);
				if (itemp && isUnderRlvShare(itemp))
				{
					...
					std::deque<std::string> res;
					res.emplace_back(getFullPath(itemp, ""));
		...
		if (attachp)
		{
			...
			{
				LLViewerObject* vobjp =
					attachp->mAttachedObjects[i];
				if (vobjp)
				{
					itemp = getItemAux(vobjp, getRlvShare());
					if (itemp && !isUnderRlvShare(itemp))
					{
	...
	LLInventoryCategory* parentp = gInventory.getCategory(parent_id);
		...
		!itemp->getPermissions().allowModifyBy(gAgentID))
	{
		if (findAttachmentPointFromName(parentp->getName()))
		{
			...
			parent_id = parentp->getParentUUID();
			parentp = gInventory.getCategory(parent_id);
			return getFullPath(parentp);
		}
	}
...
LLInventoryItem* RLInterface::getItemAux(LLViewerObject* vobjp,
										 LLInventoryCategory* rootp)
{
	if (vobjp && rootp && isAgentAvatarValid())
	{
		LLInventoryModel::cat_array_t* cats;
		LLInventoryModel::item_array_t* items;
		gInventory.getDirectDescendentsOf(rootp->getUUID(), cats, items);
		...
		for (S32 i = 0, count = items->size(); i < count; ++i)
		{
			...
			if (itemp && !itemp->getIsLinkType() &&
				(itemp->getType() == LLAssetType::AT_OBJECT ||
				 itemp->getType() == LLAssetType::AT_CLOTHING) &&
				gAgentAvatarp->getWornAttachment(itemp->getUUID()) == vobjp)
			{
				// Found the itemp in the current category
				return itemp;
			}
		}
		...
		{
			LLInventoryCategory* catp = (*cats)[i];
			LLInventoryItem* itemp = getItemAux(vobjp, catp);
			if (itemp)
			{
				return itemp;
			}
...
{
	LLViewerObject* vobjp = gObjectList.findObject(worn_object_id);
	if (vobjp)
	{
		vobjp = vobjp->getRootEdit();
		if (vobjp->isAttachment())
		{
			return gInventory.getItem(vobjp->getAttachmentItemID());
		}
	}
...
bool RLInterface::canDetachAllSelectedObjects()
{
	for (LLObjectSelection::iterator iter = gSelectMgr.getSelection()->begin(),
		 ...
		 iter != end; ++iter)
	{
		LLViewerObject* vobjp = (*iter)->getObject();
		if (vobjp && !canDetach(vobjp))
		{
			return false;
		}
	}
...
bool RLInterface::canAttachCategory(LLInventoryCategory* catp,
									bool with_exceptions)
{
	if (!catp || !isAgentAvatarValid()) return true;
#if RL_ALLOW_ATTACH_DETACH_RECENTLY_RECEIVED_ITEMS
	if (isInventoryFolderNew(catp)) return true;
#endif
	bool shared = isUnderRlvShare(catp);
	...
	return canAttachCategoryAux(catp, false, false, with_exceptions);
}

bool RLInterface::canAttachCategoryAux(LLInventoryCategory* catp,
									   ...
									   bool with_exceptions)
{
	...
	if (catp)
	{
		// Check @attachthis:catp in all restrictions
		...
		folder_lock = isFolderLockedWithoutException(catp, "attach");
		...
		for (S32 i = 0, count = items->size(); i < count; ++i)
		{
			...
			if (itemp)
			{
				if (itemp->getType() == LLAssetType::AT_OBJECT)
				{
					LLViewerJointAttachment* attachp = NULL;
					if (in_no_mod)
					{
						if (count > 1 ||
							itemp->getPermissions().allowModifyBy(gAgentID))
						...
						LLInventoryCategory* parentp =
							gInventory.getCategory(catp->getParentUUID());
						attachp =
							findAttachmentPointFromName(parentp->getName());
					}
					else
					{
						attachp = findAttachmentPointFromName(itemp->getName());
					}
					if (attachp &&
						contains(restriction + ":" + attachp->getName()))
					{
						return false;
					}
				}
				...
				{
					LLViewerWearable* wearable =
						gAgentWearables.getWearableFromItemID(itemp->getLinkedUUID());
					if (wearable &&
		...
		for (S32 i = 0, count = cats->size(); i < count; ++i)
		{
			...
			if (catp)
			{
				...
				{
					if (!canAttachCategoryAux(catp, false, true,
											  with_exceptions))
					{
						return false;
					}
	...
	if (catp == getRlvShare()) return true;

	if (!in_no_mod && folder_lock == FolderNotLocked)
	{
		// Check for @attachallthis in the parent
		return canAttachCategoryAux(gInventory.getCategory(catp->getParentUUID()),
...
bool RLInterface::canDetachCategory(LLInventoryCategory* catp,
									bool with_exceptions)
{
	if (!catp || !isAgentAvatarValid()) return true;
	...
	return canDetachCategoryAux(catp, false, false, with_exceptions);
}

bool RLInterface::canDetachCategoryAux(LLInventoryCategory* catp,
									   bool in_parent, bool in_no_mod,
									   bool with_exceptions)
{
	if (!isAgentAvatarValid()) return true;
	...
	if (catp)
	{
		// check @detachthis:catp in all restrictions
		std::string path_to_check;
		...
		folder_lock = isFolderLockedWithoutException(catp, "detach");
		if (folder_lock == FolderLockedNoException)
		{
			return false;
		}
		...
		while (it != mSpecialObjectBehaviours.end())
		{
			if (it->second.find(restriction + ":") == 0)
			{
				...
				path_to_check = it->second.substr(restriction.length()+1);
				restricted_cat = getCategoryUnderRlvShare(path_to_check);
				if (restricted_cat == catp) return false;
			}
			++it;
		}
#endif
		...
		for (S32 i = 0, count = items->size(); i < count; ++i)
		{
			...
			if (itemp)
			{
				if (itemp->getType() == LLAssetType::AT_OBJECT)
				{
					...
					LLViewerObject* vobjp =
						gAgentAvatarp->getWornAttachment(itemp->getLinkedUUID());
					if (vobjp)
					{
						if (!isAllowed(vobjp->getRootEdit()->getID(),
									   restriction))
						{
							return false;
						}
						if (!in_parent &&
							!isAllowed(vobjp->getRootEdit()->getID(),
									   "detachallthis"))
						{
		...
		for (S32 i = 0, count = cats->size(); i < count; ++i)
		{
			...
			if (catp)
			{
				...
				if (!name.empty() && name[0] == '.' &&
					findAttachmentPointFromName(name))
				{
					if (!canDetachCategoryAux(catp, false, true))
					{
						return false;
					}
				}
	...
	if (!in_no_mod && folder_lock == FolderNotLocked)
	{
		...
		return canDetachCategoryAux(gInventory.getCategory(catp->getParentUUID()),
									true, false, with_exceptions);
	}
...
bool RLInterface::canUnwear(LLViewerInventoryItem* itemp)
{
	if (itemp && !isRestoringOutfit())
	{
		if (itemp->getType() == LLAssetType::AT_OBJECT)
		{
			return canDetach(itemp);
		}
		if (itemp->getType() == LLAssetType::AT_CLOTHING ||
			itemp->getType() == LLAssetType::AT_BODYPART)
		{
			if (!canUnwear(itemp->getWearableType()))
			{
				return false;
			}

			LLInventoryCategory* parentp =
				gInventory.getCategory(itemp->getParentUUID());
			if (!canDetachCategory(parentp))
			{
				return false;
			}
		}
...
bool RLInterface::canWear(LLViewerInventoryItem* itemp)
{
	if (itemp && !isRestoringOutfit())
	{
#if RL_ALLOW_ATTACH_DETACH_RECENTLY_RECEIVED_ITEMS
		if (isInventoryItemNew(itemp)) return true;
#endif
		LLInventoryCategory* parentp =
			gInventory.getCategory(itemp->getParentUUID());
		if (itemp->getType() == LLAssetType::AT_OBJECT)
		{
			LLViewerJointAttachment* attachp =
				findAttachmentPointFromName(itemp->getName());
			if (attachp && !canAttach(NULL, attachp->getName()))
			{
				return false;
			}
			return canAttachCategory(parentp);
		}
		if (itemp->getType() == LLAssetType::AT_CLOTHING ||
			itemp->getType() == LLAssetType::AT_BODYPART)
		{
			if (!canWear(itemp->getWearableType()) ||
				!canAttachCategory(parentp))
			{
				return false;
			}
		}
...
bool RLInterface::canDetach(LLViewerInventoryItem* itemp)
{
	if (!itemp || isRestoringOutfit()) return true;
	...
	if (itemp->getType() == LLAssetType::AT_OBJECT)
	{
		...
		return canDetach(gAgentAvatarp->getWornAttachment(itemp->getLinkedUUID()));
	}
	else if (itemp->getType() == LLAssetType::AT_CLOTHING)
	{
		LLInventoryCategory* parentp =
		...
		const LLViewerWearable* wearable =
			gAgentWearables.getWearableFromItemID(itemp->getUUID());
		if (wearable)
		{
...
bool RLInterface::canDetach(LLViewerObject* vobjp)
{
	if (!vobjp || isRestoringOutfit()) return true;

	LLViewerObject* rootp = vobjp->getRootEdit();
	if (!rootp) return true;

	// Check all the current restrictions, if "detach" is issued from a child
	// prim of the root prim of vobjp, then the whole object is
	// undetachable
	for (rl_map_it_t it = mSpecialObjectBehaviours.begin(),
					 end = mSpecialObjectBehaviours.end();
		 it != end; ++it)
	{
		if (it->second == "detach")
		{
			...
			if (this_prim && this_prim->getRootEdit() == rootp)
			{
				return false;
			}
		}
	...
	const LLUUID& obj_id = vobjp->getID();
	...
	LLInventoryItem* itemp = getItem(rootp->getID());
	if (itemp)
	{
		if (mHandleNoStrip)
		{
...
bool RLInterface::canDetach(std::string attachpt)
{
	...
	if (contains("remattach")) return false;
	if (contains("remattach:" + attachpt)) return false;
	LLViewerJointAttachment* attachp = findAttachmentPointFromName(attachpt,
																   true);
	return canDetachAllObjectsFromAttachment(attachp);
}
...
bool RLInterface::canAttach(LLViewerInventoryItem* itemp)
{
	if (!itemp || isRestoringOutfit()) return true;
	...
	LLViewerJointAttachment* attachp =
		findAttachmentPointFromName(itemp->getName());
	if (attachp && contains("addattach:" + attachp->getName()))
	{
		return false;
	}
...
bool RLInterface::canEdit(LLViewerObject* vobjp)
{
	if (!vobjp) return false;
	...
	if (containsWithoutException("edit", rootp->getID().asString()))
	{
		return false;
	}

	bool is_attachment = vobjp->isAttachment();
	...
	if (contains("editobj:" + rootp->getID().asString()))
	{
		return false;
	}

	return !mContainsInteract || vobjp->isHUDAttachment();
}

bool RLInterface::canTouch(LLViewerObject* vobjp,
						   LLVector3 pick_intersection)
{
	...
	// can touch it
	if (!isAllowed(rootp->getID(), "touchme"))
	{
		return true;
	}

	bool is_hud = rootp->isHUDAttachment();
	...
	if (is_hud &&
		containsWithoutException("touchhud",
								 vobjp->getRootEdit()->getID().asString()))
	{
		return false;
	}

	if (contains("touchthis:" + rootp->getID().asString()))
	{
		return false;
	}

	if (!canTouchFar(vobjp, pick_intersection))
	{
		return false;
	}
	...
	if (rootp->isAttachment())
	{
		if (!is_hud)
		{
			...
			LLInventoryItem* inv_item = getItem(rootp->getID());
			if (inv_item)
			{
	...
	else if (containsWithoutException("touchworld", rootp->getID().asString()))
	{
		return false;
	}
...
bool RLInterface::canTouchFar(LLViewerObject* vobjp,
							  LLVector3 pick_intersection)
{
	if (!vobjp || vobjp->isHUDAttachment()) return true;

	if (mContainsInteract) return false;

	LLVector3 pos = vobjp->getPositionRegion();
...
#if RL_ALLOW_ATTACH_DETACH_RECENTLY_RECEIVED_ITEMS
bool RLInterface::isInventoryFolderNew(LLInventoryCategory* catp)
{
	return catp && mReceivedInventoryFolders.count(catp->getName()) != 0;
}

bool RLInterface::isInventoryItemNew(LLInventoryItem* itemp)
{
	if (!itemp) return false;
...
S32 RLInterface::avatarVisibility(LLVOAvatar* avatarp)
{
	...
	if ((mShowavsDistMax == EXTREMUM && mCamDistDrawMax == EXTREMUM) ||
		avatarp->isSelf() || avatarp->isUIAvatar())
	{
		return 1;
	}
