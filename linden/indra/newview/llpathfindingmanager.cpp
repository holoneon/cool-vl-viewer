													  ...
													  bool get_status_only)
{
	...
	if (!region->capabilitiesReceived())
	{
		...
		region->setCapsReceivedCB(std::bind(&LLPathfindingManager::handleDeferredGetNavMeshForRegion,
	...
	gCoros.launch("LLPathfindingManager::navMeshStatusRequestCoro",
				  std::bind(&LLPathfindingManager::navMeshStatusRequestCoro,
											  ...
											  object_request_cb_t cb) const
{
	...
	if (!region->capabilitiesReceived())
	{
		...
		region->setCapsReceivedCB(std::bind(&LLPathfindingManager::handleDeferredGetLinksetsForRegion,
											this, _1, req_id, cb));
		return;
	}
	...
	gCoros.launch("LLPathfindingManager::linksetObjectsCoro",
				  std::bind(&LLPathfindingManager::linksetObjectsCoro, this,
							object_url, responder, LLSD()));
	if (with_terrain)
	{
		gCoros.launch("LLPathfindingManager::linksetTerrainCoro",
					  std::bind(&LLPathfindingManager::linksetTerrainCoro,
											  ...
											  object_request_cb_t cb) const
{
	...
	{
		gCoros.launch("LLPathfindingManager::linksetObjectsCoro",
					  std::bind(&LLPathfindingManager::linksetObjectsCoro,
												...
												object_request_cb_t cb) const
{
	...
	if (!region->capabilitiesReceived())
	{
		...
		region->setCapsReceivedCB(std::bind(&LLPathfindingManager::handleDeferredGetCharactersForRegion,
											this, _1, req_id, cb));
		return;
	}
	...
	gCoros.launch("LLPathfindingManager::charactersCoro",
				  std::bind(&LLPathfindingManager::charactersCoro, this,
							char_url, req_id, cb));
}
...
void LLPathfindingManager::requestGetAgentState()
{
	...
	if (!region->capabilitiesReceived())
	{
		region->setCapsReceivedCB(std::bind(&LLPathfindingManager::handleDeferredGetAgentStateForRegion,
											this, _1));
		return;
	}
	...
	gCoros.launch("LLPathfindingManager::navAgentStateRequestCoro",
				  std::bind(&LLPathfindingManager::navAgentStateRequestCoro,
							this, agent_url));
}
...
void LLPathfindingManager::requestRebakeNavMesh(rebake_navmesh_cb_t cb)
{
	...
	gCoros.launch("LLPathfindingManager::navMeshRebakeCoro",
				  std::bind(&LLPathfindingManager::navMeshRebakeCoro, this,
							status_url, cb));
}
