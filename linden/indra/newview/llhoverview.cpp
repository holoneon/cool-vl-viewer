...
{
	LLViewerObject* hit_objp = getLastHoverObject();
	if (hit_objp && hit_objp == mLastObjectWithFullText)
	{
	...
	if (hit_objp)
	{
		mLastParcelWithFullText = NULL;
		mText.clear();
		if (hit_objp->isHUDAttachment())
		{
			// No hover tips for HUD elements, since they can obscure what the
			// HUD is displaying
			mLastObjectWithFullText = hit_objp;
			return;
		}

		if (hit_objp->isAttachment())
		{
			// Get root of attachment then parent, which is avatar
			LLViewerObject* rootp = hit_objp->getRootEdit();
			hit_objp = (LLViewerObject*)rootp->getParent();
			if (!hit_objp)
			{
				// Strange parenting issue, bail out
				return;
			}
		}
		...
		{
			LLNameValue* title = hit_objp->getNVPair("Title");
			LLNameValue* firstname = hit_objp->getNVPair("FirstName");
			LLNameValue* lastname =  hit_objp->getNVPair("LastName");
			if (firstname && lastname)
			{
				...
				if (LLAvatarNameCache::useDisplayNames())
				{
					...
					if (LLAvatarNameCache::get(hit_objp->getID(),
											   &avatar_name))
					{
...
//mk
				...
				LLViewerObject* object = hit_objp;
