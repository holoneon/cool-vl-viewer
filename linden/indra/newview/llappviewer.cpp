 ...
 * Copyright (c) 2009-2026, Henri Beauchamp
 *
...
#ifndef CEF_VERSION
# include "cef/dullahan_version.h"	// For CHROME_VERSION_MAJOR
#endif
...
#include "llwindow.h"
#if LL_WINDOWS
# include "llwindowwindows.h"		// For gIgnoreHiDPIEvents
#elif LL_LINUX
# include "llwindowlinux.h"			// For gUseFullDesktop
#endif
...
public:
	// Configures the command line parser according the given config file.
	// 'config_filename' is the name of the XML based LLSD config file.
	// 'clp' is a reference to the command line parser object to configure.
	void configure(const std::string& config_filename, LLControlGroup* group);

private:
	static void setControlValueCB(const strings_vec_t& value,
								  const std::string& opt_name,
								  LLControlGroup* ctrl_group);
};

//static
void LLControlGroupCLP::setControlValueCB(const strings_vec_t& value,
										  const std::string& opt_name,
										  LLControlGroup* ctrl_group)
{
								  ...
								  LLControlGroup* ctrl_group)
{
		 ...
		 ++it)
	{
		...
		std::function<void(const strings_vec_t&)> callback;
		if (ctrl_group && option_params.has("map-to"))
		{
			std::string ctrl_name = option_params["map-to"].asString();
			callback = std::bind(&LLControlGroupCLP::setControlValueCB, _1,
...
{
	LLControlVariable* ctrlp = NULL;

#if !LL_FAST_TIMERS_ENABLED
	ctrlp = gSavedSettings.getControl("FastTimersAlwaysEnabled");
	if (ctrlp)
	{
		ctrlp->setHiddenFromUser(true);
	}
#endif

#if LL_PENDING_MESH_REQUEST_SORTING
	ctrlp = gSavedSettings.getControl("DelayPendingMeshFetchesOnTP");
	if (ctrlp)
	{
		ctrlp->setHiddenFromUser(true);
	}
...
#if !HB_DEBUG_OBJECT
	ctrlp = gSavedSettings.getControl("DebugObjectId");
	if (ctrlp)
	{
		ctrlp->setHiddenFromUser(true);
	}
	ctrlp = gSavedSettings.getControl("DebugEditedObject");
	if (ctrlp)
	{
		ctrlp->setHiddenFromUser(true);
	}
...
#if !LL_PUPPETRY || !LL_USE_NEW_DESERIALIZE
	ctrlp = gSavedSettings.getControl("PuppetryBinaryInputStream");
	if (ctrlp)
	{
		ctrlp->setHiddenFromUser(true);
	}
...
#if !LL_PUPPETRY
	ctrlp = gSavedSettings.getControl("PuppetryAllowed");
	if (ctrlp)
	{
		ctrlp->setHiddenFromUser(true);
	}
	...
	ctrlp = gSavedSettings.getControl("PuppetryBinaryOutputStream");
	if (ctrlp)
	{
		ctrlp->setHiddenFromUser(true);
	}
	ctrlp = gSavedSettings.getControl("PuppetryCamera");
	if (ctrlp)
	{
		ctrlp->setHiddenFromUser(true);
	}
	ctrlp = gSavedSettings.getControl("PuppetryCameraOption");
	if (ctrlp)
	{
		ctrlp->setHiddenFromUser(true);
	}
	ctrlp = gSavedSettings.getControl("PuppetryLastCommand");
	if (ctrlp)
	{
		ctrlp->setHiddenFromUser(true);
	}
	ctrlp = gSavedSettings.getControl("PuppetryParts");
	if (ctrlp)
	{
		ctrlp->setHiddenFromUser(true);
	}
	ctrlp = gSavedSettings.getControl("PuppetrySendAttachmentsData");
	if (ctrlp)
	{
		ctrlp->setHiddenFromUser(true);
	}
	ctrlp = gSavedSettings.getControl("PuppetryUseServerEcho");
	if (ctrlp)
	{
		ctrlp->setHiddenFromUser(true);
	}
	...
	ctrlp = gSavedSettings.getControl("RenderHiDPI");
	if (ctrlp)
	{
		ctrlp->setHiddenFromUser(true);
	}
#else
	// Not (yet ?) used under Windows.
	ctrlp = gSavedSettings.getControl("FullDesktop");
	if (ctrlp)
	{
		ctrlp->setHiddenFromUser(true);
	}
	// No D-Bus under Windows.
	ctrlp = gSavedPerAccountSettings.getControl("LuaAcceptDbusCommands");
	if (ctrlp)
	{
		ctrlp->setHiddenFromUser(true);
	}
...
#if LL_WINDOWS
	ctrlp = gSavedSettings.getControl("ShowConsoleWindow");
	if (ctrlp)
	{
		ctrlp->setHiddenFromUser(false);
	}
	ctrlp = gSavedSettings.getControl("IgnoreHiDPIEvents");
	if (ctrlp)
	{
		ctrlp->setHiddenFromUser(false);
	}
	ctrlp = gSavedSettings.getControl("MediaStreamsUseVLC");
	if (ctrlp)
	{
		ctrlp->setHiddenFromUser(false);
	}
...
#if !LL_LINUX || !LL_FMOD
	ctrlp = gSavedSettings.getControl("FMODDisableALSA");
	if (ctrlp)
	{
		ctrlp->setHiddenFromUser(true);
	}
	ctrlp = gSavedSettings.getControl("FMODDisablePulseAudio");
	if (ctrlp)
	{
		ctrlp->setHiddenFromUser(true);
	}
#endif
#if !LL_FMOD
	ctrlp = gSavedSettings.getControl("AudioDisableFMOD");
	if (ctrlp)
	{
		ctrlp->setHiddenFromUser(true);
	}
#endif
#if !LL_OPENAL
	ctrlp = gSavedSettings.getControl("AudioDisableOpenAL");
	if (ctrlp)
	{
		ctrlp->setHiddenFromUser(true);
	}
	...
	ctrlp = gSavedSettings.getControl("CEFPreferredFont");
	if (ctrlp)
	{
		ctrlp->setHiddenFromUser(true);
	}
	ctrlp = gSavedSettings.getControl("CEFMinimumFontSize");
	if (ctrlp)
	{
		ctrlp->setHiddenFromUser(true);
	}
	ctrlp = gSavedSettings.getControl("CEFDefaultFontSize");
	if (ctrlp)
	{
		ctrlp->setHiddenFromUser(true);
	}
	ctrlp = gSavedSettings.getControl("CEFRemoteFonts");
	if (ctrlp)
	{
		ctrlp->setHiddenFromUser(true);
	}
	...
	std::string plugin_file = gDirUtil.getLLPluginFilename("media_plugin_cef");
	gHasCEF = LLFile::isfile(plugin_file);
	if (!gHasCEF)
	{
		llwarns << "No web browser plugin found !" << llendl;
		gSavedSettings.setBool("MediaStreamsUseCEF", false);
	}

	plugin_file = gDirUtil.getLLPluginFilename("media_plugin_gstreamer");
	gHasGstreamer = LLFile::isfile(plugin_file);
	plugin_file = gDirUtil.getLLPluginFilename("media_plugin_vlc");
	gHasVLC = LLFile::isfile(plugin_file);
	if (!gHasGstreamer && !gHasVLC)
	{
		llwarns << "No streaming media plugin found !" << llendl;
		if (gHasCEF)
		{
			llinfos << "Will try and use the CEF plugin to play media streams..."
					<< llendl;
		}
		gSavedSettings.setBool("MediaStreamsUseCEF", true);
	}

	plugin_file = gDirUtil.getLLPluginFilename("voice_plugin_webrtc");
	gHasWebRTC = LLFile::isfile(plugin_file);
	if (!gHasWebRTC)
	{
		llwarns << "No WebRTC voice plugin found !" << llendl;		
	}
}
...
static void settings_to_globals()
{
	...
	LLSettingsSky::sAutoAdjustProbeAmbiance =
		gSavedSettings.getF32("RenderSkyAutoAdjustProbeAmbiance");
	...
	LLFontGL::setColorUse(gSavedSettings.getBool("AllowColoredFonts"));
	LLFontVertexBuffer::enable(gSavedSettings.getBool("UseFontVertexBuffers"));
...
LLApp::InitState LLAppViewer::init()
{
	...
	std::string colors_base_filename =
		gDirUtil.findSkinnedFilename("colors_base.xml");
	...
	std::string user_colors_filename =
		gDirUtil.findSkinnedFilename("colors.xml");
	...
	// calculated in LLProcessorInfo(). Also must be done *after* the call to
	// settings_to_globals(). HB
	initThreads();

	// Workaround for Wine bug. HB
#if LL_WINDOWS
	if (gAppViewerp->isRunningUnderWine())
	{
		std::string test_file = gDirUtil.getFullPath(LL_PATH_LOGS, "test.txt");
		// Note: has_wine_append_file_write_bug() sets LLFile::sFlushOnWrite to
		// true when the Wine bug is detected. HB
		if (has_wine_append_file_write_bug(test_file))
		{
...
//mk
	...
	LLGLManager::getGLInfo(gDebugInfo);
	LLGLManager::printGLInfoString();

	// Load key settings
	bind_keyboard_functions();

	// Load Default bindings
	if (!gViewerKeyboard.loadBindings(gDirUtil.getFullPath(LL_PATH_APP_SETTINGS,
														   "keys.ini")))
	{
		llerrs << "Unable to open keys.ini" << llendl;
	}
	// Load Custom bindings (override defaults)
	gViewerKeyboard.loadBindings(gDirUtil.getFullPath(LL_PATH_APP_SETTINGS,
													  "custom_keys.ini"));
	...
	// Register the Lua UI command registration function. HB
	LLUICtrl::setRegisterLuaCommandFunc(register_ui_lua_command);
	// Try and start the automation script, if any. HB
...
void LLAppViewer::checkMemory()
{
	...
	constexpr F32 MEMORY_TRIM_INTERVAL = 60.f;	// In seconds
	constexpr F32 MEMORY_CHECK_INTERVAL = 15.f;	// In seconds
	static U32 checks_counter = 0;
	static LLCachedControl<bool> check(gSavedSettings, "MinFreePhysMemCheck");
	static LLCachedControl<U32> min_mem_mb(gSavedSettings, "MinFreePhysMemMB");
	static LLCachedControl<U32> mem_pct(gSavedSettings, "MinFreePhysMemPct");
	bool need_limits_check = false;
	if (!gMemoryCheckTimer.getStarted() || gMemoryCheckTimer.hasExpired())
	{
		checks_counter = 0;
		need_limits_check = true;
		gMemoryCheckTimer.start();
		gMemoryCheckTimer.setTimerExpirySec(MEMORY_TRIM_INTERVAL);
		// Update memory info after trimming the heap when possible/supported.
		LLMemory::updateMemoryInfo(true);
	}
	// When the user wants to watch the memory usage, check for free memory
	// every MEMORY_CHECK_INTERVAL, but without trimming (which is costlier):
	// in case of low memory condition, gMemoryCheckTimer will be expired in
	// LLViewerTexture::updateClass() after the texture discard bias will have
	// been increased, and a trimming will ensue at next frame as a result. HB
	else if (check)
	{
		F32 elapsed = gMemoryCheckTimer.getElapsedTimeF32();
		if (U32(elapsed / MEMORY_CHECK_INTERVAL) > checks_counter)
		{
			++checks_counter;
			need_limits_check = true;
	...
	if (!check)
	{
		...
		{
			reported = 0;
			llinfos << "Not checking any more for minimum free physical memory."
					<< llendl;
		}
		gLowMemory = false;
		return;
	}
	// Note: this algorithm (or rather the calculation implemented in
	// LLMemory::getAvailablePhysicalMemKB()) is too naive to be 100% reliable:
	// it may cause "low memory" false positives due to how some other programs
	// running along the viewer do not release to the OS the memory they
	// "freed" (they do not have "heap trimming" like we do) and instead keep
	// it in their "available memory" pool, causing the corresponding physical
	// RAM to stay marked as "in use" by the OS. HB
	if (need_limits_check)
	{
		U32 limit;
		if (min_mem_mb)
		{
		...
		{
			static U32 total = LLMemory::getMaxPhysicalMemKB() >> 10;
			limit = llclamp(total * llclamp((U32)mem_pct, 1, 50) / 100, 128,
							4096);
		}
		if (limit != reported)
		{
			reported = limit;
			llinfos << "Will from now on check for minimum free physical memory: "
					<< limit << "MB" << llendl;
		}
		U32 available = LLMemory::getAvailablePhysicalMemKB() >> 10;
		gLowMemory = available < limit;
		if (gLowMemory)
		{
			llwarns << "Low free physical memory: " << available << "MB left."
					<< llendl;
		}
	}
}
...
void LLAppViewer::frame(LLEventPump& mainloop)
{
	...
	if (!isExiting())
	{
		...
		if (!isExiting())
		{
			...
			if (gUsePBRShaders)
			{
				...
				if (LLHeroProbeManager::sNeedsReregister ||
					LLReflectionMapManager::sNeedsReregister)
				{
					gObjectList.reRegisterReflectionProbes();
					LLHeroProbeManager::sNeedsReregister = false;
				}
				if (LLReflectionMapManager::sNeedsReregister)
				{
					gWorld.reRegisterReflectionProbes();
					LLSpatialGroup::reRegisterProbes();
					LLReflectionMapManager::sNeedsReregister = false;
				}
			}
	...
	// Run background threads and sleep if needed/requested.
	{
		...
		do
		{
			...
			if (isExiting())	// Do not perform the rest when exiting. HB
			{
				return;
			}
...
// Runs the main loop until time to quit.
...
bool LLAppViewer::cleanup()
{
	...
	llinfos << "Asking any remaining plugins to shut down..." << llendl;
	...
	{
		mainloop.post(LLSD());	// Post and empty event
...
//mk
	...
	if (gViewerWindowp)
	{
		...
		gViewerWindowp = NULL;
		llinfos << "Viewer window deleted" << llendl;
	}

	LLSplashScreen::show();
	LLSplashScreen::update("Cleaning up...");
	...
	if (mPurgeOnExit)
	{
		...
		LLDirIterator::deleteFilesInDir(gDirUtil.getCacheDir());
...
void LLAppViewer::initThreads()
{
	...
	if (!mIsSiblingViewer)
	{
		...
		// Windows) this call is a no-operation and no affinity is
...
void LLAppViewer::initLogging()
{
	...
	mLogFileName = gDirUtil.getFullPath(LL_PATH_LOGS, filename);
	// Set the log file
...
void LLAppViewer::renameLog(bool on_exit)
{
	...
	std::string old_log_file = gDirUtil.getFullPath(LL_PATH_LOGS,
													PREVIOUS_LOG);
...
#endif
	{
		// Remove the last PREVIOUS_LOG log file, if any.
		LLFile::remove(old_log_file);
		std::string log_file = gDirUtil.getFullPath(LL_PATH_LOGS, CURRENT_LOG);
											...
											bool set_defaults)
{
	...
	for (LLSD::map_iterator it = files.beginMap(); it != files.endMap(); ++it)
	{
		...
		if (full_settings_path.empty())
		{
			...
			full_settings_path = gDirUtil.getFullPath(path_index, file_name);
		}
...
LLApp::InitState LLAppViewer::initConfiguration()
{
	...
	// Read command line settings.
	filename = gDirUtil.getFullPath(LL_PATH_APP_SETTINGS, "cmd_line.xml");
	...
	if (!initParseCommandLine(clp))
	{
		...
		msg << "An error was found while parsing the command line. Please see:\n"
			<< "https://wiki.secondlife.com/wiki/Client_parameters&oldid=878593\n"
	...
	{
		filename = gDirUtil.getFullPath(LL_PATH_USER_SETTINGS,
	...
	{
		const strings_vec_t& set_values = clp.getOption("set");
		...
		{
			for (strings_vec_t::const_iterator it = set_values.begin();
	...
	else if (!gDirUtil.hasSkin(skin_name))
	{
	...
	gDirUtil.setSkinFolder(skin_name);
	...
	else
	{
		...
		// Cleanup remnants of old CEF instances private caches. Since we are
		// not a second instance, there is no risk to clear the CEF cache
		// pertaining to another viewer still at login screen. HB
...
bool LLAppViewer::initWindow()
{
	...
	// Pass the hooks into newview to the llgltf library. HB
	LLGLTFSceneManager::init();
...
void LLAppViewer::writeDebugInfo(bool log_interesting_info)
{
	...
	{
		std::string filename = gDirUtil.getFullPath(LL_PATH_LOGS,
													"debug_info.log");
...
{
	LLDirIterator::deleteFilesInDir(gDirUtil.getCacheDir(), file_mask);
}
...
void LLAppViewer::writeSystemInfo()
{
	...
	gDebugInfo["CRTFilename"] = gDirUtil.getCRTFile();
	...
	// *FIX:Mani - move this down in llappviewerwindows.cpp
#if LL_WINDOWS
...
void LLAppViewer::handleViewerCrash()
{
	...
	gDebugInfo["CRTFilename"] = gDirUtil.getCRTFile();
	gDebugInfo["ViewerExePath"] = gDirUtil.getExecutablePathAndName();
	gDebugInfo["CurrentPath"] = gDirUtil.getCurPath();
	...
	{
		filename = gDirUtil.getFullPath(LL_PATH_LOGS,
...
bool LLAppViewer::anotherInstanceRunning()
{
	...
	mMarkerFileName = gDirUtil.getFullPath(LL_PATH_LOGS, MARKER_FILE_NAME);
...
void LLAppViewer::initMarkerFile()
{
	...
	std::string logout_marker_file =
		gDirUtil.getFullPath(LL_PATH_LOGS, LOGOUT_MARKER_FILE_NAME);
	std::string llerror_marker_file =
		gDirUtil.getFullPath(LL_PATH_LOGS, LLERROR_MARKER_FILE_NAME);
	std::string error_marker_file =
		gDirUtil.getFullPath(LL_PATH_LOGS, ERROR_MARKER_FILE_NAME);
...
static bool finish_quit(const LLSD& notification, const LLSD& response)
{
	...
	{
		// Some OpenSim grids can really be annoying and spuriously trigger
...
bool LLAppViewer::initCache()
{
	...
	if (!read_only)
	{
		...
		{
			if (gDirUtil.setCacheDir(cache_location))
			{
	...
	if (!gDirUtil.setCacheDir(gSavedSettings.getString("CacheLocation")))
	{
...
void LLAppViewer::sendSimpleLogoutRequest()
{
	if (gMessageSystemp)
	{
		gMessageSystemp->newMessageFast(_PREHASH_LogoutRequest);
		gMessageSystemp->nextBlockFast(_PREHASH_AgentData);
		gMessageSystemp->addUUIDFast(_PREHASH_AgentID, gAgentID);
		gMessageSystemp->addUUIDFast(_PREHASH_SessionID, gAgentSessionID);
...
void LLAppViewer::forceDisconnect(const std::string& mesg)
{
	...
	if (LLStartUp::isLoggedIn())
	{
		// In case of failed TP, attempt to send a logout request to the
		// departure sim.
...
void LLAppViewer::saveFinalSnapshot()
{
	if (gViewerWindowp && !mSavedFinalSnapshot)
	{
		...
		std::string snap_filename = gDirUtil.getLindenUserDir();
...
void LLAppViewer::loadNameCache()
{
	...
	filename = gDirUtil.getFullPath(LL_PATH_CACHE, prefix + "name.cache");
...
void LLAppViewer::saveNameCache()
{
	...
	// Display names cache
	std::string filename =
		gDirUtil.getFullPath(LL_PATH_CACHE, prefix + "avatar_name_cache.xml");
...
void LLAppViewer::saveExperienceCache()
{
	...
	std::string filename =
		gDirUtil.getFullPath(LL_PATH_CACHE, prefix + "experience_cache.xml");
...
static void toggle_360_interest_list()
{
	static LLCachedControl<bool> use_360(gSavedSettings, "Use360InterestList");
	// Triggers the interest list capability setting call
	gSavedSettings.setBool("Use360InterestList", !use_360);
}
...
//mk
	...
	if (LLFile::hadWriteErrors())	// This also resets the error flag. HB
	{
		static LLUUID last_notification_id;
		static F32 last_warning = 0.f;
		// Do not spam the user and print at most one warning every 90s.
		// Note: since the error flag is reset by LLFile::hadWriteErrors(), and
		// given this minimum interval between warnings, all errors occurring
		// within 90s of the last warning will be ignored, which is the desired
		// behaviour so that if the user frees disk space just after a warning,
		// they would not see another (now stale/irrelevant) warning popping up
		// a few seconds later, just because an error occurred before they
		// could free the space. HB
		constexpr F32 WARNING_INTERVAL = 90.f;	// In seconds.
		if (gFrameTimeSeconds - last_warning > WARNING_INTERVAL)
		{
			last_warning = gFrameTimeSeconds;
			// Also check that the last notification is not still displayed. HB
			if (last_notification_id.isNull() ||
				!gNotifications.find(last_notification_id))
			{
				LLNotificationPtr n = gNotifications.add("WriteError");
				if (n)
				{
					last_notification_id = n->getID();
				}
			}
		}
	}
	...
	if (!gDisconnected)
	{
		...
		// If we have unrezzed avatars caused by missing attachment updates,
		// toggle the 360° interest list feature to get an update. HB
		static F32 last_360_toggling = 0.f;
		static LLCachedControl<bool> decloud(gSavedSettings,
											 "AutoDecloudAvatars");
		if (decloud && gIsInSecondLife && LLVOAvatar::sNeedsAttachmentUpdate)
		{
			LLVOAvatar::sNeedsAttachmentUpdate = false;
			// Do not spam the server and cause excessive avatar flickering
			// on screen.
			constexpr F32 MIN_INTERVAL = 10.f;	// In seconds
			if (gFrameTimeSeconds - last_360_toggling > MIN_INTERVAL)
			{
				static U32 attempts = 0;
				static LLViewerRegion* aregionp = NULL;
				if (aregionp != gAgent.getRegion())
				{
					aregionp = gAgent.getRegion();
					attempts = 0;
				}
				llinfos << "Triggering avatars de-clouding workaround (attempt #"
						<< ++attempts << " in this region)." << llendl;
				// Toggle it...
				toggle_360_interest_list();
				// ... and restore it 1 second later.
				doAfterInterval(toggle_360_interest_list, 1.f);
				last_360_toggling = gFrameTimeSeconds;
			}
		}
		...
		F32 agent_update_time = agent_update_timer.getElapsedTimeF32() +
								// Extrapolate to next frame probable render
								// time, based on last frame time. HB
	...
	if (!(logoutRequestSent() && mSavedFinalSnapshot) && !isExiting())
	{
		// Actually "object update"
		LL_FAST_TIMER(FTM_OBJECTLIST_UPDATE);
		gObjectList.update();
		LLGLTFSceneManager::update();
	}
...
void LLAppViewer::sendLogoutRequest()
{
	...
	if (msg && !mLogoutRequestSent)
	{
		...
		{
			gVoiceClient.setVoiceEnabled(false);
		}
...
void LLAppViewer::idleNameCache()
{
	...
	// Launch the SLURLs resolution, which may add queries to the name
	// cache. HB
...
// 20ms=50fps (just for messages !)
constexpr F32 CHECK_MESSAGES_DEFAULT_MAX_TIME = .020f;
static F32 sCheckMessagesMaxTime = CHECK_MESSAGES_DEFAULT_MAX_TIME;
	...
	if (!speed_test)
	{
		...
		{	// Scope-guard for LockMessageChecker
...
#endif
			{
				...
				if (total_time >= sCheckMessagesMaxTime)
				{
					needs_drain = true;
					break;
				}
			...
			if (needs_drain ||
				gMessageSystemp->mPacketRing.getNumBufferedPackets() > 0)
			{
				// Rather than allow packets to silently backup on the socket
				// we drain them into our own buffer so we know how many exist.
				U32 num_buffered_packets = gMessageSystemp->drainUdpSocket();
				if (num_buffered_packets)
				{
					// Increase sCheckMessagesMaxTime so that we will eventually
					// catch up. 3.5% ~= x2 in 20 frames, ~8x in 60 frames
					sCheckMessagesMaxTime *= 1.035f;
				}
				else
				{
					// Reset sCheckMessagesMaxTime to default value
					sCheckMessagesMaxTime = CHECK_MESSAGES_DEFAULT_MAX_TIME;
				}
			}
...
void LLAppViewer::disconnectViewer()
{
	...
	gSavedPerAccountSettings.setBool("FlyingAtExit", gAgent.getFlying());
