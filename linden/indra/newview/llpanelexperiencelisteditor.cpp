...
void LLPanelExperienceListEditor::checkButtonsEnabled(LLUICtrl*, void* data)
{
	...
	{
		LLScrollListCtrl::items_vec_t items = self->mItems->getAllSelected();
		for (size_t i = 0, count = items.size(); i < count; ++i)
		{
			LLScrollListItem* itemp = items[i];	// Cannot be NULL. HB
			if (self->mSticky(itemp->getValue()))
			{
...
void LLPanelExperienceListEditor::onAdd(void* data)
{
	...
	LLFloaterExperiencePicker* picker =
		LLFloaterExperiencePicker::show(std::bind(&LLPanelExperienceListEditor::addExperienceIds,
												  self, _1),
...
void LLPanelExperienceListEditor::onRemove(void* data)
{
	...
	for (size_t i = 0, count = items.size(); i < count; ++i)
	{
		LLScrollListItem* itemp = items[i];	// Cannot be NULL. HB
#if 0	// Now handled by the callback
		self->mExperienceIds.erase(itemp->getValue());
#endif
		self->mRemovedCallback(itemp->getValue());
	}
...
void LLPanelExperienceListEditor::onItems(LLUICtrl*, void* data)
{
		 ...
		 it != end; ++it)
	{
		...
		exp->get(experience,
				 std::bind(&LLPanelExperienceListEditor::experienceDetailsCallback,
