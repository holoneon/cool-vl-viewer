...
// *TODO: deduce these constants from the XUI definition. HB
constexpr S32 UI_HEIGHT_LONG = 546;
constexpr S32 UI_HEIGHT_SHORT = UI_HEIGHT_LONG - 250;
constexpr S32 UI_WIDTH = 215;
...
bool LLSnapshotLivePreview::onIdle()
{
									...
									getMaxImageSize()))
	{
		...
		if (getSnapshotType() == SNAPSHOT_TEXTURE)
		{
			...
			scaled->biasedScaleToPowerOfTwo(gMaxImageSizeDefault);
		...
		if (!scaled->isBufferInvalid())
		{
			...
			if (mPreviewImageEncoded->getWidth() > gMaxImageSizeDefault ||
				mPreviewImageEncoded->getHeight() > gMaxImageSizeDefault)
			{
			...
			else
			{
				...
				scaled->expandToPowerOfTwo(gMaxImageSizeDefault, false);
			}
...
//mk
		...
		S32 expected_upload_cost =
			LLEconomy::getInstance()->getTextureUploadCost(scaled->getWidth(),
														   scaled->getHeight());
...
static U32 get_last_snapshot_width()
{
	static LLCachedControl<U32> type(gSavedSettings, "LastSnapshotType");
	static LLCachedControl<U32> postcard(gSavedSettings,
										 "LastSnapshotToEmailWidth");
	static LLCachedControl<U32> texture(gSavedPerAccountSettings,
										"LastSnapshotToInventoryWidth");
	static LLCachedControl<U32> thumbnail(gSavedSettings,
										  "LastSnapshotThumbnailWidth");
	static LLCachedControl<U32> local(gSavedSettings,
...
static void set_last_snapshot_width(U32 width)
{
	static LLCachedControl<U32> type(gSavedSettings, "LastSnapshotType");
	switch ((U32)type)
	{
		case LLSnapshotLivePreview::SNAPSHOT_POSTCARD:
			gSavedSettings.setU32("LastSnapshotToEmailWidth", width);
			break;

		case LLSnapshotLivePreview::SNAPSHOT_TEXTURE:
			gSavedPerAccountSettings.setU32("LastSnapshotToInventoryWidth",
											width);
			break;

		case LLSnapshotLivePreview::SNAPSHOT_THUMBNAIL:
			gSavedSettings.setU32("LastSnapshotThumbnailWidth", width);
			break;

		default:
			gSavedSettings.setU32("LastSnapshotToDiskWidth", width);
	}
}

static U32 get_last_snapshot_height()
{
	static LLCachedControl<U32> type(gSavedSettings, "LastSnapshotType");
	static LLCachedControl<U32> postcard(gSavedSettings,
...
static void set_last_snapshot_height(U32 height)
{
	static LLCachedControl<U32> type(gSavedSettings, "LastSnapshotType");
	switch ((U32)type)
	{
		case LLSnapshotLivePreview::SNAPSHOT_POSTCARD:
			gSavedSettings.setU32("LastSnapshotToEmailHeight", height);
			break;

		case LLSnapshotLivePreview::SNAPSHOT_TEXTURE:
			gSavedPerAccountSettings.setU32("LastSnapshotToInventoryHeight",
											height);
			break;

		case LLSnapshotLivePreview::SNAPSHOT_THUMBNAIL:
			gSavedSettings.setU32("LastSnapshotThumbnailHeight", height);
			break;

		default:
			gSavedSettings.setU32("LastSnapshotToDiskHeight", height);
	}
}
...
bool LLFloaterSnapshot::postBuild()
{
	...
	mImageWidthSpinner->setValue(get_last_snapshot_width());
	mImageHeightSpinner->setValue(get_last_snapshot_height());
...
void LLFloaterSnapshot::resetSnapshotSizeOnUI(S32 width, S32 height)
{
	...
	if (width >= 0 && height >= 0)
	{
		set_last_snapshot_width(width);
		set_last_snapshot_height(height);
	}
	else
	{
		llwarns << "Negative snapshot dimension passed. Not saved." << llendl;
	}
}
...
void LLFloaterSnapshot::comboSetCustom(LLComboBox* combop)
{
	...
	{
		S32 index = combop->getCurrentIndex();
		// Change index if the grid dow not support 2K textures and this is the
		// the current (invalid) choice in the combo. HB
		if (gMaxImageSizeDefault <= 1024 && index == 5)	// 2048x2048
		{
			index = 4;	// 1024x1024
		}
		gSavedPerAccountSettings.setS32("SnapshotTextureLastResolution",
										index);
	}
...
void LLFloaterSnapshot::updateLayout()
{
	...
	if (!is_advance)
	{
		delta_height = UI_HEIGHT_SHORT - UI_HEIGHT_LONG;
	}
	...
	if (!is_advance)
	{
		...
		gSavedPerAccountSettings.setS32("SnapshotTextureLastResolution", 0);
	...
	if (mFreezeFrameCheck->get())
	{
		...
		reshape(getRect().getWidth(), UI_HEIGHT_LONG + delta_height);
...
void LLFloaterSnapshot::updateControls()
{
	...
	static LLCachedControl<S32> texture_res(gSavedPerAccountSettings,
...
//mk
	...
	switch (shot_type)
	{
		case LLSnapshotLivePreview::SNAPSHOT_POSTCARD:
			...
			if (is_advance)
			{
				updateResolution(mPostcardSizeCombo, this);
			}
			break;
		...
		case LLSnapshotLivePreview::SNAPSHOT_TEXTURE:
		{
			...
			if (is_advance)
			{
				updateResolution(mTextureSizeCombo, this);
			}
			break;
		}
		...
		case LLSnapshotLivePreview::SNAPSHOT_LOCAL:
			...
			if (is_advance)
			{
				updateResolution(mLocalSizeCombo, this);
			}
			break;
		...
		case LLSnapshotLivePreview::SNAPSHOT_THUMBNAIL:
			...
			if (is_advance)
			{
				updateResolution(mThumbnailSizeCombo, this);
			}
			break;
	...
	// Update expected upload cost, which now depends on the texture size. HB
	S32 expected_cost = 0;
	if (!is_thumbnail)
	{
		S32 w, h;
		mLivePreview->getSize(w, h);
		expected_cost = LLEconomy::getInstance()->getTextureUploadCost(w, h);
	}
	mTempCheck->setVisible(has_temp_upload && is_advance && expected_cost > 0);
	if (isTempAsset())
	{
		expected_cost = 0;
	}
	childSetLabelArg("texture", "[AMOUNT]", llformat("%d", expected_cost));
	mUploadButton->setLabelArg("[AMOUNT]", llformat("%d", expected_cost));
...
//static
void LLFloaterSnapshot::onCommitSave(LLUICtrl* ctrlp, void* datap)
{
	if (ctrlp && datap)
	{
		if (ctrlp->getValue().asString() == "save as")
		{
...
void LLFloaterSnapshot::onClickMore(void* data)
{
	...
	self->translate(0, UI_HEIGHT_SHORT - UI_HEIGHT_LONG);
	self->reshape(self->getRect().getWidth(), UI_HEIGHT_LONG);
	self->updateControls();
	self->updateLayout();
...
void LLFloaterSnapshot::onClickLess(void* data)
{
	...
	self->translate(0, UI_HEIGHT_LONG - UI_HEIGHT_SHORT);
	self->reshape(self->getRect().getWidth(), UI_HEIGHT_SHORT);
	self->updateControls();
	self->updateLayout();
...
//static
void LLFloaterSnapshot::onClickKeepAspectCheck(LLUICtrl* ctrlp, void* datap)
{
	LLFloaterSnapshot* self = (LLFloaterSnapshot*)datap;
	...
	self->mLivePreview->mKeepAspectRatio = checkp->get();
...
//static
void LLFloaterSnapshot::onCommitQuality(LLUICtrl* ctrlp, void* datap)
{
	LLFloaterSnapshot* self = (LLFloaterSnapshot*)datap;
	LLSliderCtrl* sliderp = (LLSliderCtrl*)ctrlp;
	if (self && sliderp) 
	{
		S32 quality_val = S32(sliderp->getValue().asReal());
...
//static
void LLFloaterSnapshot::onCommitResolution(LLUICtrl* ctrlp, void* datap)
{
	updateResolution(ctrlp, datap, true);
}

//static
void LLFloaterSnapshot::onCommitFreezeFrame(LLUICtrl*, void* datap)
{
	LLFloaterSnapshot* self = (LLFloaterSnapshot*)datap;
	if (self)
	{
...
//static
void LLFloaterSnapshot::updateResolution(LLUICtrl* ctrlp, void* datap,
										 bool do_update)
{
	if (!datap || !ctrlp) return;
	...
	S32 index = self->mTextureSizeCombo->getCurrentIndex();
	// Disable the 2048x2048 combo entry, if the grid does not support it. HB
	LLScrollListItem* itemp = self->mTextureSizeCombo->getItemByIndex(5);
	if (itemp)
	{
		bool has_2k = gMaxImageSizeDefault > 1024;
		itemp->setEnabled(has_2k);
		if (!has_2k && index == 5)	// 2048x2048
		{
			index = 4;	// 1024x1024
		}
	}
	gSavedPerAccountSettings.setS32("SnapshotTextureLastResolution", index);
	gSavedSettings.setS32("SnapshotLocalLastResolution",
	...
	std::string sdstring = combop->getSelectedValue();
	LLSD sdres;
	...
	if (combop->getCurrentIndex() >= 0)
	{
		...
		else if (width == -1 || height == -1)
		{
			...
			self->mLivePreview->setSize(get_last_snapshot_width(),
...
//static
void LLFloaterSnapshot::onCommitLayerTypes(LLUICtrl* ctrlp, void* datap)
{
	LLFloaterSnapshot* self = (LLFloaterSnapshot*)datap;
	if (self && ctrlp)
	{
		U32 type = ((LLComboBox*)ctrlp)->getCurrentIndex();
...
//static
void LLFloaterSnapshot::onCommitSnapshotType(LLUICtrl*, void* datap)
{
	LLFloaterSnapshot* self = (LLFloaterSnapshot*)datap;
	if (self)
	{
...
//static
void LLFloaterSnapshot::onCommitSnapshotFormat(LLUICtrl*, void* datap)
{
	LLFloaterSnapshot* self = (LLFloaterSnapshot*)datap;
	if (self)
	{
...
//static
void LLFloaterSnapshot::onCommitCustomResolution(LLUICtrl* ctrlp, void* datap)
{
	LLFloaterSnapshot* self = (LLFloaterSnapshot*)datap;
	LLSpinCtrl* spinp = (LLSpinCtrl*)ctrlp;
	if (!self || !spinp)
	{
		return;
	}

	S32 w = (S32)self->mImageWidthSpinner->getValue().asReal();
	S32 h = (S32)self->mImageHeightSpinner->getValue().asReal();
	S32 curw, curh;
	self->mLivePreview->getSize(curw, curh);
	if (w != curw || h != curh)
	{
		self->mLivePreview->setMaxImageSize((S32)spinp->getMaxValue());
		// Check image size changes the value of height and width
		if (self->mLivePreview->checkImageSize(w, h, w != curw))
		{
			self->resetSnapshotSizeOnUI(w, h);
		}
	...
	set_last_snapshot_width(w);
	set_last_snapshot_height(h);
