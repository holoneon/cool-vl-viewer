...
LLMaterialMgr::LLMaterialMgr()
{
	...
	gWorld.setRegionRemovedCallback(std::bind(&LLMaterialMgr::onRegionRemoved,
											  this, _1));
}
	...
	if (!isGetPending(region_id, material_id))
	{
		...
		if (qit == mGetQueue.end())
		{
			LL_DEBUGS("Materials") << "mGetQueue add region: " << region_id
								   ...
								   << LL_ENDL;
			qit = mGetQueue.emplace(region_id, material_queue_t()).first;
		}
...
void LLMaterialMgr::get(const LLUUID& region_id,
						const LLMaterialID& material_id,
						LLMaterialMgr::get_cb_t::slot_type cb)
{
	material_map_t::const_iterator mit = mMaterials.find(material_id);
	if (mit != mMaterials.end())
	{
	...
	else
	{
		...
		get_cb_map_t::iterator cb_it = mGetCallbacks.find(material_id);
		if (cb_it == mGetCallbacks.end())
		{
			cb_it = mGetCallbacks.emplace(material_id,
										  std::make_unique<get_cb_t>()).first;
		}
		cb_it->second->connect(cb);
	}
}

void LLMaterialMgr::getTE(const LLUUID& region_id,
						  const LLMaterialID& material_id, U32 te,
						  LLMaterialMgr::get_te_cb_t::slot_type cb)
{
	material_map_t::const_iterator mit = mMaterials.find(material_id);
	if (mit != mMaterials.end())
	{
	...
	else
	{
		...
		get_te_cb_map_t::iterator cb_it = mGetTECallbacks.find(te_mat_pair);
		if (cb_it == mGetTECallbacks.end())
		{
			cb_it =
				mGetTECallbacks.emplace(te_mat_pair,
										std::make_unique<get_te_cb_t>()).first;
		}
		cb_it->second->connect(cb);
	}
}
...
void LLMaterialMgr::getAll(const LLUUID& region_id,
						   LLMaterialMgr::get_all_cb_t::slot_type cb)
{
	if (!isGetAllPending(region_id))
	{
		mGetAllQueue.emplace(region_id);
	...
	get_all_cb_map_t::iterator cb_it = mGetAllCallbacks.find(region_id);
	if (cb_it == mGetAllCallbacks.end())
	{
		cb_it =
			mGetAllCallbacks.emplace(region_id,
									 std::make_unique<get_all_cb_t>()).first;
	}
	cb_it->second->connect(cb);
}
...
void LLMaterialMgr::put(const LLUUID& object_id, U8 te, const LLMaterial& mat)
{
	...
	if (qit == mPutQueue.end())
	{
		LL_DEBUGS("Materials") << "mPutQueue insert object " << object_id
							   << LL_ENDL;
		qit = mPutQueue.emplace(object_id, facematerial_map_t()).first;
	}
										 ...
										 const LLMaterialPtr& material_ptr)
{
	...
	{
		te_mat_pair.mTE = i++;
		get_te_cb_map_t::iterator te_it = mGetTECallbacks.find(te_mat_pair);
	...
	while (loop_rqit != mGetQueue.end())
	{
		...
		LLCore::HttpHandler::ptr_t
			handler(new LLMaterialHttpHandler("POST",
											  std::bind(&LLMaterialMgr::onGetResponse,
														this, _1, _2,
														region_id)));
...
void LLMaterialMgr::processGetAllQueue()
{
	...
	while (loop_rit != mGetAllQueue.end())
	{
		...
		gCoros.launch("LLMaterialMgr::processGetAllQueueCoro",
					  std::bind(&LLMaterialMgr::processGetAllQueueCoro, this,
								url, region_id));

		regionp->resetMaterialsCapThrottle();
		mGetAllPending[region_id] = LLFrameTimer::getTotalSeconds();
		 ...
		 it != end; ++it)
	{
		...
		LLCore::HttpHandler::ptr_t
			handler(new LLMaterialHttpHandler("PUT",
											  std::bind(&LLMaterialMgr::onPutResponse,
														this, _1, _2)));
		LLCore::HttpHandle handle =
...
//static
void LLMaterialMgr::cleanupClass()
{
	if (LLMaterialMgr::instanceExists())
	{
		llinfos << "Cleaning up the materials manager queues..." << llendl;
		LLMaterialMgr* self = LLMaterialMgr::getInstance();
		self->mGetCallbacks.clear();
		self->mGetTECallbacks.clear();
		self->mGetAllCallbacks.clear();
		self->mGetPending.clear();
		self->mGetQueue.clear();
		self->mPutQueue.clear();
		self->mGetAllQueue.clear();
		self->mGetAllRequested.clear();
		self->mGetAllPending.clear();
		self->mMaterials.clear();
		llinfos << "Materials manager queues emptied." << llendl;
	}
}
