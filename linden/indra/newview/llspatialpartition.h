...
#define SG_STATE_INHERIT_MASK U32(OCCLUDED)
#define SG_INITIAL_STATE_MASK ((U32)DIRTY | (U32)GEOM_DIRTY)
...
public:
	...
	LLDrawInfo(const LLDrawInfo&) = delete;

	const LLDrawInfo& operator=(const LLDrawInfo&) = delete;
...
public:
	...
	bool								mIsDebugged;
#endif
	// Set to 'true' when a this face pertains to one of the agent's avatar
	// attachments. HB
	bool								mIsAgentAttachment;
	// Set to 'true' when a legacy material is used. HB
	bool								mHasALM;
	// Set to 'true' when a PBR material is used. HB
	bool								mHasPBR;
	// Set to 'true' when a fallback is provided for non-PBR viewers. HB
	bool								mHasLegacyFallback;
	// Set to 'true' when we use our hack to render the base color texture
	// to replace a missing fallback diffuse texture or legacy material. HB
	bool								mHasBasecolorFallback;
...
public:
	...
	LLSpatialGroup(const LLSpatialGroup&) = delete;

	const LLSpatialGroup& operator=(const LLSpatialGroup&) = delete;
	...
	// Allows to re-register the spatial groups with the reflection map manager
	// after the latter got reset. HB
	static void reRegisterProbes();
...
public:
	...
	typedef flat_hset<LLSpatialGroup*> groups_set_t;
	static groups_set_t			sNodes;
...
public:
	...
	virtual void rebuildGeom(LLSpatialGroup* groupp) = 0;
	virtual void rebuildMesh(LLSpatialGroup* groupp) = 0;
	virtual void getGeometry(LLSpatialGroup* groupp) = 0;
	virtual void addGeometryCount(LLSpatialGroup* groupp, U32& vertex_count,
								  U32& index_count);
...
// Used in llgltfscenemanager.cpp
...
// This is set to true in LLReflectionMap::autoAdjustOrigin() when we are
// searching for reflection probes intersections for rendering. HB
extern bool gPickingProbe;
