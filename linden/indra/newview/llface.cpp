...
void LLFace::init(LLDrawable* drawablep, LLViewerObject* objp)
{
	...
	mLastVirtualSizeTime = 0.f;
	mRandomTimeNoise = ll_frand() * 0.1f;
	mVSize = 0.f;
	mPixelArea = 16.f;
	mState = GLOBAL;
	mDrawOrderIndex = 0;
...
void LLFace::printDebugInfo() const
{
	...
	llinfos << "Face: " << std::hex << intptr_t(this) << std::dec << llendl;
	...
	if (poolp)
	{
		...
		U32 pool_references = 0;
		for (size_t i = 0, count = poolp->mReferences.size(); i < count; ++i)
		{
			LLFace* facep = poolp->mReferences[i];
			if (facep == this)
			{
...
bool LLFace::calcAlignedPlanarTE(const LLFace* align_to,  LLVector2* res_st_offset,
								 ...
								 S32 map) const
{
	if (!align_to || map < 0 || map > LLRender::EMISSIVE_MAP)
	{
		return false;
	}
	...
	F32 map_off_s = 0.f;
	F32 map_off_t = 0.f;
	if (map >= LLRender::BASECOLOR_MAP)
	{
		LLGLTFMaterial* gltfp = orig_tep->getGLTFRenderMaterial();
		if (!gltfp)
		{
			return false;
		}
		...
		const LLGLTFMaterial::TextureTransform& tt =
			gltfp->mTextureTransform[map - LLRender::BASECOLOR_MAP];
		LLGLTFMaterial::PBRTransformToTexture(tt, map_scl_s, map_scl_t,
											  map_off_s, map_off_t, map_rot);
	}
	else
	{
	...
	*res_st_offset = orig_st_offset + (LLVector2)centers_dist;
	...
	if (map >= LLRender::BASECOLOR_MAP)
	{
		...
		LLGLTFMaterial::textureTransformToPBR(st_scale.mV[VX], st_scale.mV[VY],
											  res_st_offset->mV[VX],
											  res_st_offset->mV[VY],
											  z_ang, *res_st_scale,
											  *res_st_offset, *res_st_rot);
	}
	else
	{
							   ...
							   const U16& index_offset, bool force_rebuild)
{
	...
	if (!vf.mTexCoords || !vf.mNormals || !vf.mPositions || !vf.mIndices)
	{
...
//MK
	bool rlv_cam_textures = gRLenabled && gRLInterface.mContainsCamTextures &&
							getViewerObject() &&
							!getViewerObject()->isAttachment();
//mk
...
//mk
		if (rebuild_tcoord)
		{
			...
			{
				// When we do override the diffuse texture with the PBR base
		...
		if (!gUsePBRShaders)
		{
			gltfp = NULL;	// Do not use the PBR material in non-PBR mode.
		}
		if (gltfp)
		{
...
//mk
	...
	if (full_rebuild)
	{
		...
		volatile __m256i* dst = (__m256i*)indicesp.get();
		__m256i* src = (__m256i*)vf.mIndices;
		__m256i offset = _mm256_set1_epi16(index_offset);

		U32 end = num_indices / 16;

		for (U32 i = 0; i < end; ++i)
		{
			__m256i res = _mm256_add_epi16(src[i], offset);
			_mm256_storeu_si256((__m256i*)dst++, res);
		}

		U16* idx = (U16*)dst;
		for (U32 i = end * 16; i < num_indices; ++i)
		{
...
{
	constexpr F32 VSIZE_UPDATE_PERIOD = 0.1f;
	if (gFrameTimeSeconds - mLastVirtualSizeTime < VSIZE_UPDATE_PERIOD)
	{
		return mVSize;
	}
	mLastVirtualSizeTime = gFrameTimeSeconds +
						   // Add 10% noise to avoid recalulating all virtual
						   // sizes at the same frame.
						   mRandomTimeNoise * VSIZE_UPDATE_PERIOD;
	...
	else
	{
		...
		mVSize = mPixelArea / llclamp(texel_area, 0.015625f, 128.f);
	}

	mVSize = adjustPixelArea(mImportanceToCamera, mVSize);

	// If it is a large image, shrink face_area by considering the partial
	// overlapping:
	if (mImportanceToCamera < 1.f &&
		mVSize > LLViewerTexture::sMinLargeImageSize &&
		mImportanceToCamera > LEAST_IMPORTANCE_FOR_LARGE_IMAGE)
	{
		LLViewerTexture* tex = mTexture[LLRender::DIFFUSE_MAP].get();
		if (tex && tex->isLargeImage())
		{
			mVSize *= adjustPartialOverlapPixelArea(cos_angle_to_view_dir,
													radius);
		}
	}
...
bool LLFace::calcPixelArea(F32& cos_angle_to_view_dir, F32& radius)
{
	...
	if (isState(LLFace::RIGGED))
	{
		...
		LLVOAvatar* avatarp = mVObjp.notNull() ? mVObjp->getAvatar() : NULL;
	...
	else if (mDrawablep && mDrawablep->getSpatialGroup() && mVObjp.notNull() &&
			 mVObjp->getPartitionType() == LLViewerRegion::PARTITION_PARTICLE)
	{
		// Use box of spatial group for particles (over approximates size, but
		// we do not actually have a good size per particle).
		const LLVector4a* exts = mDrawablep->getSpatialGroup()->getExtents();
		size.setSub(exts[1], exts[0]);
		center.setAdd(exts[1], exts[0]);
		center.mul(0.5f);
	}	 
	else
	{
										  ...
										  F32 radius)
{
	...
	{
		alpha = llclamp(0.5f * (radius + screen_radius - d) / radius,
						0.f, 1.f);
	}
	return alpha * alpha;
#endif
...
bool LLFace::verify(const U32* indices_array) const
{
	...
	if (mGeomIndex + mGeomCount > mVertexBuffer->getNumVerts())
	{
		ok = false;
		llwarns << "Face " << std::hex << intptr_t(this) << std::dec
				<<  " references invalid vertices." << llendl;
	}
	...
	if (mIndicesIndex + mIndicesCount > mVertexBuffer->getNumIndices())
	{
		ok = false;
		llwarns << "Face " << std::hex << intptr_t(this) << std::dec
				<< " references invalid indices." << llendl;
	}
	...
	for (U32 i = 0; i < indices_count; ++i)
	{
		...
		{
			llwarns << "Face " << std::hex << intptr_t(this) << std::dec 
					<< ": index too low. i: " << i << " - Index:"
					<< indicesp[i] << " - GStart: " << geom_start << llendl;
			ok = false;
		}
		else if (delta >= geom_count)
		{
			llwarns << "Face " << std::hex << intptr_t(this) << std::dec 
					<< ": index too high. i: " << i << " - Index: "
					<< indicesp[i] << " - GEnd: " << geom_start + geom_count
					<< llendl;
			ok = false;
		}
	}
...
bool LLFace::canBatchTexture() const
{
	...
	const LLViewerTexture* texp = getTexture();
