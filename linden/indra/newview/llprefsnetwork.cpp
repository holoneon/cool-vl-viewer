...
#include "llviewerthrottle.h"		// For gViewerThrottle.getSettingName()
...
bool LLPrefsNetwork::postBuild()
{
	...
	const char* bw_name = gViewerThrottle.getSettingName();
	childSetValue("max_bandwidth", S32(gSavedSettings.getU32(bw_name)));
	...
	childSetText("cache_path", gDirUtil.getCacheDir());
	...
	// With CEF 120 and newer, caches are per-instance ones and auto-cleared
	// on each instance exit, and at each viewer startup. HB
	childSetVisible("clear_browser_cache", false);
#else
...
void LLPrefsNetwork::apply()
{
	...
	U32 bw = childGetValue("max_bandwidth").asInteger();
	if (!gIsInSecondLife && bw > 1024)
	{
		gNotifications.add("OpenSimUDPBandwithLimit");
	}
	gSavedSettings.setU32(gViewerThrottle.getSettingName(), bw);
...
{
	std::string suggestion = gDirUtil.getFullPath(LL_PATH_CACHE);
...
void LLPrefsNetwork::onClickResetCache(void* data)
{
	...
	self->childSetText("cache_path", gDirUtil.getCacheDir(true));
}
