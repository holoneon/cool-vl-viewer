...
#include "llinitdestroyclass.h"	// Also includes <functional>
...
public:
	...
	// true if the baseitem this points to does not exist in memory.
...
typedef std::function<void(const LLUUID&)> inventory_func_t;
typedef std::function<void()> nullary_inv_func_t;
...
// Returns the Id of the associated resident for a calling card: this is non-
// trivial since while calling cards used to always be identified as created
// by the associated resident (including for the cards automatically given by
// the server when befriending someone), LL decided in v2 viewers to recreate
// "missing" calling cards in inventory (meaning their creator is the resident
// running the viewer) and to instead store the Id of the resident they are
// associated with in the description... When passed true for 'allow_self',
// this function returns the agent Id for their own calling cards, else a null
// UUID is returned for the agent calling cards. HB
