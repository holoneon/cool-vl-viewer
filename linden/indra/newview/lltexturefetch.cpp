...
private:
	...
	public:
		// Threads: Ttf
		LL_INLINE CacheReadResponder(const LLUUID& id, LLImageFormatted* imgp)
		:	mID(id),
			mStartTime(0.f)
		{
			setImage(imgp);
		...
		void completed(bool success) override
		{
			if (gTextureFetchp)
			{
				LLTextureFetchWorker* workerp = gTextureFetchp->getWorker(mID);
				if (workerp)
				{
 					workerp->callbackCacheRead(success, mFormattedImage,
	...
	public:
		...
		void completed(bool success) override
		{
			if (gTextureFetchp)
			{
				...
				if (workerp)
				{
					workerp->callbackCacheWrite(success);
				}
			}
		}
	...
	public:
		...
		// Threads: Tid
		void completed(bool success, LLImageRaw* rp, LLImageRaw* ap) override
		{
			if (gTextureFetchp)
			{
				LLTextureFetchWorker* workerp = gTextureFetchp->getWorker(mID);
				if (workerp)
				{
					workerp->callbackDecoded(success, rp, ap);
				}
			}
		}
...
public:
	...
	// Threads: Ttf
	// Locks: Mw
	S32 callbackHttpGet(LLCore::HttpResponse* responsep, bool partial,
						bool success);

	// Threads: Ttc
	void callbackCacheRead(bool success, LLImageFormatted* imagep,
	...
	// Threads: Tid
	void callbackDecoded(bool success, LLImageRaw* rawp, LLImageRaw* auxp);
...
void LLTextureFetchWorker::checkLoadedDiscard()
{
	if (mLoadedDiscard >= 0 && mLoadedDiscard <= MAX_DISCARD_LEVEL)
	{
		return;
	}
	llwarns << "Texture " << mID << " mLoadedDiscard is " << mLoadedDiscard;
	if (mLoadedDiscard < 0)
	{
		llcont << " and should be >= 0";
	}
	else
	{
		llcont << " and should be <= " << MAX_DISCARD_LEVEL;
	}
	llcont << llendl;
}
...
bool LLTextureFetchWorker::doWork(S32 param)
{
	...
	if (mState == WAIT_HTTP_REQ)
	{
		...
		if (mRequestedSize < 0)
		{
			bool may_be_missing = LLTerrain::isAsset(mID);
			...
			// Fallback and try UDP
			if (mCanUseNET && mFTType != FTT_LOCAL_FILE && !may_be_missing)
			{
		...
		if (mFormattedImage.isNull())
		{
			...
			std::string extension = LLDir::getExtension(mUrl);
			mFormattedImage =
...
decode_image:
	if (mState == DECODE_IMAGE)
	{
		...
		// If we have the entire image data, decode the full resolution image.
		// Note: this is essential for proper decoding of sculpt textures and
		// to avoid temporarily weird colored textures after TPs. HB
		S32 discard = mHaveAllData ? 0 : mLoadedDiscard;
		if (discard > MAX_DISCARD_LEVEL)
		{
			llwarns << "Decode entered for " << mID
					<< " with invalid discard: " << discard
					<< ". Cannot decode. Aborted." << llendl;
			mState = DONE;
			goto fetch_done; // Fall through
		}
									   ...
									   LLCore::HttpResponse* response)
{
	...
	if (!status)
	{
		...
		// them. Also, do not complain if the Id was a "terrain asset" which is
		// in the end a material and not a texture. HB
								   ...
								   bool can_use_http)
{
	...
	S32 desired_size;
	std::string exten = LLDir::getExtension(url);
