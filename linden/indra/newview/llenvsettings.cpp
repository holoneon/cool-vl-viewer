											...
											bool update_name)
{
	...
	LLResourceUploadInfo::ptr_t info =
		std::make_shared<LLBufferedAssetUploadInfo>(inv_item_id,
													LLAssetType::AT_SETTINGS,
													buffer.str(),
													std::bind(&LLEnvSettingsBase::onAgentAssetUploadComplete,
															  _1, _2, _3, _4,
															  settings,
															  callback));
											...
											inv_result_fn callback)
{
	...
	LLResourceUploadInfo::ptr_t info =
		std::make_shared<LLBufferedAssetUploadInfo>(object_id, inv_item_id,
													LLAssetType::AT_SETTINGS,
													buffer.str(),
													std::bind(&LLEnvSettingsBase::onTaskAssetUploadComplete,
															  _1, _2, _3, _4,
															  settings,
															  callback));
	LLViewerAssetUpload::enqueueInventoryUpload(cap_url, info);
...
void LLEnvSettingsSky::applySpecial(LLShaderUniforms* targetp)
{
	...
	bool reflections = LLPipeline::sReflectionProbesEnabled;
	bool can_auto_adjust = true;	// Note: can auto adjust = pre-PBR sky

	if (gUsePBRShaders)
	{
		// Note: when reflections are off, we must use 1.f for the probe
		// ambiance, else the rendering is way too dark. HB
		F32 probe_ambiance = reflections ? getReflectionProbeAmbiance() : 1.f;
		shaderp->uniform1f(LLShaderMgr::REFLECTION_PROBE_AMBIANCE,
						   probe_ambiance);
		// Synced to sClassicMode via LLPipeline::cacheEnvironment(). HB
		can_auto_adjust = LLRender::sClassicMode;

		// Use classic shading for legacy skies
		shaderp->uniform1i(LLShaderMgr::CLASSIC_MODE, can_auto_adjust);
	...
	// *HACK: to get a proper gamma with PBR skies when HDR tone mapping is
	// disabled. HB
	if (!can_auto_adjust && reflections && !gPipeline.mToneMapped)
	{
		static LLCachedControl<F32> gfactor(gSavedSettings,
											"RenderGammaScaleFactorNoToneMap");
		gamma *= llclamp((F32)gfactor, 0.25f, 1.5f);
	}
...
void LLEnvSettingsWater::applySpecial(LLShaderUniforms* targetp)
{
	...
	if (gUsePBRShaders)
	{
		if (LLPipeline::sRenderMirrors &&
			gPipeline.mHeroProbeManager.hasActiveMirror())
		{
	...
	// we are rendering the above-water scene. Force above-water fog.
															 ...
															 LLSD& messages)
{
	...
	for (strings_set_t::iterator it = framenames.begin(),
								 end = framenames.end();
		 it != end; ++it)
	{
