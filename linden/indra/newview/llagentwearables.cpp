									 ...
									 bool rename_clothing)
{
	...
	inventory_func_t func = std::bind(&LLAgentWearables::makeNewOutfitCopy,
									  this, _1, datap);
...
void LLAgentWearables::userAttachMultipleAttachments(LLInventoryModel::item_array_t& objects)
{
	...
	for (S32 i = 0; i < obj_count; ++i)
	{
		...
		const LLInventoryItem* itemp = objects[i].get();
		msg->nextBlockFast(_PREHASH_ObjectData);
		msg->addUUIDFast(_PREHASH_ItemID, itemp->getLinkedUUID());
		msg->addUUIDFast(_PREHASH_OwnerID, itemp->getPermissions().getOwner());
		// Wear at the previous or default attachment point:
		constexpr U8 ADD_TO_DEFAULT_POINT = 0 | ATTACHMENT_ADD;
		msg->addU8Fast(_PREHASH_AttachmentPt, ADD_TO_DEFAULT_POINT);
...
// do not support multi-wearables; do not trust to be accurate.
