...
void LLFloaterPathfindingObjects::onOpen()
{
	...
	if (!mSelectionUpdateSlot.connected())
	{
		mSelectionUpdateSlot =
			gSelectMgr.mUpdateSignal.connect(std::bind(&LLFloaterPathfindingObjects::onInWorldSelectionListChanged,
													   this));
	}
	...
	if (!mRegionBoundaryCrossingSlot.connected())
	{
		mRegionBoundaryCrossingSlot =
			gAgent.addRegionChangedCB(std::bind(&LLFloaterPathfindingObjects::onRegionBoundaryCrossed,
												this));
	}
	...
	{
		mGodLevelChangeSlot =
			gAgent.registerGodLevelChanageListener(std::bind(&LLFloaterPathfindingObjects::onGodLevelChange,
															 this, _1));
	}
...
//mk
	...
	if (showBeacons())
	{
		...
		for (size_t i = 0, count = items.size(); i < count; ++i)
		{
			...
			LLViewerObject* vobjp = gObjectList.findObject(itemp->getUUID());
			if (!vobjp) continue;

			const std::string& name =
				itemp->getColumn(name_col_idx)->getValue().asString();
			gObjectList.addDebugBeacon(vobjp->getPositionAgent(), name,
...
void LLFloaterPathfindingObjects::rebuildObjectsScrollList(bool update_if_needed)
{
	if (!mHasObjectsToBeSelected)
	{
		...
		if (count)
		{
			mObjectsToBeSelected.reserve(count);
			for (size_t i = 0; i < count; ++i)
			{
				LLScrollListItem* itemp = items[i];	// Cannot be NULL. HB
				mObjectsToBeSelected.emplace_back(itemp->getUUID());
			}
		}
	}
...
{
	LLScrollListCtrl::items_vec_t items = mObjectsScrollList->getAllSelected();
	if (items.size() != 1)
	{
	...
	const LLUUID& item_id = items[0]->getUUID();
	LLVector3d tp_loc;
	LLViewerObject* vobjp = gObjectList.findObject(item_id);
	if (!vobjp)
	{
	...
	else
	{
		...
		tp_loc = vobjp->getPositionGlobal();
	}
...
LLPathfindingObjectList::ptr_t LLFloaterPathfindingObjects::getSelectedObjects() const
{
	...
	for (size_t i = 0, count = items.size(); i < count; ++i)
	{
		LLPathfindingObject::ptr_t objectp = findObject(items[i]);
		if (objectp)
		{
...
void LLFloaterPathfindingObjects::onReturnClicked(void* data)
{
	...
	params.functor(std::bind(&LLFloaterPathfindingObjects::handleReturnItemsResponse,
							 self, _1, _2));
...
void LLFloaterPathfindingObjects::onDeleteClicked(void* data)
{
	...
	params.functor(std::bind(&LLFloaterPathfindingObjects::handleDeleteItemsResponse,
							 self, _1, _2));
