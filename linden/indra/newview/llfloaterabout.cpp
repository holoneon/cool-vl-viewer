...
#include "llwindow.h"
#if LL_LINUX
# include "llwindowlinux.h"				// For gXlibThreadSafe and gXWayland
#elif LL_WINDOWS
...
LLFloaterAbout::LLFloaterAbout(const LLSD&)
{
	...
	mRegionChangedConnection =
		gAgent.addRegionChangedCB(std::bind(&LLFloaterAbout::setSupportText,
											this));
}
...
bool LLFloaterAbout::postBuild()
{
	...
	if (gHasCEF)
	{
		text->replaceTextAll("cef", getString("cef"), false);
	}
	else
	{
		text->replaceTextAll("cef\n", "", false);
	}
	if (gHasGstreamer)
	{
	...
	if (gHasVLC)
	{
		text->replaceTextAll("libvlc", getString("libvlc"), false);
	}
	else
	{
		text->replaceTextAll("libvlc\n", "", false);
	}
	if (gHasWebRTC)
	{
		text->replaceTextAll("webrtc", getString("webrtc"), false);
	}
	else
	{
		text->replaceTextAll("webrtc\n", "", false);
	}
	...
	LLMediaCtrl* browserp = getChild<LLMediaCtrl>("tos");
	if (browserp)
	{
		browserp->navigateToLocalPage("tpv", "policy.html");
	}
...
void LLFloaterAbout::setSupportText()
{
	...
	// Text style for release notes hyperlink
	...
	// Text style for viewer paths hyperlink. HB
	LLStyleSP viewer_paths_style(new LLStyle);
	viewer_paths_style->setVisible(true);
	viewer_paths_style->setFontName(LLStringUtil::null);
	// Note: this special SLURL only works when *not* clicked in a web browser
	// (works for our text editor). It opens the "viewer paths" floater. See
	// LLFloaterHandler() in llmediactrl.cpp. HB
	viewer_paths_style->setLinkHREF("secondlife:///app/floater/viewer_paths/open");
	viewer_paths_style->setColor(gSavedSettings.getColor4("HTMLLinkColor"));
...
//mk
	...
	mSupportTextEditor->appendStyledText(getString("ViewerPaths"), false,
										 false, viewer_paths_style);
...
//mk
	...
	ui_str.setArg("[MODEL]", LLGLManager::sGLRenderer);
	text.append(ui_str);
	text += '\n';
	...
	ui_str.setArg("[VERSION]", LLGLManager::sGLVersionString);
	text.append(ui_str);
	text += '\n';

	ui_str = getString("vram");
	ui_str.setArg("[AMOUNT]", llformat("%d", LLGLManager::sVRAM));
	text.append(ui_str);
	text += '\n';
	...
	U32 total_in = gViewerStats.mPacketsInTotal;
	if (total_in > 1.f)
	{
		U32 total_lost = gViewerStats.mPacketsLostTotal;
		ui_str = getString("packets_loss");
		ui_str.setArg("[STATS]",
					  llformat("%u/%u (%.1f%%)", total_lost, total_in,
							    F32(100 * total_lost) / F32(total_in)));
		text.append(ui_str);
		text += '\n';
		total_lost = gViewerStats.mPacketsDroppedTotal;
		ui_str = getString("packets_dropped");
		ui_str.setArg("[STATS]",
					  llformat("%u/%u (%.1f%%)", total_lost, total_in,
							    F32(100 * total_lost) / F32(total_in)));
		text.append(ui_str);
