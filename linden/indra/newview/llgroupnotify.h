...
class LLGroupNotifyBox final : public LLPanel,
							   ...
							   public LLInstanceTracker<LLGroupNotifyBox,
														LLUUID>
{
protected:
...
public:
	...
	// Used by HBViewerAutomation::groupNotificationAcceptOffer()
	bool hasInventory() const						{ return mHasInventory; }
	...
	// Used by HBViewerAutomation::groupNotificationAcceptOffer()
	static void onClickSaveInventory(void* datap);
...
protected:
	...
	LLGroupNotifyBox(LLNotificationPtr notifp, const std::string& subject,
					 const std::string& message, const std::string& from_name,
					 const LLUUID& group_id, const LLUUID& group_insign,
					 const std::string& group_name, time_t timestamp,
					 bool has_inventory, const std::string& inv_name,
	...
	static void onClickGroupInfo(void* datap);

	// For "next" button
	static void onClickNext(void* datap);
