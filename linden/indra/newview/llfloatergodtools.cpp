...
void LLFloaterGodTools::sendGodUpdateRegionInfo()
{
		...
		gAgent.getRegionHost() == mCurrentHost)
	{
		...
		LLMessageSystem* msg = gMessageSystemp;
		msg->newMessage(_PREHASH_GodUpdateRegionInfo);
													   ...
													   const LLSD& response)
{
	...
	msg->addString(_PREHASH_Method, "refreshmapvisibility");
	msg->addUUID(_PREHASH_Invoice, LLUUID::null);
	msg->nextBlock(_PREHASH_ParamList);
	msg->addString(_PREHASH_Parameter, gAgentID.asString());
	gAgent.sendReliableMessage();
	return false;
}
									  ...
									  const LLHost& host)
{
	...
	msg->addUUID(_PREHASH_Invoice, LLUUID::null);
	msg->nextBlock(_PREHASH_ParamList);
	msg->addString(_PREHASH_Parameter, parameter);
