...
:	LLSpatialPartition(gUsePBRShaders ? (U32)LLDrawPoolTerrain::VERTEX_DATA_MASK_PBR
									  : (U32)LLDrawPoolTerrain::VERTEX_DATA_MASK,
					   false, regionp)
{
...
bool LLVOSurfacePatch::updateGeometry(LLDrawable* drawable)
{
	...
	S32 min_comp = S32(mPatchp->getMinComposition());
	S32 max_comp = S32(ceil(mPatchp->getMaxComposition()));
	...
	// center.
	S32 new_base_comp = S32(mPatchp->getMinComposition());
	if (max_comp - min_comp + 1 > 3 &&
		mPatchp->getMinComposition() - min_comp >
			max_comp - mPatchp->getMaxComposition())
	{
