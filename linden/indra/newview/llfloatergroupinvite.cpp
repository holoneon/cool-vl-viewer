...
struct LLFloaterGroupInviteData
{
...
void LLPanelGroupInvite::impl::submitInvitations()
{
	...
	if (mRoleNames)
	{
		...
		if (role_id == gdatap->mOwnerRole && !mConfirmedOwnerInvite)
		{
			...
			gNotifications.add("GenericAlertYesCancel", args, LLSD(),
							   std::bind(&LLPanelGroupInvite::impl::inviteOwnerCallback,
										 this, _1, _2));
	...
	LLScrollListCtrl::items_vec_t items = mInvitees->getAllData();
	for (S32 i = 0, count = items.size(); i < count; ++i)
	{
		LLScrollListItem* itemp = items[i];	// Cannot be NULL. HB
		role_member_pairs[itemp->getUUID()] = role_id;
	}
...
void LLPanelGroupInvite::impl::handleRemove()
{
	...
	LLScrollListCtrl::items_vec_t selection = mInvitees->getAllSelected();
...
{
	LLFloaterGroupInviteData* datap = (LLFloaterGroupInviteData*)userdata;
	if (datap)
	{
		datap->mPanel = new LLPanelGroupInvite(datap->mGroupId, datap->mSelf);
		return datap->mPanel;
	}
	return NULL;
}
	...
	mInvitePanelp(NULL)
{
	...
	LLFloaterGroupInviteData* datap = new LLFloaterGroupInviteData(this,
																   group_id);
	LLCallbackMap::map_t factory_map;
	factory_map["invite_panel"] = LLCallbackMap(createPanel, datap);
	LLUICtrlFactory::getInstance()->buildFloater(this,
												 "floater_group_invite.xml",
												 &factory_map);
	mInvitePanelp = datap->mPanel;
	delete datap;
}
