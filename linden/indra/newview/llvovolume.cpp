...
LLPointer<LLObjectMediaDataClient> LLVOVolume::sObjectMediaClient;
LLPointer<LLObjectMediaNavigateClient> LLVOVolume::sObjectMediaNavigateClient;
uuid_list_t LLVOVolume::sObjectMediaPending;
...
{
public:
	LLMediaDataClientObjectImpl(LLVOVolume* obj, bool is_new)
	:	mObject(obj),
		mNew(is_new)
	{
		mObject->addMDCImpl();
...
void LLVOVolume::markDead()
{
	...
	// Unregister unconditionally, in case mIsHeroProbe is corrupted. HB
...
void LLVOVolume::initSharedMedia()
{
	...
	sObjectMediaClient = new LLObjectMediaDataClient(queue_delay, retry_delay,
													 max_retries, sorted_size,
													 rr_size);
	sObjectMediaNavigateClient =
		new LLObjectMediaNavigateClient(queue_delay, retry_delay, max_retries,
										sorted_size, rr_size);

	if (sObjectMediaPending.empty())
	{
		return;
	}
	// Fetch any media that got queued while media support was off. HB
	for (uuid_list_t::iterator it = sObjectMediaPending.begin(),
							   end = sObjectMediaPending.end();
		 it != end; ++it)
	{
		LLViewerObject* objectp = gObjectList.findObject(*it);
		if (objectp && !objectp->isDead())
		{
			LLVOVolume* volp = objectp->asVolume();
			if (volp)
			{
				volp->requestMediaDataUpdate(true);
			}
		}
	}
	sObjectMediaPending.clear();
}
									 ...
									 LLDataPacker* dp)
{
	...
	if (isSculpted())
	{
		const LLSculptParams* paramsp = getSculptParams();
		if (paramsp)
		{
			sculpt_id = paramsp->getSculptTexture();
			sculpt_type = paramsp->getSculptType();
		}
	}
	...
	if (!dp)
	{
		if (update_type == OUT_FULL)
		{
			...
			if (!success)
			{
				...
				{
					gObjectList.mBlackListedObjects.emplace(getID());
...
void LLVOVolume::updateTextureVirtualSize(bool forced)
{
	...
	for (S32 i = 0; i < num_faces; ++i)
	{
		...
		if (face->mTextureMatrix)
		{
			...
			// batch for this face to turn off texture animation. Do the
			// opposite when the face gets big enough. When a face can be
			// animated, it always has non-null mTextureMatrix pointer defined
			// after the first call to LLVOVolume::animateTextures(), although
			// the animation is not always turned on.
						   ...
						   bool unique_volume)
{
	...
	U8 sculpt_type = volume_params.getSculptType() & LL_SCULPT_TYPE_MASK;
	...
	if (sculpt_type == LL_SCULPT_TYPE_MESH)
	{
		...
		{
			const LLPointer<LLMeshSkinInfo> skin_infop =
				gMeshRepo.getSkinInfo(volume_params.getSculptID(), this);
			if (skin_infop.notNull())
			{
				notifySkinInfoLoaded((LLMeshSkinInfo*)skin_infop.get());
			}
		}
	}
	else if (sculpt_type == LL_SCULPT_TYPE_GLTF)
	{
		// Notify LLGLTFSceneManager about new a glTF object.
		LLGLTFSceneManager::addGLTFObject(this, volume_params.getSculptID());
	}
	else
	{
		// It should be sculptie then
		sculpt();
	}
...
void LLVOVolume::updateSculptTexture()
{
	...
	if (isSculpted() && !isMesh())
	{
		...
		if (paramsp)
		{
			const LLUUID& id = paramsp->getSculptTexture();
			if (id.notNull())
			{
				mSculptTexture =
					LLViewerTextureManager::getFetchedTexture(id, FTT_DEFAULT,
...
#if 0	// Now taken care of in LLViewerFetchedTexture::postCreateTexture(),
		// normally... Keeping this around for now, just in case. HB
...
{
	if (isDead() || mDrawable.isNull() || mDrawable->isDead())
	{
		return false;
	}
...
void LLVOVolume::updateRelativeXform(bool force_identity)
{
	...
	LLDrawable* drawp = mDrawable;

	if (drawp->isState(LLDrawable::RIGGED) && mRiggedVolume.notNull())
	{
	...
	else if (drawp->isActive() || force_identity)
	{
		// Setup relative transforms
		LLQuaternion delta_rot;
		LLVector3 delta_pos;
		// Matrix from local space to parent relative/global space
		if (!force_identity && !drawp->isSpatialRoot())
		{
...
bool LLVOVolume::lodOrSculptChanged(LLDrawable* drawp, bool& update_bounds)
{
	...
	if (new_lod != old_lod || mSculptChanged)
	{
		...
		drawp->setState(LLDrawable::REBUILD_VOLUME);
...
S32 LLVOVolume::setTEMaterialID(U8 te, const LLMaterialID& matidp)
{
	...
	matmgrp->getTE(regionp->getRegionID(), matidp, te,
				   std::bind(&LLVOVolume::setTEMaterialParamsCallbackTE,
							 getID(), _1, _2, _3));
...
void LLVOVolume::requestMediaDataUpdate(bool is_new)
{
	if (sObjectMediaClient)
	{
	...
	{
		// Store this volume Id, in case the user decides to enable media. HB
		sObjectMediaPending.emplace(mID);
	}
}
...
{
	LLReflectionProbeParams* paramsp = getReflectionProbeParams();
	if (paramsp && paramsp->getIsMirror() != is_mirror)
	{
		if (is_mirror)
		{
...
bool LLVOVolume::getReflectionProbeIsMirror() const
{
	const LLReflectionProbeParams* paramsp =
		(const LLReflectionProbeParams*)getReflectionProbeParams();
	return paramsp && paramsp->getIsMirror();
}
...
bool LLVOVolume::isMesh() const
{
	if (isSculpted())
	{
		...
		if (paramsp)
		{
			U8 sculpt_type = paramsp->getSculptType();
...
const LLPointer<LLMeshSkinInfo> LLVOVolume::getSkinInfo() const
{
	if (getVolume())
	{
		return mSkinInfo;
...
{
	return isMesh() && getSkinInfo().notNull();
}
...
{
	// Note: at the moment only volumes can be animated.
	const LLVOVolume* root_volp = getRootEdit()->asVolume();
	return root_volp &&
...
void LLVOVolume::updateRiggingInfo()
{
	...
	const LLPointer<LLMeshSkinInfo> skinp = getSkinInfo();
	if (skinp.isNull()) return;
	...
	for (S32 i = 0, count = volp->getNumVolumeFaces(); i < count; ++i)
	{
		...
		if (skinp->updateRiggingInfo(vol_face))
		{
...
void LLVOVolume::updateReflectionProbePtr(bool recreate)
{
	bool is_probe = isReflectionProbe();
	if (!is_probe || recreate || !gUsePBRShaders)
	{
	...
	if (is_probe && mReflectionProbe.isNull())
	{
		if (LLPipeline::sRenderMirrors && getReflectionProbeIsMirror())
		{
			// Note: mIsHeroProbe is automatically set by this method. HB
			gPipeline.mHeroProbeManager.registerViewerObject(this);
		}
		else
		{
	...
	// Make sure the object cache entry will always be kept visible. HB
	if (mReflectionProbe.notNull())
	{
		LLViewerRegion* regionp = getRegion();
		if (regionp)
		{
			LLVOCacheEntry* entryp = regionp->getCacheEntry(getLocalID(),
															false);
			if (entryp)
			{
				entryp->keepVisible();
...
F32 LLVOVolume::getBinRadius()
{
	...
	if (!isHUDAttachment() &&
		(!new_bin_radius ||
		 mDrawable->mDistanceWRTCamera < alpha_factor().mV[2]))
	{
		for (S32 i = 0, count = mDrawable->getNumFaces(); i < count; ++i)
		{
			LLFace* face = mDrawable->getFace(i);
	...
	else if (alpha_wrap)
	{
		const LLVector3& bounds = getScale();
		radius = llmin(bounds.mV[0], bounds.mV[1], bounds.mV[2]) * 0.5f;
		radius *= 1.f + mDrawable->mDistanceWRTCamera * alpha_factor().mV[1];
		radius += mDrawable->mDistanceWRTCamera * alpha_factor().mV[0];
	}
	else if (shrink_wrap)
	{
	...
	else if (mDrawable->isStatic())
	{
		...
		radius *= 1.f + mDrawable->mDistanceWRTCamera * dist_factor().mV[1];
		radius += mDrawable->mDistanceWRTCamera * dist_factor().mV[0];
	}
							...
							bool rebuild_face_octrees)
{
	...
	for (S32 i = face_begin; i < face_end; ++i)
	{
		...
		if (pos && dst_face.mExtents)
		{
			...
			{
				LLModel::getPerVertexSkinMatrix(weight[j], matp, final_mat);
...
:	LLSpatialPartition((U32)LLVOVolume::VERTEX_DATA_MASK, true, regionp),
...
//mk
	...
	LLDrawable* drawp = facep->getDrawable();
	if (!drawp) return;	// Paranoia

	const LLMatrix4* model_mat = NULL;
	if (!rigged)	// Rigged meshes ignore their model matrix
	{
		if (drawp->isState(LLDrawable::ANIMATED_CHILD))
		{
			model_mat = &drawp->getWorldMatrix();
		}
		else if (drawp->isActive())
		{
			model_mat = &drawp->getRenderMatrix();
		}
		else if (drawp->getRegion())
		{
			model_mat = &(drawp->getRegion()->mRenderMatrix);
		}
#if 1
	...
	bool is_media_tex = facep->hasMedia() ||
						(texp &&
						 texp->getType() == LLViewerTexture::MEDIA_TEXTURE);

	LLMaterial* matp = tep->getMaterialParams().get();
	bool has_alm = !is_media_tex && matp;

	LLGLTFMaterial* rmatp = tep->getGLTFRenderMaterial();
	bool has_pbr = !is_media_tex && rmatp;
	LLFetchedGLTFMaterial* gltfp = rmatp ? rmatp->asFetched() : NULL;

	// When we do have a PBR material, check to see if we also have a legacy
	// material or a non-default diffuse texture as fallbacks for rendering in
	// non-PBR viewers. HB
	bool has_legacy = has_pbr && (has_alm || !tep->isDefault());

	// See if this face pertains to an attachment (used for RLV below) and to
	// one of the agent's attachments (used for materials types accounting). HB
	bool is_attachment = vobjp->isAttachment();
	bool agent_attachment = is_attachment && vobjp->permYouOwner();

//MK
	// If @camtextures is set, do not show any texture in world (but show
	// attachments normally)
	if (!is_media_tex && !is_attachment && gRLenabled &&
		gRLInterface.mContainsCamTextures &&
		gRLInterface.mCamTexturesCustom.notNull())
	{
		// Switch to the cam texture and white color. This replaces the
		// original hack which assigned mCamTexturesCustom to texp, and which
		// did not properly work... HB
		if (!facep->isState(LLFace::USE_RLV_TEXTURE))
		{
			facep->switchDiffuseTex(gRLInterface.mCamTexturesCustom->getID());
			facep->setFaceColor(LLColor4::white);
			facep->setState(LLFace::USE_RLV_TEXTURE);
			gRLInterface.mNeedsVisibilityRefresh = true;
		}
		// Do not show any material, just the diffuse texture. HB
		matp = NULL;
		gltfp = NULL;
	}
	// No @camtextures restriction: restore face texture if needed. HB
	else if (facep->isState(LLFace::USE_RLV_TEXTURE))
	{
		// Reset to diffuse texture and color
		facep->switchDiffuseTex(tep->getID());
		facep->unsetFaceColor();
		facep->clearState(LLFace::USE_RLV_TEXTURE);
		gRLInterface.mNeedsVisibilityRefresh = true;
	}
//mk
	// *HACK: when we have a PBR material and are not rendering in PBR mode,
	...
	// and classic cloud faces, which do not bear a PBR material. HB
	bool has_basecolor = false;
	...
	bool may_touch_diffuse = !is_media_tex && rmatp && !matp &&
							 !gUsePBRShaders;
	// Note: in EE mode, the material might not have been fetched, so we need
	// to use the base material (rmatp) instead of the fetched material (gltfp)
	// for basecolor_id; this may cause discrepancies when the face uses a PBR
	// material override... HB
	const LLUUID& basecolor_id = may_touch_diffuse ?
		(gltfp ? gltfp->getBaseColorId() : rmatp->getBaseColorId()) :
		LLUUID::null;
	...
	if (may_touch_diffuse && use_basecolor &&
		...
		// still be able to see and edit the diffuse texture on PBR-enabled
		// faces. HB
		(!tep->isSelected() || !LLFloaterTools::isVisible()))
	{
		LLColor4 new_color = gltfp ? gltfp->mBaseColor : rmatp->mBaseColor;
		if ((gltfp && gltfp->isOpaque()) || rmatp->isOpaque())
		{
			new_color.mV[VW] = 1.f;
		}
		if (got_base_color_tex && (use_basecolor > 1 || !has_legacy))
		{
			// Set to base color texture and color
			facep->switchDiffuseTex(basecolor_id);
			facep->setFaceColor(new_color);
			has_basecolor = true;
		}
	...
	// Render without the PBR material when we are not in PBR mode. HB
	if (!gUsePBRShaders)
	{
		gltfp = NULL;
	}
	...
	else if (matp)
	{
		...
		// material: legacy materials do not have overrides !  HB
		mat_id = matp->getHash();
	...
	if (dinfop &&
		...
		dinfop->mIsAgentAttachment == agent_attachment &&
		dinfop->mMaterialID == mat_id &&
		dinfop->mHasLegacyFallback == has_legacy &&
		dinfop->mHasALM == has_alm &&
		dinfop->mHasPBR == has_pbr &&
		dinfop->mHasBasecolorFallback == has_basecolor &&
		...
		dinfop->getSkinHash() == facep->getSkinHash())
	{
		...
		if (has_pbr && dinfop->mRootObject.notNull())
		{
			dinfop->mRootObject->setUsePBR();
		}
#if HB_DEBUG_OBJECT
		dinfop->mIsDebugged = facep->mIsDebugged;
#endif
		// The rest of the draw info is up to date. Nothing else to do.
		return;
	}

	// We need a new draw info...
	U32 start = facep->getGeomIndex();
	U32 end = start + facep->getGeomCount() - 1;
	U32 offset = facep->getIndicesStart();
	U32 count = facep->getIndicesCount();
	LLPointer<LLDrawInfo> ndinfop = new LLDrawInfo(start, end, count, offset,
												   texp,
												   facep->getVertexBuffer(),
												   fullbright, bump);
	draw_vec.push_back(ndinfop);

	ndinfop->mVSize = vsize;
	ndinfop->mTextureMatrix = tex_mat;
	ndinfop->mModelMatrix = model_mat;
	ndinfop->mBump = bump;
	ndinfop->mShiny = shiny;

	static const F32 alpha[4] = { 0.f, 0.25f, 0.5f, 0.75f };
	F32 spec = alpha[shiny & TEM_SHINY_MASK];
	ndinfop->mSpecColor.set(spec, spec, spec, spec);
	ndinfop->mEnvIntensity = spec;
	ndinfop->mSpecularMap = NULL;
	ndinfop->mMaterial = matp;
	ndinfop->mGLTFMaterial = gltfp;
	ndinfop->mShaderMask = shader_mask;
	ndinfop->mAvatar = facep->mAvatar;
	ndinfop->mSkinInfo = facep->mSkinInfo;

	// Remember if this face pertains to one of the agent's avatar attachments.
	// HB
	ndinfop->mIsAgentAttachment = agent_attachment;

	// Store material usage stats for this face. HB
	ndinfop->mHasALM = has_alm;
	ndinfop->mHasPBR = has_pbr;
	ndinfop->mHasLegacyFallback = has_legacy;
	ndinfop->mHasBasecolorFallback = has_basecolor;
	ndinfop->mRootObject = vobjp->getRootEdit();
	if (has_pbr)
	{
		ndinfop->mRootObject->setUsePBR();
	}

	if (gltfp)
	{
		// Just remember the material Id; render pools will reference the PBR
		// material.
		ndinfop->mMaterialID = mat_id;
	}
	else if (matp)
	{
		...
		if (matp->getSpecularID().notNull())
		{
			...
			ndinfop->mSpecColor.set(spec_col.mV[0] * ONE255TH,
									spec_col.mV[1] * ONE255TH,
									spec_col.mV[2] * ONE255TH, alpha);

			ndinfop->mEnvIntensity = matp->getEnvironmentIntensity() *
									 ONE255TH;
			ndinfop->mSpecularMap = vobjp->getTESpecularMap(te_offset);
		}
		ndinfop->mAlphaMaskCutoff = matp->getAlphaMaskCutoff() * ONE255TH;
		ndinfop->mDiffuseAlphaMode = matp->getDiffuseAlphaMode();
		ndinfop->mNormalMap = vobjp->getTENormalMap(te_offset);
	}
	else if (type == LLRenderPass::PASS_GRASS)
	{
		ndinfop->mAlphaMaskCutoff = 0.5f;
	}
	else
	{
		ndinfop->mAlphaMaskCutoff = 0.33f;
	}
#if 0	// Always populate the ndinfop
	if (type == LLRenderPass::PASS_ALPHA)
#endif
	{
		facep->setDrawInfo(ndinfop);	// For alpha sorting
	}
	ndinfop->mExtents[0] = facep->mExtents[0];
	ndinfop->mExtents[1] = facep->mExtents[1];

	if (index < FACE_DO_NOT_BATCH_TEXTURES)
	{
		// Initialize texture list for texture batching
		ndinfop->mTextureList.resize(index + 1);
		ndinfop->mTextureListVSize.resize(index + 1);
		ndinfop->mTextureList[index] = texp;
		ndinfop->mTextureListVSize[index] = vsize;
	}
#if HB_DEBUG_OBJECT
	ndinfop->mIsDebugged = facep->mIsDebugged;
#endif
	ndinfop->validate();
}
...
// Helper function for transparency tests during rendering. HB
...
void LLVolumeGeometryManager::rebuildGeom(LLSpatialGroup* groupp)
{
	...
	if (bridgep)
	{
		LLViewerObject* vobjp =
			bridgep->mDrawable ? bridgep->mDrawable->getVObj().get() : NULL;
		if (vobjp)
		{
			if (bridgep->mAvatar.isNull())
			{
				bridgep->mAvatar = vobjp->getAvatar();
			}
			voattachavp = vobjp->getAvatarAncestor();
			vovolp = vobjp->asVolume();
		}
		voavp = bridgep->mAvatar;
	}
	if (voattachavp)
	{
...

	{
			 ...
			 it != groupp->getDataEnd(); ++it)
		{
...
#if 0		// We do need this info also for the material indicator and for
			// the base color replacement hack in EE mode. HB
			if (gUsePBRShaders)
#endif
			{
			...
			const LLPointer<LLMeshSkinInfo> skin_infop =
				is_mesh ? vobjp->getSkinInfo() : nullptr;
			if (skin_infop.notNull())
			{
			...
			for (S32 i = 0, count = drawablep->getNumFaces(); i < count; ++i)
			{
				...
				if (gUsePBRShaders)
				{
					...
					if (gltfp)
					{
						LLFetchedGLTFMaterial* fmatp = gltfp->asFetched();
						if (fmatp)
						{
#if 0						// Do not do this: we still want to be able to
							// edit those textures, even for faces with a PBR
							// material. Also, DIFFUSE_MAP must be preserved in
							// case this is a media texture anyway. HB
							// Tell texture streaming system to ignore legacy
							// textures.
							facep->setTexture(LLRender::DIFFUSE_MAP, NULL);
							facep->setTexture(LLRender::NORMAL_MAP, NULL);
							facep->setTexture(LLRender::SPECULAR_MAP, NULL);
#endif
							// Let texture streaming system know about PBR
							// textures
							facep->setTexture(LLRender::BASECOLOR_MAP,
											  fmatp->mBaseColorTexture);
							facep->setTexture(LLRender::GLTF_NORMAL_MAP,
											  fmatp->mNormalTexture);
							facep->setTexture(LLRender::METALLIC_ROUGHNESS_MAP,
											  fmatp->mMetallicRoughnessTexture);
							facep->setTexture(LLRender::EMISSIVE_MAP,
											  fmatp->mEmissiveTexture);
						}
					}
				}
				...
				if (facep->hasGeometry())
				{
...
#if 0				// Fixed in gPipeline.getPoolTypeFromTE(). HB
					if (gltfp &&
					...
					else
					{
						...
						if (use_wl_shaders)
						{
							...
							{
								// Do not render if fully transparent and not
								// highlighting transparent faces.
								if (debugging_alpha ||
									gltfp->mAlphaMode !=
										LLGLTFMaterial::ALPHA_MODE_BLEND ||
									gltfp->mBaseColor.mV[3] >= 0.001f)
								{
										  ...
										  bool batch_textures, bool rigged)
{
	...
	if (gUsePBRShaders)
	{
		tex_idx_channels = LLShaderMgr::sIndexedTextureChannels;
	}
	...
	else if (LLGLManager::sGLSLVersionMajor > 1 ||
			 LLGLManager::sGLSLVersionMinor >= 30)
	{
		// - 1 to always reserve one for shiny for now just for simplicity
		tex_idx_channels = LLShaderMgr::sIndexedTextureChannels - 1;
	}
...
//mk
		...
		while (face_iter < i)
		{
			...
			if (gUsePBRShaders)
			{
				// Do not render objects which act as a handle for a glTF
				// scene, unless we are in build mode. HB
				if (vobjp && vobjp->mGLTFAsset.notNull() &&
...
//mk
			...
			{
				// Shiny
				if (texp && texp->getPrimaryFormat() == GL_ALPHA)
				{
					// Invisiprim + shiny
					bool do_it = true;
					if (gUsePBRShaders)
			...
			{
				if (!is_alpha && texp && texp->getPrimaryFormat() == GL_ALPHA)
				{
					// Invisiprim
					bool do_it = true;
					if (gUsePBRShaders)
