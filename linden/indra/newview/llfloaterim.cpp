...
strings_map_t LLFloaterIM::sMsgStringsMap;
	...
	mProfileButtonEnabled(true)
{
	...
	// On Agni, we may get an empty voice channel info for voice servers: reset
...
void LLFloaterIMSession::lookupName()
{
	LLAvatarNameCache::get(mOtherParticipantUUID,
						   std::bind(&LLFloaterIMSession::onAvatarNameLookup,
									 _1, _2, (void*)this));
}
...
void LLFloaterIMSession::markAsBot()
{
	childShow("bot_text");
}
...
void LLFloaterIMSession::processSessionUpdate(const LLSD& session_update)
{
		...
		session_update["moderated_mode"].has("voice"))
	{
		...
		std::string title = mSessionLabel;
		if (voice_moderated)
		{
			title += " " + getString("moderated_chat_label");
		}
		setTitle(title);
