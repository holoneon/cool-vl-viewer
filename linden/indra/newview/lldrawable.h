...
public:
	...
	LLDrawable(LLViewerObject* vobjp, bool new_entry = false);
	...
	LL_INLINE const LLViewerObject* getVObj() const		{ return mVObjp.get(); }
	LLVOVolume*	getVOVolume() const; // cast mVObjp to LLVOVolume if OK
	...
	LL_INLINE void setLit(bool lit)						{ lit ? clearState(UNLIT)
															  : setState(UNLIT); }
