...
#include "llagent.h"
#include "llappviewer.h"			// For gMainloopWorkp
...
bool LLEnvironmentRequest::initiate(LLEnvironment::env_apply_fn cb)
{
	...
	if (!regionp->capabilitiesReceived())
	{
		llinfos << "Deferring environment settings request until capabilities are received."
				<< llendl;
		regionp->setCapsReceivedCB(std::bind(&LLEnvironmentRequest::onRegionCapsReceived,
											 _1, cb));
		return false;
	}
...
bool LLEnvironmentRequest::doRequest(LLEnvironment::env_apply_fn cb)
{
	...
	gCoros.launch("LLEnvironmentRequest::environmentRequestCoro",
				  std::bind(&LLEnvironmentRequest::environmentRequestCoro, url,
							cb));
	return true;
}
...
public:
		...
		mCycleOffset(cycleoffset)
	{
		...
		setOnFinished(std::bind(&LLTrackBlenderLoopingTime::onFinishedSpan,
								this));
	}
	...
	void switchTrack(S32 trackno, F32)
	{
		...
		F64 now = getAdjustedNow() + (F64)LLEnvironment::TRANSITION_ALTITUDE;
...
public:
	bool operator()(const LLDispatcher*, const std::string&,
...
public:
						 ...
						 F64 transition)
	{
			 ...
			 ++it)
		{
			...
			{
				if (transition != (F64)LLEnvironment::TRANSITION_INSTANT)
				{
					typename Injection::ptr_t injection =
...
private:
	...
	typedef std::map<std::string, LLUUID, std::less<> > key_to_expid_t;
...
:	LLEnvironment::DayInstance(env)
{
	...
	mEnvChangeConnection =
		gEnvironment.setEnvironmentChanged(std::bind(&LLDayInjection::onEnvironmentChanged,
													 this, _1));
	mParcelChangeConnection =
		gViewerParcelMgr.addAgentParcelChangedCB(std::bind(&LLDayInjection::onParcelChange,
														   this));
}
									 ...
									 F64 transition_time)
{
	...
	if (transition_time == (F64)LLEnvironment::TRANSITION_INSTANT &&
...
const LLUUID LLEnvironment::KNOWN_SKY_PBR_MIDDAY("c46226b4-0e43-5a56-9708-d27ca1df3292");
...
void LLEnvironment::initClass()
{
	...
	mRegionChangedConnection =
		gAgent.addRegionChangedCB(std::bind(&LLEnvironment::onRegionChange,
											this));
	mParcelChangedConnection =
		gViewerParcelMgr.addAgentParcelChangedCB(std::bind(&LLEnvironment::onParcelChange,
														   this));
	mPositionChangedConnection =
		gAgent.setPosChangeCallback(std::bind(&LLEnvironment::onAgentPositionHasChanged,
											  this, _1));
	...
	pump.listen(LISTENER_NAME,
				std::bind(&LLEnvironment::listenExperiencePump, this, _1));
...
void LLEnvironment::onRegionChange()
{
	...
	regionp->setCapsReceivedCB(std::bind(&LLEnvironment::requestRegion,
										 env_apply_fn()));
}
									  ...
									  S32 env_version)
{
	...
	if (!dayp)
	{
#if 1	// Problem: loading the default day cycle also loads the default
		// Windlight sky and water settings... HB
		envp->setDay(LLEnvSettingsDay::buildDefaultDayCycle(), 14400, 0);
		dayp = envp->getDayCycle();
#else	// Sadly, this does not work. FIXME. HB
		LLSD day_cycle = LLSettingsDay::buildCycleFromFixed(skyp, waterp);
		LLEnvSettingsDay::ptr_t cyclep =
			std::make_shared<LLEnvSettingsDay>(day_cycle);
		cyclep->initialize();
		envp->setDay(cyclep->buildClone(), 14400, 0);
		dayp = envp->getDayCycle();
		skyp = envp->getSky();
		waterp = envp->getWater();
#endif
	}
...
//static
void LLEnvironment::setMiddayPBR()
{
	static LLCachedControl<std::string> asset_id(gSavedSettings,
												 "PBRSkyMiddayAssetId");
	if (gAgent.hasExtendedEnvironment())
	{
		gEnvironment.setEnvironment(ENV_LOCAL, LLUUID(asset_id()),
									TRANSITION_INSTANT);
		gEnvironment.setSelectedEnvironment(ENV_LOCAL, TRANSITION_INSTANT);
	}
}
								   ...
								   S32 transition)
{
	...
	LLEnvSettingsBase::getSettingsAsset(asset_id,
										std::bind(&LLEnvironment::onSetEnvAssetLoaded,
												  this, env, transition, _1,
												   _2, _3));
}
...
void LLEnvironment::updateEnvironment(F64 transition, bool forced)
{
	...
	if (transition == (F64)TRANSITION_INSTANT)
	{
												 ...
												 const LLSettingsBase::ptr_t& settings)
{
	...
	// "Fix reflections blue hue" feature, for PBR rendering with legacy
	// (EE/WL) sky settings. HB
	...
	bool do_desaturate = gUsePBRShaders && desaturate &&
						 // The desaturation only applies to legacy (EE/WL)
						 // sky settings. HB
						 LLRender::sClassicMode &&
						 // Do not apply this fix to mirrors; we do not want
						 // desaturating the mirror reflections since the
						 // reflected scene has already been desaturated. HB
						 !gPipeline.mHeroProbeManager.isMirrorPass();

	bool is_irradiance = gPipeline.mReflectionMapManager.isRadiancePass();
...
void LLEnvironment::dirtyUniforms()
{
	...
	for (U32 i = 0, count = shaders.size(); i < count; ++i)
	{
			 ...
			 shaderp->mShaderGroup == LLGLSLShader::SG_WATER))
		{
			...
			if (shaderp->mRiggedVariant)
			{
				shaderp->mRiggedVariant->mUniformsDirty = true;
			}
		}

		for (size_t j = 0, vcount = shaderp->mGLTFVariants.size(); j < vcount;
			 ++j)
		{
			shaderp->mGLTFVariants[j].mUniformsDirty = true;
		}
	}
}
...
void LLEnvironment::requestParcel(S32 parcel_id, env_apply_fn cb)
{
	...
	gCoros.launch("LLEnvironment::coroRequestEnvironment",
				  std::bind(&LLEnvironment::coroRequestEnvironment, this,
							parcel_id, cb));
}
								 ...
								 env_apply_fn cb)
{
	...
	gCoros.launch("LLEnvironment::coroUpdateEnvironment",
				  std::bind(&LLEnvironment::coroUpdateEnvironment, this,
...
void LLEnvironment::resetParcel(S32 parcel_id, env_apply_fn cb)
{
	gCoros.launch("LLEnvironment::coroResetEnvironment",
				  std::bind(&LLEnvironment::coroResetEnvironment, this,
...
void LLEnvironment::coroRequestEnvironment(S32 parcel_id, env_apply_fn apply)
{
	...
	if (!gMainloopWorkp || !apply || !result.has(KEY_ENVIRONMENT))
	{
		return;
	}
	// Post environment change to main thread
	const LLSD& environment = result[KEY_ENVIRONMENT];
	gMainloopWorkp->post([=]()
						{
							envinfo_ptr_t envinfo =
								EnvironmentInfo::extract(environment);
...
LLEnvironment::envinfo_ptr_t LLEnvironment::EnvironmentInfo::extract(const LLSD& env)
{
...
LLEnvironment::envinfo_ptr_t
	LLEnvironment::EnvironmentInfo::extractLegacy(const LLSD& legacy)
{
...
LLSettingsWater::ptr_t
	LLEnvironment::createWaterFromLegacyPreset(const std::string& filename,
											   LLSD& messages)
{
	std::string name = LLDir::getBaseFileName(filename, true);
	std::string file = LLDir::getBaseFileName(filename);
	std::string path = LLDir::getDirName(filename);
