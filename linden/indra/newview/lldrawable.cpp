...
void LLDrawable::markDead()
{
	...
	// Hold a LLPointer on 'this' to prevent it from getting destroyed during
	// this method execution, which may happen in cleanupReferences() for
	// spatial brigdes (detected by ASAN on viewer shutdown)... HB
	LLPointer<LLDrawable> drawablep = this;
	// Mark ourselves dead now.
	drawablep->setState(DEAD);
...
bool LLDrawable::updateMove()
{
	...
	if (isState(MOVE_UNDAMPED) ||
		// Force undamped movement for attached objects in mouse-look to
		// prevent animation bork for linkset with animated parts.
		(gAgent.cameraMouselook() && !isRoot() && !mVObjp->isRiggedMesh() &&
		 mVObjp->getAvatar() && mVObjp->getAvatar()->isSelf()))
	{
		return updateMoveUndamped();
	}

	return updateMoveDamped();
}
