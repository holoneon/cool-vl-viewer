...
//mk
	...
	if (parcelp)
	{
		mNameEditor->setText(parcelp->getName());
		mDescEditor->setText(parcelp->getDesc());
...
//virtual
void LLPanelPick::processProperties(S32 type, void* datap)
{
	...
	LLAvatarPickInfo* infop = (LLAvatarPickInfo*)datap;
	if (infop->mPickId != mPickID)
	{
		return;	// Not for us.
	}
	...
	std::string location_text = createLocationText(infop->mUserName,
												   infop->mParcelName,
												   mSimName,
												   mPosGlobal);
	...
	mEnabledCheck->set(infop->mEnabled);

	mSortOrderEditor->setText(llformat("%d", infop->mSortOrder));
}
...
//static
void LLPanelPick::onClickTeleport(void* datap)
{
	LLPanelPick* self = (LLPanelPick*)datap;
...
//static
void LLPanelPick::onClickMap(void* datap)
{
	LLPanelPick* self = (LLPanelPick*)datap;
...
//static
void LLPanelPick::onClickSetLocation(void* datap)
{
//MK
	if (gRLenabled && gRLInterface.mContainsShowloc)
	{
...
//static
void LLPanelPick::onCommitAny(LLUICtrl*, void* datap)
{
	LLPanelPick* self = (LLPanelPick*)datap;
	...
	if (self && self->mDataReceived)
	{
		...
		LLTabContainer* tabp = (LLTabContainer*)self->getParent();
		if (tabp)
		{
			tabp->setCurrentTabName(self->mNameEditor->getText());
		}
	}
}
