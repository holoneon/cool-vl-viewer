...
bool join_group_response(const LLSD& notification, const LLSD& response)
{
	...
	else if (use_cap)
	{
		...
		gCoros.launch("groupInvitationResponse",
					  std::bind(&join_group_response_coro, url, group_id,
								accept_invite));
	}
	else
	{
	...
	switch (request_type)
	{
		case REQUEST_GET:
			gCoros.launch("LLGroupMgr::getGroupBanRequestCoro",
						  std::bind(&LLGroupMgr::getGroupBanRequestCoro,
									cap_url, group_id));
			break;

		case REQUEST_POST:
			gCoros.launch("LLGroupMgr::postGroupBanRequestCoro",
						  std::bind(&LLGroupMgr::postGroupBanRequestCoro,
...
void LLGroupMgr::sendCapGroupMembersRequest(const LLUUID& group_id)
{
	...
	gCoros.launch("LLGroupMgr::groupMembersRequestCoro",
				  std::bind(&LLGroupMgr::groupMembersRequestCoro, cap_url,
							group_id));
}
