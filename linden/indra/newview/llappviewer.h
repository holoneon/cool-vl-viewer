...
// reenter glib (in the render loop, via LLWindowLinux) in a glib callback, so
// I decided to keep it disabled. HB
...
public:
	LLAppViewer();
	~LLAppViewer() override;

	//
	// Main application logic
	//

	// LLApp overrides (pure virtuals in LLApp).
	InitState init() override;		// Override for application initialization
	bool cleanup() override;		// Override for application cleanup
	bool mainLoop() override;

	// Exit codes for the viewer program (useful when scripting unattended
	// viewer operations with bot avatars, for example). HB
	enum EExitCode
	{
		EXIT_OK = 0,
		EXIT_INIT_FAILED,
	...
	LL_INLINE virtual bool probeVulkan(std::string& v)	{ return false; }
	...
	virtual void handleLoginComplete();
	...
	bool isSavedMACValid() const						{ return mSavedMACValid; }
	...
	// Sends a logout request to the simulator,without creating a marker file:
	// meant for lost network case, or forced shutdowns, to try and properly
	// logout the agent from the grid (avoids lost attachments states and a
	// "ghost" agent in the presence server).
	static void sendSimpleLogoutRequest();
...
protected:
	bool initWindow();	// Initialize the viewer's window.
	...
	LL_INLINE virtual bool initAppMessagesHandler()		{ return false; }

	LL_INLINE virtual bool sendURLToOtherInstance(const std::string& url)
	{
		return false;
	}
...
private:
	...
	// Was the saved cyphered MAC address valid ?
	bool						mSavedMACValid;
...
// Plugin presence flags
extern bool					gHasCEF;
extern bool					gHasGstreamer;
extern bool					gHasVLC;
extern bool					gHasWebRTC;
