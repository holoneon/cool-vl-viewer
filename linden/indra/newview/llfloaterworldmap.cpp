...
bool LLFloaterWorldMap::postBuild()
{
	...
	mTeleportArrivingConnection =
		gViewerParcelMgr.setTPArrivingCallback(std::bind(&LLFloaterWorldMap::onTeleportArriving));
...
void LLFloaterWorldMap::onOpen()
{
	...
	// In OpenSim, the minimum altitude can vary and go below 0.
	if (!gIsInSecondLife)
	{
		LLViewerRegion* regionp = gAgent.getRegion();
		if (regionp)
		{
			mSpinZ->setMinValue(regionp->getMinSimHeight());
		}
	}
}
...
{
	LLSimInfo* sim_infop = gWorldMap.simInfoFromName(region_name);
	// In OpenSim, the minimum altitude can vary and go below 0.
	F32 min_z = 0.f;
	if (!gIsInSecondLife)
	{
		...
		if (regionp)
		{
			min_z = regionp->getMinSimHeight();
		}
	}
	F32 altitude = llclamp((F32)z_coord, min_z, 4096.f);
	if (sim_infop)
	{
		LLVector3 local_pos;
		local_pos.mV[VX] = (F32)x_coord;
		local_pos.mV[VY] = (F32)y_coord;
		local_pos.mV[VZ] = altitude;
		LLVector3d global_pos = sim_infop->getGlobalPos(local_pos);
		trackLocation(global_pos);
		setDefaultBtn(mTeleportButton);
		// Force an update of the number of agents in this sim
		sim_infop->mAgentsUpdateTime = 0.0;
	}
	else
	{
		...
		mSpinZ->setValue(LLSD(altitude));
		LLPanelWorldMap::setDefaultZ(altitude);
...
void LLFloaterWorldMap::onTabChanged(void* userdata, bool from_click)
{
	...
	if (self)
	{
		...
		if (first_tab_change)
		{
			...
			doAfterInterval(std::bind(&force_reload_tiles, index), delay);
		}
	}
}
