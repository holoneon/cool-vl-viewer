...
# Cool VL Viewer wrapper script v3.68 (c)2007-2026 Henri Beauchamp.
...
# programs, so to avoid polluting them with libraries such as libjemalloc.so
# (they may be incompatible with it) or libcef.so (which is badly linked and
# re-exports functions from static libraries it got linked with, such as
# libnssutil.a). SAVED_LD_PRELOAD is also used by the plugin launcher code,
# in order to retore the system value when not launching a CEF plugin. Finally,
# both SAVED_LD_PRELOAD and SAVED_LD_LIBRARY_PATH are used by HBExternalEditor.
...
export LD_LIBRARY_PATH="$run_path/lib"${LD_LIBRARY_PATH:+":$LD_LIBRARY_PATH"}
...
if [ "$1" == "--check-libs" ] ; then
	...
	cando="yes"
	if ldd $viewer_binary 2>&1 | grep -q 'not ' ; then
...
# Check for ALSA sound configuration, so that WebRTC can find and list all ALSA
# devices.
if ! [ -f ~/.asoundrc ] ; then
	touch ~/.asoundrc
fi
if ! grep -q "defaults.namehint.extended on" ~/.asoundrc ; then
	echo "defaults.namehint.extended on" >> ~/.asoundrc
	echo "Configured ALSA to list audio device extended name hints."
fi
if ! grep -q "defaults.namehint.showall on" ~/.asoundrc ; then
	echo "defaults.namehint.showall on" >> ~/.asoundrc
	echo "Configured ALSA to list all audio device name hints."
fi

# If the user did not already override it, use our custom fontconfig
# configuration for CEF so to exclude bitmap and OTB fonts which CEF now (as of
# v142) fails to render at all. When it exists, CEF_FONTCONFIG_FILE will be
# copied in FONFTCONFIG_FILE by LLPluginProcessParent::init() into the
# environment of the plugin process.
if [ "$FONTCONFIG_FILE" == "" ] ; then
	file="$run_path/fonts/fonts-no-bitmap.conf"
	# fonts/fonts-no-bitmap.conf includes /etc/fonts/fonts.conf and only adds
	# its overrides to it, so we need it to exist on the system.
	if ! [ -f /etc/fonts/fonts.conf ] ; then
		echo "WARNING: could not find /etc/fonts/fonts.conf on this system; cannot override it, which might cause failed fonts rendering in Dullahan/CEF."
	elif [ -f "$file" ] ; then
		export CEF_FONTCONFIG_FILE="$file"
	else
		echo "WARNING: missing $file file, which might cause failed fonts rendering in Dullahan/CEF."
	fi
fi
...
# work properly).
if [ -f "$run_path/lib/libjemalloc.so.2" ] ; then
	export LD_PRELOAD="$run_path/lib/libjemalloc.so.2"${LD_PRELOAD:+" $LD_PRELOAD"}
fi
...
export LD_PRELOAD=${LD_PRELOAD:+"$LD_PRELOAD "}"$run_path/lib/libcef.so"

# Run under gdb when --debug was passed as the first option in the command
# line. If you have a symbols file in bin/ then it will be auto-loaded as well.
...
if [ "$LL_REPORT" != "" ] ; then
	...
	if [ -f "$logs_dir/SecondLife.exec_marker" ] ; then
		...
		if which lsof &>/dev/null ; then
			lsof | grep -q "$logs_dir/CoolVLViewer.log"
			if (( $? == 0 )); then
...
function report() {
	...
	if [ "$dialogcmd" == "Xdialog" ]; then
		...
		elif [ "$logfile" != "" ] && grep -qa " WARNING: " "$logfile" ; then
