...
void LLFloaterMute::refreshMuteList()
{
	...
	LLScrollListItem* itemp = mMuteList->getFirstSelected();
	LLUUID selected_id;
	if (itemp)
	{
		 ...
		 it != mutes.end(); ++it)
	{
		...
		LLScrollListItem* itemp = mMuteList->addStringUUIDItem(text, id);
		if (itemp && gObjectList.findObject(id))
		{
			LLScrollListText* text_cell =
				dynamic_cast<LLScrollListText*>(itemp->getColumn(0));
			if (text_cell)
			{
...
void LLFloaterMute::selectMute(const std::string& name)
{
	...
	LLScrollListCtrl::items_vec_t data = self->mMuteList->getAllData();
	std::string label;
	LLUUID id;
	for (S32 i = 0, count = data.size(); i < count; ++i)
	{
		LLScrollListItem* itemp = data[i];	// Cannot be NULL. HB
		id = itemp->getUUID();
		LLMute mute(id);
		label = itemp->getColumn(0)->getValue().asString();
		mute.setFromDisplayName(label); // trims off the suffix from mute.mName
		if (mute.mName == name)
		{
			self->mMuteList->selectItem(itemp);
			self->mMuteList->scrollToShowSelected();
			break;
		}
	}
