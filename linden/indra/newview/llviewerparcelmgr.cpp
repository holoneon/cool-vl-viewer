...
bool LLViewerParcelMgr::allowAgentBuild(const LLParcel* parcelp) const
{
	return parcelp->getAllowModify();
}
...
bool LLViewerParcelMgr::allowAgentVoice(const LLViewerRegion* regionp,
										const LLParcel* parcelp) const
{
	return regionp && parcelp && regionp->isVoiceEnabled() &&
		   parcelp->getParcelFlagAllowVoice();
}

bool LLViewerParcelMgr::allowAgentFly(const LLViewerRegion* regionp,
									  const LLParcel* parcelp) const
{
	return regionp && parcelp && !regionp->getBlockFly() &&
           parcelp->getAllowFly();
}

// Can the agent be pushed around by LLPushObject ?
bool LLViewerParcelMgr::allowAgentPush(const LLViewerRegion* regionp,
									   const LLParcel* parcelp) const
{
	return regionp && parcelp && !regionp->getRestrictPushObject() &&
		   !parcelp->getRestrictPushObject();
}

bool LLViewerParcelMgr::allowAgentScripts(const LLViewerRegion* regionp,
										  const LLParcel* parcelp) const
{
	...
	// technically correct.
	return regionp && parcelp && parcelp->getAllowOtherScripts() &&
		   !regionp->getRegionFlag(REGION_FLAGS_SKIP_SCRIPTS) &&
		   !regionp->getRegionFlag(REGION_FLAGS_ESTATE_SKIP_SCRIPTS);
}

bool LLViewerParcelMgr::allowAgentDamage(const LLViewerRegion* regionp,
										 const LLParcel* parcelp) const
{
	return (regionp && regionp->getAllowDamage()) ||
		   (parcelp && parcelp->getAllowDamage());
}

bool LLViewerParcelMgr::isOwnedAt(const LLVector3d& pos_global) const
{
	LLViewerRegion* regionp = gWorld.getRegionFromPosGlobal(pos_global);
	if (!regionp) return false;

	LLViewerParcelOverlay* overlayp = regionp->getParcelOverlay();
	if (!overlayp) return false;

	LLVector3 pos_region = regionp->getPosRegionFromGlobal(pos_global);

	return overlayp->isOwned(pos_region);
}
...
bool LLViewerParcelMgr::isOwnedSelfAt(const LLVector3d& pos_global) const
{
	...
	return overlayp->isOwnedSelf(pos_region);
}
...
bool LLViewerParcelMgr::isOwnedOtherAt(const LLVector3d& pos_global) const
{
	...
	return overlayp->isOwnedOther(pos_region);
}
...
bool LLViewerParcelMgr::isSoundLocal(const LLVector3d& pos_global) const
{
	...
	return overlayp->isSoundLocal(pos_region);
}
...
{
	LLViewerRegion* regionp = gWorld.getRegionFromPosGlobal(pos_global);
	if (!gAgent.getRegion() || regionp != gAgent.getRegion())
	{
...
void LLViewerParcelMgr::sendParcelGodForceOwner(const LLUUID& owner_id)
{
	...
	llinfos << "Region " << regionp->getOriginGlobal() << llendl;

	LLSD payload;
	payload["owner_id"] = owner_id;
	payload["parcel_local_id"] = mCurrentParcel->getLocalID();
	payload["region_host"] = regionp->getHost().getIPandPort();
																	...
																	bool remove_contribution)
{
	...
	if (is_claim)
	{
		...
		if (regionp != gWorld.getRegionFromPosGlobal(east_north_region_check))
		{
	...
	ParcelBuyInfo* infop = new ParcelBuyInfo;

	infop->mAgent = agent_id;
	infop->mSession = session_id;
	infop->mGroup = group_id;
	infop->mIsGroupOwned = is_group_owned;
	infop->mIsClaim = is_claim;
	infop->mRemoveContribution = remove_contribution;
	infop->mHost = regionp->getHost();
	infop->mPrice = mCurrentParcel->getSalePrice();
	infop->mArea = mCurrentParcel->getArea();

	if (!is_claim)
	{
		infop->mParcelID = mCurrentParcel->getLocalID();
	}
	else
	{
		// *FIXME: make it work for cross-region selections
		LLVector3 west_south_bottom_region =
			regionp->getPosRegionFromGlobal(mWestSouth);
		LLVector3 east_north_top_region =
			regionp->getPosRegionFromGlobal(mEastNorth);

		infop->mWest = west_south_bottom_region.mV[VX];
		infop->mSouth = west_south_bottom_region.mV[VY];
		infop->mEast = east_north_top_region.mV[VX];
		infop->mNorth = east_north_top_region.mV[VY];
	}

	return infop;
}

void LLViewerParcelMgr::sendParcelBuy(ParcelBuyInfo* infop)
{
	LLMessageSystem* msg = gMessageSystemp;
	msg->newMessage(infop->mIsClaim ? _PREHASH_ParcelClaim
									: _PREHASH_ParcelBuy);
	msg->nextBlock(_PREHASH_AgentData);
	msg->addUUID(_PREHASH_AgentID, infop->mAgent);
	msg->addUUID(_PREHASH_SessionID, infop->mSession);
	msg->nextBlock(_PREHASH_Data);
	msg->addUUID(_PREHASH_GroupID, infop->mGroup);
	msg->addBool(_PREHASH_IsGroupOwned, infop->mIsGroupOwned);
	if (!infop->mIsClaim)
	{
		msg->addBool(_PREHASH_RemoveContribution, infop->mRemoveContribution);
		msg->addS32(_PREHASH_LocalID, infop->mParcelID);
	}
	msg->addBool(_PREHASH_Final, true);	// Do not allow escrow buys
	if (infop->mIsClaim)
	{
		msg->nextBlock(_PREHASH_ParcelData);
		msg->addF32(_PREHASH_West,  infop->mWest);
		msg->addF32(_PREHASH_South, infop->mSouth);
		msg->addF32(_PREHASH_East,  infop->mEast);
		msg->addF32(_PREHASH_North, infop->mNorth);
	}
	else // ParcelBuy
	{
		msg->nextBlock(_PREHASH_ParcelData);
		msg->addS32(_PREHASH_Price, infop->mPrice);
		msg->addS32(_PREHASH_Area, infop->mArea);
	}
	msg->sendReliable(infop->mHost);
}
...
void LLViewerParcelMgr::makeLandmarkAtSelection()
{
	...
	LLViewerRegion* regionp = gWorld.getRegionFromPosGlobal(global_center);
	...
	buffer = llformat("%s in %s (%d, %d)", "My land",
					  regionp->getName().c_str(), pos_x, pos_y);
	name.assign(buffer);
...
void LLViewerParcelMgr::sendParcelPropertiesUpdate(LLParcel* parcelp)
{
	if (!parcelp) return;

	LLViewerRegion* regionp = gWorld.getRegionFromID(parcelp->getRegionID());
	if (!regionp) return;

	LLSD body;
	const std::string& url = regionp->getCapability("ParcelPropertiesUpdate");
	if (url.empty())
	{
		LLMessageSystem* msg = gMessageSystemp;
	...
	// properties update call to complete first). HB
	if (gStatusBarp && parcelp->getLocalID() == mAgentParcel->getLocalID())
	{
...
bool LLViewerParcelMgr::requestParcelProperties(const LLVector3d& pos, S32 id)
{
	...
	LLVector3 wsb_region = regionp->getPosRegionFromGlobal(pos);
...
void LLViewerParcelMgr::processParcelOverlay(LLMessageSystem* msg, void**)
{
	...
	if (regionp)
	{
		regionp->mParcelOverlay->uncompressLandOverlay(sequence_id,
													   sPackedOverlay);
	}
}
...
void LLViewerParcelMgr::processParcelProperties(LLMessageSystem* msg, void**)
{
	...
	// Variable region size support
	LLViewerRegion* regionp = gWorld.getRegion(msg->getSender());
	if (regionp)
	{
		gViewerParcelMgr.mParcelsPerEdge = (S32)(regionp->getWidth() /
												 PARCEL_GRID_STEP_METERS);
	}
	else
	{
	...
	if (sequence_id == SELECTED_PARCEL_SEQ_ID)
	{
		...
		parcelp = gViewerParcelMgr.mCurrentParcel;
	}
	else if (sequence_id == HOVERED_PARCEL_SEQ_ID)
	{
		gViewerParcelMgr.mHoverRequestResult = PARCEL_RESULT_SUCCESS;
		parcelp = gViewerParcelMgr.mHoverParcel;
	}
	...
	{
		gViewerParcelMgr.mHoverRequestResult = PARCEL_RESULT_SUCCESS;
		parcelp = gViewerParcelMgr.mCollisionParcel;
	}
			 ...
			 sequence_id > gViewerParcelMgr.mAgentParcelSequenceID)
	{
		...
		parcelp = gViewerParcelMgr.mAgentParcel;
	}
	else
	{
	...
	if (parcelp)
	{
		if (regionp)
		{
			parcelp->setRegionID(regionp->getRegionID());
		}
		...
		if (local_id == gViewerParcelMgr.mAgentParcel->getLocalID())
		{
			...
			agent_parcel_update =
				regionp && agent_regionp &&
				regionp->getRegionID() == agent_regionp->getRegionID();
		}

		S32 cur_env_version = parcelp->getParcelEnvironmentVersion();

		parcelp->init(owner_id, false, false, false, claim_date,
					  claim_price_per_meter, rent_price_per_meter, area,
					  other_prims, parcel_prim_bonus, is_group_owned);
		parcelp->setLocalID(local_id);
		parcelp->setAABBMin(aabb_min);
		parcelp->setAABBMax(aabb_max);

		parcelp->setAuctionID(auction_id);
		parcelp->setOwnershipStatus((LLParcel::EOwnershipStatus)status);

		parcelp->setSimWideMaxPrimCapacity(sw_max_prims);
		parcelp->setSimWidePrimCount(sw_total_prims);
		parcelp->setMaxPrimCapacity(max_prims);
		parcelp->setOwnerPrimCount(owner_prims);
		parcelp->setGroupPrimCount(group_prims);
		parcelp->setOtherPrimCount(other_prims);
		parcelp->setSelectedPrimCount(selected_prims);
		parcelp->setParcelPrimBonus(parcel_prim_bonus);

		parcelp->setCleanOtherTime(other_clean_time);
		parcelp->setRegionPushOverride(region_push_override);
		parcelp->setRegionDenyAnonymousOverride(region_deny_anonymous_override);
		parcelp->setRegionDenyAgeUnverifiedOverride(region_deny_age_unverified_override);
		parcelp->setRegionAllowAccessOverride(region_allow_access_override);
		parcelp->setParcelEnvironmentVersion(cur_env_version);
		parcelp->setRegionAllowEnvironmentOverride(region_allow_env_override);
		parcelp->setObscureMOAP(bool(extended_flags));

		parcelp->unpackMessage(msg);

		if (parcelp == gViewerParcelMgr.mAgentParcel)
		{
		...
		if (regionp)
		{
			if (!snap_selection)
			{
				...
				LLVector3 west_south =
					regionp->getPosRegionFromGlobal(gViewerParcelMgr.mWestSouth);
				LLVector3 east_north =
					regionp->getPosRegionFromGlobal(gViewerParcelMgr.mEastNorth);
			...
			else if (local_id == 0)
			{
				...
				gViewerParcelMgr.mWestSouth =
					regionp->getPosGlobalFromRegion(aabb_min);
				gViewerParcelMgr.mEastNorth =
					regionp->getPosGlobalFromRegion(aabb_max);
			...
			else
			{
				...
				S32 bitmap_size = gViewerParcelMgr.mParcelsPerEdge *
								  gViewerParcelMgr.mParcelsPerEdge / 8;
				S32 size = msg->getSizeFast(_PREHASH_ParcelData,
											_PREHASH_Bitmap);
				if (size != bitmap_size)
				{
					llwarns << "Parcel Id: " << local_id
							<< ". Expected bitmap size: " << size
							<< " - Got size: " << bitmap_size
							<< " - Bitmap might be corrupted." << llendl;
					bitmap_size = size;
				}
				U8* bitmap = new U8[bitmap_size];
				msg->getBinaryDataFast(_PREHASH_ParcelData, _PREHASH_Bitmap,
			 ...
			 sequence_id == COLLISION_BANNED_PARCEL_SEQ_ID)
	{
		...
		LLViewerParcelOverlay* overlayp = regionp ? regionp->mParcelOverlay
												  : NULL;
		if (overlayp)
		{
			...
			// false positives sometimes when the parcel data is changing. HB
			overlayp->resetCollisionBitmap();
#endif
			overlayp->readCollisionBitmap(bitmap);
		}
		delete[] bitmap;
		bitmap = NULL;
	}
	else if (sequence_id == HOVERED_PARCEL_SEQ_ID)
	{
		if (regionp)
		{
			gViewerParcelMgr.mHoverWestSouth = regionp->getPosGlobalFromRegion(aabb_min);
			gViewerParcelMgr.mHoverEastNorth = regionp->getPosGlobalFromRegion(aabb_max);
		}
		else
		{
	...
	else if (sequence_id != UPDATE_AGENT_PARCEL_SEQ_ID)
	{
		...
		if (gAudiop)
		{
			if (parcelp)
			{
				std::string music_url_raw = parcelp->getMusicURL();
		...
		// Now check for video
		LLViewerParcelMedia::update(parcelp);
	}
}
												...
												LLHandle<LLParcelInfoObserver> obs_handle)
{
	...
	gCoros.launch("LLViewerParcelMgr::regionParcelInfoCoro",
				  std::bind(&LLViewerParcelMgr::regionParcelInfoCoro, this,
