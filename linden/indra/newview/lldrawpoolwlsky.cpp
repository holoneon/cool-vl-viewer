...
static LLStaticHashedString sHdriSplitScreen("hdri_split_screen");
...
bool LLDrawPoolWLSky::sUseHDRI = false;

///////////////////////////////////////////////////////////////////////////////
// HDRI sky preview stuff

LLPointer<LLImageGL> LLDrawPoolWLSky::sEXRImage;

//static
std::string LLDrawPoolWLSky::loadHDRISky(const std::string& filename)
{
	float* out;	// width * height * RGBA
	int width, height;
	const char* err = NULL;
	if (LoadEXRWithLayer(&out, &width, &height, filename.c_str(), NULL,
						 &err) != TINYEXR_SUCCESS)
	{
		std::string error_msg = "Unknown";
		if (err)
		{
			error_msg.assign(err);
	...
	sEXRImage = new LLImageGL(tex_name, 4, GL_TEXTURE_2D, GL_RGB16F, GL_RGB16F,
							  GL_FLOAT, LLTexUnit::TAM_CLAMP);
	sEXRImage->setHasMipMaps(true);
	sEXRImage->setUseMipMaps(true);
	sEXRImage->setFilteringOption(LLTexUnit::TFO_TRILINEAR);

	LLTexUnit* unit0 = gGL.getTexUnit(0);
	unit0->bind(sEXRImage);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16F, width, height, 0, GL_RGBA,
				 GL_FLOAT, out);
	image_bound(width, height, GL_RGB16F);
...
//static
LLImageGL* LLDrawPoolWLSky::getHDRISky()
{
	return sEXRImage.get();
}

//static
void LLDrawPoolWLSky::resetHDRISky()
{
	sEXRImage = NULL;
}
...
// In this method, which is invoked before each set of render passes, we check
// if we need to use the HDRI sky texture and cache the result in sUseHDRI for
// speed (which avoids to have and check it at each render pass). HB
//virtual
void LLDrawPoolWLSky::prerender()
{
	static LLCachedControl<bool> use_hdri(gSavedSettings, "RenderHDRIEnabled");
	if (!use_hdri || !gUsePBRShaders || sEXRImage.isNull())
	{
		sUseHDRI = false;
		return;
	}
	// Always use HDRI for reflection probes when available.
	static LLCachedControl<bool> irr_only(gSavedSettings,
										  "RenderHDRIIrradianceOnly");
	if (gCubeSnapshot &&
		(!irr_only || !gPipeline.mReflectionMapManager.isRadiancePass()))
	{
		sUseHDRI = true;
		return;
	}
	// Fallback to EE sky when split screen is zero.
	static LLCachedControl<F32> split(gSavedSettings, "RenderHDRISplitScreen");
	sUseHDRI = split > 0.f;
}
...
void LLDrawPoolWLSky::endDeferredPass(S32)
{
	...
	if (gUsePBRShaders)
	{
		// Clear the depth buffer so that haze shaders can use unwritten depth
		// as a mask.
...
void LLDrawPoolWLSky::renderSkyHazeDeferred() const
{
	...
	if (sUseHDRI)
	{
		sSkyShader = &gEnvironmentMapProgram;
	}
	...
	if (sUseHDRI)
	{
		S32 chan = sSkyShader->enableTexture(LLShaderMgr::ENVIRONMENT_MAP);
		if (chan > -1)
		{
			gGL.getTexUnit(chan)->bind(sEXRImage);
		}
		static LLCachedControl<F32> hdri_exp(gSavedSettings,
											 "RenderHDRIExposure");
		static LLCachedControl<F32> hdri_rot(gSavedSettings,
											 "RenderHDRIRotation");
		static LLCachedControl<F32> hdri_split(gSavedSettings,
											   "RenderHDRISplitScreen");
		sSkyShader->uniform1f(LLShaderMgr::SKY_HDR_SCALE, powf(2.f, hdri_exp));
		LLMatrix3 rot;
		rot.setRot(0.f, hdri_rot * DEG_TO_RAD, 0.f);
		sSkyShader->uniformMatrix3fv(LLShaderMgr::DEFERRED_ENV_MAT,
									 1, GL_FALSE, (F32*)rot.mMatrix);
		sSkyShader->uniform1f(sHdriSplitScreen,
							  gCubeSnapshot ? 1.f : hdri_split);
	}
	...
	if (gUsePBRShaders)
	{
		...
		sSkyShader->uniform1i(LLShaderMgr::DEFAULT_PROBE_RENDER,
							  LLPipeline::sDefaultProbeRender ? 1 : 0);
...
{
	if (sUseHDRI || !gPipeline.canUseWindLightShaders() ||
	...
	if (gUsePBRShaders)
	{
		sCloudShader->uniform1i(LLShaderMgr::DEFAULT_PROBE_RENDER,
								LLPipeline::sDefaultProbeRender ? 1 : 0);
