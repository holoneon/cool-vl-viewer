...
void LLTextureCacheLocalFileWorker::doRead()
{
	...
	S32 bytes = (S32)LLFile::readEx(mFileName, mReadData, mOffset, mDataSize);
	if (bytes != mDataSize)
	{
...
void LLTextureCacheRemoteWorker::doRead()
{
	...
	if (mOffset < TEXTURE_CACHE_ENTRY_SIZE)
	{
		...
		S32 bytes_read =
			(S32)LLFile::readEx(gTextureCachep->mHeaderDataFileName, mReadData,
								offset, size);
		if (bytes_read != size)
		{
	...
	if (filesize && filesize + TEXTURE_CACHE_ENTRY_SIZE > mOffset)
	{
		...
		S32 bytes_read = (S32)LLFile::readEx(filename, mReadData + data_offset,
...
void LLTextureCacheRemoteWorker::doWrite()
{
	...
	if (mDataSize < TEXTURE_CACHE_ENTRY_SIZE)
	{
		...
		bytes_written =
			(S32)LLFile::writeEx(gTextureCachep->mHeaderDataFileName,
	...
	LL_DEBUGS("TextureCache") << "Writing Body: " << filename << " - Bytes: "
							  << file_size << LL_ENDL;
	bytes_written = (S32)LLFile::writeEx(filename,
...
std::string LLTextureCache::getLocalFileName(const LLUUID& id)
{
	...
	std::string filename = gDirUtil.getFullPath(LL_PATH_SKINS, "default",
												"textures", idstr);
	return filename;
}
...
{
	mHeaderEntriesFileName = gDirUtil.getFullPath(location, textures_dirname,
												  entries_filename);
	mHeaderDataFileName = gDirUtil.getFullPath(location, textures_dirname,
											   cache_filename);
	mTexturesDirName = gDirUtil.getFullPath(location, textures_dirname);
}
...
U32 LLTextureCache::openAndReadEntries(std::vector<Entry>& entries)
{
	...
	{
		llwarns << "Failed to read cache entries: out of memory ?" << llendl;
		closeHeaderEntriesFile();
		purgeAllTextures(false);
		return 0;		
	}
