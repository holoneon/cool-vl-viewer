...
#include "llviewermessage.h"		// give_money()
#include "llviewerobjectlist.h"
#include "llviewerregion.h"

using namespace std::placeholders;

// A small struct used to track callback information
struct LLGiveMoneyInfo
{
	LLGiveMoneyInfo(LLFloaterPay* floaterp, S32 amount)
	:	mFloater(floaterp),
		mAmount(amount)
	{
	}
...
//static
void LLFloaterPay::payViaObject(const LLUUID& object_id)
{
	LLViewerObject* object = gObjectList.findObject(object_id);
	if (!object || !object->getRegion()) return;

	LLFloaterPay* floater = new LLFloaterPay("Give L$", object_id, true);
	if (!floater) return;

	LLSelectNode* nodep = NULL;
	if (floater->mObjectSelection.notNull())
	{
		nodep = floater->mObjectSelection->getFirstRootNode();
	}
	if (!nodep)
	{
	...
	LLUUID owner_id;
	bool is_group = false;
	nodep->mPermissions->getOwnership(owner_id, is_group);

	floater->childSetText("obj_name_text", nodep->mName);

	floater->finishPayUI(owner_id, is_group);
}

//static
void LLFloaterPay::payDirectly(const LLUUID& target_id, bool is_group)
{
	LLFloaterPay* floater = new LLFloaterPay("Give L$", target_id, false);
	if (!floater) return;
...
LLFloaterPay::LLFloaterPay(const std::string& name, const LLUUID& uuid,
	...
	mTargetIsGroup(false)
{
	...
	LLGiveMoneyInfo* infop = new LLGiveMoneyInfo(this, PAY_BUTTON_DEFAULT_0);
	mCallbackData.push_back(infop);

	childSetAction("fastpay 1", onGive, infop);
	childSetVisible("fastpay 1", false);

	mQuickPayButton[i] = getChild<LLButton>("fastpay 1");
	mQuickPayInfo[i++] = infop;

	infop = new LLGiveMoneyInfo(this, PAY_BUTTON_DEFAULT_1);
	mCallbackData.push_back(infop);

	childSetAction("fastpay 5", onGive, infop);
	childSetVisible("fastpay 5", false);

	mQuickPayButton[i] = getChild<LLButton>("fastpay 5");
	mQuickPayInfo[i++] = infop;

	infop = new LLGiveMoneyInfo(this, PAY_BUTTON_DEFAULT_2);
	mCallbackData.push_back(infop);

	childSetAction("fastpay 10", onGive, infop);
	childSetVisible("fastpay 10", false);

	mQuickPayButton[i] = getChild<LLButton>("fastpay 10");
	mQuickPayInfo[i++] = infop;

	infop = new LLGiveMoneyInfo(this, PAY_BUTTON_DEFAULT_3);
	mCallbackData.push_back(infop);

	childSetAction("fastpay 20", onGive, infop);
	...
	infop = new LLGiveMoneyInfo(this, 0);
	mCallbackData.push_back(infop);

	childSetAction("pay btn", onGive, infop);
...
void LLFloaterPay::finishPayUI(const LLUUID& target_id, bool is_group)
{
	...
	gCacheNamep->get(target_id, is_group,
					 std::bind(&LLFloaterPay::onCacheOwnerName, _1, _2, _3,
							   this));
...
bool LLFloaterPay::give(S32 amount)
{
	...
	if (mTargetIsObject)
	{
		LLViewerObject* destobjp = gObjectList.findObject(mTargetUUID);
		LLViewerRegion* regionp = destobjp ? destobjp->getRegion() : NULL;
		if (!regionp)
		{
			return false;
		}
		...
		if (mObjectSelection.notNull())
		{
			...
			obj_name = nodep->mName;
		}

		S32 tx_type = TRANS_PAY_OBJECT;
		if (destobjp->isAvatar())
		{
			tx_type = TRANS_GIFT;
		}

		give_money(mTargetUUID, regionp, amount, false, tx_type, obj_name);
		mObjectSelection = NULL;

		// Request the object owner in order to check if the owner needs to be
		// unmuted
	...
	else
	{
		...
		give_money(mTargetUUID, gAgent.getRegion(), amount, mTargetIsGroup,
				   TRANS_GIFT, message);
...
{
	LLGiveMoneyInfo* infop = (LLGiveMoneyInfo*)data;
	if (infop && infop->mFloater)
	{
		if (!infop->mFloater->give(infop->mAmount))
		{
			gNotifications.add("PayObjectFailed");
		}
		infop->mFloater->close();
	}
}
