 ...
 * Copyright (c) 2011-2026, Henri Beauchamp
 *
...
// Do not move upwards, due to conflicting definitions in that header !
#include "glod/glod.h"
...
constexpr F32 PREVIEW_ZOOM_LIMIT = 50.f;
...
static bool find_model(LLModelLoader::scene& scene,
					   const std::string& name_to_match,
					   LLModel*& base_model_outp, LLMatrix4& mat_out)
{
	for (LLModelLoader::scene::iterator scene_it = scene.begin(),
										scene_end = scene.end();
		 scene_it != scene_end; ++scene_it)
    {
			 ...
			 it != end; ++it)
		{
			...
			LLModel* base_modelp = base_instance.mModel;
			if (base_modelp && base_modelp->mLabel == name_to_match)
			{
				mat_out = scene_it->first;
				base_model_outp = base_modelp;
				return true;
			}
		}
	}
...
void LLFloaterModelUploadBase::requestAgentUploadPermissions()
{
	...
	gCoros.launch("LLFloaterModelUploadBase::requestUploadPermCoro",
				  std::bind(&LLFloaterModelUploadBase::requestUploadPermCoro,
...
:	LLFloaterModelUploadBase(),
	...
	mLastMouseY(0)
{
	// Set the convex decomposition system based on the user's choice. HB
	LLConvexDecomposition::setUseVHACD(gSavedSettings.getBool("UseVHACD"));
...
bool LLFloaterModelPreview::postBuild()
{
	...
	mModelScaling = getChild<LLSpinCtrl>("import_scale");
	mModelScaling->setCommitCallback(onImportScaleCommit);
	mModelScaling->setCallbackUserData(this);
	...
	LLView* previewp = getChild<LLView>("preview_panel");
	mPreviewRect = previewp->getRect();
...
void LLFloaterModelPreview::initModelPreview()
{
	...
	mModelPreview->setDetailsCallback(std::bind(&LLFloaterModelPreview::setDetails,
												this, _1, _2, _3));
	mModelPreview->setModelUpdatedCallback(std::bind(&LLFloaterModelPreview::modelUpdated,
													 this, _1));
}

void LLFloaterModelPreview::onViewOptionChecked(LLUICtrl* ctrlp, void* datap)
{
	LLFloaterModelPreview* self = (LLFloaterModelPreview*)datap;
	if (!self) return;

	LLModelPreview* mp = self->mModelPreview;
	if (mp)
	{
		const std::string& name = ctrlp->getName();
...
{
	LLModelPreview* previewp = NULL;
	LLFloaterModelPreview* self = findInstance();
	if (self)
	{
		previewp = self->mModelPreview;
	}
	return previewp;
}

//static
void LLFloaterModelPreview::onTabChanged(void* datap, bool)
{
	LLFloaterModelPreview* self = (LLFloaterModelPreview*)datap;
...
// This method does not necessarily reflect the actual source of meshes, just
// user choices, some meshes could have been matched from different LODs, but
// should be good enough for statistics.
void LLFloaterModelPreview::fillLODSourceStats(strings_map_t& sources) const
{
	sources.clear();
	...
	for (S32 lod = 0; lod <= LLModel::LOD_HIGH; ++lod)
	{
		...
		{
			sources[lod_string] = "lod above";
		}
		else if (mLODMode[lod] == LLModelPreview::LOD_FROM_FILE)
		{
			ext = gDirUtil.getExtension(mModelPreview->mLODFile[lod]);
			if (ext == "dae" || ext == "gltf" || ext == "glb" || ext == "slm")
			{
				sources[lod_string] = ext;
			}
			else
			{
				sources[lod_string] = "unknown file";
			}
		}
		else
		{
			sources[lod_string] = "generated";
		}
	}

	const std::string& phys = mModelPreview->mLODFile[LLModel::LOD_PHYSICS];
	if (phys.empty())
	{
		S32 search_lod = mModelPreview->mPhysicsSearchLOD;
		if (search_lod >= 0 && search_lod <= 3)
		{
			sources["physics"] = lod_name[search_lod];
		}
		else
		{
			sources["physics"] = "none";
		}
	}
	else if (gDirUtil.getBaseFileName(phys) == "cube.dae")
	{
		sources["physics"] = "bounding box";
	}
	else
	{
		ext = gDirUtil.getExtension(phys);
		if (ext == "dae" || ext == "gltf" || ext == "glb" || ext == "slm")
		{
			sources["physics"] = ext;
		}
		...
		{
			sources["physics"] = "unknown file";
		}
	}
}

//static
void LLFloaterModelPreview::onClickCalculateBtn(void* datap)
{
	LLFloaterModelPreview* self = (LLFloaterModelPreview*)datap;
	...
	strings_map_t lod_sources;
	self->fillLODSourceStats(lod_sources);

	gMeshRepo.uploadModel(self->mModelPreview->mUploadData, lod_sources,
...
//static
void LLFloaterModelPreview::onImportScaleCommit(LLUICtrl*, void* datap)
{
	LLFloaterModelPreview* self = (LLFloaterModelPreview*)datap;
...
//static
void LLFloaterModelPreview::onPelvisOffsetCommit(LLUICtrl*, void* datap)
{
	LLFloaterModelPreview* self = (LLFloaterModelPreview*)datap;
	if (self && self->mModelPreview)
	{
...
//static
void LLFloaterModelPreview::onUploadJointsCommit(LLUICtrl*, void* datap)
{
	LLFloaterModelPreview* self = (LLFloaterModelPreview*)datap;
	if (self && self->mModelPreview)
	{
...
//static
void LLFloaterModelPreview::onUploadSkinCommit(LLUICtrl*, void* datap)
{
	LLFloaterModelPreview* self = (LLFloaterModelPreview*)datap;
	if (self && self->mModelPreview)
	{
...
//static
void LLFloaterModelPreview::onPreviewLODCommit(LLUICtrl* ctrlp, void* datap)
{
	LLFloaterModelPreview* self = (LLFloaterModelPreview*)datap;
	if (self && ctrlp && self->mModelPreview)
	{
		LLComboBox* combo = (LLComboBox*)ctrlp;
...
//static
void LLFloaterModelPreview::onGenerateNormalsCommit(LLUICtrl*, void* datap)
{
	LLFloaterModelPreview* self = (LLFloaterModelPreview*)datap;
	if (self && self->mModelPreview)
	{
...
//static
void LLFloaterModelPreview::toggleGenerateNormals(LLUICtrl*, void* datap)
{
	LLFloaterModelPreview* self = (LLFloaterModelPreview*)datap;
	if (self && self->mModelPreview)
	{
...
//static
void LLFloaterModelPreview::onExplodeCommit(LLUICtrl*, void* datap)
{
	LLFloaterModelPreview* self = (LLFloaterModelPreview*)datap;
	if (self && self->mModelPreview)
	{
...
//static
void LLFloaterModelPreview::onAutoFillCommit(LLUICtrl*, void* datap)
{
	LLFloaterModelPreview* self = (LLFloaterModelPreview*)datap;
	if (self && self->mModelPreview)
	{
...
void LLFloaterModelPreview::draw()
{
	if (LLApp::isExiting())
	{
		// Do not try and draw meshes (especially physics hulls) while the mesh
		// repository is about to get shut down !  HB
		return;
	}
...
//static
void LLFloaterModelPreview::onPhysicsParamCommit(LLUICtrl* ctrlp,
												 void* datap)
{
	LLFloaterModelPreview* self = findInstance();
	if (self)
	{
		LLCDParam* paramp = (LLCDParam*)datap;
		LLSD value = ctrlp->getValue();
		self->mDecompParams[paramp->mName] = value;
	}
}

//static
void LLFloaterModelPreview::onPhysicsStageExecute(LLUICtrl*, void* datap)
{
	LLFloaterModelPreview* self = findInstance();
	if (self && datap)
	{
		LLCDStageData* stage_datap = (LLCDStageData*)datap;
		std::string stage = stage_datap->mName;
		...
		if (stage == "Analyze")
		{
			...
			self->childSetVisible("Analyze", false);
			self->childSetVisible("analyze_cancel", true);
		}
		else
		{
			llwarns << "Unsupported stage: " << stage << ". Ignored."
					<< llendl;
			return;
		}
		...
		if (self->mModelPreview)
		{
				 ...
				 i < count; ++i)
			{
				...
				DecompRequest* requestp = new DecompRequest(stage, modelp);
				self->mCurRequest.insert(requestp);
...
//static
void LLFloaterModelPreview::onPhysicsBrowse(void* datap)
{
	LLFloaterModelPreview* self = (LLFloaterModelPreview*)datap;
	if (self)
	{
...
//static
void LLFloaterModelPreview::onPhysicsUseLOD(LLUICtrl*, void* datap)
{
	LLFloaterModelPreview* self = (LLFloaterModelPreview*)datap;
	if (!self) return;
...
//static
void LLFloaterModelPreview::onCancel(void* datap)
{
	LLFloaterModelPreview* self = (LLFloaterModelPreview*)datap;
	if (self)
	{
...
//static
void LLFloaterModelPreview::onPhysicsStageCancel(void* datap)
{
	LLFloaterModelPreview* self = (LLFloaterModelPreview*)datap;
	if (self)
	{
		for (std::set<LLPointer<DecompRequest> >::iterator
			 ...
			 it != end; ++it)
		{
			DecompRequest* requestp = *it;
			requestp->mContinue = 0;
		}
...
{
	childSetAction("analyze_cancel", onPhysicsStageCancel, this);
	...
	const LLCDStageData* stage = NULL;
	S32 stage_count = decomp->getStages(&stage);
	LL_DEBUGS("MeshUpload") << "stage_count = " << stage_count << LL_ENDL;

	const LLCDParam* param = NULL;
	S32 param_count = decomp->getParameters(&param);
	LL_DEBUGS("MeshUpload") << "param_count = " << param_count << LL_ENDL;

	for (S32 j = stage_count - 1; j >= 0; --j)
	{
		LLButton* buttonp = getChild<LLButton>(stage[j].mName, true, false);
		if (buttonp)
		{
			buttonp->setCommitCallback(onPhysicsStageExecute);
			buttonp->setCallbackUserData((void*)&stage[j]);
		}
		...
		for (S32 i = 0; i < param_count; ++i)
		{
			...
			if (param[i].mType == LLCDParam::LLCD_FLOAT)
			{
				...
				LLUICtrl* ctrlp = getChild<LLUICtrl>(name.c_str());
				if (LLSliderCtrl* slidp = dynamic_cast<LLSliderCtrl*>(ctrlp))
				{
					LL_DEBUGS("MeshUpload") << name
											<< " corresponds to a slider."
											<< LL_ENDL;
					slidp->setMinValue(param[i].mDetails.mRange.mLow.mFloat);
					slidp->setMaxValue(param[i].mDetails.mRange.mHigh.mFloat);
					slidp->setIncrement(param[i].mDetails.mRange.mDelta.mFloat);
					slidp->setValue(param[i].mDefault.mFloat);
					slidp->setCommitCallback(onPhysicsParamCommit);
					slidp->setCallbackUserData((void*) &param[i]);
				}
				else if (LLSpinCtrl* spinp = dynamic_cast<LLSpinCtrl*>(ctrlp))
				{
					LL_DEBUGS("MeshUpload") << name
					...
					spinp->setMinValue(param[i].mDetails.mRange.mLow.mFloat * coefficient);
					spinp->setMaxValue(param[i].mDetails.mRange.mHigh.mFloat * coefficient);
					spinp->setIncrement(param[i].mDetails.mRange.mDelta.mFloat * coefficient);
					spinp->setValue(param[i].mDefault.mFloat * coefficient);
					spinp->setCommitCallback(onPhysicsParamCommit);
					spinp->setCallbackUserData((void*) &param[i]);
				}
				else if (LLComboBox* combop = dynamic_cast<LLComboBox*>(ctrlp))
				{
					LL_DEBUGS("MeshUpload") << name
					...
					if (name == "Cosine%")
					{
						createSmoothComboBox(combop, min, max);
						combop->setValue(0);
					}
					else
					{
						for (F32 value = min; value <= max; value += delta)
						{
							...
							combop->add(label, value, ADD_BOTTOM, true);
						}
						combop->setValue(param[i].mDefault.mFloat);
					}

					combop->setCommitCallback(onPhysicsParamCommit);
					combop->setCallbackUserData((void*)&param[i]);
				}
				else
				{
			...
			else if (param[i].mType == LLCDParam::LLCD_INTEGER)
			{
				...
				if (LLSliderCtrl* slidp = dynamic_cast<LLSliderCtrl*>(ctrlp))
				{
					slidp->setMinValue(param[i].mDetails.mRange.mLow.mIntOrEnumValue);
					slidp->setMaxValue(param[i].mDetails.mRange.mHigh.mIntOrEnumValue);
					slidp->setIncrement(param[i].mDetails.mRange.mDelta.mIntOrEnumValue);
					slidp->setValue(param[i].mDefault.mIntOrEnumValue);
					slidp->setCommitCallback(onPhysicsParamCommit);
					slidp->setCallbackUserData((void*) &param[i]);
				}
				else if (LLComboBox* combop = dynamic_cast<LLComboBox*>(ctrlp))
				{
						 ...
						 k += param[i].mDetails.mRange.mDelta.mIntOrEnumValue)
					{
						...
						combop->add(name, k, ADD_BOTTOM, true);
					}
					combop->setValue(param[i].mDefault.mIntOrEnumValue);
					combop->setCommitCallback(onPhysicsParamCommit);
					combop->setCallbackUserData((void*) &param[i]);
				}
				else
			...
			else if (param[i].mType == LLCDParam::LLCD_BOOLEAN)
			{
				...
				LLCheckBoxCtrl* checkp = getChild<LLCheckBoxCtrl>(name.c_str(),
																  true, false);
				if (checkp)
				{
					checkp->setValue(param[i].mDefault.mBool);
					checkp->setCommitCallback(onPhysicsParamCommit);
					checkp->setCallbackUserData((void*)&param[i]);
				}
			...
			else if (param[i].mType == LLCDParam::LLCD_ENUM)
			{
				...
				LLComboBox* combop = getChild<LLComboBox>(name.c_str(), true,
														  false);
				if (combop)
				{
...
void LLFloaterModelPreview::createSmoothComboBox(LLComboBox* combop,
												 F32 min, F32 max)
{
	F32 delta = (max - min) / SMOOTH_VALUES_NUMBER;
	S32 ilabel = 0;

	combop->add("0 (none)", ADD_BOTTOM, true);
...
F32 LLFloaterModelPreview::getModelScaling() const
{
	return mModelScaling->getValue().asReal();
}
...
void LLFloaterModelPreview::onPermissionsReceived(const LLSD& result)
{
	...
	{
		llofstream of("perm_received.xml");
		LLSDSerialize::toPrettyXML(result, of);
	}
													 ...
													 const std::string& reason)
{
	LLSD args;
	args["MESSAGE"] = reason;
	gNotifications.add("MeshUploadPermError", args);
}
	...
	if (mJointOverrides[display_lod].empty())
	{
			 ...
			 it != end; ++it)
		{
				 ...
				 mit != mend; ++mit)
			{
				...
				LLModel* modelp = instance.mModel;
				const LLMeshSkinInfo* skin = &modelp->mSkinInfo;
				...
				if (bind_count && bind_count != joint_count)
				{
					...
					out << "Invalid joint overrides for model: "
						<< modelp->getName() << " - Amount of joints "
				...
				if (bind_count)
				{
					...
					for (U32 j = 0; j < joint_count; ++j)
					{
						...
						if (jointp->aboveJointPosThreshold(joint_pos))
						{
							...
							data.mPosOverrides[modelp->getName()] = joint_pos;
						}
						else
						{
							// Default value which would nott be accounted for
							data.mModelsNoOverrides.insert(modelp->getName());
						}
					}
				}
				else
...
//static
void LLFloaterModelPreview::onJointListSelection(LLUICtrl*, void* datap)
{
	LLFloaterModelPreview* self = (LLFloaterModelPreview*)datap;
	if (!self || !self->mModelPreview) return;
	...
	for (JointOverrideData::pos_override_map_t::const_iterator
	...
	element["columns"][2]["value"] = "-";
	element["columns"][3]["value"] = "-";
	for (strings_set_t::const_iterator it = data.mModelsNoOverrides.begin(),
...
//static
void LLFloaterModelPreview::onReset(void* datap)
{
	LLFloaterModelPreview* self = (LLFloaterModelPreview*)datap;
	if (!self) return;
...
//static
void LLFloaterModelPreview::onUpload(void* datap)
{
	LLFloaterModelPreview* self = (LLFloaterModelPreview*)datap;
	if (!self || !self->mModelPreview) return;
...
//static
void LLFloaterModelPreview::onClickValidateURL(void* datap)
{
	LLFloaterModelPreview* self = (LLFloaterModelPreview*)datap;
...
void LLFloaterModelPreview::resetDisplayOptions()
{
	...
	view_option_map_t::iterator option_it = mModelPreview->mViewOption.begin();
	for ( ; option_it != mModelPreview->mViewOption.end(); ++option_it)
	{
		LLUICtrl* ctrlp = getChild<LLUICtrl>(option_it->first.c_str());
		ctrlp->setValue(false);
	}
}
													  ...
													  std::string upload_url)
{
	...
	doOnIdleOneTime(std::bind(&LLFloaterModelPreview::handleModelPhysicsFeeReceived,
							  this));
}
														  ...
														  const LLSD& result)
{
	llwarns << "LLFloaterModelPreview::setModelPhysicsFeeErrorStatus(" << status
			<< " : " << reason << ")" << llendl;
	mSentFeeRequest = false;
	doOnIdleOneTime(std::bind(&LLFloaterModelPreview::toggleCalculateButton,
							  this, true));
...
void LLFloaterModelPreview::DecompRequest::completed()
{
	...
	if (mContinue)
	{
		...
		if (self)
		{
			if (mContinue && self->mModelPreview)
			{
				self->mModelPreview->mDirty = true;
				self->mModelPreview->refresh();
			}
...
void LLModelPreview::updateDimentionsAndOffsets()
{
	...
	F32 scale = mFMP->getModelScaling() * 2.f;
...
void LLModelPreview::rebuildUploadData()
{
	...
	LLSpinCtrl* scale_spinp = mFMP->getScaleSpinner();

	F32 scale = scale_spinp->getValue().asReal();
	LLMatrix4 scale_mat;
			 ...
			 mit != iter->second.end(); )
		{
				 ...
				 --i)
			{
				...
				if (!legacy_matching)
				{
					...
					LLMatrix4 transform;
					bool found = find_model(mScene[i], name_to_match,
											lod_modelp, transform);
					...
					if (!found && i != LLModel::LOD_PHYSICS)
					{
						...
						S32 search_lod = i > LLModel::LOD_HIGH ? LLModel::LOD_HIGH
															   : i;
						while (search_lod <= LLModel::LOD_HIGH)
						{
							std::string name_to_match = instance.mLabel;
							llassert(!name_to_match.empty());
							...
							// in LOD 'search_lod'
							if (find_model(mScene[search_lod], name_to_match,
										   lod_modelp, transform))
							{
								break;
							}
				...
				if (!lod_modelp && i == LLModel::LOD_PHYSICS && mWarnPhysModel)
				{
					...
					out << "No physics model specified for: "
						<< instance.mLabel;
					if (mDefaultPhysModel.notNull())
					{
						out << ". Using: " << DEFAULT_PHYSICS_MESH_NAME;
						lod_modelp = mDefaultPhysModel;
					}
					llwarns << out.str() << llendl;
					// Flash log tab if no default available.
					mFMP->addLineToLog(out.str(), mDefaultPhysModel.isNull());
				}
				...
				high_lod_model->mSkinInfo.mJointNames.size() > 0)
			{
				...
				LLQuaternion bind_rot = bind_shape_mat.getUnscaledQuaternion();
	...
	scale_spinp->setMaxValue(max_import_scale);
	if (max_import_scale < scale)
	{
		scale_spinp->setValue(max_import_scale);
	}
...
void LLModelPreview::getJointAliases(strings_map_t& joint_map)
{
	if (!mPreviewAvatar)
	{
		joint_map.clear();
							   ...
							   bool force_disable_slm, bool allow_preprocess)
{
	...
	if (LLDir::getExtension(filename) == "dae")
	{
	...
	if (lod != LLModel::LOD_PHYSICS)
	{
		LLLineEditor* editp = mFMP->getChild<LLLineEditor>("description_form");
		editp->setValue(LLDir::getBaseFileName(filename, true));
	}
...
void LLModelPreview::warnTextureScaling()
{
	if (mFMP)
	{
		std::string report = mFMP->getString("textures_scaled_down");
		llinfos << report << llendl;
		mFMP->addLineToLog(report);
	}
}
...
void LLModelPreview::loadModelCallback(S32 lod)
{
	...
	if (!mNumOfFetchingTextures && mTexturesNeedScaling)
	{
		warnTextureScaling();
	}
	...
	else
	{
		...
		if (lod == LLModel::LOD_PHYSICS &&
			// This makes sense for multiple models only
			mBaseModel.size() > 1 &&
			// Just on the off-chance
			mModelLoader->mModelList.size() == 1 &&
			mModelLoader->mScene.size() == 1 &&
			LLDir::getBaseFileName(mModelLoader->mFilename) == "cube.dae")
		{
			// Create a copy of the just loaded model for each model in
			// mBaseModel
			const LLModel* origp = mModelLoader->mModelList.front();
			const LLModelInstance& mi = mModelLoader->mScene.begin()->second.front();
			for (size_t i = 1, count = mBaseModel.size(); i < count; ++i)
			{
				LLPointer<LLModel> modelp(new LLModel(origp->getParams(),
													  origp->getDetail()));
				modelp->mLabel = origp->mLabel;
				modelp->copyVolumeFaces(origp);
				modelp->mPosition = origp->mPosition;
				modelp->mMaterialList = origp->mMaterialList;
				mModel[lod].push_back(modelp);
				mScene[lod][mi.mTransform].push_back(LLModelInstance(modelp,
													 modelp->mLabel,
													 mi.mTransform,
													 mi.mMaterial));
			}
		...
		else
		{
			...
			if (lod == LLModel::LOD_PHYSICS)
			{
				...
				LLModel* out_model = NULL;
				std::string name = DEFAULT_PHYSICS_MESH_NAME +
								   get_lod_suffix(lod);
				find_model(mScene[lod], name, out_model, t);
				mDefaultPhysModel = out_model;
				mWarnPhysModel = true;
			}
				...
				!gSavedSettings.getBool("ImporterLegacyMatching"))
			{
				...
				for (S32 idx = 0, cnt = mModel[lod].size(); idx < cnt; ++idx)
				{
					...
					LLModel* found_modelp = NULL;
					if (find_model(mBaseScene, loaded_name, found_modelp, t))
					{
				...
				if (!name_based && !has_submodels)
				{
						 ...
						 ++idx)
					{
						...
						if (loaded_name != name)
						{
							...
							// Rename the corresponding instance as well
							bool found = false;
							for (auto& p : mScene[lod])
							{
								for (auto& i : p.second)
								{
									if (i.mModel == mModel[lod][idx])
									{
										i.mLabel = name;
										found = true;
										break;
								 ...
								 bool enforce_tri_limit)
{
		 ...
		 iter != endIter; ++iter)
	{
			 ...
			 instance != end_instance; ++instance)
		{
			...
			if (modelp)
			{
				instanced_triangle_count += modelp->getNumTriangles();
			}
		}
	}
	...
	if (object_dirty)
	{
			 ...
			 iter != mBaseModel.end(); ++iter)
		{
			...
			if (mObject[modelp] != 0)
			{
				glodDeleteObject(mObject[modelp]);
			}

			mObject[modelp] = cur_name++;

			glodNewObject(mObject[modelp], mGroup, GLOD_DISCRETE);
			if (stop_gloderror("glodNewObject"))
			{
				return false;
			}

			if (iter == mBaseModel.begin() && !modelp->mSkinWeights.empty())
			{
			...
			for (U32 i = 0; i < mVertexBuffer[5][modelp].size(); ++i)
			{
				LLVertexBuffer* buff = mVertexBuffer[5][modelp][i];
				...
				glodInsertElements(mObject[modelp], i, GL_TRIANGLES,
			...
			glodBuildObject(mObject[modelp]);
	...
	for (S32 lod = start; lod >= end; --lod)
	{
		...
		for (U32 i = 0; i < mBaseModel.size(); ++i)
		{
				 ...
				 iter != end; ++iter)
			{
				...
				{
					if (iter->second[j].mModel == modelp)
					{
		...
		for (U32 mdl_idx = 0; mdl_idx < mBaseModel.size(); ++mdl_idx)
		{
			...
			if (model_meshopt_mode == MESH_OPTIMIZER_SLOPPY)
			{
					 ...
					 face_idx < count; ++face_idx)
				{
					...
					{
						if (genMeshOptimizerPerFace(base, target_model,
													face_idx, indices_decim,
													lod_err_thres,
													MESH_OPTIMIZER_FULL) < 0)
						{
							failure = true;
						}
					}
...
void LLModelPreview::updateStatusMessages()
{
		 ...
		 iter != end; ++iter)
	{
		...
		for (S32 i = 0; i < LLModel::NUM_LODS - 1; ++i)
		{
			LLModel* lod_modelp = instance.mLOD[i];
			if (!lod_modelp)
			{
			...
			S32 cur_tris = 0;
			S32 cur_verts = 0;
			S32 cur_submeshes = lod_modelp->getNumVolumeFaces();

			for (S32 j = 0; j < cur_submeshes; ++j)
			{
				// For each submesh (face), add triangles and vertices to
				// current total
				const LLVolumeFace& face = lod_modelp->getVolumeFace(j);
			...
			{
				std::ostringstream out;
				out << "Instance: " << lod_modelp->mLabel << " - LOD" << i
					<< " - Verts: " << cur_verts << " - Tris: " << cur_tris
					<< " - Faces: " << cur_submeshes;
				const strings_vec_t& mat_names = lod_modelp->mMaterialList;
				for (size_t k = 0, mcount = mat_names.size(); k < mcount; ++k)
				{
					out << " - Material: " << mat_names[k];
				}
				llinfos << out.str() << llendl;
				mFMP->addLineToLog(out.str());
			}
	...
	{
		LLModel* modelp = mModel[LLModel::LOD_PHYSICS][i];
		if (modelp)
		{
			for (U32 j = 0, count2 = modelp->mPhysics.mHull.size(); j < count2;
				 ++j)
			{
				if (modelp->mPhysics.mHull[j].size() > 256)
				{
					physics_off_limit = true;
					llwarns << "Physical model " << modelp->mLabel
	...
	if (mModelNoErrors && mModelLoader && mNumOfFetchingTextures &&
		 ...
		 iter != end; ++iter)
	{
			 ...
			 instance != end_instance; ++instance)
		{
			...
			S32 cur_submeshes = modelp->getNumVolumeFaces();

			LLModel::hull_decomp_t& decomp = modelp->mPhysics.mHull;
			if (decomp.empty())
			{
				...
				for (S32 j = 0; j < cur_submeshes; ++j)
				{
					// For each submesh (face), add triangles and vertices to
					// current total
					const LLVolumeFace& face = modelp->getVolumeFace(j);
	...
	// Explode physics hull UI
	mFMP->childSetEnabled("physics_explode_label", enable);
	mFMP->childSetEnabled("physics_explode", enable);

	// Enable/disable "Step 2" UI
	bool using_vhacd = LLConvexDecomposition::usingVHACD();
	bool enable_full = enable && using_vhacd;
	...
	if (using_vhacd)
	{
		mFMP->childSetVisible("vhacd_limits", true);
	}
	else
	{
		mFMP->childSetEnabled("second_step_label", enable);
		mFMP->childSetVisible("hacd_limits", true);
	}

	if (mFMP->mCurRequest.empty())
	{
		mFMP->childSetVisible("Analyze", true);
		if (phys_tris || phys_hulls > 0)
		{
			mFMP->childEnable("Analyze");
		}
		mFMP->childSetVisible("analyze_cancel", false);
	}
	else
	{
		mFMP->childEnable("analyze_cancel");
	}
...
{
	LLModelLoader::model_list* modelp = NULL;
	...
	for (LLModelLoader::model_list::iterator iter = modelp->begin();
		 iter != modelp->end(); ++iter)
	{
		LLModel* modelp = *iter;
		if (!modelp) continue;
		...
		bool skinned = include_skin_weights && !modelp->mSkinWeights.empty();

		LLMatrix4a mat_normal;
		if (skinned)
		{
			mat_normal.loadu(modelp->mSkinInfo.mBindShapeMatrix);
			mat_normal.invert();
		...
		{
			const LLVolumeFace& vf = modelp->getVolumeFace(i);
			...
			if (skinned)
			{
				...
				bind_shape_matrix.loadu(modelp->mSkinInfo.mBindShapeMatrix);
			...
			mVertexBuffer[lod][modelp].emplace_back(vb);
...
U32 LLModelPreview::countRootModels(LLModelLoader::model_list models)
{
	...
	{
		LLModel* modelp = *model_iter;
		if (modelp && modelp->mSubmodelID == 0)
		{
			++root_models;
		}
		++model_iter;
...
void LLModelPreview::loadedCallback(LLModelLoader::scene& scene,
									...
									S32 lod, void* datap)
{
	LLModelPreview* self = (LLModelPreview*)datap;
	if (self && self == LLFloaterModelPreview::getModelPreview())
	{
		self->loadModelCallback(lod);

		const LLVOAvatar* avatarp = self->mPreviewAvatar;
		if (avatarp && avatarp->mDrawable)
		{
			...
			self->mGroundPlane[0].set(min[0], min[1], ground);
			self->mGroundPlane[1].set(max[0], min[1], ground);
			self->mGroundPlane[2].set(max[0], max[1], ground);
			self->mGroundPlane[3].set(min[0], max[1], ground);
		}
	}
}

//static
void LLModelPreview::stateChangedCallback(U32 state, void* datap)
{
	LLModelPreview* self = (LLModelPreview*)datap;
	if (self && self == LLFloaterModelPreview::getModelPreview())
	{
		self->setLoadState(state);
	}
}
										   ...
										   void* datap)
{
		...
		self->mPreviewAvatar)
	{
		...
		return self->mPreviewAvatar->getJoint(joint_key);
	}
	return NULL;
}

//static
U32 LLModelPreview::loadTextures(LLImportMaterial& material, void* datap)
{
	LLModelPreview* previewp = (LLModelPreview*)datap;
	if (!previewp || previewp != LLFloaterModelPreview::getModelPreview())
	{
		// Stale callback: model preview gone or changed.
		LL_DEBUGS("MeshUpload") << "Stale callback: ignored." << LL_ENDL;
		material.mUserData = NULL;
		return 0;
	}

	// If the model loader (e.g. glFT one) already found a valid texture Id,
	// then make sure the corresponding fetched texture gets fully loaded. HB
	const LLUUID& tex_id = material.getDiffuseMap();
	if (tex_id.notNull())
	{
		LLViewerFetchedTexture* texp =
		...
		if (texp)
		{
			LL_DEBUGS("MeshUpload") << "Boosting texture: " << tex_id
									<< LL_ENDL;
			texp->setBoostLevel(LLGLTexture::BOOST_SCULPTED);
			texp->setLoadedCallback(textureLoadedCallback, 0, true, false,
									datap, NULL, false);
			texp->forceToSaveRawImage(0, F32_MAX);
			return 1;
		}
		// Else, fall back to try and load a texture file. HB
	}
	...
	std::string filename = LLURI::unescape(material.mDiffuseMapFilename);
	std::string fullpath = filename;
	LL_DEBUGS("MeshUpload") << "Trying: " << fullpath << LL_ENDL;
	bool found = LLFile::exists(fullpath);
	if (!found)
	{
		// Try to find the texture file somewhere in the loaded LODs
		// directories. HB
		std::vector<std::string> paths;
		for (S32 lod = LLModel::LOD_HIGH; lod >= LLModel::LOD_IMPOSTOR; --lod)
		{
			const std::string& lod_file = previewp->mLODFile[lod];
			if (!lod_file.empty())
			{
				paths.emplace_back(LLDir::getDirName(lod_file) +
								   LL_DIR_DELIM_STR);
			}
		}
		for (size_t i = 0, count = paths.size(); i < count && !found; ++i)
		{
			fullpath = paths[i] + filename;
			LL_DEBUGS("MeshUpload") << "Trying: " << fullpath << LL_ENDL;
			found = LLFile::exists(fullpath);
		}
	}
	if (!found)
	{
		llwarns << "Could not find texture file: " << filename << llendl;
		material.mUserData = NULL;
		return 0;
	}

	llinfos << "Loading texture: " << fullpath << llendl;
	...
	if (!LLViewerTextureList::createRawImageFromFile(fullpath, imagep))
	{
		llwarns << "Could not create raw image from file: " << fullpath
				<< llendl;
		material.mUserData = NULL;
		return 0;
	}
	texp = LLViewerTextureManager::getFetchedTexture(imagep.get());
#if 0	// Do not do this: this empties the texture we just created !  HB
	if (texp->getDiscardLevel() < texp->getMaxDiscardLevel())
	{
		// File was loaded previously, reload image to get potential changes.
		texp->clearFetchedResults();
	}
#else
	// *HACK: mark as a sculpt texture so that it does not get evicted from
	// VRAM after a while by the texture fetcher, while still in use. HB
	texp->setBoostLevel(LLGLTexture::BOOST_SCULPTED);
#endif
	texp->setLoadedCallback(textureLoadedCallback, 0, true, false, datap, NULL,
							false);
	texp->forceToSaveRawImage(0, F32_MAX);
	material.setDiffuseMap(texp->getID());	// Record tex ID
	++previewp->mNumOfFetchingTextures;
	return 1;
}
...
static void upload_matrix_palette(LLVOAvatar* avp, LLMeshSkinInfo* skinp)
{
	...
	U32 count = avp->initSkinningMatrixPalette(mat4a, skinp);
	U32 idx = 0;
...
bool LLModelPreview::render()
{
		 ...
		 iter != end; ++iter)
	{
			 ...
			 model_iter != model_end; ++model_iter)
		{
			...
			LLModel* modelp = instance.mModel;
			modelp->mPelvisOffset = mPelvisZOffset;
	...
	F32 z_near = 0.001f;
	F32 z_far = mCameraDistance * 10.f + mPreviewScale.length() +
	...
	// Scale the camera distance and offset with the model upload scaling. HB
	F32 model_scaling = mFMP->getModelScaling();
	cam_dist *= model_scaling;
	offset *= model_scaling;
	...
	if (!mModel[mPreviewLOD].empty())
	{
		...
		if (!skin_weight)
		{
				 ...
				 iter != end; ++iter)
			{
				...
				LLModel* modelp = instance.mLOD[mPreviewLOD];
				if (!modelp)
				{
					continue;
				}
				...
				for (U32 i = 0,
						 count = mVertexBuffer[mPreviewLOD][modelp].size();
					 i < count; ++i)
				{
					LLVertexBuffer* buffer =
						mVertexBuffer[mPreviewLOD][modelp][i];
			...
			if (physics)
			{
				...
				for (U32 pass = 0; pass < 2; ++pass)
				{
						 ...
						 iter != end; ++iter)
					{
						...
						LLModel* modelp = instance.mLOD[LLModel::LOD_PHYSICS];
						if (!modelp)
						{
							continue;
						}
						...
						if (decomp)
						{
							...
							LLModel::Decomposition& physics = modelp->mPhysics;
								...
								if (!physics.mMesh.empty())
								{
										 ...
										 i < count; ++i)
									{
										if (explode > 0.f)
										{
											...
											LLVector3 offset =
												modelp->mHullCenter[i] -
												modelp->mCenterOfHullCenters;
											offset *= explode;
						...
						if (render_mesh)
						{
							...
							{
								for (U32 i = 0,
										 count = mVertexBuffer[LLModel::LOD_PHYSICS][modelp].size();
									 i < count; ++i)
								{
									LLVertexBuffer* buffer =
										mVertexBuffer[LLModel::LOD_PHYSICS][modelp][i];
		...
		else if (mPreviewAvatar)
		{
				 ...
				 iter != end; ++iter)
			{
					 ...
					 model_iter != model_end; ++model_iter)
				{
					...
					LLModel* modelp = instance.mModel;
					if (!modelp || modelp->mSkinWeights.empty())
					{
						continue;
					}
						 ...
						 i < count; ++i)
					{
						...
						else
						{
							...
							mPreviewAvatar->initSkinningMatrixPalette(mat, skin);
							LLMatrix4a bind_shape_matrix;
								 ...
								 j < count2; ++j)
							{
								...
								LLModel::getPerVertexSkinMatrix(weight[j], mat,
																final_mat,
																true);
...
void LLModelPreview::setPreviewLOD(S32 lod)
{
	...
	if (lod != mPreviewLOD && mFMP)
	{
		...
		LLComboBox* combop = mFMP->getChild<LLComboBox>("preview_lod_combo");
		// Combo box list of lods is in reverse order
		combop->setCurrentByIndex(NUM_LOD - 1 - mPreviewLOD);
										   ...
										   void* datap)
{
	...
	if (texp &&
		(texp->getOriginalWidth() > gMaxImageSizeDefault ||
		 texp->getOriginalHeight() > gMaxImageSizeDefault))
	{
		self->mTexturesNeedScaling = true;
	}
	if (self->mNumOfFetchingTextures > 0)
	{
		--self->mNumOfFetchingTextures;
	}
	if (!self->mNumOfFetchingTextures && self->mTexturesNeedScaling)
	{
		self->warnTextureScaling();
	}
}
