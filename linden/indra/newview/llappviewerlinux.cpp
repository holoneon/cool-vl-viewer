...
#include "json.hpp"				// For Vulkan API version extraction
...
static LL_INLINE bool do_basic_glibc_backtrace()
{
	...
	std::string strace_filename = gDirUtil.getFullPath(LL_PATH_LOGS,
													   "stack_trace.log");
...
{
	std::string app_filename = gDirUtil.getExecutablePathAndName();
...
void LLAppViewerLinux::initLogging()
{
	...
	std::string old_stack_file = gDirUtil.getFullPath(LL_PATH_LOGS,
													  "stack_trace.log");
...
// via LLWindowLinux::processMiscNativeEvents(), so there was strictly no point
	...
	if (has_vulkan == -1)
	{
		...
		if (getenv("VK_ICD_FILENAMES"))
		{
			...
			if (pos > 1)
			{
				...
				// *TODO: also check for any other listed files (unlikely) ?
		...
		if (has_vulkan == 0 &&
			(LLGLManager::sIsNVIDIA || LLGLManager::sIsAMD ||
			 LLGLManager::sIsIntel))
		{
		...
		if (has_vulkan == 1)
		{
			...
			if (file.is_open())
			{
				...
				lljson root = lljson::parse(file, NULL, false, true);
				...
				{
					auto icd_it = root.find("ICD");
					if (icd_it != root.end())
					{
						lljson icd = *icd_it;
						auto ver_it = icd.find("api_version");
						if (ver_it != icd.end())
						{
							vk_api_version = to_string(ver_it.value());
