...
bool LLPrefsVoice::postBuild()
{
	...
	mDeviceSettingsBtn = getChild<LLButton>("device_settings_btn");
	mDeviceSettingsBtn->setClickedCallback(onClickVoiceDeviceSettings, this);
	...
	onCommitEnableVoiceChat(getChild<LLCheckBoxCtrl>("enable_voice_check"),
							this);

	childSetValue("modifier_combo",
	...
	childSetValue("auto_disengage_mic_check",
				  gSavedSettings.getBool("AutoDisengageMic"));
	childSetValue("push_to_talk_toggle_check",
				  gSavedSettings.getBool("PushToTalkToggle"));
	childSetValue("ear_location",
				  gSavedSettings.getS32("VoiceEarLocation"));
	childSetValue("enable_lip_sync_check",
				  gSavedSettings.getBool("LipSyncEnabled"));
	childSetValue("agc_check",
				  gSavedSettings.getBool("VoiceWebRTCAutomaticGainControl"));
	childSetValue("echo_check",
				  gSavedSettings.getBool("VoiceWebRTCEchoCancellation"));
	childSetValue("noise_suppression_level",
				  gSavedSettings.getU32("VoiceWebRTCNoiseSuppression"));
...
{
	mDeviceSettingsBtn->setEnabled(LLStartUp::isLoggedIn() &&
								   gVoiceClient.isVoiceWorking());
	LLPanel::draw();
}
...
void LLPrefsVoice::apply()
{
	...
	gSavedSettings.setBool("VoiceWebRTCAutomaticGainControl",
						   childGetValue("agc_check"));
	gSavedSettings.setBool("VoiceWebRTCEchoCancellation",
						   childGetValue("echo_check"));
	gSavedSettings.setU32("VoiceWebRTCNoiseSuppression",
						   childGetValue("noise_suppression_level").asInteger());

	LLFloaterVoiceDeviceSettings* floaterp =
		LLFloaterVoiceDeviceSettings::findInstance();
	if (floaterp)
	{
		floaterp->apply();
	}
	// Propagate AGC, echo cancellation, noise suppression level...
...
void LLPrefsVoice::onCommitEnableVoiceChat(LLUICtrl* ctrl, void* user_data)
{
	...
	self->childSetEnabled("agc_check", enable);
	self->childSetEnabled("echo_check", enable);
	self->childSetEnabled("noise_suppression_level", enable);
}
