...
bool LLPanelLogin::postBuild()
{
	...
	LLSLURL::eType slurl_type = slurl.getType();
	...
	std::string login_hist_filepath =
		gDirUtil.getFullPath(LL_PATH_USER_SETTINGS, "saved_grids_login.xml");
...
void LLPanelLogin::showTokenInputLine(bool show)
{
	if (sInstance)
	{
		...
		if (show)
		{
			sInstance->mTokenEditor->setFocus(true);
		}
		else
		{
			sInstance->mTokenEditor->clear();
		}
	}
}
...
void LLPanelLogin::onSelectServer(LLUICtrl*, void* user_data)
{
	...
	// Hide the MFA input line, since we change grid. HB
	showTokenInputLine(false);
...
void LLPanelLogin::onSelectLoginEntry(LLUICtrl* ctrl, void* data)
{
	...
	{
		// Hide the MFA input line, since we changed account. HB
		showTokenInputLine(false);
