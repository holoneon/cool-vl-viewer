							 ...
							 const std::string& label)
{
	...
	static LLColor4 c_col, col_next, col_edge, col_edge_next;
	...
	gGL.pushMatrix();
	{
		...
		const F32 x = gViewerCamera.getLeftAxis().mV[0];
		const F32 y = gViewerCamera.getLeftAxis().mV[1];
		...
		for (U32 i = 0; i < BEACON_VERTS; ++i)
		{
			...
			col_edge = fogged_color * LLColor4(a, a, a, 0.f);
			col_edge_next = fogged_color * LLColor4(an, an, an, 0.f);
	...
	hud_textp->setFont(fontp);
	hud_textp->setZCompare(false);
	hud_textp->setColor(LLColor4(1.f, 1.f, 1.f,
								 llclamp((dist - FADE_DIST) / FADE_DIST, 0.2f,
										 1.f)));

	hud_textp->setString(utf8str_to_wstring(label +
											llformat("\n%.0f m", dist)));
...
void LLTracker::drawMarker(const LLVector3d& pos_global, const LLColor4& color)
{
		...
		gViewerCamera.projectPosAgentToScreenEdge(pos_local, screen))
	{
		...
		S32 x_center = S32(0.5f * (F32)rect.getWidth());
		S32 y_center = S32(0.5f * (F32)rect.getHeight());
		...
		if (dist > 0.f)
		{
			...
			mHUDArrowCenterX = S32(x_clamped);
			mHUDArrowCenterY = S32(y_clamped);
		}
		else
		{
...
void LLTracker::setLandmarkVisited()
{
	...
	msg->newMessage(_PREHASH_ChangeInventoryItemFlags);
	...
	msg->addUUID(_PREHASH_ItemID, mTrackedLandmarkItemID);
