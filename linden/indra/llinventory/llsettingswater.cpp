...
//static
void LLSettingsWater::adjustTime(LLSD& settings, F32 position)
{
	if (!settings.has(SETTING_NORMAL_SCALE))
	{
	...
	// Give the normal scale offset some variability over track time...
	F32 offset = position * 0.5f - 0.25f;
	settings[SETTING_NORMAL_SCALE] =
		LLVector3(2.f + offset, 2.f + offset, 2.f + offset).getValue();
}
