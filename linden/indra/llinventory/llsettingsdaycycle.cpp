...
LLSD LLSettingsDay::getSettings() const
{
	...
	typedef std::map<std::string, LLSettingsBase::ptr_t,
					 std::less<> > str_ptr_map_t;
...
bool LLSettingsDay::initialize(bool validate_frames)
{
	...
	std::map<std::string, LLSettingsBase::ptr_t, std::less<> > used;
...
//static
LLSD LLSettingsDay::buildCycleFromFixed(const LLSettingsSky::ptr_t& skyp,
										const LLSettingsWater::ptr_t& waterp)
{
	...
	if (skyp)
	{
		sky = skyp->getSettings();
		sky_name = sky[SETTING_NAME].asString();
	}
	if (sky_name.empty())
	{
		sky_name = "_default_";
	...
	if (waterp)
	{
		water = waterp->getSettings();
		water_name = water[SETTING_NAME].asString();
	}
	if (water_name.empty())
	{
		water_name = "_default_";
	}

	LLSD cycle;
	cycle[SETTING_TYPE] = "daycycle";
	cycle[SETTING_NAME] = sky_name;
	...
	constexpr U32 FRAME_COUNT = 8;
	constexpr F32 FRAME_STEP = 1.f / (F32)FRAME_COUNT;
	std::string sky_frame_name, water_frame_name;
	F32 time = 0.f;
	for (U32 i = 0; i < FRAME_COUNT; ++i)
	{
		sky_frame_name = "sky:" + sky_name;
		sky_frame_name += 'a' + i;

		water_frame_name = "water:" + water_name;
		water_frame_name += 'a' + i;
		...
		LLSettingsWater::adjustTime(water, time);
		frames[water_frame_name] = water;
		LLSettingsSky::adjustTime(sky, time);
		frames[sky_frame_name] = sky;
	...
	cycle[SETTING_TRACKS] = tracks;
	cycle[SETTING_FRAMES] = frames;
...
void LLSettingsDay::setSkyAtKeyframe(const LLSettingsSky::ptr_t& sky,
...
LLSettingsSky::ptr_t LLSettingsDay::getSkyAtKeyframe(F32 keyframe, S32 track) const
{
	...
	return std::dynamic_pointer_cast<LLSettingsSky>(getSettingsAtKeyframe(keyframe,
																		  track));
}
