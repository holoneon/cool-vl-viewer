...
public:
	...
	LL_INLINE F32 getHDRMin(bool auto_adjust) const
	{
		return !auto_adjust && canAutoAdjust() ? 0.f : mHDRMin;
	}

	LL_INLINE F32 getHDRMax(bool auto_adjust) const
	{
		return !auto_adjust && canAutoAdjust() ? 0.f : mHDRMax;
	}

	LL_INLINE F32 getHDROffset(bool auto_adjust) const
	{
		return !auto_adjust && canAutoAdjust() ? 1.f : mHDROffset;
	}

	LL_INLINE F32 getTonemapMix(bool auto_adjust) const
	{
		return !auto_adjust && canAutoAdjust() ? 0.f : mTonemapMix;
	}

	LL_INLINE void setTonemapMix(F32 mix)				{ mTonemapMix = mix; }
	...
	LL_INLINE const LLColor3& getMoonlightColor() const
	{
	...
	LL_INLINE const LLColor3& getSunlightColor() const
	{
		update();
...
protected:
	// Note: we need an override for this method, because we must deal with
	// "reflection_probe_ambiance", which does not exist in pre-PBR sky
	// settings. HB
	LLSD interpolateSDValue(const std::string& name, const LLSD& value,
							const LLSD& other, const parammapping_t& defaults,
							F64 mix, const stringset_t& slerps) const override;
...
private:
	...
	mutable F32					mHDRMax;
	mutable F32					mHDRMin;
	mutable F32					mHDROffset;
	mutable F32					mTonemapMix;
