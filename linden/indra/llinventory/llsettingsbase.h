...
public:
	...
	LL_INLINE void setName(const std::string& val)
	{
		// Note: skips setLLSD()
		mSettingName = val;
	...
	// Calculate or caches custom settings, taken from the updated LLSD.
	...
	public:
		...
		typedef std::function<bool(LLSD&, U32)> verify_fn_t;

		Validator(const std::string& name, bool required, LLSD::Type type,
				  verify_fn_t verify = nullptr, const LLSD& defval = LLSD())
		:	mName(name),
			mRequired(required),
			mType(type),
			mVerify(verify),
...
protected:
	...
	virtual LLSD interpolateSDValue(const std::string& name, const LLSD& value,
									const LLSD& other,
