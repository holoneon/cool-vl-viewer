...
public:
	...
	// Defaults to created by system. Sets owner to nobody and all permissions
	// to PERM_ALL.
	LLPermissions();

	// Permissions for item created and owned by some avatar (typically, the
	// agent avatar). Avoids a call to set() when creating such a new inventory
	// item. Sets all permissions to PERM_ALL. HB
	LLPermissions(const LLUUID& creator_id);

	// Replaces set(const LLPermissions&). HB
	LLPermissions& operator=(const LLPermissions& rhs);

	bool operator==(const LLPermissions& rhs) const;
	bool operator!=(const LLPermissions& rhs) const;

	void reset();	// Resets to "created by system". HB

	// Regroups the now removed init() and initMasks(PermissionMask, ...)
	// methods and is faster. HB
	void set(const LLUUID& creator_id, const LLUUID& owner_id,
			 const LLUUID& last_owner_id, const LLUUID& group_id,
			 PermissionMask base_mask, PermissionMask owner_mask,
			 PermissionMask everyone_mask, PermissionMask group_mask,
			 PermissionMask next_mask);
	...
	// Returns the agent Id of the agent that created the item.
	LL_INLINE const LLUUID& getCreator() const			{ return mCreator; }

	// Returns the agent Id of the owner. returns LLUUID::null if group
	// owned or public (a really big group).
	LL_INLINE const LLUUID& getOwner() const			{ return mOwner; }

	// Returns the group Id of the group associated with the object.
	LL_INLINE const LLUUID& getGroup() const			{ return mGroup; }

	// Returns the agent Id of the last agent owner. Only returns LLUUID::null
	...
	// Returns true when owned by Alexandria Linden. HB
	bool fromLibrary() const;
...
// Helper functions
void mask_to_string(U32 mask, char* str, bool export_support = false);
std::string mask_to_string(U32 mask, bool export_support = false);
