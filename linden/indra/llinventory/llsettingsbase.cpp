...
// Used only for parameters that are not changed via setLLSD(). HB
void LLSettingsBase::setCachedValues()
{
	if (mSettings.has(SETTING_NAME))
	{
		mSettingName = mSettings[SETTING_NAME].asString();
	}
	else
	{
		mSettingName.clear();
...
//virtual
void LLSettingsBase::updateSettings()
{
	if (mSettings.has(SETTING_ID))
	{
		mSettingID = mSettings[SETTING_ID].asUUID();
	}
	else
	{
		mSettingID.setNull();
	...
	{
		mSettingFlags = mSettings[SETTING_ASSETID].asInteger();
	}
	else
	{
		mSettingFlags = 0;
	}
	mDirty = mReplaced = false;
}

void LLSettingsBase::setFlag(U32 flag)
{
	update();
	mSettingFlags |= flag;
	if (mSettingFlags)
	{
		mSettings[SETTING_FLAGS] = mSettingFlags;
	}
	else
	{
...
void LLSettingsBase::clearFlag(U32 flag)
{
	update();
	mSettingFlags &= ~flag;
	if (mSettingFlags)
									   ...
									   bool partial)
{
	static Validator validateName(SETTING_NAME, false, LLSD::TypeString,
								  std::bind(&Validator::verifyStringLength,
											_1, _2, 63));
...
bool LLSettingsBase::Validator::verify(LLSD& data, U32 flags) const
{
	...
	if (mVerify && !mVerify(data[mName], flags))
	{
