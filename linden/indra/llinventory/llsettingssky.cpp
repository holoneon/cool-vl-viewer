...
LLSettingsSky::validation_list_t LLSettingsSky::legacyHazeValidationList()
{
	...
	if (legacy_haze.empty())
	{
		legacy_haze.emplace_back(SETTING_AMBIENT, false, LLSD::TypeArray,
								 std::bind(&Validator::verifyVectorMinMax,
										   _1, _2,
		...
		legacy_haze.emplace_back(SETTING_HAZE_DENSITY, false, LLSD::TypeReal,
								 std::bind(&Validator::verifyFloatRange,
...
LLSettingsSky::validation_list_t LLSettingsSky::rayleighValidationList()
{
	...
	if (rayleigh.empty())
	{
		rayleigh.emplace_back(SETTING_DENSITY_PROFILE_WIDTH, false,
							 LLSD::TypeReal,
							 std::bind(&Validator::verifyFloatRange, _1, _2,
...
//static
void LLSettingsSky::adjustTime(LLSD& settings, F32 position)
{
	if (!settings.has(SETTING_SUN_ROTATION))
	{
		// Settings empty or invalid: use default settings.
	...
	LLQuaternion moonquat =
		convert_azimuth_and_altitude_to_quat(altitude + F_PI * 0.125f,
											 azimuth + F_PI * 0.125f);
	settings[SETTING_MOON_ROTATION] = moonquat.getValue();
	settings[SETTING_SUN_ROTATION] = sunquat.getValue();
}
...
//virtual
LLSD LLSettingsSky::interpolateSDValue(const std::string& key_name,
									   const LLSD& value,
...
{
	// SETTING_REFLECTION_PROBE_AMBIANCE does not exist in legacy (pre-PBR) sky
	// settings, so we must deal with such a case here... HB
	if (key_name == SETTING_REFLECTION_PROBE_AMBIANCE)
	{
		
		if (value.isUndefined() && other_value.isUndefined())
		{
			return value;
		}
		if (value.isUndefined())
		{
			return LLSettingsBase::interpolateSDValue(key_name,
													  LLSD::Real(0.0),
													  other_value, defaults,
													  mix, slerps);
		}
		if (other_value.isUndefined())
		{
			return LLSettingsBase::interpolateSDValue(key_name, value,
													  LLSD::Real(0.0),
													  defaults, mix, slerps);
		}
	}
	return LLSettingsBase::interpolateSDValue(key_name, value, other_value,
											  defaults, mix, slerps);
}
