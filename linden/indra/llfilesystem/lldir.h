...
public:
	...
	///////////////////////////////////////////////////////////////////////////
	// OS-specific methods implemented in lldir_<os>.cpp.
	...
	///////////////////////////////////////////////////////////////////////////
	// Methods common to all OSes and implemented in lldir.cpp
	...
	// Return true when a "colors_base.xml" file can be found for a skin which
	// resides in a 'skin_folder' sub-folder of either the viewer installation
	// directory or the user overrides directory. HB
	bool hasSkin(const char* skin_folder) const;

	LL_INLINE bool hasSkin(const std::string& skin_folder) const
	{
		return hasSkin(skin_folder.c_str());
	}
	...
	LL_INLINE bool hasLegacySkinOverrides() const
	{
		return mHasLegacySkinOverrides;
	}
	...
	// Expanded filename
	std::string getFullPath(ELLPath location, const std::string& subdir1,
							const std::string& subdir2,
							const std::string& filename) const;

	LL_INLINE std::string getFullPath(ELLPath loc,
									  const std::string& fname) const
	{
		return getFullPath(loc, LLStringUtil::null, fname);
	}

	LL_INLINE std::string getFullPath(ELLPath loc, const std::string& subdir,
									  const std::string& fname) const
	{
		return getFullPath(loc, LLStringUtil::null, subdir, fname);
	}

	LL_INLINE std::string getFullPath(ELLPath location) const
	{
		return getFullPath(location, LLStringUtil::null);
	}

	// Base and directory name extraction
	static std::string getBaseFileName(const std::string& filepath,
										bool strip_exten = false);
...
private:
	...
	// true when user skin overrides have been detected in the skin/ override
	// folder and no cvlv-skin/ override folder is present.
	bool		mHasLegacySkinOverrides;
};

extern LLDir gDirUtil;
