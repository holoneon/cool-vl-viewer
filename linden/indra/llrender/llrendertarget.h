...
public:
	...
	bool allocate(U32 resx, U32 resy, U32 color_fmt, bool depth = false,
				  ...
				  // Optional target name, for debugging purpose, for now only
				  // used with LLTexUnit::TMG_AUTO. HB
	...
	// Blits depth from source render target into this target's depth buffer.
	// Supports different resolutions (nearest-filter downsample/upsample).
	// This target must have its own depth buffer allocated. Used only by the
	// PBR renderer.
	...
	LL_INLINE U32 getMipLevels() const					{ return mMipLevels; }
	// Rebinds FBO depth attachment at a specific mip level and set viewport.
	// Used for Hi-Z pyramid generation. Caller must call resetDepthMipLevel()
	// when done.
	void bindDepthMipLevel(S32 level);
	// Restores FBO depth attachment to mip level 0 and full viewport.
	void resetDepthMipLevel();
	// Rebinds FBO color attachment at a specific mip level and set viewport.
	// Used for per-mip SSR filter. Caller must call resetColorMipLevel() when
	// done.
	void bindColorMipLevel(S32 level, U32 attachment = 0);
	// Restores FBO color attachment to mip level 0 and full viewport.
	...
	// *HACK: for PBR material preview only.
...
protected:
	...
	S32							mBytesAllocated;	// Bytes allocated in VRAM
	bool						mUseDepth;
