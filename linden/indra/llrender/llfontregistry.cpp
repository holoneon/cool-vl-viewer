...
static bool font_desc_init_from_xml(LLXMLNodePtr node, LLFontDescriptor& desc)
{
...
#endif
			desc.addFontFile(fontname, char_functor);
		}
...
///////////////////////////////////////////////////////////////////////////////
// LLFontDescriptor class
///////////////////////////////////////////////////////////////////////////////

//static
LLFontDescriptor::char_functor_map_t
	LLFontDescriptor::sCharFunctors({ { "is_emoji", LLStringOps::isEmoji } });
...
LLFontDescriptor LLFontDescriptor::normalize() const
{
	...
	static const std::string little_str("little");
	...
	if (removeSubString(new_name, little_str))
	{
		new_size = "Little";
	}
	...
	if (removeSubString(new_name, large_str) ||
		removeSubString(new_name, big_str))
	{
		new_size = "Large";
	}
	...
	return LLFontDescriptor(new_name, new_size, new_style, mFontFiles);
}

void LLFontDescriptor::addFontFile(const std::string& file_name,
								   const std::string& char_functor)
{
	char_functor_map_t::const_iterator it = sCharFunctors.find(char_functor);
	mFontFiles.push_back(LLFontFileInfo(file_name,
										it != sCharFunctors.end() ? it->second : NULL));
}

///////////////////////////////////////////////////////////////////////////////
// LLFontRegistry class
///////////////////////////////////////////////////////////////////////////////

LLFontRegistry::LLFontRegistry(const strings_vec_t& xui_paths,
...
bool LLFontRegistry::parseFontInfo(const std::string& xml_filename)
{
	...
	{
		full_filename = gDirUtil.findSkinnedFilename(mXUIPaths[i],
													 xml_filename);
...
LLFontGL* LLFontRegistry::createFont(const LLFontDescriptor& desc)
{
	...
	if (match_desc)
	{
		...
		if (it != mFontMap.end() && it->second != NULL)
		{
			...
			fontp->mFontDescriptor = desc;
			fontp->mFontFreetype = it->second->mFontFreetype;
			mFontMap[desc] = fontp;
			return fontp;
		}
	}
	...
	if (match_desc)
	{
		...
		font_files = match_desc->getFontFiles();
		...
		// elsewhere.
		std::transform(mUltimateFallbackList.begin(),
					   mUltimateFallbackList.end(),
					   std::back_inserter(font_files),
					   [](const std::string& file_name)
					   {
							return LLFontFileInfo(file_name);
					   });
	}
	else
	{
		...
		font_files.emplace_back(fname + ".ttf");
		font_files.emplace_back(fname + ".otf");
		font_files.emplace_back(fname + ".ttc");
		font_files.emplace_back(fname + ".otc");
#if LL_LINUX
		...
		if (nfname != fname)
		{
			font_files.emplace_back(nfname + ".ttf");
			font_files.emplace_back(nfname + ".otf");
			font_files.emplace_back(nfname + ".ttc");
			font_files.emplace_back(nfname + ".otc");
#if LL_LINUX
			// Linux file systems are case-sensitive...
			font_files.emplace_back(nfname + ".TTF");
			font_files.emplace_back(nfname + ".OTF");
	...
	LLFontGL* resultp = NULL;

	// The first font will get pulled will be the "head" font, set to non-
	//fallback and become the "head" font, set to non-fallback. The rest will
	...
	font_paths.emplace_back(gDirUtil.getAppRODataDir() + "/fonts/");
	...
	else	// Try and find a matching font file name among system fonts... HB
	{
		...
		for (strings_set_t::iterator it = linux_paths.begin(),
									 end = linux_paths.end();
			 it != end; ++it)
		{
	...
	for (S32 i = 0, count = font_files.size(); i < count; ++i)
	{
		LLFontGL* fontp = new LLFontGL;
		const std::string& file_name = font_files[i].mFileName;
		...
		if (fontp)
		{
			...
			else
			{
				LL_DEBUGS("FontRegistry") << "Adding: " << font_path
										  << LL_ENDL;
				resultp->mFontFreetype->addFallbackFont(fontp->mFontFreetype,
														font_files[i].mCharFunctor);
				delete fontp;
				fontp = NULL;
			}
		}
	...
	if (resultp)
	{
		...
		resultp->mFontDescriptor = norm_desc;
	}
	else
	{
	...
	mFontMap[norm_desc] = resultp;
#else
	if (resultp)
	{
		llinfos << "Created font " << desc.getName() << llendl;
		resultp->mFontDescriptor = desc;
	}
	else
	{
		llwarns << "Failure to create font " << desc.getName()
...
void LLFontRegistry::dump()
{
		 ...
		 font_it != end; ++font_it)
	{
		...
		const font_file_info_vec_t& font_files = desc.getFontFiles();
		for (S32 i = 0, count = font_files.size(); i < count; ++i)
		{
			llinfos << "  file: " << font_files[i].mFileName << llendl;
		}
	}
}
