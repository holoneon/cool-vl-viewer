...
public:
	...
	// 5 - modify LLShaderMgr::sReservedAttribs
	...
	LL_INLINE void unmapBuffer()					{ flushBuffers(); }

	// Flushes any pending mapped buffers
	...
	// Used for glTF primitves rendering: does not sync GL matrices, and the
	// render mode must be specified.
	void drawRangeFast(U32 mode, U32 start, U32 end, U32 count,
					   U32 idx_offset) const;
...
protected:
	...
	// Actually unmaps buffer
	void doUnmapBuffer();
	// Adds bugget to set of mapped buffers
	void mapBuffer();
...
protected:
	...
	// true while buffer is mapped. Must be set to true only by mapBuffer() and
	// reset to false only by flushBuffers() in order to keep sMappedBuffers in
	// proper sync.
	bool					mMapped;
...
private:
	// List of mapped buffers: must not be LLPointer<LLVertexBuffer> to avoid
	// breaking non-ref-counted LLVertexBuffer instances.
...
class alignas(16) LLVertexBufferData
{
protected:
	LOG_CLASS(LLVertexBufferData);

public:
	LL_ALIGNED16_NEW_DELETE

	LL_INLINE LLVertexBufferData()
	:	mVB(NULL),
		mMode(0),
		mCount(0),
		mTexName(0)
	...
	LL_INLINE LLVertexBufferData(LLVertexBuffer* buffp, U8 mode, U32 count,
								 U32 tex_name, const LLMatrix4a& model,
								 const LLMatrix4a& proj, const LLMatrix4a& tex)
	:	mVB(buffp),
		mMode(mode),
		mCount(count),
		mTexName(tex_name),
	...
	typedef std::list<LLVertexBufferData> list_t;
