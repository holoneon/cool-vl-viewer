...
// The gl_draw_box*() functions below used to be drawBox*() functions
// implemented in llspatialpartition.cpp (for debug display features) but are
// also used elsewhere, so I grouped them here. HB

// Note: unbinds the current vertex buffer.
void gl_draw_box(const LLVector3& c, const LLVector3& r);

LL_INLINE void gl_draw_box(const LLVector4a& c, const LLVector4a& r)
{
	gl_draw_box(reinterpret_cast<const LLVector3&>(c),
				reinterpret_cast<const LLVector3&>(r));
}

void gl_draw_box_outline(const LLVector3& pos, const LLVector3& size);

LL_INLINE void gl_draw_box_outline(const LLVector4a& pos,
								   const LLVector4a& size)
{
	gl_draw_box_outline(reinterpret_cast<const LLVector3&>(pos),
