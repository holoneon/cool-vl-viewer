...
protected:
	...
	// MUST already be active and bound before calling those.
...
public:
	...
	LL_INLINE U8 getMode() const					{ return mMode; }
	...
	// When 'flush_before' is true, a flush() call is done before setting the
	// line width. However, neither the flush() or the glLineWidth() calls are
	// performed when using the core GL profile. The very purpose of the
	// 'flush_before' parameter is to avoid pointless and costly flush() calls
	// when under the core GL profile. HB
	void lineWidth(F32 width, bool flush_before = false);

	void vertexBatchPreTransformed(LLVector4a* verts, S32 vert_count);
	void vertexBatchPreTransformed(LLVector4a* verts, LLVector2* uvs,
								   S32 vert_count);
	void vertexBatchPreTransformed(LLVector4a* verts, LLVector2* uvs,
...
private:
	...
	LLVertexBuffer* genBuffer(U32 attribute_mask, U32 count);

public:
	static U32					sCurrentFrame;
	static bool					sGLCoreProfile;
	static bool					sUseBufferCache;
	static bool					sClassicMode;
