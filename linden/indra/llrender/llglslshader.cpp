						 ...
						 const char* geometry_shader)
{
	...
	mFeatures.calculatesLighting = mFeatures.calculatesAtmospherics =
		...
		mFeatures.isDeferred = mFeatures.hasFullGBuffer = mFeatures.hasShadows =
...
{
	for (size_t i = 0, count = mGLTFVariants.size(); i < count; ++i)
	{
		mGLTFVariants[i].unload();
	}
	mGLTFVariants.clear();
	mShaderFiles.clear();
	mDefines.clear();
								...
								const char** varyings)
{
	...
	if (LLGLManager::sGLSLVersionMajor < 2 &&
		LLGLManager::sGLSLVersionMinor < 3)
	{
	...
	const S32 indexed_channels = mFeatures.mIndexedTextureChannels;
	if (indexed_channels > 0)
	{
		// Override texture channels for indexed texture rendering
		bind();

		for (S32 i = 0; i < indexed_channels; ++i)
		{
			LLStaticHashedString uniform_name(llformat("tex%d", i));
			uniform1i(uniform_name, i);
		}

		// Get the true number of active texture channels
		mActiveTextureChannels = indexed_channels;
		// Adjust any texture channels that might have been overwritten
		for (size_t i = 0, count = mTexture.size(); i < count; ++i)
		{
			S32& tex = mTexture[i];
			if (tex > -1)
			{
				tex += indexed_channels;
				uniform1i(i, tex);
			}
			if (tex >= mActiveTextureChannels)
			{
				mActiveTextureChannels = tex + 1;
			}
		}
	...
	LL_DEBUGS("ShaderLoading") << mName << " has " << mActiveTextureChannels
							   << " active texture channels:";
	for (size_t i = 0, count = mTexture.size(); i < count; ++i)
	{
		if (mTexture[i] > -1)
		{
			LL_CONT << "\n    Texture " << LLShaderMgr::sReservedUniforms[i]
					<< " assigned to channel " << mTexture[i];
		}
	}
	LL_CONT << LL_ENDL;
...
void LLGLSLShader::addPermutations(const strings_map_t& defines)
{
	for (strings_map_t::const_iterator it = defines.begin(),
									   end = defines.end();
		 it != end; ++it)
	{
...
bool LLGLSLShader::mapUniforms(const hash_vector_t* uniforms)
{
	...
	if (gUsePBRShaders)
	{
		...
		for (U32 i = 0; i < NUM_UNIFORM_BLOCKS; ++i)
		{
			...
			GLuint idx = glGetUniformBlockIndex(mProgramObject, ubo_names[i]);
			if (idx != GL_INVALID_INDEX)
			{
				// Set this index to a binding index
				glUniformBlockBinding(mProgramObject, idx, i);
			}
		}
	}
...
void LLGLSLShader::bindVariant(U8 variant)
{
	if ((size_t)variant < mGLTFVariants.size())
	{
							  ...
							  LLTexUnit::eTextureType mode)
{
	...
	if (channel >= 0)
	{
		...
		if (unitp)	// Paranoia ?  HB
		{
			unitp->bindFast(texp);
		}
		else
		{
			llwarns_once << "No texture unit for channel: " << channel
						 << llendl;
		}
	}
	return channel;
...
S32 LLGLSLShader::disableTexture(S32 uniform, LLTexUnit::eTextureType mode)
{
	...
	if (channel >= 0)
	{
		...
		else if (unitp->getCurrType() != LLTexUnit::TT_NONE)
		{
...
void LLGLSLShader::uniform4uiv(U32 index, U32 count, const U32* v)
{
	if (mProgramObject)
	{
		S32 uniform = getUniform(__LINE__, index);
...
void LLGLSLShader::uniform4uiv(const LLStaticHashedString& uniform, U32 count,
							   const U32* v)
{
	S32 location = getUniformLocation(uniform);
	if (location >= 0)
...
void LLGLSLShader::uniform4f(const LLStaticHashedString& uniform, F32 x,
							 F32 y, F32 z, F32 w)
{
	S32 location = getUniformLocation(uniform);
	if (location >= 0)
...
void LLShaderUniforms::apply(LLGLSLShader* shaderp)
{
	if (!mActive)
	{
		return;
	}
	for (size_t i = 0, count = mIntegers.size(); i < count; ++i)
	{
		const IntSetting& uniform = mIntegers[i];
		shaderp->uniform1i(uniform.mUniform, uniform.mValue);
	}
	for (size_t i = 0, count = mFloats.size(); i < count; ++i)
	{
		const FloatSetting& uniform = mFloats[i];
		shaderp->uniform1f(uniform.mUniform, uniform.mValue);
	}
	for (size_t i = 0, count = mVectors.size(); i < count; ++i)
	{
		const VectorSetting& uniform = mVectors[i];
		shaderp->uniform4fv(uniform.mUniform, 1, uniform.mValue.mV);
	}
	for (size_t i = 0, count = mVector3s.size(); i < count; ++i)
	{
		const Vector3Setting& uniform = mVector3s[i];
		shaderp->uniform3fv(uniform.mUniform, 1, uniform.mValue.mV);
	}
}
