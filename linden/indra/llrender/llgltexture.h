...
public:
	...
	// Overridden for fetched textures only; made virtual so that it can be
	// called from the llgltf library using a LLGLTexture pointer on a fecthed
	// texture. HB
	virtual void addTextureStats(F32 vsize, bool needs_gltexture = true) const
	{
	}
	...
	LL_INLINE void setMegaTexture(bool b = true)		{ mIsMegaTexture = b; }
...
protected:
	...
	// Used to allow overriding the download render limit for textures that do
	// deserve being larger than 1024x1024. Typically only for PBR terrain
	// textures, for now. HB
	bool					mIsMegaTexture;
...
extern S32 gMaxImageSizeDefault;
