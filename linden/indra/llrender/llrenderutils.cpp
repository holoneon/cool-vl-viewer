									  ...
									  const LLRectf& scale_rect)
{
	...
	alignas(16) thread_local LLVector4a pos[NUM_VERTICES];
	thread_local LLVector2 uv[NUM_VERTICES];
	...
	gGL.begin(LLRender::TRIANGLES);
	{
		...
		uv[index].set(uv_center_rect.mLeft, uv_rect.mBottom);
		...
		uv[index].set(uv_rect.mLeft, uv_center_rect.mBottom);
		...
		uv[index].set(uv_center_rect.mRight, uv_rect.mBottom);
		...
		uv[index].set(uv_rect.mRight, uv_center_rect.mBottom);
		...
		uv[index].set(uv_rect.mLeft, uv_center_rect.mTop);
		...
		uv[index].set(uv_rect.mRight, uv_center_rect.mTop);
		...
		uv[index].set(uv_center_rect.mLeft, uv_rect.mTop);
		...
		uv[index].set(uv_center_rect.mRight, uv_rect.mTop);
								  ...
								  const LLRectf& uv_rect)
{
	...
	if (degrees == 0.f)
	{
		...
		alignas(16) LLVector4a pos[NUM_VERTICES];
		...
		gGL.begin(LLRender::TRIANGLES);
		{
			...
			uv[index].set(uv_rect.mRight, uv_rect.mTop);
			pos[index++].set(ui_translation.mV[VX] + scaled_width,
							 ui_translation.mV[VY] + scaled_height, 0.f);

			uv[index].set(uv_rect.mLeft, uv_rect.mTop);
			pos[index++].set(ui_translation.mV[VX],
							 ui_translation.mV[VY] + scaled_height, 0.f);

			uv[index].set(uv_rect.mLeft, uv_rect.mBottom);
			pos[index++].set(ui_translation.mV[VX], ui_translation.mV[VY],
							 0.f);
							  ...
							  S32 border_size, U32 edges)
{
	...
	{
		border_width_left.set(border_scale, 0.f);
	}
	if ((edges & (~(U32)ROUNDED_RECT_LEFT)) != 0)
	{
		border_width_right.set(border_scale, 0.f);
	}
	if ((edges & (~(U32)ROUNDED_RECT_TOP)) != 0)
	{
		border_height_bottom.set(0.f, border_scale);
	}
	if ((edges & (~(U32)ROUNDED_RECT_BOTTOM)) != 0)
	{
		border_height_top.set(0.f, border_scale);
	}
...
void gl_draw_box(const LLVector3& c, const LLVector3& r)
{
...
void gl_draw_box_outline(const LLVector3& pos, const LLVector3& size)
{
...
void LLRenderSphere::renderGGL()
{
	...
	{
		constexpr U32 nverts = (LATITUDE_SLICES + 1) * (LONGITUDE_SLICES + 1);
		constexpr U32 nindices = LATITUDE_SLICES * LONGITUDE_SLICES * 6;
		mVertexBuffer = new LLVertexBuffer(LLVertexBuffer::MAP_VERTEX);
		mVertexBuffer->allocateBuffer(nverts, nindices);
		...
		constexpr S32 stride = LONGITUDE_SLICES + 1;
		for (S32 lat_i = 0; lat_i < LATITUDE_SLICES + 1; ++lat_i)
		{
			...
			for (S32 lon_i = 0; lon_i < LONGITUDE_SLICES + 1; ++lon_i)
			{
				...
				v[lat_i * stride + lon_i] = mSpherePoints[lat_i][lon_i];

				// Populate indices.
				if (lat_i < LATITUDE_SLICES && lon_i < LONGITUDE_SLICES)
				{
					S32 idx = (lat_i * LONGITUDE_SLICES + lon_i) * 6;
					i[idx++] = lat_i * stride + lon_i;
					i[idx++] = lat_i * stride + lon_i + 1;
					i[idx++] = (lat_i + 1) * stride + lon_i;
					i[idx++] = (lat_i + 1) * stride + lon_i;
					i[idx++] = lat_i * stride + lon_i + 1;
					i[idx] = (lat_i + 1) * stride + lon_i + 1;
				}
			}
		}
	...
	if (shaderp && shaderp->mAttributeMask == LLVertexBuffer::MAP_VERTEX)
	{
		// Note: the LLVertexBuffer::MAP_VERTEX parameter is ignored when in
		// PBR rendering mode. HB
		mVertexBuffer->setBuffer(LLVertexBuffer::MAP_VERTEX);
		mVertexBuffer->drawRangeFast(0, mVertexBuffer->getNumVerts(),
									 mVertexBuffer->getNumIndices(), 0);
		return;
	}

	// No shader (may happen when not in PBR mode), or shader wants colors in
	// the vertex stream, use slow path.
	gGL.begin(LLRender::TRIANGLES);
