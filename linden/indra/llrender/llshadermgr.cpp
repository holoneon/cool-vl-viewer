...
strings_vec_t LLShaderMgr::sReservedAttribs;
strings_vec_t LLShaderMgr::sReservedUniforms;
...
bool LLShaderMgr::attachShaderFeatures(LLGLSLShader* shader)
{
	...
	// Note: disableTextureIndex is no more in use by the PBR renderer. HB
	bool disable_tex_index = features->disableTextureIndex;
	if (gUsePBRShaders)
	{
		disable_tex_index = features->mIndexedTextureChannels <= 1;
	}
	...
	else if (gUsePBRShaders && (features->hasGamma || features->isDeferred) &&
			 !shader->attachVertexObject("windlight/atmosphericsVarsV.glsl"))
	{
		return false;
	}
	...
	if (gUsePBRShaders && features->hasFullGBuffer &&
		!shader->attachFragmentObject("deferred/gbufferUtil.glsl"))
	{
		return false;
	}
...
static std::string get_shader_log(GLuint object)
{
	...
	if (LLGLManager::sIsIntel && res.find("No errors") == 0)
	{
		res.clear();
	}
								   ...
								   S32 texture_index_channels)
{
	...
	S32 major_version = LLGLManager::sGLSLVersionMajor;
	S32 minor_version = LLGLManager::sGLSLVersionMinor;
	...
	else
	{
		...
		else
		{
			...
			if (minor_version >= 50 && LLGLManager::sHasGpuShader5 &&
				!gUsePBRShaders)
			{
	...
	if (gUsePBRShaders)
	{
		...
		header.emplace_back("#define GET_GBUFFER_FLAG(data, flag) (abs(data-flag)< 0.1)\n");
	}
	...
	if (defines)
	{
		for (strings_map_t::iterator iter = defines->begin(),
									 end = defines->end();
			 iter != end; ++iter)
		{
	...
	if (gUsePBRShaders)
	{
		...
		header.emplace_back("struct GBufferInfo { vec4 albedo; vec4 specular; vec3 normal; vec4 emissive; float gbufferFlag; float envIntensity; };\n");
	}
...
void LLShaderMgr::initAttribsAndUniforms()
{
	...
	llassert_always(sReservedUniforms.size() == size_t(OBJECT_PLANE_T + 1));
	...
	llassert_always(sReservedUniforms.size() ==
	...
	llassert_always(sReservedUniforms.size() == size_t(MULTI_LIGHT_FAR_Z + 1));
	...
	llassert_always(sReservedUniforms.size() == size_t(PROJECTOR_LOD + 1));
	...
	sReservedUniforms.emplace_back("metallicRoughnessMap");	// For PBR shaders
	sReservedUniforms.emplace_back("normalMap");
	sReservedUniforms.emplace_back("occlusionMap");			// For PBR shaders
	...
	sReservedUniforms.emplace_back("classic_mode");			// For PBR shaders
	...
	llassert_always(sReservedUniforms.size() == size_t(GLOW_NOISE_MAP + 1));

	sReservedUniforms.emplace_back("minimum_alpha");
	sReservedUniforms.emplace_back("emissive_brightness");
	sReservedUniforms.emplace_back("alpha_gamma");			// For PBR shaders
	...
	sReservedUniforms.emplace_back("maxThickness");
	sReservedUniforms.emplace_back("depthBias");
	sReservedUniforms.emplace_back("glossySampleCount");
	sReservedUniforms.emplace_back("noiseSine");
	sReservedUniforms.emplace_back("maxZDepth");
	sReservedUniforms.emplace_back("maxRoughness");

	llassert_always(sReservedUniforms.size() ==
						size_t(DEFERRED_SSR_MAX_ROUGHNESS + 1));
	...
	sReservedUniforms.emplace_back("default_probe_render");
	sReservedUniforms.emplace_back("reflection_probe_quality");
	...
	llassert_always(sReservedUniforms.size() == size_t(DEFERRED_SHADOW5 + 1));
	...
	sReservedUniforms.emplace_back("exclusionTex");	// For PBR shaders only
	...
	sReservedUniforms.emplace_back("edgesTex");
	sReservedUniforms.emplace_back("areaTex");
	sReservedUniforms.emplace_back("searchTex");
	sReservedUniforms.emplace_back("blendTex");
