...
#include "llatomic.h"
#include "llimage.h"		// 	LL_IMAGE_AREANA_NEW_DELETE
#include "llmutex.h"
...
public:
	...
	// with caution.
	LLImageGL(U32 tex_name, U32 components, U32 target, S32 internal_fmt,
			  S32 primary_fmt, S32 format_type,
	...
	// When a bad format is detected, 'valid' is set to false, or true
	// otherwise. HB
	static U32 dataFormatBits(S32 dataformat, bool& valid);
	static S64 dataFormatBytes(S32 dataformat, U32 width, U32 height);
	static U32 dataFormatComponents(S32 dataformat);

	// The two following methods need to be called every frame
	static void freeDeletedTextures(bool delete_all = false);
	...
	// Returns true on success, false on failure to recognize intformat,
	// pixformat or pixtype as valid. HB
	static bool setManualImage(U32 target, S32 miplevel, S32 intformat,
	...
	// Used by LLGLTexture, LLCubeMap and LLCubeMapArray.
	void setTexName(U32 name);

	// Similar to setTexName(), but will call deleteTextures on mTexName if
	// mTexName is not 0 or texname.
	// Used by LLBumpImageList and LLViewerMediaImpl.
...
private:
	...
	// Used to log image address in hexadecimal, or texture owner UUID. HB
...
private:
	...
	GLsync					mTexNameSync;
	// Prevents heavy use of sSyncMutex. HB
	LLAtomicBool			mHasSync;

	LLAtomicU32				mNewTexName;
	U32						mTexName;
	...
	// Protects mTexNameSync. HB
	static LLMutex			sSyncMutex;
...
public:
	...
	// Atomic so that we do not need a mutex to read it. HB
...
private:
	static constexpr U32 FREE_LIST_SIZE = 2;
	static std::vector<U32>	sFreeList[FREE_LIST_SIZE];
	// Current frame number.
	static U32				sCurrentFrame;
	static bool				sDelayedTextureDelete;

	// Protects sImageList and sFreeList. HB
