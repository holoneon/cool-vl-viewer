...
void LLTransferManager::init()
{
	...
	msg->setHandlerFunc(_PREHASH_TransferRequest, processTransferRequest,
						NULL);
	msg->setHandlerFunc(_PREHASH_TransferInfo, processTransferInfo, NULL);
	msg->setHandlerFunc(_PREHASH_TransferPacket, processTransferPacket, NULL);
	msg->setHandlerFunc(_PREHASH_TransferAbort, processTransferAbort, NULL);
}
...
void LLTransferManager::processTransferRequest(LLMessageSystem* msgp, void**)
{
	...
	msgp->getUUID(_PREHASH_TransferInfo, _PREHASH_TransferID, transfer_id);
	msgp->getS32(_PREHASH_TransferInfo, _PREHASH_SourceType,
				 (S32&)source_type);
	msgp->getS32(_PREHASH_TransferInfo, _PREHASH_ChannelType,
				 (S32&)channel_type);
	msgp->getF32(_PREHASH_TransferInfo, _PREHASH_Priority, priority);
	...
	S32 size = msgp->getSize(_PREHASH_TransferInfo, _PREHASH_Params);
	if (size < 0)
	{
	...
	msgp->getBinaryData(_PREHASH_TransferInfo, _PREHASH_Params, tmp, size);
...
void LLTransferManager::processTransferInfo(LLMessageSystem* msgp, void**)
{
	...
	msgp->getUUID(_PREHASH_TransferInfo, _PREHASH_TransferID, transfer_id);
	msgp->getS32(_PREHASH_TransferInfo, _PREHASH_TargetType,
				 (S32&)target_type);
	msgp->getS32(_PREHASH_TransferInfo, _PREHASH_ChannelType,
				 (S32&)channel_type);
	msgp->getS32(_PREHASH_TransferInfo, _PREHASH_Status, (S32&)status);
	msgp->getS32(_PREHASH_TransferInfo, _PREHASH_Size, size);
	...
	// Unpack the params
	S32 params_size = msgp->getSize(_PREHASH_TransferInfo, _PREHASH_Params);
	if (params_size < 0)
	{
	...
	{
		U8 tmp[MAX_PARAMS_SIZE];
		msgp->getBinaryData(_PREHASH_TransferInfo, _PREHASH_Params, tmp,
							params_size);
		LLDataPackerBinaryBuffer dpb(tmp, MAX_PARAMS_SIZE);
...
void LLTransferManager::processTransferPacket(LLMessageSystem* msgp, void**)
{
	...
	LLTSCode status;
	S32 size;
	msgp->getUUID(_PREHASH_TransferData, _PREHASH_TransferID, transfer_id);
	msgp->getS32(_PREHASH_TransferData, _PREHASH_ChannelType,
				 (S32&)channel_type);
	msgp->getS32(_PREHASH_TransferData, _PREHASH_Packet, packet_id);
	msgp->getS32(_PREHASH_TransferData, _PREHASH_Status, (S32&)status);
	...
	size = msgp->getSize(_PREHASH_TransferData, _PREHASH_Data);
	...
	if (size > 0)
	{
		...
		msgp->getBinaryData(_PREHASH_TransferData, _PREHASH_Data, tmp_data,
							size);
	}
...
void LLTransferSourceChannel::updateTransfers()
{
		 ...
		 iter != mTransferSources.mMap.end() && !done; )
	{
		...
		msg->newMessage(_PREHASH_TransferPacket);
		msg->nextBlock(_PREHASH_TransferData);
		msg->addUUID(_PREHASH_TransferID, tsp->getID());
		msg->addS32(_PREHASH_ChannelType, getChannelType());
		// *HACK: need a REAL packet id
		msg->addS32(_PREHASH_Packet, packet_id);
		msg->addS32(_PREHASH_Status, status);
		msg->addBinaryData(_PREHASH_Data, datap, data_size);
												  ...
												  F32 priority)
{
	...
	LLMessageSystem* msg = gMessageSystemp;
	msg->newMessage(_PREHASH_TransferRequest);
	msg->nextBlock(_PREHASH_TransferInfo);
	msg->addUUID(_PREHASH_TransferID, targetp->getID());
	msg->addS32(_PREHASH_SourceType, params.getType());
	msg->addS32(_PREHASH_ChannelType, getChannelType());
	msg->addF32(_PREHASH_Priority, priority);

	U8 tmp[MAX_PARAMS_SIZE];
	LLDataPackerBinaryBuffer dp(tmp, MAX_PARAMS_SIZE);
	params.packParams(dp);
	S32 len = dp.getCurrentSize();
	msg->addBinaryData(_PREHASH_Params, tmp, len);
...
{
	LLMessageSystem* msg = gMessageSystemp;
	msg->newMessage(_PREHASH_TransferInfo);
	msg->nextBlock(_PREHASH_TransferInfo);
	msg->addUUID(_PREHASH_TransferID, getID());
	msg->addS32(_PREHASH_TargetType, LLTTT_UNKNOWN);
	msg->addS32(_PREHASH_ChannelType, mChannelp->getChannelType());
	msg->addS32(_PREHASH_Status, status);
	msg->addS32(_PREHASH_Size, mSize);
	U8 tmp[MAX_PARAMS_SIZE];
	LLDataPackerBinaryBuffer dp(tmp, MAX_PARAMS_SIZE);
	packParams(dp);
	S32 len = dp.getCurrentSize();
...
void LLTransferSource::abortTransfer()
{
	...
	LLMessageSystem* msg = gMessageSystemp;
	msg->newMessage(_PREHASH_TransferAbort);
	msg->nextBlock(_PREHASH_TransferInfo);
	msg->addUUID(_PREHASH_TransferID, getID());
	msg->addS32(_PREHASH_ChannelType, mChannelp->getChannelType());
	msg->sendReliable(mChannelp->getHost());
