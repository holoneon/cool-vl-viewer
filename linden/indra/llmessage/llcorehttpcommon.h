...
struct HttpStatus
{
	...
	HttpStatus();
	HttpStatus(type_enum_t type, type_enum_t status);
	...
	// Tests for successful status in the code regardless of error source
	// (internal, libcurl). Returns true' when status is successful.
	LL_INLINE operator bool() const
	{
		return mDetails && mDetails->mStatus == HE_SUCCESS;
	}

	// Inverse of previous operator. Returns true on any error condition.
	...
	// Converts to single numeric representation. Mainly for logging or other
	...
	// Converts status to a C string representation. For success, returns an
	// empty string. For failure statuses, a string as appropriate for the
	// source of the error code (libcurl easy, libcurl multi or LLCore itself).
	const char* toString() const;

	// Converts status to a compact string representation of the form:
	...
	LL_INLINE type_enum_t getStatus() const
	{
	...
	LL_INLINE const std::string& getMessage() const
	{
		return mDetails ? mDetails->mMessage : LLStringUtil::null;
	}
...
private:
	struct Details
	{
		LL_INLINE Details(type_enum_t type, type_enum_t status)
		:	mErrorData(NULL),
			mType(type),
			mStatus(status)
		...
		LL_INLINE Details(const Details& rhs)
...
namespace LLHttp
{
	...
	// Declared here, but implemented in llcorehttplibcurl.cpp, since they do
	// involve calling the curl library. The idea is to avoid including curl.h
	// as much as possible from other viewer modules... HB
    void initialize();
    void cleanup();
