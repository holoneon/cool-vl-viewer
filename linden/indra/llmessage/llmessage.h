...
constexpr S32 MESSAGE_MAX_PER_FRAME = 500;
...
public:
	...
	LLMessageSystem(const std::string& filename, U32 port,
	...
	// Gets the binary data from the current message.
	// 'blockname' is the name of the block in the message (from the message
	// template), 'varname' is the variable name in the block, 'datap' is the
	// pointerto the buffer where to store the binary data, 'size' is the
	// expected size (set to zero to get any amount of data up to max_size and
	// make sure max_size is set in that case !), 'blocknum' is the block
	// number (when any), 'max_size the' max number of bytes to read.
	...
	LL_INLINE F32 getBufferLoadRate() const
	{
		return mPacketRing.getBufferLoadRate();
	}
