...
public:
	...
	// Threading: callable by any thread but uses getState() and acquires its
	// weaknesses.
	LL_INLINE static bool isStopped()
	{
		...
		// but will very soon). *TODO: now that we migrated to std::thread,
		// check whether the "joinability" issue is still present or not...
...
protected:
	...
	S32 processRequestQueue(ELoopSpeed loop);
...
protected:
	...
	// Shared data
	std::unique_ptr<HttpThread>		mThread;
	HttpRequestQueue*				mRequestQueue;		// Refcounted
	LLAtomicU32						mExitRequested;
	static volatile EState			sState;

	// Consumer-thread-only data (working-thread-only data); simple pointers,
	// have ownership
	HttpPolicy*						mPolicy;
