...
void LLExperienceCache::initSingleton()
{
	...
	doPeriodically(std::bind(&LLExperienceCache::idle, this),
...
void LLExperienceCache::requestExperiences()
{
	...
	constexpr U32 PAGE_SIZE = EXP_URL_SEND_THRESHOLD / UUID_STR_LENGTH;
	const std::string page_size = llformat("?page_size=%d", PAGE_SIZE);
	...
	while (!mRequestQueue.empty() && !sShutdown)
	{
		...
		if (mRequestQueue.empty() || uri.size() > EXP_URL_SEND_THRESHOLD)
		{
			...
			cpmgr->enqueueCoprocedure("ExpCache", "RequestExperiences",
									  std::bind(&LLExperienceCache::requestExperiencesCoro,
...
bool LLExperienceCache::idle()
{
	if (sShutdown)
	{
		return true;	// Aborts/unregisters doPeriodically(). HB
	}

	constexpr F32 ERASE_EXPIRED_TIMEOUT = 60.f;	// In seconds
												  ...
												  experience_get_fn_t fn)
{
	...
	cpmgr->enqueueCoprocedure("ExpCache", "Fetch Associated",
							  std::bind(&LLExperienceCache::fetchAssociatedExperienceCoro,
											 ...
											 experience_get_fn_t fn)
{
	...
	cpmgr->enqueueCoprocedure("ExpCache", "Search Name",
							  std::bind(&LLExperienceCache::findExperienceByNameCoro,
											...
											experience_get_fn_t fn)
{
	...
	cpmgr->enqueueCoprocedure("ExpCache", "Group Experiences",
							  std::bind(&LLExperienceCache::getGroupExperiencesCoro,
										this, _1, group_id, fn));
}
											 ...
											 experience_get_fn_t fn)
{
	...
	cpmgr->enqueueCoprocedure("ExpCache", "Region Experiences",
							  std::bind(&LLExperienceCache::regionExperiencesCoro,
												...
												experience_get_fn_t fn)
{
	...
	permissionInvoker_fn invoker(std::bind(COROCAST2(&LLCoreHttpUtil::HttpCoroutineAdapter::getAndSuspend),
										   // _1 -> adapter
										   // _2 -> url
										   _1, _2,
	...
	cpmgr->enqueueCoprocedure("ExpCache", "Preferences Set",
							  std::bind(&LLExperienceCache::experiencePermissionCoro,
												...
												experience_get_fn_t fn)
{
	...
	permissionInvoker_fn invoker(std::bind(COROCAST(&LLCoreHttpUtil::HttpCoroutineAdapter::putAndSuspend),
										   // _1 -> adapter
										   // _2 -> url
										   _1, _2, data,
										   LLCore::HttpOptions::ptr_t(),
												   ...
												   experience_get_fn_t fn)
{
	...
	std::string url = mCapability("ExperiencePreferences") + "?" +
					  exp_id.asString();
	permissionInvoker_fn invoker(std::bind(COROCAST2(&LLCoreHttpUtil::HttpCoroutineAdapter::deleteAndSuspend),
										   // _1 -> adapter
										   // _2 -> url
										   _1, _2,
										   LLCore::HttpOptions::ptr_t(),
										   ...
										   experience_get_fn_t fn)
{
	...
	cpmgr->enqueueCoprocedure("ExpCache", "IsAdmin",
							  std::bind(&LLExperienceCache::getExperienceAdminCoro,
										this, _1, exp_id, fn));
}
...
void LLExperienceCache::updateExperience(LLSD upd_data, experience_get_fn_t fn)
{
	...
	LLCoprocedureManager* cpmgr = LLCoprocedureManager::getInstance();
	cpmgr->enqueueCoprocedure("ExpCache", "IsAdmin",
							  std::bind(&LLExperienceCache::updateExperienceCoro,
										this, _1, upd_data, fn));
}
