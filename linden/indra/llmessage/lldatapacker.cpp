...
bool LLDataPackerBinaryBuffer::packBinaryData(const U8* valuep, S32 size,
											  const char* name)
{
	...
	if (mWriteEnabled)
	{
		htonmemcpy(mCurBufferp, valuep, MVT_VARIABLE, size);
	}
	mCurBufferp += size;
...
bool LLDataPackerBinaryBuffer::unpackBinaryData(U8* valuep, S32 value_size,
												S32& size, const char* name)
{
	if (!verifyLength(4, name))
	{
		llwarns << "Invalid data for '" << name << "', aborting !" << llendl;
		size = 0;
		return false;
	}
	...
	if (value_size < 0)
	{
		llwarns << "Negative buffer size for '" << name << "', aborting !"
				<< llendl;
		size = 0;
		return false;
	}
	...
	if (size < 0)
	{
		llwarns << "Invalid unpacked size for '" << name << "', aborting !"
				<< llendl;
		size = 0;
		return false;
	}
	...
	if (!valuep)
	{
		llwarns << "NULL value pointer for '" << name << "', aborting !"
				<< llendl;
		return false;
	}
	...
	if (value_size < size)
	{
		llwarns << "Buffer too small for '" << name << "', truncating !"
				<< llendl;
		count = value_size;
	}
	htonmemcpy(valuep, mCurBufferp, MVT_VARIABLE, count);
	mCurBufferp += size;

	return true;
}

bool LLDataPackerBinaryBuffer::packBinaryDataFixed(const U8* valuep, S32 size,
												   const char* name)
{
	if (!verifyLength(size, name))
	{
...
bool LLDataPackerBinaryBuffer::unpackBinaryDataFixed(U8* valuep, S32 size,
													 const char* name)
{
	if (!verifyLength(size, name))
	{
	...
	htonmemcpy(valuep, mCurBufferp, MVT_VARIABLE, size);
	mCurBufferp += size;

	return true;
}

bool LLDataPackerBinaryBuffer::packU8(U8 valuep, const char* name)
{
...
bool LLDataPackerBinaryBuffer::unpackU8(U8& valuep, const char* name)
{
	if (!verifyLength(sizeof(U8), name))
	{
		return false;
...
bool LLDataPackerAsciiBuffer::packBinaryData(const U8* valuep, S32 size,
											 const char* name)
{
	writeIndentedName(name);
...
bool LLDataPackerAsciiBuffer::unpackBinaryData(U8* valuep, S32 value_size,
											   S32& size, const char* name)
{
	if (value_size < 0)
	{
...
bool LLDataPackerAsciiBuffer::packBinaryDataFixed(const U8* valuep, S32 size,
												  const char* name)
{
	writeIndentedName(name);
	if (mWriteEnabled)
...
bool LLDataPackerAsciiBuffer::unpackBinaryDataFixed(U8* valuep, S32 size,
													const char* name)
{
	char valuestr[DP_BUFSIZE];
	if (getValueStr(name, valuestr, DP_BUFSIZE))
...
bool LLDataPackerAsciiFile::packBinaryData(const U8* valuep, S32 size,
										   const char* name)
{
	writeIndentedName(name);
	if (mFP)
	...
	else if (mOutputStream)
	{
		...
		*mOutputStream << buffer;
		...
		for (S32 i = 0; i < size; ++i)
		{
			snprintf(buffer, sizeof(buffer), "%02x ", valuep[i]);
			*mOutputStream << buffer;
		}
		*mOutputStream << "\n";
	}
...
bool LLDataPackerAsciiFile::unpackBinaryData(U8* valuep, S32 value_size,
											 S32& size, const char* name)
{
	if (value_size < 0)
	{
...
bool LLDataPackerAsciiFile::packBinaryDataFixed(const U8* valuep, S32 size,
												const char* name)
{
	writeIndentedName(name);
	if (mFP)
...
bool LLDataPackerAsciiFile::unpackBinaryDataFixed(U8* valuep, S32 size,
												  const char* name)
{
	char valuestr[DP_BUFSIZE];
	if (getValueStr(name, valuestr, DP_BUFSIZE))
...
bool LLDataPackerAsciiFile::packU8(U8 valuep, const char* name)
{
	writeIndentedName(name);
	if (mFP)
	{
