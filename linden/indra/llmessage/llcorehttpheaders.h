...
public:
	LL_INLINE HttpHeaders()
	:	mStandardHeaders(false)
	{
	}
...
public:
	...
	// When this returns true, HttpCoroutineAdapter::checkDefaultHeaders() sets
	// standard header entries instead of SL-specific ones (llsd+xml). HB
	LL_INLINE bool isStandardHeader() const			{ return mStandardHeaders; }
	// Flags the header as needing to be standard HTTP ones and not SL-specific
	// ones. HB
	LL_INLINE void setStandardHeader(bool b = true)	{ mStandardHeaders = b; }

protected:
	container_t	mHeaders;
	// Normally 'false', to communicate with SL sim servers, so that the
	// llsd+xml content type is set in the header and so that the UDP port
	// number is transmitted via the HTTP_X_SECONDLIFE_UDP_LISTEN_PORT header
	// field. This should be set to true when you want to communicate with a
	// "normal" HTTP server (serving web pages). HB
	bool		mStandardHeaders;
};
