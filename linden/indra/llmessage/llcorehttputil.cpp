...
:	mHttpRequest(request)
{
	...
	mBoundListener = pump.listen(LLEventPump::ANONYMOUS,
								 std::bind(&HttpRequestPumper::pollRequest,
										   this));
}
...
LLSD HttpCoroutineAdapter::getAndSuspend_(const std::string& url,
										  ...
										  HttpCoroHandler::ptr_t& handler)
{
	checkDefaultHeaders(headers, true);
											   ...
											   bool skip_content_type)
{
	...
	bool standard = headers->isStandardHeader();
	if (!headers->find(HTTP_OUT_HEADER_ACCEPT))
	{
		headers->append(HTTP_OUT_HEADER_ACCEPT,
						standard ? HTTP_CONTENT_ANY
								 : HTTP_CONTENT_LLSD_XML);
	}
	// Note: for a GET, we do not send any body, so the content type is
	// irrelevant. We however keep setting this header entry when communicating
	// with SL severs (i.e. non-"standard" servers), *just in case* LL would,
	// sometime, somehow, filter out GETs based on it... HB
	if ((!skip_content_type || !standard) &&
		 !headers->find(HTTP_OUT_HEADER_CONTENT_TYPE))
	{

		headers->append(HTTP_OUT_HEADER_CONTENT_TYPE,
						standard ? HTTP_CONTENT_WWW_FORM
								 : HTTP_CONTENT_LLSD_XML);
	}

	// Note: when we want standard header to query a non-grid HTTP server, do
	// NOT leak out our UDP listening port !  HB
	if (!standard && gMessageSystemp &&
		!headers->find("X-SecondLife-UDP-Listen-Port"))
	{
										   ...
										   completionCallback_t failure)
{
	gCoros.launch("HttpCoroutineAdapter::genericGetCoro",
				  std::bind(&HttpCoroutineAdapter::trivialGetCoro, url,
...
{
	completionCallback_t cb_succ = nullptr;
	if (!success.empty())
	{
		cb_succ = (completionCallback_t)std::bind(&logMessageSuccess,
												  "HttpCoroutineAdapter", url,
												  success);
	}

	completionCallback_t cb_fail = nullptr;
	if (!failure.empty())
	{
		cb_fail = (completionCallback_t)std::bind(&logMessageFail,
												  "HttpCoroutineAdapter", url,
												  failure);
	}
											...
											completionCallback_t failure)
{
	gCoros.launch("HttpCoroutineAdapter::genericPostCoro",
				  std::bind(&HttpCoroutineAdapter::trivialPostCoro, url,
