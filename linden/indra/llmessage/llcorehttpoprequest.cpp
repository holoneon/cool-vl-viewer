...
HttpStatus HttpOpRequest::prepareRequest(HttpService* service)
{
	...
	if (mReqOptions)
	{
		...
		last_modified = mReqOptions->getLastModified();
	}
	...
	if (last_modified)
	{
		code = curl_easy_setopt(mCurlHandle, CURLOPT_TIMECONDITION,
							   CURL_TIMECOND_IFMODSINCE);
		check_curl_easy_code(code, CURLOPT_TIMECONDITION);
#if LIBCURL_VERSION_MAJOR == 7 && LIBCURL_VERSION_MINOR < 60
		code = curl_easy_setopt(mCurlHandle, CURLOPT_TIMEVALUE,
							   (long)last_modified);
		check_curl_easy_code(code, CURLOPT_TIMEVALUE);
#else
		code = curl_easy_setopt(mCurlHandle, CURLOPT_TIMEVALUE_LARGE,
							   (curl_off_t)last_modified);
		check_curl_easy_code(code, CURLOPT_TIMEVALUE_LARGE);
#endif
  	}
...
int HttpOpRequest::seekCallback(void* userdata, curl_off_t offset, int origin)
{
	...
	{
		new_pos = static_cast<curl_off_t>(op->mReqBody->size()) + offset;
	}
	else if (origin == SEEK_CUR)
	{
		new_pos = static_cast<curl_off_t>(op->mCurlBodyPos) + offset;
	}
	else
	{
		return 2;
