...
void LLMessageSystem::init()
{
    ...
    mLostPackets = 0;            	// Total lost packets in
...
{
	mPacketRing.setMaxBufferSize(packet_ring_max_size);
...
void LLMessageSystem::loadTemplateFile(const std::string& filename)
{
	...
	std::string template_body = LLFile::getContents(filename);
	if (template_body.empty())
	{
		llerrs << "Failed to open template file: " << filename << llendl;
#if 0	// Enable if we ever change the above for llwarns
		mError = true;
		return;
#endif
...
U32 LLMessageSystem::drainUdpSocket()
{
	return mPacketRing.drainSocket(this, mSocket);
}
...
S32 LLMessageSystem::sendMessage(const LLHost& host)
{
	...
	if (mMessageBuilder == mLLSDMessageBuilder)
	{
		...
		{
			cb = std::bind(mReliablePacketParams.mCallback,
						   mReliablePacketParams.mCallbackData, _1);
		}
		gCoros.launch("LLMessageSystem::sendUntrustedSimulatorMessageCoro",
					  std::bind(&LLMessageSystem::sendUntrustedSimulatorMessageCoro,
								this, host,
											...
											void** user)
{
	...
	const LLUseCircuitCodeResponder* responderp =
		(const LLUseCircuitCodeResponder*)user;
	if (responderp)
	{
		responderp->complete(msg->getSender(), id);
	}
}
							   ...
							   const LLSD& msg_body)
{
	...
	LLSD context;
	const LLHTTPNode* handlerp = messageRootNode().traverse(path, context);
	if (!handlerp)
	{
	...
	LL_DEBUGS("Messaging") << "Received: " << msg_name << " from " << sender_ip
						   << LL_ENDL;
	LLSD message;
	message["sender"] = sender_ip;
	message["body"] = msg_body;
	handlerp->post(responsep, context, message);
}
...
{
	gMessageSystemp = new LLMessageSystem(template_name, port,
	...
	msg->setHandlerFuncFast(_PREHASH_UseCircuitCode,
							LLMessageSystem::processUseCircuitCode, NULL);
...
void LLMessageSystem::summarizeLogs(std::ostream& str)
{
	...
	buffer = llformat("SendPacket failures:       %20d",
					  mSendPacketFailureCount);
	str << buffer << std::endl;
	buffer = llformat("Packets lost on network:   %20d", mLostPackets);
	str << buffer << std::endl;
	buffer = llformat("Packets dropped by viewer: %20d", mDroppedPackets);
	str << buffer << std::endl;
...
///////////////////////////////////////////////////////////////////////////////
// Endian-aware memcopy()

// Do not let this inlined. HB
static LL_NO_INLINE void swizzle_size_error(size_t n, size_t s)
{
...
#if LL_BIG_ENDIAN
void* htonmemcpy(void* s, const void* ct, EMsgVariableType type, size_t n)
{
	switch (type)
	{
		case MVT_U16:
		...
		case MVT_LLVector3:
		 // We only send x, y, z and infer w (we set x, y, z to ensure that
		// w >= 0)
		...
		case MVT_S16Array:
			...
			for (S32 i = 1, length = n % 2; i < length; ++i)
			{
		...
		case MVT_BOOL:
		case MVT_LLUUID:
		// These two are swizzled in the getters and setters
		case MVT_IP_ADDR:
		case MVT_IP_PORT:
		default:
...
void* htonmemcpy(void* s, const void* ct, EMsgVariableType type, size_t n)
{
	switch (type)
	{
		...
		case MVT_S16:
			...
			// Let the compiler optimize memcpy() for 2 bytes. HB
			return memcpy(s, ct, 2);
		...
		case MVT_F32:
			...
			// Let the compiler optimize memcpy() for 4 bytes. HB
			return memcpy(s, ct, 4);
		...
		case MVT_F64:
			...
			// Let the compiler optimize memcpy() for 8 bytes. HB
			return memcpy(s, ct, 8);
		...
		case MVT_LLQuaternion:
			...
			// Let the compiler optimize memcpy() for 12 bytes. HB
			return memcpy(s, ct, 12);
		...
		case MVT_LLVector3d:
			...
			// Let the compiler optimize memcpy() for 24 bytes. HB
			return memcpy(s, ct, 24);
		...
		case MVT_LLVector4:
			...
			// Let the compiler optimize memcpy() for 16 bytes. HB
			return memcpy(s, ct, 16);
		...
		case MVT_U16Vec3:
			...
			// Let the compiler optimize memcpy() for 6 bytes. HB
			return memcpy(s, ct, 6);
