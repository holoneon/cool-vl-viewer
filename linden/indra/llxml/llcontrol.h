...
protected:
	typedef std::map<std::string, LLControlVariablePtr,
					 std::less<> > ctrl_name_table_t;
	ctrl_name_table_t	mNameTable;

	strings_set_t		mWarnings;
...
private:
	LL_NO_INLINE void bindToControl(LLControlGroup& group, const char* name)
	{
		...
		mConnection =
			ctrlp->getSignal()->connect(std::bind(&LLControlCache<T>::handleValueChange,
...
public:
	...
	LL_INLINE operator std::function<const T&()>() const
	{
		return std::function<const T&()>(*this);
	}
