						  ...
						  LLXMLNode* defaults_tree)
{
	...
	bool read_success = !xml.empty();
	if (read_success &&
		parseBuffer(xml.data(), xml.size(), node, defaults_tree))
	{
		return true;
	}
	// It does happen we ask for an optional file that may not exist (e.g.
	// textures.xml for a custom skin): do not warn for those. HB
	if (read_success)
	{
		llwarns << "Failed to parse XML file: " << filename << llendl;
	}
	else	// Still let it be known, as a debug message. HB
	{
		LL_DEBUGS("XMLNode") << "Could not read XML file: " << filename
							 << LL_ENDL;
	}
	node = NULL;
	return false;
...
//static
bool LLXMLNode::parseBuffer(const char* buffer, U64 length, LLXMLNodePtr& node,
							LLXMLNode* defaults)
{
	// Init
	...
	LL_DEBUGS("XMLNode") << "Parser created" << LL_ENDL;
	...
	LL_DEBUGS("XMLNode") << "Root node created" << LL_ENDL;

	// Do the parsing
	bool success = XML_Parse(my_parser, buffer, length, true) == XML_STATUS_OK;
	if (!success)
	{
	...
	node = return_node;
	LL_DEBUGS("XMLNode") << "Parse success" << LL_ENDL;
	return true;
}
...
bool LLXMLNode::parseXmlRpcArrayValue(LLSD& target)
{
	...
	if (!datap)
	{
		llwarns << "No inner XML element" << llendl;
		return false;
	}
	if (!datap->hasName("data"))
	{
		llwarns << "No inner XML element (<data> expected, got: "
				<< datap->mName->mString << ")" << llendl;
		return false;
	}
	if (datap->getNextSibling().get())
	{
		llwarns << "Multiple inner XML elements (single <data> expected)"
				<< llendl;
		return false;
	}
...
bool LLXMLNode::parseXmlRpcStructValue(LLSD& target)
{
		 ...
		 itemp = itemp->getNextSibling().get())
	{
		...
		{
			llwarns << "Invalid inner XML element (<member> expected, got: <"
					<< itemp->mName->mString << ">" << llendl;
			return false;
		}
		name.clear();
		value.clear();
		...
		if (name.empty())
		{
			llwarns << "Empty struct member name" << llendl;
			return false;
		}
...
bool LLXMLNode::fromXMLRPCValue(LLSD& target)
{
	...
	if (!hasName("value"))
	{
		llwarns << "Invalid XML element (<value> expected), got: <"
				<< mName->mString << ">" << llendl;
		return false;
	}
	...
	if (!childp)
	{
		llwarns << "No inner XML element (value type expected)" << llendl;
	...
	{
		llwarns << "Multiple inner XML elements (single expected)" << llendl;
		return false;
	}
	...
	{
		std::string decoded =
			LLBase64::decode(childp->getTextContents().c_str());
	...
	llwarns << "Unknown inner XML element (known value type expected)"
			<< llendl;
