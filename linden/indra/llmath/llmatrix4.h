...
public:
	...
	// This is for now used only for extracting the rotation from a bind shape
	// matrix that already has scales baked in. Note: this used to be a static
	// method in (the now gone) LLSkinningUtil, but it really belongs here ! HB
	LLQuaternion getUnscaledQuaternion() const;
