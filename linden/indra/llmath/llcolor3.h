...
public:
	...
	// Desaturate the color by a given factor, also optionnally darken it. HB
	LL_INLINE void desaturate(F32 factor, F32 darkening = 1.f)
	{
		if (factor > 1.f)
		{
			F32 h, s, l;
	...
	// Tries and multiples the color by 'factor', but without going beyond 1.f
	// on any of its component (i.e. 'factor' is reduced if needed to avoid
	// overshooting the maximum on any component). A negative factor would also
	// cause this method to do nothing at all. HB
	void adjust(F32 factor);
