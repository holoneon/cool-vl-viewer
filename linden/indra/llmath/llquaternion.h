...
public:
	...
	// Sets Quaternion without normalizing. HB
	const LLQuaternion&	setRaw(const F32* q);
	...
	LL_INLINE F64 operator[](int idx) const			{ return mQ[idx]; }
...
LL_INLINE const LLQuaternion& LLQuaternion::setRaw(const F32* q)
{
	mQ[VX] = q[VX];
	mQ[VY] = q[VY];
	mQ[VZ] = q[VZ];
