...
LLMotion::LLMotionInitStatus LLKeyframeMotion::onInitialize(LLCharacter* chr)
{
	...
	{
		sStaticAnimsDir = gDirUtil.getFullPath(LL_PATH_CHARACTER, "anims");
...
bool LLKeyframeMotion::onActivate()
{
	...
	const LLUUID& emote_id = mJointMotionList->mEmoteID;
	if (emote_id.notNull())
	{
		// Do not start emote if already active to avoid recursion
		if (!mCharacter->isMotionActive(emote_id))
		{
			mCharacter->startMotion(emote_id);
		}
	}
								   ...
								   bool allow_invalid_joints)
{
	...
	{
		llwarns_once << "Cannot read version number for motion " << asset_id
					 << llendl;
		return false;
	}
	U16 sub_version;
	...
	{
		llwarns_once << "Cannot read sub-version number for motion " << asset_id
					 << llendl;
		return false;
	}
	...
	{
		llwarns_once << "Bad animation version " << version << "."
					 << sub_version << " for motion " << asset_id << llendl;
		llassert(false);
		return false;
	}
	...
	{
		llwarns_once << "Cannot read animation base priority for motion "
					 << asset_id << llendl;
		return false;
	}
	...
	{
		llwarns_once << "Bad animation base priority "
					 << mJointMotionList->mBasePriority << " for motion "
					 << asset_id << llendl;
	...
	{
		llwarns_once << "Cannot read duration for motion " << asset_id
					 << llendl;
		delete mJointMotionList;
		mJointMotionList = NULL;
		return false;
	...
	{
		llwarns_once << "Invalid animation duration for motion " << asset_id
					 << llendl;
		delete mJointMotionList;
		mJointMotionList = NULL;
		return false;
	...
	{
		llwarns_once << "Cannot read optional emote animation name for motion "
					 << asset_id << llendl;
		delete mJointMotionList;
		mJointMotionList = NULL;
		return false;
	...
	if (!mJointMotionList->mEmoteName.empty())
	{
		if (mJointMotionList->mEmoteName == mID.asString())
		{
			llwarns_once << "Malformed animation mEmoteName==mID for motion "
						 << asset_id << llendl;
			delete mJointMotionList;
			mJointMotionList = NULL;
			return false;
		...
		if (mJointMotionList->mEmoteName == "Closed_Mouth")
		{
			mJointMotionList->mEmoteName.clear();
		}
		else
		{
			mJointMotionList->mEmoteID =
				gAnimLibrary.stringToAnimState(mJointMotionList->mEmoteName);
			if (mJointMotionList->mEmoteID.isNull())
			{
				llwarns_once << "Unknown emote name for motion " << asset_id
							 << llendl;
				mJointMotionList->mEmoteName.clear();
			}
		}
	...
	{
		llwarns_once << "Cannot read loop point for motion " << asset_id
					 << llendl;
		delete mJointMotionList;
		mJointMotionList = NULL;
		return false;
	...
	{
		llwarns << "Cannot read loop flag for motion " << asset_id << llendl;
		delete mJointMotionList;
		mJointMotionList = NULL;
		return false;
	}
	...
	{
		llwarns_once << "Cannot read ease-in duration for motion " << asset_id
					 << llendl;
		delete mJointMotionList;
		mJointMotionList = NULL;
		return false;
	...
	{
		llwarns_once << "Cannot read ease-out duration for motion " << asset_id
					 << llendl;
		delete mJointMotionList;
		mJointMotionList = NULL;
		return false;
	...
	{
		llwarns_once << "Cannot read hand pose for motion " << asset_id
					 << llendl;
		delete mJointMotionList;
		mJointMotionList = NULL;
		return false;
	...
	{
		llwarns_once << "Invalid LLHandMotion::eHandPose index: " << word
					 << " for motion " << asset_id << llendl;
		delete mJointMotionList;
		mJointMotionList = NULL;
		return false;
	}
	...
	U32 num_motions = 0;
	U32 rotation_duplicates = 0;
	U32 position_duplicates = 0;
	if (!dp.unpackU32(num_motions, "num_joints"))
	{
		llwarns_once << "Cannot read number of joints for motion " << asset_id
					 << llendl;
		delete mJointMotionList;
		mJointMotionList = NULL;
		return false;
	...
	if (num_motions == 0)
	{
		llwarns_once << "No joint in animation for motion " << asset_id
					 << llendl;
		delete mJointMotionList;
		mJointMotionList = NULL;
		return false;
	...
	{
		llwarns_once << "Too many joints in animation for motion " << asset_id
					 << llendl;
		delete mJointMotionList;
		mJointMotionList = NULL;
		return false;
	...
	for (U32 i = 0; i < num_motions; ++i)
	{
		...
		{
			llwarns_once << "Cannot read joint name for motion " << asset_id
						 << llendl;
			delete mJointMotionList;
			mJointMotionList = NULL;
			return false;
		...
		{
			llwarns_once << "Attempted to animate special '" << joint_name
						 << "' joint for motion " << asset_id << llendl;
			delete mJointMotionList;
			mJointMotionList = NULL;
			return false;
		}
		...
		{
			llwarns_once << "Joint not found: " << joint_name << "Motion: "
						 << asset_id << llendl;
		...
		{
			llwarns_once << "Cannot read joint priority for motion " << asset_id
						 << llendl;
			delete mJointMotionList;
			mJointMotionList = NULL;
			return false;
		...
		{
			llwarns_once << "Joint priority unknown - too low for motion "
						 << asset_id << llendl;
			delete mJointMotionList;
			mJointMotionList = NULL;
			return false;
		...
		{
			llwarns_once << "Cannot read number of rotation keys for motion "
						 << asset_id << llendl;
			delete mJointMotionList;
			mJointMotionList = NULL;
			return false;
		...
		for (S32 k = 0; k < joint_motion->mRotationCurve.mNumKeys; ++k)
		{
			...
			if (old_version)
			{
				...
				{
					llwarns_once << "Cannot read rotation key (" << k
								 << ") for motion " << asset_id << llendl;
					delete mJointMotionList;
					mJointMotionList = NULL;
					return false;
			...
			else
			{
				...
				{
					llwarns_once << "Invalid frame time for motion " << asset_id
								 << llendl;
					delete mJointMotionList;
					mJointMotionList = NULL;
					return false;
			...
			{
				llwarns_once << "Non-finite angle in rotation key for motion "
							 << asset_id << llendl;
				success = false;
			}
		...
		if (joint_motion->mRotationCurve.mNumKeys >
				(S32)joint_motion->mRotationCurve.mKeys.size())
		{
			++rotation_duplicates;
		}
		...
		{
			llwarns_once << "Cannot read number of position keys for motion "
						 << asset_id << llendl;
			delete mJointMotionList;
			mJointMotionList = NULL;
			return false;
		...
		for (S32 k = 0; k < joint_motion->mPositionCurve.mNumKeys; ++k)
		{
			...
			if (old_version)
			{
				...
				{
					llwarns_once << "Cannot read position key (" << k
								 << ") for motion " << asset_id << llendl;
					delete mJointMotionList;
					mJointMotionList = NULL;
					return false;
			...
			{
				llwarns_once << "Non-finite position in key for motion "
							 << asset_id << llendl;
				success = false;
			}
		...
		if (joint_motion->mPositionCurve.mNumKeys >
				(S32)joint_motion->mPositionCurve.mKeys.size())
		{
			++position_duplicates;
		}
	...
	if (rotation_duplicates)
	{
		llinfos_once << "Motion " << asset_id
					 << " had duplicated rotation keys that were removed"
					 << llendl;
	}
	if (position_duplicates)
	{
		llinfos_once << "Motion " << asset_id
					 << " had duplicated position keys that were removed"
					 << llendl;
	}
	...
	{
		llwarns_once << "Cannot read the number of constraints for motion "
					 << asset_id << llendl;
		delete mJointMotionList;
		mJointMotionList = NULL;
		return false;
	...
	{
		llwarns_once << "Bad number of constraints: " << num_constraints
					 << " - Motion " << asset_id << llendl;
		delete mJointMotionList;
		mJointMotionList = NULL;
		return false;
	}
	...
	for (S32 i = 0; i < num_constraints; ++i)
	{
		...
		{
			llwarns_once << "Cannot read constraint chain length for motion "
						 << asset_id << llendl;
			delete constraintp;
			delete mJointMotionList;
			mJointMotionList = NULL;
		...
		{
			llwarns_once << "Invalid constraint chain length for motion "
						 << asset_id << llendl;
			delete constraintp;
			delete mJointMotionList;
			mJointMotionList = NULL;
		...
		{
			llwarns_once << "Cannot read constraint type for motion "
						 << asset_id << llendl;
			delete constraintp;
			delete mJointMotionList;
			mJointMotionList = NULL;
		...
		{
			llwarns_once << "Invalid constraint type for motion " << asset_id
						 << llendl;
			delete constraintp;
			delete mJointMotionList;
			mJointMotionList = NULL;
		...
		if (!dp.unpackBinaryDataFixed(bin_data, BIN_DATA_LENGTH,
										  "source_volume"))
		{
			llwarns_once << "Cannot read source volume name for motion "
						 << asset_id << llendl;
			delete constraintp;
			delete mJointMotionList;
			mJointMotionList = NULL;
		...
		{
			llwarns_once << "Not a valid source constraint volume: " << str
						 << " - Motion " << asset_id << llendl;
			delete constraintp;
			delete mJointMotionList;
			mJointMotionList = NULL;
		...
		if (!dp.unpackVector3(constraintp->mSourceConstraintOffset,
							  "source_offset"))
		{
			llwarns_once << "Cannot read constraint source offset for motion "
						 << asset_id << llendl;
			delete constraintp;
			delete mJointMotionList;
			mJointMotionList = NULL;
		...
		{
			llwarns_once << "Non-finite constraint source offset for motion "
						 << asset_id << llendl;
			delete constraintp;
			delete mJointMotionList;
			mJointMotionList = NULL;
		...
		if (!dp.unpackBinaryDataFixed(bin_data, BIN_DATA_LENGTH,
									  "target_volume"))
		{
			llwarns_once << "Cannot read target volume name for motion "
						 << asset_id << llendl;
			delete constraintp;
			delete mJointMotionList;
			mJointMotionList = NULL;
		...
		else
		{
			...
			if (constraintp->mSourceConstraintVolId == -1)
			{
				llwarns_once << "Not a valid target constraint volume: " << str
							 << " - Motion " << asset_id << llendl;
				delete constraintp;
				delete mJointMotionList;
				mJointMotionList = NULL;
		...
		if (!dp.unpackVector3(constraintp->mTargetConstraintOffset,
							  "target_offset"))
		{
			llwarns_once << "Cannot read constraint target offset for motion "
						 << asset_id << llendl;
			delete constraintp;
			delete mJointMotionList;
			mJointMotionList = NULL;
		...
		{
			llwarns_once << "Non-finite constraint target offset for motion "
						 << asset_id << llendl;
			delete constraintp;
			delete mJointMotionList;
			mJointMotionList = NULL;
		...
		{
			llwarns_once << "Cannot read constraint target direction for motion "
						 << asset_id << llendl;
			delete constraintp;
			delete mJointMotionList;
			mJointMotionList = NULL;
		...
		{
			llwarns_once << "Non-finite constraint target direction for motion "
						 << asset_id << llendl;
			delete constraintp;
			delete mJointMotionList;
			mJointMotionList = NULL;
		...
		{
			llwarns_once << "Cannot read constraint ease in start time for motion "
						 << asset_id << llendl;
			delete constraintp;
			delete mJointMotionList;
			mJointMotionList = NULL;
		...
		{
			llwarns_once << "Cannot read constraint ease in stop time for motion "
						 << asset_id << llendl;
			delete constraintp;
			delete mJointMotionList;
			mJointMotionList = NULL;
		...
		{
			llwarns_once << "Cannot read constraint ease out start time for motion "
						 << asset_id << llendl;
			delete constraintp;
			delete mJointMotionList;
			mJointMotionList = NULL;
		...
		{
			llwarns_once << "Cannot read constraint ease out stop time for motion "
						 << asset_id << llendl;
			delete constraintp;
			delete mJointMotionList;
			mJointMotionList = NULL;
		...
		for (S32 i = 0, count = constraintp->mChainLength + 1; i < count; ++i)
		{
			...
			if (!parent)
			{
				llwarns_once << "Joint with no parent: " << joint->getName()
				 ...
				 j < count2; ++j)
			{
				...
				if (!constraint_joint)
				{
					llwarns_once << "Invalid joint " << j << " in motion "
								 << asset_id << llendl;
					delete constraintp;
					delete mJointMotionList;
					mJointMotionList = NULL;
			...
			{
				llwarns_once << "No joint index for constraint " << i
							 << " for motion " << asset_id << llendl;
				delete constraintp;
				delete mJointMotionList;
				mJointMotionList = NULL;
...
bool LLKeyframeMotion::dumpToFile(const std::string& name)
{
	...
	std::string extension = LLDir::getExtension(filename);
	if (extension != "anim" && extension != "tmp")
	{
		filename += ".anim";
	}
	if (LLDir::getDirName(filename).empty())
	{
		filename = gDirUtil.getFullPath(LL_PATH_CACHE, filename);
	}
