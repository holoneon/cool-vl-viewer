...
std::string LLDate::toHTTPDateString(const char* fmt) const
{
	llassert_always(fmt && *fmt);
	time_t loc_seconds = (time_t)mSecondsSinceEpoch;
	return toHTTPDateString(gmtime(&loc_seconds), fmt);
}
...
bool LLDate::fromString(const std::string& iso8601_date)
{
	boost::iostreams::stream<boost::iostreams::array_source>
		stream(iso8601_date.data(), iso8601_date.size());
