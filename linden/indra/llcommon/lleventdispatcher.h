...
public:
	...
	// specifying the std::bind() expression. The passed method
...
private:
	...
	typedef std::map<std::string, std::shared_ptr<DispatchEntry>,
					 std::less<>  > DispatchMap;
...
private:
				   ...
				   const METHOD& method, const LLSD& required)
	{
		...
		{
			add(name, desc, std::bind(method, downcast, PH1), required);
		}
	}
...
LLEventDispatcher::make_invoker(Function f)
{
	...
	// we are std::bind()ing it rather than calling it directly.
	return std::bind(&invoker<Function>::template apply<boost::fusion::nil>, f,
					 PH1, boost::fusion::nil());
}
...
LLEventDispatcher::make_invoker(Method f, const InstanceGetter& getter)
{
	...
	return std::bind(&invoker<Method>::template method_apply<InstanceGetter>,
					 f, PH1, getter);
}
