...
// Methods that forward listeners (e.g. constructed with std::bind()) should
...
public:
	...
	// object constructed by std::bind() is not intended to be written out.
...
private:
	...
	// lifespan.
	typedef std::map<std::string, LLEventPump*, std::less<> > pump_map_t;
	pump_map_t	mPumpMap;
	...
	// destroyed.
	typedef std::set<LLEventPump*> pump_set_t;
	pump_set_t	mOurPumps;
...
public:
	...
	// name list
	const static strings_vec_t empty;
	...
	LLBoundListener listen(const std::string& name,
						   ...
						   const strings_vec_t& after = strings_vec_t(),
						   const strings_vec_t& before = strings_vec_t())
	{
...
protected:
	...
	// this map.
	typedef std::map<std::string, boost::signals2::connection,
					 std::less<> > connect_map_t;
