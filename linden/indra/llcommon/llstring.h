...
// Used for various maps in the viewer classes. Replaces joint_alias_map_t,
// JointMap, etc and avoids having to include large headers with such per-class
// typedefs to use the latter in another header. Note that we use the new C++14
// std::less<void> template (instead of the default std:less<std::string>) as
// the comparator, so to avoid useless string copies when searching for a key
// in this map (e.g. when searching from a C-string). HB
typedef std::map<std::string, std::string, std::less<> > strings_map_t;
// Same thing with sets of strings... HB
typedef std::set<std::string, std::less<> > strings_set_t;
// Used in several places as well. HB
typedef std::vector<std::string> strings_vec_t;
...
public:
	...
	static const std::string& getDatetimeCode(const std::string& key);
...
private:
	static strings_map_t			sDatetimeToCodes;
...
public:
	...
	// Case-insensitive version of above. Uses current locale on Windows,
	// and falls back to a non-locale aware comparison on Linux.
...
private:
	static size_type getSubstitution(const string_type& instr,
									 size_type& start,
									 std::vector<string_type >& tokens);
	static size_t ci_find(string_type& str1, string_type& str2, size_t idx);
};
...
// We should never use UTF16 except when communicating with Windows !
typedef std::basic_string<llchar16> llutf16string;
...
// No such conversions needed under Linux, or Windows with /Zc:wchar_t- MSVC
// compilation option.
# define LL_CONVERT_ALIAS(TO, FROM, EXPR)
...
template<typename T>
size_t LLStringUtilBase<T>::ci_find(std::basic_string<T>& str1,
									std::basic_string<T>& str2, size_t idx)
{
	auto it = std::search(str1.begin() + idx, str1.end(),
						  str2.begin(), str2.end(),
...
//static
template<class T>
void LLStringUtilBase<T>::replaceString(std::basic_string<T>& str,
										std::basic_string<T> target,
										std::basic_string<T> replacement,
										bool case_sensitive)
{
	size_type ssz = str.size();
	size_type tsz = target.size();
	if (ssz == 0 || ssz < tsz)
	{
		return;
	}
	...
	if (case_sensitive)
	{
		while ((i = str.find(target, i)) != std::basic_string<T>::npos)
		{
	...
	// Case-insensitve replacement of target. HB
	while ((i = ci_find(str, target, i)) != std::basic_string<T>::npos)
	{
		str.replace(i, target.length(), replacement);
		// Avoid infinite defeat if replacement contains target
		i += replacement.length();
