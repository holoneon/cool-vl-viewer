...
// LLProcess object's Status would not update until the next "mainloop" tick.
...
private:
	...
	typedef std::vector<std::unique_ptr<BasePipe> > pipe_vec_t;
	pipe_vec_t					mPipes;
	...
	void addPoll(const LLProcess&)
	{
		...
		if (mCount++ == 0)
		{
			...
			mConnection = pump.listen("LLProcessListener",
									  std::bind(&LLProcessListener::tick,
												this, _1));
		}
	}
...
public:
		...
		mStream(&mStreambuf)
	{
		...
		mConnection = pump.listen(LLEventPump::inventName("WritePipe"),
								  std::bind(&WritePipeImpl::tick, this, _1));
#if !LL_WINDOWS
...
public:
		...
		mEOF(false)
	{
		...
		mConnection = pump.listen(LLEventPump::inventName("ReadPipe"),
								  std::bind(&ReadPipeImpl::tick, this, _1));
	}
...
:	mPool(NULL)
{
	...
	for (size_t i = 0; i < NSLOTS; ++i)
	{
		...
		{
			mPipes[i] =
				std::make_unique<ReadPipeImpl>(desc, pipe, FILESLOT(i));
		}
	}
}
...
public:
	...
	typedef std::function<LLBoundListener
							(LLEventPump&,
							 const std::string& listener)> connect_func_t;
		...
		if (request["dest"].isDefined())
		{
			...
			saveListener(source_name, listener_name,
						 source.listen(listener_name,
									   std::bind(&LLEventPump::post, &dest,
												 _1)));
		}
		else
		{
...
static strings_set_t sKnownInterpreters =
{
#if LL_WINDOWS
...
public:
	...
	:	mDonePump("LLLeap", true),
		...
		mBinaryInput(false),
		mBinaryOutput(false),
		mListener(new LLLeapListener(std::bind(&LLLeapImpl::connect, this, _1,
											   _2)))
	{
		...
		mDonePump.listen("LLLeap",
						 std::bind(&LLLeapImpl::badLaunch, this, _1));
		...
		mDonePump.listen("LLLeap", std::bind(&LLLeapImpl::done, this, _1));
		...
		mStdoutConnection =
			childout.getPump().listen("prefix",
									  std::bind(&LLLeapImpl::rstdout, this,
												_1));
		mStdoutDataConnection =
			childout.getPump().listen("data",
									  std::bind(&LLLeapImpl::rstdoutData,
												this));
		...
		mStderrConnection =
			childerr.getPump().listen("LLLeap",
									  std::bind(&LLLeapImpl::rstderr, this,
												_1));
...
private:
	...
	LLBoundListener connect(LLEventPump& pump, const std::string& listener)
	{
		...
		return pump.listen(listener,
						   std::bind(&LLLeapImpl::wstdin, this,
									 pump.getName(), _1));
	}
