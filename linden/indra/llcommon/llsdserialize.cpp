...
// Utility function
F64 string_to_real(const std::string& in_string)
{
	boost::iostreams::stream<boost::iostreams::array_source>
		i_stream(in_string.data(), in_string.size());
	F64 value;
	i_stream >> value;
	// We would probably like to ignore all trailing whitespace as well but,
	// for now, simply eat the next character, and make sure we reached the end
	// of the string.
	return i_stream.get() == EOF ? value : 0.0;
}
...
// unique_ptr is destroyed at thread exit, it also destroys the storage. HB
...
private:
	...
	typedef std::deque<LLSD*> stack_queue_t;
	stack_queue_t	mStack;
...
S32 LLSDXMLParser::Impl::parse(std::istream& input, LLSD& data)
{
	...
	if (status == XML_STATUS_ERROR && !mGracefullStop)
	{
		if (buffer)
		{
			...
			if (mEmitErrors)
			{
				llwarns << "XML_STATUS_ERROR parsing: " << (char*)buffer
						<< llendl;
			}
		}
		else if (mEmitErrors)
		{
			llwarns << "XML_STATUS_ERROR, NULL buffer" << llendl;
		}
		data = LLSD();
		return LLSDParser::PARSE_FAILURE;
	}
...
void LLSDXMLParser::Impl::endElementHandler(const XML_Char* name)
{
	...
	switch (element)
	{
		...
		case ELEMENT_BOOL:
			value.assign(mCurrentContent == "true" || mCurrentContent == "1");
			break;
		...
		case ELEMENT_INTEGER:
		{
			...
			{
				value.assign((LLSD::Integer)string_to_real(mCurrentContent));
			}
			break;
		}
		...
		case ELEMENT_REAL:
		{
			value.assign(string_to_real(mCurrentContent));
			break;
		}

		case ELEMENT_STRING:
			value.assign(mCurrentContent);
			break;

		case ELEMENT_UUID:
			value.assign(LLUUID(mCurrentContent));
			break;

		case ELEMENT_DATE:
			value.assign(LLDate(mCurrentContent));
			break;

		case ELEMENT_URI:
			value.assign(LLURI(mCurrentContent));
			break;
