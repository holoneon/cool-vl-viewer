...
#include "llsdserialize.h"		// For string_to_real()
...
public:
	...
	static void assignUndefined(Impl*& var);
	static void assign(Impl*& var, const LLSD::Impl* other);
	...
	virtual void assign(Impl*& var, const char*) const;
	...
	LL_INLINE virtual String asXMLRPCValue() const			{ return "<nil/>"; }
...
private:
	...
	static LLSD::map_t	sEmptyMap;
};

//static
LLSD::map_t LLSD::Impl::sEmptyMap;
...
#endif
{
	...
	public:
		...
		// *NOTE: The reason that false is not converted to "false" is because
		// that would break roundtripping, such as, for example;
		//  LLSD(false).asString().asBoolean()
		// There are many reasons for wanting LLSD("false").asBoolean() to be
		// true such as "everything else seems to work that way".
		LL_INLINE LLSD::String asString() const override
		{
			return mValue ? "true" : "";
		}

		LL_INLINE LLSD::String asXMLRPCValue() const override
		{
			return mValue ? "<boolean>1</boolean>" : "<boolean>0</boolean>";
		}
	};
	...
	public:
		...
		LL_INLINE LLSD::String asXMLRPCValue() const override
		{
			return llformat("<int>%d</int>", mValue);
		}
	};
	...
	public:
		...
		LL_INLINE LLSD::String asXMLRPCValue() const override
		{
			return llformat("<double>%lg</double>", mValue);
		}
	};
	...
	public:
		...
		using LLSD::Impl::assign; // Unhiding base class virtuals...
		LL_INLINE void assign(LLSD::Impl*& var, const char* value)
		{
			if (shared())
			{
				Impl::assign(var, value);
		...
		LL_INLINE LLSD::Integer asInteger() const override
		{
			return (LLSD::Integer)string_to_real(mValue);
		}

		LL_INLINE LLSD::Real asReal() const override
		{
			return string_to_real(mValue);
		}
		...
		LL_INLINE LLSD::String asXMLRPCValue() const override
		{
			return "<string>" + LLStringFn::xml_encode(mValue) + "</string>";
		}
	};
	...
	public:
		...
		LL_INLINE LLSD::String asXMLRPCValue() const override
		{
			return "<string>" + mValue.asString() + "</string>";
		}
	};
	...
	public:
		...
		LL_INLINE LLSD::String asXMLRPCValue() const override
		{
			return "<dateTime.iso8601>" + mValue.toHTTPDateString("%FT%T") +
				   "</dateTime.iso8601>";
		}
	};
	...
	public:
		...
		LL_INLINE LLSD::String asXMLRPCValue() const override
		{
			return "<string>" + LLStringFn::xml_encode(mValue.asString()) +
				   "</string>";
		}
	};
	...
	public:
		...
		{
			return "<base64>" +
				   LLBase64::encode((const char*)mValue.data(),
									mValue.size()) + "</base64>";
		}
	};
	...
	private:
	protected:
		LL_INLINE ImplMap(const LLSD::map_t& data)
		:	mData(data)
		{
		}
	...
	public:
		...
		LLSD::String asXMLRPCValue() const override
		{
			std::ostringstream os;
			os << "<struct>";
			for (const auto& it : mData)
			{
				os << "<member><name>" << LLStringFn::xml_encode(it.first)
				   << "</name>" << it.second.asXMLRPCValue() << "</member>";
			}
			os << "</struct>";
			return os.str();
		}
		...
		using LLSD::Impl::get;		// Unhiding LLSD::get()
		using LLSD::Impl::erase;	// Unhiding LLSD::erase()
		using LLSD::Impl::ref;		// Unhiding LLSD::ref()
	...
	{
		LLSD::map_t::const_iterator i = mData.find(k);
		return i != mData.end();
	}
	...
	LLSD ImplMap::getKeys() const
	{
		...
		for (LLSD::map_t::const_iterator it = mData.begin(), end = mData.end();
			 it != end; ++it)
		{
	...
	{
		mData.insert(LLSD::map_t::value_type(k, v));
	}
	...
	{
		LLSD::map_t::const_iterator it = mData.find(k);
		return it != mData.end() ?  it->second : undef();
	}
...
//virtual
void LLSD::Impl::assign(Impl*& var, const char* v) const
{
