...
public:
	...
	// Param type used to express lists of other node keys.
...
public:
	...
	LL_INLINE node_range get_node_range()
	{
		...
		return generic_range<node_iterator>(
			std::bind<NODE&>(&DepNode::node,
							 std::bind<DepNode&>(&DepNodeMapEntry::second,
												 PH1)));
	}
	...
	const_node_range get_node_range() const
	{
		...
		return generic_range<const_node_iterator>(
			std::bind<const NODE&>(&DepNode::node,
								   std::bind<const DepNode&>(&DepNodeMapEntry::second,
															 PH1)));
	}
	...
	LL_INLINE const_key_range get_key_range() const
	{
		...
		return generic_range<const_key_iterator>(std::bind<const KEY&>(&DepNodeMapEntry::first,
																	   PH1));
	}
...
LLDependencies<KEY, NODE>::get_after_range(const KEY_OR_ITER& key_iter) const
{
	return LLDependencies_dep_range_from<KEY_OR_ITER>()(*this, key_iter,
			std::bind<const typename DepNode::dep_set&>(&DepNode::after, PH1));
}
...
{
	return LLDependencies_dep_range_from<KEY_OR_ITER>()(*this, key_iter,
		std::bind<const typename DepNode::dep_set&>(&DepNode::before, PH1));
}
