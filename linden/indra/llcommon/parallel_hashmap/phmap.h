...
// Note: since PHMAP_DISABLE_MIX has been added to phmap v1.4.1, let's use this
// define instead of my PHMAP_NO_MIXING).
...
// --------------------------------------------------------------------------
template <typename T>
LL_INLINE uint32_t TrailingZeros(T x) {
    uint32_t res;
...
// --------------------------------------------------------------------------
template <typename T>
LL_INLINE uint32_t LeadingZeros(T x) {
    uint32_t res;
    PHMAP_IF_CONSTEXPR(sizeof(T) == 8)
...
public:
    ...
    LL_INLINE const Inner& get_inner(size_t idx) const {
        return  sets_[idx];
    }
