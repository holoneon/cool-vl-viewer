...
// Note: I had to remove "-" from UNRESERVED_CHARS since some CEF versions did
// not like it in path components... But then, it is now (2024-03) apparently
// the other way around, and I re-added it... Go figure !!!  HB
#if 1
# define UNRESERVED_DASH "-"
#else
# define UNRESERVED_DASH ""
#endif
...
LL_NO_INLINE static bool is_char_hex(char hex)
{
	if (hex >= '0' && hex <= '9')
	{
		return true;
...
LL_NO_INLINE static std::string escapePathComponent(const std::string& s)
{
	...
	if (allowed.empty())
	{
		allowed = UNRESERVED_CHARS SUB_DELIMS UNRESERVED_DASH ":@";
		std::sort(allowed.begin(), allowed.end());
	}
	return LLURI::escape(s, allowed, true);
}
