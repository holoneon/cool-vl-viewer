...
{
	pump_map_t::iterator it = mPumpMap.find(name);
	if (it != mPumpMap.end())
	{
...
void LLEventPumps::flush()
{
	...
	for (pump_map_t::iterator it = mPumpMap.begin(), end = mPumpMap.end();
		 it != end; ++it)
	{
		it->second->flush();
	}
...
{
	std::pair<pump_map_t::iterator, bool> inserted =
	...
	pump_map_t::iterator pmi(inserted.first), pmend(mPumpMap.end());
	...
	while (++pmi != pmend)
	{
		...
		// Okay, the first character of the suffix is a digit, it is worth at
...
void LLEventPumps::unregister(const LLEventPump& pump)
{
	...
	pump_map_t::iterator it = mPumpMap.find(pump.getName());
	if (it != mPumpMap.end())
	{
		mPumpMap.erase(it);
	}
	...
	pump_set_t::iterator it2 = mOurPumps.find(const_cast<LLEventPump*>(&pump));
...
// Static data member
const strings_vec_t LLEventPump::empty;
...
LLBoundListener LLEventPump::listen_impl(const std::string& name,
										 ...
										 const strings_vec_t& after,
										 const strings_vec_t& before)
{
	if (!mSignal)
	{
	...
	if (named)
	{
		...
		// In some cases the user might disconnect a connection explicitly and
		// we can end up retaining in mConnections a zombie connection object
		// that has already been disconnected. Such a connection object cannot
		// be reconnected since disconnection happens with the destruction of
		// the listener object. That means it is safe to overwrite a
		// disconnected connection object with the new one we are attempting.
		// The case we want to prevent is only when the existing connection
		// object is still connected.
			 ...
			 dmi != sorted_range.end(); ++dmi)
		{
			...
			if (dmi->second < previous)
			{
				...
				typedef std::vector<std::pair<float,
											  std::string> > sortnames_vec_t;
				sortnames_vec_t sortnames;
					 ...
					 cdmi != cdmend; ++cdmi)
				{
					if (cdmi->first != name)
					{
						sortnames.push_back(sortnames_vec_t::value_type(cdmi->second,
																		cdmi->first));
					}
				}
				...
				out << "New listener '" << name << "' on "
					...
					<< "'\nwas: ";
				sortnames_vec_t::const_iterator sni = sortnames.begin();
				sortnames_vec_t::const_iterator snend = sortnames.end();
				if (sni != snend)
				{
					out << sni->second;
					while (++sni != snend)
...
LLListenerOrPumpName::LLListenerOrPumpName(const std::string& pumpname)
	...
	// listener
:	mListener(std::bind(&LLEventPump::post,
						std::ref(gEventPumps.obtain(pumpname)), _1))
{
}
