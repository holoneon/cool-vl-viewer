...
namespace
{
	...
	const strings_vec_t					gEmptyValue;
...
public:
	...
	typedef std::function<void(const strings_vec_t&)> notify_callback_t;
	...
	{
		const strings_vec_t* value =
			boost::any_cast<const strings_vec_t>(&value_store);
		if (mNotifyCallback)
		{
...
protected:
				...
				const std::vector<std::string>& new_tokens) const
	{
		...
		{
			value_store = boost::any(strings_vec_t());
		}
		strings_vec_t* tv = boost::any_cast<strings_vec_t>(&value_store);
...
void LLCommandLineParser::addOptionDesc(const std::string& option_name,
										std::function<void(const strings_vec_t&)> notify_callback,
	...
	catch (LLCLPLastOption&)
	{
			 ...
			 i != gVariableMap.end(); )
		{
			...
			else
			{
				...
				strings_vec_t* tv =
					boost::any_cast<strings_vec_t>(&(tempI->second.value()));
				if (tv && !tv->empty())
				{
...
void LLCommandLineParser::printOptions() const
{
		 ...
		 i != gVariableMap.end(); ++i)
	{
		...
		strings_vec_t values = i->second.as<strings_vec_t>();
		std::ostringstream oss;
		oss << name << ": ";
...
const strings_vec_t& LLCommandLineParser::getOption(const std::string& name) const
{
	if (hasOption(name))
	{
		return gVariableMap[name].as<strings_vec_t>();
	}
	return gEmptyValue;
}
