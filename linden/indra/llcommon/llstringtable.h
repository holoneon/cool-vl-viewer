...
private:
	...
	{
		str_set_t& stringset = mStringList[hashval];
		LLStdStringHandle handle = &s;
		// Compares actual strings:
		str_set_t::iterator iter = stringset.find(handle);
...
private:
	typedef std::set<LLStdStringHandle,
					 compare_pointer_contents<std::string> > str_set_t;
	str_set_t*		mStringList; // [mTableSize]
