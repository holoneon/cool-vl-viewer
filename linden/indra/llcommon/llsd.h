...
public:
	...
	// Returns:
	// "<value><((type))>((scalar value or recursive calls))</((type))></value>"
	...
	LLSD get(const String& k) const;

	bool has(const char* k) const;
	LLSD get(const char* k) const;

	// Returns an LLSD array with keys as strings
	LLSD getKeys() const;

	void insert(const String& k, const LLSD& v);
	void erase(const String& k);

	LLSD& with(const char* k, const LLSD& v);

	LL_INLINE LLSD& with(const String& k, const LLSD& v)
	{
		return with(k.c_str(), v);
	}

	LLSD& operator[](const String& k);
	LL_INLINE LLSD& operator[](const char* k)	{ return (*this)[String(k)]; }

	const LLSD& operator[](const String& k) const;

	LL_INLINE const LLSD& operator[](const char* k) const
	{
	...
	LLSD get(size_t i) const;
	void set(size_t i, const LLSD& v);

	void insert(size_t i, const LLSD& v);
	void erase(size_t i);

	LLSD& with(size_t i, const LLSD& v);

	LLSD& append(const LLSD& v);

	// Accept size_t so we can index relative to size()
	const LLSD& operator[](size_t i) const;
	LLSD& operator[](size_t i);
	...
	// We use C++14 std::less<void> to avoid string copies when searching for
	// a key in this map (e.g. when searching from a C-string).
