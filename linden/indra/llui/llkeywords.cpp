...
constexpr U32 KEYWORD_FILE_CURRENT_VERSION = 3;

LL_INLINE bool LLKeywordToken::isHead(const llwchar* s, bool lua_syntax,
									  bool search_multi_line_comment) const
{
	const llwchar* t = mToken.c_str();
	size_t len = mToken.size();

	if (search_multi_line_comment)
	{
		if (lua_syntax)
		{
			if (len == 4 && t[0] == '-' && t[1] == '-' && t[2] == '[' &&
				t[3] == '[')
			{
				// Special case for Lua-like ]] end comment token
				return s[0] == ']' && s[1] == ']';
			}
		}
		else if (len == 2 && t[0] == '/' && t[1] == '*')
		{
	...
	return std::memcmp((const void*)s, (const void*)t,
					   len * sizeof(llwchar)) == 0;
}
...
{
	LL_DEBUGS("Keywords") << "Adding token: " << key_in << LL_ENDL;
							  ...
							  const LLColor4 &default_color)
{
	...
	// Reserve capacity for segments based on an estimated average of 8
	// characters per segment.
	constexpr S32 AVERAGE_SEGMENT_LENGTH = 8;
	seg_list->reserve(text_len / AVERAGE_SEGMENT_LENGTH);

	seg_list->push_back(new LLTextSegment(LLColor3(default_color), 0,
										  text_len));

	bool has_tokens = !mLineTokenList.empty();
		...
		while (*cur && *cur != '\n')
		{
			// Check against delimiters
			{
				...
				if (cur_delimiter)
				{
					...
					{
						while (*cur &&
							   !cur_delimiter->isHead(cur, mLuaSyntax, true))
						{
							...
							if (*cur == '\\')
							{
								...
								if (cur_delimiter->isHead(cur, mLuaSyntax,
														  true))
								{
						...
						if (mLuaSyntax)
						{
							end_delim_len = cur_delimiter->getLength();
						}
						else
						{
							// *HACK: for Lua --[[ ... ]] comment. HB
							end_delim_len = llmin(2,
...
#endif
				while (isalnum(*p) || *p == '_' || (mLuaSyntax && *p == '.') || (mLuaSyntax && *p == ':'))
				{
					++p;
				}
				S32 seg_len = p - cur;
