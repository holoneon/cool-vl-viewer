...
public:
	...
	// For Lua scripted automated responses. HB
	bool simulateClickButton(S32 i);
	...
	typedef void (*lua_alert_cb_t)(const std::string& name,
								   const LLUUID& dialog_id,
								   const std::string& message,
								   const std::vector<std::string>& buttons);
	LL_INLINE static void setLuaCallback(lua_alert_cb_t fn)
	{
		sLuaCallback = fn;
	}
...
private:
	...
	typedef std::map<std::string, LLAlertDialog*, std::less<> > active_map_t;
	static active_map_t		sUniqueActiveMap;
	...
	static lua_alert_cb_t	sLuaCallback;
