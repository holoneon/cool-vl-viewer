...
public:
	...
	bool initPanelXML(LLXMLNodePtr nodep, LLView* parentp,
					  LLUICtrlFactory* factoryp);
	void initChildrenXML(LLXMLNodePtr nodep, LLUICtrlFactory* factoryp);
	void setPanelParameters(LLXMLNodePtr nodep, LLView* parentp);
	...
	// LLSliderCtrl / LLMultiSlider / LLSpinCtrl
...
private:
	...
	mutable strings_set_t	mExpectedMembers;
	mutable strings_set_t	mNewExpectedMembers;
...
public:
	...
	void removeCtrl(LLUICtrl* ctrlp) override;
	...
	void addPanel(LLPanel* panelp, S32 min_width, S32 min_height,
				  bool auto_resize, bool user_resize,
				  EAnimate animate = NO_ANIMATE, S32 index = S32_MAX);
	void removePanel(LLPanel* panelp);
	void collapsePanel(LLPanel* panelp, bool collapsed = true);
