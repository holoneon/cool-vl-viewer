...
public:
	...
	virtual void draw(const LLColor4& color,
					  const LLColor4& highlight_color) = 0;
...
class LLScrollListSeparator final : public LLScrollListCell
{
public:
	LLScrollListSeparator(S32 width);

	void draw(const LLColor4& color, const LLColor4& highlight) override;
...
{
public:
	LLScrollListText(const std::string& text, const LLFontGL* fontp,
	...
	LL_INLINE S32 getContentWidth() const override			{ return mFontGL->getWidth(mText.getString()); }
...
class LLColumnHeader final : public LLComboBox
{
public:
...
class LLScrollListItemComment final : public LLScrollListItem
{
public:
...
class LLScrollListItemSeparator final : public LLScrollListItem
{
public:
...
public:
	...
	bool selectItemByLabel(const std::string& label,
	...
	LLScrollListItem* getItemByLabel(const std::string& label,
									 bool case_sensitive = true,
									 S32 column = 0) const;
	LLScrollListItem* getItemByValue(const std::string& value) const;
	...
	S32 getLastSelectedIndex() const;

	typedef std::vector<LLScrollListItem*> items_vec_t;
	items_vec_t getAllSelected() const;
	...
	void scrollToShowSelected();	// Shows first selected
...
private:
	...
	typedef std::map<std::string, LLScrollListColumn,
					 std::less<> > column_map_t;
