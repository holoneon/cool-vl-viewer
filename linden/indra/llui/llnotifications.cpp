...
///////////////////////////////////////////////////////////////////////////////
// LLNotificationFunctorRegistry class

void LLNotificationFunctorRegistry::registerFunctor(const std::string& name,
													notification_cb_t fn)
{
	if (mMap.count(name))
	{
		llerrs << "Attempt to store duplicate name: " << name << llendl;
	}
	LL_DEBUGS("FunctorRegistry") << "Registered functor for: " << name
								 << LL_ENDL;
	mMap.emplace(name, fn);
}

void LLNotificationFunctorRegistry::unregisterFunctor(const std::string& name)
{
	if (mMap.erase(name))
	{
		LL_DEBUGS("FunctorRegistry") << "Unregistered functor for: " << name
									 << LL_ENDL;
	}
	else
	{
		llwarns << "Could not find functor for: " << name << llendl;
	}
}

notification_cb_t LLNotificationFunctorRegistry::getFunctor(const std::string& name) const
{
...
//static
void LLNotificationFunctorRegistry::doNothing(const LLSD&, const LLSD& payload)
{
...
LLNotificationFunctorRegistration::LLNotificationFunctorRegistration(const std::string& name,
																	 notification_cb_t fn)
{
	LLNotificationFunctorRegistry::getInstance()->registerFunctor(name, fn);
}

///////////////////////////////////////////////////////////////////////////////
// LLNotificationHistoryChannel class
...
public:
	...
	:	LLNotificationChannel("History", "Visible", &historyFilter,
		...
		mFileName(filename)
	{
		connectChanged(std::bind(&LLNotificationHistoryChannel::historyHandler,
								 this, _1));
...
private:
	...
	void savePersistentNotifications()
	{
		...
		for (set_t::iterator it = mItems.begin(), end = mItems.end();
			 it != end; ++it)
		{
...
///////////////////////////////////////////////////////////////////////////////
// LLNotification class

// LLNotification::Params sub-class
LLNotification::Params& LLNotification::Params::functor(notification_cb_t fn)
{
	...
	mIgnored(false)
{
	...
	mfn_tName = sd["responseFunctor"].asString();
...
LLSD LLNotification::asLLSD()
{
	...
	output["responseFunctor"] = mfn_tName;
	return output;
}
...
LLSD LLNotification::getResponseTemplate(EResponseTemplateType type)
{
	...
	if (!mForm)
	{
		llwarns << "Null form when getting response template for notification: "
				<< getName() << llendl;
		return response;
	}
...
void LLNotification::respond(const LLSD& response, bool save)
{
	...
	LLNotificationFunctorRegistry* registryp =
		LLNotificationFunctorRegistry::getInstance();

	LL_DEBUGS("FunctorRegistry") << "Searching the response functor for: "
								 << mfn_tName << LL_ENDL;
	notification_cb_t functor = registryp->getFunctor(mfn_tName);
	// Call the functor
	functor(asLLSD(), response);

	if (mTemporaryResponder)
	{
		registryp->unregisterFunctor(mfn_tName);
		mfn_tName = "";
		mTemporaryResponder = false;
	}
...
void LLNotification::setfn_t(const std::string& functor_name)
{
	if (mTemporaryResponder)
	{
		// Get rid of the old one
		LLNotificationFunctorRegistry::getInstance()->unregisterFunctor(mfn_tName);
	}
	mfn_tName = functor_name;
	mTemporaryResponder = false;
}
...
void LLNotificationChannelBase::connectChanged(const signal_t::slot_type& slot)
{
	...
	// notifications
	for (set_t::iterator it = mItems.begin(), end = mItems.end();it != end;
		 ++it)
	{
		LLNotificationPtr n = *it;
		if (n)	// Paranoia
...
void LLNotificationChannelBase::connectPassedFilter(const signal_t::slot_type& slot)
{
...
void LLNotificationChannelBase::connectFailedFilter(const signal_t::slot_type& slot)
{
...
bool LLNotificationChannelBase::updateItem(const LLSD& payload)
{
	...
	return notifp && updateItemInternal(payload, notifp);
}

// FIX QUIT NOT WORKING

// Internal call, for use in avoiding lookup
bool LLNotificationChannelBase::updateItemInternal(const LLSD& payload,
	...
	mParent(parent)
{
	...
	if (parent.empty())
	{
		gNotifications.connectChanged(std::bind(&LLNotificationChannelBase::updateItem,
												this, _1));
	}
	else
	{
		LLNotificationChannelPtr p = gNotifications.getChannel(parent);
		signal_t::slot_type f =
			std::bind(&LLNotificationChannelBase::updateItem, this, _1);
		p->connectChanged(f);
	}
}

void LLNotificationChannel::setComparator(comparator_t comparator)
{
	mComparator = comparator;
	set_t s2(mComparator);
...
std::string LLNotificationChannel::summarize()
{
	...
	for (set_t::iterator it = begin(); it != end(); ++it)
	{
		s += (*it)->summarize();
		s += "\n  ";
	}
...
// We need a negated filter (was formerly making use of !boost::bind). HB
bool LLNotifications::nonExpirationFilter(LLNotificationPtr notifp)
{
	return !expirationFilter(notifp);
}
...
bool LLNotifications::uniqueFilter(LLNotificationPtr notifp)
{
	...
	for (map_t::iterator it = mUniqueNotifications.find(notifp->getName()),
...
{
	channel_map_t::iterator p = mChannels.find(chan_name);
	if (p == mChannels.end())
	{
...
void LLNotifications::createDefaultChannels()
{
	...
	LLNotificationChannel::buildChannel("Expiration", "",
										std::bind(&LLNotifications::expirationFilter,
												  this, _1));
	LLNotificationChannel::buildChannel("Unexpired", "",
										std::bind(&LLNotifications::nonExpirationFilter,
												  this, _1));
	...
	getChannel("Expiration")->connectChanged(std::bind(&LLNotifications::expirationHandler,
													   this, _1));
	getChannel("Unique")->connectChanged(std::bind(&LLNotifications::uniqueHandler,
												   this, _1));
	getChannel("Unique")->connectFailedFilter(std::bind(&LLNotifications::failedUniquenessTest,
														this, _1));
									...
									S32 option)
{
	...
	{
		llwarns << "Cannot force response for notification with null form: "
...
void LLNotifications::getTemplateNames(strings_vec_t& names) const
{
	for (template_map_t::const_iterator it = mTemplates.begin(),
										end = mTemplates.end();
		 it != end; ++it)
	{
...
void replaceSubstitutionStrings(LLXMLNodePtr node, strings_map_t& replacements)
{
		 ...
		 it != end; ++it)
	{
		...
		if (value[0] == '$')
		{
			...
			std::string replacement;
			strings_map_t::const_iterator found = replacements.find(value);
...
LLXMLNodePtr LLNotifications::checkForXMLTemplate(LLXMLNodePtr itemp)
{
	if (itemp.notNull() && itemp->hasName("usetemplate"))
	{
		std::string replacement;
		if (itemp->getAttributeString("name", replacement))
		{
			strings_map_t replacements;
			for (LLXMLAttribList::const_iterator
					it = itemp->mAttributes.begin(),
					end = itemp->mAttributes.end();
				 it != end; ++it)
			{
				if (it->second)	// Paranoia
				{
...
bool LLNotifications::loadTemplates()
{
	...
	for (LLXMLNodePtr itemp = root->getFirstChild(); itemp.notNull();
		 itemp = itemp->getNextSibling())
	{
		// We do this FIRST so that item can be changed if we encounter a
		// usetemplate; we just replace the current xml node and keep
		// processing.
		itemp = checkForXMLTemplate(itemp);

		if (itemp->hasName("global"))
		{
			std::string global_name;
			if (itemp->getAttributeString("name", global_name))
			{
				mGlobalStrings[global_name] = itemp->getTextContents();
			}
			continue;
		}

		if (itemp->hasName("template"))
		{
			// Store an XML template; templates must have a single node (can
			// contain other nodes)
			std::string name;
			itemp->getAttributeString("name", name);
			LLXMLNodePtr ptr = itemp->getFirstChild();
			mXmlTemplates[name] = ptr;
			continue;
		}

		if (!itemp->hasName("notification"))
		{
            llwarns << "Unexpected entity " << itemp->getName()->mString <<
		...
		if (!itemp->getAttributeString("name", templatep->mName))
		{
		...
		templatep->mMessage = itemp->getTextContents();
		templatep->mDefaultFunctor = templatep->mName;
		itemp->getAttributeString("type", templatep->mType);
		itemp->getAttributeString("icon", templatep->mIcon);
		itemp->getAttributeString("label", templatep->mLabel);
		itemp->getAttributeU32("duration", templatep->mExpireSeconds);
		itemp->getAttributeU32("expireOption", templatep->mExpireOption);

		std::string priority;
		itemp->getAttributeString("priority", priority);
		...
		itemp->getAttributeString("functor", templatep->mDefaultFunctor);

		bool persist = false;
		itemp->getAttributeBool("persist", persist);
		templatep->mPersist = persist;

		std::string sound;
		itemp->getAttributeString("sound", sound);
		...
		for (LLXMLNodePtr child = itemp->getFirstChild(); !child.isNull();
			 child = child->getNextSibling())
		{
...
void LLNotifications::add(const LLNotificationPtr notifp)
{
	...
	set_t::iterator it = mItems.find(notifp);
	if (it != mItems.end())
	{
	...
	updateItemInternal(LLSD().with("sigtype", "add").with("id", notifp->getID()),
					   notifp);
}
...
void LLNotifications::cancel(LLNotificationPtr notifp)
{
	...
	updateItemInternal(LLSD().with("sigtype", "delete").with("id", notifp->getID()),
					   notifp);
	notifp->cancel();
}
...
void LLNotifications::update(const LLNotificationPtr notifp)
{
	...
	if (it != mItems.end())
	{
		updateItemInternal(LLSD().with("sigtype", "change").with("id",
																 notifp->getID()),
						   notifp);
	}
}
...
LLNotificationPtr LLNotifications::find(LLUUID uuid)
{
	...
	set_t::iterator it = mItems.find(target);
	if (it == mItems.end())
	{
...
{
	strings_map_t::const_iterator it = mGlobalStrings.find(key);
