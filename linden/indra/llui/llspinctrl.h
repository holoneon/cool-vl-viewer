...
public:
	...
	void forceSetValue(const LLSD& value);
	...
	LL_INLINE void setPrevalidate(LLLinePrevalidateFunc cb)
	{
		if (mEditor)
		{
			mEditor->setPrevalidate(cb);
		}
	}
	...
	typedef std::string (*preprocess_cb_t)(const std::string& text);
	LL_INLINE void setPreProcessCallback(preprocess_cb_t cb)
	{
		mPreProcessCallback = cb;
	}
...
private:
	...
	LLTextBox*		mLabelBox;
	preprocess_cb_t	mPreProcessCallback;
