...
fast_hset<LLView*> LLUI::sSingletonInstances;
...
//static
bool show_xui_names_cb(const LLSD& newvalue)
{
					 ...
					 const std::string& language)
{
	...
	if (controlp)
	{
		controlp->getSignal()->connect(std::bind(&show_xui_names_cb, _2));
	}
...
void LLUI::connectRefreshSettingsSafe(const char* name)
{
	...
	controlp->getSignal()->connect(std::bind(&LLUI::refreshSettings));
}
...
void LLUI::setLineWidth(F32 width)
{
	...
	{
		// Line width is always 1.f under core GL profile, and gGL.lineWidth()
		// is then a no-operation: we should not waste time calculating the
		// scale for nothing in that case. HB
		return;
	}
	width *= lerp(sGLScaleFactor.mV[VX], sGLScaleFactor.mV[VY], 0.5f);
	gGL.lineWidth(width, true);
}
...
std::string LLUI::locateSkin(const std::string& filename)
{
	...
	if (!LLFile::exists(found_file))
	{
		...
		found_file = gDirUtil.getFullPath(LL_PATH_USER_SETTINGS, filename);
	}
	if (sConfigGroup && sConfigGroup->controlExists("Language"))
	{
		if (!LLFile::exists(found_file))
		{
			...
			local_skin +=  filename;
			found_file = gDirUtil.findSkinnedFilename(local_skin);
		}
	}
	if (!LLFile::exists(found_file))
	{
	...
	if (!LLFile::exists(found_file))
	{
		found_file = gDirUtil.getFullPath(LL_PATH_APP_SETTINGS, filename);
	}
	return found_file;
}
...
//static
void LLUI::addSingletonInstance(LLView* instancep)
{
	sSingletonInstances.insert(instancep);
}

//static
void LLUI::removeSingletonInstance(LLView* instancep)
{
	sSingletonInstances.erase(instancep);
}

//static
void LLUI::deleteSingletonInstances()
{
	llinfos << "Destroying UI singletons..." << llendl;
	while (!sSingletonInstances.empty())
	{
		// The LLUISingleton destructor will automatically remove its address
		// from sSingletonInstances by calling removeSingletonInstance(). HB
		delete *sSingletonInstances.begin();
	}
	llinfos << "UI singletons destroyed." << llendl;
}

///////////////////////////////////////////////////////////////////////////////
// LLScreenClipRect class
///////////////////////////////////////////////////////////////////////////////
