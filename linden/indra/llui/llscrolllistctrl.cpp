...
void LLScrollListIcon::draw(const LLColor4& color, const LLColor4&)
{
	if (mIcon)
	{
...
void LLScrollListCheck::draw(const LLColor4&, const LLColor4&)
{
	mCheckBox->draw();
}
...
void LLScrollListSeparator::draw(const LLColor4& color, const LLColor4&)
{
...
LLScrollListText::LLScrollListText(const std::string& text,
								   const LLFontGL* fontp, S32 width, U8 style,
...
void LLScrollListText::setText(const std::string& text)
{
	mText = text;
	mFontVB.reset();
}
...
{
	return ll_roundp(mFontGL->getLineHeight());
}
							...
							const LLColor4& highlight_color)
{
	...
	if (mHighlightCount > 0)
	{
		...
		{
			case LLFontGL::LEFT:
				left = mFontGL->getWidth(mText.getString(), 0,
										 mHighlightOffset);
				break;

			case LLFontGL::RIGHT:
				left = getWidth() - mFontGL->getWidth(mText.getString(),
													  mHighlightOffset,
													  S32_MAX);
				break;

			case LLFontGL::HCENTER:
				left = (getWidth() - mFontGL->getWidth(mText.getString())) / 2;
		}
		LLRect highlight_rect(left - 2, ll_roundp(mFontGL->getLineHeight()) + 1,
							  left + mFontGL->getWidth(mText.getString(),
	...
	mFontVB.render(mFontGL, mText.getWString(), 0, start_x, 2.f, display_color,
				   mFontAlignment, LLFontGL::BOTTOM, mFontStyle, string_chars,
				   getWidth(), &right_x, true);
}
...
LLScrollListCtrl::items_vec_t LLScrollListCtrl::getAllSelected() const
{
	items_vec_t ret;
...
uuid_vec_t LLScrollListCtrl::getSelectedIDs()
{
	...
	items_vec_t selected = getAllSelected();
	for (items_vec_t::iterator it = selected.begin(), end = selected.end();
		 it != end; ++it)
	{
...
{
	if (!getCanSelect() || mItemList.empty())
	{
		return -1;
	}
	...
	for (item_list::const_iterator it = mItemList.begin(),
		 ...
		 it != end; ++it)
	{
		const LLScrollListItem* itemp = *it;
		if (itemp && itemp->getSelected())
		{
...
S32 LLScrollListCtrl::getLastSelectedIndex() const
{
	if (!getCanSelect() || mItemList.empty())
	{
		return -1;
	...
	S32 cur_selected_idx = mItemList.size() - 1;
	for (item_list::const_reverse_iterator it = mItemList.rbegin(),
										   rend = mItemList.rend();
		 it != rend; ++it)
	{
		const LLScrollListItem* itemp = *it;
...
LLScrollListCtrl::items_vec_t LLScrollListCtrl::getAllData() const
{
	items_vec_t ret;
...
void LLScrollListCtrl::selectPrevItem(bool extend_selection)
{
	...
	LLScrollListItem* prev_itemp = NULL;
		 ...
		 iter != end; ++iter)
	{
		...
		// Do not allow to select a disabled item
		if (itemp->getEnabled())
		{
			prev_itemp = itemp;
		}
	...
	selectItem(prev_itemp, !extend_selection);
	if (extend_selection)
	{
		mNeedsScroll = true;
	}
...
void LLScrollListCtrl::selectNextItem(bool extend_selection)
{
	...
	LLScrollListItem* next_itemp = NULL;
	...
	selectItem(next_itemp, !extend_selection);
	if (extend_selection)
	{
		S32 pos = getLastSelectedIndex();
		if (pos >= 0)
		{
			setScrollPos(pos);
		}
		mNeedsScroll = false;	// Do not scroll to show first selected ! HB
	}
												   ...
												   S32 column) const
{
		 ...
		 iter != end; ++iter)
	{
		LLScrollListItem* itemp = *iter;
		// Only select enabled items with matching names
		item_text = itemp->getColumn(column)->getValue().asString();
		if (!case_sensitive)
		{
			LLStringUtil::toLower(target_text);
		}
...
LLScrollListItem* LLScrollListCtrl::getItemByValue(const std::string& value) const
{
	for (item_list::const_iterator iter = mItemList.begin(),
								   end = mItemList.end();
		 iter != end; ++iter)
	{
		LLScrollListItem* itemp = *iter;
		if (itemp && itemp->getValue().asString() == value)
		{
			return itemp;
		}
	}
													  ...
													  S32 column_width)
{
	...
	if (getItemCount() < mMaxItemCount)
	{
		itemp = new LLScrollListItem(enabled, NULL, id);
		itemp->addColumn(item_text, font, column_width);
		addItem(itemp, pos);
	}
	return itemp;
}
			...
			switch (key)
			{
				case KEY_UP:
					if (mAllowKeyboardMovement || hasFocus())
					{
						...
						selectPrevItem(mask == MASK_SHIFT);
						mNeedsScroll = true;
				...
				case KEY_DOWN:
					...
					{
						// Commit implicit in call
						selectNextItem(mask == MASK_SHIFT);
						mNeedsScroll = true;
						if (mCommitOnKeyboardMovement &&
							!mCommitOnSelectionChange)
						{
...
//virtual
const std::string& LLScrollListCtrl::getTag() const
{
	return LL_SCROLL_LIST_CTRL_TAG;
}
...
{
	std::string buffer;
	items_vec_t items = getAllSelected();
