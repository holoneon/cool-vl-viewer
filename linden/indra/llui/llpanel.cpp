...
LLXMLNodePtr LLPanel::getXML(bool save_children) const
{
	...
	strings_map_t::const_iterator i = mUIStrings.begin();
	strings_map_t::const_iterator end = mUIStrings.end();
	for ( ; i != end; ++i)
	{
							   ...
							   const LLStringUtil::format_map_t& args) const
{
	...
	strings_map_t::const_iterator found_it = mUIStrings.find(name);
...
void LLPanel::childDisplayNotFound()
{
	...
	std::string msg;
	for (strings_set_t::iterator it = mNewExpectedMembers.begin(),
								 end = mNewExpectedMembers.end();
		 it != end; ++it)
	{
		msg.append(*it);
		msg.append("\n");
		mExpectedMembers.emplace(*it);
	}
