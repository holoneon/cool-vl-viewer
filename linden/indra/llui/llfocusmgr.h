...
public:
	...
	// LLFocusableElement is not also derived from LLUICtrl, or true when it
	// is.
	// Note: this was renamed from isUICtrl(), which was ambiguous: we also got
	// isCtrl() in the Cool VL Viewer to distinguish LLView from LLUICtrl. HB
	virtual bool isFocusableCtrl() const = 0;
...
protected:
	void (*mFocusLostCallback)(LLFocusableElement* callerp, void* userdata);
	void (*mFocusReceivedCallback)(LLFocusableElement* ctrlp, void* userdata);
	void (*mFocusChangedCallback)(LLFocusableElement* ctrlp, void* userdata);
...
public:
	...
	void setMouseCapture(LLMouseHandler* new_captorp);

	LL_INLINE LLMouseHandler* getMouseCapture() const	{ return mMouseCaptor; }

	void removeMouseCaptureWithoutCallback(const LLMouseHandler* captorp);
	bool childHasMouseCapture(const LLView* parentp) const;

	// Keyboard Focus

	// new_focus = NULL to release the focus:
	void setKeyboardFocus(LLFocusableElement* new_focusp, bool lock = false,
	...
	bool childHasKeyboardFocus(const LLView* parentp) const;
	void removeKeyboardFocusWithoutCallback(const LLFocusableElement* focusp);
	...
	LLUICtrl* getLastFocusForGroup(LLView* viewp) const;
	void clearLastFocusForGroup(LLView* viewp);
	...
	void setTopCtrl(LLUICtrl* ctrlp);
	LL_INLINE LLUICtrl* getTopCtrl() const				{ return mTopCtrl; }
	void removeTopCtrlWithoutCallback(const LLUICtrl* ctrlp);
	bool childIsTopCtrl(const LLView* parentp) const;

	// All Three

	void releaseFocusIfNeeded(const LLView* top_viewp);
	void lockFocus();
	LL_INLINE void unlockFocus()						{ mLockedView = NULL; }
