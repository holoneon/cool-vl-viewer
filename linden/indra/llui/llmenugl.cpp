...
//
// Note: a menu label is split into 4 columns. The left column, the label
// column, the accelerator column, and the right column. The left column is
// used for displaying boolean values for toggle and check controls. The right
// column is used for submenus.
...
static const std::string SEPARATOR_NAME("separator");
static const std::string TEAROFF_SEPARATOR_LABEL("~~~~~~~~~~~");
static const std::string SEPARATOR_LABEL("-----------");
static const std::string VERTICAL_SEPARATOR_LABEL("|");
static const std::string CHECK_MARK("X");
static const std::string BRANCH_MARK(">");
...
LLColor4 LLMenuItemGL::sEnabledColor(0.f, 0.f, 0.f, 1.f);
LLColor4 LLMenuItemGL::sDisabledColor(0.5f, 0.5f, 0.5f, 1.f);
LLColor4 LLMenuItemGL::sHighlightBackground(0.f, 0.f, 0.7f, 1.f);
LLColor4 LLMenuItemGL::sHighlightForeground(1.f, 1.f, 1.f, 1.f);
...
//LLColor4 LLMenuGL::sBackgroundColor(0.8f, 0.8f, 0.f, 1.f);
...
:	LLView(name, true),
	...
	mDrawBoolLabel(false),
	mDrawBranchLabel(false)
{
	setLabel(label);
}
...
{
	return ll_roundp(mFontGL->getLineHeight()) + MENU_ITEM_PADDING;
}
...
U32 LLMenuItemGL::getNominalWidth() const
{
	...
	if (KEY_NONE != mAcceleratorKey)
	{
		...
		width += mFontGL->getWidth(temp);
	}
	width += mFontGL->getWidth(mLabel.getWString().c_str());
	return width;
}
...
void LLMenuItemGL::draw()
{
	...
	if (mBriefItem)
	{
		mFontVB.render(mFontGL, mLabel, 0, 0.5f * BRIEF_PAD_PIXELS, 0.f, color,
	...
	else
	{
		if (mDrawBoolLabel)
		{
			static const LLWString check = utf8str_to_wstring(CHECK_MARK);
			mFontGL->render(check, 0, (F32)LEFT_PAD_PIXELS,
							(F32)MENU_ITEM_PADDING * 0.5f + 1.f, color,
							LLFontGL::LEFT, LLFontGL::BOTTOM, font_style);
		}
		mFontVB.render(mFontGL, mLabel.getWString(), 0, (F32)LEFT_PLAIN_PIXELS,
					   (F32)MENU_ITEM_PADDING * 0.5f + 1.f, color,
					   LLFontGL::LEFT, LLFontGL::BOTTOM, font_style);
		if (!mDrawAccelLabel.empty())
		{
			mFontGL->render(mDrawAccelLabel.getWString(), 0,
		...
		if (mDrawBranchLabel)
		{
			static const LLWString branch = utf8str_to_wstring(BRANCH_MARK);
			mFontGL->render(branch, 0,
	...
	if (menup && menup->jumpKeysActive() && LLMenuGL::getKeyboardMode())
	{
		...
		{
			S32 x_begin = LEFT_PLAIN_PIXELS + mFontGL->getWidth(mLabel, 0,
																offset);
			S32 x_end = LEFT_PLAIN_PIXELS + mFontGL->getWidth(mLabel, 0,
															  offset + 1);
										 ...
										 LLView* context)
{
	...
	if (!enabled_control.empty())
	{
		...
		control->getSignal()->connect(std::bind(&LLView::controlListener, _2,
												getHandle(), "enabled"));
...
{
	mChecked = mDrawBoolLabel = value.asBoolean();
}
...
{
	mDrawBoolLabel = mChecked ||
					 (mCheckCallback && mCheckCallback(getUserData()));
...
public:
	...
	U32 getNominalWidth() const override;
...
void LLMenuItemBranchDownGL::draw()
{
	...
	getFontVB().render(getFont(), mLabel.getWString(), 0,
					   (F32)getRect().getWidth() * 0.5f,
...
void LLMenuGL::createJumpKeys()
{
	...
	strings_set_t unique_words;
	strings_set_t shared_words;
