...
{
	std::string filename = gDirUtil.getFullPath(LL_PATH_SKINS, "paths.xml");
...
bool LLUICtrlFactory::getLayeredXMLNode(const std::string& xui_filename,
										LLXMLNodePtr& root)
{
	std::string full_filename = gDirUtil.findSkinnedFilename(sXUIPaths.front(),
															 xui_filename);
		 ...
		 it != end; ++it)
	{
		...
		layer_filename = gDirUtil.findSkinnedFilename(*it, xui_filename);
