...
public:
	...
	// The two following methods shall only be called from LLUISingleton, to
	// register and unregister the singleton instances, in a type-agnostic way,
	// and allow them to be destroyed cleanly on viewer exit. HB
	static void addSingletonInstance(LLView* instancep);
	static void removeSingletonInstance(LLView* instancep);
	// Called on viewer exit, to destroy all UI singleton instances. HB
...
private:
	// This set contains all the addresses of the UI singletons that have been
	// created during the viewer session; it is used to destroy all these
	// instances on viewer exit. Note that all singleton instances are derived
	// from a LLView, which is the parent class for all UI sub-classes. HB
	static fast_hset<LLView*>			sSingletonInstances;
...
protected:
	...
	LL_INLINE LLUISingleton()
	{
		sInstance = static_cast<T*>(this);
		LLUI::addSingletonInstance(sInstance);
	}

	LL_INLINE ~LLUISingleton()
	{
		LLUI::removeSingletonInstance(sInstance);
		sInstance = NULL;
	}

public:
	LL_INLINE static T* findInstance(const LLSD& key = LLSD())
	{
		return sInstance;
	}

	LL_INLINE static T* createInstance(const LLSD& key = LLSD())
	{
		if (!sInstance)
		{
			sInstance = new T(key);
