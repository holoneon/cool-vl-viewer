...
class LLTextSLURLSubstitutions
{
public:
	LL_INLINE LLTextSLURLSubstitutions(LLTextEditor* parentp,
									   bool with_mentions,
									   bool echo_to_console)
	:	mParent(parentp),
		mWithMentions(with_mentions),
		mEchoToConsole(echo_to_console)
	{
		sInstances.insert(this);
	}

	LL_INLINE ~LLTextSLURLSubstitutions()
	{
		sInstances.erase(this);
	}

	LL_INLINE void registerPending(const uuid_list_t& ids)
	{
		mPendingIds.insert(ids.begin(), ids.end());
	}

	static void substitute(const LLUUID& id, const std::string& slurl,
						   const std::string& substitute, bool is_mention);
	static void substitutionDone(const LLUUID& id);

public:
	LLTextEditor*		mParent;
	uuid_list_t			mPendingIds;
	bool				mWithMentions;
	bool				mEchoToConsole;

	typedef fast_hset<LLTextSLURLSubstitutions*> instances_t;
	static instances_t	sInstances;
};

//static
LLTextSLURLSubstitutions::instances_t LLTextSLURLSubstitutions::sInstances;

//static
void LLTextSLURLSubstitutions::substitute(const LLUUID& id,
										  const std::string& slurl,
										  const std::string& substitute,
										  bool is_mention)
{
	for (instances_t::iterator it = sInstances.begin(), end = sInstances.end();
		 it != end; ++it)
	{
		LLTextSLURLSubstitutions* instancep = *it;
		if (!instancep->mPendingIds.count(id))
		{
			continue;
		}
		LLTextEditor* editp = instancep->mParent;
		// Make sure text segments will be preserved
		editp->setPreserveSegments(true);
		bool read_only = editp->isReadOnly();
		if (is_mention && instancep->mWithMentions)
		{
			editp->replaceTextAll(slurl, "@" + substitute, true);
		}
		else
		{
			editp->replaceTextAll(slurl, substitute, true);
		}
		if (instancep->mEchoToConsole)
		{
			if (is_mention)
			{
		...
		// LLTextEditor::replaceTextAll() resets the read only status, so we
		// must reassert it if needed.
		if (read_only)
		{
...
//static
void LLTextSLURLSubstitutions::substitutionDone(const LLUUID& id)
{
	for (instances_t::iterator it = sInstances.begin(), end = sInstances.end();
		 it != end; ++it)
	{
		LLTextSLURLSubstitutions* instancep = *it;
		instancep->mPendingIds.erase(id);
	}
}
...
LLTextEditor::LLTextEditor(const std::string& name, const LLRect& rect,
						   U32 max_length,	// In bytes
						   const std::string& default_text, LLFontGL* fontp,
...
LLTextEditor::~LLTextEditor()
{
	...
	if (mSLURLSubstitutions)
	{
		delete mSLURLSubstitutions;
		mSLURLSubstitutions = NULL;
	}
...
bool LLTextEditor::truncate()
{
	...
	if (mWText.size() >= (size_t)(mMaxTextByteLength / 4))
	{
		...
		if (utf8_byte_size > (S32)mMaxTextByteLength)
		{
...
void LLTextEditor::registerPendingSLURLs(const uuid_list_t& pending_ids,
										 bool with_mentions,
										 bool echo_to_console)
{
	if (!mSLURLSubstitutions)
	{
		mSLURLSubstitutions = new LLTextSLURLSubstitutions(this, with_mentions,
														   echo_to_console);
	}
	mSLURLSubstitutions->registerPending(pending_ids);
}

//static
void LLTextEditor::substituteSLURL(const LLUUID& id, const std::string& slurl,
								   const std::string& substitute,
								   bool is_mention)
{
	LLTextSLURLSubstitutions::substitute(id, slurl, substitute, is_mention);
}

//static
void LLTextEditor::substitutionDone(const LLUUID& id)
{
	LLTextSLURLSubstitutions::substitutionDone(id);
}
...
void LLTextEditor::indentSelectedLines(S32 spaces)
{
	...
	if (spaces < 0)
	{
		// Expand 'right' to end of previous line, else we would un-indent past
		// the selection end. HB
		while (right > left)
		{
			if (text[--right] == '\n')
			{
	...
	if (spaces < 0)
	{
		// Set 'right' back to the end of last un-indented line. HB
		while (right > left)
		{
			if (text[--right] == '\n')
			{
...
S32 LLTextEditor::addChar(S32 pos, llwchar wc)
{
	if (wstring_utf8_length(mWText) +
			wchar_utf8_length(wc) >= (S32)mMaxTextByteLength)
	{
...
bool LLTextEditor::handleSpecialKey(KEY key, MASK mask, bool* return_key_hit)
{
	*return_key_hit = false;
	...
	case KEY_TAB:
		...
		else if (!(mask & MASK_SHIFT))	// *TODO: implement un-tabbing. HB
		{
			if (hasSelection())
			{
				deleteSelection(false);
...
bool LLTextEditor::handleKeyHere(KEY key, MASK mask)
{
	...
	{
		// Special case for TAB. If we want to move to next field, report not
...
//virtual
bool LLTextEditor::canDelete() const
{
...
void LLTextEditor::drawText()
{
	...
	const LLFontGL* num_fontp = mShowLineNumbers ? LLFontGL::getFontMonospace()
												 : NULL;
	F32 num_spacing = 0.f;
	S32 num_offset = 0;
	if (num_fontp)
	{
		num_spacing = F32(ll_roundp(num_fontp->getLineHeight())) * 0.5f;
		num_offset = UI_TEXTEDITOR_LINE_NUMBER_MARGIN;
	}
	S32 line_height = ll_roundp(mGLFont->getLineHeight());
	F32 text_y = (F32)(mTextRect.mTop - line_height);
	while (mTextRect.mBottom <= text_y && cur_line < num_lines)
	{
		...
		F32 text_x = text_start + num_offset;

		// Draw the line numbers
		if (!cur_line_is_continuation && num_fontp)
		{
			F32 y_top = text_y + num_spacing;
			...
			const LLColor4 fg_color = is_cur_line ? mCursorColor
												  : mReadOnlyFgColor;
			num_fontp->render(ltext,				// String to draw
							  0,					// Begin offset
							  3.,					// x
							  y_top,				// y
							  fg_color,
		...
		while (seg_start < line_end)
		{
			...
			if (clipped_len > 0)
			{
				LLStyleSP stylep = cur_segment->getStyle();
				if (stylep->isImage() &&
					cur_segment->getStart() >= seg_start &&
					cur_segment->getStart() <= clipped_end)
				{
					S32 style_height = stylep->mImageHeight;
					S32 style_width = stylep->mImageWidth;
					LLUIImagePtr imagep = stylep->getImage();
					imagep->draw(ll_round(text_x),
								 ll_round(text_y) + line_height - style_height,
								 style_width, style_height);
				}

				bool is_embedded = cur_segment == mHoverSegment &&
								   stylep->getIsEmbeddedItem();
				if (is_embedded)
				{
					stylep->mUnderline = true;
				}
									 ...
									 LLStyleSP stylep)
{
	...
	enum { WHOLE, START, MIDDLE, END };
	S32 part = WHOLE;
	S32 start = 0, end = 0;
	std::string text = new_text;
	while (findHTML(text, &start, &end))
	{
		LLStyleSP htmlp = new LLStyle;
		htmlp->setVisible(true);
		htmlp->setColor(mLinkColor);
		if (stylep)
		{
			htmlp->setFontName(stylep->getFontString());
		}
		htmlp->mUnderline = true;
		...
		htmlp->setLinkHREF(text.substr(start, end - start));
		appendText(text.substr(start, end - start), allow_undo,
				   prepend_newline, htmlp);
							  ...
							  bool prepend_newline, const LLStyleSP stylep)
{
	...
	if (stylep)
	{
		mSegments.push_back(new LLTextSegment(stylep, old_length,
											  getLength()));
	}
...
S32 LLTextEditor::removeStringNoUndo(S32 pos, S32 length)
{
	...
	length = llmin(length, text_len - pos);
...
void LLTextEditor::loadKeywords(const std::string& syntax_file,
								const std::string& kewyords_file,
								const std::vector<std::string>& funcs,
								const std::vector<std::string>& tooltips,
								const LLColor3& color, bool lua_syntax)
{
	mKeywords.clear();
	mKeywords.setLuaSyntax(lua_syntax);

	if (mKeywords.loadFromFile(syntax_file))
	{
		if (mKeywords.loadFromFile(kewyords_file))
		{
			S32 count = llmin(funcs.size(), tooltips.size());
			std::string name;
			for (S32 i = 0; i < count; ++i)
			{
				name = utf8str_trim(funcs[i]);
				mKeywords.addToken(LLKeywordToken::WORD, name, color,
								   tooltips[i]);
			}
...
bool LLTextEditor::importBuffer(const char* buffer, S32 length)
{
	...
	if (text_len > (S32)mMaxTextByteLength)
	{
