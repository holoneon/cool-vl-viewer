...
const std::string LL_UI_CRTL_TAG = "ui_ctrl";
static LLRegisterWidget<LLUICtrl> r28(LL_UI_CRTL_TAG);

//static
LLUICtrl::lua_reg_cmd_fn_t LLUICtrl::sRegisterLuaCommand = NULL;
...
//virtual
const std::string& LLUICtrl::getTag() const
{
	return LL_UI_CRTL_TAG;
}
...
{
	nodep->getAttributeBool("tab_stop", mTabStop);

	// See if we got a lua_command attribute, and if any, try and register it
	// on behalf of our floater (only successful when it is a Lua floater). HB
	if (sRegisterLuaCommand)
	{
		std::string lua_command;
		nodep->getAttributeString("lua_command", lua_command);
		if (!lua_command.empty())
		{
			sRegisterLuaCommand(this, parentp, lua_command);
		}
	}
...
//static
LLView* LLUICtrl::fromXML(LLXMLNodePtr nodep, LLView* parentp,
						  class LLUICtrlFactory* factoryp)
{
	LLUICtrl* ctrlp = new LLUICtrl();
	ctrlp->initFromXML(nodep, parentp);
	return ctrlp;
}
