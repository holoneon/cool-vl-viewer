				 ...
				 line_it != end; ++line_it)
			{
					 ...
					 seg_it != end2; ++seg_it)
				{
					...
					LLColor4 color(scolor.mV[VX], scolor.mV[VY], scolor.mV[VZ],
								   scolor.mV[VW] * alpha);
					seg_it->mFontBuffer.render(mFont, seg_it->mText, 0,
											   seg_it->mXPosition - 8,
											   y_pos -  y_off,
											   color, LLFontGL::LEFT,
											   LLFontGL::BASELINE,
...
void LLConsole::replaceParaText(Paragraph* parap, const LLWString& search_text,
								...
								bool case_insensitive, bool new_paragraph)
{
	...
	final_text = para_text = parap->mParagraphText;
	if (case_insensitive)
	{
	...
	while ((pos = para_text.find(search_text)) != std::string::npos)
	{
		...
		if (offset)
		{
			...
			for (paragraph_color_segments_t::iterator
					seg_it = parap->mParagraphColorSegments.begin(),
					seg_end = parap->mParagraphColorSegments.end();
				 seg_it != seg_end; ++seg_it)
			{
				...
				if (seg_pos >= (S32)pos)
				{
					...
					{
						parap->mParagraphColorSegments.erase(seg_it);
					}
					break;
				}
	...
	if (replaced)
	{
		parap->mParagraphText = final_text;
		if (offset && !new_paragraph)
		{
			parap->updateLines((F32)getRect().getWidth(), mFont, true);
		}
	}
}
