...
public:
	typedef void (*LLUICtrlCallback)(LLUICtrl* ctrlp, void* userdata);
	...
	void initFromXML(LLXMLNodePtr nodep, LLView* parentp) override;
	...
	LL_INLINE bool isFocusableCtrl() const override					{ return true; }
	...
	static LLView* fromXML(LLXMLNodePtr nodep, LLView* parentp,
						   class LLUICtrlFactory* factoryp);
	...
	typedef void (*lua_reg_cmd_fn_t)(LLUICtrl* ctrlp, LLView* parentp,
									 const std::string& cmd);
	LL_INLINE static void setRegisterLuaCommandFunc(lua_reg_cmd_fn_t fn)
	{
		sRegisterLuaCommand = fn;
	}

protected:
	LLUICtrlCallback		mCommitCallback;
	LLUICtrlCallback		mLostTopCallback;

	typedef bool (*validate_cb_t)(LLUICtrl* ctrlp, void* userdata);
	validate_cb_t			mValidateCallback;
...
private:
	...
	static lua_reg_cmd_fn_t	sRegisterLuaCommand;
};

extern const std::string LL_UI_CRTL_TAG;
