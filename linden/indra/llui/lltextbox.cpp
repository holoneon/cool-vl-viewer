...
:	LLUICtrl(name, rect, mouse_opaque, NULL, NULL, FOLLOWS_LEFT | FOLLOWS_TOP),
	mFontGL(fontp ? const_cast<LLFontGL*>(fontp)
...
void LLTextBox::setFont(LLFontGL* fontp)
{
	mFontGL = fontp ? fontp :  LLFontGL::getFontSansSerifSmall();
}
...
void LLTextBox::setLineLengths()
{
	...
	while (cur < len)
	{
		...
		mLineLengthList.push_back((S32)run_len);
	}
}
...
void LLTextBox::setWrappedText(const std::string& in_text, F32 max_width)
{
	...
	while (cur < len)
	{
		...
		LLWString::size_type run_len = end - cur;
		if (run_len > 0)
		{
			LLWString run(wtext, cur, run_len);
			LLWString::size_type use_len =
				mFontGL->maxDrawableChars(run.c_str(), max_width, run_len,
										  true);
			final_wtext.append(wtext, cur, use_len);
			cur += use_len;
		}
...
void LLTextBox::setUseEllipses(bool b)
{
	mUseEllipses = b;
	// Needed because the last ellipses value is not kept by LLFontVertexBuffer
	mFontVB.reset();
}
...
void LLTextBox::drawText(S32 x, S32 y, const LLColor4& color)
{
	...
	{
		mFontVB.render(mFontGL, mText.getWString(), 0, (F32)x, (F32)y, color,
					   mHAlign, mVAlign, mFontStyle, S32_MAX,
					   getRect().getWidth(), NULL, mUseEllipses);
		return;
	}

	const S32 line_height = llfloor(mFontGL->getLineHeight());
	S32 cur_pos = 0;
	for (std::vector<S32>::iterator iter = mLineLengthList.begin(),
									end = mLineLengthList.end();
		 iter != end; ++iter)
	{
		...
		if (cur_pos == 0)
		{
			mFontVB.render(mFontGL, mText.getWString(), 0, (F32)x, (F32)y,
						   color, mHAlign, mVAlign, mFontStyle, line_len,
						   getRect().getWidth(), NULL, mUseEllipses);
		}
		else
		{
			mFontGL->render(mText.getWString(), cur_pos, (F32)x, (F32)y, color,
							mHAlign, mVAlign, mFontStyle, line_len,
							getRect().getWidth(), NULL, false, mUseEllipses);
		}
		cur_pos += line_len + 1;
		y -= line_height + mLineSpacing;
	}
}
...
//virtual
const std::string& LLTextBox::getTag() const
{
						   ...
						   LLUICtrlFactory* factory)
{
	...
	LLTextBox* text_box = new LLTextBox(name, LLRect(), text, fontp, false);
