...
public:
	...
	LL_INLINE virtual HBLuaFloater* asLuaFloater()		{ return NULL; }
	...
	void addChildAtEnd(LLView* viewp,  S32 tab_group = 0);
	// Removes the specified child from the view, and sets its parent to NULL.
	void removeChild(LLView* viewp, bool delete_it = false);
	...
	// Return the parent floater, if any, or NULL. HB
	...
	// Returns the "tag" this UI element is registered as via LLRegisterWidget,
	// which is the XML element name for this widget, as used in XUI files. HB
...
// Used to be in its own llrootview.h/cpp module, but what for ?... HB
class LLRootView : public LLView
{
public:
	LL_INLINE LLRootView(const std::string& name, const LLRect& rect,
