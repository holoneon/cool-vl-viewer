...
public:
	LLTextEditor(const std::string& name, const LLRect& rect,
				 U32 max_length, const std::string& default_text,
				 LLFontGL* glfontp = NULL,
	...
	static LLView* fromXML(LLXMLNodePtr nodep, LLView* parentp,
						   class LLUICtrlFactory*);
	...
	// Used by LLTextSLURLSubstitutions::substitute(). HB
	...
	void loadKeywords(const std::string& syntax_file,
					  const std::string& kewyords_file,
					  const std::vector<std::string>& funcs,
					  const std::vector<std::string>& tooltips,
					  const LLColor3& func_color, bool lua_syntax);
	...
	void registerPendingSLURLs(const uuid_list_t& pending_ids,
	...
	LL_INLINE U32 getMaxLength() const 							{ return mMaxTextByteLength; }
	LL_INLINE void setMaxLength(U32 l)							{ mMaxTextByteLength = l; }
	...
	// Allow to change the font on the fly. HB
	void setFont(LLFontGL* fontp);
	LL_INLINE LLFontGL* getFont() const							{ return mGLFont; }
...
private:
	...
	// Maximum length mText is allowed to be in bytes
	U32							mMaxTextByteLength;	
	...
	LLTextSLURLSubstitutions*	mSLURLSubstitutions;
