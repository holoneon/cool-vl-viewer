...
void LLAlertDialog::initClass()
{
	...
	LLNotificationChannel::buildChannel("AlertModal", "Visible",
										LLNotificationFilters::filterBy<std::string>(&LLNotification::getType,
																					 "alertmodal"));
	gNotifications.getChannel("Alerts")->connectChanged(std::bind(&onNewNotification,
																  _1, false));
	gNotifications.getChannel("AlertModal")->connectChanged(std::bind(&onNewNotification,
																	  _1,
																	  true));
}
	...
	mNote(notification)
{
	...
	// It is ok for the edit text body to be empty, but we want the name to
	// exist if we are going to draw it
	...
	LLRect rect;
	// Note: '+ 4' (pixels) to avoid spurious truncations of the message last
	// character, due to various rounding errors. HB
	rect.setLeftTopAndSize(msg_x, msg_y, text_rect.getWidth() + 4,
						   text_rect.getHeight());
