...
// Adding or changing an argument will update the result string, preserving the
// origianl. Thus, subsequent changes to arguments or even the original string
...
// Implementation note:
// Attempting to have operator[](const std::string& s) return mArgs[s] fails
// because we have to call format() after the assignment happens.
