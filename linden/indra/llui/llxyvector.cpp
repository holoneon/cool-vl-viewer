...
void LLXYVector::draw()
{
	...
	// *TODO: arrow does not display well with small values. E.g. (0.1, 0.05)
	// will point to the right, as if it is (0.1, 0.0) as position will be
	// offset by a single pixes on X, and 0 pixels on Y.
	if ((fabs(mValueX) >= mIncrementX || fabs(mValueY) >= mIncrementY) &&
		// At least 1 pixel displacement
		abs(point_x - center_x) + abs(point_y - center_y) >= 1)
	{
...
//virtual
const std::string& LLXYVector::getTag() const
{
	return LL_XY_VECTOR_TAG;
}
