...
void LLLineEditor::setLabel(const std::string& label)
{
	mLabel = label;
	mFontVB.reset();
}
...
//virtual
bool LLLineEditor::setTextArg(const std::string& key, const std::string& text)
{
...
//virtual
bool LLLineEditor::setLabelArg(const std::string& key, const std::string& text)
{
...
S32 LLLineEditor::calculateCursorFromMouse(S32 local_mouse_x)
{
	...
	return mScrollHPos +
		   mFontGL->charFromPixelOffset(wtext, mScrollHPos,
...
void LLLineEditor::setCursor(S32 pos)
{
	...
	if (pixels_after_scroll > mMaxHPixels)
	{
		...
		S32 width_chars_to_left = mFontGL->getWidth(wtext, 0, mScrollHPos);
		S32 last_visible_char =
			mFontGL->maxDrawableChars(wtext,
									  llmax(0.f, (F32)(mMaxHPixels -
													   mMinHPixels +
													   width_chars_to_left)));
		S32 min_scroll =
			mFontGL->firstDrawableChar(wtext,
...
{
	SpellMenuBind* spellp = (SpellMenuBind*)data;
	LLLineEditor* line = spellp->mOrigin;
	if (spellp && line)
	{
		LL_DEBUGS("SpellCheck") << spellp->mMenuItem->getName()
								<< " : " << spellp->mOrigin->getName()
								<< " : " << spellp->mWord << LL_ENDL;
		line->spellReplace(spellp);
		// Make it update:
...
void LLLineEditor::spellShow(void* data)
{
	...
	if (spellp && line)
	{
		line->mShowMisspelled = (spellp->mWord == "Show Misspellings");
		// Make it update:
		line->mKeystrokeTimer.reset();
		line->mPrevSpelledText.erase();
	}
...
void LLLineEditor::spellAdd(void* data)
{
	...
	if (spellp && line)
	{
		LLSpellCheck::getInstance()->addToCustomDictionary(spellp->mWord);
		// Make it update:
		line->mKeystrokeTimer.reset();
		line->mPrevSpelledText.erase();
	}
...
void LLLineEditor::spellIgnore(void* data)
{
	...
	if (spellp && line)
	{
		LLSpellCheck::getInstance()->addToIgnoreList(spellp->mWord);
		// Make it update:
		line->mKeystrokeTimer.reset();
		line->mPrevSpelledText.erase();
	}
...
bool LLLineEditor::handleRightMouseDown(S32 x, S32 y, MASK mask)
{
	...
	LLMenuGL* menup = createContextMenu();
	if (!menup)
	{
		return true;
	}

	SpellMenuBind* spellp;
	LLMenuItemCallGL* mitemp;

	// Remove old suggestions
	for (S32 i = 0, count = mSuggestionMenuItems.size(); i < count; ++i)
	{
		spellp = mSuggestionMenuItems[i];
		if (spellp)
		{
			mitemp = spellp->mMenuItem;
			menup->remove(mitemp);
			mitemp->die();
			//delete spellp->mMenuItem;
			//spellp->mMenuItem = NULL;
			delete spellp;
		}
	}
	mSuggestionMenuItems.clear();

	LLSpellCheck* checkerp = NULL;
	// Not read-only, spell_check="true" in XUI and spell checking enabled
	bool spell_check = !mReadOnly && mSpellCheck;
	if (spell_check)
	{
		checkerp = LLSpellCheck::getInstance();
		spell_check = checkerp->getSpellCheck();
	}
	menup->setItemVisible("spell_sep", spell_check);
	if (spell_check)
	{
		// Search for word matches
		bool is_word_part = getWordBoundriesAt(pos, &word_start, &word_len);
		if (is_word_part)
		{
			const LLWString& text = mText.getWString();
			std::string part(text.begin(), text.end());
			std::string selected_word = part.substr(word_start, word_len);
			if (!checkerp->checkSpelling(selected_word))
			{
				// misspelled word here
				std::vector<std::string> suggestions;
				S32 count = checkerp->getSuggestions(selected_word,
													 suggestions);
				for (S32 i = 0; i < count; ++i)
				{
					spellp = new SpellMenuBind;
					spellp->mOrigin = this;
					spellp->mWord = suggestions[i];
					spellp->mWordPositionEnd = word_start + word_len;
					spellp->mWordPositionStart = word_start;
					mitemp = new LLMenuItemCallGL(spellp->mWord,
													 spellCorrect,
													 NULL, spellp);
					spellp->mMenuItem = mitemp;
					mSuggestionMenuItems.push_back(spellp);
					menup->append(mitemp);
				}
				spellp = new SpellMenuBind;
				spellp->mOrigin = this;
				spellp->mWord = selected_word;
				spellp->mWordPositionEnd = word_start + word_len;
				spellp->mWordPositionStart = word_start;
				mitemp = new LLMenuItemCallGL("Add word", spellAdd,
												 NULL, spellp);
				spellp->mMenuItem = mitemp;
				mSuggestionMenuItems.push_back(spellp);
				menup->append(mitemp);
				...
				spellp->mWordPositionEnd = word_start + word_len;
				spellp->mWordPositionStart = word_start;
				mitemp = new LLMenuItemCallGL("Ignore word", spellIgnore,
												 NULL, spellp);
				spellp->mMenuItem = mitemp;
				mSuggestionMenuItems.push_back(spellp);
				menup->append(mitemp);
		...
		if (mShowMisspelled)
		{
			spellp->mWord = "Hide misspellings";
		}
		else
		{
			spellp->mWord = "Show misspellings";
		}
		mitemp = new LLMenuItemCallGL(spellp->mWord, spellShow, NULL,
										 spellp);
		spellp->mMenuItem = mitemp;
		mSuggestionMenuItems.push_back(spellp);
		menup->append(mitemp);
	...
	menup->updateParent(LLMenuGL::sMenuContainer);
...
bool LLLineEditor::handleSpecialKey(KEY key, MASK mask)
{
	...
	switch (key)
	{
		...
		// Handle ctrl-up-arrow if we have a history enabled line editor.
		case KEY_UP:
		...
		// Handle ctrl-down-arrow if we have a history enabled line editor
		case KEY_DOWN:
			if (mHaveHistory  && mask == MASK_CONTROL)
			{
				if (!mLineHistory.empty() &&
...
bool LLLineEditor::canDelete() const
{
	return !mReadOnly &&
...
void LLLineEditor::draw()
{
	...
	if (gFocusMgr.getKeyboardFocus() == this && hasSelection())
	{
		...
		if (select_left > mScrollHPos)
		{
			...
			rendered_text = mFontVB.render(mFontGL, mText, mScrollHPos,
										   rendered_pixels_right, text_bottom,
										   text_color, LLFontGL::LEFT,
										   LLFontGL::BOTTOM, LLFontGL::NORMAL,
										   select_left - mScrollHPos,
										   mMaxHPixels -
										   ll_round(rendered_pixels_right),
			...
			rendered_text < text_len)
		{
			...
			// Selected middle
			S32 width = mFontGL->getWidth(mText.getWString().c_str(),
			...
			rendered_text += mFontGL->render(mText, mScrollHPos + rendered_text,
											 rendered_pixels_right, text_bottom,
			...
			rendered_text < text_len)
		{
			...
			mFontGL->render(mText, mScrollHPos + rendered_text,
	...
	{
		mFontVB.render(mFontGL, mText, mScrollHPos, rendered_pixels_right,
					   text_bottom, text_color, LLFontGL::LEFT,
	...
	if (gFocusMgr.getKeyboardFocus() == this)
	{
		...
		if (gShowTextEditCursor && !mReadOnly)
		{
			...
			if (elapsed < CURSOR_FLASH_DELAY || (S32(elapsed * 2) & 1))
			{
				...
				if (ins_mode)
				{
					...
					S32 wswidth = mFontGL->getWidth(space.c_str());
					S32 width = mFontGL->getWidth(mText.getWString().c_str(),
												  mCursorPos, 1) + 1;
				...
				if (ins_mode)
				{
					mFontGL->render(mText, mCursorPos,
									(F32)(cursor_left +
		...
		{
			mFontVB.render(mFontGL, mLabel.getWString(), 0, mMinHPixels,
...
S32 LLLineEditor::findPixelNearestPos(S32 cursor_offset) const
{
	...
	if (mDrawAsterixes)
	{
		...
		width = mFontGL->getWidth(asterix.c_str(), mScrollHPos, dpos);
	}
	else
	{
		width = mFontGL->getWidth(mText.getWString().c_str(), mScrollHPos,
								  dpos);
	}
...
//virtual
const std::string& LLLineEditor::getTag() const
{
...
LLXMLNodePtr LLLineEditor::getXML(bool save_children) const
{
	...
	node->createChild("font",
					  true)->setStringValue(LLFontGL::nameFromFont(mFontGL));
...
void LLLineEditor::resetPreedit()
{
	...
	if (preedit_pos >= 0 && preedit_pos < size && len > 0 && end <= size)
	{
	...
	{
		llwarns << "Index out of bounds. Start: " << preedit_pos << " - End:"
				<< end << " - Length: " << size << llendl;
	}
	...
	// Do not reset keystroke timer nor invoke keystroke callback, because a
	// call to updatePreedit should be follow soon in normal course of
...
{
	return ll_roundp(mFontGL->getLineHeight() * LLUI::sGLScaleFactor.mV[VY]);
}
	...
	mCommitCallbackUserData(NULL)
{
	...
	mClearSearchButton->setToolTip(LLTrans::getString("TooltipClearSearch"));
...
//virtual
const std::string& LLSearchEditor::getTag() const
{
...
LLXMLNodePtr LLSearchEditor::getXML(bool save_children) const
{
	...
	nodep->setName(LL_SEARCH_EDITOR_TAG);
	return nodep;
}

//static
LLView* LLSearchEditor::fromXML(LLXMLNodePtr nodep, LLView* parentp,
								LLUICtrlFactory*)
{
	...
	S32 max_text_length = 128;
	nodep->getAttributeS32("max_length", max_text_length);

	std::string text = nodep->getValue().substr(0, max_text_length - 1);

	LLSearchEditor* self = new LLSearchEditor(name, rect, max_text_length);

	std::string label;
	if (nodep->getAttributeString("label", label))
	{
