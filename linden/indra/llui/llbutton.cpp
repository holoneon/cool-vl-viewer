...
void LLButton::init(void (*click_callback)(void*), void* callback_data,
					const LLFontGL* fontp, const char* control_name)
{
	mFontGL = fontp ? fontp : LLFontGL::getFontSansSerif();
	mLastLabelState = 0;

	// *HACK: to make sure there is space for at least one character
	if (getRect().getWidth() - mRightHPad - mLeftHPad < mFontGL->getWidth(" "))
	{
		// Use old defaults
...
//virtual
bool LLButton::setLabelArg(const std::string& key, const std::string& text)
{
...
void LLButton::setLabelUnselected(const std::string& label)
{
	mUnselectedLabel = label;
	mFontVB.reset();
}

void LLButton::setLabelSelected(const std::string& label)
{
...
void LLButton::setDisabledLabel(const std::string& label)
{
	mDisabledLabel = label;
	mFontVB.reset();
}

void LLButton::setDisabledSelectedLabel(const std::string& label)
{
...
void LLButton::draw()
{
	...
	if (mLastLabelState != label_state)
	{
		mLastLabelState = label_state;
		mFontVB.reset();
	}
	...
	if (!label.empty())
	{
		...
		mFontVB.render(mFontGL, label, 0, (F32)x,
					   (F32)(gButtonVPad + y_offset), label_color,
					   mHAlign, LLFontGL::BOTTOM,
					   mDropShadowedText ? LLFontGL::DROP_SHADOW_SOFT
										 : LLFontGL::NORMAL,
					   S32_MAX, text_width, NULL, false);
	}
...
//virtual
const std::string& LLButton::getTag() const
{
