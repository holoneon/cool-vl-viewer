 ...
 * 
 * Copyright (c) 2009 LordGregGreg Back, 2012-2025 Henri Beauchamp 
 * 
...
std::string LLSpellCheck::getDicFullPath(const std::string& file)
{
	...
	std::string path = gDirUtil.getFullPath(LL_PATH_USER_SETTINGS,
											"dictionaries", file);
	if (!LLFile::exists(path))
	{
		path = gDirUtil.getFullPath(LL_PATH_APP_SETTINGS, "dictionaries",
									file);
	}
...
strings_set_t LLSpellCheck::getBaseDicts()
{
	strings_set_t names;	
	std::string name;
	std::string path = gDirUtil.getFullPath(LL_PATH_USER_SETTINGS,
											"dictionaries", "");
	{
		...
		{
			LLStringUtil::truncate(name, name.size() - 4);
...
strings_set_t LLSpellCheck::getExtraDicts()
{
	strings_set_t names;	
	std::string name;
	std::string root;
											...
											"dictionaries", "");
	{
		...
		{
			root = name;
			LLStringUtil::truncate(root, root.size() - 4);
...
void LLSpellCheck::setDictionary(const std::string& dict_name)
{
	...
	LLStringUtil::truncate(dicdicpath, dicdicpath.size() - 4);
	dicdicpath += ".dic";
	...
	std::string fname = gDirUtil.getFullPath(LL_PATH_USER_SETTINGS,
											 "dictionaries", "custom.dic");
	if (!LLFile::exists(fname))
	{
		llinfos << "Creating custom.dic..." << llendl;
		LLFile::mkdir(gDirUtil.getFullPath(LL_PATH_USER_SETTINGS,
										   "dictionaries"));
	...
	strings_set_t to_install = getExtraDicts();
	for (strings_set_t::iterator it = to_install.begin(), end = to_install.end();
		 it != end; ++it)
	{
		addDictionary(*it);
	}
...
void LLSpellCheck::addToCustomDictionary(const std::string& word)
{
	...
	{
		mHunspell->add(utf8_to_iso8859(word));
	}
	else
#endif
	{
...
void LLSpellCheck::addToIgnoreList(const std::string& word)
{
	if (word.size() > 2)
	{
		...
		LL_DEBUGS("SpellCheck") << "Added \"" << lc_word
...
void LLSpellCheck::addWordsToIgnoreList(const std::string& words)
{
	...
	{
		while (i < len && LLStringUtil::isPartOfLexicalWord(words[i]))
		{
			if (words[i] != '\'' ||
				(!word.empty() && i + 1 < len &&
				 words[i + 1] != '\'' &&
...
bool LLSpellCheck::checkSpelling(const std::string& word)
{
	...
	if (mDictEncoding != "utf-8")
	{
		return mHunspell->spell(utf8_to_iso8859(word));
	}
#endif
	return mHunspell->spell(word);
}
								 ...
								 std::vector<std::string>& suggestions)
{
	...
	if (mDictEncoding != "utf-8")
	{
		for (size_t i = 0, count = suggestions.size(); i < count; ++i)
		{
			suggestions[i] = iso8859_to_utf8(suggestions[i]);
		}
	}
#endif
	return (S32)suggestions.size();
}
