...
LLKeyboardLinux::LLKeyboardLinux()
{
	...
	// Build inverse map
	for (std::map<U32, KEY>::iterator it = mTranslateKeyMap.begin(),
									  end = mTranslateKeyMap.end();
		 it != end; ++it)
	{
		mInvTranslateKeyMap[it->second] = it->first;
	}
	...
	for (std::map<U32, KEY>::iterator it = mTranslateNumpadMap.begin(),
									  end = mTranslateNumpadMap.end();
		 it != end; ++it)
	{
		mInvTranslateNumpadMap[it->second] = it->first;
	}
}
...
//virtual
void LLKeyboardLinux::resetMaskKeys()
{
...
MASK LLKeyboardLinux::updateModifiers(U32 mask)
{
	// translate the mask
...
//virtual
bool LLKeyboardLinux::handleKeyDown(U32 key, U32 mask)
{
...
//virtual
bool LLKeyboardLinux::handleKeyUp(U32 key, U32 mask)
{
	U32 translated_code = adjustNativekeyFromUnhandledMask(key, mask);
	U32	translated_mask = updateModifiers(mask);
...
//virtual
MASK LLKeyboardLinux::currentMask(bool for_mouse_event)
{
...
//virtual
void LLKeyboardLinux::scanKeyboard()
{
...
bool LLKeyboardLinux::translateNumpadKey(U32 os_key, KEY* translated_key,
										 MASK mask)
{
...
U32 LLKeyboardLinux::inverseTranslateNumpadKey(KEY translated_key)
{
	if (mNumpadDistinct == ND_NUMLOCK_ON)
	{
