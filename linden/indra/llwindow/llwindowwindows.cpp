...
//static
bool LLWindowWindows::sIsClassRegistered = false;

bool LLWindowWindows::sLanguageTextInputAllowed = true;
bool LLWindowWindows::sWinIMEOpened = false;
HKL LLWindowWindows::sWinInputLocale = 0;
DWORD LLWindowWindows::sWinIMEConversionMode = IME_CMODE_NATIVE;
DWORD LLWindowWindows::sWinIMESentenceMode = IME_SMODE_AUTOMATIC;
LLCoordWindow LLWindowWindows::sWinIMEWindowPosition(-1, -1);

LLWindowWindows::LLWindowWindows(const std::string& title, S32 x, S32 y,
	...
	mRawLParam(0)
{
	...
	gKeyboardp = new LLKeyboardWindows();
	...
	// LLAppViewerWindows::sendURLToOtherInstance()
...
LLWindowWindows::~LLWindowWindows()
{
	delete[] mWindowTitle;
	mWindowTitle = NULL;
...
void LLWindowWindows::setWindowTitle(const std::string& title)
{
...
void LLWindowWindows::hide()
{
	setMouseClipping(false);
	ShowWindow(mWindowHandle, SW_HIDE);
}

void LLWindowWindows::minimize()
{
	setMouseClipping(false);
	showCursor();
...
bool LLWindowWindows::getPosition(LLCoordScreen* position)
{
	RECT window_rect;
...
bool LLWindowWindows::getSize(LLCoordScreen* size)
{
	RECT window_rect;
...
bool LLWindowWindows::getSize(LLCoordWindow* size)
{
	RECT client_rect;
...
bool LLWindowWindows::setPosition(const LLCoordScreen position)
{
	if (mWindowHandle)
	{
		LLCoordScreen size;
...
bool LLWindowWindows::setSize(const LLCoordScreen size)
{
	if (mWindowHandle)
	{
...
bool LLWindowWindows::switchContext(bool fullscreen, const LLCoordScreen& size,
									bool disable_vsync,
...
//virtual
void* LLWindowWindows::createSharedContext()
{
	S32 attribs[] =
	{
		// Start at 4.6
...
//virtual
void LLWindowWindows::makeContextCurrent(void* context)
{
	if (!mhDC)
	{
...
//virtual
void LLWindowWindows::destroySharedContext(void* context)
{
...
void LLWindowWindows::moveWindow(const LLCoordScreen& position,
...
bool LLWindowWindows::setCursorPosition(const LLCoordWindow& position)
{
...
bool LLWindowWindows::getCursorPosition(LLCoordWindow* position)
{
	POINT cursor_point;
...
void LLWindowWindows::hideCursor()
{
...
void LLWindowWindows::showCursor()
{
...
void LLWindowWindows::showCursorFromMouseMove()
{
...
void LLWindowWindows::hideCursorUntilMouseMove()
{
...
HCURSOR LLWindowWindows::loadColorCursor(LPCTSTR name)
{
...
void LLWindowWindows::initCursors()
{
...
//virtual
void LLWindowWindows::setCursor(ECursorType cursor)
{
	if (mCursorFrozen)
	{
		return;
...
void LLWindowWindows::captureMouse()
{
	SetCapture(mWindowHandle);
}

void LLWindowWindows::releaseMouse()
{
	ReleaseCapture();
}

void LLWindowWindows::delayInputProcessing()
{
	mInputProcessingPaused = true;
}

void LLWindowWindows::gatherInput()
{
	MSG msg;
	int msg_count = 0;
...
LRESULT CALLBACK LLWindowWindows::mainWindowProc(HWND h_wnd, UINT u_msg,
												 WPARAM w_param,
												 LPARAM l_param)
{
	LLWindowWindows* window_imp =
		(LLWindowWindows*)GetWindowLongPtr(h_wnd, GWLP_USERDATA);
	if (window_imp)
	{
		...
		{
			// Generally means that OS is going to shut down or user is going
			// to log off.
			llinfos << "WM_QUERYENDSESSION: wParam=" << w_param << "; lParam="
					<< l_param << llendl;
			// 1 = ok to end session. 0 no longer works by itself, use
		...
		case WM_ENDSESSION:
		{
			// OS session is shutting down, initiate cleanup. Comes after
			// WM_QUERYENDSESSION.
			llinfos << "WM_ENDSESSION: wParam=" << w_param << "; lParam="
					<< l_param << llendl;
			U32 end_session_flags = (U32)w_param;
			...
			constexpr U32 CLOSE_FLAGS = ENDSESSION_CLOSEAPP |
										ENDSESSION_CRITICAL |
										ENDSESSION_LOGOFF;
			if (end_session_flags & CLOSE_FLAGS)
			{
				if (callbacksp->handleSessionExit(window_imp))
				{
					// Get the app to initiate cleanup.
					callbacksp->handleQuit(window_imp);
			...
			// Do not need to post quit or destroy window, if session is ending
			// OS is going to take care of it.
			return 0;
		}
		...
		case WM_CHAR:
		{
			...
			// Windows API document, but WM_UNICHAR did not work as specified
...
bool LLWindowWindows::convertCoords(LLCoordGL from, LLCoordWindow* to)
{
	S32 client_height;
	RECT client_rect;
...
bool LLWindowWindows::convertCoords(LLCoordWindow from, LLCoordGL* to)
{
	S32 client_height;
	RECT client_rect;
...
bool LLWindowWindows::convertCoords(LLCoordScreen from, LLCoordWindow* to)
{
	if (!mWindowHandle || !to)
	{
		return false;
...
bool LLWindowWindows::convertCoords(LLCoordWindow from, LLCoordScreen* to)
{
	if (!mWindowHandle || !to)
	{
		return false;
...
bool LLWindowWindows::convertCoords(LLCoordScreen from, LLCoordGL* to)
{
...
bool LLWindowWindows::convertCoords(LLCoordGL from, LLCoordScreen* to)
{
	LLCoordWindow window_coord;
	return convertCoords(from, &window_coord) &&
		convertCoords(window_coord, to);
...
bool LLWindowWindows::isClipboardTextAvailable()
{
	return IsClipboardFormatAvailable(CF_UNICODETEXT);
}

// Funky UTF-16 to wide character conversion routines, needed for some
// characters (such as emojis) when pasting from the clipboard (for these,
// ll_convert_wide_to_wstring() does not work properly)...
static S32 wchart_to_llwchar(const wchar_t* inchars, llwchar* outchar)
{
...
static LLWString convert_wide_to_wstring(const wchar_t* in)
{
	LLWString wout;
	if (!in) return wout;
...
bool LLWindowWindows::pasteTextFromClipboard(LLWString& dst)
{
	bool success = false;
	...
	if (IsClipboardFormatAvailable(CF_UNICODETEXT))
	{
		if (OpenClipboard(mWindowHandle))
		{
			...
			if (h_data)
			{
				wchar_t* utf16str = (wchar_t*)GlobalLock(h_data);
				if (utf16str)
				{
					dst = convert_wide_to_wstring(utf16str);
...
bool LLWindowWindows::copyTextToClipboard(const LLWString& wstr)
{
	bool success = false;
...
//virtual
bool LLWindowWindows::isPrimaryTextAvailable()
{
	return !mPrimaryClipboard.empty();
}

//virtual
bool LLWindowWindows::pasteTextFromPrimary(LLWString& text)
{
...
// virtual
bool LLWindowWindows::copyTextToPrimary(const LLWString& text)
{
...
void LLWindowWindows::setMouseClipping(bool b)
{
...
bool LLWindowWindows::getClientRectInScreenSpace(RECT* rectp)
{
	bool success = false;
...
void LLWindowWindows::flashIcon(F32 seconds)
{
	FLASHWINFO flash_info;
...
bool LLWindowWindows::restoreGamma()
{
	if (mCustomGammaSet)
	{
		mCustomGammaSet = false;
...
bool LLWindowWindows::setGamma(F32 gamma)
{
...
LLWindow::LLWindowResolution* LLWindowWindows::getSupportedResolutions(S32& num_res)
{
...
F32 LLWindowWindows::getNativeAspectRatio()
{
...
F32 LLWindowWindows::getPixelAspectRatio()
{
...
bool LLWindowWindows::setDisplayResolution(S32 width, S32 height, S32 refresh)
{
	DEVMODE dev_mode;
	::ZeroMemory(&dev_mode, sizeof(DEVMODE));
	...
	if (!success)
	{
		llwarns << "Failure to set resolution " << width << "x" << height
				<< " @ " << refresh << "Hz" << llendl;
	}
...
bool LLWindowWindows::setFullscreenResolution()
{
	if (mFullscreen)
	{
...
bool LLWindowWindows::resetDisplayResolution()
{
...
void LLWindowWindows::swapBuffers()
{
	if (mhDC)
	{
...
// LLSplashScreenImp
//
LLSplashScreenWindows::LLSplashScreenWindows()
:	mWindow(NULL)
{
}

void LLSplashScreenWindows::showImpl()
{
	// This appears to work ???
	HINSTANCE hinst = GetModuleHandle(NULL);
	mWindow = CreateDialog(hinst, TEXT("SPLASHSCREEN"), NULL,	// No parent
						   (DLGPROC)LLSplashScreenWindows::windowProc);
	ShowWindow(mWindow, SW_SHOW);
}

void LLSplashScreenWindows::updateImpl(const std::string& mesg)
{
	if (mWindow)
	{
		WCHAR w_mesg[1024];
...
void LLSplashScreenWindows::hideImpl()
{
	if (mWindow)
	{
...
//static
LRESULT CALLBACK LLSplashScreenWindows::windowProc(HWND h_wnd, UINT u_msg,
												   WPARAM w_param,
												   LPARAM l_param)
{
...
S32 OSMessageBoxWindows(const std::string& text, const std::string& caption,
						U32 type)
{
	UINT uType;
	switch (type)
...
void LLWindowWindows::spawnWebBrowser(const std::string& escaped_url, bool async)
{
	bool found = false;
...
LLSD LLWindowWindows::getNativeKeyData()
{
...
void* LLWindowWindows::getPlatformWindow()
{
	return (void*)mWindowHandle;
}

void LLWindowWindows::bringToFront()
{
	BringWindowToTop(mWindowHandle);
}

// set (OS) window focus back to the client
void LLWindowWindows::focusClient()
{
	SetFocus(mWindowHandle);
}

void LLWindowWindows::allowLanguageTextInput(LLPreeditor* preeditor, bool b)
{
...
void LLWindowWindows::fillCandidateForm(const LLCoordGL& caret,
										const LLRect& bounds,
										CANDIDATEFORM* form)
{
...
void LLWindowWindows::setLanguageTextInput(const LLCoordGL& position)
{
	if (sLanguageTextInputAllowed)
	{
		HIMC himc = ImmGetContext(mWindowHandle);
...
void LLWindowWindows::fillCharPosition(const LLCoordGL& caret,
									   const LLRect& bounds,
...
void LLWindowWindows::fillCompositionLogfont(LOGFONT* logfont)
{
	...
	logfont->lfHeight = mPreeditor ? mPreeditor->getPreeditFontSize() : 10;
	logfont->lfWeight = FW_NORMAL;
}

U32 LLWindowWindows::fillReconvertString(const LLWString &text,
...
void LLWindowWindows::updateLanguageTextInputArea()
{
	if (!mPreeditor)
	{
		return;
	...
	if (mPreeditor->getPreeditLocation(-1, &caret_coord, &preedit_bounds, NULL))
	{
		...
		HIMC himc = ImmGetContext(mWindowHandle);
		// Windows document says there may be up to 4 candidate windows.
...
void LLWindowWindows::interruptLanguageTextInput()
{
	if (mPreeditor)
	{
		HIMC himc = ImmGetContext(mWindowHandle);
		...
		// Windows document says there will be no composition string after
...
void LLWindowWindows::handleStartCompositionMessage()
{
...
void LLWindowWindows::handleCompositionMessage(U32 indexes)
{
	if (!mPreeditor)
	{
		return;
...
bool LLWindowWindows::handleImeRequests(WPARAM request, LPARAM param,
										LRESULT* result)
{
	if (!mPreeditor)
	{
...
//static
void LLWindowWindows::setDPIAwareness()
{
...
F32 LLWindowWindows::getSystemUISize()
{
	F32 scale_value = 1.f;
	if (!gHiDPISupport)
	{
...
//static
std::vector<std::string> LLWindowWindows::getDynamicFallbackFontList()
{
