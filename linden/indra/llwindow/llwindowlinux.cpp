...
// Here because llwindowlinux.h includes Xlib.h
#include <X11/Xatom.h>
#include <X11/Xutil.h>
// Here because llwindowlinux.h includes SDL.h
...
S32 exec_cmd(const std::string& cmd, const std::string& arg)
{
	...
	if (pid == 0)
	{
		...
		close(1);
		close(2);
		// End ourselves by running the command
...
//static
void LLWindowLinux::initXlibThreads()
{
...
LLWindowLinux::LLWindowLinux(const std::string& title, S32 x, S32 y, U32 width,
...
:	LLWindow(fullscreen, flags),
	...
	mCaptured(false),
	mGrabbyKeyFlags(0),
	mFSAASamples(fsaa_samples & (U32_MAX - 1)),
	...
	mWindowTitle(title.empty() ? "SL viewer" : title)
{
	...
	gKeyboardp = new LLKeyboardLinux();

	// This should have already been called in LLSplashScreenLinux, but better
	// safe than sorry...
	initXlibThreads();
...
//virtual
void LLWindowLinux::setWindowTitle(const std::string& title)
{
...
//virtual
bool LLWindowLinux::getFullScreenSize(S32& width, S32& height)
{
...
void LLWindowLinux::setWindowIcon()
{
...
bool LLWindowLinux::createContext(S32 x, S32 y, S32 width, S32 height,
								  S32 bits, bool fullscreen,
								  bool disable_vsync)
{
	...
	if (mFullscreen)
	{
		...
		if (SDL_SetWindowDisplayMode(mWindow, &closest_mode) == 0)
		{
			...
			llinfos << "Running at " << mFullscreenWidth << "x"
					<< mFullscreenHeight << " @ " << mFullscreenRefresh << "Hz"
					<< llendl;
		}
		else
		{
	...
	if (vram_mb > 0 && !LLGLManager::sVRAM)
	{
		LLGLManager::sVRAM = vram_mb;
	}
	...
	{
		close();
		setupFailure("The viewer requires True Color to run.\n"
					 "Please, setup the X11 display for 32 bits color.");
		return false;
	}
...
// modes.
//virtual
bool LLWindowLinux::switchContext(bool fullscreen, const LLCoordScreen& size,
								  bool disable_vsync,
...
//virtual
void* LLWindowLinux::createSharedContext()
{
	LLSharedOpenGLContext* ctxp = new LLSharedOpenGLContext;
	ctxp->mContext = SDL_GL_CreateContext(mWindow);
	if (ctxp->mContext)
	{
...
//virtual
void LLWindowLinux::makeContextCurrent(void* ctxp)
{
	if (ctxp)
	{
		SDL_GL_MakeCurrent(mWindow, ((LLSharedOpenGLContext*)ctxp)->mContext);
	}
	else
	{
...
//virtual
void LLWindowLinux::destroySharedContext(void* ctxp)
{
	if (ctxp)	// Ignore attempts to destroy invalid contexts. HB
	{
		LLSharedOpenGLContext* scp = (LLSharedOpenGLContext*)ctxp;
		SDL_GL_DeleteContext(scp->mContext);
		delete scp;
	}
}

//virtual
void LLWindowLinux::destroyContext()
{
	SDL_StopTextInput();
...
//virtual
bool LLWindowLinux::getPosition(LLCoordScreen* position)
{
	if (position && mWindow)
	{
...
//virtual
bool LLWindowLinux::getSize(LLCoordScreen* size)
{
	if (mWindow && size)
	{
...
//virtual
bool LLWindowLinux::getSize(LLCoordWindow* size)
{
	if (mWindow && size)
	{
		SDL_GetWindowSize(mWindow, &size->mX, &size->mY);
...
//virtual
bool LLWindowLinux::setSize(const LLCoordScreen size)
{
	if (!mWindow)
	{
		return false;
...
//virtual
bool LLWindowLinux::restoreGamma()
{
	if (!mCustomGammaSet)
	{
		return true;
...
//virtual
bool LLWindowLinux::setGamma(F32 gamma)
{
...
//virtual
void LLWindowLinux::setMouseClipping(bool b)
{
#if 0
...
//virtual
bool LLWindowLinux::setCursorPosition(const LLCoordWindow& position)
{
...
//virtual
bool LLWindowLinux::getCursorPosition(LLCoordWindow* position)
{
	// Point cursor_point
	LLCoordScreen screen_pos;
...
//virtual
F32 LLWindowLinux::getNativeAspectRatio()
{
...
//virtual
F32 LLWindowLinux::getPixelAspectRatio()
{
...
//virtual
void LLWindowLinux::beforeDialog()
{
...
//virtual
void LLWindowLinux::afterDialog()
{
	llinfos << "called." << llendl;
...
void LLWindowLinux::x11_set_urgent(bool urgent)
{
...
//virtual
void LLWindowLinux::flashIcon(F32 seconds)
{
...
///////////////////////////////////////////////////////////////////////////////
// Clipboards (primary & secondary) implementation (c)2015-2025 Henri Beauchamp
///////////////////////////////////////////////////////////////////////////////
...
void LLWindowLinux::initialiseX11Clipboard()
{
	if (mSDL_Display)
	{
		...
		// Text type atom
		XA_TEXT = XInternAtom(mSDL_Display, "TEXT", False);
		...
		// Our own SL_PASTE_BUFFER atom
		PVT_PASTE_BUFFER = XInternAtom(mSDL_Display, "SL_PASTE_BUFFER", False);

		// Allow SDL_SYSWMEVENT events to get passed via SDL_PollEvent()
		SDL_EventState(SDL_SYSWMEVENT, SDL_ENABLE);
	}
}

// Processes clipboard-related events, searching for clipboard requests from
// the X server. Called from gatherInput() on receiving SDL_SYSWMEVENT events.
void LLWindowLinux::processX11ClipboardEvent(SDL_Event& evt)
{
	...
	if (xevent.type == SelectionRequest)
	{
		...
		bool is_primary = req.selection == XA_PRIMARY;
		bool is_clipboard = req.selection == XA_CLIPBOARD;
		std::string target;
		LL_DEBUGS("Window") << "Got event type SelectionRequest. Selection type: "
							<< (is_primary ? "Primary" : (is_clipboard ? "Clipboard"
													   : "Unknown !"));
		// Note: some badly coded applications do not use standard X11 internal
		// Atoms. Let's retrieve the target Atom name to log it.
		if (req.target == None)
		{
			target = "None";
		}
		...
		{
			char* atom_name = XGetAtomName(mSDL_Display, req.target);
			if (atom_name)
			{
				target.assign(atom_name);
				XFree(atom_name);
			}
		...
		LL_CONT << " - Request target: " << target  << LL_ENDL;
		...
		{
			Atom possible_targets[] =
			{
				XA_STRING,
				XA_UTF8_STRING,
				XA_TEXT,
			};
			XChangeProperty(mSDL_Display, req.requestor, req.property,
							XA_ATOM, 32, PropModeReplace,
			                (unsigned char*)possible_targets, 3);
		}
		// They want a string in a format/encoding we can indeed provide.
		// Note: we do not convert our UTF-8 text into 8 bits text that
		// XA_STRING or XA_TEXT might be expecting, meaning all characters
		// code above 127 (i.e. non-ASCII characaters) will not display
		// properly... But knowadays, almost everything uses UTF-8, so...
				 ...
				 req.target == XA_TEXT)
		{
			...
			if (is_primary)
			{
				utf8 = wstring_to_utf8str(getPrimaryText());
			}
			else if (is_clipboard)
			{
				utf8 = wstring_to_utf8str(getSecondaryText());
			}
			XChangeProperty(mSDL_Display, req.requestor, req.property,
							req.target, 8, PropModeReplace,
							(unsigned char*)utf8.c_str(), utf8.length());
		}
		// We cannot provide them with what they want: reply with None property
		else if (req.target != None)
		{
			LL_DEBUGS("Window") << "Unhandled selection target: " << target
								<< LL_ENDL;
			reply.property = None;
		}
...
bool LLWindowLinux::convertX11Selection(Atom selection, Atom target)
{
	XDeleteProperty(mSDL_Display, mSDL_XWindowID, PVT_PASTE_BUFFER);
	XFlush(mSDL_Display);

	XConvertSelection(mSDL_Display, selection, target, PVT_PASTE_BUFFER,
	...
	while (!response && SDL_GetTicks() - start < maxticks)
	{
		...
		if (event.type == SDL_SYSWMEVENT)
		{
			...
			if (xevent.type == SelectionNotify &&
				xevent.xselection.requestor == mSDL_XWindowID)
			{
				response = true;
			}
		}
...
bool LLWindowLinux::getSelectionText(Atom selection, LLWString& text)
{
	...
	if (owner == mSDL_XWindowID)
	{
		// We own the selection, so we just use our internal buffer, instead
		// of going through a X11 request (to ourselves !) to recover it...
...
bool LLWindowLinux::setSelectionText(Atom selection, const LLWString& text)
{
	...
	// Set ourselves as the owner of the selection atom
	XSetSelectionOwner(mSDL_Display, selection, mSDL_XWindowID, CurrentTime);

	// Check that we succeeded to acquire ownership.
	Window owner = XGetSelectionOwner(mSDL_Display, selection);
	return owner == mSDL_XWindowID;
}

//virtual
bool LLWindowLinux::isClipboardTextAvailable()
{
	return mSDL_Display &&
...
//virtual
bool LLWindowLinux::pasteTextFromClipboard(LLWString& text)
{
	return getSelectionText(XA_CLIPBOARD, text);
}

//virtual
bool LLWindowLinux::copyTextToClipboard(const LLWString& text)
{
	return setSelectionText(XA_CLIPBOARD, text);
}

//virtual
bool LLWindowLinux::isPrimaryTextAvailable()
{
	if (mSDL_Display)
	{
		// When the primary selection is owned, there is some available text.
		if (XGetSelectionOwner(mSDL_Display, XA_PRIMARY) != None)
		{
			return true;
		}
		// The primary selection may also be owned by "None", so we need to try
		// and retrieve the buffer text to see if there is any...
		LLWString text;
...
//virtual
bool LLWindowLinux::pasteTextFromPrimary(LLWString& text)
{
	return getSelectionText(XA_PRIMARY, text);
}

//virtual
bool LLWindowLinux::copyTextToPrimary(const LLWString& text)
{
	return setSelectionText(XA_PRIMARY, text);
}

#else	// HB_USE_NATIVE_X11_CLIPBOARD
...
bool LLWindowLinux::pasteTextFromClipboard(LLWString& text)
{
	if (isClipboardTextAvailable())
	{
		...
		if (utf8_text)
		{
			text = utf8str_to_wstring(utf8_text);
			SDL_free(utf8_text);
			return true;
		}
	}
...
bool LLWindowLinux::copyTextToClipboard(const LLWString& text)
{
	const std::string utf8_text = wstring_to_utf8str(text);
	return !SDL_SetClipboardText(utf8_text.c_str());
}
...
bool LLWindowLinux::pasteTextFromPrimary(LLWString& text)
{
	...
	{
		char* utf8_text = SDL_GetPrimarySelectionText();
		if (utf8_text)
		{
			text = utf8str_to_wstring(utf8_text);
			SDL_free(utf8_text);
...
{
	const std::string utf8_text = wstring_to_utf8str(text);
	return !SDL_SetPrimarySelectionText(utf8_text.c_str());
}

#endif	// HB_USE_NATIVE_X11_CLIPBOARD

///////////////////////////////////////////////////////////////////////////////

//virtual
LLWindow::LLWindowResolution* LLWindowLinux::getSupportedResolutions(S32& num_res)
{
	if (!mSupportedResolutions)
	{
	...
	num_res = mNumSupportedResolutions;
	return mSupportedResolutions;
}

//virtual
bool LLWindowLinux::convertCoords(LLCoordGL from, LLCoordWindow* to)
{
	if (to && mWindow)
	{
		to->mX = from.mX;
...
//virtual
bool LLWindowLinux::convertCoords(LLCoordWindow from, LLCoordGL* to)
{
	if (to && mWindow)
	{
		to->mX = from.mX;
...
//virtual
bool LLWindowLinux::convertCoords(LLCoordScreen from, LLCoordWindow* to)
{
	if (to)
	{
...
//virtual
bool LLWindowLinux::convertCoords(LLCoordWindow from, LLCoordScreen* to)
{
	if (to)
	{
		// In the fullscreen case, window and screen coordinates are the same.
...
//virtual
bool LLWindowLinux::convertCoords(LLCoordScreen from, LLCoordGL* to)
{
...
//virtual
bool LLWindowLinux::convertCoords(LLCoordGL from, LLCoordScreen* to)
{
	LLCoordWindow wcoord;
	return convertCoords(from, &wcoord) && convertCoords(wcoord, to);
}

void LLWindowLinux::setupFailure(const std::string& text)
{
	destroyContext();
	OSMessageBox(text);
}

bool LLWindowLinux::SDLReallyCaptureInput(bool capture)
{
	mCaptured = capture;
...
U32 LLWindowLinux::SDLCheckGrabbyKeys(U32 keysym, bool gain)
{
...
//virtual
void LLWindowLinux::gatherInput()
{
	...
	while (SDL_PollEvent(&event))
	{
		switch (event.type)
		{
			...
			case SDL_SYSWMEVENT:
			{
				if (mSDL_Display)
				{
					processX11ClipboardEvent(event);
				}
				break;
			}
			...
			{
				// We do not use/need this event, so just ignore it and do not
				// report it as "unhandled" below. HB
				LL_DEBUGS("Window") << "Got SDL_CLIPBOARDUPDATE (ignored)."
									<< LL_ENDL;
				break;
			}
...
//virtual
void LLWindowLinux::setCursor(ECursorType cursor)
{
	if (mCursorFrozen)
	{
		return;
...
void LLWindowLinux::initCursors()
{
...
void LLWindowLinux::quitCursors()
{
	if (mWindow)
	{
		for (S32 i = 0; i < UI_CURSOR_COUNT; ++i)
...
//virtual
void LLWindowLinux::captureMouse()
{
...
//virtual
void LLWindowLinux::releaseMouse()
{
...
//virtual
void LLWindowLinux::showCursorFromMouseMove()
{
...
//virtual
void LLWindowLinux::hideCursorUntilMouseMove()
{
	if (!mHideCursorPermanent)
	{
		hideCursor();
...
//virtual
LLSD LLWindowLinux::getNativeKeyData()
{
...
//virtual
void LLWindowLinux::spawnWebBrowser(const std::string& escaped_url, bool async)
{
	bool found = false;
...
//virtual
void* LLWindowLinux::getPlatformWindow()
{
	return NULL;
}
...
//virtual
void LLWindowLinux::bringToFront()
{
	if (mWindow)
	{
...
//static
std::vector<std::string> LLWindowLinux::getDynamicFallbackFontList()
{
	...
	if (fs)
	{
		...
		for (int i = 0; i < fs->nfont; ++i)
		{
				...
				&& filename)
			{
				...
				if (lc_name.rfind(".otb") == len - 4 ||
					lc_name.rfind(".pcf") == len - 4 ||
					lc_name.rfind(".pcf.gz") == len - 7 ||
					lc_name.rfind(".bdf") == len - 4 ||
					lc_name.rfind(".bdf.gz") == len - 7)
				{
...
void LLSplashScreenLinux::showImpl()
{
	if (mImpl)
	{
		mImpl->show();
...
void LLSplashScreenLinux::updateImpl(const std::string& msg)
{
	if (mImpl)
	{
		mImpl->update(msg);
...
void LLSplashScreenLinux::hideImpl()
{
	if (mImpl)
	{
		mImpl->hide();
