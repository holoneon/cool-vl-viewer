...
class LLWindowWindows final : public LLWindow
{
	friend class LLWindow;

protected:
	LOG_CLASS(LLWindowWindows);
...
protected:
	LLWindowWindows(const std::string& title, S32 x, S32 y, U32 width,
					U32 height, U32 flags, bool fullscreen, bool disable_vsync,
					U32 fsaa_samples);
	~LLWindowWindows() override;
...
protected:	// Platform specific variables
	...
	// They are all static, since one context is shared by all LLWindowWindows
	// instances.
...
class LLSplashScreenWindows final : public LLSplashScreen
{
public:
	LLSplashScreenWindows();
