...
{
	if (hasError(mDstDb,
				 sqlite3_exec(mDstDb, "BEGIN TRANSACTION;", NULL, NULL, NULL)))
	{
		return false;
	}
	...
	while ((result = sqlite3_step(read_stmt)) == SQLITE_ROW)
	{
		...
		sqlite3_stmt* replace_stmt = NULL;
		if (hasError(mDstDb,
	...
	if (hasError(mDstDb, sqlite3_exec(mDstDb, "COMMIT;", NULL, NULL, NULL)))
	{
		return false;
	}
	return true;
...
bool HBCookiesMerger::merge()
{
	...
	strings_set_t tables = getTables();
