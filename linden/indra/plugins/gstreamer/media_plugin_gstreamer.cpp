...
constexpr double MIN_LOOP_SEC = 1.0;
...
static void extract_metadata(const GstTagList* listp, const gchar* tag,
							 gpointer user_datap)
{
	if (!user_datap) return;

	LLStreamMetadata* outp = (LLStreamMetadata*)user_datap;
	gchar* str = NULL;
	if (llgst_tag_list_get_string(listp, GST_TAG_ARTIST, &str))
	{
		outp->mArtist.assign(str);
		llg_free(str);
	}
	if (llgst_tag_list_get_string(listp, GST_TAG_TITLE, &str))
	{
		outp->mTitle.assign(str);
		llg_free(str);
	}
}

LL_INLINE static void llgst_caps_unref(GstCaps* capsp)
{
	llgst_mini_object_unref(GST_MINI_OBJECT_CAST(capsp));
}

LL_INLINE static void llgst_sample_unref(GstSample* samplep)
{
	llgst_mini_object_unref(GST_MINI_OBJECT_CAST(samplep));
}
...
public:
	...
	static void postDebugMessage(const std::string& msg, bool warn = true);
...
private:
	...
	static void postError(GError* errp, bool warn = true);
...
private:
	...
	static MediaPluginGStreamer*	sInstance;
	static bool						sDoneInit;
	static bool						sEnableMediaPluginDebugging;
};

//static
MediaPluginGStreamer* MediaPluginGStreamer::sInstance = NULL;
bool MediaPluginGStreamer::sDoneInit = false;
bool MediaPluginGStreamer::sEnableMediaPluginDebugging = false;

static void log_function(GstDebugCategory*, GstDebugLevel, const gchar* file,
						 const gchar* function, gint line, GObject*,
						 GstDebugMessage* message, gpointer)
{
	std::stringstream msg;
	msg << file << ": " << line << "(" << function << "): "
		<< llgst_debug_message_get(message);
	// Note: log_function() threshold is set to GST_LEVEL_WARNING, so all
	// messages are of warning severity or worse. HB
	MediaPluginGStreamer::postDebugMessage(msg.str());
}

MediaPluginGStreamer::MediaPluginGStreamer(LLPluginInstance::sendMessageFunction fn,
										   void* host_user_data)
:	MediaPluginBase(fn, host_user_data),
	mBusWatchID(0),
	mSeekWanted(false),
	mIsLooping(false),
...
//static
void MediaPluginGStreamer::postDebugMessage(const std::string& msg, bool warn)
{
	// Gstreamer tends to be quite verbose, so limit logging to plugin
	// debugging. Note that these log messages used to go to std::cerr in
	// the old plugin code (so they were not seen unless you ran the viewer
	// from a terminal/console), but are now redirected to the viewer log file
	// by the code below. HB
	if (sEnableMediaPluginDebugging && sInstance)
	{
		std::string log_msg = "GStreamer plugin (pid ";
# if LL_WINDOWS
...
# endif
		...
		sInstance->sendMessage(debug_message);
	}
}

//static
void MediaPluginGStreamer::postError(GError* errp, bool warn)
{
	std::string error;
	if (errp)
	{
		if (errp->code)
		{
			if (errp->message)
			{
				error.assign(errp->message);
			}
			if (error.empty())
			{
				error = llformat("Error code: %d", errp->code);
			}
		}
		llg_error_free(errp);
	}
	if (!error.empty())
	{
		postDebugMessage(error, warn);
	}
}

gboolean MediaPluginGStreamer::processGSTEvents(GstBus* busp, GstMessage* msgp)
{
	if (!msgp)
	{
	...
	switch (GST_MESSAGE_TYPE(msgp))
	{
		case GST_MESSAGE_BUFFERING:
		{
			...
			{
				gint percent = 0;
				llgst_message_parse_buffering(msgp, &percent);
			}
			break;
		}
		...
		case GST_MESSAGE_STATE_CHANGED:
		{
			...
			llgst_message_parse_state_changed(msgp, &old_state, &new_state,
											  &pending_state);
			switch (new_state)
			{
		...
		{
			if (llgst_message_parse_tag)
			{
				LLStreamMetadata mdata;
				GstTagList* tagsp = NULL;
				llgst_message_parse_tag(msgp, &tagsp);
				llgst_tag_list_foreach(tagsp, extract_metadata, &mdata);
				llgst_tag_list_unref(tagsp);
		...
		{
			if (llgst_message_parse_error)
			{
				GError* errp = NULL;
				gchar* debugp = NULL;
				llgst_message_parse_error(msgp, &errp, &debugp);
				postError(errp);	// Frees errp
				llg_free(debugp);
		...
		case GST_MESSAGE_WARNING:
		{
			if (llgst_message_parse_info)
			{
				GError* errp = NULL;
				gchar* debugp = NULL;
				llgst_message_parse_info(msgp, &errp, &debugp);
				postError(errp);	// Frees errp
				llg_free(debugp);
			}
			break;
		...
		case GST_MESSAGE_INFO:
		{
			if (llgst_message_parse_info)
			{
				...
				gchar* debugp = NULL;
				llgst_message_parse_info(msgp, &errp, &debugp);
				postError(errp, false);	// Frees errp
				llg_free(debugp);
			}
			break;
		}
...
extern "C"
{
gboolean llmediaimplgstreamer_bus_callback(GstBus* busp, GstMessage* msgp,
										   gpointer datap)
{
	MediaPluginGStreamer* implp = (MediaPluginGStreamer*)datap;
	return implp && implp->processGSTEvents(busp, msgp);
}
} // extern "C"
...
bool MediaPluginGStreamer::update(int milliseconds)
{
	...
	if (!mAppSink || mTextureWidth <= 0 || mTextureHeight <= 0)
	{
		return true;
	}
	...
	gint width, height;
	GstStructure* structp = llgst_caps_get_structure(capsp, 0);
	llgst_structure_get_int(structp, "width", &width);
	llgst_structure_get_int(structp, "height", &height);
	...
	// very low.
#if 0
	F32 x_scale = INTERNAL_TEXTURE_SIZE / mTextureWidth;
	F32 y_scale = INTERNAL_TEXTURE_SIZE / mTextureHeight;
	U8* dest = mPixels + (mTextureHeight - 1) * mTextureWidth * mDepth;
	for (int row = 0; row < mTextureHeight; ++row)
	{
		int source_row = y_scale * (F32)row;
		if (source_row >= height) continue;

		const U8* texel_in = map.data + source_row * width * 3;
		U8* texel_out = dest - row * mTextureWidth * mDepth;
		for (int col = 0; col < mTextureWidth; ++col)
		{
			int source_col = x_scale * (F32)col;
			if (source_col >= width) continue;

			source_col *= 3;
			texel_out[0] = texel_in[source_col++];
			texel_out[1] = texel_in[source_col++];
			texel_out[2] = texel_in[source_col];
			texel_out += mDepth;
		}
	}
#else
...
bool MediaPluginGStreamer::seek(double time_sec)
{
	...
	if (!success)
	{
		postDebugMessage(llformat("Failed to seek at time: %f", time_sec));
	}
	return success;
}
...
bool MediaPluginGStreamer::getTimePos(double& sec_out)
{
	...
	if (!got_position)
	{
		postDebugMessage("Failed to get time position");
	}
...
bool MediaPluginGStreamer::load()
{
	...
	{
		setStatus(STATUS_ERROR);
		postDebugMessage("Failed to get the pump");
		return false; // error
	}
	...
	{
		setStatus(STATUS_ERROR);
		postDebugMessage("Failed to instantiate a playbin");
		return false; // error
	}
	...
	{
		setStatus(STATUS_ERROR);
		postDebugMessage("Failed to get the bus");
		return false; // error
	}
	...
	{
		setStatus(STATUS_ERROR);
		postDebugMessage("Failed to get the sink");
		return false;
	}

	llg_object_set(mPlaybin, "video-sink", mAppSink, NULL);
	postDebugMessage("Got the media sink alright", false);
...
bool MediaPluginGStreamer::startup()
{
	...
	{
		postDebugMessage("Failed to find Gstreamer library. Aborted");
		return false;
	}
	...
	llgst_debug_add_log_function(log_function, NULL, NULL);
	...
	{
		postError(err);
		postDebugMessage("Failed to start Gstreamer. Aborted");
		return false;
	}
	...
	if (message_in.parse(message_string) >= 0)
	{
		...
		if (message_class == LLPLUGIN_MESSAGE_CLASS_BASE)
		{
			...
			else if (message_name == "shm_added")
			{
				...
				mSharedSegments.insert(shared_mem_map_t::value_type(name,
																	info));
			}
		...
		else if (message_class == LLPLUGIN_MESSAGE_CLASS_MEDIA)
		{
			...
			else if (message_name == "size_change")
			{
				...
				if (!name.empty())
				{
					...
					shared_mem_map_t::iterator it = mSharedSegments.find(name);
			...
			{
				sEnableMediaPluginDebugging =
					message_in.getValueBoolean("enable");
				llgst_debug_set_active(sEnableMediaPluginDebugging);
			}
		}
