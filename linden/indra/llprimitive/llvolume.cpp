...
// "error: 'get_temporary_buffer<...>' is deprecated" seen with clang 18. HB
					 ...
					 F32 twist_scale)
{
	...
	pt->mPos.set(lerp(0.f, params.getShear().mV[VX], s) +
				 lerp(-skew, skew, t) * 0.5f,
				 c + lerp(0.f, params.getShear().mV[VY], s), s);
	pt->mScale.set(hole_x * lerp(taper_x_begin, taper_x_end, t),
				   hole_y * lerp(taper_y_begin, taper_y_end, t), 0.f, 1.f);
	pt->mTexT = t;
	...
	switch (params.getCurveType() & 0xf0)
	{
		...
		case LL_PCODE_PATH_TEST:
		{
			...
			for (S32 i = 0; i < np; ++i)
			{
				...
				mPath[i].mPos.set(0.f,
								  ...
								  lerp(-0.5f, cosf(twist_angle) * 0.5f, t));
				mPath[i].mScale.set(lerp(1.f, params.getScale().mV[VX], t),
									lerp(1.f, params.getScale().mV[VY], t),
									0.f, 1.f);
				mPath[i].mTexT = t;
		...
		default:
		{
			...
			for (S32 i = 0; i < np; ++i)
			{
				...
				LLMatrix3 tmp(quat);
				mPath[i].mRot.loadu(tmp);
				mPath[i].mScale.set(lerp(start_scale.mV[VX],
										 end_scale.mV[VX], t),
									lerp(start_scale.mV[VY],
										 end_scale.mV[VY], t),
									0.f, 1.f);
				mPath[i].mTexT = t;
			}
										  ...
										  S32 face_mask)
{
	...
	// Scratch array for per-face silhouette edge information.
	std::vector<S32> edge;
		 ...
		 iter != mVolumeFaces.end(); ++iter)
	{
		...
		if (!(face_mask & (0x1 << cur_index++)) || !face.mNumIndices)
		{
			continue;
		}
		...
		S32 num_faces = face.mNumIndices / 3;
			...
			for (S32 j = 0; j < num_faces; ++j)
			{
				...
				for (S32 k = 0; k < 3; ++k)
				{
					...
					if (index != -1 && f_facing[index] == (AWAY | TOWARDS))
					{
						...
						f_facing[edge[j * 3 + k]] = f_facing[j];
						continue;
					}

					// index == -1 ==> no neighbor, MUST be a silhouette edge
					if (index == -1 || !(f_facing[index] & f_facing[j]))
					{
...
void LLVolumeFace::remap()
{
	...
	// Documentation for meshopt_generateVertexRemapMulti claims that remap
	// should use vertice count, but all examples use indice count. There are
	// out of bounds crashes when using vertice count. To be on the safe side
	// use bigger of the two.
	std::vector<U32> remap(llmax(mNumIndices, mNumVertices), 0);
	...
	size_t pos_bytes = sizeof(LLVector4a) * 2 * vert_count + tc_bytes;
	LLVector4a* remap_pos = (LLVector4a*)allocate_volume_mem_64(pos_bytes);
	if (!remap_pos)
	{
	...
	// Zero all bytes to avoid non-zero padding bytes when vertex structure
	// has gaps.
	// See: https://github.com/zeux/meshoptimizer/blob/master/README.md in
	// the "Indexing" chapter. HB
	memset((void*)remap_pos, 0, pos_bytes);
...
public:
		...
		error(false)
	{
		...
		{
			constexpr size_t ELEM_SIZE = sizeof(LLVector2) +
										 2 * sizeof(LLVector3) +
										 sizeof(LLVector4);
			size_t bytes = ELEM_SIZE * count;
			if (has_weights)
			{
				bytes += sizeof(LLVector4) * count;
			}
	...
	LL_INLINE U32 GetNumFaces() const
	{
		return error ? 0 : face->mNumIndices / 3;
	}

	LL_INLINE U32 GetNumVerticesOfFace(U32) const
	{
		return 3;
	}

	LL_INLINE mikk::float3 GetPosition(U32 face_num, U32 vert_num) const
	{
		const F32* v = error ? LLVector3::zero.mV
							 : p[face_num * 3 + vert_num].mV;
		return mikk::float3(v);
	}

	LL_INLINE mikk::float3 GetTexCoord(U32 face_num, U32 vert_num) const
	{
		const F32* uv = error ? LLVector2::zero.mV
							  : tc[face_num * 3 + vert_num].mV;
		return mikk::float3(uv[0], uv[1], 1.f);
	}

	LL_INLINE mikk::float3 GetNormal(U32 face_num, U32 vert_num) const
	{
		const F32* normal = error ? LLVector3::zero.mV
								  : n[face_num * 3 + vert_num].mV;
		return mikk::float3(normal);
	}

	LL_INLINE void SetTangentSpace(U32 face_num, U32 vert_num, mikk::float3 f,
								   bool orientation)
	{
		if (!error)
		{
			U32 i = face_num * 3 + vert_num;
			t[i].set(f.x, f.y, f.z, orientation ? 1.f : -1.f);
		}
	}
...
bool LLVolumeFace::cacheOptimize(bool gen_tangents)
{
	...
	if (gen_tangents && mNormals && mTexCoords)
	{
		...
		catch (const std::bad_alloc&)
		{
			...
			llwarns << "Out of memory trying to generate tangent space"
					<< llendl;
			return false;
		}
		catch (...)
		{
			llwarns << "Mikktspace::genTangSpace() failed" << llendl;
			return false;
		}
		...
		if (!data.p.empty())
		{
			...
			{
				vert_count =
					meshopt_generateVertexRemapMulti(&remap[0], NULL,
													 data.p.size(),
													 data.p.size(), mos,
													 stream_count);
			}
			...
			{
				LLMemory::allocationFailed();
				llwarns << "Out of memory trying to generate vertices"
						<< llendl;
				return false;
			}
		}

		if (vert_count > 0 && vert_count < 65535)
		{
			bool success= resizeVertices(vert_count);
			...
			for (S32 i = 0; i < mNumIndices; ++i)
			{
				...
				if (dst_idx == U32_MAX)
				{
					...
					llwarns_once << "U32_MAX destination index, substituting"
								 << llendl;
				}
				else if ((S32)dst_idx > mNumVertices)
				{
					dst_idx = mNumVertices - 1;
					llwarns_once << "Too large a destination index: using last index"
								 << llendl;
				}
				if (src_idx >= (U32)data.p.size())
				{
					...
					llwarns_once << "Invalid source index, substituting"
								 << llendl;
				}
				mIndices[i] = dst_idx;
		...
		else
		{
			if (vert_count <= 0)
			{
				llwarns_once << "Failed to process a model." << llendl;
			}
								...
								const LLVector4a& size0)
{
	...
	mOctree = new LLVolumeOctree(center0, size0);
...
void LLVolumeFace::generateSilhouetteEdge(const LLVolume* volp,
										  std::vector<S32>& edge) const
{
	edge.clear();
	...
	if (mTypeMask & CAP_MASK)
	{
		...
		const LLAlignedArray<LLVector4a, 64>& profile =
			volp->getProfile().mVertices;
		S32 grid_size = (profile.size() - 1) / 4;
		edge.resize(6 * grid_size * grid_size);
		S32 cur_edge = 0;
	...
	constexpr U32 END_OR_SIDE_MASK = END_MASK | SIDE_MASK;
	if (!(mTypeMask & END_OR_SIDE_MASK))
	{
		llwarns_once << "Unknown/uninitialized face type: " << mTypeMask
					 << llendl;
		llassert(false);
		return;
	}

	edge.resize(6 * (mNumS - 1) * (mNumT - 1));
	S32 cur_edge = 0;
	bool flat_face = (mTypeMask & FLAT_MASK) != 0;
	bool open_profile = volp->getProfile().isOpen();
	bool open_path = volp->getPath().isOpen();
	for (S32 t = 0; t < mNumT - 1; ++t)
	{
		for (S32 s = 0; s < mNumS - 1; ++s)
		{
			...
			else if (flat_face || open_profile)
			{
				// No neighbor
				edge[cur_edge++] = -1;
			}
...
bool LLVolumeFace::createUnCutCubeCap(LLVolume* volp, bool partial_build)
{
	const LLAlignedArray<LLVector4a, 64>& mesh = volp->getMesh();
	const LLAlignedArray<LLVector4a, 64>& profile = volp->getProfile().mVertices;
	S32 max_s = volp->getProfile().getTotal();
	S32 max_t = volp->getPath().mPath.size();
...
bool LLVolumeFace::createCap(LLVolume* volp, bool partial_build)
{
	constexpr U32 HOLLOW_OR_OPEN_MASK = HOLLOW_MASK | OPEN_MASK;
	...
	if (!(mTypeMask & HOLLOW_OR_OPEN_MASK) &&
		...
		volp->getParams().getProfileParams().getCurveType() ==
			LL_PCODE_PROFILE_SQUARE)
	{
		return createUnCutCubeCap(volp, partial_build);
	}
							 ...
							 LLVector4a* tangentp)
{
	...
	for (U32 a = 0; a < triangle_count; ++a)
	{
		...
		const LLVector4a& v1 = vertp[i1];
		const LLVector4a& v2 = vertp[i2];
		const LLVector4a& v3 = vertp[i3];

		const LLVector2& w1 = tcoordp[i1];
		const LLVector2& w2 = tcoordp[i2];
		const LLVector2& w3 = tcoordp[i3];
	...
	for (U32 a = 0; a < vertex_count; ++a)
	{
		...
		{
			// Degenerate, make up a value
			tangentp[a].set(0.f, 0.f, 1.f, 1.f);
		}
	}
...
void LLVolumeFace::createTangents()
{
	if (!mTangents)
	{
		...
		if (!calculate_tangent_array(mNumVertices, mPositions, mNormals,
...
bool LLVolumeFace::resizeVertices(S32 num_verts)
{
	...
	if (mNumVertices)
	{
		...
		// See: https://github.com/zeux/meshoptimizer/blob/master/README.md in
		// the "Indexing" chapter. HB
		memset((void*)mPositions, 0, bytes);
	...
	// Force an update
	mJointRiggingInfoTab.clear(true);
							  ...
							  const LLVector2& tc, S32 max_indice)
{
	...
	if (new_verts > mNumAllocatedVertices)
	{
		...
		// See: https://github.com/zeux/meshoptimizer/blob/master/README.md in
		// the "Indexing" chapter. HB
		memset((void*)mPositions, 0, new_size);
...
bool LLVolumeFace::createSide(LLVolume* volp, bool partial_build)
{
	bool flat = (mTypeMask & FLAT_MASK) != 0;

	U8 sculpt_type = volp->getParams().getSculptType();
	U8 sculpt_stitching = sculpt_type & LL_SCULPT_TYPE_MASK;
	bool sculpt_invert = (sculpt_type & LL_SCULPT_FLAG_INVERT) != 0;
	bool sculpt_mirror = (sculpt_type & LL_SCULPT_FLAG_MIRROR) != 0;
	...
	if (sculpt_stitching == LL_SCULPT_TYPE_NONE)
	{
		...
		if (!s_bottom_converges && !volp->getProfile().isOpen())
		{
			// Wrap normals on S
			LLVector4a n;
		...
		if (volp->getPathType() == LL_PCODE_PATH_CIRCLE &&
			(volp->getProfileType() &
...
void LLJointRiggingInfoTab::clear(bool set_needs_update)
{
	if (mRigInfoPtr)
	{
		delete[] mRigInfoPtr;
...
void LLJointRiggingInfoTab::resize(U32 size)
{
	...
	LLJointRiggingInfo* new_infop = new LLJointRiggingInfo[size];
	if (mSize)
	{
		...
		for (U32 i = 0; i < min_size; ++i)
		{
			...
			LLVector4a* new_extents = new_infop[i].getRiggedExtents();
			new_extents[0] = old_extents[0];
			new_extents[1] = old_extents[1];
		}
		delete[] mRigInfoPtr;
	}
	mRigInfoPtr = new_infop;
	mSize = size;
}
...
void LLJointRiggingInfoTab::merge(const LLJointRiggingInfoTab& src)
{
	...
	U32 min_size = llmin(mSize, src.mSize);
	for (U32 i = 0; i < min_size; ++i)
	{
