...
public:
	...
	LL_INLINE LLJointRiggingInfoTab()
	:	mRigInfoPtr(NULL),
		mSize(0),
		mNeedsUpdate(false)
	{
	...
	LL_INLINE ~LLJointRiggingInfoTab()				{ clear(); }

	// No copy.
	LLJointRiggingInfoTab(const LLJointRiggingInfoTab&) = delete;
	LLJointRiggingInfoTab& operator=(const LLJointRiggingInfoTab&) = delete;

	LL_INLINE bool needsUpdate()					{ return mNeedsUpdate; }
	LL_INLINE void setNeedsUpdate(bool b = true)	{ mNeedsUpdate = b; }

	void clear(bool set_needs_update = false);
...
public:
	...
	bool create(LLVolume* volp, bool partial_build = false);
	void createTangents();
	...
	void generateSilhouetteEdge(const LLVolume* volp,
...
private:
    ...
    bool createUnCutCubeCap(LLVolume* volp, bool partial_build = false);
    bool createCap(LLVolume* volp, bool partial_build = false);
    bool createSide(LLVolume* volp, bool partial_build = false);
...
public:
	...
	LL_INLINE void copyVolumeFaces(const LLVolume* volp)
	{
		if (volp)
		{
			mVolumeFaces = volp->mVolumeFaces;
			mSculptLevel = 0;
		}
	}
	...
	LL_INLINE bool isMeshAssetLoaded() const			{ return mIsMeshAssetLoaded; }
