...
// gcc 13 sees array bound issues where there are none... HB
...
void LLPrimitive::packTEMessage(LLMessageSystem* mesgsys) const
{
	...
	S32 last_face_index = llmin((S32)getNumTEs(), MAX_TES) - 1;
								 ...
								 char const* block_name, S32 block_num)
{
	...
	static LLTEContents tec;
	memset((void*)&tec, 0, sizeof(tec));
...
S32 LLPrimitive::unpackTEMessage(LLDataPacker& dp)
{
	...
	S32 size;
	if (!dp.unpackBinaryData(tec.packed_buffer, MAX_TE_BUFFER, size,
							 _PREHASH_TextureEntry))
	{
	...
	bool success =
		LLTEField::unpack<LLUUID>(tec.image_data, face_count, cur_ptr,
								  buffer_end, MVT_LLUUID) &&
		LLTEField::unpack<LLColor4U>(tec.colors, face_count, cur_ptr,
									 buffer_end, MVT_U8) &&
		LLTEField::unpack<F32>(tec.scale_s, face_count, cur_ptr, buffer_end,
							   MVT_F32) &&
		LLTEField::unpack<F32>(tec.scale_t, face_count, cur_ptr, buffer_end,
							   MVT_F32) &&
		LLTEField::unpack<S16>(tec.offset_s, face_count, cur_ptr, buffer_end,
							   MVT_S16) &&
		LLTEField::unpack<S16>(tec.offset_t, face_count, cur_ptr, buffer_end,
							   MVT_S16) &&
		LLTEField::unpack<S16>(tec.image_rot, face_count, cur_ptr, buffer_end,
							   MVT_S16) &&
		LLTEField::unpack<U8>(tec.bump, face_count, cur_ptr, buffer_end,
							  MVT_U8) &&
		LLTEField::unpack<U8>(tec.media_flags, face_count, cur_ptr, buffer_end,
							  MVT_U8) &&
		LLTEField::unpack<U8>(tec.glow, face_count, cur_ptr, buffer_end,
							  MVT_U8);
	if (!success)
	{
		llwarns << "Failure parsing texture entry message due to malformed TE field."
	...
	for (U32 i = 0; i < face_count; ++i)
	{
		retval |= setTETexture(i, tec.image_data[i]);
		retval |= setTEScale(i, tec.scale_s[i], tec.scale_t[i]);
		retval |= setTEOffset(i, (F32)tec.offset_s[i] / (F32)0x7FFF,
							  (F32)tec.offset_t[i] / (F32)0x7FFF);
...
bool LLRenderMaterialParams::pack(LLDataPacker& dp) const
{
	...
	U8 count = (U8)llmin(mEntries.size(), 14);
