											  ...
											  LLSD& log_msg)
{
	...
	for (U32 i = 0; i < index_count; i += idx_stride)
	{
		...
		if (pos_source)
		{
			if (pos_offset < 0 || i + pos_offset >= index_count)
			{
				return LLModel::BAD_ELEMENT;
			}
		...
		if (tc_source)
		{
			if (tc_offset < 0 || i + tc_offset >= index_count)
			{
				return LLModel::BAD_ELEMENT;
			}
		...
		if (norm_source)
		{
			if (norm_offset < 0 || i + norm_offset >= index_count)
			{
				return LLModel::BAD_ELEMENT;
			}
...
:	LLModelLoader(filename, lod, load_cb, joint_lookup_func, texture_load_func,
				  ...
				  joint_alias_map, max_joints_per_mesh, model_limit),
								  ...
								  domMesh* mesh, domSkin* skin)
{
	...
	if (skeleton_count)
	{
		...
		for (U32 i = 0; i < skeleton_count; ++i)
		{
			...
			strings_map_t::const_iterator joint_it = mJointMap.begin();
	...
	if (!missing_skel_or_scene)
	{
		...
		for (strings_map_t::const_iterator it = mJointMap.begin(),
										   end = mJointMap.end();
			 it != end; ++it)
		{
		...
		if (geo)
		{
			...
			if (mesh)
			{
				...
				while (i != mModelsMap[mesh].end())
				{
					...
					mats_map_t materials = getMaterials(model, instance_geo,
														dae);
...
mats_map_t LLDAELoader::getMaterials(LLModel* model,
									 domInstance_geometry* instance_geo,
									 DAE* dae)
{
	mats_map_t materials;
