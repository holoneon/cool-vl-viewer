...
public:
	...
	// Returns the size of vol_face.mJointRiggingInfoTab after update. Note:
	// this used to be a static method in LLSkinningUtil, but it really belongs
	// here !  HB	
	U32 updateRiggingInfo(LLVolumeFace& vol_face) const;
...
public:
	...
	typedef std::vector<LLMatrix4> matrix_list_t;
...
public:
	...
	// hull_decomp_t is a vector of convex hulls; each convex hull is a set of
	// points.
	typedef std::vector<std::vector<LLVector3> > hull_decomp_t;
	typedef std::vector<LLVector3> hull_t;
	...
	LL_INLINE strings_vec_t& getMaterialList()		{ return mMaterialList; }
	...
	void setConvexHullDecomposition(const hull_decomp_t& hull,
	...
	// Note: this used to be a static method in (the now gone) LLSkinningUtil,
	// but it is best kept here... HB
...
typedef std::map<std::string, LLImportMaterial, std::less<> > mats_map_t;

class LLModelInstanceBase
{
public:
	LLModelInstanceBase(LLModel* modelp, const LLMatrix4& transform,
						const mats_map_t& materials)
	:	mModel(modelp),
...
public:
	LLModelInstance(LLModel* modelp, const std::string& label,
					const LLMatrix4& transform, const mats_map_t& materials)
