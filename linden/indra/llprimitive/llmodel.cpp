...
{
	static const LLMatrix4 identity;	// LLMatrix4() initializes to identity

	LLSD ret;

	U32 joint_names_count = mJointNames.size();
	// Inverse bind matrix
	U32 max_count = mInvBindMatrix.size();
	for (U32 i = 0; i < joint_names_count; ++i)
	{
		ret["joint_names"][i] = mJointNames[i];
		if (i >= max_count)
		{
			llwarns << "Joint index " << i << " (" << mJointNames[i]
					<< ") exceeds inverse bind matrix size: " << max_count
					<< llendl;
		}
		// For model to work at all there must be a matching bind matrix, so
		// supply an indentity one when it is not the case.
		const LLMatrix4& ibm = max_count > i ? mInvBindMatrix[i] : identity;
		for (U32 j = 0; j < 4; ++j)
		{
			for (U32 k = 0; k < 4; ++k)
			{
				ret["inverse_bind_matrix"][i][j * 4 + k] = ibm.mMatrix[j][k];
			}
		}
	}
	...
	max_count = mAlternateBindMatrix.size();
	if (include_joints && max_count > 0)
	{
		for (U32 i = 0; i < joint_names_count; ++i)
		{
			if (i >= max_count)
			{
				llwarns << "Joint index " << i << " (" << mJointNames[i]
						<< ") exceeds alternate bind matrix size: "
						<< max_count << llendl;
			}
			// If there is not enough to match mJointNames, either supply no
			// alternate matrixes at all or supply replacements.
			const LLMatrix4& abm = max_count > i ? mAlternateBindMatrix[i]
												 : identity;
			for (U32 j = 0; j < 4; ++j)
			{
				for (U32 k = 0; k < 4; ++k)
...
U32 LLMeshSkinInfo::updateRiggingInfo(LLVolumeFace& vol_face) const
{
	S32 num_verts = vol_face.mNumVertices;
	U32 max_count = mJointKeys.size();
	if (num_verts <= 0 || !max_count || !vol_face.mWeights)
	{
		return 0;
	}

	LLJointRiggingInfoTab& riginfotab = vol_face.mJointRiggingInfoTab;
	U32 size = riginfotab.size();
	if (size || !riginfotab.needsUpdate())
	{
		return size;
	}
	...
	// NOTE: we cannot (any more) parallelize this loop with OpenMP since
	// this method is (now) also called from the mesh repository which is
	...
	for (S32 i = 0; i < num_verts; ++i)
	{
		...
		F32* weightsp = vol_face.mWeights[i].getF32ptr();
		F32 scale = 0.f;
		...
		U32 shape_mat_size = mInvBindShapeMatrix.size();
		for (U32 k = 0; k < 4; ++k)
		{
			...
			S32 i = mJointKeys[joint_index] - 2;
			if (i >= 0 && i < (S32)LL_CHARACTER_MAX_ANIMATED_JOINTS)
			{
				riginfotab[i].setIsRiggedTo();
				if (shape_mat_size > joint_index)
				{
					inv_bind.loadu(mInvBindShapeMatrix[joint_index]);
				}
				else
				{
					inv_bind.setIdentity();
				}
				inv_bind.affineTransform(pos, pos_joint_space);
				pos_joint_space.mul(wght[k]);
				LLVector4a* extentsp = riginfotab[i].getRiggedExtents();
				update_min_max(extentsp[0], extentsp[1], pos_joint_space);
			}
		}
	}

	riginfotab.setNeedsUpdate(false);
	return riginfotab.size();
}
...
void LLModel::normalizeVolumeFacesAndWeights()
{
	...
	// For all of the volume faces in the model, loop over them and see what
	// the extents of the volume along each axis.
	LLVector4a min = mVolumeFaces[0].mExtents[0];
	LLVector4a max = mVolumeFaces[0].mExtents[1];
	for (U32 i = 1, count = mVolumeFaces.size(); i < count; ++i)
	{
		LLVolumeFace& face = mVolumeFaces[i];
		update_min_max(min, max, face.mExtents[0]);
		update_min_max(min, max, face.mExtents[1]);
	...
	for (S32 i = 0, count = mVolumeFaces.size(); i < count; ++i)
	{
		...
		// For all the positions, we scale the positions to fit within the unit
		// cube.
		LLVector4a* pos = (LLVector4a*)face.mPositions;
		LLVector4a* norm = (LLVector4a*)face.mNormals;
		LLVector4a* t = (LLVector4a*)face.mTangents;
	...
	{
		LLVector4a pos(weights.first.mV[VX], weights.first.mV[VY],
					   weights.first.mV[VZ]);
		pos.add(trans);
		pos.mul(scale);
		LLVector3 scaled_pos(pos.getF32ptr());
		mPosition.emplace_back(scaled_pos);
		mSkinWeights[scaled_pos] = weights.second;
	}

	// mNormalizedScale is the scale at which we would need to multiply the
	// model by to get the original size of the model instead of the normalized
	// size.
	LLVector4a normalized_scale;
	normalized_scale.splat(1.f);
	normalized_scale.div(scale);
	...
	// Remember normalized scale so original dimensions can be recovered for
	// mesh processing (i.e. tangent generation).
...
void LLModel::setConvexHullDecomposition(const LLModel::hull_decomp_t& hull,
										 const std::vector<PhysicsMesh>& mesh)
{
	mPhysics.mHull = hull;
	mPhysics.mMesh = mesh;
	updateHullCenters();
								 ...
								 S32& mdl_face_cnt)
{
	...
	std::map<std::string, U32, std::less<> > index_map;

	// Build a map of material slot names to face indexes
	bool reorder = false;
	strings_set_t base_mat;
	strings_set_t cur_mat;
		...
		base_mat == cur_mat)
	{
		...
		strings_vec_t new_material_list;
...
#define HB_SSE2_OPTIMIZED 1
//static
void LLModel::getPerVertexSkinMatrix(const LLVector4a& weights,
									 const LLMatrix4a* matp,
									 LLMatrix4a& final_mat,
									 bool handle_bad_scale)
{
	bool valid_weights = true;

#if HB_SSE2_OPTIMIZED
...
///////////////////////////////////////////////////////////////////////////////
// LLModel::Decomposition sub-class
///////////////////////////////////////////////////////////////////////////////
