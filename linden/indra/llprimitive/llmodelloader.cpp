...
void LLModelLoader::run()
{
	...
	if (!error.empty())
	{
		llwarns << "Exception encountered while loading model: "
				<< mFilename << "- Exception: " << error << llendl;
		LLSD args;
...
bool LLModelLoader::doLoadModel()
{
	if (mTrySLM)
	{
		...
		{
			time_t slm_time = LLFile::lastModified(slm_filename);
			if (slm_time)	// If .slm file exists
			{
				time_t model_time = LLFile::lastModified(mFilename);
...
bool LLModelLoader::loadFromSLM(const std::string& filename)
{
	...
	for (U32 i = 0, count = instance.size(); i < count; ++i)
	{
		...
		for (U32 lod = 0; lod < LLModel::NUM_LODS; ++lod)
		{
			...
			if (idx >= lod_size)
			{
				instances[i].mLOD[lod] = model[lod].front();
				continue;
			}
...
void LLModelLoader::loadTextures()
{
		 ...
		 iter != end; ++iter)
	{
		...
		{
			for (mats_map_t::iterator j = iter->second[i].mMaterial.begin(),
				 ...
				 j != end2; ++j)
			{
				...
				{
					mTextureLoadFunc(material, mUserData);
				}
			}
		}
	}
